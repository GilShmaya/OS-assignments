
kernel/kernel:     file format elf64-littleriscv


Disassembly of section .text:

0000000080000000 <_entry>:
    80000000:	00009117          	auipc	sp,0x9
    80000004:	93013103          	ld	sp,-1744(sp) # 80008930 <_GLOBAL_OFFSET_TABLE_+0x8>
    80000008:	6505                	lui	a0,0x1
    8000000a:	f14025f3          	csrr	a1,mhartid
    8000000e:	0585                	addi	a1,a1,1
    80000010:	02b50533          	mul	a0,a0,a1
    80000014:	912a                	add	sp,sp,a0
    80000016:	078000ef          	jal	ra,8000008e <start>

000000008000001a <spin>:
    8000001a:	a001                	j	8000001a <spin>

000000008000001c <timerinit>:
// which arrive at timervec in kernelvec.S,
// which turns them into software interrupts for
// devintr() in trap.c.
void
timerinit()
{
    8000001c:	1141                	addi	sp,sp,-16
    8000001e:	e422                	sd	s0,8(sp)
    80000020:	0800                	addi	s0,sp,16
// which hart (core) is this?
static inline uint64
r_mhartid()
{
  uint64 x;
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    80000022:	f14027f3          	csrr	a5,mhartid
  // each CPU has a separate source of timer interrupts.
  int id = r_mhartid();
    80000026:	0007869b          	sext.w	a3,a5

  // ask the CLINT for a timer interrupt.
  int interval = 1000000; // cycles; about 1/10th second in qemu.
  *(uint64*)CLINT_MTIMECMP(id) = *(uint64*)CLINT_MTIME + interval;
    8000002a:	0037979b          	slliw	a5,a5,0x3
    8000002e:	02004737          	lui	a4,0x2004
    80000032:	97ba                	add	a5,a5,a4
    80000034:	0200c737          	lui	a4,0x200c
    80000038:	ff873583          	ld	a1,-8(a4) # 200bff8 <_entry-0x7dff4008>
    8000003c:	000f4637          	lui	a2,0xf4
    80000040:	24060613          	addi	a2,a2,576 # f4240 <_entry-0x7ff0bdc0>
    80000044:	95b2                	add	a1,a1,a2
    80000046:	e38c                	sd	a1,0(a5)

  // prepare information in scratch[] for timervec.
  // scratch[0..2] : space for timervec to save registers.
  // scratch[3] : address of CLINT MTIMECMP register.
  // scratch[4] : desired interval (in cycles) between timer interrupts.
  uint64 *scratch = &timer_scratch[id][0];
    80000048:	00269713          	slli	a4,a3,0x2
    8000004c:	9736                	add	a4,a4,a3
    8000004e:	00371693          	slli	a3,a4,0x3
    80000052:	00009717          	auipc	a4,0x9
    80000056:	00e70713          	addi	a4,a4,14 # 80009060 <timer_scratch>
    8000005a:	9736                	add	a4,a4,a3
  scratch[3] = CLINT_MTIMECMP(id);
    8000005c:	ef1c                	sd	a5,24(a4)
  scratch[4] = interval;
    8000005e:	f310                	sd	a2,32(a4)
}

static inline void 
w_mscratch(uint64 x)
{
  asm volatile("csrw mscratch, %0" : : "r" (x));
    80000060:	34071073          	csrw	mscratch,a4
  asm volatile("csrw mtvec, %0" : : "r" (x));
    80000064:	00006797          	auipc	a5,0x6
    80000068:	06c78793          	addi	a5,a5,108 # 800060d0 <timervec>
    8000006c:	30579073          	csrw	mtvec,a5
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000070:	300027f3          	csrr	a5,mstatus

  // set the machine-mode trap handler.
  w_mtvec((uint64)timervec);

  // enable machine-mode interrupts.
  w_mstatus(r_mstatus() | MSTATUS_MIE);
    80000074:	0087e793          	ori	a5,a5,8
  asm volatile("csrw mstatus, %0" : : "r" (x));
    80000078:	30079073          	csrw	mstatus,a5
  asm volatile("csrr %0, mie" : "=r" (x) );
    8000007c:	304027f3          	csrr	a5,mie

  // enable machine-mode timer interrupts.
  w_mie(r_mie() | MIE_MTIE);
    80000080:	0807e793          	ori	a5,a5,128
  asm volatile("csrw mie, %0" : : "r" (x));
    80000084:	30479073          	csrw	mie,a5
}
    80000088:	6422                	ld	s0,8(sp)
    8000008a:	0141                	addi	sp,sp,16
    8000008c:	8082                	ret

000000008000008e <start>:
{
    8000008e:	1141                	addi	sp,sp,-16
    80000090:	e406                	sd	ra,8(sp)
    80000092:	e022                	sd	s0,0(sp)
    80000094:	0800                	addi	s0,sp,16
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000096:	300027f3          	csrr	a5,mstatus
  x &= ~MSTATUS_MPP_MASK;
    8000009a:	7779                	lui	a4,0xffffe
    8000009c:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffd87ff>
    800000a0:	8ff9                	and	a5,a5,a4
  x |= MSTATUS_MPP_S;
    800000a2:	6705                	lui	a4,0x1
    800000a4:	80070713          	addi	a4,a4,-2048 # 800 <_entry-0x7ffff800>
    800000a8:	8fd9                	or	a5,a5,a4
  asm volatile("csrw mstatus, %0" : : "r" (x));
    800000aa:	30079073          	csrw	mstatus,a5
  asm volatile("csrw mepc, %0" : : "r" (x));
    800000ae:	00001797          	auipc	a5,0x1
    800000b2:	de078793          	addi	a5,a5,-544 # 80000e8e <main>
    800000b6:	34179073          	csrw	mepc,a5
  asm volatile("csrw satp, %0" : : "r" (x));
    800000ba:	4781                	li	a5,0
    800000bc:	18079073          	csrw	satp,a5
  asm volatile("csrw medeleg, %0" : : "r" (x));
    800000c0:	67c1                	lui	a5,0x10
    800000c2:	17fd                	addi	a5,a5,-1
    800000c4:	30279073          	csrw	medeleg,a5
  asm volatile("csrw mideleg, %0" : : "r" (x));
    800000c8:	30379073          	csrw	mideleg,a5
  asm volatile("csrr %0, sie" : "=r" (x) );
    800000cc:	104027f3          	csrr	a5,sie
  w_sie(r_sie() | SIE_SEIE | SIE_STIE | SIE_SSIE);
    800000d0:	2227e793          	ori	a5,a5,546
  asm volatile("csrw sie, %0" : : "r" (x));
    800000d4:	10479073          	csrw	sie,a5
  asm volatile("csrw pmpaddr0, %0" : : "r" (x));
    800000d8:	57fd                	li	a5,-1
    800000da:	83a9                	srli	a5,a5,0xa
    800000dc:	3b079073          	csrw	pmpaddr0,a5
  asm volatile("csrw pmpcfg0, %0" : : "r" (x));
    800000e0:	47bd                	li	a5,15
    800000e2:	3a079073          	csrw	pmpcfg0,a5
  timerinit();
    800000e6:	00000097          	auipc	ra,0x0
    800000ea:	f36080e7          	jalr	-202(ra) # 8000001c <timerinit>
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    800000ee:	f14027f3          	csrr	a5,mhartid
  w_tp(id);
    800000f2:	2781                	sext.w	a5,a5
}

static inline void 
w_tp(uint64 x)
{
  asm volatile("mv tp, %0" : : "r" (x));
    800000f4:	823e                	mv	tp,a5
  asm volatile("mret");
    800000f6:	30200073          	mret
}
    800000fa:	60a2                	ld	ra,8(sp)
    800000fc:	6402                	ld	s0,0(sp)
    800000fe:	0141                	addi	sp,sp,16
    80000100:	8082                	ret

0000000080000102 <consolewrite>:
//
// user write()s to the console go here.
//
int
consolewrite(int user_src, uint64 src, int n)
{
    80000102:	715d                	addi	sp,sp,-80
    80000104:	e486                	sd	ra,72(sp)
    80000106:	e0a2                	sd	s0,64(sp)
    80000108:	fc26                	sd	s1,56(sp)
    8000010a:	f84a                	sd	s2,48(sp)
    8000010c:	f44e                	sd	s3,40(sp)
    8000010e:	f052                	sd	s4,32(sp)
    80000110:	ec56                	sd	s5,24(sp)
    80000112:	0880                	addi	s0,sp,80
  int i;

  for(i = 0; i < n; i++){
    80000114:	04c05663          	blez	a2,80000160 <consolewrite+0x5e>
    80000118:	8a2a                	mv	s4,a0
    8000011a:	84ae                	mv	s1,a1
    8000011c:	89b2                	mv	s3,a2
    8000011e:	4901                	li	s2,0
    char c;
    if(either_copyin(&c, user_src, src+i, 1) == -1)
    80000120:	5afd                	li	s5,-1
    80000122:	4685                	li	a3,1
    80000124:	8626                	mv	a2,s1
    80000126:	85d2                	mv	a1,s4
    80000128:	fbf40513          	addi	a0,s0,-65
    8000012c:	00002097          	auipc	ra,0x2
    80000130:	750080e7          	jalr	1872(ra) # 8000287c <either_copyin>
    80000134:	01550c63          	beq	a0,s5,8000014c <consolewrite+0x4a>
      break;
    uartputc(c);
    80000138:	fbf44503          	lbu	a0,-65(s0)
    8000013c:	00000097          	auipc	ra,0x0
    80000140:	78e080e7          	jalr	1934(ra) # 800008ca <uartputc>
  for(i = 0; i < n; i++){
    80000144:	2905                	addiw	s2,s2,1
    80000146:	0485                	addi	s1,s1,1
    80000148:	fd299de3          	bne	s3,s2,80000122 <consolewrite+0x20>
  }

  return i;
}
    8000014c:	854a                	mv	a0,s2
    8000014e:	60a6                	ld	ra,72(sp)
    80000150:	6406                	ld	s0,64(sp)
    80000152:	74e2                	ld	s1,56(sp)
    80000154:	7942                	ld	s2,48(sp)
    80000156:	79a2                	ld	s3,40(sp)
    80000158:	7a02                	ld	s4,32(sp)
    8000015a:	6ae2                	ld	s5,24(sp)
    8000015c:	6161                	addi	sp,sp,80
    8000015e:	8082                	ret
  for(i = 0; i < n; i++){
    80000160:	4901                	li	s2,0
    80000162:	b7ed                	j	8000014c <consolewrite+0x4a>

0000000080000164 <consoleread>:
// user_dist indicates whether dst is a user
// or kernel address.
//
int
consoleread(int user_dst, uint64 dst, int n)
{
    80000164:	7119                	addi	sp,sp,-128
    80000166:	fc86                	sd	ra,120(sp)
    80000168:	f8a2                	sd	s0,112(sp)
    8000016a:	f4a6                	sd	s1,104(sp)
    8000016c:	f0ca                	sd	s2,96(sp)
    8000016e:	ecce                	sd	s3,88(sp)
    80000170:	e8d2                	sd	s4,80(sp)
    80000172:	e4d6                	sd	s5,72(sp)
    80000174:	e0da                	sd	s6,64(sp)
    80000176:	fc5e                	sd	s7,56(sp)
    80000178:	f862                	sd	s8,48(sp)
    8000017a:	f466                	sd	s9,40(sp)
    8000017c:	f06a                	sd	s10,32(sp)
    8000017e:	ec6e                	sd	s11,24(sp)
    80000180:	0100                	addi	s0,sp,128
    80000182:	8b2a                	mv	s6,a0
    80000184:	8aae                	mv	s5,a1
    80000186:	8a32                	mv	s4,a2
  uint target;
  int c;
  char cbuf;

  target = n;
    80000188:	00060b9b          	sext.w	s7,a2
  acquire(&cons.lock);
    8000018c:	00011517          	auipc	a0,0x11
    80000190:	01450513          	addi	a0,a0,20 # 800111a0 <cons>
    80000194:	00001097          	auipc	ra,0x1
    80000198:	a50080e7          	jalr	-1456(ra) # 80000be4 <acquire>
  while(n > 0){
    // wait until interrupt handler has put some
    // input into cons.buffer.
    while(cons.r == cons.w){
    8000019c:	00011497          	auipc	s1,0x11
    800001a0:	00448493          	addi	s1,s1,4 # 800111a0 <cons>
      if(myproc()->killed){
        release(&cons.lock);
        return -1;
      }
      sleep(&cons.r, &cons.lock);
    800001a4:	89a6                	mv	s3,s1
    800001a6:	00011917          	auipc	s2,0x11
    800001aa:	09290913          	addi	s2,s2,146 # 80011238 <cons+0x98>
    }

    c = cons.buf[cons.r++ % INPUT_BUF];

    if(c == C('D')){  // end-of-file
    800001ae:	4c91                	li	s9,4
      break;
    }

    // copy the input byte to the user-space buffer.
    cbuf = c;
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    800001b0:	5d7d                	li	s10,-1
      break;

    dst++;
    --n;

    if(c == '\n'){
    800001b2:	4da9                	li	s11,10
  while(n > 0){
    800001b4:	07405863          	blez	s4,80000224 <consoleread+0xc0>
    while(cons.r == cons.w){
    800001b8:	0984a783          	lw	a5,152(s1)
    800001bc:	09c4a703          	lw	a4,156(s1)
    800001c0:	02f71463          	bne	a4,a5,800001e8 <consoleread+0x84>
      if(myproc()->killed){
    800001c4:	00002097          	auipc	ra,0x2
    800001c8:	81a080e7          	jalr	-2022(ra) # 800019de <myproc>
    800001cc:	551c                	lw	a5,40(a0)
    800001ce:	e7b5                	bnez	a5,8000023a <consoleread+0xd6>
      sleep(&cons.r, &cons.lock);
    800001d0:	85ce                	mv	a1,s3
    800001d2:	854a                	mv	a0,s2
    800001d4:	00002097          	auipc	ra,0x2
    800001d8:	1dc080e7          	jalr	476(ra) # 800023b0 <sleep>
    while(cons.r == cons.w){
    800001dc:	0984a783          	lw	a5,152(s1)
    800001e0:	09c4a703          	lw	a4,156(s1)
    800001e4:	fef700e3          	beq	a4,a5,800001c4 <consoleread+0x60>
    c = cons.buf[cons.r++ % INPUT_BUF];
    800001e8:	0017871b          	addiw	a4,a5,1
    800001ec:	08e4ac23          	sw	a4,152(s1)
    800001f0:	07f7f713          	andi	a4,a5,127
    800001f4:	9726                	add	a4,a4,s1
    800001f6:	01874703          	lbu	a4,24(a4)
    800001fa:	00070c1b          	sext.w	s8,a4
    if(c == C('D')){  // end-of-file
    800001fe:	079c0663          	beq	s8,s9,8000026a <consoleread+0x106>
    cbuf = c;
    80000202:	f8e407a3          	sb	a4,-113(s0)
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    80000206:	4685                	li	a3,1
    80000208:	f8f40613          	addi	a2,s0,-113
    8000020c:	85d6                	mv	a1,s5
    8000020e:	855a                	mv	a0,s6
    80000210:	00002097          	auipc	ra,0x2
    80000214:	616080e7          	jalr	1558(ra) # 80002826 <either_copyout>
    80000218:	01a50663          	beq	a0,s10,80000224 <consoleread+0xc0>
    dst++;
    8000021c:	0a85                	addi	s5,s5,1
    --n;
    8000021e:	3a7d                	addiw	s4,s4,-1
    if(c == '\n'){
    80000220:	f9bc1ae3          	bne	s8,s11,800001b4 <consoleread+0x50>
      // a whole line has arrived, return to
      // the user-level read().
      break;
    }
  }
  release(&cons.lock);
    80000224:	00011517          	auipc	a0,0x11
    80000228:	f7c50513          	addi	a0,a0,-132 # 800111a0 <cons>
    8000022c:	00001097          	auipc	ra,0x1
    80000230:	a6c080e7          	jalr	-1428(ra) # 80000c98 <release>

  return target - n;
    80000234:	414b853b          	subw	a0,s7,s4
    80000238:	a811                	j	8000024c <consoleread+0xe8>
        release(&cons.lock);
    8000023a:	00011517          	auipc	a0,0x11
    8000023e:	f6650513          	addi	a0,a0,-154 # 800111a0 <cons>
    80000242:	00001097          	auipc	ra,0x1
    80000246:	a56080e7          	jalr	-1450(ra) # 80000c98 <release>
        return -1;
    8000024a:	557d                	li	a0,-1
}
    8000024c:	70e6                	ld	ra,120(sp)
    8000024e:	7446                	ld	s0,112(sp)
    80000250:	74a6                	ld	s1,104(sp)
    80000252:	7906                	ld	s2,96(sp)
    80000254:	69e6                	ld	s3,88(sp)
    80000256:	6a46                	ld	s4,80(sp)
    80000258:	6aa6                	ld	s5,72(sp)
    8000025a:	6b06                	ld	s6,64(sp)
    8000025c:	7be2                	ld	s7,56(sp)
    8000025e:	7c42                	ld	s8,48(sp)
    80000260:	7ca2                	ld	s9,40(sp)
    80000262:	7d02                	ld	s10,32(sp)
    80000264:	6de2                	ld	s11,24(sp)
    80000266:	6109                	addi	sp,sp,128
    80000268:	8082                	ret
      if(n < target){
    8000026a:	000a071b          	sext.w	a4,s4
    8000026e:	fb777be3          	bgeu	a4,s7,80000224 <consoleread+0xc0>
        cons.r--;
    80000272:	00011717          	auipc	a4,0x11
    80000276:	fcf72323          	sw	a5,-58(a4) # 80011238 <cons+0x98>
    8000027a:	b76d                	j	80000224 <consoleread+0xc0>

000000008000027c <consputc>:
{
    8000027c:	1141                	addi	sp,sp,-16
    8000027e:	e406                	sd	ra,8(sp)
    80000280:	e022                	sd	s0,0(sp)
    80000282:	0800                	addi	s0,sp,16
  if(c == BACKSPACE){
    80000284:	10000793          	li	a5,256
    80000288:	00f50a63          	beq	a0,a5,8000029c <consputc+0x20>
    uartputc_sync(c);
    8000028c:	00000097          	auipc	ra,0x0
    80000290:	564080e7          	jalr	1380(ra) # 800007f0 <uartputc_sync>
}
    80000294:	60a2                	ld	ra,8(sp)
    80000296:	6402                	ld	s0,0(sp)
    80000298:	0141                	addi	sp,sp,16
    8000029a:	8082                	ret
    uartputc_sync('\b'); uartputc_sync(' '); uartputc_sync('\b');
    8000029c:	4521                	li	a0,8
    8000029e:	00000097          	auipc	ra,0x0
    800002a2:	552080e7          	jalr	1362(ra) # 800007f0 <uartputc_sync>
    800002a6:	02000513          	li	a0,32
    800002aa:	00000097          	auipc	ra,0x0
    800002ae:	546080e7          	jalr	1350(ra) # 800007f0 <uartputc_sync>
    800002b2:	4521                	li	a0,8
    800002b4:	00000097          	auipc	ra,0x0
    800002b8:	53c080e7          	jalr	1340(ra) # 800007f0 <uartputc_sync>
    800002bc:	bfe1                	j	80000294 <consputc+0x18>

00000000800002be <consoleintr>:
// do erase/kill processing, append to cons.buf,
// wake up consoleread() if a whole line has arrived.
//
void
consoleintr(int c)
{
    800002be:	1101                	addi	sp,sp,-32
    800002c0:	ec06                	sd	ra,24(sp)
    800002c2:	e822                	sd	s0,16(sp)
    800002c4:	e426                	sd	s1,8(sp)
    800002c6:	e04a                	sd	s2,0(sp)
    800002c8:	1000                	addi	s0,sp,32
    800002ca:	84aa                	mv	s1,a0
  acquire(&cons.lock);
    800002cc:	00011517          	auipc	a0,0x11
    800002d0:	ed450513          	addi	a0,a0,-300 # 800111a0 <cons>
    800002d4:	00001097          	auipc	ra,0x1
    800002d8:	910080e7          	jalr	-1776(ra) # 80000be4 <acquire>

  switch(c){
    800002dc:	47d5                	li	a5,21
    800002de:	0af48663          	beq	s1,a5,8000038a <consoleintr+0xcc>
    800002e2:	0297ca63          	blt	a5,s1,80000316 <consoleintr+0x58>
    800002e6:	47a1                	li	a5,8
    800002e8:	0ef48763          	beq	s1,a5,800003d6 <consoleintr+0x118>
    800002ec:	47c1                	li	a5,16
    800002ee:	10f49a63          	bne	s1,a5,80000402 <consoleintr+0x144>
  case C('P'):  // Print process list.
    procdump();
    800002f2:	00002097          	auipc	ra,0x2
    800002f6:	5e0080e7          	jalr	1504(ra) # 800028d2 <procdump>
      }
    }
    break;
  }
  
  release(&cons.lock);
    800002fa:	00011517          	auipc	a0,0x11
    800002fe:	ea650513          	addi	a0,a0,-346 # 800111a0 <cons>
    80000302:	00001097          	auipc	ra,0x1
    80000306:	996080e7          	jalr	-1642(ra) # 80000c98 <release>
}
    8000030a:	60e2                	ld	ra,24(sp)
    8000030c:	6442                	ld	s0,16(sp)
    8000030e:	64a2                	ld	s1,8(sp)
    80000310:	6902                	ld	s2,0(sp)
    80000312:	6105                	addi	sp,sp,32
    80000314:	8082                	ret
  switch(c){
    80000316:	07f00793          	li	a5,127
    8000031a:	0af48e63          	beq	s1,a5,800003d6 <consoleintr+0x118>
    if(c != 0 && cons.e-cons.r < INPUT_BUF){
    8000031e:	00011717          	auipc	a4,0x11
    80000322:	e8270713          	addi	a4,a4,-382 # 800111a0 <cons>
    80000326:	0a072783          	lw	a5,160(a4)
    8000032a:	09872703          	lw	a4,152(a4)
    8000032e:	9f99                	subw	a5,a5,a4
    80000330:	07f00713          	li	a4,127
    80000334:	fcf763e3          	bltu	a4,a5,800002fa <consoleintr+0x3c>
      c = (c == '\r') ? '\n' : c;
    80000338:	47b5                	li	a5,13
    8000033a:	0cf48763          	beq	s1,a5,80000408 <consoleintr+0x14a>
      consputc(c);
    8000033e:	8526                	mv	a0,s1
    80000340:	00000097          	auipc	ra,0x0
    80000344:	f3c080e7          	jalr	-196(ra) # 8000027c <consputc>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000348:	00011797          	auipc	a5,0x11
    8000034c:	e5878793          	addi	a5,a5,-424 # 800111a0 <cons>
    80000350:	0a07a703          	lw	a4,160(a5)
    80000354:	0017069b          	addiw	a3,a4,1
    80000358:	0006861b          	sext.w	a2,a3
    8000035c:	0ad7a023          	sw	a3,160(a5)
    80000360:	07f77713          	andi	a4,a4,127
    80000364:	97ba                	add	a5,a5,a4
    80000366:	00978c23          	sb	s1,24(a5)
      if(c == '\n' || c == C('D') || cons.e == cons.r+INPUT_BUF){
    8000036a:	47a9                	li	a5,10
    8000036c:	0cf48563          	beq	s1,a5,80000436 <consoleintr+0x178>
    80000370:	4791                	li	a5,4
    80000372:	0cf48263          	beq	s1,a5,80000436 <consoleintr+0x178>
    80000376:	00011797          	auipc	a5,0x11
    8000037a:	ec27a783          	lw	a5,-318(a5) # 80011238 <cons+0x98>
    8000037e:	0807879b          	addiw	a5,a5,128
    80000382:	f6f61ce3          	bne	a2,a5,800002fa <consoleintr+0x3c>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000386:	863e                	mv	a2,a5
    80000388:	a07d                	j	80000436 <consoleintr+0x178>
    while(cons.e != cons.w &&
    8000038a:	00011717          	auipc	a4,0x11
    8000038e:	e1670713          	addi	a4,a4,-490 # 800111a0 <cons>
    80000392:	0a072783          	lw	a5,160(a4)
    80000396:	09c72703          	lw	a4,156(a4)
          cons.buf[(cons.e-1) % INPUT_BUF] != '\n'){
    8000039a:	00011497          	auipc	s1,0x11
    8000039e:	e0648493          	addi	s1,s1,-506 # 800111a0 <cons>
    while(cons.e != cons.w &&
    800003a2:	4929                	li	s2,10
    800003a4:	f4f70be3          	beq	a4,a5,800002fa <consoleintr+0x3c>
          cons.buf[(cons.e-1) % INPUT_BUF] != '\n'){
    800003a8:	37fd                	addiw	a5,a5,-1
    800003aa:	07f7f713          	andi	a4,a5,127
    800003ae:	9726                	add	a4,a4,s1
    while(cons.e != cons.w &&
    800003b0:	01874703          	lbu	a4,24(a4)
    800003b4:	f52703e3          	beq	a4,s2,800002fa <consoleintr+0x3c>
      cons.e--;
    800003b8:	0af4a023          	sw	a5,160(s1)
      consputc(BACKSPACE);
    800003bc:	10000513          	li	a0,256
    800003c0:	00000097          	auipc	ra,0x0
    800003c4:	ebc080e7          	jalr	-324(ra) # 8000027c <consputc>
    while(cons.e != cons.w &&
    800003c8:	0a04a783          	lw	a5,160(s1)
    800003cc:	09c4a703          	lw	a4,156(s1)
    800003d0:	fcf71ce3          	bne	a4,a5,800003a8 <consoleintr+0xea>
    800003d4:	b71d                	j	800002fa <consoleintr+0x3c>
    if(cons.e != cons.w){
    800003d6:	00011717          	auipc	a4,0x11
    800003da:	dca70713          	addi	a4,a4,-566 # 800111a0 <cons>
    800003de:	0a072783          	lw	a5,160(a4)
    800003e2:	09c72703          	lw	a4,156(a4)
    800003e6:	f0f70ae3          	beq	a4,a5,800002fa <consoleintr+0x3c>
      cons.e--;
    800003ea:	37fd                	addiw	a5,a5,-1
    800003ec:	00011717          	auipc	a4,0x11
    800003f0:	e4f72a23          	sw	a5,-428(a4) # 80011240 <cons+0xa0>
      consputc(BACKSPACE);
    800003f4:	10000513          	li	a0,256
    800003f8:	00000097          	auipc	ra,0x0
    800003fc:	e84080e7          	jalr	-380(ra) # 8000027c <consputc>
    80000400:	bded                	j	800002fa <consoleintr+0x3c>
    if(c != 0 && cons.e-cons.r < INPUT_BUF){
    80000402:	ee048ce3          	beqz	s1,800002fa <consoleintr+0x3c>
    80000406:	bf21                	j	8000031e <consoleintr+0x60>
      consputc(c);
    80000408:	4529                	li	a0,10
    8000040a:	00000097          	auipc	ra,0x0
    8000040e:	e72080e7          	jalr	-398(ra) # 8000027c <consputc>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000412:	00011797          	auipc	a5,0x11
    80000416:	d8e78793          	addi	a5,a5,-626 # 800111a0 <cons>
    8000041a:	0a07a703          	lw	a4,160(a5)
    8000041e:	0017069b          	addiw	a3,a4,1
    80000422:	0006861b          	sext.w	a2,a3
    80000426:	0ad7a023          	sw	a3,160(a5)
    8000042a:	07f77713          	andi	a4,a4,127
    8000042e:	97ba                	add	a5,a5,a4
    80000430:	4729                	li	a4,10
    80000432:	00e78c23          	sb	a4,24(a5)
        cons.w = cons.e;
    80000436:	00011797          	auipc	a5,0x11
    8000043a:	e0c7a323          	sw	a2,-506(a5) # 8001123c <cons+0x9c>
        wakeup(&cons.r);
    8000043e:	00011517          	auipc	a0,0x11
    80000442:	dfa50513          	addi	a0,a0,-518 # 80011238 <cons+0x98>
    80000446:	00002097          	auipc	ra,0x2
    8000044a:	100080e7          	jalr	256(ra) # 80002546 <wakeup>
    8000044e:	b575                	j	800002fa <consoleintr+0x3c>

0000000080000450 <consoleinit>:

void
consoleinit(void)
{
    80000450:	1141                	addi	sp,sp,-16
    80000452:	e406                	sd	ra,8(sp)
    80000454:	e022                	sd	s0,0(sp)
    80000456:	0800                	addi	s0,sp,16
  initlock(&cons.lock, "cons");
    80000458:	00008597          	auipc	a1,0x8
    8000045c:	bb858593          	addi	a1,a1,-1096 # 80008010 <etext+0x10>
    80000460:	00011517          	auipc	a0,0x11
    80000464:	d4050513          	addi	a0,a0,-704 # 800111a0 <cons>
    80000468:	00000097          	auipc	ra,0x0
    8000046c:	6ec080e7          	jalr	1772(ra) # 80000b54 <initlock>

  uartinit();
    80000470:	00000097          	auipc	ra,0x0
    80000474:	330080e7          	jalr	816(ra) # 800007a0 <uartinit>

  // connect read and write system calls
  // to consoleread and consolewrite.
  devsw[CONSOLE].read = consoleread;
    80000478:	00021797          	auipc	a5,0x21
    8000047c:	6c078793          	addi	a5,a5,1728 # 80021b38 <devsw>
    80000480:	00000717          	auipc	a4,0x0
    80000484:	ce470713          	addi	a4,a4,-796 # 80000164 <consoleread>
    80000488:	eb98                	sd	a4,16(a5)
  devsw[CONSOLE].write = consolewrite;
    8000048a:	00000717          	auipc	a4,0x0
    8000048e:	c7870713          	addi	a4,a4,-904 # 80000102 <consolewrite>
    80000492:	ef98                	sd	a4,24(a5)
}
    80000494:	60a2                	ld	ra,8(sp)
    80000496:	6402                	ld	s0,0(sp)
    80000498:	0141                	addi	sp,sp,16
    8000049a:	8082                	ret

000000008000049c <printint>:

static char digits[] = "0123456789abcdef";

static void
printint(int xx, int base, int sign)
{
    8000049c:	7179                	addi	sp,sp,-48
    8000049e:	f406                	sd	ra,40(sp)
    800004a0:	f022                	sd	s0,32(sp)
    800004a2:	ec26                	sd	s1,24(sp)
    800004a4:	e84a                	sd	s2,16(sp)
    800004a6:	1800                	addi	s0,sp,48
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
    800004a8:	c219                	beqz	a2,800004ae <printint+0x12>
    800004aa:	08054663          	bltz	a0,80000536 <printint+0x9a>
    x = -xx;
  else
    x = xx;
    800004ae:	2501                	sext.w	a0,a0
    800004b0:	4881                	li	a7,0
    800004b2:	fd040693          	addi	a3,s0,-48

  i = 0;
    800004b6:	4701                	li	a4,0
  do {
    buf[i++] = digits[x % base];
    800004b8:	2581                	sext.w	a1,a1
    800004ba:	00008617          	auipc	a2,0x8
    800004be:	b8660613          	addi	a2,a2,-1146 # 80008040 <digits>
    800004c2:	883a                	mv	a6,a4
    800004c4:	2705                	addiw	a4,a4,1
    800004c6:	02b577bb          	remuw	a5,a0,a1
    800004ca:	1782                	slli	a5,a5,0x20
    800004cc:	9381                	srli	a5,a5,0x20
    800004ce:	97b2                	add	a5,a5,a2
    800004d0:	0007c783          	lbu	a5,0(a5)
    800004d4:	00f68023          	sb	a5,0(a3)
  } while((x /= base) != 0);
    800004d8:	0005079b          	sext.w	a5,a0
    800004dc:	02b5553b          	divuw	a0,a0,a1
    800004e0:	0685                	addi	a3,a3,1
    800004e2:	feb7f0e3          	bgeu	a5,a1,800004c2 <printint+0x26>

  if(sign)
    800004e6:	00088b63          	beqz	a7,800004fc <printint+0x60>
    buf[i++] = '-';
    800004ea:	fe040793          	addi	a5,s0,-32
    800004ee:	973e                	add	a4,a4,a5
    800004f0:	02d00793          	li	a5,45
    800004f4:	fef70823          	sb	a5,-16(a4)
    800004f8:	0028071b          	addiw	a4,a6,2

  while(--i >= 0)
    800004fc:	02e05763          	blez	a4,8000052a <printint+0x8e>
    80000500:	fd040793          	addi	a5,s0,-48
    80000504:	00e784b3          	add	s1,a5,a4
    80000508:	fff78913          	addi	s2,a5,-1
    8000050c:	993a                	add	s2,s2,a4
    8000050e:	377d                	addiw	a4,a4,-1
    80000510:	1702                	slli	a4,a4,0x20
    80000512:	9301                	srli	a4,a4,0x20
    80000514:	40e90933          	sub	s2,s2,a4
    consputc(buf[i]);
    80000518:	fff4c503          	lbu	a0,-1(s1)
    8000051c:	00000097          	auipc	ra,0x0
    80000520:	d60080e7          	jalr	-672(ra) # 8000027c <consputc>
  while(--i >= 0)
    80000524:	14fd                	addi	s1,s1,-1
    80000526:	ff2499e3          	bne	s1,s2,80000518 <printint+0x7c>
}
    8000052a:	70a2                	ld	ra,40(sp)
    8000052c:	7402                	ld	s0,32(sp)
    8000052e:	64e2                	ld	s1,24(sp)
    80000530:	6942                	ld	s2,16(sp)
    80000532:	6145                	addi	sp,sp,48
    80000534:	8082                	ret
    x = -xx;
    80000536:	40a0053b          	negw	a0,a0
  if(sign && (sign = xx < 0))
    8000053a:	4885                	li	a7,1
    x = -xx;
    8000053c:	bf9d                	j	800004b2 <printint+0x16>

000000008000053e <panic>:
    release(&pr.lock);
}

void
panic(char *s)
{
    8000053e:	1101                	addi	sp,sp,-32
    80000540:	ec06                	sd	ra,24(sp)
    80000542:	e822                	sd	s0,16(sp)
    80000544:	e426                	sd	s1,8(sp)
    80000546:	1000                	addi	s0,sp,32
    80000548:	84aa                	mv	s1,a0
  pr.locking = 0;
    8000054a:	00011797          	auipc	a5,0x11
    8000054e:	d007ab23          	sw	zero,-746(a5) # 80011260 <pr+0x18>
  printf("panic: ");
    80000552:	00008517          	auipc	a0,0x8
    80000556:	ac650513          	addi	a0,a0,-1338 # 80008018 <etext+0x18>
    8000055a:	00000097          	auipc	ra,0x0
    8000055e:	02e080e7          	jalr	46(ra) # 80000588 <printf>
  printf(s);
    80000562:	8526                	mv	a0,s1
    80000564:	00000097          	auipc	ra,0x0
    80000568:	024080e7          	jalr	36(ra) # 80000588 <printf>
  printf("\n");
    8000056c:	00008517          	auipc	a0,0x8
    80000570:	db450513          	addi	a0,a0,-588 # 80008320 <digits+0x2e0>
    80000574:	00000097          	auipc	ra,0x0
    80000578:	014080e7          	jalr	20(ra) # 80000588 <printf>
  panicked = 1; // freeze uart output from other CPUs
    8000057c:	4785                	li	a5,1
    8000057e:	00009717          	auipc	a4,0x9
    80000582:	a8f72123          	sw	a5,-1406(a4) # 80009000 <panicked>
  for(;;)
    80000586:	a001                	j	80000586 <panic+0x48>

0000000080000588 <printf>:
{
    80000588:	7131                	addi	sp,sp,-192
    8000058a:	fc86                	sd	ra,120(sp)
    8000058c:	f8a2                	sd	s0,112(sp)
    8000058e:	f4a6                	sd	s1,104(sp)
    80000590:	f0ca                	sd	s2,96(sp)
    80000592:	ecce                	sd	s3,88(sp)
    80000594:	e8d2                	sd	s4,80(sp)
    80000596:	e4d6                	sd	s5,72(sp)
    80000598:	e0da                	sd	s6,64(sp)
    8000059a:	fc5e                	sd	s7,56(sp)
    8000059c:	f862                	sd	s8,48(sp)
    8000059e:	f466                	sd	s9,40(sp)
    800005a0:	f06a                	sd	s10,32(sp)
    800005a2:	ec6e                	sd	s11,24(sp)
    800005a4:	0100                	addi	s0,sp,128
    800005a6:	8a2a                	mv	s4,a0
    800005a8:	e40c                	sd	a1,8(s0)
    800005aa:	e810                	sd	a2,16(s0)
    800005ac:	ec14                	sd	a3,24(s0)
    800005ae:	f018                	sd	a4,32(s0)
    800005b0:	f41c                	sd	a5,40(s0)
    800005b2:	03043823          	sd	a6,48(s0)
    800005b6:	03143c23          	sd	a7,56(s0)
  locking = pr.locking;
    800005ba:	00011d97          	auipc	s11,0x11
    800005be:	ca6dad83          	lw	s11,-858(s11) # 80011260 <pr+0x18>
  if(locking)
    800005c2:	020d9b63          	bnez	s11,800005f8 <printf+0x70>
  if (fmt == 0)
    800005c6:	040a0263          	beqz	s4,8000060a <printf+0x82>
  va_start(ap, fmt);
    800005ca:	00840793          	addi	a5,s0,8
    800005ce:	f8f43423          	sd	a5,-120(s0)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
    800005d2:	000a4503          	lbu	a0,0(s4)
    800005d6:	16050263          	beqz	a0,8000073a <printf+0x1b2>
    800005da:	4481                	li	s1,0
    if(c != '%'){
    800005dc:	02500a93          	li	s5,37
    switch(c){
    800005e0:	07000b13          	li	s6,112
  consputc('x');
    800005e4:	4d41                	li	s10,16
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    800005e6:	00008b97          	auipc	s7,0x8
    800005ea:	a5ab8b93          	addi	s7,s7,-1446 # 80008040 <digits>
    switch(c){
    800005ee:	07300c93          	li	s9,115
    800005f2:	06400c13          	li	s8,100
    800005f6:	a82d                	j	80000630 <printf+0xa8>
    acquire(&pr.lock);
    800005f8:	00011517          	auipc	a0,0x11
    800005fc:	c5050513          	addi	a0,a0,-944 # 80011248 <pr>
    80000600:	00000097          	auipc	ra,0x0
    80000604:	5e4080e7          	jalr	1508(ra) # 80000be4 <acquire>
    80000608:	bf7d                	j	800005c6 <printf+0x3e>
    panic("null fmt");
    8000060a:	00008517          	auipc	a0,0x8
    8000060e:	a1e50513          	addi	a0,a0,-1506 # 80008028 <etext+0x28>
    80000612:	00000097          	auipc	ra,0x0
    80000616:	f2c080e7          	jalr	-212(ra) # 8000053e <panic>
      consputc(c);
    8000061a:	00000097          	auipc	ra,0x0
    8000061e:	c62080e7          	jalr	-926(ra) # 8000027c <consputc>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
    80000622:	2485                	addiw	s1,s1,1
    80000624:	009a07b3          	add	a5,s4,s1
    80000628:	0007c503          	lbu	a0,0(a5)
    8000062c:	10050763          	beqz	a0,8000073a <printf+0x1b2>
    if(c != '%'){
    80000630:	ff5515e3          	bne	a0,s5,8000061a <printf+0x92>
    c = fmt[++i] & 0xff;
    80000634:	2485                	addiw	s1,s1,1
    80000636:	009a07b3          	add	a5,s4,s1
    8000063a:	0007c783          	lbu	a5,0(a5)
    8000063e:	0007891b          	sext.w	s2,a5
    if(c == 0)
    80000642:	cfe5                	beqz	a5,8000073a <printf+0x1b2>
    switch(c){
    80000644:	05678a63          	beq	a5,s6,80000698 <printf+0x110>
    80000648:	02fb7663          	bgeu	s6,a5,80000674 <printf+0xec>
    8000064c:	09978963          	beq	a5,s9,800006de <printf+0x156>
    80000650:	07800713          	li	a4,120
    80000654:	0ce79863          	bne	a5,a4,80000724 <printf+0x19c>
      printint(va_arg(ap, int), 16, 1);
    80000658:	f8843783          	ld	a5,-120(s0)
    8000065c:	00878713          	addi	a4,a5,8
    80000660:	f8e43423          	sd	a4,-120(s0)
    80000664:	4605                	li	a2,1
    80000666:	85ea                	mv	a1,s10
    80000668:	4388                	lw	a0,0(a5)
    8000066a:	00000097          	auipc	ra,0x0
    8000066e:	e32080e7          	jalr	-462(ra) # 8000049c <printint>
      break;
    80000672:	bf45                	j	80000622 <printf+0x9a>
    switch(c){
    80000674:	0b578263          	beq	a5,s5,80000718 <printf+0x190>
    80000678:	0b879663          	bne	a5,s8,80000724 <printf+0x19c>
      printint(va_arg(ap, int), 10, 1);
    8000067c:	f8843783          	ld	a5,-120(s0)
    80000680:	00878713          	addi	a4,a5,8
    80000684:	f8e43423          	sd	a4,-120(s0)
    80000688:	4605                	li	a2,1
    8000068a:	45a9                	li	a1,10
    8000068c:	4388                	lw	a0,0(a5)
    8000068e:	00000097          	auipc	ra,0x0
    80000692:	e0e080e7          	jalr	-498(ra) # 8000049c <printint>
      break;
    80000696:	b771                	j	80000622 <printf+0x9a>
      printptr(va_arg(ap, uint64));
    80000698:	f8843783          	ld	a5,-120(s0)
    8000069c:	00878713          	addi	a4,a5,8
    800006a0:	f8e43423          	sd	a4,-120(s0)
    800006a4:	0007b983          	ld	s3,0(a5)
  consputc('0');
    800006a8:	03000513          	li	a0,48
    800006ac:	00000097          	auipc	ra,0x0
    800006b0:	bd0080e7          	jalr	-1072(ra) # 8000027c <consputc>
  consputc('x');
    800006b4:	07800513          	li	a0,120
    800006b8:	00000097          	auipc	ra,0x0
    800006bc:	bc4080e7          	jalr	-1084(ra) # 8000027c <consputc>
    800006c0:	896a                	mv	s2,s10
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    800006c2:	03c9d793          	srli	a5,s3,0x3c
    800006c6:	97de                	add	a5,a5,s7
    800006c8:	0007c503          	lbu	a0,0(a5)
    800006cc:	00000097          	auipc	ra,0x0
    800006d0:	bb0080e7          	jalr	-1104(ra) # 8000027c <consputc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
    800006d4:	0992                	slli	s3,s3,0x4
    800006d6:	397d                	addiw	s2,s2,-1
    800006d8:	fe0915e3          	bnez	s2,800006c2 <printf+0x13a>
    800006dc:	b799                	j	80000622 <printf+0x9a>
      if((s = va_arg(ap, char*)) == 0)
    800006de:	f8843783          	ld	a5,-120(s0)
    800006e2:	00878713          	addi	a4,a5,8
    800006e6:	f8e43423          	sd	a4,-120(s0)
    800006ea:	0007b903          	ld	s2,0(a5)
    800006ee:	00090e63          	beqz	s2,8000070a <printf+0x182>
      for(; *s; s++)
    800006f2:	00094503          	lbu	a0,0(s2)
    800006f6:	d515                	beqz	a0,80000622 <printf+0x9a>
        consputc(*s);
    800006f8:	00000097          	auipc	ra,0x0
    800006fc:	b84080e7          	jalr	-1148(ra) # 8000027c <consputc>
      for(; *s; s++)
    80000700:	0905                	addi	s2,s2,1
    80000702:	00094503          	lbu	a0,0(s2)
    80000706:	f96d                	bnez	a0,800006f8 <printf+0x170>
    80000708:	bf29                	j	80000622 <printf+0x9a>
        s = "(null)";
    8000070a:	00008917          	auipc	s2,0x8
    8000070e:	91690913          	addi	s2,s2,-1770 # 80008020 <etext+0x20>
      for(; *s; s++)
    80000712:	02800513          	li	a0,40
    80000716:	b7cd                	j	800006f8 <printf+0x170>
      consputc('%');
    80000718:	8556                	mv	a0,s5
    8000071a:	00000097          	auipc	ra,0x0
    8000071e:	b62080e7          	jalr	-1182(ra) # 8000027c <consputc>
      break;
    80000722:	b701                	j	80000622 <printf+0x9a>
      consputc('%');
    80000724:	8556                	mv	a0,s5
    80000726:	00000097          	auipc	ra,0x0
    8000072a:	b56080e7          	jalr	-1194(ra) # 8000027c <consputc>
      consputc(c);
    8000072e:	854a                	mv	a0,s2
    80000730:	00000097          	auipc	ra,0x0
    80000734:	b4c080e7          	jalr	-1204(ra) # 8000027c <consputc>
      break;
    80000738:	b5ed                	j	80000622 <printf+0x9a>
  if(locking)
    8000073a:	020d9163          	bnez	s11,8000075c <printf+0x1d4>
}
    8000073e:	70e6                	ld	ra,120(sp)
    80000740:	7446                	ld	s0,112(sp)
    80000742:	74a6                	ld	s1,104(sp)
    80000744:	7906                	ld	s2,96(sp)
    80000746:	69e6                	ld	s3,88(sp)
    80000748:	6a46                	ld	s4,80(sp)
    8000074a:	6aa6                	ld	s5,72(sp)
    8000074c:	6b06                	ld	s6,64(sp)
    8000074e:	7be2                	ld	s7,56(sp)
    80000750:	7c42                	ld	s8,48(sp)
    80000752:	7ca2                	ld	s9,40(sp)
    80000754:	7d02                	ld	s10,32(sp)
    80000756:	6de2                	ld	s11,24(sp)
    80000758:	6129                	addi	sp,sp,192
    8000075a:	8082                	ret
    release(&pr.lock);
    8000075c:	00011517          	auipc	a0,0x11
    80000760:	aec50513          	addi	a0,a0,-1300 # 80011248 <pr>
    80000764:	00000097          	auipc	ra,0x0
    80000768:	534080e7          	jalr	1332(ra) # 80000c98 <release>
}
    8000076c:	bfc9                	j	8000073e <printf+0x1b6>

000000008000076e <printfinit>:
    ;
}

void
printfinit(void)
{
    8000076e:	1101                	addi	sp,sp,-32
    80000770:	ec06                	sd	ra,24(sp)
    80000772:	e822                	sd	s0,16(sp)
    80000774:	e426                	sd	s1,8(sp)
    80000776:	1000                	addi	s0,sp,32
  initlock(&pr.lock, "pr");
    80000778:	00011497          	auipc	s1,0x11
    8000077c:	ad048493          	addi	s1,s1,-1328 # 80011248 <pr>
    80000780:	00008597          	auipc	a1,0x8
    80000784:	8b858593          	addi	a1,a1,-1864 # 80008038 <etext+0x38>
    80000788:	8526                	mv	a0,s1
    8000078a:	00000097          	auipc	ra,0x0
    8000078e:	3ca080e7          	jalr	970(ra) # 80000b54 <initlock>
  pr.locking = 1;
    80000792:	4785                	li	a5,1
    80000794:	cc9c                	sw	a5,24(s1)
}
    80000796:	60e2                	ld	ra,24(sp)
    80000798:	6442                	ld	s0,16(sp)
    8000079a:	64a2                	ld	s1,8(sp)
    8000079c:	6105                	addi	sp,sp,32
    8000079e:	8082                	ret

00000000800007a0 <uartinit>:

void uartstart();

void
uartinit(void)
{
    800007a0:	1141                	addi	sp,sp,-16
    800007a2:	e406                	sd	ra,8(sp)
    800007a4:	e022                	sd	s0,0(sp)
    800007a6:	0800                	addi	s0,sp,16
  // disable interrupts.
  WriteReg(IER, 0x00);
    800007a8:	100007b7          	lui	a5,0x10000
    800007ac:	000780a3          	sb	zero,1(a5) # 10000001 <_entry-0x6fffffff>

  // special mode to set baud rate.
  WriteReg(LCR, LCR_BAUD_LATCH);
    800007b0:	f8000713          	li	a4,-128
    800007b4:	00e781a3          	sb	a4,3(a5)

  // LSB for baud rate of 38.4K.
  WriteReg(0, 0x03);
    800007b8:	470d                	li	a4,3
    800007ba:	00e78023          	sb	a4,0(a5)

  // MSB for baud rate of 38.4K.
  WriteReg(1, 0x00);
    800007be:	000780a3          	sb	zero,1(a5)

  // leave set-baud mode,
  // and set word length to 8 bits, no parity.
  WriteReg(LCR, LCR_EIGHT_BITS);
    800007c2:	00e781a3          	sb	a4,3(a5)

  // reset and enable FIFOs.
  WriteReg(FCR, FCR_FIFO_ENABLE | FCR_FIFO_CLEAR);
    800007c6:	469d                	li	a3,7
    800007c8:	00d78123          	sb	a3,2(a5)

  // enable transmit and receive interrupts.
  WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);
    800007cc:	00e780a3          	sb	a4,1(a5)

  initlock(&uart_tx_lock, "uart");
    800007d0:	00008597          	auipc	a1,0x8
    800007d4:	88858593          	addi	a1,a1,-1912 # 80008058 <digits+0x18>
    800007d8:	00011517          	auipc	a0,0x11
    800007dc:	a9050513          	addi	a0,a0,-1392 # 80011268 <uart_tx_lock>
    800007e0:	00000097          	auipc	ra,0x0
    800007e4:	374080e7          	jalr	884(ra) # 80000b54 <initlock>
}
    800007e8:	60a2                	ld	ra,8(sp)
    800007ea:	6402                	ld	s0,0(sp)
    800007ec:	0141                	addi	sp,sp,16
    800007ee:	8082                	ret

00000000800007f0 <uartputc_sync>:
// use interrupts, for use by kernel printf() and
// to echo characters. it spins waiting for the uart's
// output register to be empty.
void
uartputc_sync(int c)
{
    800007f0:	1101                	addi	sp,sp,-32
    800007f2:	ec06                	sd	ra,24(sp)
    800007f4:	e822                	sd	s0,16(sp)
    800007f6:	e426                	sd	s1,8(sp)
    800007f8:	1000                	addi	s0,sp,32
    800007fa:	84aa                	mv	s1,a0
  push_off();
    800007fc:	00000097          	auipc	ra,0x0
    80000800:	39c080e7          	jalr	924(ra) # 80000b98 <push_off>

  if(panicked){
    80000804:	00008797          	auipc	a5,0x8
    80000808:	7fc7a783          	lw	a5,2044(a5) # 80009000 <panicked>
    for(;;)
      ;
  }

  // wait for Transmit Holding Empty to be set in LSR.
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    8000080c:	10000737          	lui	a4,0x10000
  if(panicked){
    80000810:	c391                	beqz	a5,80000814 <uartputc_sync+0x24>
    for(;;)
    80000812:	a001                	j	80000812 <uartputc_sync+0x22>
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    80000814:	00574783          	lbu	a5,5(a4) # 10000005 <_entry-0x6ffffffb>
    80000818:	0ff7f793          	andi	a5,a5,255
    8000081c:	0207f793          	andi	a5,a5,32
    80000820:	dbf5                	beqz	a5,80000814 <uartputc_sync+0x24>
    ;
  WriteReg(THR, c);
    80000822:	0ff4f793          	andi	a5,s1,255
    80000826:	10000737          	lui	a4,0x10000
    8000082a:	00f70023          	sb	a5,0(a4) # 10000000 <_entry-0x70000000>

  pop_off();
    8000082e:	00000097          	auipc	ra,0x0
    80000832:	40a080e7          	jalr	1034(ra) # 80000c38 <pop_off>
}
    80000836:	60e2                	ld	ra,24(sp)
    80000838:	6442                	ld	s0,16(sp)
    8000083a:	64a2                	ld	s1,8(sp)
    8000083c:	6105                	addi	sp,sp,32
    8000083e:	8082                	ret

0000000080000840 <uartstart>:
// called from both the top- and bottom-half.
void
uartstart()
{
  while(1){
    if(uart_tx_w == uart_tx_r){
    80000840:	00008717          	auipc	a4,0x8
    80000844:	7c873703          	ld	a4,1992(a4) # 80009008 <uart_tx_r>
    80000848:	00008797          	auipc	a5,0x8
    8000084c:	7c87b783          	ld	a5,1992(a5) # 80009010 <uart_tx_w>
    80000850:	06e78c63          	beq	a5,a4,800008c8 <uartstart+0x88>
{
    80000854:	7139                	addi	sp,sp,-64
    80000856:	fc06                	sd	ra,56(sp)
    80000858:	f822                	sd	s0,48(sp)
    8000085a:	f426                	sd	s1,40(sp)
    8000085c:	f04a                	sd	s2,32(sp)
    8000085e:	ec4e                	sd	s3,24(sp)
    80000860:	e852                	sd	s4,16(sp)
    80000862:	e456                	sd	s5,8(sp)
    80000864:	0080                	addi	s0,sp,64
      // transmit buffer is empty.
      return;
    }
    
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){
    80000866:	10000937          	lui	s2,0x10000
      // so we cannot give it another byte.
      // it will interrupt when it's ready for a new byte.
      return;
    }
    
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE];
    8000086a:	00011a17          	auipc	s4,0x11
    8000086e:	9fea0a13          	addi	s4,s4,-1538 # 80011268 <uart_tx_lock>
    uart_tx_r += 1;
    80000872:	00008497          	auipc	s1,0x8
    80000876:	79648493          	addi	s1,s1,1942 # 80009008 <uart_tx_r>
    if(uart_tx_w == uart_tx_r){
    8000087a:	00008997          	auipc	s3,0x8
    8000087e:	79698993          	addi	s3,s3,1942 # 80009010 <uart_tx_w>
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){
    80000882:	00594783          	lbu	a5,5(s2) # 10000005 <_entry-0x6ffffffb>
    80000886:	0ff7f793          	andi	a5,a5,255
    8000088a:	0207f793          	andi	a5,a5,32
    8000088e:	c785                	beqz	a5,800008b6 <uartstart+0x76>
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE];
    80000890:	01f77793          	andi	a5,a4,31
    80000894:	97d2                	add	a5,a5,s4
    80000896:	0187ca83          	lbu	s5,24(a5)
    uart_tx_r += 1;
    8000089a:	0705                	addi	a4,a4,1
    8000089c:	e098                	sd	a4,0(s1)
    
    // maybe uartputc() is waiting for space in the buffer.
    wakeup(&uart_tx_r);
    8000089e:	8526                	mv	a0,s1
    800008a0:	00002097          	auipc	ra,0x2
    800008a4:	ca6080e7          	jalr	-858(ra) # 80002546 <wakeup>
    
    WriteReg(THR, c);
    800008a8:	01590023          	sb	s5,0(s2)
    if(uart_tx_w == uart_tx_r){
    800008ac:	6098                	ld	a4,0(s1)
    800008ae:	0009b783          	ld	a5,0(s3)
    800008b2:	fce798e3          	bne	a5,a4,80000882 <uartstart+0x42>
  }
}
    800008b6:	70e2                	ld	ra,56(sp)
    800008b8:	7442                	ld	s0,48(sp)
    800008ba:	74a2                	ld	s1,40(sp)
    800008bc:	7902                	ld	s2,32(sp)
    800008be:	69e2                	ld	s3,24(sp)
    800008c0:	6a42                	ld	s4,16(sp)
    800008c2:	6aa2                	ld	s5,8(sp)
    800008c4:	6121                	addi	sp,sp,64
    800008c6:	8082                	ret
    800008c8:	8082                	ret

00000000800008ca <uartputc>:
{
    800008ca:	7179                	addi	sp,sp,-48
    800008cc:	f406                	sd	ra,40(sp)
    800008ce:	f022                	sd	s0,32(sp)
    800008d0:	ec26                	sd	s1,24(sp)
    800008d2:	e84a                	sd	s2,16(sp)
    800008d4:	e44e                	sd	s3,8(sp)
    800008d6:	e052                	sd	s4,0(sp)
    800008d8:	1800                	addi	s0,sp,48
    800008da:	89aa                	mv	s3,a0
  acquire(&uart_tx_lock);
    800008dc:	00011517          	auipc	a0,0x11
    800008e0:	98c50513          	addi	a0,a0,-1652 # 80011268 <uart_tx_lock>
    800008e4:	00000097          	auipc	ra,0x0
    800008e8:	300080e7          	jalr	768(ra) # 80000be4 <acquire>
  if(panicked){
    800008ec:	00008797          	auipc	a5,0x8
    800008f0:	7147a783          	lw	a5,1812(a5) # 80009000 <panicked>
    800008f4:	c391                	beqz	a5,800008f8 <uartputc+0x2e>
    for(;;)
    800008f6:	a001                	j	800008f6 <uartputc+0x2c>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    800008f8:	00008797          	auipc	a5,0x8
    800008fc:	7187b783          	ld	a5,1816(a5) # 80009010 <uart_tx_w>
    80000900:	00008717          	auipc	a4,0x8
    80000904:	70873703          	ld	a4,1800(a4) # 80009008 <uart_tx_r>
    80000908:	02070713          	addi	a4,a4,32
    8000090c:	02f71b63          	bne	a4,a5,80000942 <uartputc+0x78>
      sleep(&uart_tx_r, &uart_tx_lock);
    80000910:	00011a17          	auipc	s4,0x11
    80000914:	958a0a13          	addi	s4,s4,-1704 # 80011268 <uart_tx_lock>
    80000918:	00008497          	auipc	s1,0x8
    8000091c:	6f048493          	addi	s1,s1,1776 # 80009008 <uart_tx_r>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    80000920:	00008917          	auipc	s2,0x8
    80000924:	6f090913          	addi	s2,s2,1776 # 80009010 <uart_tx_w>
      sleep(&uart_tx_r, &uart_tx_lock);
    80000928:	85d2                	mv	a1,s4
    8000092a:	8526                	mv	a0,s1
    8000092c:	00002097          	auipc	ra,0x2
    80000930:	a84080e7          	jalr	-1404(ra) # 800023b0 <sleep>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    80000934:	00093783          	ld	a5,0(s2)
    80000938:	6098                	ld	a4,0(s1)
    8000093a:	02070713          	addi	a4,a4,32
    8000093e:	fef705e3          	beq	a4,a5,80000928 <uartputc+0x5e>
      uart_tx_buf[uart_tx_w % UART_TX_BUF_SIZE] = c;
    80000942:	00011497          	auipc	s1,0x11
    80000946:	92648493          	addi	s1,s1,-1754 # 80011268 <uart_tx_lock>
    8000094a:	01f7f713          	andi	a4,a5,31
    8000094e:	9726                	add	a4,a4,s1
    80000950:	01370c23          	sb	s3,24(a4)
      uart_tx_w += 1;
    80000954:	0785                	addi	a5,a5,1
    80000956:	00008717          	auipc	a4,0x8
    8000095a:	6af73d23          	sd	a5,1722(a4) # 80009010 <uart_tx_w>
      uartstart();
    8000095e:	00000097          	auipc	ra,0x0
    80000962:	ee2080e7          	jalr	-286(ra) # 80000840 <uartstart>
      release(&uart_tx_lock);
    80000966:	8526                	mv	a0,s1
    80000968:	00000097          	auipc	ra,0x0
    8000096c:	330080e7          	jalr	816(ra) # 80000c98 <release>
}
    80000970:	70a2                	ld	ra,40(sp)
    80000972:	7402                	ld	s0,32(sp)
    80000974:	64e2                	ld	s1,24(sp)
    80000976:	6942                	ld	s2,16(sp)
    80000978:	69a2                	ld	s3,8(sp)
    8000097a:	6a02                	ld	s4,0(sp)
    8000097c:	6145                	addi	sp,sp,48
    8000097e:	8082                	ret

0000000080000980 <uartgetc>:

// read one input character from the UART.
// return -1 if none is waiting.
int
uartgetc(void)
{
    80000980:	1141                	addi	sp,sp,-16
    80000982:	e422                	sd	s0,8(sp)
    80000984:	0800                	addi	s0,sp,16
  if(ReadReg(LSR) & 0x01){
    80000986:	100007b7          	lui	a5,0x10000
    8000098a:	0057c783          	lbu	a5,5(a5) # 10000005 <_entry-0x6ffffffb>
    8000098e:	8b85                	andi	a5,a5,1
    80000990:	cb91                	beqz	a5,800009a4 <uartgetc+0x24>
    // input data is ready.
    return ReadReg(RHR);
    80000992:	100007b7          	lui	a5,0x10000
    80000996:	0007c503          	lbu	a0,0(a5) # 10000000 <_entry-0x70000000>
    8000099a:	0ff57513          	andi	a0,a0,255
  } else {
    return -1;
  }
}
    8000099e:	6422                	ld	s0,8(sp)
    800009a0:	0141                	addi	sp,sp,16
    800009a2:	8082                	ret
    return -1;
    800009a4:	557d                	li	a0,-1
    800009a6:	bfe5                	j	8000099e <uartgetc+0x1e>

00000000800009a8 <uartintr>:
// handle a uart interrupt, raised because input has
// arrived, or the uart is ready for more output, or
// both. called from trap.c.
void
uartintr(void)
{
    800009a8:	1101                	addi	sp,sp,-32
    800009aa:	ec06                	sd	ra,24(sp)
    800009ac:	e822                	sd	s0,16(sp)
    800009ae:	e426                	sd	s1,8(sp)
    800009b0:	1000                	addi	s0,sp,32
  // read and process incoming characters.
  while(1){
    int c = uartgetc();
    if(c == -1)
    800009b2:	54fd                	li	s1,-1
    int c = uartgetc();
    800009b4:	00000097          	auipc	ra,0x0
    800009b8:	fcc080e7          	jalr	-52(ra) # 80000980 <uartgetc>
    if(c == -1)
    800009bc:	00950763          	beq	a0,s1,800009ca <uartintr+0x22>
      break;
    consoleintr(c);
    800009c0:	00000097          	auipc	ra,0x0
    800009c4:	8fe080e7          	jalr	-1794(ra) # 800002be <consoleintr>
  while(1){
    800009c8:	b7f5                	j	800009b4 <uartintr+0xc>
  }

  // send buffered characters.
  acquire(&uart_tx_lock);
    800009ca:	00011497          	auipc	s1,0x11
    800009ce:	89e48493          	addi	s1,s1,-1890 # 80011268 <uart_tx_lock>
    800009d2:	8526                	mv	a0,s1
    800009d4:	00000097          	auipc	ra,0x0
    800009d8:	210080e7          	jalr	528(ra) # 80000be4 <acquire>
  uartstart();
    800009dc:	00000097          	auipc	ra,0x0
    800009e0:	e64080e7          	jalr	-412(ra) # 80000840 <uartstart>
  release(&uart_tx_lock);
    800009e4:	8526                	mv	a0,s1
    800009e6:	00000097          	auipc	ra,0x0
    800009ea:	2b2080e7          	jalr	690(ra) # 80000c98 <release>
}
    800009ee:	60e2                	ld	ra,24(sp)
    800009f0:	6442                	ld	s0,16(sp)
    800009f2:	64a2                	ld	s1,8(sp)
    800009f4:	6105                	addi	sp,sp,32
    800009f6:	8082                	ret

00000000800009f8 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
    800009f8:	1101                	addi	sp,sp,-32
    800009fa:	ec06                	sd	ra,24(sp)
    800009fc:	e822                	sd	s0,16(sp)
    800009fe:	e426                	sd	s1,8(sp)
    80000a00:	e04a                	sd	s2,0(sp)
    80000a02:	1000                	addi	s0,sp,32
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    80000a04:	03451793          	slli	a5,a0,0x34
    80000a08:	ebb9                	bnez	a5,80000a5e <kfree+0x66>
    80000a0a:	84aa                	mv	s1,a0
    80000a0c:	00025797          	auipc	a5,0x25
    80000a10:	5f478793          	addi	a5,a5,1524 # 80026000 <end>
    80000a14:	04f56563          	bltu	a0,a5,80000a5e <kfree+0x66>
    80000a18:	47c5                	li	a5,17
    80000a1a:	07ee                	slli	a5,a5,0x1b
    80000a1c:	04f57163          	bgeu	a0,a5,80000a5e <kfree+0x66>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);
    80000a20:	6605                	lui	a2,0x1
    80000a22:	4585                	li	a1,1
    80000a24:	00000097          	auipc	ra,0x0
    80000a28:	2bc080e7          	jalr	700(ra) # 80000ce0 <memset>

  r = (struct run*)pa;

  acquire(&kmem.lock);
    80000a2c:	00011917          	auipc	s2,0x11
    80000a30:	87490913          	addi	s2,s2,-1932 # 800112a0 <kmem>
    80000a34:	854a                	mv	a0,s2
    80000a36:	00000097          	auipc	ra,0x0
    80000a3a:	1ae080e7          	jalr	430(ra) # 80000be4 <acquire>
  r->next = kmem.freelist;
    80000a3e:	01893783          	ld	a5,24(s2)
    80000a42:	e09c                	sd	a5,0(s1)
  kmem.freelist = r;
    80000a44:	00993c23          	sd	s1,24(s2)
  release(&kmem.lock);
    80000a48:	854a                	mv	a0,s2
    80000a4a:	00000097          	auipc	ra,0x0
    80000a4e:	24e080e7          	jalr	590(ra) # 80000c98 <release>
}
    80000a52:	60e2                	ld	ra,24(sp)
    80000a54:	6442                	ld	s0,16(sp)
    80000a56:	64a2                	ld	s1,8(sp)
    80000a58:	6902                	ld	s2,0(sp)
    80000a5a:	6105                	addi	sp,sp,32
    80000a5c:	8082                	ret
    panic("kfree");
    80000a5e:	00007517          	auipc	a0,0x7
    80000a62:	60250513          	addi	a0,a0,1538 # 80008060 <digits+0x20>
    80000a66:	00000097          	auipc	ra,0x0
    80000a6a:	ad8080e7          	jalr	-1320(ra) # 8000053e <panic>

0000000080000a6e <freerange>:
{
    80000a6e:	7179                	addi	sp,sp,-48
    80000a70:	f406                	sd	ra,40(sp)
    80000a72:	f022                	sd	s0,32(sp)
    80000a74:	ec26                	sd	s1,24(sp)
    80000a76:	e84a                	sd	s2,16(sp)
    80000a78:	e44e                	sd	s3,8(sp)
    80000a7a:	e052                	sd	s4,0(sp)
    80000a7c:	1800                	addi	s0,sp,48
  p = (char*)PGROUNDUP((uint64)pa_start);
    80000a7e:	6785                	lui	a5,0x1
    80000a80:	fff78493          	addi	s1,a5,-1 # fff <_entry-0x7ffff001>
    80000a84:	94aa                	add	s1,s1,a0
    80000a86:	757d                	lui	a0,0xfffff
    80000a88:	8ce9                	and	s1,s1,a0
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a8a:	94be                	add	s1,s1,a5
    80000a8c:	0095ee63          	bltu	a1,s1,80000aa8 <freerange+0x3a>
    80000a90:	892e                	mv	s2,a1
    kfree(p);
    80000a92:	7a7d                	lui	s4,0xfffff
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a94:	6985                	lui	s3,0x1
    kfree(p);
    80000a96:	01448533          	add	a0,s1,s4
    80000a9a:	00000097          	auipc	ra,0x0
    80000a9e:	f5e080e7          	jalr	-162(ra) # 800009f8 <kfree>
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000aa2:	94ce                	add	s1,s1,s3
    80000aa4:	fe9979e3          	bgeu	s2,s1,80000a96 <freerange+0x28>
}
    80000aa8:	70a2                	ld	ra,40(sp)
    80000aaa:	7402                	ld	s0,32(sp)
    80000aac:	64e2                	ld	s1,24(sp)
    80000aae:	6942                	ld	s2,16(sp)
    80000ab0:	69a2                	ld	s3,8(sp)
    80000ab2:	6a02                	ld	s4,0(sp)
    80000ab4:	6145                	addi	sp,sp,48
    80000ab6:	8082                	ret

0000000080000ab8 <kinit>:
{
    80000ab8:	1141                	addi	sp,sp,-16
    80000aba:	e406                	sd	ra,8(sp)
    80000abc:	e022                	sd	s0,0(sp)
    80000abe:	0800                	addi	s0,sp,16
  initlock(&kmem.lock, "kmem");
    80000ac0:	00007597          	auipc	a1,0x7
    80000ac4:	5a858593          	addi	a1,a1,1448 # 80008068 <digits+0x28>
    80000ac8:	00010517          	auipc	a0,0x10
    80000acc:	7d850513          	addi	a0,a0,2008 # 800112a0 <kmem>
    80000ad0:	00000097          	auipc	ra,0x0
    80000ad4:	084080e7          	jalr	132(ra) # 80000b54 <initlock>
  freerange(end, (void*)PHYSTOP);
    80000ad8:	45c5                	li	a1,17
    80000ada:	05ee                	slli	a1,a1,0x1b
    80000adc:	00025517          	auipc	a0,0x25
    80000ae0:	52450513          	addi	a0,a0,1316 # 80026000 <end>
    80000ae4:	00000097          	auipc	ra,0x0
    80000ae8:	f8a080e7          	jalr	-118(ra) # 80000a6e <freerange>
}
    80000aec:	60a2                	ld	ra,8(sp)
    80000aee:	6402                	ld	s0,0(sp)
    80000af0:	0141                	addi	sp,sp,16
    80000af2:	8082                	ret

0000000080000af4 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
    80000af4:	1101                	addi	sp,sp,-32
    80000af6:	ec06                	sd	ra,24(sp)
    80000af8:	e822                	sd	s0,16(sp)
    80000afa:	e426                	sd	s1,8(sp)
    80000afc:	1000                	addi	s0,sp,32
  struct run *r;

  acquire(&kmem.lock);
    80000afe:	00010497          	auipc	s1,0x10
    80000b02:	7a248493          	addi	s1,s1,1954 # 800112a0 <kmem>
    80000b06:	8526                	mv	a0,s1
    80000b08:	00000097          	auipc	ra,0x0
    80000b0c:	0dc080e7          	jalr	220(ra) # 80000be4 <acquire>
  r = kmem.freelist;
    80000b10:	6c84                	ld	s1,24(s1)
  if(r)
    80000b12:	c885                	beqz	s1,80000b42 <kalloc+0x4e>
    kmem.freelist = r->next;
    80000b14:	609c                	ld	a5,0(s1)
    80000b16:	00010517          	auipc	a0,0x10
    80000b1a:	78a50513          	addi	a0,a0,1930 # 800112a0 <kmem>
    80000b1e:	ed1c                	sd	a5,24(a0)
  release(&kmem.lock);
    80000b20:	00000097          	auipc	ra,0x0
    80000b24:	178080e7          	jalr	376(ra) # 80000c98 <release>

  if(r)
    memset((char*)r, 5, PGSIZE); // fill with junk
    80000b28:	6605                	lui	a2,0x1
    80000b2a:	4595                	li	a1,5
    80000b2c:	8526                	mv	a0,s1
    80000b2e:	00000097          	auipc	ra,0x0
    80000b32:	1b2080e7          	jalr	434(ra) # 80000ce0 <memset>
  return (void*)r;
}
    80000b36:	8526                	mv	a0,s1
    80000b38:	60e2                	ld	ra,24(sp)
    80000b3a:	6442                	ld	s0,16(sp)
    80000b3c:	64a2                	ld	s1,8(sp)
    80000b3e:	6105                	addi	sp,sp,32
    80000b40:	8082                	ret
  release(&kmem.lock);
    80000b42:	00010517          	auipc	a0,0x10
    80000b46:	75e50513          	addi	a0,a0,1886 # 800112a0 <kmem>
    80000b4a:	00000097          	auipc	ra,0x0
    80000b4e:	14e080e7          	jalr	334(ra) # 80000c98 <release>
  if(r)
    80000b52:	b7d5                	j	80000b36 <kalloc+0x42>

0000000080000b54 <initlock>:
#include "proc.h"
#include "defs.h"

void
initlock(struct spinlock *lk, char *name)
{
    80000b54:	1141                	addi	sp,sp,-16
    80000b56:	e422                	sd	s0,8(sp)
    80000b58:	0800                	addi	s0,sp,16
  lk->name = name;
    80000b5a:	e50c                	sd	a1,8(a0)
  lk->locked = 0;
    80000b5c:	00052023          	sw	zero,0(a0)
  lk->cpu = 0;
    80000b60:	00053823          	sd	zero,16(a0)
}
    80000b64:	6422                	ld	s0,8(sp)
    80000b66:	0141                	addi	sp,sp,16
    80000b68:	8082                	ret

0000000080000b6a <holding>:
// Interrupts must be off.
int
holding(struct spinlock *lk)
{
  int r;
  r = (lk->locked && lk->cpu == mycpu());
    80000b6a:	411c                	lw	a5,0(a0)
    80000b6c:	e399                	bnez	a5,80000b72 <holding+0x8>
    80000b6e:	4501                	li	a0,0
  return r;
}
    80000b70:	8082                	ret
{
    80000b72:	1101                	addi	sp,sp,-32
    80000b74:	ec06                	sd	ra,24(sp)
    80000b76:	e822                	sd	s0,16(sp)
    80000b78:	e426                	sd	s1,8(sp)
    80000b7a:	1000                	addi	s0,sp,32
  r = (lk->locked && lk->cpu == mycpu());
    80000b7c:	6904                	ld	s1,16(a0)
    80000b7e:	00001097          	auipc	ra,0x1
    80000b82:	e44080e7          	jalr	-444(ra) # 800019c2 <mycpu>
    80000b86:	40a48533          	sub	a0,s1,a0
    80000b8a:	00153513          	seqz	a0,a0
}
    80000b8e:	60e2                	ld	ra,24(sp)
    80000b90:	6442                	ld	s0,16(sp)
    80000b92:	64a2                	ld	s1,8(sp)
    80000b94:	6105                	addi	sp,sp,32
    80000b96:	8082                	ret

0000000080000b98 <push_off>:
// it takes two pop_off()s to undo two push_off()s.  Also, if interrupts
// are initially off, then push_off, pop_off leaves them off.

void
push_off(void)
{
    80000b98:	1101                	addi	sp,sp,-32
    80000b9a:	ec06                	sd	ra,24(sp)
    80000b9c:	e822                	sd	s0,16(sp)
    80000b9e:	e426                	sd	s1,8(sp)
    80000ba0:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000ba2:	100024f3          	csrr	s1,sstatus
    80000ba6:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80000baa:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000bac:	10079073          	csrw	sstatus,a5
  int old = intr_get();

  intr_off();
  if(mycpu()->noff == 0)
    80000bb0:	00001097          	auipc	ra,0x1
    80000bb4:	e12080e7          	jalr	-494(ra) # 800019c2 <mycpu>
    80000bb8:	5d3c                	lw	a5,120(a0)
    80000bba:	cf89                	beqz	a5,80000bd4 <push_off+0x3c>
    mycpu()->intena = old;
  mycpu()->noff += 1;
    80000bbc:	00001097          	auipc	ra,0x1
    80000bc0:	e06080e7          	jalr	-506(ra) # 800019c2 <mycpu>
    80000bc4:	5d3c                	lw	a5,120(a0)
    80000bc6:	2785                	addiw	a5,a5,1
    80000bc8:	dd3c                	sw	a5,120(a0)
}
    80000bca:	60e2                	ld	ra,24(sp)
    80000bcc:	6442                	ld	s0,16(sp)
    80000bce:	64a2                	ld	s1,8(sp)
    80000bd0:	6105                	addi	sp,sp,32
    80000bd2:	8082                	ret
    mycpu()->intena = old;
    80000bd4:	00001097          	auipc	ra,0x1
    80000bd8:	dee080e7          	jalr	-530(ra) # 800019c2 <mycpu>
  return (x & SSTATUS_SIE) != 0;
    80000bdc:	8085                	srli	s1,s1,0x1
    80000bde:	8885                	andi	s1,s1,1
    80000be0:	dd64                	sw	s1,124(a0)
    80000be2:	bfe9                	j	80000bbc <push_off+0x24>

0000000080000be4 <acquire>:
{
    80000be4:	1101                	addi	sp,sp,-32
    80000be6:	ec06                	sd	ra,24(sp)
    80000be8:	e822                	sd	s0,16(sp)
    80000bea:	e426                	sd	s1,8(sp)
    80000bec:	1000                	addi	s0,sp,32
    80000bee:	84aa                	mv	s1,a0
  push_off(); // disable interrupts to avoid deadlock.
    80000bf0:	00000097          	auipc	ra,0x0
    80000bf4:	fa8080e7          	jalr	-88(ra) # 80000b98 <push_off>
  if(holding(lk))
    80000bf8:	8526                	mv	a0,s1
    80000bfa:	00000097          	auipc	ra,0x0
    80000bfe:	f70080e7          	jalr	-144(ra) # 80000b6a <holding>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000c02:	4705                	li	a4,1
  if(holding(lk))
    80000c04:	e115                	bnez	a0,80000c28 <acquire+0x44>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000c06:	87ba                	mv	a5,a4
    80000c08:	0cf4a7af          	amoswap.w.aq	a5,a5,(s1)
    80000c0c:	2781                	sext.w	a5,a5
    80000c0e:	ffe5                	bnez	a5,80000c06 <acquire+0x22>
  __sync_synchronize();
    80000c10:	0ff0000f          	fence
  lk->cpu = mycpu();
    80000c14:	00001097          	auipc	ra,0x1
    80000c18:	dae080e7          	jalr	-594(ra) # 800019c2 <mycpu>
    80000c1c:	e888                	sd	a0,16(s1)
}
    80000c1e:	60e2                	ld	ra,24(sp)
    80000c20:	6442                	ld	s0,16(sp)
    80000c22:	64a2                	ld	s1,8(sp)
    80000c24:	6105                	addi	sp,sp,32
    80000c26:	8082                	ret
    panic("acquire");
    80000c28:	00007517          	auipc	a0,0x7
    80000c2c:	44850513          	addi	a0,a0,1096 # 80008070 <digits+0x30>
    80000c30:	00000097          	auipc	ra,0x0
    80000c34:	90e080e7          	jalr	-1778(ra) # 8000053e <panic>

0000000080000c38 <pop_off>:

void
pop_off(void)
{
    80000c38:	1141                	addi	sp,sp,-16
    80000c3a:	e406                	sd	ra,8(sp)
    80000c3c:	e022                	sd	s0,0(sp)
    80000c3e:	0800                	addi	s0,sp,16
  struct cpu *c = mycpu();
    80000c40:	00001097          	auipc	ra,0x1
    80000c44:	d82080e7          	jalr	-638(ra) # 800019c2 <mycpu>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c48:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80000c4c:	8b89                	andi	a5,a5,2
  if(intr_get())
    80000c4e:	e78d                	bnez	a5,80000c78 <pop_off+0x40>
    panic("pop_off - interruptible");
  if(c->noff < 1)
    80000c50:	5d3c                	lw	a5,120(a0)
    80000c52:	02f05b63          	blez	a5,80000c88 <pop_off+0x50>
    panic("pop_off");
  c->noff -= 1;
    80000c56:	37fd                	addiw	a5,a5,-1
    80000c58:	0007871b          	sext.w	a4,a5
    80000c5c:	dd3c                	sw	a5,120(a0)
  if(c->noff == 0 && c->intena)
    80000c5e:	eb09                	bnez	a4,80000c70 <pop_off+0x38>
    80000c60:	5d7c                	lw	a5,124(a0)
    80000c62:	c799                	beqz	a5,80000c70 <pop_off+0x38>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c64:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80000c68:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000c6c:	10079073          	csrw	sstatus,a5
    intr_on();
}
    80000c70:	60a2                	ld	ra,8(sp)
    80000c72:	6402                	ld	s0,0(sp)
    80000c74:	0141                	addi	sp,sp,16
    80000c76:	8082                	ret
    panic("pop_off - interruptible");
    80000c78:	00007517          	auipc	a0,0x7
    80000c7c:	40050513          	addi	a0,a0,1024 # 80008078 <digits+0x38>
    80000c80:	00000097          	auipc	ra,0x0
    80000c84:	8be080e7          	jalr	-1858(ra) # 8000053e <panic>
    panic("pop_off");
    80000c88:	00007517          	auipc	a0,0x7
    80000c8c:	40850513          	addi	a0,a0,1032 # 80008090 <digits+0x50>
    80000c90:	00000097          	auipc	ra,0x0
    80000c94:	8ae080e7          	jalr	-1874(ra) # 8000053e <panic>

0000000080000c98 <release>:
{
    80000c98:	1101                	addi	sp,sp,-32
    80000c9a:	ec06                	sd	ra,24(sp)
    80000c9c:	e822                	sd	s0,16(sp)
    80000c9e:	e426                	sd	s1,8(sp)
    80000ca0:	1000                	addi	s0,sp,32
    80000ca2:	84aa                	mv	s1,a0
  if(!holding(lk))
    80000ca4:	00000097          	auipc	ra,0x0
    80000ca8:	ec6080e7          	jalr	-314(ra) # 80000b6a <holding>
    80000cac:	c115                	beqz	a0,80000cd0 <release+0x38>
  lk->cpu = 0;
    80000cae:	0004b823          	sd	zero,16(s1)
  __sync_synchronize();
    80000cb2:	0ff0000f          	fence
  __sync_lock_release(&lk->locked);
    80000cb6:	0f50000f          	fence	iorw,ow
    80000cba:	0804a02f          	amoswap.w	zero,zero,(s1)
  pop_off();
    80000cbe:	00000097          	auipc	ra,0x0
    80000cc2:	f7a080e7          	jalr	-134(ra) # 80000c38 <pop_off>
}
    80000cc6:	60e2                	ld	ra,24(sp)
    80000cc8:	6442                	ld	s0,16(sp)
    80000cca:	64a2                	ld	s1,8(sp)
    80000ccc:	6105                	addi	sp,sp,32
    80000cce:	8082                	ret
    panic("release");
    80000cd0:	00007517          	auipc	a0,0x7
    80000cd4:	3c850513          	addi	a0,a0,968 # 80008098 <digits+0x58>
    80000cd8:	00000097          	auipc	ra,0x0
    80000cdc:	866080e7          	jalr	-1946(ra) # 8000053e <panic>

0000000080000ce0 <memset>:
#include "types.h"

void*
memset(void *dst, int c, uint n)
{
    80000ce0:	1141                	addi	sp,sp,-16
    80000ce2:	e422                	sd	s0,8(sp)
    80000ce4:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
    80000ce6:	ce09                	beqz	a2,80000d00 <memset+0x20>
    80000ce8:	87aa                	mv	a5,a0
    80000cea:	fff6071b          	addiw	a4,a2,-1
    80000cee:	1702                	slli	a4,a4,0x20
    80000cf0:	9301                	srli	a4,a4,0x20
    80000cf2:	0705                	addi	a4,a4,1
    80000cf4:	972a                	add	a4,a4,a0
    cdst[i] = c;
    80000cf6:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
    80000cfa:	0785                	addi	a5,a5,1
    80000cfc:	fee79de3          	bne	a5,a4,80000cf6 <memset+0x16>
  }
  return dst;
}
    80000d00:	6422                	ld	s0,8(sp)
    80000d02:	0141                	addi	sp,sp,16
    80000d04:	8082                	ret

0000000080000d06 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
    80000d06:	1141                	addi	sp,sp,-16
    80000d08:	e422                	sd	s0,8(sp)
    80000d0a:	0800                	addi	s0,sp,16
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
    80000d0c:	ca05                	beqz	a2,80000d3c <memcmp+0x36>
    80000d0e:	fff6069b          	addiw	a3,a2,-1
    80000d12:	1682                	slli	a3,a3,0x20
    80000d14:	9281                	srli	a3,a3,0x20
    80000d16:	0685                	addi	a3,a3,1
    80000d18:	96aa                	add	a3,a3,a0
    if(*s1 != *s2)
    80000d1a:	00054783          	lbu	a5,0(a0)
    80000d1e:	0005c703          	lbu	a4,0(a1)
    80000d22:	00e79863          	bne	a5,a4,80000d32 <memcmp+0x2c>
      return *s1 - *s2;
    s1++, s2++;
    80000d26:	0505                	addi	a0,a0,1
    80000d28:	0585                	addi	a1,a1,1
  while(n-- > 0){
    80000d2a:	fed518e3          	bne	a0,a3,80000d1a <memcmp+0x14>
  }

  return 0;
    80000d2e:	4501                	li	a0,0
    80000d30:	a019                	j	80000d36 <memcmp+0x30>
      return *s1 - *s2;
    80000d32:	40e7853b          	subw	a0,a5,a4
}
    80000d36:	6422                	ld	s0,8(sp)
    80000d38:	0141                	addi	sp,sp,16
    80000d3a:	8082                	ret
  return 0;
    80000d3c:	4501                	li	a0,0
    80000d3e:	bfe5                	j	80000d36 <memcmp+0x30>

0000000080000d40 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
    80000d40:	1141                	addi	sp,sp,-16
    80000d42:	e422                	sd	s0,8(sp)
    80000d44:	0800                	addi	s0,sp,16
  const char *s;
  char *d;

  if(n == 0)
    80000d46:	ca0d                	beqz	a2,80000d78 <memmove+0x38>
    return dst;
  
  s = src;
  d = dst;
  if(s < d && s + n > d){
    80000d48:	00a5f963          	bgeu	a1,a0,80000d5a <memmove+0x1a>
    80000d4c:	02061693          	slli	a3,a2,0x20
    80000d50:	9281                	srli	a3,a3,0x20
    80000d52:	00d58733          	add	a4,a1,a3
    80000d56:	02e56463          	bltu	a0,a4,80000d7e <memmove+0x3e>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
    80000d5a:	fff6079b          	addiw	a5,a2,-1
    80000d5e:	1782                	slli	a5,a5,0x20
    80000d60:	9381                	srli	a5,a5,0x20
    80000d62:	0785                	addi	a5,a5,1
    80000d64:	97ae                	add	a5,a5,a1
    80000d66:	872a                	mv	a4,a0
      *d++ = *s++;
    80000d68:	0585                	addi	a1,a1,1
    80000d6a:	0705                	addi	a4,a4,1
    80000d6c:	fff5c683          	lbu	a3,-1(a1)
    80000d70:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
    80000d74:	fef59ae3          	bne	a1,a5,80000d68 <memmove+0x28>

  return dst;
}
    80000d78:	6422                	ld	s0,8(sp)
    80000d7a:	0141                	addi	sp,sp,16
    80000d7c:	8082                	ret
    d += n;
    80000d7e:	96aa                	add	a3,a3,a0
    while(n-- > 0)
    80000d80:	fff6079b          	addiw	a5,a2,-1
    80000d84:	1782                	slli	a5,a5,0x20
    80000d86:	9381                	srli	a5,a5,0x20
    80000d88:	fff7c793          	not	a5,a5
    80000d8c:	97ba                	add	a5,a5,a4
      *--d = *--s;
    80000d8e:	177d                	addi	a4,a4,-1
    80000d90:	16fd                	addi	a3,a3,-1
    80000d92:	00074603          	lbu	a2,0(a4)
    80000d96:	00c68023          	sb	a2,0(a3)
    while(n-- > 0)
    80000d9a:	fef71ae3          	bne	a4,a5,80000d8e <memmove+0x4e>
    80000d9e:	bfe9                	j	80000d78 <memmove+0x38>

0000000080000da0 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
    80000da0:	1141                	addi	sp,sp,-16
    80000da2:	e406                	sd	ra,8(sp)
    80000da4:	e022                	sd	s0,0(sp)
    80000da6:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
    80000da8:	00000097          	auipc	ra,0x0
    80000dac:	f98080e7          	jalr	-104(ra) # 80000d40 <memmove>
}
    80000db0:	60a2                	ld	ra,8(sp)
    80000db2:	6402                	ld	s0,0(sp)
    80000db4:	0141                	addi	sp,sp,16
    80000db6:	8082                	ret

0000000080000db8 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
    80000db8:	1141                	addi	sp,sp,-16
    80000dba:	e422                	sd	s0,8(sp)
    80000dbc:	0800                	addi	s0,sp,16
  while(n > 0 && *p && *p == *q)
    80000dbe:	ce11                	beqz	a2,80000dda <strncmp+0x22>
    80000dc0:	00054783          	lbu	a5,0(a0)
    80000dc4:	cf89                	beqz	a5,80000dde <strncmp+0x26>
    80000dc6:	0005c703          	lbu	a4,0(a1)
    80000dca:	00f71a63          	bne	a4,a5,80000dde <strncmp+0x26>
    n--, p++, q++;
    80000dce:	367d                	addiw	a2,a2,-1
    80000dd0:	0505                	addi	a0,a0,1
    80000dd2:	0585                	addi	a1,a1,1
  while(n > 0 && *p && *p == *q)
    80000dd4:	f675                	bnez	a2,80000dc0 <strncmp+0x8>
  if(n == 0)
    return 0;
    80000dd6:	4501                	li	a0,0
    80000dd8:	a809                	j	80000dea <strncmp+0x32>
    80000dda:	4501                	li	a0,0
    80000ddc:	a039                	j	80000dea <strncmp+0x32>
  if(n == 0)
    80000dde:	ca09                	beqz	a2,80000df0 <strncmp+0x38>
  return (uchar)*p - (uchar)*q;
    80000de0:	00054503          	lbu	a0,0(a0)
    80000de4:	0005c783          	lbu	a5,0(a1)
    80000de8:	9d1d                	subw	a0,a0,a5
}
    80000dea:	6422                	ld	s0,8(sp)
    80000dec:	0141                	addi	sp,sp,16
    80000dee:	8082                	ret
    return 0;
    80000df0:	4501                	li	a0,0
    80000df2:	bfe5                	j	80000dea <strncmp+0x32>

0000000080000df4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
    80000df4:	1141                	addi	sp,sp,-16
    80000df6:	e422                	sd	s0,8(sp)
    80000df8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    80000dfa:	872a                	mv	a4,a0
    80000dfc:	8832                	mv	a6,a2
    80000dfe:	367d                	addiw	a2,a2,-1
    80000e00:	01005963          	blez	a6,80000e12 <strncpy+0x1e>
    80000e04:	0705                	addi	a4,a4,1
    80000e06:	0005c783          	lbu	a5,0(a1)
    80000e0a:	fef70fa3          	sb	a5,-1(a4)
    80000e0e:	0585                	addi	a1,a1,1
    80000e10:	f7f5                	bnez	a5,80000dfc <strncpy+0x8>
    ;
  while(n-- > 0)
    80000e12:	00c05d63          	blez	a2,80000e2c <strncpy+0x38>
    80000e16:	86ba                	mv	a3,a4
    *s++ = 0;
    80000e18:	0685                	addi	a3,a3,1
    80000e1a:	fe068fa3          	sb	zero,-1(a3)
  while(n-- > 0)
    80000e1e:	fff6c793          	not	a5,a3
    80000e22:	9fb9                	addw	a5,a5,a4
    80000e24:	010787bb          	addw	a5,a5,a6
    80000e28:	fef048e3          	bgtz	a5,80000e18 <strncpy+0x24>
  return os;
}
    80000e2c:	6422                	ld	s0,8(sp)
    80000e2e:	0141                	addi	sp,sp,16
    80000e30:	8082                	ret

0000000080000e32 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
    80000e32:	1141                	addi	sp,sp,-16
    80000e34:	e422                	sd	s0,8(sp)
    80000e36:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  if(n <= 0)
    80000e38:	02c05363          	blez	a2,80000e5e <safestrcpy+0x2c>
    80000e3c:	fff6069b          	addiw	a3,a2,-1
    80000e40:	1682                	slli	a3,a3,0x20
    80000e42:	9281                	srli	a3,a3,0x20
    80000e44:	96ae                	add	a3,a3,a1
    80000e46:	87aa                	mv	a5,a0
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
    80000e48:	00d58963          	beq	a1,a3,80000e5a <safestrcpy+0x28>
    80000e4c:	0585                	addi	a1,a1,1
    80000e4e:	0785                	addi	a5,a5,1
    80000e50:	fff5c703          	lbu	a4,-1(a1)
    80000e54:	fee78fa3          	sb	a4,-1(a5)
    80000e58:	fb65                	bnez	a4,80000e48 <safestrcpy+0x16>
    ;
  *s = 0;
    80000e5a:	00078023          	sb	zero,0(a5)
  return os;
}
    80000e5e:	6422                	ld	s0,8(sp)
    80000e60:	0141                	addi	sp,sp,16
    80000e62:	8082                	ret

0000000080000e64 <strlen>:

int
strlen(const char *s)
{
    80000e64:	1141                	addi	sp,sp,-16
    80000e66:	e422                	sd	s0,8(sp)
    80000e68:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
    80000e6a:	00054783          	lbu	a5,0(a0)
    80000e6e:	cf91                	beqz	a5,80000e8a <strlen+0x26>
    80000e70:	0505                	addi	a0,a0,1
    80000e72:	87aa                	mv	a5,a0
    80000e74:	4685                	li	a3,1
    80000e76:	9e89                	subw	a3,a3,a0
    80000e78:	00f6853b          	addw	a0,a3,a5
    80000e7c:	0785                	addi	a5,a5,1
    80000e7e:	fff7c703          	lbu	a4,-1(a5)
    80000e82:	fb7d                	bnez	a4,80000e78 <strlen+0x14>
    ;
  return n;
}
    80000e84:	6422                	ld	s0,8(sp)
    80000e86:	0141                	addi	sp,sp,16
    80000e88:	8082                	ret
  for(n = 0; s[n]; n++)
    80000e8a:	4501                	li	a0,0
    80000e8c:	bfe5                	j	80000e84 <strlen+0x20>

0000000080000e8e <main>:
volatile static int started = 0;

// start() jumps here in supervisor mode on all CPUs.
void
main()
{
    80000e8e:	1141                	addi	sp,sp,-16
    80000e90:	e406                	sd	ra,8(sp)
    80000e92:	e022                	sd	s0,0(sp)
    80000e94:	0800                	addi	s0,sp,16
  if(cpuid() == 0){
    80000e96:	00001097          	auipc	ra,0x1
    80000e9a:	b1c080e7          	jalr	-1252(ra) # 800019b2 <cpuid>
    virtio_disk_init(); // emulated hard disk
    userinit();      // first user process
    __sync_synchronize();
    started = 1;
  } else {
    while(started == 0)
    80000e9e:	00008717          	auipc	a4,0x8
    80000ea2:	17a70713          	addi	a4,a4,378 # 80009018 <started>
  if(cpuid() == 0){
    80000ea6:	c539                	beqz	a0,80000ef4 <main+0x66>
    while(started == 0)
    80000ea8:	431c                	lw	a5,0(a4)
    80000eaa:	2781                	sext.w	a5,a5
    80000eac:	dff5                	beqz	a5,80000ea8 <main+0x1a>
      ;
    __sync_synchronize();
    80000eae:	0ff0000f          	fence
    printf("hart %d starting\n", cpuid());
    80000eb2:	00001097          	auipc	ra,0x1
    80000eb6:	b00080e7          	jalr	-1280(ra) # 800019b2 <cpuid>
    80000eba:	85aa                	mv	a1,a0
    80000ebc:	00007517          	auipc	a0,0x7
    80000ec0:	1fc50513          	addi	a0,a0,508 # 800080b8 <digits+0x78>
    80000ec4:	fffff097          	auipc	ra,0xfffff
    80000ec8:	6c4080e7          	jalr	1732(ra) # 80000588 <printf>
    kvminithart();    // turn on paging
    80000ecc:	00000097          	auipc	ra,0x0
    80000ed0:	0e0080e7          	jalr	224(ra) # 80000fac <kvminithart>
    trapinithart();   // install kernel trap vector
    80000ed4:	00002097          	auipc	ra,0x2
    80000ed8:	c7e080e7          	jalr	-898(ra) # 80002b52 <trapinithart>
    plicinithart();   // ask PLIC for device interrupts
    80000edc:	00005097          	auipc	ra,0x5
    80000ee0:	234080e7          	jalr	564(ra) # 80006110 <plicinithart>
  }

  scheduler();        
    80000ee4:	00001097          	auipc	ra,0x1
    80000ee8:	390080e7          	jalr	912(ra) # 80002274 <scheduler>
}
    80000eec:	60a2                	ld	ra,8(sp)
    80000eee:	6402                	ld	s0,0(sp)
    80000ef0:	0141                	addi	sp,sp,16
    80000ef2:	8082                	ret
    consoleinit();
    80000ef4:	fffff097          	auipc	ra,0xfffff
    80000ef8:	55c080e7          	jalr	1372(ra) # 80000450 <consoleinit>
    printfinit();
    80000efc:	00000097          	auipc	ra,0x0
    80000f00:	872080e7          	jalr	-1934(ra) # 8000076e <printfinit>
    printf("\n");
    80000f04:	00007517          	auipc	a0,0x7
    80000f08:	41c50513          	addi	a0,a0,1052 # 80008320 <digits+0x2e0>
    80000f0c:	fffff097          	auipc	ra,0xfffff
    80000f10:	67c080e7          	jalr	1660(ra) # 80000588 <printf>
    printf("xv6 kernel is booting\n");
    80000f14:	00007517          	auipc	a0,0x7
    80000f18:	18c50513          	addi	a0,a0,396 # 800080a0 <digits+0x60>
    80000f1c:	fffff097          	auipc	ra,0xfffff
    80000f20:	66c080e7          	jalr	1644(ra) # 80000588 <printf>
    printf("\n");
    80000f24:	00007517          	auipc	a0,0x7
    80000f28:	3fc50513          	addi	a0,a0,1020 # 80008320 <digits+0x2e0>
    80000f2c:	fffff097          	auipc	ra,0xfffff
    80000f30:	65c080e7          	jalr	1628(ra) # 80000588 <printf>
    kinit();         // physical page allocator
    80000f34:	00000097          	auipc	ra,0x0
    80000f38:	b84080e7          	jalr	-1148(ra) # 80000ab8 <kinit>
    kvminit();       // create kernel page table
    80000f3c:	00000097          	auipc	ra,0x0
    80000f40:	322080e7          	jalr	802(ra) # 8000125e <kvminit>
    kvminithart();   // turn on paging
    80000f44:	00000097          	auipc	ra,0x0
    80000f48:	068080e7          	jalr	104(ra) # 80000fac <kvminithart>
    procinit();      // process table
    80000f4c:	00001097          	auipc	ra,0x1
    80000f50:	9a6080e7          	jalr	-1626(ra) # 800018f2 <procinit>
    trapinit();      // trap vectors
    80000f54:	00002097          	auipc	ra,0x2
    80000f58:	bd6080e7          	jalr	-1066(ra) # 80002b2a <trapinit>
    trapinithart();  // install kernel trap vector
    80000f5c:	00002097          	auipc	ra,0x2
    80000f60:	bf6080e7          	jalr	-1034(ra) # 80002b52 <trapinithart>
    plicinit();      // set up interrupt controller
    80000f64:	00005097          	auipc	ra,0x5
    80000f68:	196080e7          	jalr	406(ra) # 800060fa <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000f6c:	00005097          	auipc	ra,0x5
    80000f70:	1a4080e7          	jalr	420(ra) # 80006110 <plicinithart>
    binit();         // buffer cache
    80000f74:	00002097          	auipc	ra,0x2
    80000f78:	386080e7          	jalr	902(ra) # 800032fa <binit>
    iinit();         // inode table
    80000f7c:	00003097          	auipc	ra,0x3
    80000f80:	a16080e7          	jalr	-1514(ra) # 80003992 <iinit>
    fileinit();      // file table
    80000f84:	00004097          	auipc	ra,0x4
    80000f88:	9c0080e7          	jalr	-1600(ra) # 80004944 <fileinit>
    virtio_disk_init(); // emulated hard disk
    80000f8c:	00005097          	auipc	ra,0x5
    80000f90:	2a6080e7          	jalr	678(ra) # 80006232 <virtio_disk_init>
    userinit();      // first user process
    80000f94:	00001097          	auipc	ra,0x1
    80000f98:	d2a080e7          	jalr	-726(ra) # 80001cbe <userinit>
    __sync_synchronize();
    80000f9c:	0ff0000f          	fence
    started = 1;
    80000fa0:	4785                	li	a5,1
    80000fa2:	00008717          	auipc	a4,0x8
    80000fa6:	06f72b23          	sw	a5,118(a4) # 80009018 <started>
    80000faa:	bf2d                	j	80000ee4 <main+0x56>

0000000080000fac <kvminithart>:

// Switch h/w page table register to the kernel's page table,
// and enable paging.
void
kvminithart()
{
    80000fac:	1141                	addi	sp,sp,-16
    80000fae:	e422                	sd	s0,8(sp)
    80000fb0:	0800                	addi	s0,sp,16
  w_satp(MAKE_SATP(kernel_pagetable));
    80000fb2:	00008797          	auipc	a5,0x8
    80000fb6:	06e7b783          	ld	a5,110(a5) # 80009020 <kernel_pagetable>
    80000fba:	83b1                	srli	a5,a5,0xc
    80000fbc:	577d                	li	a4,-1
    80000fbe:	177e                	slli	a4,a4,0x3f
    80000fc0:	8fd9                	or	a5,a5,a4
  asm volatile("csrw satp, %0" : : "r" (x));
    80000fc2:	18079073          	csrw	satp,a5
// flush the TLB.
static inline void
sfence_vma()
{
  // the zero, zero means flush all TLB entries.
  asm volatile("sfence.vma zero, zero");
    80000fc6:	12000073          	sfence.vma
  sfence_vma();
}
    80000fca:	6422                	ld	s0,8(sp)
    80000fcc:	0141                	addi	sp,sp,16
    80000fce:	8082                	ret

0000000080000fd0 <walk>:
//   21..29 -- 9 bits of level-1 index.
//   12..20 -- 9 bits of level-0 index.
//    0..11 -- 12 bits of byte offset within the page.
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)
{
    80000fd0:	7139                	addi	sp,sp,-64
    80000fd2:	fc06                	sd	ra,56(sp)
    80000fd4:	f822                	sd	s0,48(sp)
    80000fd6:	f426                	sd	s1,40(sp)
    80000fd8:	f04a                	sd	s2,32(sp)
    80000fda:	ec4e                	sd	s3,24(sp)
    80000fdc:	e852                	sd	s4,16(sp)
    80000fde:	e456                	sd	s5,8(sp)
    80000fe0:	e05a                	sd	s6,0(sp)
    80000fe2:	0080                	addi	s0,sp,64
    80000fe4:	84aa                	mv	s1,a0
    80000fe6:	89ae                	mv	s3,a1
    80000fe8:	8ab2                	mv	s5,a2
  if(va >= MAXVA)
    80000fea:	57fd                	li	a5,-1
    80000fec:	83e9                	srli	a5,a5,0x1a
    80000fee:	4a79                	li	s4,30
    panic("walk");

  for(int level = 2; level > 0; level--) {
    80000ff0:	4b31                	li	s6,12
  if(va >= MAXVA)
    80000ff2:	04b7f263          	bgeu	a5,a1,80001036 <walk+0x66>
    panic("walk");
    80000ff6:	00007517          	auipc	a0,0x7
    80000ffa:	0da50513          	addi	a0,a0,218 # 800080d0 <digits+0x90>
    80000ffe:	fffff097          	auipc	ra,0xfffff
    80001002:	540080e7          	jalr	1344(ra) # 8000053e <panic>
    pte_t *pte = &pagetable[PX(level, va)];
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0)
    80001006:	060a8663          	beqz	s5,80001072 <walk+0xa2>
    8000100a:	00000097          	auipc	ra,0x0
    8000100e:	aea080e7          	jalr	-1302(ra) # 80000af4 <kalloc>
    80001012:	84aa                	mv	s1,a0
    80001014:	c529                	beqz	a0,8000105e <walk+0x8e>
        return 0;
      memset(pagetable, 0, PGSIZE);
    80001016:	6605                	lui	a2,0x1
    80001018:	4581                	li	a1,0
    8000101a:	00000097          	auipc	ra,0x0
    8000101e:	cc6080e7          	jalr	-826(ra) # 80000ce0 <memset>
      *pte = PA2PTE(pagetable) | PTE_V;
    80001022:	00c4d793          	srli	a5,s1,0xc
    80001026:	07aa                	slli	a5,a5,0xa
    80001028:	0017e793          	ori	a5,a5,1
    8000102c:	00f93023          	sd	a5,0(s2)
  for(int level = 2; level > 0; level--) {
    80001030:	3a5d                	addiw	s4,s4,-9
    80001032:	036a0063          	beq	s4,s6,80001052 <walk+0x82>
    pte_t *pte = &pagetable[PX(level, va)];
    80001036:	0149d933          	srl	s2,s3,s4
    8000103a:	1ff97913          	andi	s2,s2,511
    8000103e:	090e                	slli	s2,s2,0x3
    80001040:	9926                	add	s2,s2,s1
    if(*pte & PTE_V) {
    80001042:	00093483          	ld	s1,0(s2)
    80001046:	0014f793          	andi	a5,s1,1
    8000104a:	dfd5                	beqz	a5,80001006 <walk+0x36>
      pagetable = (pagetable_t)PTE2PA(*pte);
    8000104c:	80a9                	srli	s1,s1,0xa
    8000104e:	04b2                	slli	s1,s1,0xc
    80001050:	b7c5                	j	80001030 <walk+0x60>
    }
  }
  return &pagetable[PX(0, va)];
    80001052:	00c9d513          	srli	a0,s3,0xc
    80001056:	1ff57513          	andi	a0,a0,511
    8000105a:	050e                	slli	a0,a0,0x3
    8000105c:	9526                	add	a0,a0,s1
}
    8000105e:	70e2                	ld	ra,56(sp)
    80001060:	7442                	ld	s0,48(sp)
    80001062:	74a2                	ld	s1,40(sp)
    80001064:	7902                	ld	s2,32(sp)
    80001066:	69e2                	ld	s3,24(sp)
    80001068:	6a42                	ld	s4,16(sp)
    8000106a:	6aa2                	ld	s5,8(sp)
    8000106c:	6b02                	ld	s6,0(sp)
    8000106e:	6121                	addi	sp,sp,64
    80001070:	8082                	ret
        return 0;
    80001072:	4501                	li	a0,0
    80001074:	b7ed                	j	8000105e <walk+0x8e>

0000000080001076 <walkaddr>:
walkaddr(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;

  if(va >= MAXVA)
    80001076:	57fd                	li	a5,-1
    80001078:	83e9                	srli	a5,a5,0x1a
    8000107a:	00b7f463          	bgeu	a5,a1,80001082 <walkaddr+0xc>
    return 0;
    8000107e:	4501                	li	a0,0
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  pa = PTE2PA(*pte);
  return pa;
}
    80001080:	8082                	ret
{
    80001082:	1141                	addi	sp,sp,-16
    80001084:	e406                	sd	ra,8(sp)
    80001086:	e022                	sd	s0,0(sp)
    80001088:	0800                	addi	s0,sp,16
  pte = walk(pagetable, va, 0);
    8000108a:	4601                	li	a2,0
    8000108c:	00000097          	auipc	ra,0x0
    80001090:	f44080e7          	jalr	-188(ra) # 80000fd0 <walk>
  if(pte == 0)
    80001094:	c105                	beqz	a0,800010b4 <walkaddr+0x3e>
  if((*pte & PTE_V) == 0)
    80001096:	611c                	ld	a5,0(a0)
  if((*pte & PTE_U) == 0)
    80001098:	0117f693          	andi	a3,a5,17
    8000109c:	4745                	li	a4,17
    return 0;
    8000109e:	4501                	li	a0,0
  if((*pte & PTE_U) == 0)
    800010a0:	00e68663          	beq	a3,a4,800010ac <walkaddr+0x36>
}
    800010a4:	60a2                	ld	ra,8(sp)
    800010a6:	6402                	ld	s0,0(sp)
    800010a8:	0141                	addi	sp,sp,16
    800010aa:	8082                	ret
  pa = PTE2PA(*pte);
    800010ac:	00a7d513          	srli	a0,a5,0xa
    800010b0:	0532                	slli	a0,a0,0xc
  return pa;
    800010b2:	bfcd                	j	800010a4 <walkaddr+0x2e>
    return 0;
    800010b4:	4501                	li	a0,0
    800010b6:	b7fd                	j	800010a4 <walkaddr+0x2e>

00000000800010b8 <mappages>:
// physical addresses starting at pa. va and size might not
// be page-aligned. Returns 0 on success, -1 if walk() couldn't
// allocate a needed page-table page.
int
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
    800010b8:	715d                	addi	sp,sp,-80
    800010ba:	e486                	sd	ra,72(sp)
    800010bc:	e0a2                	sd	s0,64(sp)
    800010be:	fc26                	sd	s1,56(sp)
    800010c0:	f84a                	sd	s2,48(sp)
    800010c2:	f44e                	sd	s3,40(sp)
    800010c4:	f052                	sd	s4,32(sp)
    800010c6:	ec56                	sd	s5,24(sp)
    800010c8:	e85a                	sd	s6,16(sp)
    800010ca:	e45e                	sd	s7,8(sp)
    800010cc:	0880                	addi	s0,sp,80
  uint64 a, last;
  pte_t *pte;

  if(size == 0)
    800010ce:	c205                	beqz	a2,800010ee <mappages+0x36>
    800010d0:	8aaa                	mv	s5,a0
    800010d2:	8b3a                	mv	s6,a4
    panic("mappages: size");
  
  a = PGROUNDDOWN(va);
    800010d4:	77fd                	lui	a5,0xfffff
    800010d6:	00f5fa33          	and	s4,a1,a5
  last = PGROUNDDOWN(va + size - 1);
    800010da:	15fd                	addi	a1,a1,-1
    800010dc:	00c589b3          	add	s3,a1,a2
    800010e0:	00f9f9b3          	and	s3,s3,a5
  a = PGROUNDDOWN(va);
    800010e4:	8952                	mv	s2,s4
    800010e6:	41468a33          	sub	s4,a3,s4
    if(*pte & PTE_V)
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V;
    if(a == last)
      break;
    a += PGSIZE;
    800010ea:	6b85                	lui	s7,0x1
    800010ec:	a015                	j	80001110 <mappages+0x58>
    panic("mappages: size");
    800010ee:	00007517          	auipc	a0,0x7
    800010f2:	fea50513          	addi	a0,a0,-22 # 800080d8 <digits+0x98>
    800010f6:	fffff097          	auipc	ra,0xfffff
    800010fa:	448080e7          	jalr	1096(ra) # 8000053e <panic>
      panic("mappages: remap");
    800010fe:	00007517          	auipc	a0,0x7
    80001102:	fea50513          	addi	a0,a0,-22 # 800080e8 <digits+0xa8>
    80001106:	fffff097          	auipc	ra,0xfffff
    8000110a:	438080e7          	jalr	1080(ra) # 8000053e <panic>
    a += PGSIZE;
    8000110e:	995e                	add	s2,s2,s7
  for(;;){
    80001110:	012a04b3          	add	s1,s4,s2
    if((pte = walk(pagetable, a, 1)) == 0)
    80001114:	4605                	li	a2,1
    80001116:	85ca                	mv	a1,s2
    80001118:	8556                	mv	a0,s5
    8000111a:	00000097          	auipc	ra,0x0
    8000111e:	eb6080e7          	jalr	-330(ra) # 80000fd0 <walk>
    80001122:	cd19                	beqz	a0,80001140 <mappages+0x88>
    if(*pte & PTE_V)
    80001124:	611c                	ld	a5,0(a0)
    80001126:	8b85                	andi	a5,a5,1
    80001128:	fbf9                	bnez	a5,800010fe <mappages+0x46>
    *pte = PA2PTE(pa) | perm | PTE_V;
    8000112a:	80b1                	srli	s1,s1,0xc
    8000112c:	04aa                	slli	s1,s1,0xa
    8000112e:	0164e4b3          	or	s1,s1,s6
    80001132:	0014e493          	ori	s1,s1,1
    80001136:	e104                	sd	s1,0(a0)
    if(a == last)
    80001138:	fd391be3          	bne	s2,s3,8000110e <mappages+0x56>
    pa += PGSIZE;
  }
  return 0;
    8000113c:	4501                	li	a0,0
    8000113e:	a011                	j	80001142 <mappages+0x8a>
      return -1;
    80001140:	557d                	li	a0,-1
}
    80001142:	60a6                	ld	ra,72(sp)
    80001144:	6406                	ld	s0,64(sp)
    80001146:	74e2                	ld	s1,56(sp)
    80001148:	7942                	ld	s2,48(sp)
    8000114a:	79a2                	ld	s3,40(sp)
    8000114c:	7a02                	ld	s4,32(sp)
    8000114e:	6ae2                	ld	s5,24(sp)
    80001150:	6b42                	ld	s6,16(sp)
    80001152:	6ba2                	ld	s7,8(sp)
    80001154:	6161                	addi	sp,sp,80
    80001156:	8082                	ret

0000000080001158 <kvmmap>:
{
    80001158:	1141                	addi	sp,sp,-16
    8000115a:	e406                	sd	ra,8(sp)
    8000115c:	e022                	sd	s0,0(sp)
    8000115e:	0800                	addi	s0,sp,16
    80001160:	87b6                	mv	a5,a3
  if(mappages(kpgtbl, va, sz, pa, perm) != 0)
    80001162:	86b2                	mv	a3,a2
    80001164:	863e                	mv	a2,a5
    80001166:	00000097          	auipc	ra,0x0
    8000116a:	f52080e7          	jalr	-174(ra) # 800010b8 <mappages>
    8000116e:	e509                	bnez	a0,80001178 <kvmmap+0x20>
}
    80001170:	60a2                	ld	ra,8(sp)
    80001172:	6402                	ld	s0,0(sp)
    80001174:	0141                	addi	sp,sp,16
    80001176:	8082                	ret
    panic("kvmmap");
    80001178:	00007517          	auipc	a0,0x7
    8000117c:	f8050513          	addi	a0,a0,-128 # 800080f8 <digits+0xb8>
    80001180:	fffff097          	auipc	ra,0xfffff
    80001184:	3be080e7          	jalr	958(ra) # 8000053e <panic>

0000000080001188 <kvmmake>:
{
    80001188:	1101                	addi	sp,sp,-32
    8000118a:	ec06                	sd	ra,24(sp)
    8000118c:	e822                	sd	s0,16(sp)
    8000118e:	e426                	sd	s1,8(sp)
    80001190:	e04a                	sd	s2,0(sp)
    80001192:	1000                	addi	s0,sp,32
  kpgtbl = (pagetable_t) kalloc();
    80001194:	00000097          	auipc	ra,0x0
    80001198:	960080e7          	jalr	-1696(ra) # 80000af4 <kalloc>
    8000119c:	84aa                	mv	s1,a0
  memset(kpgtbl, 0, PGSIZE);
    8000119e:	6605                	lui	a2,0x1
    800011a0:	4581                	li	a1,0
    800011a2:	00000097          	auipc	ra,0x0
    800011a6:	b3e080e7          	jalr	-1218(ra) # 80000ce0 <memset>
  kvmmap(kpgtbl, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    800011aa:	4719                	li	a4,6
    800011ac:	6685                	lui	a3,0x1
    800011ae:	10000637          	lui	a2,0x10000
    800011b2:	100005b7          	lui	a1,0x10000
    800011b6:	8526                	mv	a0,s1
    800011b8:	00000097          	auipc	ra,0x0
    800011bc:	fa0080e7          	jalr	-96(ra) # 80001158 <kvmmap>
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    800011c0:	4719                	li	a4,6
    800011c2:	6685                	lui	a3,0x1
    800011c4:	10001637          	lui	a2,0x10001
    800011c8:	100015b7          	lui	a1,0x10001
    800011cc:	8526                	mv	a0,s1
    800011ce:	00000097          	auipc	ra,0x0
    800011d2:	f8a080e7          	jalr	-118(ra) # 80001158 <kvmmap>
  kvmmap(kpgtbl, PLIC, PLIC, 0x400000, PTE_R | PTE_W);
    800011d6:	4719                	li	a4,6
    800011d8:	004006b7          	lui	a3,0x400
    800011dc:	0c000637          	lui	a2,0xc000
    800011e0:	0c0005b7          	lui	a1,0xc000
    800011e4:	8526                	mv	a0,s1
    800011e6:	00000097          	auipc	ra,0x0
    800011ea:	f72080e7          	jalr	-142(ra) # 80001158 <kvmmap>
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
    800011ee:	00007917          	auipc	s2,0x7
    800011f2:	e1290913          	addi	s2,s2,-494 # 80008000 <etext>
    800011f6:	4729                	li	a4,10
    800011f8:	80007697          	auipc	a3,0x80007
    800011fc:	e0868693          	addi	a3,a3,-504 # 8000 <_entry-0x7fff8000>
    80001200:	4605                	li	a2,1
    80001202:	067e                	slli	a2,a2,0x1f
    80001204:	85b2                	mv	a1,a2
    80001206:	8526                	mv	a0,s1
    80001208:	00000097          	auipc	ra,0x0
    8000120c:	f50080e7          	jalr	-176(ra) # 80001158 <kvmmap>
  kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
    80001210:	4719                	li	a4,6
    80001212:	46c5                	li	a3,17
    80001214:	06ee                	slli	a3,a3,0x1b
    80001216:	412686b3          	sub	a3,a3,s2
    8000121a:	864a                	mv	a2,s2
    8000121c:	85ca                	mv	a1,s2
    8000121e:	8526                	mv	a0,s1
    80001220:	00000097          	auipc	ra,0x0
    80001224:	f38080e7          	jalr	-200(ra) # 80001158 <kvmmap>
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    80001228:	4729                	li	a4,10
    8000122a:	6685                	lui	a3,0x1
    8000122c:	00006617          	auipc	a2,0x6
    80001230:	dd460613          	addi	a2,a2,-556 # 80007000 <_trampoline>
    80001234:	040005b7          	lui	a1,0x4000
    80001238:	15fd                	addi	a1,a1,-1
    8000123a:	05b2                	slli	a1,a1,0xc
    8000123c:	8526                	mv	a0,s1
    8000123e:	00000097          	auipc	ra,0x0
    80001242:	f1a080e7          	jalr	-230(ra) # 80001158 <kvmmap>
  proc_mapstacks(kpgtbl);
    80001246:	8526                	mv	a0,s1
    80001248:	00000097          	auipc	ra,0x0
    8000124c:	614080e7          	jalr	1556(ra) # 8000185c <proc_mapstacks>
}
    80001250:	8526                	mv	a0,s1
    80001252:	60e2                	ld	ra,24(sp)
    80001254:	6442                	ld	s0,16(sp)
    80001256:	64a2                	ld	s1,8(sp)
    80001258:	6902                	ld	s2,0(sp)
    8000125a:	6105                	addi	sp,sp,32
    8000125c:	8082                	ret

000000008000125e <kvminit>:
{
    8000125e:	1141                	addi	sp,sp,-16
    80001260:	e406                	sd	ra,8(sp)
    80001262:	e022                	sd	s0,0(sp)
    80001264:	0800                	addi	s0,sp,16
  kernel_pagetable = kvmmake();
    80001266:	00000097          	auipc	ra,0x0
    8000126a:	f22080e7          	jalr	-222(ra) # 80001188 <kvmmake>
    8000126e:	00008797          	auipc	a5,0x8
    80001272:	daa7b923          	sd	a0,-590(a5) # 80009020 <kernel_pagetable>
}
    80001276:	60a2                	ld	ra,8(sp)
    80001278:	6402                	ld	s0,0(sp)
    8000127a:	0141                	addi	sp,sp,16
    8000127c:	8082                	ret

000000008000127e <uvmunmap>:
// Remove npages of mappings starting from va. va must be
// page-aligned. The mappings must exist.
// Optionally free the physical memory.
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
    8000127e:	715d                	addi	sp,sp,-80
    80001280:	e486                	sd	ra,72(sp)
    80001282:	e0a2                	sd	s0,64(sp)
    80001284:	fc26                	sd	s1,56(sp)
    80001286:	f84a                	sd	s2,48(sp)
    80001288:	f44e                	sd	s3,40(sp)
    8000128a:	f052                	sd	s4,32(sp)
    8000128c:	ec56                	sd	s5,24(sp)
    8000128e:	e85a                	sd	s6,16(sp)
    80001290:	e45e                	sd	s7,8(sp)
    80001292:	0880                	addi	s0,sp,80
  uint64 a;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    80001294:	03459793          	slli	a5,a1,0x34
    80001298:	e795                	bnez	a5,800012c4 <uvmunmap+0x46>
    8000129a:	8a2a                	mv	s4,a0
    8000129c:	892e                	mv	s2,a1
    8000129e:	8ab6                	mv	s5,a3
    panic("uvmunmap: not aligned");

  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800012a0:	0632                	slli	a2,a2,0xc
    800012a2:	00b609b3          	add	s3,a2,a1
    if((pte = walk(pagetable, a, 0)) == 0)
      panic("uvmunmap: walk");
    if((*pte & PTE_V) == 0)
      panic("uvmunmap: not mapped");
    if(PTE_FLAGS(*pte) == PTE_V)
    800012a6:	4b85                	li	s7,1
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800012a8:	6b05                	lui	s6,0x1
    800012aa:	0735e863          	bltu	a1,s3,8000131a <uvmunmap+0x9c>
      uint64 pa = PTE2PA(*pte);
      kfree((void*)pa);
    }
    *pte = 0;
  }
}
    800012ae:	60a6                	ld	ra,72(sp)
    800012b0:	6406                	ld	s0,64(sp)
    800012b2:	74e2                	ld	s1,56(sp)
    800012b4:	7942                	ld	s2,48(sp)
    800012b6:	79a2                	ld	s3,40(sp)
    800012b8:	7a02                	ld	s4,32(sp)
    800012ba:	6ae2                	ld	s5,24(sp)
    800012bc:	6b42                	ld	s6,16(sp)
    800012be:	6ba2                	ld	s7,8(sp)
    800012c0:	6161                	addi	sp,sp,80
    800012c2:	8082                	ret
    panic("uvmunmap: not aligned");
    800012c4:	00007517          	auipc	a0,0x7
    800012c8:	e3c50513          	addi	a0,a0,-452 # 80008100 <digits+0xc0>
    800012cc:	fffff097          	auipc	ra,0xfffff
    800012d0:	272080e7          	jalr	626(ra) # 8000053e <panic>
      panic("uvmunmap: walk");
    800012d4:	00007517          	auipc	a0,0x7
    800012d8:	e4450513          	addi	a0,a0,-444 # 80008118 <digits+0xd8>
    800012dc:	fffff097          	auipc	ra,0xfffff
    800012e0:	262080e7          	jalr	610(ra) # 8000053e <panic>
      panic("uvmunmap: not mapped");
    800012e4:	00007517          	auipc	a0,0x7
    800012e8:	e4450513          	addi	a0,a0,-444 # 80008128 <digits+0xe8>
    800012ec:	fffff097          	auipc	ra,0xfffff
    800012f0:	252080e7          	jalr	594(ra) # 8000053e <panic>
      panic("uvmunmap: not a leaf");
    800012f4:	00007517          	auipc	a0,0x7
    800012f8:	e4c50513          	addi	a0,a0,-436 # 80008140 <digits+0x100>
    800012fc:	fffff097          	auipc	ra,0xfffff
    80001300:	242080e7          	jalr	578(ra) # 8000053e <panic>
      uint64 pa = PTE2PA(*pte);
    80001304:	8129                	srli	a0,a0,0xa
      kfree((void*)pa);
    80001306:	0532                	slli	a0,a0,0xc
    80001308:	fffff097          	auipc	ra,0xfffff
    8000130c:	6f0080e7          	jalr	1776(ra) # 800009f8 <kfree>
    *pte = 0;
    80001310:	0004b023          	sd	zero,0(s1)
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    80001314:	995a                	add	s2,s2,s6
    80001316:	f9397ce3          	bgeu	s2,s3,800012ae <uvmunmap+0x30>
    if((pte = walk(pagetable, a, 0)) == 0)
    8000131a:	4601                	li	a2,0
    8000131c:	85ca                	mv	a1,s2
    8000131e:	8552                	mv	a0,s4
    80001320:	00000097          	auipc	ra,0x0
    80001324:	cb0080e7          	jalr	-848(ra) # 80000fd0 <walk>
    80001328:	84aa                	mv	s1,a0
    8000132a:	d54d                	beqz	a0,800012d4 <uvmunmap+0x56>
    if((*pte & PTE_V) == 0)
    8000132c:	6108                	ld	a0,0(a0)
    8000132e:	00157793          	andi	a5,a0,1
    80001332:	dbcd                	beqz	a5,800012e4 <uvmunmap+0x66>
    if(PTE_FLAGS(*pte) == PTE_V)
    80001334:	3ff57793          	andi	a5,a0,1023
    80001338:	fb778ee3          	beq	a5,s7,800012f4 <uvmunmap+0x76>
    if(do_free){
    8000133c:	fc0a8ae3          	beqz	s5,80001310 <uvmunmap+0x92>
    80001340:	b7d1                	j	80001304 <uvmunmap+0x86>

0000000080001342 <uvmcreate>:

// create an empty user page table.
// returns 0 if out of memory.
pagetable_t
uvmcreate()
{
    80001342:	1101                	addi	sp,sp,-32
    80001344:	ec06                	sd	ra,24(sp)
    80001346:	e822                	sd	s0,16(sp)
    80001348:	e426                	sd	s1,8(sp)
    8000134a:	1000                	addi	s0,sp,32
  pagetable_t pagetable;
  pagetable = (pagetable_t) kalloc();
    8000134c:	fffff097          	auipc	ra,0xfffff
    80001350:	7a8080e7          	jalr	1960(ra) # 80000af4 <kalloc>
    80001354:	84aa                	mv	s1,a0
  if(pagetable == 0)
    80001356:	c519                	beqz	a0,80001364 <uvmcreate+0x22>
    return 0;
  memset(pagetable, 0, PGSIZE);
    80001358:	6605                	lui	a2,0x1
    8000135a:	4581                	li	a1,0
    8000135c:	00000097          	auipc	ra,0x0
    80001360:	984080e7          	jalr	-1660(ra) # 80000ce0 <memset>
  return pagetable;
}
    80001364:	8526                	mv	a0,s1
    80001366:	60e2                	ld	ra,24(sp)
    80001368:	6442                	ld	s0,16(sp)
    8000136a:	64a2                	ld	s1,8(sp)
    8000136c:	6105                	addi	sp,sp,32
    8000136e:	8082                	ret

0000000080001370 <uvminit>:
// Load the user initcode into address 0 of pagetable,
// for the very first process.
// sz must be less than a page.
void
uvminit(pagetable_t pagetable, uchar *src, uint sz)
{
    80001370:	7179                	addi	sp,sp,-48
    80001372:	f406                	sd	ra,40(sp)
    80001374:	f022                	sd	s0,32(sp)
    80001376:	ec26                	sd	s1,24(sp)
    80001378:	e84a                	sd	s2,16(sp)
    8000137a:	e44e                	sd	s3,8(sp)
    8000137c:	e052                	sd	s4,0(sp)
    8000137e:	1800                	addi	s0,sp,48
  char *mem;

  if(sz >= PGSIZE)
    80001380:	6785                	lui	a5,0x1
    80001382:	04f67863          	bgeu	a2,a5,800013d2 <uvminit+0x62>
    80001386:	8a2a                	mv	s4,a0
    80001388:	89ae                	mv	s3,a1
    8000138a:	84b2                	mv	s1,a2
    panic("inituvm: more than a page");
  mem = kalloc();
    8000138c:	fffff097          	auipc	ra,0xfffff
    80001390:	768080e7          	jalr	1896(ra) # 80000af4 <kalloc>
    80001394:	892a                	mv	s2,a0
  memset(mem, 0, PGSIZE);
    80001396:	6605                	lui	a2,0x1
    80001398:	4581                	li	a1,0
    8000139a:	00000097          	auipc	ra,0x0
    8000139e:	946080e7          	jalr	-1722(ra) # 80000ce0 <memset>
  mappages(pagetable, 0, PGSIZE, (uint64)mem, PTE_W|PTE_R|PTE_X|PTE_U);
    800013a2:	4779                	li	a4,30
    800013a4:	86ca                	mv	a3,s2
    800013a6:	6605                	lui	a2,0x1
    800013a8:	4581                	li	a1,0
    800013aa:	8552                	mv	a0,s4
    800013ac:	00000097          	auipc	ra,0x0
    800013b0:	d0c080e7          	jalr	-756(ra) # 800010b8 <mappages>
  memmove(mem, src, sz);
    800013b4:	8626                	mv	a2,s1
    800013b6:	85ce                	mv	a1,s3
    800013b8:	854a                	mv	a0,s2
    800013ba:	00000097          	auipc	ra,0x0
    800013be:	986080e7          	jalr	-1658(ra) # 80000d40 <memmove>
}
    800013c2:	70a2                	ld	ra,40(sp)
    800013c4:	7402                	ld	s0,32(sp)
    800013c6:	64e2                	ld	s1,24(sp)
    800013c8:	6942                	ld	s2,16(sp)
    800013ca:	69a2                	ld	s3,8(sp)
    800013cc:	6a02                	ld	s4,0(sp)
    800013ce:	6145                	addi	sp,sp,48
    800013d0:	8082                	ret
    panic("inituvm: more than a page");
    800013d2:	00007517          	auipc	a0,0x7
    800013d6:	d8650513          	addi	a0,a0,-634 # 80008158 <digits+0x118>
    800013da:	fffff097          	auipc	ra,0xfffff
    800013de:	164080e7          	jalr	356(ra) # 8000053e <panic>

00000000800013e2 <uvmdealloc>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    800013e2:	1101                	addi	sp,sp,-32
    800013e4:	ec06                	sd	ra,24(sp)
    800013e6:	e822                	sd	s0,16(sp)
    800013e8:	e426                	sd	s1,8(sp)
    800013ea:	1000                	addi	s0,sp,32
  if(newsz >= oldsz)
    return oldsz;
    800013ec:	84ae                	mv	s1,a1
  if(newsz >= oldsz)
    800013ee:	00b67d63          	bgeu	a2,a1,80001408 <uvmdealloc+0x26>
    800013f2:	84b2                	mv	s1,a2

  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    800013f4:	6785                	lui	a5,0x1
    800013f6:	17fd                	addi	a5,a5,-1
    800013f8:	00f60733          	add	a4,a2,a5
    800013fc:	767d                	lui	a2,0xfffff
    800013fe:	8f71                	and	a4,a4,a2
    80001400:	97ae                	add	a5,a5,a1
    80001402:	8ff1                	and	a5,a5,a2
    80001404:	00f76863          	bltu	a4,a5,80001414 <uvmdealloc+0x32>
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
  }

  return newsz;
}
    80001408:	8526                	mv	a0,s1
    8000140a:	60e2                	ld	ra,24(sp)
    8000140c:	6442                	ld	s0,16(sp)
    8000140e:	64a2                	ld	s1,8(sp)
    80001410:	6105                	addi	sp,sp,32
    80001412:	8082                	ret
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    80001414:	8f99                	sub	a5,a5,a4
    80001416:	83b1                	srli	a5,a5,0xc
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
    80001418:	4685                	li	a3,1
    8000141a:	0007861b          	sext.w	a2,a5
    8000141e:	85ba                	mv	a1,a4
    80001420:	00000097          	auipc	ra,0x0
    80001424:	e5e080e7          	jalr	-418(ra) # 8000127e <uvmunmap>
    80001428:	b7c5                	j	80001408 <uvmdealloc+0x26>

000000008000142a <uvmalloc>:
  if(newsz < oldsz)
    8000142a:	0ab66163          	bltu	a2,a1,800014cc <uvmalloc+0xa2>
{
    8000142e:	7139                	addi	sp,sp,-64
    80001430:	fc06                	sd	ra,56(sp)
    80001432:	f822                	sd	s0,48(sp)
    80001434:	f426                	sd	s1,40(sp)
    80001436:	f04a                	sd	s2,32(sp)
    80001438:	ec4e                	sd	s3,24(sp)
    8000143a:	e852                	sd	s4,16(sp)
    8000143c:	e456                	sd	s5,8(sp)
    8000143e:	0080                	addi	s0,sp,64
    80001440:	8aaa                	mv	s5,a0
    80001442:	8a32                	mv	s4,a2
  oldsz = PGROUNDUP(oldsz);
    80001444:	6985                	lui	s3,0x1
    80001446:	19fd                	addi	s3,s3,-1
    80001448:	95ce                	add	a1,a1,s3
    8000144a:	79fd                	lui	s3,0xfffff
    8000144c:	0135f9b3          	and	s3,a1,s3
  for(a = oldsz; a < newsz; a += PGSIZE){
    80001450:	08c9f063          	bgeu	s3,a2,800014d0 <uvmalloc+0xa6>
    80001454:	894e                	mv	s2,s3
    mem = kalloc();
    80001456:	fffff097          	auipc	ra,0xfffff
    8000145a:	69e080e7          	jalr	1694(ra) # 80000af4 <kalloc>
    8000145e:	84aa                	mv	s1,a0
    if(mem == 0){
    80001460:	c51d                	beqz	a0,8000148e <uvmalloc+0x64>
    memset(mem, 0, PGSIZE);
    80001462:	6605                	lui	a2,0x1
    80001464:	4581                	li	a1,0
    80001466:	00000097          	auipc	ra,0x0
    8000146a:	87a080e7          	jalr	-1926(ra) # 80000ce0 <memset>
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_W|PTE_X|PTE_R|PTE_U) != 0){
    8000146e:	4779                	li	a4,30
    80001470:	86a6                	mv	a3,s1
    80001472:	6605                	lui	a2,0x1
    80001474:	85ca                	mv	a1,s2
    80001476:	8556                	mv	a0,s5
    80001478:	00000097          	auipc	ra,0x0
    8000147c:	c40080e7          	jalr	-960(ra) # 800010b8 <mappages>
    80001480:	e905                	bnez	a0,800014b0 <uvmalloc+0x86>
  for(a = oldsz; a < newsz; a += PGSIZE){
    80001482:	6785                	lui	a5,0x1
    80001484:	993e                	add	s2,s2,a5
    80001486:	fd4968e3          	bltu	s2,s4,80001456 <uvmalloc+0x2c>
  return newsz;
    8000148a:	8552                	mv	a0,s4
    8000148c:	a809                	j	8000149e <uvmalloc+0x74>
      uvmdealloc(pagetable, a, oldsz);
    8000148e:	864e                	mv	a2,s3
    80001490:	85ca                	mv	a1,s2
    80001492:	8556                	mv	a0,s5
    80001494:	00000097          	auipc	ra,0x0
    80001498:	f4e080e7          	jalr	-178(ra) # 800013e2 <uvmdealloc>
      return 0;
    8000149c:	4501                	li	a0,0
}
    8000149e:	70e2                	ld	ra,56(sp)
    800014a0:	7442                	ld	s0,48(sp)
    800014a2:	74a2                	ld	s1,40(sp)
    800014a4:	7902                	ld	s2,32(sp)
    800014a6:	69e2                	ld	s3,24(sp)
    800014a8:	6a42                	ld	s4,16(sp)
    800014aa:	6aa2                	ld	s5,8(sp)
    800014ac:	6121                	addi	sp,sp,64
    800014ae:	8082                	ret
      kfree(mem);
    800014b0:	8526                	mv	a0,s1
    800014b2:	fffff097          	auipc	ra,0xfffff
    800014b6:	546080e7          	jalr	1350(ra) # 800009f8 <kfree>
      uvmdealloc(pagetable, a, oldsz);
    800014ba:	864e                	mv	a2,s3
    800014bc:	85ca                	mv	a1,s2
    800014be:	8556                	mv	a0,s5
    800014c0:	00000097          	auipc	ra,0x0
    800014c4:	f22080e7          	jalr	-222(ra) # 800013e2 <uvmdealloc>
      return 0;
    800014c8:	4501                	li	a0,0
    800014ca:	bfd1                	j	8000149e <uvmalloc+0x74>
    return oldsz;
    800014cc:	852e                	mv	a0,a1
}
    800014ce:	8082                	ret
  return newsz;
    800014d0:	8532                	mv	a0,a2
    800014d2:	b7f1                	j	8000149e <uvmalloc+0x74>

00000000800014d4 <freewalk>:

// Recursively free page-table pages.
// All leaf mappings must already have been removed.
void
freewalk(pagetable_t pagetable)
{
    800014d4:	7179                	addi	sp,sp,-48
    800014d6:	f406                	sd	ra,40(sp)
    800014d8:	f022                	sd	s0,32(sp)
    800014da:	ec26                	sd	s1,24(sp)
    800014dc:	e84a                	sd	s2,16(sp)
    800014de:	e44e                	sd	s3,8(sp)
    800014e0:	e052                	sd	s4,0(sp)
    800014e2:	1800                	addi	s0,sp,48
    800014e4:	8a2a                	mv	s4,a0
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    800014e6:	84aa                	mv	s1,a0
    800014e8:	6905                	lui	s2,0x1
    800014ea:	992a                	add	s2,s2,a0
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    800014ec:	4985                	li	s3,1
    800014ee:	a821                	j	80001506 <freewalk+0x32>
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
    800014f0:	8129                	srli	a0,a0,0xa
      freewalk((pagetable_t)child);
    800014f2:	0532                	slli	a0,a0,0xc
    800014f4:	00000097          	auipc	ra,0x0
    800014f8:	fe0080e7          	jalr	-32(ra) # 800014d4 <freewalk>
      pagetable[i] = 0;
    800014fc:	0004b023          	sd	zero,0(s1)
  for(int i = 0; i < 512; i++){
    80001500:	04a1                	addi	s1,s1,8
    80001502:	03248163          	beq	s1,s2,80001524 <freewalk+0x50>
    pte_t pte = pagetable[i];
    80001506:	6088                	ld	a0,0(s1)
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001508:	00f57793          	andi	a5,a0,15
    8000150c:	ff3782e3          	beq	a5,s3,800014f0 <freewalk+0x1c>
    } else if(pte & PTE_V){
    80001510:	8905                	andi	a0,a0,1
    80001512:	d57d                	beqz	a0,80001500 <freewalk+0x2c>
      panic("freewalk: leaf");
    80001514:	00007517          	auipc	a0,0x7
    80001518:	c6450513          	addi	a0,a0,-924 # 80008178 <digits+0x138>
    8000151c:	fffff097          	auipc	ra,0xfffff
    80001520:	022080e7          	jalr	34(ra) # 8000053e <panic>
    }
  }
  kfree((void*)pagetable);
    80001524:	8552                	mv	a0,s4
    80001526:	fffff097          	auipc	ra,0xfffff
    8000152a:	4d2080e7          	jalr	1234(ra) # 800009f8 <kfree>
}
    8000152e:	70a2                	ld	ra,40(sp)
    80001530:	7402                	ld	s0,32(sp)
    80001532:	64e2                	ld	s1,24(sp)
    80001534:	6942                	ld	s2,16(sp)
    80001536:	69a2                	ld	s3,8(sp)
    80001538:	6a02                	ld	s4,0(sp)
    8000153a:	6145                	addi	sp,sp,48
    8000153c:	8082                	ret

000000008000153e <uvmfree>:

// Free user memory pages,
// then free page-table pages.
void
uvmfree(pagetable_t pagetable, uint64 sz)
{
    8000153e:	1101                	addi	sp,sp,-32
    80001540:	ec06                	sd	ra,24(sp)
    80001542:	e822                	sd	s0,16(sp)
    80001544:	e426                	sd	s1,8(sp)
    80001546:	1000                	addi	s0,sp,32
    80001548:	84aa                	mv	s1,a0
  if(sz > 0)
    8000154a:	e999                	bnez	a1,80001560 <uvmfree+0x22>
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  freewalk(pagetable);
    8000154c:	8526                	mv	a0,s1
    8000154e:	00000097          	auipc	ra,0x0
    80001552:	f86080e7          	jalr	-122(ra) # 800014d4 <freewalk>
}
    80001556:	60e2                	ld	ra,24(sp)
    80001558:	6442                	ld	s0,16(sp)
    8000155a:	64a2                	ld	s1,8(sp)
    8000155c:	6105                	addi	sp,sp,32
    8000155e:	8082                	ret
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
    80001560:	6605                	lui	a2,0x1
    80001562:	167d                	addi	a2,a2,-1
    80001564:	962e                	add	a2,a2,a1
    80001566:	4685                	li	a3,1
    80001568:	8231                	srli	a2,a2,0xc
    8000156a:	4581                	li	a1,0
    8000156c:	00000097          	auipc	ra,0x0
    80001570:	d12080e7          	jalr	-750(ra) # 8000127e <uvmunmap>
    80001574:	bfe1                	j	8000154c <uvmfree+0xe>

0000000080001576 <uvmcopy>:
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;

  for(i = 0; i < sz; i += PGSIZE){
    80001576:	c679                	beqz	a2,80001644 <uvmcopy+0xce>
{
    80001578:	715d                	addi	sp,sp,-80
    8000157a:	e486                	sd	ra,72(sp)
    8000157c:	e0a2                	sd	s0,64(sp)
    8000157e:	fc26                	sd	s1,56(sp)
    80001580:	f84a                	sd	s2,48(sp)
    80001582:	f44e                	sd	s3,40(sp)
    80001584:	f052                	sd	s4,32(sp)
    80001586:	ec56                	sd	s5,24(sp)
    80001588:	e85a                	sd	s6,16(sp)
    8000158a:	e45e                	sd	s7,8(sp)
    8000158c:	0880                	addi	s0,sp,80
    8000158e:	8b2a                	mv	s6,a0
    80001590:	8aae                	mv	s5,a1
    80001592:	8a32                	mv	s4,a2
  for(i = 0; i < sz; i += PGSIZE){
    80001594:	4981                	li	s3,0
    if((pte = walk(old, i, 0)) == 0)
    80001596:	4601                	li	a2,0
    80001598:	85ce                	mv	a1,s3
    8000159a:	855a                	mv	a0,s6
    8000159c:	00000097          	auipc	ra,0x0
    800015a0:	a34080e7          	jalr	-1484(ra) # 80000fd0 <walk>
    800015a4:	c531                	beqz	a0,800015f0 <uvmcopy+0x7a>
      panic("uvmcopy: pte should exist");
    if((*pte & PTE_V) == 0)
    800015a6:	6118                	ld	a4,0(a0)
    800015a8:	00177793          	andi	a5,a4,1
    800015ac:	cbb1                	beqz	a5,80001600 <uvmcopy+0x8a>
      panic("uvmcopy: page not present");
    pa = PTE2PA(*pte);
    800015ae:	00a75593          	srli	a1,a4,0xa
    800015b2:	00c59b93          	slli	s7,a1,0xc
    flags = PTE_FLAGS(*pte);
    800015b6:	3ff77493          	andi	s1,a4,1023
    if((mem = kalloc()) == 0)
    800015ba:	fffff097          	auipc	ra,0xfffff
    800015be:	53a080e7          	jalr	1338(ra) # 80000af4 <kalloc>
    800015c2:	892a                	mv	s2,a0
    800015c4:	c939                	beqz	a0,8000161a <uvmcopy+0xa4>
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    800015c6:	6605                	lui	a2,0x1
    800015c8:	85de                	mv	a1,s7
    800015ca:	fffff097          	auipc	ra,0xfffff
    800015ce:	776080e7          	jalr	1910(ra) # 80000d40 <memmove>
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
    800015d2:	8726                	mv	a4,s1
    800015d4:	86ca                	mv	a3,s2
    800015d6:	6605                	lui	a2,0x1
    800015d8:	85ce                	mv	a1,s3
    800015da:	8556                	mv	a0,s5
    800015dc:	00000097          	auipc	ra,0x0
    800015e0:	adc080e7          	jalr	-1316(ra) # 800010b8 <mappages>
    800015e4:	e515                	bnez	a0,80001610 <uvmcopy+0x9a>
  for(i = 0; i < sz; i += PGSIZE){
    800015e6:	6785                	lui	a5,0x1
    800015e8:	99be                	add	s3,s3,a5
    800015ea:	fb49e6e3          	bltu	s3,s4,80001596 <uvmcopy+0x20>
    800015ee:	a081                	j	8000162e <uvmcopy+0xb8>
      panic("uvmcopy: pte should exist");
    800015f0:	00007517          	auipc	a0,0x7
    800015f4:	b9850513          	addi	a0,a0,-1128 # 80008188 <digits+0x148>
    800015f8:	fffff097          	auipc	ra,0xfffff
    800015fc:	f46080e7          	jalr	-186(ra) # 8000053e <panic>
      panic("uvmcopy: page not present");
    80001600:	00007517          	auipc	a0,0x7
    80001604:	ba850513          	addi	a0,a0,-1112 # 800081a8 <digits+0x168>
    80001608:	fffff097          	auipc	ra,0xfffff
    8000160c:	f36080e7          	jalr	-202(ra) # 8000053e <panic>
      kfree(mem);
    80001610:	854a                	mv	a0,s2
    80001612:	fffff097          	auipc	ra,0xfffff
    80001616:	3e6080e7          	jalr	998(ra) # 800009f8 <kfree>
    }
  }
  return 0;

 err:
  uvmunmap(new, 0, i / PGSIZE, 1);
    8000161a:	4685                	li	a3,1
    8000161c:	00c9d613          	srli	a2,s3,0xc
    80001620:	4581                	li	a1,0
    80001622:	8556                	mv	a0,s5
    80001624:	00000097          	auipc	ra,0x0
    80001628:	c5a080e7          	jalr	-934(ra) # 8000127e <uvmunmap>
  return -1;
    8000162c:	557d                	li	a0,-1
}
    8000162e:	60a6                	ld	ra,72(sp)
    80001630:	6406                	ld	s0,64(sp)
    80001632:	74e2                	ld	s1,56(sp)
    80001634:	7942                	ld	s2,48(sp)
    80001636:	79a2                	ld	s3,40(sp)
    80001638:	7a02                	ld	s4,32(sp)
    8000163a:	6ae2                	ld	s5,24(sp)
    8000163c:	6b42                	ld	s6,16(sp)
    8000163e:	6ba2                	ld	s7,8(sp)
    80001640:	6161                	addi	sp,sp,80
    80001642:	8082                	ret
  return 0;
    80001644:	4501                	li	a0,0
}
    80001646:	8082                	ret

0000000080001648 <uvmclear>:

// mark a PTE invalid for user access.
// used by exec for the user stack guard page.
void
uvmclear(pagetable_t pagetable, uint64 va)
{
    80001648:	1141                	addi	sp,sp,-16
    8000164a:	e406                	sd	ra,8(sp)
    8000164c:	e022                	sd	s0,0(sp)
    8000164e:	0800                	addi	s0,sp,16
  pte_t *pte;
  
  pte = walk(pagetable, va, 0);
    80001650:	4601                	li	a2,0
    80001652:	00000097          	auipc	ra,0x0
    80001656:	97e080e7          	jalr	-1666(ra) # 80000fd0 <walk>
  if(pte == 0)
    8000165a:	c901                	beqz	a0,8000166a <uvmclear+0x22>
    panic("uvmclear");
  *pte &= ~PTE_U;
    8000165c:	611c                	ld	a5,0(a0)
    8000165e:	9bbd                	andi	a5,a5,-17
    80001660:	e11c                	sd	a5,0(a0)
}
    80001662:	60a2                	ld	ra,8(sp)
    80001664:	6402                	ld	s0,0(sp)
    80001666:	0141                	addi	sp,sp,16
    80001668:	8082                	ret
    panic("uvmclear");
    8000166a:	00007517          	auipc	a0,0x7
    8000166e:	b5e50513          	addi	a0,a0,-1186 # 800081c8 <digits+0x188>
    80001672:	fffff097          	auipc	ra,0xfffff
    80001676:	ecc080e7          	jalr	-308(ra) # 8000053e <panic>

000000008000167a <copyout>:
int
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  uint64 n, va0, pa0;

  while(len > 0){
    8000167a:	c6bd                	beqz	a3,800016e8 <copyout+0x6e>
{
    8000167c:	715d                	addi	sp,sp,-80
    8000167e:	e486                	sd	ra,72(sp)
    80001680:	e0a2                	sd	s0,64(sp)
    80001682:	fc26                	sd	s1,56(sp)
    80001684:	f84a                	sd	s2,48(sp)
    80001686:	f44e                	sd	s3,40(sp)
    80001688:	f052                	sd	s4,32(sp)
    8000168a:	ec56                	sd	s5,24(sp)
    8000168c:	e85a                	sd	s6,16(sp)
    8000168e:	e45e                	sd	s7,8(sp)
    80001690:	e062                	sd	s8,0(sp)
    80001692:	0880                	addi	s0,sp,80
    80001694:	8b2a                	mv	s6,a0
    80001696:	8c2e                	mv	s8,a1
    80001698:	8a32                	mv	s4,a2
    8000169a:	89b6                	mv	s3,a3
    va0 = PGROUNDDOWN(dstva);
    8000169c:	7bfd                	lui	s7,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (dstva - va0);
    8000169e:	6a85                	lui	s5,0x1
    800016a0:	a015                	j	800016c4 <copyout+0x4a>
    if(n > len)
      n = len;
    memmove((void *)(pa0 + (dstva - va0)), src, n);
    800016a2:	9562                	add	a0,a0,s8
    800016a4:	0004861b          	sext.w	a2,s1
    800016a8:	85d2                	mv	a1,s4
    800016aa:	41250533          	sub	a0,a0,s2
    800016ae:	fffff097          	auipc	ra,0xfffff
    800016b2:	692080e7          	jalr	1682(ra) # 80000d40 <memmove>

    len -= n;
    800016b6:	409989b3          	sub	s3,s3,s1
    src += n;
    800016ba:	9a26                	add	s4,s4,s1
    dstva = va0 + PGSIZE;
    800016bc:	01590c33          	add	s8,s2,s5
  while(len > 0){
    800016c0:	02098263          	beqz	s3,800016e4 <copyout+0x6a>
    va0 = PGROUNDDOWN(dstva);
    800016c4:	017c7933          	and	s2,s8,s7
    pa0 = walkaddr(pagetable, va0);
    800016c8:	85ca                	mv	a1,s2
    800016ca:	855a                	mv	a0,s6
    800016cc:	00000097          	auipc	ra,0x0
    800016d0:	9aa080e7          	jalr	-1622(ra) # 80001076 <walkaddr>
    if(pa0 == 0)
    800016d4:	cd01                	beqz	a0,800016ec <copyout+0x72>
    n = PGSIZE - (dstva - va0);
    800016d6:	418904b3          	sub	s1,s2,s8
    800016da:	94d6                	add	s1,s1,s5
    if(n > len)
    800016dc:	fc99f3e3          	bgeu	s3,s1,800016a2 <copyout+0x28>
    800016e0:	84ce                	mv	s1,s3
    800016e2:	b7c1                	j	800016a2 <copyout+0x28>
  }
  return 0;
    800016e4:	4501                	li	a0,0
    800016e6:	a021                	j	800016ee <copyout+0x74>
    800016e8:	4501                	li	a0,0
}
    800016ea:	8082                	ret
      return -1;
    800016ec:	557d                	li	a0,-1
}
    800016ee:	60a6                	ld	ra,72(sp)
    800016f0:	6406                	ld	s0,64(sp)
    800016f2:	74e2                	ld	s1,56(sp)
    800016f4:	7942                	ld	s2,48(sp)
    800016f6:	79a2                	ld	s3,40(sp)
    800016f8:	7a02                	ld	s4,32(sp)
    800016fa:	6ae2                	ld	s5,24(sp)
    800016fc:	6b42                	ld	s6,16(sp)
    800016fe:	6ba2                	ld	s7,8(sp)
    80001700:	6c02                	ld	s8,0(sp)
    80001702:	6161                	addi	sp,sp,80
    80001704:	8082                	ret

0000000080001706 <copyin>:
int
copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len)
{
  uint64 n, va0, pa0;

  while(len > 0){
    80001706:	c6bd                	beqz	a3,80001774 <copyin+0x6e>
{
    80001708:	715d                	addi	sp,sp,-80
    8000170a:	e486                	sd	ra,72(sp)
    8000170c:	e0a2                	sd	s0,64(sp)
    8000170e:	fc26                	sd	s1,56(sp)
    80001710:	f84a                	sd	s2,48(sp)
    80001712:	f44e                	sd	s3,40(sp)
    80001714:	f052                	sd	s4,32(sp)
    80001716:	ec56                	sd	s5,24(sp)
    80001718:	e85a                	sd	s6,16(sp)
    8000171a:	e45e                	sd	s7,8(sp)
    8000171c:	e062                	sd	s8,0(sp)
    8000171e:	0880                	addi	s0,sp,80
    80001720:	8b2a                	mv	s6,a0
    80001722:	8a2e                	mv	s4,a1
    80001724:	8c32                	mv	s8,a2
    80001726:	89b6                	mv	s3,a3
    va0 = PGROUNDDOWN(srcva);
    80001728:	7bfd                	lui	s7,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    8000172a:	6a85                	lui	s5,0x1
    8000172c:	a015                	j	80001750 <copyin+0x4a>
    if(n > len)
      n = len;
    memmove(dst, (void *)(pa0 + (srcva - va0)), n);
    8000172e:	9562                	add	a0,a0,s8
    80001730:	0004861b          	sext.w	a2,s1
    80001734:	412505b3          	sub	a1,a0,s2
    80001738:	8552                	mv	a0,s4
    8000173a:	fffff097          	auipc	ra,0xfffff
    8000173e:	606080e7          	jalr	1542(ra) # 80000d40 <memmove>

    len -= n;
    80001742:	409989b3          	sub	s3,s3,s1
    dst += n;
    80001746:	9a26                	add	s4,s4,s1
    srcva = va0 + PGSIZE;
    80001748:	01590c33          	add	s8,s2,s5
  while(len > 0){
    8000174c:	02098263          	beqz	s3,80001770 <copyin+0x6a>
    va0 = PGROUNDDOWN(srcva);
    80001750:	017c7933          	and	s2,s8,s7
    pa0 = walkaddr(pagetable, va0);
    80001754:	85ca                	mv	a1,s2
    80001756:	855a                	mv	a0,s6
    80001758:	00000097          	auipc	ra,0x0
    8000175c:	91e080e7          	jalr	-1762(ra) # 80001076 <walkaddr>
    if(pa0 == 0)
    80001760:	cd01                	beqz	a0,80001778 <copyin+0x72>
    n = PGSIZE - (srcva - va0);
    80001762:	418904b3          	sub	s1,s2,s8
    80001766:	94d6                	add	s1,s1,s5
    if(n > len)
    80001768:	fc99f3e3          	bgeu	s3,s1,8000172e <copyin+0x28>
    8000176c:	84ce                	mv	s1,s3
    8000176e:	b7c1                	j	8000172e <copyin+0x28>
  }
  return 0;
    80001770:	4501                	li	a0,0
    80001772:	a021                	j	8000177a <copyin+0x74>
    80001774:	4501                	li	a0,0
}
    80001776:	8082                	ret
      return -1;
    80001778:	557d                	li	a0,-1
}
    8000177a:	60a6                	ld	ra,72(sp)
    8000177c:	6406                	ld	s0,64(sp)
    8000177e:	74e2                	ld	s1,56(sp)
    80001780:	7942                	ld	s2,48(sp)
    80001782:	79a2                	ld	s3,40(sp)
    80001784:	7a02                	ld	s4,32(sp)
    80001786:	6ae2                	ld	s5,24(sp)
    80001788:	6b42                	ld	s6,16(sp)
    8000178a:	6ba2                	ld	s7,8(sp)
    8000178c:	6c02                	ld	s8,0(sp)
    8000178e:	6161                	addi	sp,sp,80
    80001790:	8082                	ret

0000000080001792 <copyinstr>:
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  uint64 n, va0, pa0;
  int got_null = 0;

  while(got_null == 0 && max > 0){
    80001792:	c6c5                	beqz	a3,8000183a <copyinstr+0xa8>
{
    80001794:	715d                	addi	sp,sp,-80
    80001796:	e486                	sd	ra,72(sp)
    80001798:	e0a2                	sd	s0,64(sp)
    8000179a:	fc26                	sd	s1,56(sp)
    8000179c:	f84a                	sd	s2,48(sp)
    8000179e:	f44e                	sd	s3,40(sp)
    800017a0:	f052                	sd	s4,32(sp)
    800017a2:	ec56                	sd	s5,24(sp)
    800017a4:	e85a                	sd	s6,16(sp)
    800017a6:	e45e                	sd	s7,8(sp)
    800017a8:	0880                	addi	s0,sp,80
    800017aa:	8a2a                	mv	s4,a0
    800017ac:	8b2e                	mv	s6,a1
    800017ae:	8bb2                	mv	s7,a2
    800017b0:	84b6                	mv	s1,a3
    va0 = PGROUNDDOWN(srcva);
    800017b2:	7afd                	lui	s5,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    800017b4:	6985                	lui	s3,0x1
    800017b6:	a035                	j	800017e2 <copyinstr+0x50>
      n = max;

    char *p = (char *) (pa0 + (srcva - va0));
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
    800017b8:	00078023          	sb	zero,0(a5) # 1000 <_entry-0x7ffff000>
    800017bc:	4785                	li	a5,1
      dst++;
    }

    srcva = va0 + PGSIZE;
  }
  if(got_null){
    800017be:	0017b793          	seqz	a5,a5
    800017c2:	40f00533          	neg	a0,a5
    return 0;
  } else {
    return -1;
  }
}
    800017c6:	60a6                	ld	ra,72(sp)
    800017c8:	6406                	ld	s0,64(sp)
    800017ca:	74e2                	ld	s1,56(sp)
    800017cc:	7942                	ld	s2,48(sp)
    800017ce:	79a2                	ld	s3,40(sp)
    800017d0:	7a02                	ld	s4,32(sp)
    800017d2:	6ae2                	ld	s5,24(sp)
    800017d4:	6b42                	ld	s6,16(sp)
    800017d6:	6ba2                	ld	s7,8(sp)
    800017d8:	6161                	addi	sp,sp,80
    800017da:	8082                	ret
    srcva = va0 + PGSIZE;
    800017dc:	01390bb3          	add	s7,s2,s3
  while(got_null == 0 && max > 0){
    800017e0:	c8a9                	beqz	s1,80001832 <copyinstr+0xa0>
    va0 = PGROUNDDOWN(srcva);
    800017e2:	015bf933          	and	s2,s7,s5
    pa0 = walkaddr(pagetable, va0);
    800017e6:	85ca                	mv	a1,s2
    800017e8:	8552                	mv	a0,s4
    800017ea:	00000097          	auipc	ra,0x0
    800017ee:	88c080e7          	jalr	-1908(ra) # 80001076 <walkaddr>
    if(pa0 == 0)
    800017f2:	c131                	beqz	a0,80001836 <copyinstr+0xa4>
    n = PGSIZE - (srcva - va0);
    800017f4:	41790833          	sub	a6,s2,s7
    800017f8:	984e                	add	a6,a6,s3
    if(n > max)
    800017fa:	0104f363          	bgeu	s1,a6,80001800 <copyinstr+0x6e>
    800017fe:	8826                	mv	a6,s1
    char *p = (char *) (pa0 + (srcva - va0));
    80001800:	955e                	add	a0,a0,s7
    80001802:	41250533          	sub	a0,a0,s2
    while(n > 0){
    80001806:	fc080be3          	beqz	a6,800017dc <copyinstr+0x4a>
    8000180a:	985a                	add	a6,a6,s6
    8000180c:	87da                	mv	a5,s6
      if(*p == '\0'){
    8000180e:	41650633          	sub	a2,a0,s6
    80001812:	14fd                	addi	s1,s1,-1
    80001814:	9b26                	add	s6,s6,s1
    80001816:	00f60733          	add	a4,a2,a5
    8000181a:	00074703          	lbu	a4,0(a4)
    8000181e:	df49                	beqz	a4,800017b8 <copyinstr+0x26>
        *dst = *p;
    80001820:	00e78023          	sb	a4,0(a5)
      --max;
    80001824:	40fb04b3          	sub	s1,s6,a5
      dst++;
    80001828:	0785                	addi	a5,a5,1
    while(n > 0){
    8000182a:	ff0796e3          	bne	a5,a6,80001816 <copyinstr+0x84>
      dst++;
    8000182e:	8b42                	mv	s6,a6
    80001830:	b775                	j	800017dc <copyinstr+0x4a>
    80001832:	4781                	li	a5,0
    80001834:	b769                	j	800017be <copyinstr+0x2c>
      return -1;
    80001836:	557d                	li	a0,-1
    80001838:	b779                	j	800017c6 <copyinstr+0x34>
  int got_null = 0;
    8000183a:	4781                	li	a5,0
  if(got_null){
    8000183c:	0017b793          	seqz	a5,a5
    80001840:	40f00533          	neg	a0,a5
}
    80001844:	8082                	ret

0000000080001846 <update_last_runnable_time>:
// memory model when using p->parent.
// must be acquired before any p->lock.
struct spinlock wait_lock;

void 
update_last_runnable_time(struct proc *p){
    80001846:	1141                	addi	sp,sp,-16
    80001848:	e422                	sd	s0,8(sp)
    8000184a:	0800                	addi	s0,sp,16
  #ifdef FCFS
    p->last_runnable_time = ticks;
    8000184c:	00008797          	auipc	a5,0x8
    80001850:	8047a783          	lw	a5,-2044(a5) # 80009050 <ticks>
    80001854:	dd5c                	sw	a5,60(a0)
  #endif
}  
    80001856:	6422                	ld	s0,8(sp)
    80001858:	0141                	addi	sp,sp,16
    8000185a:	8082                	ret

000000008000185c <proc_mapstacks>:

// Allocate a page for each process's kernel stack.
// Map it high in memory, followed by an invalid
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl) {
    8000185c:	7139                	addi	sp,sp,-64
    8000185e:	fc06                	sd	ra,56(sp)
    80001860:	f822                	sd	s0,48(sp)
    80001862:	f426                	sd	s1,40(sp)
    80001864:	f04a                	sd	s2,32(sp)
    80001866:	ec4e                	sd	s3,24(sp)
    80001868:	e852                	sd	s4,16(sp)
    8000186a:	e456                	sd	s5,8(sp)
    8000186c:	e05a                	sd	s6,0(sp)
    8000186e:	0080                	addi	s0,sp,64
    80001870:	89aa                	mv	s3,a0
  struct proc *p;
  
  for(p = proc; p < &proc[NPROC]; p++) {
    80001872:	00010497          	auipc	s1,0x10
    80001876:	e7e48493          	addi	s1,s1,-386 # 800116f0 <proc>
    char *pa = kalloc();
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));
    8000187a:	8b26                	mv	s6,s1
    8000187c:	00006a97          	auipc	s5,0x6
    80001880:	784a8a93          	addi	s5,s5,1924 # 80008000 <etext>
    80001884:	04000937          	lui	s2,0x4000
    80001888:	197d                	addi	s2,s2,-1
    8000188a:	0932                	slli	s2,s2,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    8000188c:	00016a17          	auipc	s4,0x16
    80001890:	064a0a13          	addi	s4,s4,100 # 800178f0 <tickslock>
    char *pa = kalloc();
    80001894:	fffff097          	auipc	ra,0xfffff
    80001898:	260080e7          	jalr	608(ra) # 80000af4 <kalloc>
    8000189c:	862a                	mv	a2,a0
    if(pa == 0)
    8000189e:	c131                	beqz	a0,800018e2 <proc_mapstacks+0x86>
    uint64 va = KSTACK((int) (p - proc));
    800018a0:	416485b3          	sub	a1,s1,s6
    800018a4:	858d                	srai	a1,a1,0x3
    800018a6:	000ab783          	ld	a5,0(s5)
    800018aa:	02f585b3          	mul	a1,a1,a5
    800018ae:	2585                	addiw	a1,a1,1
    800018b0:	00d5959b          	slliw	a1,a1,0xd
    kvmmap(kpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
    800018b4:	4719                	li	a4,6
    800018b6:	6685                	lui	a3,0x1
    800018b8:	40b905b3          	sub	a1,s2,a1
    800018bc:	854e                	mv	a0,s3
    800018be:	00000097          	auipc	ra,0x0
    800018c2:	89a080e7          	jalr	-1894(ra) # 80001158 <kvmmap>
  for(p = proc; p < &proc[NPROC]; p++) {
    800018c6:	18848493          	addi	s1,s1,392
    800018ca:	fd4495e3          	bne	s1,s4,80001894 <proc_mapstacks+0x38>
  }
}
    800018ce:	70e2                	ld	ra,56(sp)
    800018d0:	7442                	ld	s0,48(sp)
    800018d2:	74a2                	ld	s1,40(sp)
    800018d4:	7902                	ld	s2,32(sp)
    800018d6:	69e2                	ld	s3,24(sp)
    800018d8:	6a42                	ld	s4,16(sp)
    800018da:	6aa2                	ld	s5,8(sp)
    800018dc:	6b02                	ld	s6,0(sp)
    800018de:	6121                	addi	sp,sp,64
    800018e0:	8082                	ret
      panic("kalloc");
    800018e2:	00007517          	auipc	a0,0x7
    800018e6:	8f650513          	addi	a0,a0,-1802 # 800081d8 <digits+0x198>
    800018ea:	fffff097          	auipc	ra,0xfffff
    800018ee:	c54080e7          	jalr	-940(ra) # 8000053e <panic>

00000000800018f2 <procinit>:

// initialize the proc table at boot time.
void
procinit(void)
{
    800018f2:	7139                	addi	sp,sp,-64
    800018f4:	fc06                	sd	ra,56(sp)
    800018f6:	f822                	sd	s0,48(sp)
    800018f8:	f426                	sd	s1,40(sp)
    800018fa:	f04a                	sd	s2,32(sp)
    800018fc:	ec4e                	sd	s3,24(sp)
    800018fe:	e852                	sd	s4,16(sp)
    80001900:	e456                	sd	s5,8(sp)
    80001902:	e05a                	sd	s6,0(sp)
    80001904:	0080                	addi	s0,sp,64
  struct proc *p;
  
  initlock(&pid_lock, "nextpid");
    80001906:	00007597          	auipc	a1,0x7
    8000190a:	8da58593          	addi	a1,a1,-1830 # 800081e0 <digits+0x1a0>
    8000190e:	00010517          	auipc	a0,0x10
    80001912:	9b250513          	addi	a0,a0,-1614 # 800112c0 <pid_lock>
    80001916:	fffff097          	auipc	ra,0xfffff
    8000191a:	23e080e7          	jalr	574(ra) # 80000b54 <initlock>
  initlock(&wait_lock, "wait_lock");
    8000191e:	00007597          	auipc	a1,0x7
    80001922:	8ca58593          	addi	a1,a1,-1846 # 800081e8 <digits+0x1a8>
    80001926:	00010517          	auipc	a0,0x10
    8000192a:	9b250513          	addi	a0,a0,-1614 # 800112d8 <wait_lock>
    8000192e:	fffff097          	auipc	ra,0xfffff
    80001932:	226080e7          	jalr	550(ra) # 80000b54 <initlock>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001936:	00010497          	auipc	s1,0x10
    8000193a:	dba48493          	addi	s1,s1,-582 # 800116f0 <proc>
      initlock(&p->lock, "proc");
    8000193e:	00007b17          	auipc	s6,0x7
    80001942:	8bab0b13          	addi	s6,s6,-1862 # 800081f8 <digits+0x1b8>
      p->kstack = KSTACK((int) (p - proc));
    80001946:	8aa6                	mv	s5,s1
    80001948:	00006a17          	auipc	s4,0x6
    8000194c:	6b8a0a13          	addi	s4,s4,1720 # 80008000 <etext>
    80001950:	04000937          	lui	s2,0x4000
    80001954:	197d                	addi	s2,s2,-1
    80001956:	0932                	slli	s2,s2,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    80001958:	00016997          	auipc	s3,0x16
    8000195c:	f9898993          	addi	s3,s3,-104 # 800178f0 <tickslock>
      initlock(&p->lock, "proc");
    80001960:	85da                	mv	a1,s6
    80001962:	8526                	mv	a0,s1
    80001964:	fffff097          	auipc	ra,0xfffff
    80001968:	1f0080e7          	jalr	496(ra) # 80000b54 <initlock>
      p->kstack = KSTACK((int) (p - proc));
    8000196c:	415487b3          	sub	a5,s1,s5
    80001970:	878d                	srai	a5,a5,0x3
    80001972:	000a3703          	ld	a4,0(s4)
    80001976:	02e787b3          	mul	a5,a5,a4
    8000197a:	2785                	addiw	a5,a5,1
    8000197c:	00d7979b          	slliw	a5,a5,0xd
    80001980:	40f907b3          	sub	a5,s2,a5
    80001984:	f0bc                	sd	a5,96(s1)
  for(p = proc; p < &proc[NPROC]; p++) {
    80001986:	18848493          	addi	s1,s1,392
    8000198a:	fd349be3          	bne	s1,s3,80001960 <procinit+0x6e>
  }

  start_time = ticks;
    8000198e:	00007797          	auipc	a5,0x7
    80001992:	6c27a783          	lw	a5,1730(a5) # 80009050 <ticks>
    80001996:	00007717          	auipc	a4,0x7
    8000199a:	68f72b23          	sw	a5,1686(a4) # 8000902c <start_time>
}
    8000199e:	70e2                	ld	ra,56(sp)
    800019a0:	7442                	ld	s0,48(sp)
    800019a2:	74a2                	ld	s1,40(sp)
    800019a4:	7902                	ld	s2,32(sp)
    800019a6:	69e2                	ld	s3,24(sp)
    800019a8:	6a42                	ld	s4,16(sp)
    800019aa:	6aa2                	ld	s5,8(sp)
    800019ac:	6b02                	ld	s6,0(sp)
    800019ae:	6121                	addi	sp,sp,64
    800019b0:	8082                	ret

00000000800019b2 <cpuid>:
// Must be called with interrupts disabled,
// to prevent race with process being moved
// to a different CPU.
int
cpuid()
{
    800019b2:	1141                	addi	sp,sp,-16
    800019b4:	e422                	sd	s0,8(sp)
    800019b6:	0800                	addi	s0,sp,16
  asm volatile("mv %0, tp" : "=r" (x) );
    800019b8:	8512                	mv	a0,tp
  int id = r_tp();
  return id;
}
    800019ba:	2501                	sext.w	a0,a0
    800019bc:	6422                	ld	s0,8(sp)
    800019be:	0141                	addi	sp,sp,16
    800019c0:	8082                	ret

00000000800019c2 <mycpu>:

// Return this CPU's cpu struct.
// Interrupts must be disabled.
struct cpu*
mycpu(void) {
    800019c2:	1141                	addi	sp,sp,-16
    800019c4:	e422                	sd	s0,8(sp)
    800019c6:	0800                	addi	s0,sp,16
    800019c8:	8792                	mv	a5,tp
  int id = cpuid();
  struct cpu *c = &cpus[id];
    800019ca:	2781                	sext.w	a5,a5
    800019cc:	079e                	slli	a5,a5,0x7
  return c;
}
    800019ce:	00010517          	auipc	a0,0x10
    800019d2:	92250513          	addi	a0,a0,-1758 # 800112f0 <cpus>
    800019d6:	953e                	add	a0,a0,a5
    800019d8:	6422                	ld	s0,8(sp)
    800019da:	0141                	addi	sp,sp,16
    800019dc:	8082                	ret

00000000800019de <myproc>:

// Return the current struct proc *, or zero if none.
struct proc*
myproc(void) {
    800019de:	1101                	addi	sp,sp,-32
    800019e0:	ec06                	sd	ra,24(sp)
    800019e2:	e822                	sd	s0,16(sp)
    800019e4:	e426                	sd	s1,8(sp)
    800019e6:	1000                	addi	s0,sp,32
  push_off();
    800019e8:	fffff097          	auipc	ra,0xfffff
    800019ec:	1b0080e7          	jalr	432(ra) # 80000b98 <push_off>
    800019f0:	8792                	mv	a5,tp
  struct cpu *c = mycpu();
  struct proc *p = c->proc;
    800019f2:	2781                	sext.w	a5,a5
    800019f4:	079e                	slli	a5,a5,0x7
    800019f6:	00010717          	auipc	a4,0x10
    800019fa:	8ca70713          	addi	a4,a4,-1846 # 800112c0 <pid_lock>
    800019fe:	97ba                	add	a5,a5,a4
    80001a00:	7b84                	ld	s1,48(a5)
  pop_off();
    80001a02:	fffff097          	auipc	ra,0xfffff
    80001a06:	236080e7          	jalr	566(ra) # 80000c38 <pop_off>
  return p;
}
    80001a0a:	8526                	mv	a0,s1
    80001a0c:	60e2                	ld	ra,24(sp)
    80001a0e:	6442                	ld	s0,16(sp)
    80001a10:	64a2                	ld	s1,8(sp)
    80001a12:	6105                	addi	sp,sp,32
    80001a14:	8082                	ret

0000000080001a16 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch to forkret.
void
forkret(void)
{
    80001a16:	1141                	addi	sp,sp,-16
    80001a18:	e406                	sd	ra,8(sp)
    80001a1a:	e022                	sd	s0,0(sp)
    80001a1c:	0800                	addi	s0,sp,16
  static int first = 1;

  // Still holding p->lock from scheduler.
  release(&myproc()->lock);
    80001a1e:	00000097          	auipc	ra,0x0
    80001a22:	fc0080e7          	jalr	-64(ra) # 800019de <myproc>
    80001a26:	fffff097          	auipc	ra,0xfffff
    80001a2a:	272080e7          	jalr	626(ra) # 80000c98 <release>

  if (first) {
    80001a2e:	00007797          	auipc	a5,0x7
    80001a32:	eb27a783          	lw	a5,-334(a5) # 800088e0 <first.1753>
    80001a36:	eb89                	bnez	a5,80001a48 <forkret+0x32>
    // be run from main().
    first = 0;
    fsinit(ROOTDEV);
  }

  usertrapret();
    80001a38:	00001097          	auipc	ra,0x1
    80001a3c:	132080e7          	jalr	306(ra) # 80002b6a <usertrapret>
}
    80001a40:	60a2                	ld	ra,8(sp)
    80001a42:	6402                	ld	s0,0(sp)
    80001a44:	0141                	addi	sp,sp,16
    80001a46:	8082                	ret
    first = 0;
    80001a48:	00007797          	auipc	a5,0x7
    80001a4c:	e807ac23          	sw	zero,-360(a5) # 800088e0 <first.1753>
    fsinit(ROOTDEV);
    80001a50:	4505                	li	a0,1
    80001a52:	00002097          	auipc	ra,0x2
    80001a56:	ec0080e7          	jalr	-320(ra) # 80003912 <fsinit>
    80001a5a:	bff9                	j	80001a38 <forkret+0x22>

0000000080001a5c <allocpid>:
allocpid() {
    80001a5c:	1101                	addi	sp,sp,-32
    80001a5e:	ec06                	sd	ra,24(sp)
    80001a60:	e822                	sd	s0,16(sp)
    80001a62:	e426                	sd	s1,8(sp)
    80001a64:	e04a                	sd	s2,0(sp)
    80001a66:	1000                	addi	s0,sp,32
  acquire(&pid_lock);
    80001a68:	00010917          	auipc	s2,0x10
    80001a6c:	85890913          	addi	s2,s2,-1960 # 800112c0 <pid_lock>
    80001a70:	854a                	mv	a0,s2
    80001a72:	fffff097          	auipc	ra,0xfffff
    80001a76:	172080e7          	jalr	370(ra) # 80000be4 <acquire>
  pid = nextpid;
    80001a7a:	00007797          	auipc	a5,0x7
    80001a7e:	e6a78793          	addi	a5,a5,-406 # 800088e4 <nextpid>
    80001a82:	4384                	lw	s1,0(a5)
  nextpid = nextpid + 1;
    80001a84:	0014871b          	addiw	a4,s1,1
    80001a88:	c398                	sw	a4,0(a5)
  release(&pid_lock);
    80001a8a:	854a                	mv	a0,s2
    80001a8c:	fffff097          	auipc	ra,0xfffff
    80001a90:	20c080e7          	jalr	524(ra) # 80000c98 <release>
}
    80001a94:	8526                	mv	a0,s1
    80001a96:	60e2                	ld	ra,24(sp)
    80001a98:	6442                	ld	s0,16(sp)
    80001a9a:	64a2                	ld	s1,8(sp)
    80001a9c:	6902                	ld	s2,0(sp)
    80001a9e:	6105                	addi	sp,sp,32
    80001aa0:	8082                	ret

0000000080001aa2 <proc_pagetable>:
{
    80001aa2:	1101                	addi	sp,sp,-32
    80001aa4:	ec06                	sd	ra,24(sp)
    80001aa6:	e822                	sd	s0,16(sp)
    80001aa8:	e426                	sd	s1,8(sp)
    80001aaa:	e04a                	sd	s2,0(sp)
    80001aac:	1000                	addi	s0,sp,32
    80001aae:	892a                	mv	s2,a0
  pagetable = uvmcreate();
    80001ab0:	00000097          	auipc	ra,0x0
    80001ab4:	892080e7          	jalr	-1902(ra) # 80001342 <uvmcreate>
    80001ab8:	84aa                	mv	s1,a0
  if(pagetable == 0)
    80001aba:	c121                	beqz	a0,80001afa <proc_pagetable+0x58>
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
    80001abc:	4729                	li	a4,10
    80001abe:	00005697          	auipc	a3,0x5
    80001ac2:	54268693          	addi	a3,a3,1346 # 80007000 <_trampoline>
    80001ac6:	6605                	lui	a2,0x1
    80001ac8:	040005b7          	lui	a1,0x4000
    80001acc:	15fd                	addi	a1,a1,-1
    80001ace:	05b2                	slli	a1,a1,0xc
    80001ad0:	fffff097          	auipc	ra,0xfffff
    80001ad4:	5e8080e7          	jalr	1512(ra) # 800010b8 <mappages>
    80001ad8:	02054863          	bltz	a0,80001b08 <proc_pagetable+0x66>
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
    80001adc:	4719                	li	a4,6
    80001ade:	07893683          	ld	a3,120(s2)
    80001ae2:	6605                	lui	a2,0x1
    80001ae4:	020005b7          	lui	a1,0x2000
    80001ae8:	15fd                	addi	a1,a1,-1
    80001aea:	05b6                	slli	a1,a1,0xd
    80001aec:	8526                	mv	a0,s1
    80001aee:	fffff097          	auipc	ra,0xfffff
    80001af2:	5ca080e7          	jalr	1482(ra) # 800010b8 <mappages>
    80001af6:	02054163          	bltz	a0,80001b18 <proc_pagetable+0x76>
}
    80001afa:	8526                	mv	a0,s1
    80001afc:	60e2                	ld	ra,24(sp)
    80001afe:	6442                	ld	s0,16(sp)
    80001b00:	64a2                	ld	s1,8(sp)
    80001b02:	6902                	ld	s2,0(sp)
    80001b04:	6105                	addi	sp,sp,32
    80001b06:	8082                	ret
    uvmfree(pagetable, 0);
    80001b08:	4581                	li	a1,0
    80001b0a:	8526                	mv	a0,s1
    80001b0c:	00000097          	auipc	ra,0x0
    80001b10:	a32080e7          	jalr	-1486(ra) # 8000153e <uvmfree>
    return 0;
    80001b14:	4481                	li	s1,0
    80001b16:	b7d5                	j	80001afa <proc_pagetable+0x58>
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001b18:	4681                	li	a3,0
    80001b1a:	4605                	li	a2,1
    80001b1c:	040005b7          	lui	a1,0x4000
    80001b20:	15fd                	addi	a1,a1,-1
    80001b22:	05b2                	slli	a1,a1,0xc
    80001b24:	8526                	mv	a0,s1
    80001b26:	fffff097          	auipc	ra,0xfffff
    80001b2a:	758080e7          	jalr	1880(ra) # 8000127e <uvmunmap>
    uvmfree(pagetable, 0);
    80001b2e:	4581                	li	a1,0
    80001b30:	8526                	mv	a0,s1
    80001b32:	00000097          	auipc	ra,0x0
    80001b36:	a0c080e7          	jalr	-1524(ra) # 8000153e <uvmfree>
    return 0;
    80001b3a:	4481                	li	s1,0
    80001b3c:	bf7d                	j	80001afa <proc_pagetable+0x58>

0000000080001b3e <proc_freepagetable>:
{
    80001b3e:	1101                	addi	sp,sp,-32
    80001b40:	ec06                	sd	ra,24(sp)
    80001b42:	e822                	sd	s0,16(sp)
    80001b44:	e426                	sd	s1,8(sp)
    80001b46:	e04a                	sd	s2,0(sp)
    80001b48:	1000                	addi	s0,sp,32
    80001b4a:	84aa                	mv	s1,a0
    80001b4c:	892e                	mv	s2,a1
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001b4e:	4681                	li	a3,0
    80001b50:	4605                	li	a2,1
    80001b52:	040005b7          	lui	a1,0x4000
    80001b56:	15fd                	addi	a1,a1,-1
    80001b58:	05b2                	slli	a1,a1,0xc
    80001b5a:	fffff097          	auipc	ra,0xfffff
    80001b5e:	724080e7          	jalr	1828(ra) # 8000127e <uvmunmap>
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
    80001b62:	4681                	li	a3,0
    80001b64:	4605                	li	a2,1
    80001b66:	020005b7          	lui	a1,0x2000
    80001b6a:	15fd                	addi	a1,a1,-1
    80001b6c:	05b6                	slli	a1,a1,0xd
    80001b6e:	8526                	mv	a0,s1
    80001b70:	fffff097          	auipc	ra,0xfffff
    80001b74:	70e080e7          	jalr	1806(ra) # 8000127e <uvmunmap>
  uvmfree(pagetable, sz);
    80001b78:	85ca                	mv	a1,s2
    80001b7a:	8526                	mv	a0,s1
    80001b7c:	00000097          	auipc	ra,0x0
    80001b80:	9c2080e7          	jalr	-1598(ra) # 8000153e <uvmfree>
}
    80001b84:	60e2                	ld	ra,24(sp)
    80001b86:	6442                	ld	s0,16(sp)
    80001b88:	64a2                	ld	s1,8(sp)
    80001b8a:	6902                	ld	s2,0(sp)
    80001b8c:	6105                	addi	sp,sp,32
    80001b8e:	8082                	ret

0000000080001b90 <freeproc>:
{
    80001b90:	1101                	addi	sp,sp,-32
    80001b92:	ec06                	sd	ra,24(sp)
    80001b94:	e822                	sd	s0,16(sp)
    80001b96:	e426                	sd	s1,8(sp)
    80001b98:	1000                	addi	s0,sp,32
    80001b9a:	84aa                	mv	s1,a0
  if(p->trapframe)
    80001b9c:	7d28                	ld	a0,120(a0)
    80001b9e:	c509                	beqz	a0,80001ba8 <freeproc+0x18>
    kfree((void*)p->trapframe);
    80001ba0:	fffff097          	auipc	ra,0xfffff
    80001ba4:	e58080e7          	jalr	-424(ra) # 800009f8 <kfree>
  p->trapframe = 0;
    80001ba8:	0604bc23          	sd	zero,120(s1)
  if(p->pagetable)
    80001bac:	78a8                	ld	a0,112(s1)
    80001bae:	c511                	beqz	a0,80001bba <freeproc+0x2a>
    proc_freepagetable(p->pagetable, p->sz);
    80001bb0:	74ac                	ld	a1,104(s1)
    80001bb2:	00000097          	auipc	ra,0x0
    80001bb6:	f8c080e7          	jalr	-116(ra) # 80001b3e <proc_freepagetable>
  p->pagetable = 0;
    80001bba:	0604b823          	sd	zero,112(s1)
  p->sz = 0;
    80001bbe:	0604b423          	sd	zero,104(s1)
  p->pid = 0;
    80001bc2:	0204a823          	sw	zero,48(s1)
  p->parent = 0;
    80001bc6:	0404bc23          	sd	zero,88(s1)
  p->name[0] = 0;
    80001bca:	16048c23          	sb	zero,376(s1)
  p->chan = 0;
    80001bce:	0204b023          	sd	zero,32(s1)
  p->killed = 0;
    80001bd2:	0204a423          	sw	zero,40(s1)
  p->xstate = 0;
    80001bd6:	0204a623          	sw	zero,44(s1)
  p->state = UNUSED;
    80001bda:	0004ac23          	sw	zero,24(s1)
}
    80001bde:	60e2                	ld	ra,24(sp)
    80001be0:	6442                	ld	s0,16(sp)
    80001be2:	64a2                	ld	s1,8(sp)
    80001be4:	6105                	addi	sp,sp,32
    80001be6:	8082                	ret

0000000080001be8 <allocproc>:
{
    80001be8:	1101                	addi	sp,sp,-32
    80001bea:	ec06                	sd	ra,24(sp)
    80001bec:	e822                	sd	s0,16(sp)
    80001bee:	e426                	sd	s1,8(sp)
    80001bf0:	e04a                	sd	s2,0(sp)
    80001bf2:	1000                	addi	s0,sp,32
  for(p = proc; p < &proc[NPROC]; p++) {
    80001bf4:	00010497          	auipc	s1,0x10
    80001bf8:	afc48493          	addi	s1,s1,-1284 # 800116f0 <proc>
    80001bfc:	00016917          	auipc	s2,0x16
    80001c00:	cf490913          	addi	s2,s2,-780 # 800178f0 <tickslock>
    acquire(&p->lock);
    80001c04:	8526                	mv	a0,s1
    80001c06:	fffff097          	auipc	ra,0xfffff
    80001c0a:	fde080e7          	jalr	-34(ra) # 80000be4 <acquire>
    if(p->state == UNUSED) {
    80001c0e:	4c9c                	lw	a5,24(s1)
    80001c10:	cf81                	beqz	a5,80001c28 <allocproc+0x40>
      release(&p->lock);
    80001c12:	8526                	mv	a0,s1
    80001c14:	fffff097          	auipc	ra,0xfffff
    80001c18:	084080e7          	jalr	132(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001c1c:	18848493          	addi	s1,s1,392
    80001c20:	ff2492e3          	bne	s1,s2,80001c04 <allocproc+0x1c>
  return 0;
    80001c24:	4481                	li	s1,0
    80001c26:	a8a9                	j	80001c80 <allocproc+0x98>
  p->pid = allocpid();
    80001c28:	00000097          	auipc	ra,0x0
    80001c2c:	e34080e7          	jalr	-460(ra) # 80001a5c <allocpid>
    80001c30:	d888                	sw	a0,48(s1)
  p->state = USED;
    80001c32:	4785                	li	a5,1
    80001c34:	cc9c                	sw	a5,24(s1)
  p->mean_ticks = 0;
    80001c36:	0204aa23          	sw	zero,52(s1)
  p->last_ticks = 0;
    80001c3a:	0204ac23          	sw	zero,56(s1)
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    80001c3e:	fffff097          	auipc	ra,0xfffff
    80001c42:	eb6080e7          	jalr	-330(ra) # 80000af4 <kalloc>
    80001c46:	892a                	mv	s2,a0
    80001c48:	fca8                	sd	a0,120(s1)
    80001c4a:	c131                	beqz	a0,80001c8e <allocproc+0xa6>
  p->pagetable = proc_pagetable(p);
    80001c4c:	8526                	mv	a0,s1
    80001c4e:	00000097          	auipc	ra,0x0
    80001c52:	e54080e7          	jalr	-428(ra) # 80001aa2 <proc_pagetable>
    80001c56:	892a                	mv	s2,a0
    80001c58:	f8a8                	sd	a0,112(s1)
  if(p->pagetable == 0){
    80001c5a:	c531                	beqz	a0,80001ca6 <allocproc+0xbe>
  memset(&p->context, 0, sizeof(p->context));
    80001c5c:	07000613          	li	a2,112
    80001c60:	4581                	li	a1,0
    80001c62:	08048513          	addi	a0,s1,128
    80001c66:	fffff097          	auipc	ra,0xfffff
    80001c6a:	07a080e7          	jalr	122(ra) # 80000ce0 <memset>
  p->context.ra = (uint64)forkret;
    80001c6e:	00000797          	auipc	a5,0x0
    80001c72:	da878793          	addi	a5,a5,-600 # 80001a16 <forkret>
    80001c76:	e0dc                	sd	a5,128(s1)
  p->context.sp = p->kstack + PGSIZE;
    80001c78:	70bc                	ld	a5,96(s1)
    80001c7a:	6705                	lui	a4,0x1
    80001c7c:	97ba                	add	a5,a5,a4
    80001c7e:	e4dc                	sd	a5,136(s1)
}
    80001c80:	8526                	mv	a0,s1
    80001c82:	60e2                	ld	ra,24(sp)
    80001c84:	6442                	ld	s0,16(sp)
    80001c86:	64a2                	ld	s1,8(sp)
    80001c88:	6902                	ld	s2,0(sp)
    80001c8a:	6105                	addi	sp,sp,32
    80001c8c:	8082                	ret
    freeproc(p);
    80001c8e:	8526                	mv	a0,s1
    80001c90:	00000097          	auipc	ra,0x0
    80001c94:	f00080e7          	jalr	-256(ra) # 80001b90 <freeproc>
    release(&p->lock);
    80001c98:	8526                	mv	a0,s1
    80001c9a:	fffff097          	auipc	ra,0xfffff
    80001c9e:	ffe080e7          	jalr	-2(ra) # 80000c98 <release>
    return 0;
    80001ca2:	84ca                	mv	s1,s2
    80001ca4:	bff1                	j	80001c80 <allocproc+0x98>
    freeproc(p);
    80001ca6:	8526                	mv	a0,s1
    80001ca8:	00000097          	auipc	ra,0x0
    80001cac:	ee8080e7          	jalr	-280(ra) # 80001b90 <freeproc>
    release(&p->lock);
    80001cb0:	8526                	mv	a0,s1
    80001cb2:	fffff097          	auipc	ra,0xfffff
    80001cb6:	fe6080e7          	jalr	-26(ra) # 80000c98 <release>
    return 0;
    80001cba:	84ca                	mv	s1,s2
    80001cbc:	b7d1                	j	80001c80 <allocproc+0x98>

0000000080001cbe <userinit>:
{
    80001cbe:	1101                	addi	sp,sp,-32
    80001cc0:	ec06                	sd	ra,24(sp)
    80001cc2:	e822                	sd	s0,16(sp)
    80001cc4:	e426                	sd	s1,8(sp)
    80001cc6:	1000                	addi	s0,sp,32
  p = allocproc();
    80001cc8:	00000097          	auipc	ra,0x0
    80001ccc:	f20080e7          	jalr	-224(ra) # 80001be8 <allocproc>
    80001cd0:	84aa                	mv	s1,a0
  initproc = p;
    80001cd2:	00007797          	auipc	a5,0x7
    80001cd6:	36a7bb23          	sd	a0,886(a5) # 80009048 <initproc>
  uvminit(p->pagetable, initcode, sizeof(initcode));
    80001cda:	03400613          	li	a2,52
    80001cde:	00007597          	auipc	a1,0x7
    80001ce2:	c1258593          	addi	a1,a1,-1006 # 800088f0 <initcode>
    80001ce6:	7928                	ld	a0,112(a0)
    80001ce8:	fffff097          	auipc	ra,0xfffff
    80001cec:	688080e7          	jalr	1672(ra) # 80001370 <uvminit>
  p->sz = PGSIZE;
    80001cf0:	6785                	lui	a5,0x1
    80001cf2:	f4bc                	sd	a5,104(s1)
  p->trapframe->epc = 0;      // user program counter
    80001cf4:	7cb8                	ld	a4,120(s1)
    80001cf6:	00073c23          	sd	zero,24(a4) # 1018 <_entry-0x7fffefe8>
  p->trapframe->sp = PGSIZE;  // user stack pointer
    80001cfa:	7cb8                	ld	a4,120(s1)
    80001cfc:	fb1c                	sd	a5,48(a4)
  safestrcpy(p->name, "initcode", sizeof(p->name));
    80001cfe:	4641                	li	a2,16
    80001d00:	00006597          	auipc	a1,0x6
    80001d04:	50058593          	addi	a1,a1,1280 # 80008200 <digits+0x1c0>
    80001d08:	17848513          	addi	a0,s1,376
    80001d0c:	fffff097          	auipc	ra,0xfffff
    80001d10:	126080e7          	jalr	294(ra) # 80000e32 <safestrcpy>
  p->cwd = namei("/");
    80001d14:	00006517          	auipc	a0,0x6
    80001d18:	4fc50513          	addi	a0,a0,1276 # 80008210 <digits+0x1d0>
    80001d1c:	00002097          	auipc	ra,0x2
    80001d20:	624080e7          	jalr	1572(ra) # 80004340 <namei>
    80001d24:	16a4b823          	sd	a0,368(s1)
  p->state = RUNNABLE;
    80001d28:	478d                	li	a5,3
    80001d2a:	cc9c                	sw	a5,24(s1)
    p->last_runnable_time = ticks;
    80001d2c:	00007797          	auipc	a5,0x7
    80001d30:	3247a783          	lw	a5,804(a5) # 80009050 <ticks>
    80001d34:	dcdc                	sw	a5,60(s1)
  release(&p->lock);
    80001d36:	8526                	mv	a0,s1
    80001d38:	fffff097          	auipc	ra,0xfffff
    80001d3c:	f60080e7          	jalr	-160(ra) # 80000c98 <release>
}
    80001d40:	60e2                	ld	ra,24(sp)
    80001d42:	6442                	ld	s0,16(sp)
    80001d44:	64a2                	ld	s1,8(sp)
    80001d46:	6105                	addi	sp,sp,32
    80001d48:	8082                	ret

0000000080001d4a <growproc>:
{
    80001d4a:	1101                	addi	sp,sp,-32
    80001d4c:	ec06                	sd	ra,24(sp)
    80001d4e:	e822                	sd	s0,16(sp)
    80001d50:	e426                	sd	s1,8(sp)
    80001d52:	e04a                	sd	s2,0(sp)
    80001d54:	1000                	addi	s0,sp,32
    80001d56:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80001d58:	00000097          	auipc	ra,0x0
    80001d5c:	c86080e7          	jalr	-890(ra) # 800019de <myproc>
    80001d60:	892a                	mv	s2,a0
  sz = p->sz;
    80001d62:	752c                	ld	a1,104(a0)
    80001d64:	0005861b          	sext.w	a2,a1
  if(n > 0){
    80001d68:	00904f63          	bgtz	s1,80001d86 <growproc+0x3c>
  } else if(n < 0){
    80001d6c:	0204cc63          	bltz	s1,80001da4 <growproc+0x5a>
  p->sz = sz;
    80001d70:	1602                	slli	a2,a2,0x20
    80001d72:	9201                	srli	a2,a2,0x20
    80001d74:	06c93423          	sd	a2,104(s2)
  return 0;
    80001d78:	4501                	li	a0,0
}
    80001d7a:	60e2                	ld	ra,24(sp)
    80001d7c:	6442                	ld	s0,16(sp)
    80001d7e:	64a2                	ld	s1,8(sp)
    80001d80:	6902                	ld	s2,0(sp)
    80001d82:	6105                	addi	sp,sp,32
    80001d84:	8082                	ret
    if((sz = uvmalloc(p->pagetable, sz, sz + n)) == 0) {
    80001d86:	9e25                	addw	a2,a2,s1
    80001d88:	1602                	slli	a2,a2,0x20
    80001d8a:	9201                	srli	a2,a2,0x20
    80001d8c:	1582                	slli	a1,a1,0x20
    80001d8e:	9181                	srli	a1,a1,0x20
    80001d90:	7928                	ld	a0,112(a0)
    80001d92:	fffff097          	auipc	ra,0xfffff
    80001d96:	698080e7          	jalr	1688(ra) # 8000142a <uvmalloc>
    80001d9a:	0005061b          	sext.w	a2,a0
    80001d9e:	fa69                	bnez	a2,80001d70 <growproc+0x26>
      return -1;
    80001da0:	557d                	li	a0,-1
    80001da2:	bfe1                	j	80001d7a <growproc+0x30>
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    80001da4:	9e25                	addw	a2,a2,s1
    80001da6:	1602                	slli	a2,a2,0x20
    80001da8:	9201                	srli	a2,a2,0x20
    80001daa:	1582                	slli	a1,a1,0x20
    80001dac:	9181                	srli	a1,a1,0x20
    80001dae:	7928                	ld	a0,112(a0)
    80001db0:	fffff097          	auipc	ra,0xfffff
    80001db4:	632080e7          	jalr	1586(ra) # 800013e2 <uvmdealloc>
    80001db8:	0005061b          	sext.w	a2,a0
    80001dbc:	bf55                	j	80001d70 <growproc+0x26>

0000000080001dbe <fork>:
{
    80001dbe:	7179                	addi	sp,sp,-48
    80001dc0:	f406                	sd	ra,40(sp)
    80001dc2:	f022                	sd	s0,32(sp)
    80001dc4:	ec26                	sd	s1,24(sp)
    80001dc6:	e84a                	sd	s2,16(sp)
    80001dc8:	e44e                	sd	s3,8(sp)
    80001dca:	e052                	sd	s4,0(sp)
    80001dcc:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80001dce:	00000097          	auipc	ra,0x0
    80001dd2:	c10080e7          	jalr	-1008(ra) # 800019de <myproc>
    80001dd6:	892a                	mv	s2,a0
  if((np = allocproc()) == 0){
    80001dd8:	00000097          	auipc	ra,0x0
    80001ddc:	e10080e7          	jalr	-496(ra) # 80001be8 <allocproc>
    80001de0:	12050163          	beqz	a0,80001f02 <fork+0x144>
    80001de4:	89aa                	mv	s3,a0
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0){
    80001de6:	06893603          	ld	a2,104(s2)
    80001dea:	792c                	ld	a1,112(a0)
    80001dec:	07093503          	ld	a0,112(s2)
    80001df0:	fffff097          	auipc	ra,0xfffff
    80001df4:	786080e7          	jalr	1926(ra) # 80001576 <uvmcopy>
    80001df8:	04054663          	bltz	a0,80001e44 <fork+0x86>
  np->sz = p->sz;
    80001dfc:	06893783          	ld	a5,104(s2)
    80001e00:	06f9b423          	sd	a5,104(s3)
  *(np->trapframe) = *(p->trapframe);
    80001e04:	07893683          	ld	a3,120(s2)
    80001e08:	87b6                	mv	a5,a3
    80001e0a:	0789b703          	ld	a4,120(s3)
    80001e0e:	12068693          	addi	a3,a3,288
    80001e12:	0007b803          	ld	a6,0(a5)
    80001e16:	6788                	ld	a0,8(a5)
    80001e18:	6b8c                	ld	a1,16(a5)
    80001e1a:	6f90                	ld	a2,24(a5)
    80001e1c:	01073023          	sd	a6,0(a4)
    80001e20:	e708                	sd	a0,8(a4)
    80001e22:	eb0c                	sd	a1,16(a4)
    80001e24:	ef10                	sd	a2,24(a4)
    80001e26:	02078793          	addi	a5,a5,32
    80001e2a:	02070713          	addi	a4,a4,32
    80001e2e:	fed792e3          	bne	a5,a3,80001e12 <fork+0x54>
  np->trapframe->a0 = 0;
    80001e32:	0789b783          	ld	a5,120(s3)
    80001e36:	0607b823          	sd	zero,112(a5)
    80001e3a:	0f000493          	li	s1,240
  for(i = 0; i < NOFILE; i++)
    80001e3e:	17000a13          	li	s4,368
    80001e42:	a03d                	j	80001e70 <fork+0xb2>
    freeproc(np);
    80001e44:	854e                	mv	a0,s3
    80001e46:	00000097          	auipc	ra,0x0
    80001e4a:	d4a080e7          	jalr	-694(ra) # 80001b90 <freeproc>
    release(&np->lock);
    80001e4e:	854e                	mv	a0,s3
    80001e50:	fffff097          	auipc	ra,0xfffff
    80001e54:	e48080e7          	jalr	-440(ra) # 80000c98 <release>
    return -1;
    80001e58:	5a7d                	li	s4,-1
    80001e5a:	a859                	j	80001ef0 <fork+0x132>
      np->ofile[i] = filedup(p->ofile[i]);
    80001e5c:	00003097          	auipc	ra,0x3
    80001e60:	b7a080e7          	jalr	-1158(ra) # 800049d6 <filedup>
    80001e64:	009987b3          	add	a5,s3,s1
    80001e68:	e388                	sd	a0,0(a5)
  for(i = 0; i < NOFILE; i++)
    80001e6a:	04a1                	addi	s1,s1,8
    80001e6c:	01448763          	beq	s1,s4,80001e7a <fork+0xbc>
    if(p->ofile[i])
    80001e70:	009907b3          	add	a5,s2,s1
    80001e74:	6388                	ld	a0,0(a5)
    80001e76:	f17d                	bnez	a0,80001e5c <fork+0x9e>
    80001e78:	bfcd                	j	80001e6a <fork+0xac>
  np->cwd = idup(p->cwd);
    80001e7a:	17093503          	ld	a0,368(s2)
    80001e7e:	00002097          	auipc	ra,0x2
    80001e82:	cce080e7          	jalr	-818(ra) # 80003b4c <idup>
    80001e86:	16a9b823          	sd	a0,368(s3)
  safestrcpy(np->name, p->name, sizeof(p->name));
    80001e8a:	4641                	li	a2,16
    80001e8c:	17890593          	addi	a1,s2,376
    80001e90:	17898513          	addi	a0,s3,376
    80001e94:	fffff097          	auipc	ra,0xfffff
    80001e98:	f9e080e7          	jalr	-98(ra) # 80000e32 <safestrcpy>
  pid = np->pid;
    80001e9c:	0309aa03          	lw	s4,48(s3)
  release(&np->lock);
    80001ea0:	854e                	mv	a0,s3
    80001ea2:	fffff097          	auipc	ra,0xfffff
    80001ea6:	df6080e7          	jalr	-522(ra) # 80000c98 <release>
  acquire(&wait_lock);
    80001eaa:	0000f497          	auipc	s1,0xf
    80001eae:	42e48493          	addi	s1,s1,1070 # 800112d8 <wait_lock>
    80001eb2:	8526                	mv	a0,s1
    80001eb4:	fffff097          	auipc	ra,0xfffff
    80001eb8:	d30080e7          	jalr	-720(ra) # 80000be4 <acquire>
  np->parent = p;
    80001ebc:	0529bc23          	sd	s2,88(s3)
  release(&wait_lock);
    80001ec0:	8526                	mv	a0,s1
    80001ec2:	fffff097          	auipc	ra,0xfffff
    80001ec6:	dd6080e7          	jalr	-554(ra) # 80000c98 <release>
  acquire(&np->lock);
    80001eca:	854e                	mv	a0,s3
    80001ecc:	fffff097          	auipc	ra,0xfffff
    80001ed0:	d18080e7          	jalr	-744(ra) # 80000be4 <acquire>
  np->state = RUNNABLE;
    80001ed4:	478d                	li	a5,3
    80001ed6:	00f9ac23          	sw	a5,24(s3)
    p->last_runnable_time = ticks;
    80001eda:	00007797          	auipc	a5,0x7
    80001ede:	1767a783          	lw	a5,374(a5) # 80009050 <ticks>
    80001ee2:	02f92e23          	sw	a5,60(s2)
  release(&np->lock);
    80001ee6:	854e                	mv	a0,s3
    80001ee8:	fffff097          	auipc	ra,0xfffff
    80001eec:	db0080e7          	jalr	-592(ra) # 80000c98 <release>
}
    80001ef0:	8552                	mv	a0,s4
    80001ef2:	70a2                	ld	ra,40(sp)
    80001ef4:	7402                	ld	s0,32(sp)
    80001ef6:	64e2                	ld	s1,24(sp)
    80001ef8:	6942                	ld	s2,16(sp)
    80001efa:	69a2                	ld	s3,8(sp)
    80001efc:	6a02                	ld	s4,0(sp)
    80001efe:	6145                	addi	sp,sp,48
    80001f00:	8082                	ret
    return -1;
    80001f02:	5a7d                	li	s4,-1
    80001f04:	b7f5                	j	80001ef0 <fork+0x132>

0000000080001f06 <get_mean>:
{
    80001f06:	1141                	addi	sp,sp,-16
    80001f08:	e422                	sd	s0,8(sp)
    80001f0a:	0800                	addi	s0,sp,16
  return ( last_mean * num_procs + curr_time) / (num_procs + 1);
    80001f0c:	02c5053b          	mulw	a0,a0,a2
    80001f10:	9d2d                	addw	a0,a0,a1
    80001f12:	2605                	addiw	a2,a2,1
}
    80001f14:	02c5453b          	divw	a0,a0,a2
    80001f18:	6422                	ld	s0,8(sp)
    80001f1a:	0141                	addi	sp,sp,16
    80001f1c:	8082                	ret

0000000080001f1e <scheduler_default>:
{
    80001f1e:	715d                	addi	sp,sp,-80
    80001f20:	e486                	sd	ra,72(sp)
    80001f22:	e0a2                	sd	s0,64(sp)
    80001f24:	fc26                	sd	s1,56(sp)
    80001f26:	f84a                	sd	s2,48(sp)
    80001f28:	f44e                	sd	s3,40(sp)
    80001f2a:	f052                	sd	s4,32(sp)
    80001f2c:	ec56                	sd	s5,24(sp)
    80001f2e:	e85a                	sd	s6,16(sp)
    80001f30:	e45e                	sd	s7,8(sp)
    80001f32:	e062                	sd	s8,0(sp)
    80001f34:	0880                	addi	s0,sp,80
    80001f36:	8792                	mv	a5,tp
  int id = r_tp();
    80001f38:	2781                	sext.w	a5,a5
  c->proc = 0;
    80001f3a:	00779c13          	slli	s8,a5,0x7
    80001f3e:	0000f717          	auipc	a4,0xf
    80001f42:	38270713          	addi	a4,a4,898 # 800112c0 <pid_lock>
    80001f46:	9762                	add	a4,a4,s8
    80001f48:	02073823          	sd	zero,48(a4)
          swtch(&c->context, &p->context);
    80001f4c:	0000f717          	auipc	a4,0xf
    80001f50:	3ac70713          	addi	a4,a4,940 # 800112f8 <cpus+0x8>
    80001f54:	9c3a                	add	s8,s8,a4
      if(ticks >= pause_ticks){ // check if pause signal was called
    80001f56:	00007917          	auipc	s2,0x7
    80001f5a:	0fa90913          	addi	s2,s2,250 # 80009050 <ticks>
    80001f5e:	00007a17          	auipc	s4,0x7
    80001f62:	0e6a0a13          	addi	s4,s4,230 # 80009044 <pause_ticks>
          c->proc = p;
    80001f66:	079e                	slli	a5,a5,0x7
    80001f68:	0000fb17          	auipc	s6,0xf
    80001f6c:	358b0b13          	addi	s6,s6,856 # 800112c0 <pid_lock>
    80001f70:	9b3e                	add	s6,s6,a5
    for(p = proc; p < &proc[NPROC]; p++) {
    80001f72:	00016997          	auipc	s3,0x16
    80001f76:	97e98993          	addi	s3,s3,-1666 # 800178f0 <tickslock>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001f7a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80001f7e:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80001f82:	10079073          	csrw	sstatus,a5
    80001f86:	0000f497          	auipc	s1,0xf
    80001f8a:	76a48493          	addi	s1,s1,1898 # 800116f0 <proc>
        if(p->state == RUNNABLE) {
    80001f8e:	4a8d                	li	s5,3
          p->state = RUNNING;
    80001f90:	4b91                	li	s7,4
    80001f92:	a01d                	j	80001fb8 <scheduler_default+0x9a>
          p->running_time += ticks - p->start_last_running;
    80001f94:	44bc                	lw	a5,72(s1)
    80001f96:	00092703          	lw	a4,0(s2)
    80001f9a:	9fb9                	addw	a5,a5,a4
    80001f9c:	48f8                	lw	a4,84(s1)
    80001f9e:	9f99                	subw	a5,a5,a4
    80001fa0:	c4bc                	sw	a5,72(s1)
          c->proc = 0;
    80001fa2:	020b3823          	sd	zero,48(s6)
        release(&p->lock);
    80001fa6:	8526                	mv	a0,s1
    80001fa8:	fffff097          	auipc	ra,0xfffff
    80001fac:	cf0080e7          	jalr	-784(ra) # 80000c98 <release>
    for(p = proc; p < &proc[NPROC]; p++) {
    80001fb0:	18848493          	addi	s1,s1,392
    80001fb4:	fd3483e3          	beq	s1,s3,80001f7a <scheduler_default+0x5c>
      if(ticks >= pause_ticks){ // check if pause signal was called
    80001fb8:	00092703          	lw	a4,0(s2)
    80001fbc:	000a2783          	lw	a5,0(s4)
    80001fc0:	fef768e3          	bltu	a4,a5,80001fb0 <scheduler_default+0x92>
        acquire(&p->lock);
    80001fc4:	8526                	mv	a0,s1
    80001fc6:	fffff097          	auipc	ra,0xfffff
    80001fca:	c1e080e7          	jalr	-994(ra) # 80000be4 <acquire>
        if(p->state == RUNNABLE) {
    80001fce:	4c9c                	lw	a5,24(s1)
    80001fd0:	fd579be3          	bne	a5,s5,80001fa6 <scheduler_default+0x88>
          p->state = RUNNING;
    80001fd4:	0174ac23          	sw	s7,24(s1)
          c->proc = p;
    80001fd8:	029b3823          	sd	s1,48(s6)
          p->runnable_time += ticks - p->start_last_runnable;
    80001fdc:	00092703          	lw	a4,0(s2)
    80001fe0:	40fc                	lw	a5,68(s1)
    80001fe2:	9fb9                	addw	a5,a5,a4
    80001fe4:	48b4                	lw	a3,80(s1)
    80001fe6:	9f95                	subw	a5,a5,a3
    80001fe8:	c0fc                	sw	a5,68(s1)
          p->start_last_running = ticks;
    80001fea:	c8f8                	sw	a4,84(s1)
          swtch(&c->context, &p->context);
    80001fec:	08048593          	addi	a1,s1,128
    80001ff0:	8562                	mv	a0,s8
    80001ff2:	00001097          	auipc	ra,0x1
    80001ff6:	ace080e7          	jalr	-1330(ra) # 80002ac0 <swtch>
          if(p->state == RUNNABLE)
    80001ffa:	4c9c                	lw	a5,24(s1)
    80001ffc:	f9579ce3          	bne	a5,s5,80001f94 <scheduler_default+0x76>
            p->start_last_runnable = ticks;
    80002000:	00092783          	lw	a5,0(s2)
    80002004:	c8bc                	sw	a5,80(s1)
    80002006:	b779                	j	80001f94 <scheduler_default+0x76>

0000000080002008 <swap_process_ptr>:
{
    80002008:	1141                	addi	sp,sp,-16
    8000200a:	e422                	sd	s0,8(sp)
    8000200c:	0800                	addi	s0,sp,16
  struct proc *temp = *p1;
    8000200e:	611c                	ld	a5,0(a0)
  *p1 = *p2;
    80002010:	6198                	ld	a4,0(a1)
    80002012:	e118                	sd	a4,0(a0)
  *p2 = temp; 
    80002014:	e19c                	sd	a5,0(a1)
}     
    80002016:	6422                	ld	s0,8(sp)
    80002018:	0141                	addi	sp,sp,16
    8000201a:	8082                	ret

000000008000201c <make_acquired_process_running>:
make_acquired_process_running(struct cpu *c, struct proc *p){
    8000201c:	1101                	addi	sp,sp,-32
    8000201e:	ec06                	sd	ra,24(sp)
    80002020:	e822                	sd	s0,16(sp)
    80002022:	e426                	sd	s1,8(sp)
    80002024:	e04a                	sd	s2,0(sp)
    80002026:	1000                	addi	s0,sp,32
    80002028:	892a                	mv	s2,a0
    8000202a:	84ae                	mv	s1,a1
  p->state = RUNNING;
    8000202c:	4791                	li	a5,4
    8000202e:	cd9c                	sw	a5,24(a1)
  c->proc = p;
    80002030:	e10c                	sd	a1,0(a0)
  p->runnable_time += ticks - p->start_last_runnable;
    80002032:	00007717          	auipc	a4,0x7
    80002036:	01e72703          	lw	a4,30(a4) # 80009050 <ticks>
    8000203a:	41fc                	lw	a5,68(a1)
    8000203c:	9fb9                	addw	a5,a5,a4
    8000203e:	49b4                	lw	a3,80(a1)
    80002040:	9f95                	subw	a5,a5,a3
    80002042:	c1fc                	sw	a5,68(a1)
  p->start_last_running = ticks;
    80002044:	c9f8                	sw	a4,84(a1)
  swtch(&c->context, &p->context);
    80002046:	08058593          	addi	a1,a1,128
    8000204a:	0521                	addi	a0,a0,8
    8000204c:	00001097          	auipc	ra,0x1
    80002050:	a74080e7          	jalr	-1420(ra) # 80002ac0 <swtch>
  if(p->state == RUNNABLE)
    80002054:	4c98                	lw	a4,24(s1)
    80002056:	478d                	li	a5,3
    80002058:	02f70363          	beq	a4,a5,8000207e <make_acquired_process_running+0x62>
  p->running_time += ticks - p->start_last_running;
    8000205c:	44bc                	lw	a5,72(s1)
    8000205e:	00007717          	auipc	a4,0x7
    80002062:	ff272703          	lw	a4,-14(a4) # 80009050 <ticks>
    80002066:	9fb9                	addw	a5,a5,a4
    80002068:	48f8                	lw	a4,84(s1)
    8000206a:	9f99                	subw	a5,a5,a4
    8000206c:	c4bc                	sw	a5,72(s1)
  c->proc = 0;
    8000206e:	00093023          	sd	zero,0(s2)
}
    80002072:	60e2                	ld	ra,24(sp)
    80002074:	6442                	ld	s0,16(sp)
    80002076:	64a2                	ld	s1,8(sp)
    80002078:	6902                	ld	s2,0(sp)
    8000207a:	6105                	addi	sp,sp,32
    8000207c:	8082                	ret
      p->start_last_runnable = ticks;
    8000207e:	00007797          	auipc	a5,0x7
    80002082:	fd27a783          	lw	a5,-46(a5) # 80009050 <ticks>
    80002086:	c8bc                	sw	a5,80(s1)
    80002088:	bfd1                	j	8000205c <make_acquired_process_running+0x40>

000000008000208a <scheduler_sjf>:
void scheduler_sjf(void){
    8000208a:	711d                	addi	sp,sp,-96
    8000208c:	ec86                	sd	ra,88(sp)
    8000208e:	e8a2                	sd	s0,80(sp)
    80002090:	e4a6                	sd	s1,72(sp)
    80002092:	e0ca                	sd	s2,64(sp)
    80002094:	fc4e                	sd	s3,56(sp)
    80002096:	f852                	sd	s4,48(sp)
    80002098:	f456                	sd	s5,40(sp)
    8000209a:	f05a                	sd	s6,32(sp)
    8000209c:	ec5e                	sd	s7,24(sp)
    8000209e:	e862                	sd	s8,16(sp)
    800020a0:	e466                	sd	s9,8(sp)
    800020a2:	e06a                	sd	s10,0(sp)
    800020a4:	1080                	addi	s0,sp,96
  asm volatile("mv %0, tp" : "=r" (x) );
    800020a6:	8792                	mv	a5,tp
  int id = r_tp();
    800020a8:	2781                	sext.w	a5,a5
  struct cpu *c = &cpus[id];
    800020aa:	079e                	slli	a5,a5,0x7
    800020ac:	0000f717          	auipc	a4,0xf
    800020b0:	24470713          	addi	a4,a4,580 # 800112f0 <cpus>
    800020b4:	00f70d33          	add	s10,a4,a5
  c->proc = 0;
    800020b8:	0000f717          	auipc	a4,0xf
    800020bc:	20870713          	addi	a4,a4,520 # 800112c0 <pid_lock>
    800020c0:	97ba                	add	a5,a5,a4
    800020c2:	0207b823          	sd	zero,48(a5)
      if(ticks >= pause_ticks){ // check if pause signal was called
    800020c6:	00007917          	auipc	s2,0x7
    800020ca:	f8a90913          	addi	s2,s2,-118 # 80009050 <ticks>
    800020ce:	00007a17          	auipc	s4,0x7
    800020d2:	f76a0a13          	addi	s4,s4,-138 # 80009044 <pause_ticks>
        if(curr->state == RUNNABLE) {
    800020d6:	4a8d                	li	s5,3
          curr->mean_ticks = ((SECONDS_TO_TICKS - RATE) * curr->mean_ticks + curr->last_ticks * (RATE)) / SECONDS_TO_TICKS;
    800020d8:	4ba9                	li	s7,10
    for(curr = proc; curr < &proc[NPROC]; curr++) {
    800020da:	00016997          	auipc	s3,0x16
    800020de:	81698993          	addi	s3,s3,-2026 # 800178f0 <tickslock>
    p = NULL;
    800020e2:	4c01                	li	s8,0
    800020e4:	a8b1                	j	80002140 <scheduler_sjf+0xb6>
    800020e6:	8b26                	mv	s6,s1
        release(&curr->lock);
    800020e8:	8526                	mv	a0,s1
    800020ea:	fffff097          	auipc	ra,0xfffff
    800020ee:	bae080e7          	jalr	-1106(ra) # 80000c98 <release>
    for(curr = proc; curr < &proc[NPROC]; curr++) {
    800020f2:	18848493          	addi	s1,s1,392
    800020f6:	05348363          	beq	s1,s3,8000213c <scheduler_sjf+0xb2>
      if(ticks >= pause_ticks){ // check if pause signal was called
    800020fa:	00092703          	lw	a4,0(s2)
    800020fe:	000a2783          	lw	a5,0(s4)
    80002102:	fef768e3          	bltu	a4,a5,800020f2 <scheduler_sjf+0x68>
        acquire(&curr->lock);
    80002106:	8526                	mv	a0,s1
    80002108:	fffff097          	auipc	ra,0xfffff
    8000210c:	adc080e7          	jalr	-1316(ra) # 80000be4 <acquire>
        if(curr->state == RUNNABLE) {
    80002110:	4c9c                	lw	a5,24(s1)
    80002112:	fd579be3          	bne	a5,s5,800020e8 <scheduler_sjf+0x5e>
          curr->mean_ticks = ((SECONDS_TO_TICKS - RATE) * curr->mean_ticks + curr->last_ticks * (RATE)) / SECONDS_TO_TICKS;
    80002116:	58d8                	lw	a4,52(s1)
    80002118:	0027179b          	slliw	a5,a4,0x2
    8000211c:	9fb9                	addw	a5,a5,a4
    8000211e:	0017979b          	slliw	a5,a5,0x1
    80002122:	0377d7bb          	divuw	a5,a5,s7
    80002126:	0007871b          	sext.w	a4,a5
    8000212a:	d8dc                	sw	a5,52(s1)
          if(p == NULL || p->mean_ticks >= curr->mean_ticks) {
    8000212c:	fa0b0de3          	beqz	s6,800020e6 <scheduler_sjf+0x5c>
    80002130:	034b2783          	lw	a5,52(s6)
    80002134:	fae7eae3          	bltu	a5,a4,800020e8 <scheduler_sjf+0x5e>
    80002138:	8b26                	mv	s6,s1
    8000213a:	b77d                	j	800020e8 <scheduler_sjf+0x5e>
    if(p != NULL){
    8000213c:	000b1e63          	bnez	s6,80002158 <scheduler_sjf+0xce>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002140:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002144:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002148:	10079073          	csrw	sstatus,a5
    for(curr = proc; curr < &proc[NPROC]; curr++) {
    8000214c:	0000f497          	auipc	s1,0xf
    80002150:	5a448493          	addi	s1,s1,1444 # 800116f0 <proc>
    p = NULL;
    80002154:	8b62                	mv	s6,s8
    80002156:	b755                	j	800020fa <scheduler_sjf+0x70>
      acquire(&p->lock);
    80002158:	84da                	mv	s1,s6
    8000215a:	855a                	mv	a0,s6
    8000215c:	fffff097          	auipc	ra,0xfffff
    80002160:	a88080e7          	jalr	-1400(ra) # 80000be4 <acquire>
      if(p->state == RUNNABLE){
    80002164:	018b2783          	lw	a5,24(s6)
    80002168:	03579063          	bne	a5,s5,80002188 <scheduler_sjf+0xfe>
        uint start = ticks;
    8000216c:	00092c83          	lw	s9,0(s2)
        make_acquired_process_running(c, p);
    80002170:	85da                	mv	a1,s6
    80002172:	856a                	mv	a0,s10
    80002174:	00000097          	auipc	ra,0x0
    80002178:	ea8080e7          	jalr	-344(ra) # 8000201c <make_acquired_process_running>
        p->last_ticks = ticks - start;
    8000217c:	00092783          	lw	a5,0(s2)
    80002180:	419787bb          	subw	a5,a5,s9
    80002184:	02fb2c23          	sw	a5,56(s6)
      release(&p->lock);
    80002188:	8526                	mv	a0,s1
    8000218a:	fffff097          	auipc	ra,0xfffff
    8000218e:	b0e080e7          	jalr	-1266(ra) # 80000c98 <release>
    80002192:	b77d                	j	80002140 <scheduler_sjf+0xb6>

0000000080002194 <scheduler_fcfs>:
scheduler_fcfs(void) {
    80002194:	715d                	addi	sp,sp,-80
    80002196:	e486                	sd	ra,72(sp)
    80002198:	e0a2                	sd	s0,64(sp)
    8000219a:	fc26                	sd	s1,56(sp)
    8000219c:	f84a                	sd	s2,48(sp)
    8000219e:	f44e                	sd	s3,40(sp)
    800021a0:	f052                	sd	s4,32(sp)
    800021a2:	ec56                	sd	s5,24(sp)
    800021a4:	e85a                	sd	s6,16(sp)
    800021a6:	e45e                	sd	s7,8(sp)
    800021a8:	e062                	sd	s8,0(sp)
    800021aa:	0880                	addi	s0,sp,80
  asm volatile("mv %0, tp" : "=r" (x) );
    800021ac:	8792                	mv	a5,tp
  int id = r_tp();
    800021ae:	2781                	sext.w	a5,a5
  struct cpu *c = &cpus[id];
    800021b0:	079e                	slli	a5,a5,0x7
    800021b2:	0000f717          	auipc	a4,0xf
    800021b6:	13e70713          	addi	a4,a4,318 # 800112f0 <cpus>
    800021ba:	00f70c33          	add	s8,a4,a5
  c->proc = 0;
    800021be:	0000f717          	auipc	a4,0xf
    800021c2:	10270713          	addi	a4,a4,258 # 800112c0 <pid_lock>
    800021c6:	97ba                	add	a5,a5,a4
    800021c8:	0207b823          	sd	zero,48(a5)
      if(ticks >= pause_ticks){ // check if pause signal was called
    800021cc:	00007a17          	auipc	s4,0x7
    800021d0:	e84a0a13          	addi	s4,s4,-380 # 80009050 <ticks>
    800021d4:	00007997          	auipc	s3,0x7
    800021d8:	e7098993          	addi	s3,s3,-400 # 80009044 <pause_ticks>
        if(curr->state == RUNNABLE) {
    800021dc:	4a8d                	li	s5,3
    for(curr = proc; curr < &proc[NPROC]; curr++) {
    800021de:	00015917          	auipc	s2,0x15
    800021e2:	71290913          	addi	s2,s2,1810 # 800178f0 <tickslock>
    p = NULL;
    800021e6:	4b81                	li	s7,0
    800021e8:	a0a1                	j	80002230 <scheduler_fcfs+0x9c>
    800021ea:	8b26                	mv	s6,s1
        release(&curr->lock);
    800021ec:	8526                	mv	a0,s1
    800021ee:	fffff097          	auipc	ra,0xfffff
    800021f2:	aaa080e7          	jalr	-1366(ra) # 80000c98 <release>
    for(curr = proc; curr < &proc[NPROC]; curr++) {
    800021f6:	18848493          	addi	s1,s1,392
    800021fa:	03248963          	beq	s1,s2,8000222c <scheduler_fcfs+0x98>
      if(ticks >= pause_ticks){ // check if pause signal was called
    800021fe:	000a2703          	lw	a4,0(s4)
    80002202:	0009a783          	lw	a5,0(s3)
    80002206:	fef768e3          	bltu	a4,a5,800021f6 <scheduler_fcfs+0x62>
        acquire(&curr->lock);
    8000220a:	8526                	mv	a0,s1
    8000220c:	fffff097          	auipc	ra,0xfffff
    80002210:	9d8080e7          	jalr	-1576(ra) # 80000be4 <acquire>
        if(curr->state == RUNNABLE) {
    80002214:	4c9c                	lw	a5,24(s1)
    80002216:	fd579be3          	bne	a5,s5,800021ec <scheduler_fcfs+0x58>
          if(p == NULL || p->last_runnable_time > curr->last_runnable_time) {
    8000221a:	fc0b08e3          	beqz	s6,800021ea <scheduler_fcfs+0x56>
    8000221e:	03cb2703          	lw	a4,60(s6)
    80002222:	5cdc                	lw	a5,60(s1)
    80002224:	fce7f4e3          	bgeu	a5,a4,800021ec <scheduler_fcfs+0x58>
    80002228:	8b26                	mv	s6,s1
    8000222a:	b7c9                	j	800021ec <scheduler_fcfs+0x58>
    if(p != NULL){
    8000222c:	000b1e63          	bnez	s6,80002248 <scheduler_fcfs+0xb4>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002230:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002234:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002238:	10079073          	csrw	sstatus,a5
    for(curr = proc; curr < &proc[NPROC]; curr++) {
    8000223c:	0000f497          	auipc	s1,0xf
    80002240:	4b448493          	addi	s1,s1,1204 # 800116f0 <proc>
    p = NULL;
    80002244:	8b5e                	mv	s6,s7
    80002246:	bf65                	j	800021fe <scheduler_fcfs+0x6a>
      acquire(&p->lock);
    80002248:	84da                	mv	s1,s6
    8000224a:	855a                	mv	a0,s6
    8000224c:	fffff097          	auipc	ra,0xfffff
    80002250:	998080e7          	jalr	-1640(ra) # 80000be4 <acquire>
      if(p->state == RUNNABLE){
    80002254:	018b2783          	lw	a5,24(s6)
    80002258:	01579863          	bne	a5,s5,80002268 <scheduler_fcfs+0xd4>
        make_acquired_process_running(c, p);
    8000225c:	85da                	mv	a1,s6
    8000225e:	8562                	mv	a0,s8
    80002260:	00000097          	auipc	ra,0x0
    80002264:	dbc080e7          	jalr	-580(ra) # 8000201c <make_acquired_process_running>
      release(&p->lock);
    80002268:	8526                	mv	a0,s1
    8000226a:	fffff097          	auipc	ra,0xfffff
    8000226e:	a2e080e7          	jalr	-1490(ra) # 80000c98 <release>
    80002272:	bf7d                	j	80002230 <scheduler_fcfs+0x9c>

0000000080002274 <scheduler>:
{
    80002274:	1141                	addi	sp,sp,-16
    80002276:	e406                	sd	ra,8(sp)
    80002278:	e022                	sd	s0,0(sp)
    8000227a:	0800                	addi	s0,sp,16
    printf("FCFS scheduler mode\n");
    8000227c:	00006517          	auipc	a0,0x6
    80002280:	f9c50513          	addi	a0,a0,-100 # 80008218 <digits+0x1d8>
    80002284:	ffffe097          	auipc	ra,0xffffe
    80002288:	304080e7          	jalr	772(ra) # 80000588 <printf>
    scheduler_fcfs();
    8000228c:	00000097          	auipc	ra,0x0
    80002290:	f08080e7          	jalr	-248(ra) # 80002194 <scheduler_fcfs>

0000000080002294 <sched>:
{
    80002294:	7179                	addi	sp,sp,-48
    80002296:	f406                	sd	ra,40(sp)
    80002298:	f022                	sd	s0,32(sp)
    8000229a:	ec26                	sd	s1,24(sp)
    8000229c:	e84a                	sd	s2,16(sp)
    8000229e:	e44e                	sd	s3,8(sp)
    800022a0:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    800022a2:	fffff097          	auipc	ra,0xfffff
    800022a6:	73c080e7          	jalr	1852(ra) # 800019de <myproc>
    800022aa:	84aa                	mv	s1,a0
  if(!holding(&p->lock))
    800022ac:	fffff097          	auipc	ra,0xfffff
    800022b0:	8be080e7          	jalr	-1858(ra) # 80000b6a <holding>
    800022b4:	c93d                	beqz	a0,8000232a <sched+0x96>
  asm volatile("mv %0, tp" : "=r" (x) );
    800022b6:	8792                	mv	a5,tp
  if(mycpu()->noff != 1)
    800022b8:	2781                	sext.w	a5,a5
    800022ba:	079e                	slli	a5,a5,0x7
    800022bc:	0000f717          	auipc	a4,0xf
    800022c0:	00470713          	addi	a4,a4,4 # 800112c0 <pid_lock>
    800022c4:	97ba                	add	a5,a5,a4
    800022c6:	0a87a703          	lw	a4,168(a5)
    800022ca:	4785                	li	a5,1
    800022cc:	06f71763          	bne	a4,a5,8000233a <sched+0xa6>
  if(p->state == RUNNING)
    800022d0:	4c98                	lw	a4,24(s1)
    800022d2:	4791                	li	a5,4
    800022d4:	06f70b63          	beq	a4,a5,8000234a <sched+0xb6>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800022d8:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    800022dc:	8b89                	andi	a5,a5,2
  if(intr_get())
    800022de:	efb5                	bnez	a5,8000235a <sched+0xc6>
  asm volatile("mv %0, tp" : "=r" (x) );
    800022e0:	8792                	mv	a5,tp
  intena = mycpu()->intena;
    800022e2:	0000f917          	auipc	s2,0xf
    800022e6:	fde90913          	addi	s2,s2,-34 # 800112c0 <pid_lock>
    800022ea:	2781                	sext.w	a5,a5
    800022ec:	079e                	slli	a5,a5,0x7
    800022ee:	97ca                	add	a5,a5,s2
    800022f0:	0ac7a983          	lw	s3,172(a5)
    800022f4:	8792                	mv	a5,tp
  swtch(&p->context, &mycpu()->context);
    800022f6:	2781                	sext.w	a5,a5
    800022f8:	079e                	slli	a5,a5,0x7
    800022fa:	0000f597          	auipc	a1,0xf
    800022fe:	ffe58593          	addi	a1,a1,-2 # 800112f8 <cpus+0x8>
    80002302:	95be                	add	a1,a1,a5
    80002304:	08048513          	addi	a0,s1,128
    80002308:	00000097          	auipc	ra,0x0
    8000230c:	7b8080e7          	jalr	1976(ra) # 80002ac0 <swtch>
    80002310:	8792                	mv	a5,tp
  mycpu()->intena = intena;
    80002312:	2781                	sext.w	a5,a5
    80002314:	079e                	slli	a5,a5,0x7
    80002316:	97ca                	add	a5,a5,s2
    80002318:	0b37a623          	sw	s3,172(a5)
}
    8000231c:	70a2                	ld	ra,40(sp)
    8000231e:	7402                	ld	s0,32(sp)
    80002320:	64e2                	ld	s1,24(sp)
    80002322:	6942                	ld	s2,16(sp)
    80002324:	69a2                	ld	s3,8(sp)
    80002326:	6145                	addi	sp,sp,48
    80002328:	8082                	ret
    panic("sched p->lock");
    8000232a:	00006517          	auipc	a0,0x6
    8000232e:	f0650513          	addi	a0,a0,-250 # 80008230 <digits+0x1f0>
    80002332:	ffffe097          	auipc	ra,0xffffe
    80002336:	20c080e7          	jalr	524(ra) # 8000053e <panic>
    panic("sched locks");
    8000233a:	00006517          	auipc	a0,0x6
    8000233e:	f0650513          	addi	a0,a0,-250 # 80008240 <digits+0x200>
    80002342:	ffffe097          	auipc	ra,0xffffe
    80002346:	1fc080e7          	jalr	508(ra) # 8000053e <panic>
    panic("sched running");
    8000234a:	00006517          	auipc	a0,0x6
    8000234e:	f0650513          	addi	a0,a0,-250 # 80008250 <digits+0x210>
    80002352:	ffffe097          	auipc	ra,0xffffe
    80002356:	1ec080e7          	jalr	492(ra) # 8000053e <panic>
    panic("sched interruptible");
    8000235a:	00006517          	auipc	a0,0x6
    8000235e:	f0650513          	addi	a0,a0,-250 # 80008260 <digits+0x220>
    80002362:	ffffe097          	auipc	ra,0xffffe
    80002366:	1dc080e7          	jalr	476(ra) # 8000053e <panic>

000000008000236a <yield>:
{
    8000236a:	1101                	addi	sp,sp,-32
    8000236c:	ec06                	sd	ra,24(sp)
    8000236e:	e822                	sd	s0,16(sp)
    80002370:	e426                	sd	s1,8(sp)
    80002372:	1000                	addi	s0,sp,32
  struct proc *p = myproc();
    80002374:	fffff097          	auipc	ra,0xfffff
    80002378:	66a080e7          	jalr	1642(ra) # 800019de <myproc>
    8000237c:	84aa                	mv	s1,a0
  acquire(&p->lock);
    8000237e:	fffff097          	auipc	ra,0xfffff
    80002382:	866080e7          	jalr	-1946(ra) # 80000be4 <acquire>
  p->state = RUNNABLE;
    80002386:	478d                	li	a5,3
    80002388:	cc9c                	sw	a5,24(s1)
    p->last_runnable_time = ticks;
    8000238a:	00007797          	auipc	a5,0x7
    8000238e:	cc67a783          	lw	a5,-826(a5) # 80009050 <ticks>
    80002392:	dcdc                	sw	a5,60(s1)
  sched();
    80002394:	00000097          	auipc	ra,0x0
    80002398:	f00080e7          	jalr	-256(ra) # 80002294 <sched>
  release(&p->lock);
    8000239c:	8526                	mv	a0,s1
    8000239e:	fffff097          	auipc	ra,0xfffff
    800023a2:	8fa080e7          	jalr	-1798(ra) # 80000c98 <release>
}
    800023a6:	60e2                	ld	ra,24(sp)
    800023a8:	6442                	ld	s0,16(sp)
    800023aa:	64a2                	ld	s1,8(sp)
    800023ac:	6105                	addi	sp,sp,32
    800023ae:	8082                	ret

00000000800023b0 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
    800023b0:	7179                	addi	sp,sp,-48
    800023b2:	f406                	sd	ra,40(sp)
    800023b4:	f022                	sd	s0,32(sp)
    800023b6:	ec26                	sd	s1,24(sp)
    800023b8:	e84a                	sd	s2,16(sp)
    800023ba:	e44e                	sd	s3,8(sp)
    800023bc:	1800                	addi	s0,sp,48
    800023be:	89aa                	mv	s3,a0
    800023c0:	892e                	mv	s2,a1
  struct proc *p = myproc();
    800023c2:	fffff097          	auipc	ra,0xfffff
    800023c6:	61c080e7          	jalr	1564(ra) # 800019de <myproc>
    800023ca:	84aa                	mv	s1,a0
  // Once we hold p->lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup locks p->lock),
  // so it's okay to release lk.

  acquire(&p->lock);  //DOC: sleeplock1
    800023cc:	fffff097          	auipc	ra,0xfffff
    800023d0:	818080e7          	jalr	-2024(ra) # 80000be4 <acquire>
  release(lk);
    800023d4:	854a                	mv	a0,s2
    800023d6:	fffff097          	auipc	ra,0xfffff
    800023da:	8c2080e7          	jalr	-1854(ra) # 80000c98 <release>

  // Go to sleep.
  p->chan = chan;
    800023de:	0334b023          	sd	s3,32(s1)
  p->state = SLEEPING;
    800023e2:	4789                	li	a5,2
    800023e4:	cc9c                	sw	a5,24(s1)

  //update start sleep time
  p->start_last_sleeping = ticks;
    800023e6:	00007797          	auipc	a5,0x7
    800023ea:	c6a7a783          	lw	a5,-918(a5) # 80009050 <ticks>
    800023ee:	c4fc                	sw	a5,76(s1)

  sched();
    800023f0:	00000097          	auipc	ra,0x0
    800023f4:	ea4080e7          	jalr	-348(ra) # 80002294 <sched>

  // Tidy up.
  p->chan = 0;
    800023f8:	0204b023          	sd	zero,32(s1)

  // Reacquire original lock.
  release(&p->lock);
    800023fc:	8526                	mv	a0,s1
    800023fe:	fffff097          	auipc	ra,0xfffff
    80002402:	89a080e7          	jalr	-1894(ra) # 80000c98 <release>
  acquire(lk);
    80002406:	854a                	mv	a0,s2
    80002408:	ffffe097          	auipc	ra,0xffffe
    8000240c:	7dc080e7          	jalr	2012(ra) # 80000be4 <acquire>
}
    80002410:	70a2                	ld	ra,40(sp)
    80002412:	7402                	ld	s0,32(sp)
    80002414:	64e2                	ld	s1,24(sp)
    80002416:	6942                	ld	s2,16(sp)
    80002418:	69a2                	ld	s3,8(sp)
    8000241a:	6145                	addi	sp,sp,48
    8000241c:	8082                	ret

000000008000241e <wait>:
{
    8000241e:	715d                	addi	sp,sp,-80
    80002420:	e486                	sd	ra,72(sp)
    80002422:	e0a2                	sd	s0,64(sp)
    80002424:	fc26                	sd	s1,56(sp)
    80002426:	f84a                	sd	s2,48(sp)
    80002428:	f44e                	sd	s3,40(sp)
    8000242a:	f052                	sd	s4,32(sp)
    8000242c:	ec56                	sd	s5,24(sp)
    8000242e:	e85a                	sd	s6,16(sp)
    80002430:	e45e                	sd	s7,8(sp)
    80002432:	e062                	sd	s8,0(sp)
    80002434:	0880                	addi	s0,sp,80
    80002436:	8b2a                	mv	s6,a0
  struct proc *p = myproc();
    80002438:	fffff097          	auipc	ra,0xfffff
    8000243c:	5a6080e7          	jalr	1446(ra) # 800019de <myproc>
    80002440:	892a                	mv	s2,a0
  acquire(&wait_lock);
    80002442:	0000f517          	auipc	a0,0xf
    80002446:	e9650513          	addi	a0,a0,-362 # 800112d8 <wait_lock>
    8000244a:	ffffe097          	auipc	ra,0xffffe
    8000244e:	79a080e7          	jalr	1946(ra) # 80000be4 <acquire>
    havekids = 0;
    80002452:	4b81                	li	s7,0
        if(np->state == ZOMBIE){
    80002454:	4a15                	li	s4,5
    for(np = proc; np < &proc[NPROC]; np++){
    80002456:	00015997          	auipc	s3,0x15
    8000245a:	49a98993          	addi	s3,s3,1178 # 800178f0 <tickslock>
        havekids = 1;
    8000245e:	4a85                	li	s5,1
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002460:	0000fc17          	auipc	s8,0xf
    80002464:	e78c0c13          	addi	s8,s8,-392 # 800112d8 <wait_lock>
    havekids = 0;
    80002468:	875e                	mv	a4,s7
    for(np = proc; np < &proc[NPROC]; np++){
    8000246a:	0000f497          	auipc	s1,0xf
    8000246e:	28648493          	addi	s1,s1,646 # 800116f0 <proc>
    80002472:	a0bd                	j	800024e0 <wait+0xc2>
          pid = np->pid;
    80002474:	0304a983          	lw	s3,48(s1)
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&np->xstate,
    80002478:	000b0e63          	beqz	s6,80002494 <wait+0x76>
    8000247c:	4691                	li	a3,4
    8000247e:	02c48613          	addi	a2,s1,44
    80002482:	85da                	mv	a1,s6
    80002484:	07093503          	ld	a0,112(s2)
    80002488:	fffff097          	auipc	ra,0xfffff
    8000248c:	1f2080e7          	jalr	498(ra) # 8000167a <copyout>
    80002490:	02054563          	bltz	a0,800024ba <wait+0x9c>
          freeproc(np);
    80002494:	8526                	mv	a0,s1
    80002496:	fffff097          	auipc	ra,0xfffff
    8000249a:	6fa080e7          	jalr	1786(ra) # 80001b90 <freeproc>
          release(&np->lock);
    8000249e:	8526                	mv	a0,s1
    800024a0:	ffffe097          	auipc	ra,0xffffe
    800024a4:	7f8080e7          	jalr	2040(ra) # 80000c98 <release>
          release(&wait_lock);
    800024a8:	0000f517          	auipc	a0,0xf
    800024ac:	e3050513          	addi	a0,a0,-464 # 800112d8 <wait_lock>
    800024b0:	ffffe097          	auipc	ra,0xffffe
    800024b4:	7e8080e7          	jalr	2024(ra) # 80000c98 <release>
          return pid;
    800024b8:	a09d                	j	8000251e <wait+0x100>
            release(&np->lock);
    800024ba:	8526                	mv	a0,s1
    800024bc:	ffffe097          	auipc	ra,0xffffe
    800024c0:	7dc080e7          	jalr	2012(ra) # 80000c98 <release>
            release(&wait_lock);
    800024c4:	0000f517          	auipc	a0,0xf
    800024c8:	e1450513          	addi	a0,a0,-492 # 800112d8 <wait_lock>
    800024cc:	ffffe097          	auipc	ra,0xffffe
    800024d0:	7cc080e7          	jalr	1996(ra) # 80000c98 <release>
            return -1;
    800024d4:	59fd                	li	s3,-1
    800024d6:	a0a1                	j	8000251e <wait+0x100>
    for(np = proc; np < &proc[NPROC]; np++){
    800024d8:	18848493          	addi	s1,s1,392
    800024dc:	03348463          	beq	s1,s3,80002504 <wait+0xe6>
      if(np->parent == p){
    800024e0:	6cbc                	ld	a5,88(s1)
    800024e2:	ff279be3          	bne	a5,s2,800024d8 <wait+0xba>
        acquire(&np->lock);
    800024e6:	8526                	mv	a0,s1
    800024e8:	ffffe097          	auipc	ra,0xffffe
    800024ec:	6fc080e7          	jalr	1788(ra) # 80000be4 <acquire>
        if(np->state == ZOMBIE){
    800024f0:	4c9c                	lw	a5,24(s1)
    800024f2:	f94781e3          	beq	a5,s4,80002474 <wait+0x56>
        release(&np->lock);
    800024f6:	8526                	mv	a0,s1
    800024f8:	ffffe097          	auipc	ra,0xffffe
    800024fc:	7a0080e7          	jalr	1952(ra) # 80000c98 <release>
        havekids = 1;
    80002500:	8756                	mv	a4,s5
    80002502:	bfd9                	j	800024d8 <wait+0xba>
    if(!havekids || p->killed){
    80002504:	c701                	beqz	a4,8000250c <wait+0xee>
    80002506:	02892783          	lw	a5,40(s2)
    8000250a:	c79d                	beqz	a5,80002538 <wait+0x11a>
      release(&wait_lock);
    8000250c:	0000f517          	auipc	a0,0xf
    80002510:	dcc50513          	addi	a0,a0,-564 # 800112d8 <wait_lock>
    80002514:	ffffe097          	auipc	ra,0xffffe
    80002518:	784080e7          	jalr	1924(ra) # 80000c98 <release>
      return -1;
    8000251c:	59fd                	li	s3,-1
}
    8000251e:	854e                	mv	a0,s3
    80002520:	60a6                	ld	ra,72(sp)
    80002522:	6406                	ld	s0,64(sp)
    80002524:	74e2                	ld	s1,56(sp)
    80002526:	7942                	ld	s2,48(sp)
    80002528:	79a2                	ld	s3,40(sp)
    8000252a:	7a02                	ld	s4,32(sp)
    8000252c:	6ae2                	ld	s5,24(sp)
    8000252e:	6b42                	ld	s6,16(sp)
    80002530:	6ba2                	ld	s7,8(sp)
    80002532:	6c02                	ld	s8,0(sp)
    80002534:	6161                	addi	sp,sp,80
    80002536:	8082                	ret
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002538:	85e2                	mv	a1,s8
    8000253a:	854a                	mv	a0,s2
    8000253c:	00000097          	auipc	ra,0x0
    80002540:	e74080e7          	jalr	-396(ra) # 800023b0 <sleep>
    havekids = 0;
    80002544:	b715                	j	80002468 <wait+0x4a>

0000000080002546 <wakeup>:

// Wake up all processes sleeping on chan.
// Must be called without any p->lock.
void
wakeup(void *chan)
{
    80002546:	7139                	addi	sp,sp,-64
    80002548:	fc06                	sd	ra,56(sp)
    8000254a:	f822                	sd	s0,48(sp)
    8000254c:	f426                	sd	s1,40(sp)
    8000254e:	f04a                	sd	s2,32(sp)
    80002550:	ec4e                	sd	s3,24(sp)
    80002552:	e852                	sd	s4,16(sp)
    80002554:	e456                	sd	s5,8(sp)
    80002556:	e05a                	sd	s6,0(sp)
    80002558:	0080                	addi	s0,sp,64
    8000255a:	8a2a                	mv	s4,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++) {
    8000255c:	0000f497          	auipc	s1,0xf
    80002560:	19448493          	addi	s1,s1,404 # 800116f0 <proc>
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
    80002564:	4989                	li	s3,2
        p->sleeping_time += ticks - p->start_last_sleeping;
    80002566:	00007b17          	auipc	s6,0x7
    8000256a:	aeab0b13          	addi	s6,s6,-1302 # 80009050 <ticks>
        p->start_last_runnable = ticks;
        p->state = RUNNABLE;
    8000256e:	4a8d                	li	s5,3
  for(p = proc; p < &proc[NPROC]; p++) {
    80002570:	00015917          	auipc	s2,0x15
    80002574:	38090913          	addi	s2,s2,896 # 800178f0 <tickslock>
    80002578:	a811                	j	8000258c <wakeup+0x46>
        update_last_runnable_time(p);
      }
      release(&p->lock);
    8000257a:	8526                	mv	a0,s1
    8000257c:	ffffe097          	auipc	ra,0xffffe
    80002580:	71c080e7          	jalr	1820(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80002584:	18848493          	addi	s1,s1,392
    80002588:	03248f63          	beq	s1,s2,800025c6 <wakeup+0x80>
    if(p != myproc()){
    8000258c:	fffff097          	auipc	ra,0xfffff
    80002590:	452080e7          	jalr	1106(ra) # 800019de <myproc>
    80002594:	fea488e3          	beq	s1,a0,80002584 <wakeup+0x3e>
      acquire(&p->lock);
    80002598:	8526                	mv	a0,s1
    8000259a:	ffffe097          	auipc	ra,0xffffe
    8000259e:	64a080e7          	jalr	1610(ra) # 80000be4 <acquire>
      if(p->state == SLEEPING && p->chan == chan) {
    800025a2:	4c9c                	lw	a5,24(s1)
    800025a4:	fd379be3          	bne	a5,s3,8000257a <wakeup+0x34>
    800025a8:	709c                	ld	a5,32(s1)
    800025aa:	fd4798e3          	bne	a5,s4,8000257a <wakeup+0x34>
        p->sleeping_time += ticks - p->start_last_sleeping;
    800025ae:	000b2703          	lw	a4,0(s6)
    800025b2:	40bc                	lw	a5,64(s1)
    800025b4:	9fb9                	addw	a5,a5,a4
    800025b6:	44f4                	lw	a3,76(s1)
    800025b8:	9f95                	subw	a5,a5,a3
    800025ba:	c0bc                	sw	a5,64(s1)
        p->start_last_runnable = ticks;
    800025bc:	c8b8                	sw	a4,80(s1)
        p->state = RUNNABLE;
    800025be:	0154ac23          	sw	s5,24(s1)
    p->last_runnable_time = ticks;
    800025c2:	dcd8                	sw	a4,60(s1)
}  
    800025c4:	bf5d                	j	8000257a <wakeup+0x34>
    }
  }
}
    800025c6:	70e2                	ld	ra,56(sp)
    800025c8:	7442                	ld	s0,48(sp)
    800025ca:	74a2                	ld	s1,40(sp)
    800025cc:	7902                	ld	s2,32(sp)
    800025ce:	69e2                	ld	s3,24(sp)
    800025d0:	6a42                	ld	s4,16(sp)
    800025d2:	6aa2                	ld	s5,8(sp)
    800025d4:	6b02                	ld	s6,0(sp)
    800025d6:	6121                	addi	sp,sp,64
    800025d8:	8082                	ret

00000000800025da <reparent>:
{
    800025da:	7179                	addi	sp,sp,-48
    800025dc:	f406                	sd	ra,40(sp)
    800025de:	f022                	sd	s0,32(sp)
    800025e0:	ec26                	sd	s1,24(sp)
    800025e2:	e84a                	sd	s2,16(sp)
    800025e4:	e44e                	sd	s3,8(sp)
    800025e6:	e052                	sd	s4,0(sp)
    800025e8:	1800                	addi	s0,sp,48
    800025ea:	892a                	mv	s2,a0
  for(pp = proc; pp < &proc[NPROC]; pp++){
    800025ec:	0000f497          	auipc	s1,0xf
    800025f0:	10448493          	addi	s1,s1,260 # 800116f0 <proc>
      pp->parent = initproc;
    800025f4:	00007a17          	auipc	s4,0x7
    800025f8:	a54a0a13          	addi	s4,s4,-1452 # 80009048 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    800025fc:	00015997          	auipc	s3,0x15
    80002600:	2f498993          	addi	s3,s3,756 # 800178f0 <tickslock>
    80002604:	a029                	j	8000260e <reparent+0x34>
    80002606:	18848493          	addi	s1,s1,392
    8000260a:	01348d63          	beq	s1,s3,80002624 <reparent+0x4a>
    if(pp->parent == p){
    8000260e:	6cbc                	ld	a5,88(s1)
    80002610:	ff279be3          	bne	a5,s2,80002606 <reparent+0x2c>
      pp->parent = initproc;
    80002614:	000a3503          	ld	a0,0(s4)
    80002618:	eca8                	sd	a0,88(s1)
      wakeup(initproc);
    8000261a:	00000097          	auipc	ra,0x0
    8000261e:	f2c080e7          	jalr	-212(ra) # 80002546 <wakeup>
    80002622:	b7d5                	j	80002606 <reparent+0x2c>
}
    80002624:	70a2                	ld	ra,40(sp)
    80002626:	7402                	ld	s0,32(sp)
    80002628:	64e2                	ld	s1,24(sp)
    8000262a:	6942                	ld	s2,16(sp)
    8000262c:	69a2                	ld	s3,8(sp)
    8000262e:	6a02                	ld	s4,0(sp)
    80002630:	6145                	addi	sp,sp,48
    80002632:	8082                	ret

0000000080002634 <exit>:
{
    80002634:	7179                	addi	sp,sp,-48
    80002636:	f406                	sd	ra,40(sp)
    80002638:	f022                	sd	s0,32(sp)
    8000263a:	ec26                	sd	s1,24(sp)
    8000263c:	e84a                	sd	s2,16(sp)
    8000263e:	e44e                	sd	s3,8(sp)
    80002640:	e052                	sd	s4,0(sp)
    80002642:	1800                	addi	s0,sp,48
    80002644:	8a2a                	mv	s4,a0
  struct proc *p = myproc();
    80002646:	fffff097          	auipc	ra,0xfffff
    8000264a:	398080e7          	jalr	920(ra) # 800019de <myproc>
    8000264e:	892a                	mv	s2,a0
  sleeping_processes_mean = get_mean(sleeping_processes_mean, p->sleeping_time, number_processes);
    80002650:	00007517          	auipc	a0,0x7
    80002654:	9d850513          	addi	a0,a0,-1576 # 80009028 <number_processes>
    80002658:	4110                	lw	a2,0(a0)
  return ( last_mean * num_procs + curr_time) / (num_procs + 1);
    8000265a:	0016069b          	addiw	a3,a2,1
  sleeping_processes_mean = get_mean(sleeping_processes_mean, p->sleeping_time, number_processes);
    8000265e:	00007797          	auipc	a5,0x7
    80002662:	9e278793          	addi	a5,a5,-1566 # 80009040 <sleeping_processes_mean>
  return ( last_mean * num_procs + curr_time) / (num_procs + 1);
    80002666:	4398                	lw	a4,0(a5)
    80002668:	02c7073b          	mulw	a4,a4,a2
    8000266c:	04092583          	lw	a1,64(s2)
    80002670:	9f2d                	addw	a4,a4,a1
    80002672:	02d7473b          	divw	a4,a4,a3
  sleeping_processes_mean = get_mean(sleeping_processes_mean, p->sleeping_time, number_processes);
    80002676:	c398                	sw	a4,0(a5)
  running_processes_mean = get_mean(running_processes_mean, p->running_time, number_processes);
    80002678:	04892583          	lw	a1,72(s2)
    8000267c:	00007797          	auipc	a5,0x7
    80002680:	9c078793          	addi	a5,a5,-1600 # 8000903c <running_processes_mean>
  return ( last_mean * num_procs + curr_time) / (num_procs + 1);
    80002684:	4398                	lw	a4,0(a5)
    80002686:	02c7073b          	mulw	a4,a4,a2
    8000268a:	9f2d                	addw	a4,a4,a1
    8000268c:	02d7473b          	divw	a4,a4,a3
  running_processes_mean = get_mean(running_processes_mean, p->running_time, number_processes);
    80002690:	c398                	sw	a4,0(a5)
  runnable_processes_mean = get_mean(runnable_processes_mean, p->runnable_time, number_processes);
    80002692:	00007717          	auipc	a4,0x7
    80002696:	9a670713          	addi	a4,a4,-1626 # 80009038 <runnable_processes_mean>
  return ( last_mean * num_procs + curr_time) / (num_procs + 1);
    8000269a:	431c                	lw	a5,0(a4)
    8000269c:	02c787bb          	mulw	a5,a5,a2
    800026a0:	04492603          	lw	a2,68(s2)
    800026a4:	9fb1                	addw	a5,a5,a2
    800026a6:	02d7c7bb          	divw	a5,a5,a3
  runnable_processes_mean = get_mean(runnable_processes_mean, p->runnable_time, number_processes);
    800026aa:	c31c                	sw	a5,0(a4)
  number_processes++;
    800026ac:	c114                	sw	a3,0(a0)
  program_time += p->running_time;
    800026ae:	00007697          	auipc	a3,0x7
    800026b2:	98668693          	addi	a3,a3,-1658 # 80009034 <program_time>
    800026b6:	429c                	lw	a5,0(a3)
    800026b8:	00b7873b          	addw	a4,a5,a1
    800026bc:	c298                	sw	a4,0(a3)
  cpu_utilization = 100*program_time / (ticks-start_time);
    800026be:	06400793          	li	a5,100
    800026c2:	02e787bb          	mulw	a5,a5,a4
    800026c6:	00007717          	auipc	a4,0x7
    800026ca:	98a72703          	lw	a4,-1654(a4) # 80009050 <ticks>
    800026ce:	00007697          	auipc	a3,0x7
    800026d2:	95e6a683          	lw	a3,-1698(a3) # 8000902c <start_time>
    800026d6:	9f15                	subw	a4,a4,a3
    800026d8:	02e7d7bb          	divuw	a5,a5,a4
    800026dc:	00007717          	auipc	a4,0x7
    800026e0:	94f72a23          	sw	a5,-1708(a4) # 80009030 <cpu_utilization>
  if(p == initproc)
    800026e4:	00007797          	auipc	a5,0x7
    800026e8:	9647b783          	ld	a5,-1692(a5) # 80009048 <initproc>
    800026ec:	0f090493          	addi	s1,s2,240
    800026f0:	17090993          	addi	s3,s2,368
    800026f4:	03279363          	bne	a5,s2,8000271a <exit+0xe6>
    panic("init exiting");
    800026f8:	00006517          	auipc	a0,0x6
    800026fc:	b8050513          	addi	a0,a0,-1152 # 80008278 <digits+0x238>
    80002700:	ffffe097          	auipc	ra,0xffffe
    80002704:	e3e080e7          	jalr	-450(ra) # 8000053e <panic>
      fileclose(f);
    80002708:	00002097          	auipc	ra,0x2
    8000270c:	320080e7          	jalr	800(ra) # 80004a28 <fileclose>
      p->ofile[fd] = 0;
    80002710:	0004b023          	sd	zero,0(s1)
  for(int fd = 0; fd < NOFILE; fd++){
    80002714:	04a1                	addi	s1,s1,8
    80002716:	01348563          	beq	s1,s3,80002720 <exit+0xec>
    if(p->ofile[fd]){
    8000271a:	6088                	ld	a0,0(s1)
    8000271c:	f575                	bnez	a0,80002708 <exit+0xd4>
    8000271e:	bfdd                	j	80002714 <exit+0xe0>
  begin_op();
    80002720:	00002097          	auipc	ra,0x2
    80002724:	e3c080e7          	jalr	-452(ra) # 8000455c <begin_op>
  iput(p->cwd);
    80002728:	17093503          	ld	a0,368(s2)
    8000272c:	00001097          	auipc	ra,0x1
    80002730:	618080e7          	jalr	1560(ra) # 80003d44 <iput>
  end_op();
    80002734:	00002097          	auipc	ra,0x2
    80002738:	ea8080e7          	jalr	-344(ra) # 800045dc <end_op>
  p->cwd = 0;
    8000273c:	16093823          	sd	zero,368(s2)
  acquire(&wait_lock);
    80002740:	0000f497          	auipc	s1,0xf
    80002744:	b9848493          	addi	s1,s1,-1128 # 800112d8 <wait_lock>
    80002748:	8526                	mv	a0,s1
    8000274a:	ffffe097          	auipc	ra,0xffffe
    8000274e:	49a080e7          	jalr	1178(ra) # 80000be4 <acquire>
  reparent(p);
    80002752:	854a                	mv	a0,s2
    80002754:	00000097          	auipc	ra,0x0
    80002758:	e86080e7          	jalr	-378(ra) # 800025da <reparent>
  wakeup(p->parent);
    8000275c:	05893503          	ld	a0,88(s2)
    80002760:	00000097          	auipc	ra,0x0
    80002764:	de6080e7          	jalr	-538(ra) # 80002546 <wakeup>
  acquire(&p->lock);
    80002768:	854a                	mv	a0,s2
    8000276a:	ffffe097          	auipc	ra,0xffffe
    8000276e:	47a080e7          	jalr	1146(ra) # 80000be4 <acquire>
  p->xstate = status;
    80002772:	03492623          	sw	s4,44(s2)
  p->state = ZOMBIE;
    80002776:	4795                	li	a5,5
    80002778:	00f92c23          	sw	a5,24(s2)
  release(&wait_lock);
    8000277c:	8526                	mv	a0,s1
    8000277e:	ffffe097          	auipc	ra,0xffffe
    80002782:	51a080e7          	jalr	1306(ra) # 80000c98 <release>
  sched();
    80002786:	00000097          	auipc	ra,0x0
    8000278a:	b0e080e7          	jalr	-1266(ra) # 80002294 <sched>
  panic("zombie exit");
    8000278e:	00006517          	auipc	a0,0x6
    80002792:	afa50513          	addi	a0,a0,-1286 # 80008288 <digits+0x248>
    80002796:	ffffe097          	auipc	ra,0xffffe
    8000279a:	da8080e7          	jalr	-600(ra) # 8000053e <panic>

000000008000279e <kill>:
// Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).
int
kill(int pid)
{
    8000279e:	7179                	addi	sp,sp,-48
    800027a0:	f406                	sd	ra,40(sp)
    800027a2:	f022                	sd	s0,32(sp)
    800027a4:	ec26                	sd	s1,24(sp)
    800027a6:	e84a                	sd	s2,16(sp)
    800027a8:	e44e                	sd	s3,8(sp)
    800027aa:	1800                	addi	s0,sp,48
    800027ac:	892a                	mv	s2,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++){
    800027ae:	0000f497          	auipc	s1,0xf
    800027b2:	f4248493          	addi	s1,s1,-190 # 800116f0 <proc>
    800027b6:	00015997          	auipc	s3,0x15
    800027ba:	13a98993          	addi	s3,s3,314 # 800178f0 <tickslock>
    acquire(&p->lock);
    800027be:	8526                	mv	a0,s1
    800027c0:	ffffe097          	auipc	ra,0xffffe
    800027c4:	424080e7          	jalr	1060(ra) # 80000be4 <acquire>
    if(p->pid == pid){
    800027c8:	589c                	lw	a5,48(s1)
    800027ca:	01278d63          	beq	a5,s2,800027e4 <kill+0x46>
        update_last_runnable_time(p);
      }
      release(&p->lock);
      return 0;
    }
    release(&p->lock);
    800027ce:	8526                	mv	a0,s1
    800027d0:	ffffe097          	auipc	ra,0xffffe
    800027d4:	4c8080e7          	jalr	1224(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    800027d8:	18848493          	addi	s1,s1,392
    800027dc:	ff3491e3          	bne	s1,s3,800027be <kill+0x20>
  }
  return -1;
    800027e0:	557d                	li	a0,-1
    800027e2:	a829                	j	800027fc <kill+0x5e>
      p->killed = 1;
    800027e4:	4785                	li	a5,1
    800027e6:	d49c                	sw	a5,40(s1)
      if(p->state == SLEEPING){
    800027e8:	4c98                	lw	a4,24(s1)
    800027ea:	4789                	li	a5,2
    800027ec:	00f70f63          	beq	a4,a5,8000280a <kill+0x6c>
      release(&p->lock);
    800027f0:	8526                	mv	a0,s1
    800027f2:	ffffe097          	auipc	ra,0xffffe
    800027f6:	4a6080e7          	jalr	1190(ra) # 80000c98 <release>
      return 0;
    800027fa:	4501                	li	a0,0
}
    800027fc:	70a2                	ld	ra,40(sp)
    800027fe:	7402                	ld	s0,32(sp)
    80002800:	64e2                	ld	s1,24(sp)
    80002802:	6942                	ld	s2,16(sp)
    80002804:	69a2                	ld	s3,8(sp)
    80002806:	6145                	addi	sp,sp,48
    80002808:	8082                	ret
        p->sleeping_time += ticks - p->start_last_sleeping;
    8000280a:	00007717          	auipc	a4,0x7
    8000280e:	84672703          	lw	a4,-1978(a4) # 80009050 <ticks>
    80002812:	40bc                	lw	a5,64(s1)
    80002814:	9fb9                	addw	a5,a5,a4
    80002816:	44f4                	lw	a3,76(s1)
    80002818:	9f95                	subw	a5,a5,a3
    8000281a:	c0bc                	sw	a5,64(s1)
        p->start_last_runnable = ticks;
    8000281c:	c8b8                	sw	a4,80(s1)
        p->state = RUNNABLE;
    8000281e:	478d                	li	a5,3
    80002820:	cc9c                	sw	a5,24(s1)
    p->last_runnable_time = ticks;
    80002822:	dcd8                	sw	a4,60(s1)
}  
    80002824:	b7f1                	j	800027f0 <kill+0x52>

0000000080002826 <either_copyout>:
// Copy to either a user address, or kernel address,
// depending on usr_dst.
// Returns 0 on success, -1 on error.
int
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
    80002826:	7179                	addi	sp,sp,-48
    80002828:	f406                	sd	ra,40(sp)
    8000282a:	f022                	sd	s0,32(sp)
    8000282c:	ec26                	sd	s1,24(sp)
    8000282e:	e84a                	sd	s2,16(sp)
    80002830:	e44e                	sd	s3,8(sp)
    80002832:	e052                	sd	s4,0(sp)
    80002834:	1800                	addi	s0,sp,48
    80002836:	84aa                	mv	s1,a0
    80002838:	892e                	mv	s2,a1
    8000283a:	89b2                	mv	s3,a2
    8000283c:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    8000283e:	fffff097          	auipc	ra,0xfffff
    80002842:	1a0080e7          	jalr	416(ra) # 800019de <myproc>
  if(user_dst){
    80002846:	c08d                	beqz	s1,80002868 <either_copyout+0x42>
    return copyout(p->pagetable, dst, src, len);
    80002848:	86d2                	mv	a3,s4
    8000284a:	864e                	mv	a2,s3
    8000284c:	85ca                	mv	a1,s2
    8000284e:	7928                	ld	a0,112(a0)
    80002850:	fffff097          	auipc	ra,0xfffff
    80002854:	e2a080e7          	jalr	-470(ra) # 8000167a <copyout>
  } else {
    memmove((char *)dst, src, len);
    return 0;
  }
}
    80002858:	70a2                	ld	ra,40(sp)
    8000285a:	7402                	ld	s0,32(sp)
    8000285c:	64e2                	ld	s1,24(sp)
    8000285e:	6942                	ld	s2,16(sp)
    80002860:	69a2                	ld	s3,8(sp)
    80002862:	6a02                	ld	s4,0(sp)
    80002864:	6145                	addi	sp,sp,48
    80002866:	8082                	ret
    memmove((char *)dst, src, len);
    80002868:	000a061b          	sext.w	a2,s4
    8000286c:	85ce                	mv	a1,s3
    8000286e:	854a                	mv	a0,s2
    80002870:	ffffe097          	auipc	ra,0xffffe
    80002874:	4d0080e7          	jalr	1232(ra) # 80000d40 <memmove>
    return 0;
    80002878:	8526                	mv	a0,s1
    8000287a:	bff9                	j	80002858 <either_copyout+0x32>

000000008000287c <either_copyin>:
// Copy from either a user address, or kernel address,
// depending on usr_src.
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
    8000287c:	7179                	addi	sp,sp,-48
    8000287e:	f406                	sd	ra,40(sp)
    80002880:	f022                	sd	s0,32(sp)
    80002882:	ec26                	sd	s1,24(sp)
    80002884:	e84a                	sd	s2,16(sp)
    80002886:	e44e                	sd	s3,8(sp)
    80002888:	e052                	sd	s4,0(sp)
    8000288a:	1800                	addi	s0,sp,48
    8000288c:	892a                	mv	s2,a0
    8000288e:	84ae                	mv	s1,a1
    80002890:	89b2                	mv	s3,a2
    80002892:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    80002894:	fffff097          	auipc	ra,0xfffff
    80002898:	14a080e7          	jalr	330(ra) # 800019de <myproc>
  if(user_src){
    8000289c:	c08d                	beqz	s1,800028be <either_copyin+0x42>
    return copyin(p->pagetable, dst, src, len);
    8000289e:	86d2                	mv	a3,s4
    800028a0:	864e                	mv	a2,s3
    800028a2:	85ca                	mv	a1,s2
    800028a4:	7928                	ld	a0,112(a0)
    800028a6:	fffff097          	auipc	ra,0xfffff
    800028aa:	e60080e7          	jalr	-416(ra) # 80001706 <copyin>
  } else {
    memmove(dst, (char*)src, len);
    return 0;
  }
}
    800028ae:	70a2                	ld	ra,40(sp)
    800028b0:	7402                	ld	s0,32(sp)
    800028b2:	64e2                	ld	s1,24(sp)
    800028b4:	6942                	ld	s2,16(sp)
    800028b6:	69a2                	ld	s3,8(sp)
    800028b8:	6a02                	ld	s4,0(sp)
    800028ba:	6145                	addi	sp,sp,48
    800028bc:	8082                	ret
    memmove(dst, (char*)src, len);
    800028be:	000a061b          	sext.w	a2,s4
    800028c2:	85ce                	mv	a1,s3
    800028c4:	854a                	mv	a0,s2
    800028c6:	ffffe097          	auipc	ra,0xffffe
    800028ca:	47a080e7          	jalr	1146(ra) # 80000d40 <memmove>
    return 0;
    800028ce:	8526                	mv	a0,s1
    800028d0:	bff9                	j	800028ae <either_copyin+0x32>

00000000800028d2 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
    800028d2:	715d                	addi	sp,sp,-80
    800028d4:	e486                	sd	ra,72(sp)
    800028d6:	e0a2                	sd	s0,64(sp)
    800028d8:	fc26                	sd	s1,56(sp)
    800028da:	f84a                	sd	s2,48(sp)
    800028dc:	f44e                	sd	s3,40(sp)
    800028de:	f052                	sd	s4,32(sp)
    800028e0:	ec56                	sd	s5,24(sp)
    800028e2:	e85a                	sd	s6,16(sp)
    800028e4:	e45e                	sd	s7,8(sp)
    800028e6:	0880                	addi	s0,sp,80
  [ZOMBIE]    "zombie"
  };
  struct proc *p;
  char *state;

  printf("\n");
    800028e8:	00006517          	auipc	a0,0x6
    800028ec:	a3850513          	addi	a0,a0,-1480 # 80008320 <digits+0x2e0>
    800028f0:	ffffe097          	auipc	ra,0xffffe
    800028f4:	c98080e7          	jalr	-872(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    800028f8:	0000f497          	auipc	s1,0xf
    800028fc:	f7048493          	addi	s1,s1,-144 # 80011868 <proc+0x178>
    80002900:	00015917          	auipc	s2,0x15
    80002904:	16890913          	addi	s2,s2,360 # 80017a68 <bcache+0x160>
    if(p->state == UNUSED)
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002908:	4b15                	li	s6,5
      state = states[p->state];
    else
      state = "???";
    8000290a:	00006997          	auipc	s3,0x6
    8000290e:	98e98993          	addi	s3,s3,-1650 # 80008298 <digits+0x258>
    printf("%d %s %s", p->pid, state, p->name);
    80002912:	00006a97          	auipc	s5,0x6
    80002916:	98ea8a93          	addi	s5,s5,-1650 # 800082a0 <digits+0x260>
    printf("\n");
    8000291a:	00006a17          	auipc	s4,0x6
    8000291e:	a06a0a13          	addi	s4,s4,-1530 # 80008320 <digits+0x2e0>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002922:	00006b97          	auipc	s7,0x6
    80002926:	a56b8b93          	addi	s7,s7,-1450 # 80008378 <states.1790>
    8000292a:	a00d                	j	8000294c <procdump+0x7a>
    printf("%d %s %s", p->pid, state, p->name);
    8000292c:	eb86a583          	lw	a1,-328(a3)
    80002930:	8556                	mv	a0,s5
    80002932:	ffffe097          	auipc	ra,0xffffe
    80002936:	c56080e7          	jalr	-938(ra) # 80000588 <printf>
    printf("\n");
    8000293a:	8552                	mv	a0,s4
    8000293c:	ffffe097          	auipc	ra,0xffffe
    80002940:	c4c080e7          	jalr	-948(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    80002944:	18848493          	addi	s1,s1,392
    80002948:	03248163          	beq	s1,s2,8000296a <procdump+0x98>
    if(p->state == UNUSED)
    8000294c:	86a6                	mv	a3,s1
    8000294e:	ea04a783          	lw	a5,-352(s1)
    80002952:	dbed                	beqz	a5,80002944 <procdump+0x72>
      state = "???";
    80002954:	864e                	mv	a2,s3
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002956:	fcfb6be3          	bltu	s6,a5,8000292c <procdump+0x5a>
    8000295a:	1782                	slli	a5,a5,0x20
    8000295c:	9381                	srli	a5,a5,0x20
    8000295e:	078e                	slli	a5,a5,0x3
    80002960:	97de                	add	a5,a5,s7
    80002962:	6390                	ld	a2,0(a5)
    80002964:	f661                	bnez	a2,8000292c <procdump+0x5a>
      state = "???";
    80002966:	864e                	mv	a2,s3
    80002968:	b7d1                	j	8000292c <procdump+0x5a>
  }
}
    8000296a:	60a6                	ld	ra,72(sp)
    8000296c:	6406                	ld	s0,64(sp)
    8000296e:	74e2                	ld	s1,56(sp)
    80002970:	7942                	ld	s2,48(sp)
    80002972:	79a2                	ld	s3,40(sp)
    80002974:	7a02                	ld	s4,32(sp)
    80002976:	6ae2                	ld	s5,24(sp)
    80002978:	6b42                	ld	s6,16(sp)
    8000297a:	6ba2                	ld	s7,8(sp)
    8000297c:	6161                	addi	sp,sp,80
    8000297e:	8082                	ret

0000000080002980 <pause_system>:

// pause all user processes for the number of seconds specified by thesecond's integer parameter.
int pause_system(int seconds){
    80002980:	1141                	addi	sp,sp,-16
    80002982:	e406                	sd	ra,8(sp)
    80002984:	e022                	sd	s0,0(sp)
    80002986:	0800                	addi	s0,sp,16
  pause_ticks = ticks + seconds * SECONDS_TO_TICKS;
    80002988:	0025179b          	slliw	a5,a0,0x2
    8000298c:	9fa9                	addw	a5,a5,a0
    8000298e:	0017979b          	slliw	a5,a5,0x1
    80002992:	00006517          	auipc	a0,0x6
    80002996:	6be52503          	lw	a0,1726(a0) # 80009050 <ticks>
    8000299a:	9fa9                	addw	a5,a5,a0
    8000299c:	00006717          	auipc	a4,0x6
    800029a0:	6af72423          	sw	a5,1704(a4) # 80009044 <pause_ticks>
  yield();
    800029a4:	00000097          	auipc	ra,0x0
    800029a8:	9c6080e7          	jalr	-1594(ra) # 8000236a <yield>

  return 0;
}
    800029ac:	4501                	li	a0,0
    800029ae:	60a2                	ld	ra,8(sp)
    800029b0:	6402                	ld	s0,0(sp)
    800029b2:	0141                	addi	sp,sp,16
    800029b4:	8082                	ret

00000000800029b6 <kill_system>:

// terminate all user processes
int 
kill_system(void) {
    800029b6:	7179                	addi	sp,sp,-48
    800029b8:	f406                	sd	ra,40(sp)
    800029ba:	f022                	sd	s0,32(sp)
    800029bc:	ec26                	sd	s1,24(sp)
    800029be:	e84a                	sd	s2,16(sp)
    800029c0:	e44e                	sd	s3,8(sp)
    800029c2:	e052                	sd	s4,0(sp)
    800029c4:	1800                	addi	s0,sp,48
  struct proc *p;
  int pid;

  for(p = proc; p < &proc[NPROC]; p++) {
    800029c6:	0000f497          	auipc	s1,0xf
    800029ca:	d2a48493          	addi	s1,s1,-726 # 800116f0 <proc>
      acquire(&p->lock);
      pid = p->pid;
      release(&p->lock);
      if(pid != INIT_PROC_ID && pid != SHELL_PROC_ID)
    800029ce:	4a05                	li	s4,1
  for(p = proc; p < &proc[NPROC]; p++) {
    800029d0:	00015997          	auipc	s3,0x15
    800029d4:	f2098993          	addi	s3,s3,-224 # 800178f0 <tickslock>
    800029d8:	a029                	j	800029e2 <kill_system+0x2c>
    800029da:	18848493          	addi	s1,s1,392
    800029de:	03348863          	beq	s1,s3,80002a0e <kill_system+0x58>
      acquire(&p->lock);
    800029e2:	8526                	mv	a0,s1
    800029e4:	ffffe097          	auipc	ra,0xffffe
    800029e8:	200080e7          	jalr	512(ra) # 80000be4 <acquire>
      pid = p->pid;
    800029ec:	0304a903          	lw	s2,48(s1)
      release(&p->lock);
    800029f0:	8526                	mv	a0,s1
    800029f2:	ffffe097          	auipc	ra,0xffffe
    800029f6:	2a6080e7          	jalr	678(ra) # 80000c98 <release>
      if(pid != INIT_PROC_ID && pid != SHELL_PROC_ID)
    800029fa:	fff9079b          	addiw	a5,s2,-1
    800029fe:	fcfa7ee3          	bgeu	s4,a5,800029da <kill_system+0x24>
        kill(pid);
    80002a02:	854a                	mv	a0,s2
    80002a04:	00000097          	auipc	ra,0x0
    80002a08:	d9a080e7          	jalr	-614(ra) # 8000279e <kill>
    80002a0c:	b7f9                	j	800029da <kill_system+0x24>
  }
  return 0;
}
    80002a0e:	4501                	li	a0,0
    80002a10:	70a2                	ld	ra,40(sp)
    80002a12:	7402                	ld	s0,32(sp)
    80002a14:	64e2                	ld	s1,24(sp)
    80002a16:	6942                	ld	s2,16(sp)
    80002a18:	69a2                	ld	s3,8(sp)
    80002a1a:	6a02                	ld	s4,0(sp)
    80002a1c:	6145                	addi	sp,sp,48
    80002a1e:	8082                	ret

0000000080002a20 <print_stats>:

void
print_stats(void) {
    80002a20:	1141                	addi	sp,sp,-16
    80002a22:	e406                	sd	ra,8(sp)
    80002a24:	e022                	sd	s0,0(sp)
    80002a26:	0800                	addi	s0,sp,16
    printf("sleeping_processes_mean: %d\n", sleeping_processes_mean);
    80002a28:	00006597          	auipc	a1,0x6
    80002a2c:	6185a583          	lw	a1,1560(a1) # 80009040 <sleeping_processes_mean>
    80002a30:	00006517          	auipc	a0,0x6
    80002a34:	88050513          	addi	a0,a0,-1920 # 800082b0 <digits+0x270>
    80002a38:	ffffe097          	auipc	ra,0xffffe
    80002a3c:	b50080e7          	jalr	-1200(ra) # 80000588 <printf>
    printf("runnable_processes_mean: %d\n", runnable_processes_mean);
    80002a40:	00006597          	auipc	a1,0x6
    80002a44:	5f85a583          	lw	a1,1528(a1) # 80009038 <runnable_processes_mean>
    80002a48:	00006517          	auipc	a0,0x6
    80002a4c:	88850513          	addi	a0,a0,-1912 # 800082d0 <digits+0x290>
    80002a50:	ffffe097          	auipc	ra,0xffffe
    80002a54:	b38080e7          	jalr	-1224(ra) # 80000588 <printf>
    printf("running_processes_mean: %d\n", running_processes_mean);
    80002a58:	00006597          	auipc	a1,0x6
    80002a5c:	5e45a583          	lw	a1,1508(a1) # 8000903c <running_processes_mean>
    80002a60:	00006517          	auipc	a0,0x6
    80002a64:	89050513          	addi	a0,a0,-1904 # 800082f0 <digits+0x2b0>
    80002a68:	ffffe097          	auipc	ra,0xffffe
    80002a6c:	b20080e7          	jalr	-1248(ra) # 80000588 <printf>
    printf("program_time: %d\n", program_time);
    80002a70:	00006597          	auipc	a1,0x6
    80002a74:	5c45a583          	lw	a1,1476(a1) # 80009034 <program_time>
    80002a78:	00006517          	auipc	a0,0x6
    80002a7c:	89850513          	addi	a0,a0,-1896 # 80008310 <digits+0x2d0>
    80002a80:	ffffe097          	auipc	ra,0xffffe
    80002a84:	b08080e7          	jalr	-1272(ra) # 80000588 <printf>
    printf("cpu_utilization: %d%\n", cpu_utilization);
    80002a88:	00006597          	auipc	a1,0x6
    80002a8c:	5a85a583          	lw	a1,1448(a1) # 80009030 <cpu_utilization>
    80002a90:	00006517          	auipc	a0,0x6
    80002a94:	89850513          	addi	a0,a0,-1896 # 80008328 <digits+0x2e8>
    80002a98:	ffffe097          	auipc	ra,0xffffe
    80002a9c:	af0080e7          	jalr	-1296(ra) # 80000588 <printf>
    printf ("ticks : %d\n", ticks);
    80002aa0:	00006597          	auipc	a1,0x6
    80002aa4:	5b05a583          	lw	a1,1456(a1) # 80009050 <ticks>
    80002aa8:	00006517          	auipc	a0,0x6
    80002aac:	89850513          	addi	a0,a0,-1896 # 80008340 <digits+0x300>
    80002ab0:	ffffe097          	auipc	ra,0xffffe
    80002ab4:	ad8080e7          	jalr	-1320(ra) # 80000588 <printf>
}
    80002ab8:	60a2                	ld	ra,8(sp)
    80002aba:	6402                	ld	s0,0(sp)
    80002abc:	0141                	addi	sp,sp,16
    80002abe:	8082                	ret

0000000080002ac0 <swtch>:
    80002ac0:	00153023          	sd	ra,0(a0)
    80002ac4:	00253423          	sd	sp,8(a0)
    80002ac8:	e900                	sd	s0,16(a0)
    80002aca:	ed04                	sd	s1,24(a0)
    80002acc:	03253023          	sd	s2,32(a0)
    80002ad0:	03353423          	sd	s3,40(a0)
    80002ad4:	03453823          	sd	s4,48(a0)
    80002ad8:	03553c23          	sd	s5,56(a0)
    80002adc:	05653023          	sd	s6,64(a0)
    80002ae0:	05753423          	sd	s7,72(a0)
    80002ae4:	05853823          	sd	s8,80(a0)
    80002ae8:	05953c23          	sd	s9,88(a0)
    80002aec:	07a53023          	sd	s10,96(a0)
    80002af0:	07b53423          	sd	s11,104(a0)
    80002af4:	0005b083          	ld	ra,0(a1)
    80002af8:	0085b103          	ld	sp,8(a1)
    80002afc:	6980                	ld	s0,16(a1)
    80002afe:	6d84                	ld	s1,24(a1)
    80002b00:	0205b903          	ld	s2,32(a1)
    80002b04:	0285b983          	ld	s3,40(a1)
    80002b08:	0305ba03          	ld	s4,48(a1)
    80002b0c:	0385ba83          	ld	s5,56(a1)
    80002b10:	0405bb03          	ld	s6,64(a1)
    80002b14:	0485bb83          	ld	s7,72(a1)
    80002b18:	0505bc03          	ld	s8,80(a1)
    80002b1c:	0585bc83          	ld	s9,88(a1)
    80002b20:	0605bd03          	ld	s10,96(a1)
    80002b24:	0685bd83          	ld	s11,104(a1)
    80002b28:	8082                	ret

0000000080002b2a <trapinit>:

extern int devintr();

void
trapinit(void)
{
    80002b2a:	1141                	addi	sp,sp,-16
    80002b2c:	e406                	sd	ra,8(sp)
    80002b2e:	e022                	sd	s0,0(sp)
    80002b30:	0800                	addi	s0,sp,16
  initlock(&tickslock, "time");
    80002b32:	00006597          	auipc	a1,0x6
    80002b36:	87658593          	addi	a1,a1,-1930 # 800083a8 <states.1790+0x30>
    80002b3a:	00015517          	auipc	a0,0x15
    80002b3e:	db650513          	addi	a0,a0,-586 # 800178f0 <tickslock>
    80002b42:	ffffe097          	auipc	ra,0xffffe
    80002b46:	012080e7          	jalr	18(ra) # 80000b54 <initlock>
}
    80002b4a:	60a2                	ld	ra,8(sp)
    80002b4c:	6402                	ld	s0,0(sp)
    80002b4e:	0141                	addi	sp,sp,16
    80002b50:	8082                	ret

0000000080002b52 <trapinithart>:

// set up to take exceptions and traps while in the kernel.
void
trapinithart(void)
{
    80002b52:	1141                	addi	sp,sp,-16
    80002b54:	e422                	sd	s0,8(sp)
    80002b56:	0800                	addi	s0,sp,16
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002b58:	00003797          	auipc	a5,0x3
    80002b5c:	4e878793          	addi	a5,a5,1256 # 80006040 <kernelvec>
    80002b60:	10579073          	csrw	stvec,a5
  w_stvec((uint64)kernelvec);
}
    80002b64:	6422                	ld	s0,8(sp)
    80002b66:	0141                	addi	sp,sp,16
    80002b68:	8082                	ret

0000000080002b6a <usertrapret>:
//
// return to user space
//
void
usertrapret(void)
{
    80002b6a:	1141                	addi	sp,sp,-16
    80002b6c:	e406                	sd	ra,8(sp)
    80002b6e:	e022                	sd	s0,0(sp)
    80002b70:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    80002b72:	fffff097          	auipc	ra,0xfffff
    80002b76:	e6c080e7          	jalr	-404(ra) # 800019de <myproc>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002b7a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80002b7e:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002b80:	10079073          	csrw	sstatus,a5
  // kerneltrap() to usertrap(), so turn off interrupts until
  // we're back in user space, where usertrap() is correct.
  intr_off();

  // send syscalls, interrupts, and exceptions to trampoline.S
  w_stvec(TRAMPOLINE + (uservec - trampoline));
    80002b84:	00004617          	auipc	a2,0x4
    80002b88:	47c60613          	addi	a2,a2,1148 # 80007000 <_trampoline>
    80002b8c:	00004697          	auipc	a3,0x4
    80002b90:	47468693          	addi	a3,a3,1140 # 80007000 <_trampoline>
    80002b94:	8e91                	sub	a3,a3,a2
    80002b96:	040007b7          	lui	a5,0x4000
    80002b9a:	17fd                	addi	a5,a5,-1
    80002b9c:	07b2                	slli	a5,a5,0xc
    80002b9e:	96be                	add	a3,a3,a5
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002ba0:	10569073          	csrw	stvec,a3

  // set up trapframe values that uservec will need when
  // the process next re-enters the kernel.
  p->trapframe->kernel_satp = r_satp();         // kernel page table
    80002ba4:	7d38                	ld	a4,120(a0)
  asm volatile("csrr %0, satp" : "=r" (x) );
    80002ba6:	180026f3          	csrr	a3,satp
    80002baa:	e314                	sd	a3,0(a4)
  p->trapframe->kernel_sp = p->kstack + PGSIZE; // process's kernel stack
    80002bac:	7d38                	ld	a4,120(a0)
    80002bae:	7134                	ld	a3,96(a0)
    80002bb0:	6585                	lui	a1,0x1
    80002bb2:	96ae                	add	a3,a3,a1
    80002bb4:	e714                	sd	a3,8(a4)
  p->trapframe->kernel_trap = (uint64)usertrap;
    80002bb6:	7d38                	ld	a4,120(a0)
    80002bb8:	00000697          	auipc	a3,0x0
    80002bbc:	13868693          	addi	a3,a3,312 # 80002cf0 <usertrap>
    80002bc0:	eb14                	sd	a3,16(a4)
  p->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
    80002bc2:	7d38                	ld	a4,120(a0)
  asm volatile("mv %0, tp" : "=r" (x) );
    80002bc4:	8692                	mv	a3,tp
    80002bc6:	f314                	sd	a3,32(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002bc8:	100026f3          	csrr	a3,sstatus
  // set up the registers that trampoline.S's sret will use
  // to get to user space.
  
  // set S Previous Privilege mode to User.
  unsigned long x = r_sstatus();
  x &= ~SSTATUS_SPP; // clear SPP to 0 for user mode
    80002bcc:	eff6f693          	andi	a3,a3,-257
  x |= SSTATUS_SPIE; // enable interrupts in user mode
    80002bd0:	0206e693          	ori	a3,a3,32
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002bd4:	10069073          	csrw	sstatus,a3
  w_sstatus(x);

  // set S Exception Program Counter to the saved user pc.
  w_sepc(p->trapframe->epc);
    80002bd8:	7d38                	ld	a4,120(a0)
  asm volatile("csrw sepc, %0" : : "r" (x));
    80002bda:	6f18                	ld	a4,24(a4)
    80002bdc:	14171073          	csrw	sepc,a4

  // tell trampoline.S the user page table to switch to.
  uint64 satp = MAKE_SATP(p->pagetable);
    80002be0:	792c                	ld	a1,112(a0)
    80002be2:	81b1                	srli	a1,a1,0xc

  // jump to trampoline.S at the top of memory, which 
  // switches to the user page table, restores user registers,
  // and switches to user mode with sret.
  uint64 fn = TRAMPOLINE + (userret - trampoline);
    80002be4:	00004717          	auipc	a4,0x4
    80002be8:	4ac70713          	addi	a4,a4,1196 # 80007090 <userret>
    80002bec:	8f11                	sub	a4,a4,a2
    80002bee:	97ba                	add	a5,a5,a4
  ((void (*)(uint64,uint64))fn)(TRAPFRAME, satp);
    80002bf0:	577d                	li	a4,-1
    80002bf2:	177e                	slli	a4,a4,0x3f
    80002bf4:	8dd9                	or	a1,a1,a4
    80002bf6:	02000537          	lui	a0,0x2000
    80002bfa:	157d                	addi	a0,a0,-1
    80002bfc:	0536                	slli	a0,a0,0xd
    80002bfe:	9782                	jalr	a5
}
    80002c00:	60a2                	ld	ra,8(sp)
    80002c02:	6402                	ld	s0,0(sp)
    80002c04:	0141                	addi	sp,sp,16
    80002c06:	8082                	ret

0000000080002c08 <clockintr>:
  w_sstatus(sstatus);
}

void
clockintr()
{
    80002c08:	1101                	addi	sp,sp,-32
    80002c0a:	ec06                	sd	ra,24(sp)
    80002c0c:	e822                	sd	s0,16(sp)
    80002c0e:	e426                	sd	s1,8(sp)
    80002c10:	1000                	addi	s0,sp,32
  acquire(&tickslock);
    80002c12:	00015497          	auipc	s1,0x15
    80002c16:	cde48493          	addi	s1,s1,-802 # 800178f0 <tickslock>
    80002c1a:	8526                	mv	a0,s1
    80002c1c:	ffffe097          	auipc	ra,0xffffe
    80002c20:	fc8080e7          	jalr	-56(ra) # 80000be4 <acquire>
  ticks++;
    80002c24:	00006517          	auipc	a0,0x6
    80002c28:	42c50513          	addi	a0,a0,1068 # 80009050 <ticks>
    80002c2c:	411c                	lw	a5,0(a0)
    80002c2e:	2785                	addiw	a5,a5,1
    80002c30:	c11c                	sw	a5,0(a0)
  wakeup(&ticks);
    80002c32:	00000097          	auipc	ra,0x0
    80002c36:	914080e7          	jalr	-1772(ra) # 80002546 <wakeup>
  release(&tickslock);
    80002c3a:	8526                	mv	a0,s1
    80002c3c:	ffffe097          	auipc	ra,0xffffe
    80002c40:	05c080e7          	jalr	92(ra) # 80000c98 <release>
}
    80002c44:	60e2                	ld	ra,24(sp)
    80002c46:	6442                	ld	s0,16(sp)
    80002c48:	64a2                	ld	s1,8(sp)
    80002c4a:	6105                	addi	sp,sp,32
    80002c4c:	8082                	ret

0000000080002c4e <devintr>:
// returns 2 if timer interrupt,
// 1 if other device,
// 0 if not recognized.
int
devintr()
{
    80002c4e:	1101                	addi	sp,sp,-32
    80002c50:	ec06                	sd	ra,24(sp)
    80002c52:	e822                	sd	s0,16(sp)
    80002c54:	e426                	sd	s1,8(sp)
    80002c56:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002c58:	14202773          	csrr	a4,scause
  uint64 scause = r_scause();

  if((scause & 0x8000000000000000L) &&
    80002c5c:	00074d63          	bltz	a4,80002c76 <devintr+0x28>
    // now allowed to interrupt again.
    if(irq)
      plic_complete(irq);

    return 1;
  } else if(scause == 0x8000000000000001L){
    80002c60:	57fd                	li	a5,-1
    80002c62:	17fe                	slli	a5,a5,0x3f
    80002c64:	0785                	addi	a5,a5,1
    // the SSIP bit in sip.
    w_sip(r_sip() & ~2);

    return 2;
  } else {
    return 0;
    80002c66:	4501                	li	a0,0
  } else if(scause == 0x8000000000000001L){
    80002c68:	06f70363          	beq	a4,a5,80002cce <devintr+0x80>
  }
}
    80002c6c:	60e2                	ld	ra,24(sp)
    80002c6e:	6442                	ld	s0,16(sp)
    80002c70:	64a2                	ld	s1,8(sp)
    80002c72:	6105                	addi	sp,sp,32
    80002c74:	8082                	ret
     (scause & 0xff) == 9){
    80002c76:	0ff77793          	andi	a5,a4,255
  if((scause & 0x8000000000000000L) &&
    80002c7a:	46a5                	li	a3,9
    80002c7c:	fed792e3          	bne	a5,a3,80002c60 <devintr+0x12>
    int irq = plic_claim();
    80002c80:	00003097          	auipc	ra,0x3
    80002c84:	4c8080e7          	jalr	1224(ra) # 80006148 <plic_claim>
    80002c88:	84aa                	mv	s1,a0
    if(irq == UART0_IRQ){
    80002c8a:	47a9                	li	a5,10
    80002c8c:	02f50763          	beq	a0,a5,80002cba <devintr+0x6c>
    } else if(irq == VIRTIO0_IRQ){
    80002c90:	4785                	li	a5,1
    80002c92:	02f50963          	beq	a0,a5,80002cc4 <devintr+0x76>
    return 1;
    80002c96:	4505                	li	a0,1
    } else if(irq){
    80002c98:	d8f1                	beqz	s1,80002c6c <devintr+0x1e>
      printf("unexpected interrupt irq=%d\n", irq);
    80002c9a:	85a6                	mv	a1,s1
    80002c9c:	00005517          	auipc	a0,0x5
    80002ca0:	71450513          	addi	a0,a0,1812 # 800083b0 <states.1790+0x38>
    80002ca4:	ffffe097          	auipc	ra,0xffffe
    80002ca8:	8e4080e7          	jalr	-1820(ra) # 80000588 <printf>
      plic_complete(irq);
    80002cac:	8526                	mv	a0,s1
    80002cae:	00003097          	auipc	ra,0x3
    80002cb2:	4be080e7          	jalr	1214(ra) # 8000616c <plic_complete>
    return 1;
    80002cb6:	4505                	li	a0,1
    80002cb8:	bf55                	j	80002c6c <devintr+0x1e>
      uartintr();
    80002cba:	ffffe097          	auipc	ra,0xffffe
    80002cbe:	cee080e7          	jalr	-786(ra) # 800009a8 <uartintr>
    80002cc2:	b7ed                	j	80002cac <devintr+0x5e>
      virtio_disk_intr();
    80002cc4:	00004097          	auipc	ra,0x4
    80002cc8:	988080e7          	jalr	-1656(ra) # 8000664c <virtio_disk_intr>
    80002ccc:	b7c5                	j	80002cac <devintr+0x5e>
    if(cpuid() == 0){
    80002cce:	fffff097          	auipc	ra,0xfffff
    80002cd2:	ce4080e7          	jalr	-796(ra) # 800019b2 <cpuid>
    80002cd6:	c901                	beqz	a0,80002ce6 <devintr+0x98>
  asm volatile("csrr %0, sip" : "=r" (x) );
    80002cd8:	144027f3          	csrr	a5,sip
    w_sip(r_sip() & ~2);
    80002cdc:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sip, %0" : : "r" (x));
    80002cde:	14479073          	csrw	sip,a5
    return 2;
    80002ce2:	4509                	li	a0,2
    80002ce4:	b761                	j	80002c6c <devintr+0x1e>
      clockintr();
    80002ce6:	00000097          	auipc	ra,0x0
    80002cea:	f22080e7          	jalr	-222(ra) # 80002c08 <clockintr>
    80002cee:	b7ed                	j	80002cd8 <devintr+0x8a>

0000000080002cf0 <usertrap>:
{
    80002cf0:	1101                	addi	sp,sp,-32
    80002cf2:	ec06                	sd	ra,24(sp)
    80002cf4:	e822                	sd	s0,16(sp)
    80002cf6:	e426                	sd	s1,8(sp)
    80002cf8:	e04a                	sd	s2,0(sp)
    80002cfa:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002cfc:	100027f3          	csrr	a5,sstatus
  if((r_sstatus() & SSTATUS_SPP) != 0)
    80002d00:	1007f793          	andi	a5,a5,256
    80002d04:	e3ad                	bnez	a5,80002d66 <usertrap+0x76>
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002d06:	00003797          	auipc	a5,0x3
    80002d0a:	33a78793          	addi	a5,a5,826 # 80006040 <kernelvec>
    80002d0e:	10579073          	csrw	stvec,a5
  struct proc *p = myproc();
    80002d12:	fffff097          	auipc	ra,0xfffff
    80002d16:	ccc080e7          	jalr	-820(ra) # 800019de <myproc>
    80002d1a:	84aa                	mv	s1,a0
  p->trapframe->epc = r_sepc();
    80002d1c:	7d3c                	ld	a5,120(a0)
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002d1e:	14102773          	csrr	a4,sepc
    80002d22:	ef98                	sd	a4,24(a5)
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002d24:	14202773          	csrr	a4,scause
  if(r_scause() == 8){
    80002d28:	47a1                	li	a5,8
    80002d2a:	04f71c63          	bne	a4,a5,80002d82 <usertrap+0x92>
    if(p->killed)
    80002d2e:	551c                	lw	a5,40(a0)
    80002d30:	e3b9                	bnez	a5,80002d76 <usertrap+0x86>
    p->trapframe->epc += 4;
    80002d32:	7cb8                	ld	a4,120(s1)
    80002d34:	6f1c                	ld	a5,24(a4)
    80002d36:	0791                	addi	a5,a5,4
    80002d38:	ef1c                	sd	a5,24(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002d3a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002d3e:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002d42:	10079073          	csrw	sstatus,a5
    syscall();
    80002d46:	00000097          	auipc	ra,0x0
    80002d4a:	2e0080e7          	jalr	736(ra) # 80003026 <syscall>
  if(p->killed)
    80002d4e:	549c                	lw	a5,40(s1)
    80002d50:	ebc1                	bnez	a5,80002de0 <usertrap+0xf0>
  usertrapret();
    80002d52:	00000097          	auipc	ra,0x0
    80002d56:	e18080e7          	jalr	-488(ra) # 80002b6a <usertrapret>
}
    80002d5a:	60e2                	ld	ra,24(sp)
    80002d5c:	6442                	ld	s0,16(sp)
    80002d5e:	64a2                	ld	s1,8(sp)
    80002d60:	6902                	ld	s2,0(sp)
    80002d62:	6105                	addi	sp,sp,32
    80002d64:	8082                	ret
    panic("usertrap: not from user mode");
    80002d66:	00005517          	auipc	a0,0x5
    80002d6a:	66a50513          	addi	a0,a0,1642 # 800083d0 <states.1790+0x58>
    80002d6e:	ffffd097          	auipc	ra,0xffffd
    80002d72:	7d0080e7          	jalr	2000(ra) # 8000053e <panic>
      exit(-1);
    80002d76:	557d                	li	a0,-1
    80002d78:	00000097          	auipc	ra,0x0
    80002d7c:	8bc080e7          	jalr	-1860(ra) # 80002634 <exit>
    80002d80:	bf4d                	j	80002d32 <usertrap+0x42>
  } else if((which_dev = devintr()) != 0){
    80002d82:	00000097          	auipc	ra,0x0
    80002d86:	ecc080e7          	jalr	-308(ra) # 80002c4e <devintr>
    80002d8a:	892a                	mv	s2,a0
    80002d8c:	c501                	beqz	a0,80002d94 <usertrap+0xa4>
  if(p->killed)
    80002d8e:	549c                	lw	a5,40(s1)
    80002d90:	c3a1                	beqz	a5,80002dd0 <usertrap+0xe0>
    80002d92:	a815                	j	80002dc6 <usertrap+0xd6>
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002d94:	142025f3          	csrr	a1,scause
    printf("usertrap(): unexpected scause %p pid=%d\n", r_scause(), p->pid);
    80002d98:	5890                	lw	a2,48(s1)
    80002d9a:	00005517          	auipc	a0,0x5
    80002d9e:	65650513          	addi	a0,a0,1622 # 800083f0 <states.1790+0x78>
    80002da2:	ffffd097          	auipc	ra,0xffffd
    80002da6:	7e6080e7          	jalr	2022(ra) # 80000588 <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002daa:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002dae:	14302673          	csrr	a2,stval
    printf("            sepc=%p stval=%p\n", r_sepc(), r_stval());
    80002db2:	00005517          	auipc	a0,0x5
    80002db6:	66e50513          	addi	a0,a0,1646 # 80008420 <states.1790+0xa8>
    80002dba:	ffffd097          	auipc	ra,0xffffd
    80002dbe:	7ce080e7          	jalr	1998(ra) # 80000588 <printf>
    p->killed = 1;
    80002dc2:	4785                	li	a5,1
    80002dc4:	d49c                	sw	a5,40(s1)
    exit(-1);
    80002dc6:	557d                	li	a0,-1
    80002dc8:	00000097          	auipc	ra,0x0
    80002dcc:	86c080e7          	jalr	-1940(ra) # 80002634 <exit>
  if(which_dev == 2)
    80002dd0:	4789                	li	a5,2
    80002dd2:	f8f910e3          	bne	s2,a5,80002d52 <usertrap+0x62>
    yield();
    80002dd6:	fffff097          	auipc	ra,0xfffff
    80002dda:	594080e7          	jalr	1428(ra) # 8000236a <yield>
    80002dde:	bf95                	j	80002d52 <usertrap+0x62>
  int which_dev = 0;
    80002de0:	4901                	li	s2,0
    80002de2:	b7d5                	j	80002dc6 <usertrap+0xd6>

0000000080002de4 <kerneltrap>:
{
    80002de4:	7179                	addi	sp,sp,-48
    80002de6:	f406                	sd	ra,40(sp)
    80002de8:	f022                	sd	s0,32(sp)
    80002dea:	ec26                	sd	s1,24(sp)
    80002dec:	e84a                	sd	s2,16(sp)
    80002dee:	e44e                	sd	s3,8(sp)
    80002df0:	1800                	addi	s0,sp,48
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002df2:	14102973          	csrr	s2,sepc
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002df6:	100024f3          	csrr	s1,sstatus
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002dfa:	142029f3          	csrr	s3,scause
  if((sstatus & SSTATUS_SPP) == 0)
    80002dfe:	1004f793          	andi	a5,s1,256
    80002e02:	cb85                	beqz	a5,80002e32 <kerneltrap+0x4e>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002e04:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80002e08:	8b89                	andi	a5,a5,2
  if(intr_get() != 0)
    80002e0a:	ef85                	bnez	a5,80002e42 <kerneltrap+0x5e>
  if((which_dev = devintr()) == 0){
    80002e0c:	00000097          	auipc	ra,0x0
    80002e10:	e42080e7          	jalr	-446(ra) # 80002c4e <devintr>
    80002e14:	cd1d                	beqz	a0,80002e52 <kerneltrap+0x6e>
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
    80002e16:	4789                	li	a5,2
    80002e18:	06f50a63          	beq	a0,a5,80002e8c <kerneltrap+0xa8>
  asm volatile("csrw sepc, %0" : : "r" (x));
    80002e1c:	14191073          	csrw	sepc,s2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002e20:	10049073          	csrw	sstatus,s1
}
    80002e24:	70a2                	ld	ra,40(sp)
    80002e26:	7402                	ld	s0,32(sp)
    80002e28:	64e2                	ld	s1,24(sp)
    80002e2a:	6942                	ld	s2,16(sp)
    80002e2c:	69a2                	ld	s3,8(sp)
    80002e2e:	6145                	addi	sp,sp,48
    80002e30:	8082                	ret
    panic("kerneltrap: not from supervisor mode");
    80002e32:	00005517          	auipc	a0,0x5
    80002e36:	60e50513          	addi	a0,a0,1550 # 80008440 <states.1790+0xc8>
    80002e3a:	ffffd097          	auipc	ra,0xffffd
    80002e3e:	704080e7          	jalr	1796(ra) # 8000053e <panic>
    panic("kerneltrap: interrupts enabled");
    80002e42:	00005517          	auipc	a0,0x5
    80002e46:	62650513          	addi	a0,a0,1574 # 80008468 <states.1790+0xf0>
    80002e4a:	ffffd097          	auipc	ra,0xffffd
    80002e4e:	6f4080e7          	jalr	1780(ra) # 8000053e <panic>
    printf("scause %p\n", scause);
    80002e52:	85ce                	mv	a1,s3
    80002e54:	00005517          	auipc	a0,0x5
    80002e58:	63450513          	addi	a0,a0,1588 # 80008488 <states.1790+0x110>
    80002e5c:	ffffd097          	auipc	ra,0xffffd
    80002e60:	72c080e7          	jalr	1836(ra) # 80000588 <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002e64:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002e68:	14302673          	csrr	a2,stval
    printf("sepc=%p stval=%p\n", r_sepc(), r_stval());
    80002e6c:	00005517          	auipc	a0,0x5
    80002e70:	62c50513          	addi	a0,a0,1580 # 80008498 <states.1790+0x120>
    80002e74:	ffffd097          	auipc	ra,0xffffd
    80002e78:	714080e7          	jalr	1812(ra) # 80000588 <printf>
    panic("kerneltrap");
    80002e7c:	00005517          	auipc	a0,0x5
    80002e80:	63450513          	addi	a0,a0,1588 # 800084b0 <states.1790+0x138>
    80002e84:	ffffd097          	auipc	ra,0xffffd
    80002e88:	6ba080e7          	jalr	1722(ra) # 8000053e <panic>
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
    80002e8c:	fffff097          	auipc	ra,0xfffff
    80002e90:	b52080e7          	jalr	-1198(ra) # 800019de <myproc>
    80002e94:	d541                	beqz	a0,80002e1c <kerneltrap+0x38>
    80002e96:	fffff097          	auipc	ra,0xfffff
    80002e9a:	b48080e7          	jalr	-1208(ra) # 800019de <myproc>
    80002e9e:	4d18                	lw	a4,24(a0)
    80002ea0:	4791                	li	a5,4
    80002ea2:	f6f71de3          	bne	a4,a5,80002e1c <kerneltrap+0x38>
    yield();
    80002ea6:	fffff097          	auipc	ra,0xfffff
    80002eaa:	4c4080e7          	jalr	1220(ra) # 8000236a <yield>
    80002eae:	b7bd                	j	80002e1c <kerneltrap+0x38>

0000000080002eb0 <argraw>:
  return strlen(buf);
}

static uint64
argraw(int n)
{
    80002eb0:	1101                	addi	sp,sp,-32
    80002eb2:	ec06                	sd	ra,24(sp)
    80002eb4:	e822                	sd	s0,16(sp)
    80002eb6:	e426                	sd	s1,8(sp)
    80002eb8:	1000                	addi	s0,sp,32
    80002eba:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80002ebc:	fffff097          	auipc	ra,0xfffff
    80002ec0:	b22080e7          	jalr	-1246(ra) # 800019de <myproc>
  switch (n) {
    80002ec4:	4795                	li	a5,5
    80002ec6:	0497e163          	bltu	a5,s1,80002f08 <argraw+0x58>
    80002eca:	048a                	slli	s1,s1,0x2
    80002ecc:	00005717          	auipc	a4,0x5
    80002ed0:	61c70713          	addi	a4,a4,1564 # 800084e8 <states.1790+0x170>
    80002ed4:	94ba                	add	s1,s1,a4
    80002ed6:	409c                	lw	a5,0(s1)
    80002ed8:	97ba                	add	a5,a5,a4
    80002eda:	8782                	jr	a5
  case 0:
    return p->trapframe->a0;
    80002edc:	7d3c                	ld	a5,120(a0)
    80002ede:	7ba8                	ld	a0,112(a5)
  case 5:
    return p->trapframe->a5;
  }
  panic("argraw");
  return -1;
}
    80002ee0:	60e2                	ld	ra,24(sp)
    80002ee2:	6442                	ld	s0,16(sp)
    80002ee4:	64a2                	ld	s1,8(sp)
    80002ee6:	6105                	addi	sp,sp,32
    80002ee8:	8082                	ret
    return p->trapframe->a1;
    80002eea:	7d3c                	ld	a5,120(a0)
    80002eec:	7fa8                	ld	a0,120(a5)
    80002eee:	bfcd                	j	80002ee0 <argraw+0x30>
    return p->trapframe->a2;
    80002ef0:	7d3c                	ld	a5,120(a0)
    80002ef2:	63c8                	ld	a0,128(a5)
    80002ef4:	b7f5                	j	80002ee0 <argraw+0x30>
    return p->trapframe->a3;
    80002ef6:	7d3c                	ld	a5,120(a0)
    80002ef8:	67c8                	ld	a0,136(a5)
    80002efa:	b7dd                	j	80002ee0 <argraw+0x30>
    return p->trapframe->a4;
    80002efc:	7d3c                	ld	a5,120(a0)
    80002efe:	6bc8                	ld	a0,144(a5)
    80002f00:	b7c5                	j	80002ee0 <argraw+0x30>
    return p->trapframe->a5;
    80002f02:	7d3c                	ld	a5,120(a0)
    80002f04:	6fc8                	ld	a0,152(a5)
    80002f06:	bfe9                	j	80002ee0 <argraw+0x30>
  panic("argraw");
    80002f08:	00005517          	auipc	a0,0x5
    80002f0c:	5b850513          	addi	a0,a0,1464 # 800084c0 <states.1790+0x148>
    80002f10:	ffffd097          	auipc	ra,0xffffd
    80002f14:	62e080e7          	jalr	1582(ra) # 8000053e <panic>

0000000080002f18 <fetchaddr>:
{
    80002f18:	1101                	addi	sp,sp,-32
    80002f1a:	ec06                	sd	ra,24(sp)
    80002f1c:	e822                	sd	s0,16(sp)
    80002f1e:	e426                	sd	s1,8(sp)
    80002f20:	e04a                	sd	s2,0(sp)
    80002f22:	1000                	addi	s0,sp,32
    80002f24:	84aa                	mv	s1,a0
    80002f26:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80002f28:	fffff097          	auipc	ra,0xfffff
    80002f2c:	ab6080e7          	jalr	-1354(ra) # 800019de <myproc>
  if(addr >= p->sz || addr+sizeof(uint64) > p->sz)
    80002f30:	753c                	ld	a5,104(a0)
    80002f32:	02f4f863          	bgeu	s1,a5,80002f62 <fetchaddr+0x4a>
    80002f36:	00848713          	addi	a4,s1,8
    80002f3a:	02e7e663          	bltu	a5,a4,80002f66 <fetchaddr+0x4e>
  if(copyin(p->pagetable, (char *)ip, addr, sizeof(*ip)) != 0)
    80002f3e:	46a1                	li	a3,8
    80002f40:	8626                	mv	a2,s1
    80002f42:	85ca                	mv	a1,s2
    80002f44:	7928                	ld	a0,112(a0)
    80002f46:	ffffe097          	auipc	ra,0xffffe
    80002f4a:	7c0080e7          	jalr	1984(ra) # 80001706 <copyin>
    80002f4e:	00a03533          	snez	a0,a0
    80002f52:	40a00533          	neg	a0,a0
}
    80002f56:	60e2                	ld	ra,24(sp)
    80002f58:	6442                	ld	s0,16(sp)
    80002f5a:	64a2                	ld	s1,8(sp)
    80002f5c:	6902                	ld	s2,0(sp)
    80002f5e:	6105                	addi	sp,sp,32
    80002f60:	8082                	ret
    return -1;
    80002f62:	557d                	li	a0,-1
    80002f64:	bfcd                	j	80002f56 <fetchaddr+0x3e>
    80002f66:	557d                	li	a0,-1
    80002f68:	b7fd                	j	80002f56 <fetchaddr+0x3e>

0000000080002f6a <fetchstr>:
{
    80002f6a:	7179                	addi	sp,sp,-48
    80002f6c:	f406                	sd	ra,40(sp)
    80002f6e:	f022                	sd	s0,32(sp)
    80002f70:	ec26                	sd	s1,24(sp)
    80002f72:	e84a                	sd	s2,16(sp)
    80002f74:	e44e                	sd	s3,8(sp)
    80002f76:	1800                	addi	s0,sp,48
    80002f78:	892a                	mv	s2,a0
    80002f7a:	84ae                	mv	s1,a1
    80002f7c:	89b2                	mv	s3,a2
  struct proc *p = myproc();
    80002f7e:	fffff097          	auipc	ra,0xfffff
    80002f82:	a60080e7          	jalr	-1440(ra) # 800019de <myproc>
  int err = copyinstr(p->pagetable, buf, addr, max);
    80002f86:	86ce                	mv	a3,s3
    80002f88:	864a                	mv	a2,s2
    80002f8a:	85a6                	mv	a1,s1
    80002f8c:	7928                	ld	a0,112(a0)
    80002f8e:	fffff097          	auipc	ra,0xfffff
    80002f92:	804080e7          	jalr	-2044(ra) # 80001792 <copyinstr>
  if(err < 0)
    80002f96:	00054763          	bltz	a0,80002fa4 <fetchstr+0x3a>
  return strlen(buf);
    80002f9a:	8526                	mv	a0,s1
    80002f9c:	ffffe097          	auipc	ra,0xffffe
    80002fa0:	ec8080e7          	jalr	-312(ra) # 80000e64 <strlen>
}
    80002fa4:	70a2                	ld	ra,40(sp)
    80002fa6:	7402                	ld	s0,32(sp)
    80002fa8:	64e2                	ld	s1,24(sp)
    80002faa:	6942                	ld	s2,16(sp)
    80002fac:	69a2                	ld	s3,8(sp)
    80002fae:	6145                	addi	sp,sp,48
    80002fb0:	8082                	ret

0000000080002fb2 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
    80002fb2:	1101                	addi	sp,sp,-32
    80002fb4:	ec06                	sd	ra,24(sp)
    80002fb6:	e822                	sd	s0,16(sp)
    80002fb8:	e426                	sd	s1,8(sp)
    80002fba:	1000                	addi	s0,sp,32
    80002fbc:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002fbe:	00000097          	auipc	ra,0x0
    80002fc2:	ef2080e7          	jalr	-270(ra) # 80002eb0 <argraw>
    80002fc6:	c088                	sw	a0,0(s1)
  return 0;
}
    80002fc8:	4501                	li	a0,0
    80002fca:	60e2                	ld	ra,24(sp)
    80002fcc:	6442                	ld	s0,16(sp)
    80002fce:	64a2                	ld	s1,8(sp)
    80002fd0:	6105                	addi	sp,sp,32
    80002fd2:	8082                	ret

0000000080002fd4 <argaddr>:
// Retrieve an argument as a pointer.
// Doesn't check for legality, since
// copyin/copyout will do that.
int
argaddr(int n, uint64 *ip)
{
    80002fd4:	1101                	addi	sp,sp,-32
    80002fd6:	ec06                	sd	ra,24(sp)
    80002fd8:	e822                	sd	s0,16(sp)
    80002fda:	e426                	sd	s1,8(sp)
    80002fdc:	1000                	addi	s0,sp,32
    80002fde:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002fe0:	00000097          	auipc	ra,0x0
    80002fe4:	ed0080e7          	jalr	-304(ra) # 80002eb0 <argraw>
    80002fe8:	e088                	sd	a0,0(s1)
  return 0;
}
    80002fea:	4501                	li	a0,0
    80002fec:	60e2                	ld	ra,24(sp)
    80002fee:	6442                	ld	s0,16(sp)
    80002ff0:	64a2                	ld	s1,8(sp)
    80002ff2:	6105                	addi	sp,sp,32
    80002ff4:	8082                	ret

0000000080002ff6 <argstr>:
// Fetch the nth word-sized system call argument as a null-terminated string.
// Copies into buf, at most max.
// Returns string length if OK (including nul), -1 if error.
int
argstr(int n, char *buf, int max)
{
    80002ff6:	1101                	addi	sp,sp,-32
    80002ff8:	ec06                	sd	ra,24(sp)
    80002ffa:	e822                	sd	s0,16(sp)
    80002ffc:	e426                	sd	s1,8(sp)
    80002ffe:	e04a                	sd	s2,0(sp)
    80003000:	1000                	addi	s0,sp,32
    80003002:	84ae                	mv	s1,a1
    80003004:	8932                	mv	s2,a2
  *ip = argraw(n);
    80003006:	00000097          	auipc	ra,0x0
    8000300a:	eaa080e7          	jalr	-342(ra) # 80002eb0 <argraw>
  uint64 addr;
  if(argaddr(n, &addr) < 0)
    return -1;
  return fetchstr(addr, buf, max);
    8000300e:	864a                	mv	a2,s2
    80003010:	85a6                	mv	a1,s1
    80003012:	00000097          	auipc	ra,0x0
    80003016:	f58080e7          	jalr	-168(ra) # 80002f6a <fetchstr>
}
    8000301a:	60e2                	ld	ra,24(sp)
    8000301c:	6442                	ld	s0,16(sp)
    8000301e:	64a2                	ld	s1,8(sp)
    80003020:	6902                	ld	s2,0(sp)
    80003022:	6105                	addi	sp,sp,32
    80003024:	8082                	ret

0000000080003026 <syscall>:
[SYS_print_stats] sys_print_stats,
};

void
syscall(void)
{
    80003026:	1101                	addi	sp,sp,-32
    80003028:	ec06                	sd	ra,24(sp)
    8000302a:	e822                	sd	s0,16(sp)
    8000302c:	e426                	sd	s1,8(sp)
    8000302e:	e04a                	sd	s2,0(sp)
    80003030:	1000                	addi	s0,sp,32
  int num;
  struct proc *p = myproc();
    80003032:	fffff097          	auipc	ra,0xfffff
    80003036:	9ac080e7          	jalr	-1620(ra) # 800019de <myproc>
    8000303a:	84aa                	mv	s1,a0

  num = p->trapframe->a7;
    8000303c:	07853903          	ld	s2,120(a0)
    80003040:	0a893783          	ld	a5,168(s2)
    80003044:	0007869b          	sext.w	a3,a5
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    80003048:	37fd                	addiw	a5,a5,-1
    8000304a:	475d                	li	a4,23
    8000304c:	00f76f63          	bltu	a4,a5,8000306a <syscall+0x44>
    80003050:	00369713          	slli	a4,a3,0x3
    80003054:	00005797          	auipc	a5,0x5
    80003058:	4ac78793          	addi	a5,a5,1196 # 80008500 <syscalls>
    8000305c:	97ba                	add	a5,a5,a4
    8000305e:	639c                	ld	a5,0(a5)
    80003060:	c789                	beqz	a5,8000306a <syscall+0x44>
    p->trapframe->a0 = syscalls[num]();
    80003062:	9782                	jalr	a5
    80003064:	06a93823          	sd	a0,112(s2)
    80003068:	a839                	j	80003086 <syscall+0x60>
  } else {
    printf("%d %s: unknown sys call %d\n",
    8000306a:	17848613          	addi	a2,s1,376
    8000306e:	588c                	lw	a1,48(s1)
    80003070:	00005517          	auipc	a0,0x5
    80003074:	45850513          	addi	a0,a0,1112 # 800084c8 <states.1790+0x150>
    80003078:	ffffd097          	auipc	ra,0xffffd
    8000307c:	510080e7          	jalr	1296(ra) # 80000588 <printf>
            p->pid, p->name, num);
    p->trapframe->a0 = -1;
    80003080:	7cbc                	ld	a5,120(s1)
    80003082:	577d                	li	a4,-1
    80003084:	fbb8                	sd	a4,112(a5)
  }
}
    80003086:	60e2                	ld	ra,24(sp)
    80003088:	6442                	ld	s0,16(sp)
    8000308a:	64a2                	ld	s1,8(sp)
    8000308c:	6902                	ld	s2,0(sp)
    8000308e:	6105                	addi	sp,sp,32
    80003090:	8082                	ret

0000000080003092 <sys_exit>:
#include "spinlock.h"
#include "proc.h"

uint64
sys_exit(void)
{
    80003092:	1101                	addi	sp,sp,-32
    80003094:	ec06                	sd	ra,24(sp)
    80003096:	e822                	sd	s0,16(sp)
    80003098:	1000                	addi	s0,sp,32
  int n;
  if(argint(0, &n) < 0)
    8000309a:	fec40593          	addi	a1,s0,-20
    8000309e:	4501                	li	a0,0
    800030a0:	00000097          	auipc	ra,0x0
    800030a4:	f12080e7          	jalr	-238(ra) # 80002fb2 <argint>
    return -1;
    800030a8:	57fd                	li	a5,-1
  if(argint(0, &n) < 0)
    800030aa:	00054963          	bltz	a0,800030bc <sys_exit+0x2a>
  exit(n);
    800030ae:	fec42503          	lw	a0,-20(s0)
    800030b2:	fffff097          	auipc	ra,0xfffff
    800030b6:	582080e7          	jalr	1410(ra) # 80002634 <exit>
  return 0;  // not reached
    800030ba:	4781                	li	a5,0
}
    800030bc:	853e                	mv	a0,a5
    800030be:	60e2                	ld	ra,24(sp)
    800030c0:	6442                	ld	s0,16(sp)
    800030c2:	6105                	addi	sp,sp,32
    800030c4:	8082                	ret

00000000800030c6 <sys_getpid>:

uint64
sys_getpid(void)
{
    800030c6:	1141                	addi	sp,sp,-16
    800030c8:	e406                	sd	ra,8(sp)
    800030ca:	e022                	sd	s0,0(sp)
    800030cc:	0800                	addi	s0,sp,16
  return myproc()->pid;
    800030ce:	fffff097          	auipc	ra,0xfffff
    800030d2:	910080e7          	jalr	-1776(ra) # 800019de <myproc>
}
    800030d6:	5908                	lw	a0,48(a0)
    800030d8:	60a2                	ld	ra,8(sp)
    800030da:	6402                	ld	s0,0(sp)
    800030dc:	0141                	addi	sp,sp,16
    800030de:	8082                	ret

00000000800030e0 <sys_fork>:

uint64
sys_fork(void)
{
    800030e0:	1141                	addi	sp,sp,-16
    800030e2:	e406                	sd	ra,8(sp)
    800030e4:	e022                	sd	s0,0(sp)
    800030e6:	0800                	addi	s0,sp,16
  return fork();
    800030e8:	fffff097          	auipc	ra,0xfffff
    800030ec:	cd6080e7          	jalr	-810(ra) # 80001dbe <fork>
}
    800030f0:	60a2                	ld	ra,8(sp)
    800030f2:	6402                	ld	s0,0(sp)
    800030f4:	0141                	addi	sp,sp,16
    800030f6:	8082                	ret

00000000800030f8 <sys_wait>:

uint64
sys_wait(void)
{
    800030f8:	1101                	addi	sp,sp,-32
    800030fa:	ec06                	sd	ra,24(sp)
    800030fc:	e822                	sd	s0,16(sp)
    800030fe:	1000                	addi	s0,sp,32
  uint64 p;
  if(argaddr(0, &p) < 0)
    80003100:	fe840593          	addi	a1,s0,-24
    80003104:	4501                	li	a0,0
    80003106:	00000097          	auipc	ra,0x0
    8000310a:	ece080e7          	jalr	-306(ra) # 80002fd4 <argaddr>
    8000310e:	87aa                	mv	a5,a0
    return -1;
    80003110:	557d                	li	a0,-1
  if(argaddr(0, &p) < 0)
    80003112:	0007c863          	bltz	a5,80003122 <sys_wait+0x2a>
  return wait(p);
    80003116:	fe843503          	ld	a0,-24(s0)
    8000311a:	fffff097          	auipc	ra,0xfffff
    8000311e:	304080e7          	jalr	772(ra) # 8000241e <wait>
}
    80003122:	60e2                	ld	ra,24(sp)
    80003124:	6442                	ld	s0,16(sp)
    80003126:	6105                	addi	sp,sp,32
    80003128:	8082                	ret

000000008000312a <sys_sbrk>:

uint64
sys_sbrk(void)
{
    8000312a:	7179                	addi	sp,sp,-48
    8000312c:	f406                	sd	ra,40(sp)
    8000312e:	f022                	sd	s0,32(sp)
    80003130:	ec26                	sd	s1,24(sp)
    80003132:	1800                	addi	s0,sp,48
  int addr;
  int n;

  if(argint(0, &n) < 0)
    80003134:	fdc40593          	addi	a1,s0,-36
    80003138:	4501                	li	a0,0
    8000313a:	00000097          	auipc	ra,0x0
    8000313e:	e78080e7          	jalr	-392(ra) # 80002fb2 <argint>
    80003142:	87aa                	mv	a5,a0
    return -1;
    80003144:	557d                	li	a0,-1
  if(argint(0, &n) < 0)
    80003146:	0207c063          	bltz	a5,80003166 <sys_sbrk+0x3c>
  addr = myproc()->sz;
    8000314a:	fffff097          	auipc	ra,0xfffff
    8000314e:	894080e7          	jalr	-1900(ra) # 800019de <myproc>
    80003152:	5524                	lw	s1,104(a0)
  if(growproc(n) < 0)
    80003154:	fdc42503          	lw	a0,-36(s0)
    80003158:	fffff097          	auipc	ra,0xfffff
    8000315c:	bf2080e7          	jalr	-1038(ra) # 80001d4a <growproc>
    80003160:	00054863          	bltz	a0,80003170 <sys_sbrk+0x46>
    return -1;
  return addr;
    80003164:	8526                	mv	a0,s1
}
    80003166:	70a2                	ld	ra,40(sp)
    80003168:	7402                	ld	s0,32(sp)
    8000316a:	64e2                	ld	s1,24(sp)
    8000316c:	6145                	addi	sp,sp,48
    8000316e:	8082                	ret
    return -1;
    80003170:	557d                	li	a0,-1
    80003172:	bfd5                	j	80003166 <sys_sbrk+0x3c>

0000000080003174 <sys_sleep>:

uint64
sys_sleep(void)
{
    80003174:	7139                	addi	sp,sp,-64
    80003176:	fc06                	sd	ra,56(sp)
    80003178:	f822                	sd	s0,48(sp)
    8000317a:	f426                	sd	s1,40(sp)
    8000317c:	f04a                	sd	s2,32(sp)
    8000317e:	ec4e                	sd	s3,24(sp)
    80003180:	0080                	addi	s0,sp,64
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
    80003182:	fcc40593          	addi	a1,s0,-52
    80003186:	4501                	li	a0,0
    80003188:	00000097          	auipc	ra,0x0
    8000318c:	e2a080e7          	jalr	-470(ra) # 80002fb2 <argint>
    return -1;
    80003190:	57fd                	li	a5,-1
  if(argint(0, &n) < 0)
    80003192:	06054563          	bltz	a0,800031fc <sys_sleep+0x88>
  acquire(&tickslock);
    80003196:	00014517          	auipc	a0,0x14
    8000319a:	75a50513          	addi	a0,a0,1882 # 800178f0 <tickslock>
    8000319e:	ffffe097          	auipc	ra,0xffffe
    800031a2:	a46080e7          	jalr	-1466(ra) # 80000be4 <acquire>
  ticks0 = ticks;
    800031a6:	00006917          	auipc	s2,0x6
    800031aa:	eaa92903          	lw	s2,-342(s2) # 80009050 <ticks>
  while(ticks - ticks0 < n){
    800031ae:	fcc42783          	lw	a5,-52(s0)
    800031b2:	cf85                	beqz	a5,800031ea <sys_sleep+0x76>
    if(myproc()->killed){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
    800031b4:	00014997          	auipc	s3,0x14
    800031b8:	73c98993          	addi	s3,s3,1852 # 800178f0 <tickslock>
    800031bc:	00006497          	auipc	s1,0x6
    800031c0:	e9448493          	addi	s1,s1,-364 # 80009050 <ticks>
    if(myproc()->killed){
    800031c4:	fffff097          	auipc	ra,0xfffff
    800031c8:	81a080e7          	jalr	-2022(ra) # 800019de <myproc>
    800031cc:	551c                	lw	a5,40(a0)
    800031ce:	ef9d                	bnez	a5,8000320c <sys_sleep+0x98>
    sleep(&ticks, &tickslock);
    800031d0:	85ce                	mv	a1,s3
    800031d2:	8526                	mv	a0,s1
    800031d4:	fffff097          	auipc	ra,0xfffff
    800031d8:	1dc080e7          	jalr	476(ra) # 800023b0 <sleep>
  while(ticks - ticks0 < n){
    800031dc:	409c                	lw	a5,0(s1)
    800031de:	412787bb          	subw	a5,a5,s2
    800031e2:	fcc42703          	lw	a4,-52(s0)
    800031e6:	fce7efe3          	bltu	a5,a4,800031c4 <sys_sleep+0x50>
  }
  release(&tickslock);
    800031ea:	00014517          	auipc	a0,0x14
    800031ee:	70650513          	addi	a0,a0,1798 # 800178f0 <tickslock>
    800031f2:	ffffe097          	auipc	ra,0xffffe
    800031f6:	aa6080e7          	jalr	-1370(ra) # 80000c98 <release>
  return 0;
    800031fa:	4781                	li	a5,0
}
    800031fc:	853e                	mv	a0,a5
    800031fe:	70e2                	ld	ra,56(sp)
    80003200:	7442                	ld	s0,48(sp)
    80003202:	74a2                	ld	s1,40(sp)
    80003204:	7902                	ld	s2,32(sp)
    80003206:	69e2                	ld	s3,24(sp)
    80003208:	6121                	addi	sp,sp,64
    8000320a:	8082                	ret
      release(&tickslock);
    8000320c:	00014517          	auipc	a0,0x14
    80003210:	6e450513          	addi	a0,a0,1764 # 800178f0 <tickslock>
    80003214:	ffffe097          	auipc	ra,0xffffe
    80003218:	a84080e7          	jalr	-1404(ra) # 80000c98 <release>
      return -1;
    8000321c:	57fd                	li	a5,-1
    8000321e:	bff9                	j	800031fc <sys_sleep+0x88>

0000000080003220 <sys_kill>:

uint64
sys_kill(void)
{
    80003220:	1101                	addi	sp,sp,-32
    80003222:	ec06                	sd	ra,24(sp)
    80003224:	e822                	sd	s0,16(sp)
    80003226:	1000                	addi	s0,sp,32
  int pid;

  if(argint(0, &pid) < 0)
    80003228:	fec40593          	addi	a1,s0,-20
    8000322c:	4501                	li	a0,0
    8000322e:	00000097          	auipc	ra,0x0
    80003232:	d84080e7          	jalr	-636(ra) # 80002fb2 <argint>
    80003236:	87aa                	mv	a5,a0
    return -1;
    80003238:	557d                	li	a0,-1
  if(argint(0, &pid) < 0)
    8000323a:	0007c863          	bltz	a5,8000324a <sys_kill+0x2a>
  return kill(pid);
    8000323e:	fec42503          	lw	a0,-20(s0)
    80003242:	fffff097          	auipc	ra,0xfffff
    80003246:	55c080e7          	jalr	1372(ra) # 8000279e <kill>
}
    8000324a:	60e2                	ld	ra,24(sp)
    8000324c:	6442                	ld	s0,16(sp)
    8000324e:	6105                	addi	sp,sp,32
    80003250:	8082                	ret

0000000080003252 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
uint64
sys_uptime(void)
{
    80003252:	1101                	addi	sp,sp,-32
    80003254:	ec06                	sd	ra,24(sp)
    80003256:	e822                	sd	s0,16(sp)
    80003258:	e426                	sd	s1,8(sp)
    8000325a:	1000                	addi	s0,sp,32
  uint xticks;

  acquire(&tickslock);
    8000325c:	00014517          	auipc	a0,0x14
    80003260:	69450513          	addi	a0,a0,1684 # 800178f0 <tickslock>
    80003264:	ffffe097          	auipc	ra,0xffffe
    80003268:	980080e7          	jalr	-1664(ra) # 80000be4 <acquire>
  xticks = ticks;
    8000326c:	00006497          	auipc	s1,0x6
    80003270:	de44a483          	lw	s1,-540(s1) # 80009050 <ticks>
  release(&tickslock);
    80003274:	00014517          	auipc	a0,0x14
    80003278:	67c50513          	addi	a0,a0,1660 # 800178f0 <tickslock>
    8000327c:	ffffe097          	auipc	ra,0xffffe
    80003280:	a1c080e7          	jalr	-1508(ra) # 80000c98 <release>
  return xticks;
}
    80003284:	02049513          	slli	a0,s1,0x20
    80003288:	9101                	srli	a0,a0,0x20
    8000328a:	60e2                	ld	ra,24(sp)
    8000328c:	6442                	ld	s0,16(sp)
    8000328e:	64a2                	ld	s1,8(sp)
    80003290:	6105                	addi	sp,sp,32
    80003292:	8082                	ret

0000000080003294 <sys_pause_system>:

uint64
sys_pause_system(void)
{
    80003294:	1101                	addi	sp,sp,-32
    80003296:	ec06                	sd	ra,24(sp)
    80003298:	e822                	sd	s0,16(sp)
    8000329a:	1000                	addi	s0,sp,32
  int seconds;
  if(argint(0, &seconds) >= 0)
    8000329c:	fec40593          	addi	a1,s0,-20
    800032a0:	4501                	li	a0,0
    800032a2:	00000097          	auipc	ra,0x0
    800032a6:	d10080e7          	jalr	-752(ra) # 80002fb2 <argint>
    800032aa:	87aa                	mv	a5,a0
  {
    return pause_system(seconds);
  }
  return -1;
    800032ac:	557d                	li	a0,-1
  if(argint(0, &seconds) >= 0)
    800032ae:	0007d663          	bgez	a5,800032ba <sys_pause_system+0x26>
}
    800032b2:	60e2                	ld	ra,24(sp)
    800032b4:	6442                	ld	s0,16(sp)
    800032b6:	6105                	addi	sp,sp,32
    800032b8:	8082                	ret
    return pause_system(seconds);
    800032ba:	fec42503          	lw	a0,-20(s0)
    800032be:	fffff097          	auipc	ra,0xfffff
    800032c2:	6c2080e7          	jalr	1730(ra) # 80002980 <pause_system>
    800032c6:	b7f5                	j	800032b2 <sys_pause_system+0x1e>

00000000800032c8 <sys_kill_system>:

uint64
sys_kill_system(void)
{
    800032c8:	1141                	addi	sp,sp,-16
    800032ca:	e406                	sd	ra,8(sp)
    800032cc:	e022                	sd	s0,0(sp)
    800032ce:	0800                	addi	s0,sp,16
  return kill_system();
    800032d0:	fffff097          	auipc	ra,0xfffff
    800032d4:	6e6080e7          	jalr	1766(ra) # 800029b6 <kill_system>
}
    800032d8:	60a2                	ld	ra,8(sp)
    800032da:	6402                	ld	s0,0(sp)
    800032dc:	0141                	addi	sp,sp,16
    800032de:	8082                	ret

00000000800032e0 <sys_print_stats>:

uint64
sys_print_stats(void)
{
    800032e0:	1141                	addi	sp,sp,-16
    800032e2:	e406                	sd	ra,8(sp)
    800032e4:	e022                	sd	s0,0(sp)
    800032e6:	0800                	addi	s0,sp,16
  print_stats();
    800032e8:	fffff097          	auipc	ra,0xfffff
    800032ec:	738080e7          	jalr	1848(ra) # 80002a20 <print_stats>
  return 0;
    800032f0:	4501                	li	a0,0
    800032f2:	60a2                	ld	ra,8(sp)
    800032f4:	6402                	ld	s0,0(sp)
    800032f6:	0141                	addi	sp,sp,16
    800032f8:	8082                	ret

00000000800032fa <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
    800032fa:	7179                	addi	sp,sp,-48
    800032fc:	f406                	sd	ra,40(sp)
    800032fe:	f022                	sd	s0,32(sp)
    80003300:	ec26                	sd	s1,24(sp)
    80003302:	e84a                	sd	s2,16(sp)
    80003304:	e44e                	sd	s3,8(sp)
    80003306:	e052                	sd	s4,0(sp)
    80003308:	1800                	addi	s0,sp,48
  struct buf *b;

  initlock(&bcache.lock, "bcache");
    8000330a:	00005597          	auipc	a1,0x5
    8000330e:	2be58593          	addi	a1,a1,702 # 800085c8 <syscalls+0xc8>
    80003312:	00014517          	auipc	a0,0x14
    80003316:	5f650513          	addi	a0,a0,1526 # 80017908 <bcache>
    8000331a:	ffffe097          	auipc	ra,0xffffe
    8000331e:	83a080e7          	jalr	-1990(ra) # 80000b54 <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80003322:	0001c797          	auipc	a5,0x1c
    80003326:	5e678793          	addi	a5,a5,1510 # 8001f908 <bcache+0x8000>
    8000332a:	0001d717          	auipc	a4,0x1d
    8000332e:	84670713          	addi	a4,a4,-1978 # 8001fb70 <bcache+0x8268>
    80003332:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80003336:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    8000333a:	00014497          	auipc	s1,0x14
    8000333e:	5e648493          	addi	s1,s1,1510 # 80017920 <bcache+0x18>
    b->next = bcache.head.next;
    80003342:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80003344:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    80003346:	00005a17          	auipc	s4,0x5
    8000334a:	28aa0a13          	addi	s4,s4,650 # 800085d0 <syscalls+0xd0>
    b->next = bcache.head.next;
    8000334e:	2b893783          	ld	a5,696(s2)
    80003352:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    80003354:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    80003358:	85d2                	mv	a1,s4
    8000335a:	01048513          	addi	a0,s1,16
    8000335e:	00001097          	auipc	ra,0x1
    80003362:	4bc080e7          	jalr	1212(ra) # 8000481a <initsleeplock>
    bcache.head.next->prev = b;
    80003366:	2b893783          	ld	a5,696(s2)
    8000336a:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    8000336c:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80003370:	45848493          	addi	s1,s1,1112
    80003374:	fd349de3          	bne	s1,s3,8000334e <binit+0x54>
  }
}
    80003378:	70a2                	ld	ra,40(sp)
    8000337a:	7402                	ld	s0,32(sp)
    8000337c:	64e2                	ld	s1,24(sp)
    8000337e:	6942                	ld	s2,16(sp)
    80003380:	69a2                	ld	s3,8(sp)
    80003382:	6a02                	ld	s4,0(sp)
    80003384:	6145                	addi	sp,sp,48
    80003386:	8082                	ret

0000000080003388 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
    80003388:	7179                	addi	sp,sp,-48
    8000338a:	f406                	sd	ra,40(sp)
    8000338c:	f022                	sd	s0,32(sp)
    8000338e:	ec26                	sd	s1,24(sp)
    80003390:	e84a                	sd	s2,16(sp)
    80003392:	e44e                	sd	s3,8(sp)
    80003394:	1800                	addi	s0,sp,48
    80003396:	89aa                	mv	s3,a0
    80003398:	892e                	mv	s2,a1
  acquire(&bcache.lock);
    8000339a:	00014517          	auipc	a0,0x14
    8000339e:	56e50513          	addi	a0,a0,1390 # 80017908 <bcache>
    800033a2:	ffffe097          	auipc	ra,0xffffe
    800033a6:	842080e7          	jalr	-1982(ra) # 80000be4 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    800033aa:	0001d497          	auipc	s1,0x1d
    800033ae:	8164b483          	ld	s1,-2026(s1) # 8001fbc0 <bcache+0x82b8>
    800033b2:	0001c797          	auipc	a5,0x1c
    800033b6:	7be78793          	addi	a5,a5,1982 # 8001fb70 <bcache+0x8268>
    800033ba:	02f48f63          	beq	s1,a5,800033f8 <bread+0x70>
    800033be:	873e                	mv	a4,a5
    800033c0:	a021                	j	800033c8 <bread+0x40>
    800033c2:	68a4                	ld	s1,80(s1)
    800033c4:	02e48a63          	beq	s1,a4,800033f8 <bread+0x70>
    if(b->dev == dev && b->blockno == blockno){
    800033c8:	449c                	lw	a5,8(s1)
    800033ca:	ff379ce3          	bne	a5,s3,800033c2 <bread+0x3a>
    800033ce:	44dc                	lw	a5,12(s1)
    800033d0:	ff2799e3          	bne	a5,s2,800033c2 <bread+0x3a>
      b->refcnt++;
    800033d4:	40bc                	lw	a5,64(s1)
    800033d6:	2785                	addiw	a5,a5,1
    800033d8:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    800033da:	00014517          	auipc	a0,0x14
    800033de:	52e50513          	addi	a0,a0,1326 # 80017908 <bcache>
    800033e2:	ffffe097          	auipc	ra,0xffffe
    800033e6:	8b6080e7          	jalr	-1866(ra) # 80000c98 <release>
      acquiresleep(&b->lock);
    800033ea:	01048513          	addi	a0,s1,16
    800033ee:	00001097          	auipc	ra,0x1
    800033f2:	466080e7          	jalr	1126(ra) # 80004854 <acquiresleep>
      return b;
    800033f6:	a8b9                	j	80003454 <bread+0xcc>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    800033f8:	0001c497          	auipc	s1,0x1c
    800033fc:	7c04b483          	ld	s1,1984(s1) # 8001fbb8 <bcache+0x82b0>
    80003400:	0001c797          	auipc	a5,0x1c
    80003404:	77078793          	addi	a5,a5,1904 # 8001fb70 <bcache+0x8268>
    80003408:	00f48863          	beq	s1,a5,80003418 <bread+0x90>
    8000340c:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    8000340e:	40bc                	lw	a5,64(s1)
    80003410:	cf81                	beqz	a5,80003428 <bread+0xa0>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80003412:	64a4                	ld	s1,72(s1)
    80003414:	fee49de3          	bne	s1,a4,8000340e <bread+0x86>
  panic("bget: no buffers");
    80003418:	00005517          	auipc	a0,0x5
    8000341c:	1c050513          	addi	a0,a0,448 # 800085d8 <syscalls+0xd8>
    80003420:	ffffd097          	auipc	ra,0xffffd
    80003424:	11e080e7          	jalr	286(ra) # 8000053e <panic>
      b->dev = dev;
    80003428:	0134a423          	sw	s3,8(s1)
      b->blockno = blockno;
    8000342c:	0124a623          	sw	s2,12(s1)
      b->valid = 0;
    80003430:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    80003434:	4785                	li	a5,1
    80003436:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80003438:	00014517          	auipc	a0,0x14
    8000343c:	4d050513          	addi	a0,a0,1232 # 80017908 <bcache>
    80003440:	ffffe097          	auipc	ra,0xffffe
    80003444:	858080e7          	jalr	-1960(ra) # 80000c98 <release>
      acquiresleep(&b->lock);
    80003448:	01048513          	addi	a0,s1,16
    8000344c:	00001097          	auipc	ra,0x1
    80003450:	408080e7          	jalr	1032(ra) # 80004854 <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    80003454:	409c                	lw	a5,0(s1)
    80003456:	cb89                	beqz	a5,80003468 <bread+0xe0>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
}
    80003458:	8526                	mv	a0,s1
    8000345a:	70a2                	ld	ra,40(sp)
    8000345c:	7402                	ld	s0,32(sp)
    8000345e:	64e2                	ld	s1,24(sp)
    80003460:	6942                	ld	s2,16(sp)
    80003462:	69a2                	ld	s3,8(sp)
    80003464:	6145                	addi	sp,sp,48
    80003466:	8082                	ret
    virtio_disk_rw(b, 0);
    80003468:	4581                	li	a1,0
    8000346a:	8526                	mv	a0,s1
    8000346c:	00003097          	auipc	ra,0x3
    80003470:	f0a080e7          	jalr	-246(ra) # 80006376 <virtio_disk_rw>
    b->valid = 1;
    80003474:	4785                	li	a5,1
    80003476:	c09c                	sw	a5,0(s1)
  return b;
    80003478:	b7c5                	j	80003458 <bread+0xd0>

000000008000347a <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
    8000347a:	1101                	addi	sp,sp,-32
    8000347c:	ec06                	sd	ra,24(sp)
    8000347e:	e822                	sd	s0,16(sp)
    80003480:	e426                	sd	s1,8(sp)
    80003482:	1000                	addi	s0,sp,32
    80003484:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80003486:	0541                	addi	a0,a0,16
    80003488:	00001097          	auipc	ra,0x1
    8000348c:	466080e7          	jalr	1126(ra) # 800048ee <holdingsleep>
    80003490:	cd01                	beqz	a0,800034a8 <bwrite+0x2e>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80003492:	4585                	li	a1,1
    80003494:	8526                	mv	a0,s1
    80003496:	00003097          	auipc	ra,0x3
    8000349a:	ee0080e7          	jalr	-288(ra) # 80006376 <virtio_disk_rw>
}
    8000349e:	60e2                	ld	ra,24(sp)
    800034a0:	6442                	ld	s0,16(sp)
    800034a2:	64a2                	ld	s1,8(sp)
    800034a4:	6105                	addi	sp,sp,32
    800034a6:	8082                	ret
    panic("bwrite");
    800034a8:	00005517          	auipc	a0,0x5
    800034ac:	14850513          	addi	a0,a0,328 # 800085f0 <syscalls+0xf0>
    800034b0:	ffffd097          	auipc	ra,0xffffd
    800034b4:	08e080e7          	jalr	142(ra) # 8000053e <panic>

00000000800034b8 <brelse>:

// Release a locked buffer.
// Move to the head of the most-recently-used list.
void
brelse(struct buf *b)
{
    800034b8:	1101                	addi	sp,sp,-32
    800034ba:	ec06                	sd	ra,24(sp)
    800034bc:	e822                	sd	s0,16(sp)
    800034be:	e426                	sd	s1,8(sp)
    800034c0:	e04a                	sd	s2,0(sp)
    800034c2:	1000                	addi	s0,sp,32
    800034c4:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    800034c6:	01050913          	addi	s2,a0,16
    800034ca:	854a                	mv	a0,s2
    800034cc:	00001097          	auipc	ra,0x1
    800034d0:	422080e7          	jalr	1058(ra) # 800048ee <holdingsleep>
    800034d4:	c92d                	beqz	a0,80003546 <brelse+0x8e>
    panic("brelse");

  releasesleep(&b->lock);
    800034d6:	854a                	mv	a0,s2
    800034d8:	00001097          	auipc	ra,0x1
    800034dc:	3d2080e7          	jalr	978(ra) # 800048aa <releasesleep>

  acquire(&bcache.lock);
    800034e0:	00014517          	auipc	a0,0x14
    800034e4:	42850513          	addi	a0,a0,1064 # 80017908 <bcache>
    800034e8:	ffffd097          	auipc	ra,0xffffd
    800034ec:	6fc080e7          	jalr	1788(ra) # 80000be4 <acquire>
  b->refcnt--;
    800034f0:	40bc                	lw	a5,64(s1)
    800034f2:	37fd                	addiw	a5,a5,-1
    800034f4:	0007871b          	sext.w	a4,a5
    800034f8:	c0bc                	sw	a5,64(s1)
  if (b->refcnt == 0) {
    800034fa:	eb05                	bnez	a4,8000352a <brelse+0x72>
    // no one is waiting for it.
    b->next->prev = b->prev;
    800034fc:	68bc                	ld	a5,80(s1)
    800034fe:	64b8                	ld	a4,72(s1)
    80003500:	e7b8                	sd	a4,72(a5)
    b->prev->next = b->next;
    80003502:	64bc                	ld	a5,72(s1)
    80003504:	68b8                	ld	a4,80(s1)
    80003506:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    80003508:	0001c797          	auipc	a5,0x1c
    8000350c:	40078793          	addi	a5,a5,1024 # 8001f908 <bcache+0x8000>
    80003510:	2b87b703          	ld	a4,696(a5)
    80003514:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80003516:	0001c717          	auipc	a4,0x1c
    8000351a:	65a70713          	addi	a4,a4,1626 # 8001fb70 <bcache+0x8268>
    8000351e:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    80003520:	2b87b703          	ld	a4,696(a5)
    80003524:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    80003526:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    8000352a:	00014517          	auipc	a0,0x14
    8000352e:	3de50513          	addi	a0,a0,990 # 80017908 <bcache>
    80003532:	ffffd097          	auipc	ra,0xffffd
    80003536:	766080e7          	jalr	1894(ra) # 80000c98 <release>
}
    8000353a:	60e2                	ld	ra,24(sp)
    8000353c:	6442                	ld	s0,16(sp)
    8000353e:	64a2                	ld	s1,8(sp)
    80003540:	6902                	ld	s2,0(sp)
    80003542:	6105                	addi	sp,sp,32
    80003544:	8082                	ret
    panic("brelse");
    80003546:	00005517          	auipc	a0,0x5
    8000354a:	0b250513          	addi	a0,a0,178 # 800085f8 <syscalls+0xf8>
    8000354e:	ffffd097          	auipc	ra,0xffffd
    80003552:	ff0080e7          	jalr	-16(ra) # 8000053e <panic>

0000000080003556 <bpin>:

void
bpin(struct buf *b) {
    80003556:	1101                	addi	sp,sp,-32
    80003558:	ec06                	sd	ra,24(sp)
    8000355a:	e822                	sd	s0,16(sp)
    8000355c:	e426                	sd	s1,8(sp)
    8000355e:	1000                	addi	s0,sp,32
    80003560:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80003562:	00014517          	auipc	a0,0x14
    80003566:	3a650513          	addi	a0,a0,934 # 80017908 <bcache>
    8000356a:	ffffd097          	auipc	ra,0xffffd
    8000356e:	67a080e7          	jalr	1658(ra) # 80000be4 <acquire>
  b->refcnt++;
    80003572:	40bc                	lw	a5,64(s1)
    80003574:	2785                	addiw	a5,a5,1
    80003576:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80003578:	00014517          	auipc	a0,0x14
    8000357c:	39050513          	addi	a0,a0,912 # 80017908 <bcache>
    80003580:	ffffd097          	auipc	ra,0xffffd
    80003584:	718080e7          	jalr	1816(ra) # 80000c98 <release>
}
    80003588:	60e2                	ld	ra,24(sp)
    8000358a:	6442                	ld	s0,16(sp)
    8000358c:	64a2                	ld	s1,8(sp)
    8000358e:	6105                	addi	sp,sp,32
    80003590:	8082                	ret

0000000080003592 <bunpin>:

void
bunpin(struct buf *b) {
    80003592:	1101                	addi	sp,sp,-32
    80003594:	ec06                	sd	ra,24(sp)
    80003596:	e822                	sd	s0,16(sp)
    80003598:	e426                	sd	s1,8(sp)
    8000359a:	1000                	addi	s0,sp,32
    8000359c:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    8000359e:	00014517          	auipc	a0,0x14
    800035a2:	36a50513          	addi	a0,a0,874 # 80017908 <bcache>
    800035a6:	ffffd097          	auipc	ra,0xffffd
    800035aa:	63e080e7          	jalr	1598(ra) # 80000be4 <acquire>
  b->refcnt--;
    800035ae:	40bc                	lw	a5,64(s1)
    800035b0:	37fd                	addiw	a5,a5,-1
    800035b2:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    800035b4:	00014517          	auipc	a0,0x14
    800035b8:	35450513          	addi	a0,a0,852 # 80017908 <bcache>
    800035bc:	ffffd097          	auipc	ra,0xffffd
    800035c0:	6dc080e7          	jalr	1756(ra) # 80000c98 <release>
}
    800035c4:	60e2                	ld	ra,24(sp)
    800035c6:	6442                	ld	s0,16(sp)
    800035c8:	64a2                	ld	s1,8(sp)
    800035ca:	6105                	addi	sp,sp,32
    800035cc:	8082                	ret

00000000800035ce <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
    800035ce:	1101                	addi	sp,sp,-32
    800035d0:	ec06                	sd	ra,24(sp)
    800035d2:	e822                	sd	s0,16(sp)
    800035d4:	e426                	sd	s1,8(sp)
    800035d6:	e04a                	sd	s2,0(sp)
    800035d8:	1000                	addi	s0,sp,32
    800035da:	84ae                	mv	s1,a1
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
    800035dc:	00d5d59b          	srliw	a1,a1,0xd
    800035e0:	0001d797          	auipc	a5,0x1d
    800035e4:	a047a783          	lw	a5,-1532(a5) # 8001ffe4 <sb+0x1c>
    800035e8:	9dbd                	addw	a1,a1,a5
    800035ea:	00000097          	auipc	ra,0x0
    800035ee:	d9e080e7          	jalr	-610(ra) # 80003388 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    800035f2:	0074f713          	andi	a4,s1,7
    800035f6:	4785                	li	a5,1
    800035f8:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    800035fc:	14ce                	slli	s1,s1,0x33
    800035fe:	90d9                	srli	s1,s1,0x36
    80003600:	00950733          	add	a4,a0,s1
    80003604:	05874703          	lbu	a4,88(a4)
    80003608:	00e7f6b3          	and	a3,a5,a4
    8000360c:	c69d                	beqz	a3,8000363a <bfree+0x6c>
    8000360e:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    80003610:	94aa                	add	s1,s1,a0
    80003612:	fff7c793          	not	a5,a5
    80003616:	8ff9                	and	a5,a5,a4
    80003618:	04f48c23          	sb	a5,88(s1)
  log_write(bp);
    8000361c:	00001097          	auipc	ra,0x1
    80003620:	118080e7          	jalr	280(ra) # 80004734 <log_write>
  brelse(bp);
    80003624:	854a                	mv	a0,s2
    80003626:	00000097          	auipc	ra,0x0
    8000362a:	e92080e7          	jalr	-366(ra) # 800034b8 <brelse>
}
    8000362e:	60e2                	ld	ra,24(sp)
    80003630:	6442                	ld	s0,16(sp)
    80003632:	64a2                	ld	s1,8(sp)
    80003634:	6902                	ld	s2,0(sp)
    80003636:	6105                	addi	sp,sp,32
    80003638:	8082                	ret
    panic("freeing free block");
    8000363a:	00005517          	auipc	a0,0x5
    8000363e:	fc650513          	addi	a0,a0,-58 # 80008600 <syscalls+0x100>
    80003642:	ffffd097          	auipc	ra,0xffffd
    80003646:	efc080e7          	jalr	-260(ra) # 8000053e <panic>

000000008000364a <balloc>:
{
    8000364a:	711d                	addi	sp,sp,-96
    8000364c:	ec86                	sd	ra,88(sp)
    8000364e:	e8a2                	sd	s0,80(sp)
    80003650:	e4a6                	sd	s1,72(sp)
    80003652:	e0ca                	sd	s2,64(sp)
    80003654:	fc4e                	sd	s3,56(sp)
    80003656:	f852                	sd	s4,48(sp)
    80003658:	f456                	sd	s5,40(sp)
    8000365a:	f05a                	sd	s6,32(sp)
    8000365c:	ec5e                	sd	s7,24(sp)
    8000365e:	e862                	sd	s8,16(sp)
    80003660:	e466                	sd	s9,8(sp)
    80003662:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80003664:	0001d797          	auipc	a5,0x1d
    80003668:	9687a783          	lw	a5,-1688(a5) # 8001ffcc <sb+0x4>
    8000366c:	cbd1                	beqz	a5,80003700 <balloc+0xb6>
    8000366e:	8baa                	mv	s7,a0
    80003670:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80003672:	0001db17          	auipc	s6,0x1d
    80003676:	956b0b13          	addi	s6,s6,-1706 # 8001ffc8 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    8000367a:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    8000367c:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    8000367e:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80003680:	6c89                	lui	s9,0x2
    80003682:	a831                	j	8000369e <balloc+0x54>
    brelse(bp);
    80003684:	854a                	mv	a0,s2
    80003686:	00000097          	auipc	ra,0x0
    8000368a:	e32080e7          	jalr	-462(ra) # 800034b8 <brelse>
  for(b = 0; b < sb.size; b += BPB){
    8000368e:	015c87bb          	addw	a5,s9,s5
    80003692:	00078a9b          	sext.w	s5,a5
    80003696:	004b2703          	lw	a4,4(s6)
    8000369a:	06eaf363          	bgeu	s5,a4,80003700 <balloc+0xb6>
    bp = bread(dev, BBLOCK(b, sb));
    8000369e:	41fad79b          	sraiw	a5,s5,0x1f
    800036a2:	0137d79b          	srliw	a5,a5,0x13
    800036a6:	015787bb          	addw	a5,a5,s5
    800036aa:	40d7d79b          	sraiw	a5,a5,0xd
    800036ae:	01cb2583          	lw	a1,28(s6)
    800036b2:	9dbd                	addw	a1,a1,a5
    800036b4:	855e                	mv	a0,s7
    800036b6:	00000097          	auipc	ra,0x0
    800036ba:	cd2080e7          	jalr	-814(ra) # 80003388 <bread>
    800036be:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    800036c0:	004b2503          	lw	a0,4(s6)
    800036c4:	000a849b          	sext.w	s1,s5
    800036c8:	8662                	mv	a2,s8
    800036ca:	faa4fde3          	bgeu	s1,a0,80003684 <balloc+0x3a>
      m = 1 << (bi % 8);
    800036ce:	41f6579b          	sraiw	a5,a2,0x1f
    800036d2:	01d7d69b          	srliw	a3,a5,0x1d
    800036d6:	00c6873b          	addw	a4,a3,a2
    800036da:	00777793          	andi	a5,a4,7
    800036de:	9f95                	subw	a5,a5,a3
    800036e0:	00f997bb          	sllw	a5,s3,a5
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    800036e4:	4037571b          	sraiw	a4,a4,0x3
    800036e8:	00e906b3          	add	a3,s2,a4
    800036ec:	0586c683          	lbu	a3,88(a3)
    800036f0:	00d7f5b3          	and	a1,a5,a3
    800036f4:	cd91                	beqz	a1,80003710 <balloc+0xc6>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    800036f6:	2605                	addiw	a2,a2,1
    800036f8:	2485                	addiw	s1,s1,1
    800036fa:	fd4618e3          	bne	a2,s4,800036ca <balloc+0x80>
    800036fe:	b759                	j	80003684 <balloc+0x3a>
  panic("balloc: out of blocks");
    80003700:	00005517          	auipc	a0,0x5
    80003704:	f1850513          	addi	a0,a0,-232 # 80008618 <syscalls+0x118>
    80003708:	ffffd097          	auipc	ra,0xffffd
    8000370c:	e36080e7          	jalr	-458(ra) # 8000053e <panic>
        bp->data[bi/8] |= m;  // Mark block in use.
    80003710:	974a                	add	a4,a4,s2
    80003712:	8fd5                	or	a5,a5,a3
    80003714:	04f70c23          	sb	a5,88(a4)
        log_write(bp);
    80003718:	854a                	mv	a0,s2
    8000371a:	00001097          	auipc	ra,0x1
    8000371e:	01a080e7          	jalr	26(ra) # 80004734 <log_write>
        brelse(bp);
    80003722:	854a                	mv	a0,s2
    80003724:	00000097          	auipc	ra,0x0
    80003728:	d94080e7          	jalr	-620(ra) # 800034b8 <brelse>
  bp = bread(dev, bno);
    8000372c:	85a6                	mv	a1,s1
    8000372e:	855e                	mv	a0,s7
    80003730:	00000097          	auipc	ra,0x0
    80003734:	c58080e7          	jalr	-936(ra) # 80003388 <bread>
    80003738:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    8000373a:	40000613          	li	a2,1024
    8000373e:	4581                	li	a1,0
    80003740:	05850513          	addi	a0,a0,88
    80003744:	ffffd097          	auipc	ra,0xffffd
    80003748:	59c080e7          	jalr	1436(ra) # 80000ce0 <memset>
  log_write(bp);
    8000374c:	854a                	mv	a0,s2
    8000374e:	00001097          	auipc	ra,0x1
    80003752:	fe6080e7          	jalr	-26(ra) # 80004734 <log_write>
  brelse(bp);
    80003756:	854a                	mv	a0,s2
    80003758:	00000097          	auipc	ra,0x0
    8000375c:	d60080e7          	jalr	-672(ra) # 800034b8 <brelse>
}
    80003760:	8526                	mv	a0,s1
    80003762:	60e6                	ld	ra,88(sp)
    80003764:	6446                	ld	s0,80(sp)
    80003766:	64a6                	ld	s1,72(sp)
    80003768:	6906                	ld	s2,64(sp)
    8000376a:	79e2                	ld	s3,56(sp)
    8000376c:	7a42                	ld	s4,48(sp)
    8000376e:	7aa2                	ld	s5,40(sp)
    80003770:	7b02                	ld	s6,32(sp)
    80003772:	6be2                	ld	s7,24(sp)
    80003774:	6c42                	ld	s8,16(sp)
    80003776:	6ca2                	ld	s9,8(sp)
    80003778:	6125                	addi	sp,sp,96
    8000377a:	8082                	ret

000000008000377c <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
    8000377c:	7179                	addi	sp,sp,-48
    8000377e:	f406                	sd	ra,40(sp)
    80003780:	f022                	sd	s0,32(sp)
    80003782:	ec26                	sd	s1,24(sp)
    80003784:	e84a                	sd	s2,16(sp)
    80003786:	e44e                	sd	s3,8(sp)
    80003788:	e052                	sd	s4,0(sp)
    8000378a:	1800                	addi	s0,sp,48
    8000378c:	892a                	mv	s2,a0
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
    8000378e:	47ad                	li	a5,11
    80003790:	04b7fe63          	bgeu	a5,a1,800037ec <bmap+0x70>
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
    80003794:	ff45849b          	addiw	s1,a1,-12
    80003798:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    8000379c:	0ff00793          	li	a5,255
    800037a0:	0ae7e363          	bltu	a5,a4,80003846 <bmap+0xca>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
    800037a4:	08052583          	lw	a1,128(a0)
    800037a8:	c5ad                	beqz	a1,80003812 <bmap+0x96>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    800037aa:	00092503          	lw	a0,0(s2)
    800037ae:	00000097          	auipc	ra,0x0
    800037b2:	bda080e7          	jalr	-1062(ra) # 80003388 <bread>
    800037b6:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    800037b8:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    800037bc:	02049593          	slli	a1,s1,0x20
    800037c0:	9181                	srli	a1,a1,0x20
    800037c2:	058a                	slli	a1,a1,0x2
    800037c4:	00b784b3          	add	s1,a5,a1
    800037c8:	0004a983          	lw	s3,0(s1)
    800037cc:	04098d63          	beqz	s3,80003826 <bmap+0xaa>
      a[bn] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    800037d0:	8552                	mv	a0,s4
    800037d2:	00000097          	auipc	ra,0x0
    800037d6:	ce6080e7          	jalr	-794(ra) # 800034b8 <brelse>
    return addr;
  }

  panic("bmap: out of range");
}
    800037da:	854e                	mv	a0,s3
    800037dc:	70a2                	ld	ra,40(sp)
    800037de:	7402                	ld	s0,32(sp)
    800037e0:	64e2                	ld	s1,24(sp)
    800037e2:	6942                	ld	s2,16(sp)
    800037e4:	69a2                	ld	s3,8(sp)
    800037e6:	6a02                	ld	s4,0(sp)
    800037e8:	6145                	addi	sp,sp,48
    800037ea:	8082                	ret
    if((addr = ip->addrs[bn]) == 0)
    800037ec:	02059493          	slli	s1,a1,0x20
    800037f0:	9081                	srli	s1,s1,0x20
    800037f2:	048a                	slli	s1,s1,0x2
    800037f4:	94aa                	add	s1,s1,a0
    800037f6:	0504a983          	lw	s3,80(s1)
    800037fa:	fe0990e3          	bnez	s3,800037da <bmap+0x5e>
      ip->addrs[bn] = addr = balloc(ip->dev);
    800037fe:	4108                	lw	a0,0(a0)
    80003800:	00000097          	auipc	ra,0x0
    80003804:	e4a080e7          	jalr	-438(ra) # 8000364a <balloc>
    80003808:	0005099b          	sext.w	s3,a0
    8000380c:	0534a823          	sw	s3,80(s1)
    80003810:	b7e9                	j	800037da <bmap+0x5e>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    80003812:	4108                	lw	a0,0(a0)
    80003814:	00000097          	auipc	ra,0x0
    80003818:	e36080e7          	jalr	-458(ra) # 8000364a <balloc>
    8000381c:	0005059b          	sext.w	a1,a0
    80003820:	08b92023          	sw	a1,128(s2)
    80003824:	b759                	j	800037aa <bmap+0x2e>
      a[bn] = addr = balloc(ip->dev);
    80003826:	00092503          	lw	a0,0(s2)
    8000382a:	00000097          	auipc	ra,0x0
    8000382e:	e20080e7          	jalr	-480(ra) # 8000364a <balloc>
    80003832:	0005099b          	sext.w	s3,a0
    80003836:	0134a023          	sw	s3,0(s1)
      log_write(bp);
    8000383a:	8552                	mv	a0,s4
    8000383c:	00001097          	auipc	ra,0x1
    80003840:	ef8080e7          	jalr	-264(ra) # 80004734 <log_write>
    80003844:	b771                	j	800037d0 <bmap+0x54>
  panic("bmap: out of range");
    80003846:	00005517          	auipc	a0,0x5
    8000384a:	dea50513          	addi	a0,a0,-534 # 80008630 <syscalls+0x130>
    8000384e:	ffffd097          	auipc	ra,0xffffd
    80003852:	cf0080e7          	jalr	-784(ra) # 8000053e <panic>

0000000080003856 <iget>:
{
    80003856:	7179                	addi	sp,sp,-48
    80003858:	f406                	sd	ra,40(sp)
    8000385a:	f022                	sd	s0,32(sp)
    8000385c:	ec26                	sd	s1,24(sp)
    8000385e:	e84a                	sd	s2,16(sp)
    80003860:	e44e                	sd	s3,8(sp)
    80003862:	e052                	sd	s4,0(sp)
    80003864:	1800                	addi	s0,sp,48
    80003866:	89aa                	mv	s3,a0
    80003868:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    8000386a:	0001c517          	auipc	a0,0x1c
    8000386e:	77e50513          	addi	a0,a0,1918 # 8001ffe8 <itable>
    80003872:	ffffd097          	auipc	ra,0xffffd
    80003876:	372080e7          	jalr	882(ra) # 80000be4 <acquire>
  empty = 0;
    8000387a:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    8000387c:	0001c497          	auipc	s1,0x1c
    80003880:	78448493          	addi	s1,s1,1924 # 80020000 <itable+0x18>
    80003884:	0001e697          	auipc	a3,0x1e
    80003888:	20c68693          	addi	a3,a3,524 # 80021a90 <log>
    8000388c:	a039                	j	8000389a <iget+0x44>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    8000388e:	02090b63          	beqz	s2,800038c4 <iget+0x6e>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80003892:	08848493          	addi	s1,s1,136
    80003896:	02d48a63          	beq	s1,a3,800038ca <iget+0x74>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    8000389a:	449c                	lw	a5,8(s1)
    8000389c:	fef059e3          	blez	a5,8000388e <iget+0x38>
    800038a0:	4098                	lw	a4,0(s1)
    800038a2:	ff3716e3          	bne	a4,s3,8000388e <iget+0x38>
    800038a6:	40d8                	lw	a4,4(s1)
    800038a8:	ff4713e3          	bne	a4,s4,8000388e <iget+0x38>
      ip->ref++;
    800038ac:	2785                	addiw	a5,a5,1
    800038ae:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    800038b0:	0001c517          	auipc	a0,0x1c
    800038b4:	73850513          	addi	a0,a0,1848 # 8001ffe8 <itable>
    800038b8:	ffffd097          	auipc	ra,0xffffd
    800038bc:	3e0080e7          	jalr	992(ra) # 80000c98 <release>
      return ip;
    800038c0:	8926                	mv	s2,s1
    800038c2:	a03d                	j	800038f0 <iget+0x9a>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    800038c4:	f7f9                	bnez	a5,80003892 <iget+0x3c>
    800038c6:	8926                	mv	s2,s1
    800038c8:	b7e9                	j	80003892 <iget+0x3c>
  if(empty == 0)
    800038ca:	02090c63          	beqz	s2,80003902 <iget+0xac>
  ip->dev = dev;
    800038ce:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    800038d2:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    800038d6:	4785                	li	a5,1
    800038d8:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    800038dc:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    800038e0:	0001c517          	auipc	a0,0x1c
    800038e4:	70850513          	addi	a0,a0,1800 # 8001ffe8 <itable>
    800038e8:	ffffd097          	auipc	ra,0xffffd
    800038ec:	3b0080e7          	jalr	944(ra) # 80000c98 <release>
}
    800038f0:	854a                	mv	a0,s2
    800038f2:	70a2                	ld	ra,40(sp)
    800038f4:	7402                	ld	s0,32(sp)
    800038f6:	64e2                	ld	s1,24(sp)
    800038f8:	6942                	ld	s2,16(sp)
    800038fa:	69a2                	ld	s3,8(sp)
    800038fc:	6a02                	ld	s4,0(sp)
    800038fe:	6145                	addi	sp,sp,48
    80003900:	8082                	ret
    panic("iget: no inodes");
    80003902:	00005517          	auipc	a0,0x5
    80003906:	d4650513          	addi	a0,a0,-698 # 80008648 <syscalls+0x148>
    8000390a:	ffffd097          	auipc	ra,0xffffd
    8000390e:	c34080e7          	jalr	-972(ra) # 8000053e <panic>

0000000080003912 <fsinit>:
fsinit(int dev) {
    80003912:	7179                	addi	sp,sp,-48
    80003914:	f406                	sd	ra,40(sp)
    80003916:	f022                	sd	s0,32(sp)
    80003918:	ec26                	sd	s1,24(sp)
    8000391a:	e84a                	sd	s2,16(sp)
    8000391c:	e44e                	sd	s3,8(sp)
    8000391e:	1800                	addi	s0,sp,48
    80003920:	892a                	mv	s2,a0
  bp = bread(dev, 1);
    80003922:	4585                	li	a1,1
    80003924:	00000097          	auipc	ra,0x0
    80003928:	a64080e7          	jalr	-1436(ra) # 80003388 <bread>
    8000392c:	84aa                	mv	s1,a0
  memmove(sb, bp->data, sizeof(*sb));
    8000392e:	0001c997          	auipc	s3,0x1c
    80003932:	69a98993          	addi	s3,s3,1690 # 8001ffc8 <sb>
    80003936:	02000613          	li	a2,32
    8000393a:	05850593          	addi	a1,a0,88
    8000393e:	854e                	mv	a0,s3
    80003940:	ffffd097          	auipc	ra,0xffffd
    80003944:	400080e7          	jalr	1024(ra) # 80000d40 <memmove>
  brelse(bp);
    80003948:	8526                	mv	a0,s1
    8000394a:	00000097          	auipc	ra,0x0
    8000394e:	b6e080e7          	jalr	-1170(ra) # 800034b8 <brelse>
  if(sb.magic != FSMAGIC)
    80003952:	0009a703          	lw	a4,0(s3)
    80003956:	102037b7          	lui	a5,0x10203
    8000395a:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    8000395e:	02f71263          	bne	a4,a5,80003982 <fsinit+0x70>
  initlog(dev, &sb);
    80003962:	0001c597          	auipc	a1,0x1c
    80003966:	66658593          	addi	a1,a1,1638 # 8001ffc8 <sb>
    8000396a:	854a                	mv	a0,s2
    8000396c:	00001097          	auipc	ra,0x1
    80003970:	b4c080e7          	jalr	-1204(ra) # 800044b8 <initlog>
}
    80003974:	70a2                	ld	ra,40(sp)
    80003976:	7402                	ld	s0,32(sp)
    80003978:	64e2                	ld	s1,24(sp)
    8000397a:	6942                	ld	s2,16(sp)
    8000397c:	69a2                	ld	s3,8(sp)
    8000397e:	6145                	addi	sp,sp,48
    80003980:	8082                	ret
    panic("invalid file system");
    80003982:	00005517          	auipc	a0,0x5
    80003986:	cd650513          	addi	a0,a0,-810 # 80008658 <syscalls+0x158>
    8000398a:	ffffd097          	auipc	ra,0xffffd
    8000398e:	bb4080e7          	jalr	-1100(ra) # 8000053e <panic>

0000000080003992 <iinit>:
{
    80003992:	7179                	addi	sp,sp,-48
    80003994:	f406                	sd	ra,40(sp)
    80003996:	f022                	sd	s0,32(sp)
    80003998:	ec26                	sd	s1,24(sp)
    8000399a:	e84a                	sd	s2,16(sp)
    8000399c:	e44e                	sd	s3,8(sp)
    8000399e:	1800                	addi	s0,sp,48
  initlock(&itable.lock, "itable");
    800039a0:	00005597          	auipc	a1,0x5
    800039a4:	cd058593          	addi	a1,a1,-816 # 80008670 <syscalls+0x170>
    800039a8:	0001c517          	auipc	a0,0x1c
    800039ac:	64050513          	addi	a0,a0,1600 # 8001ffe8 <itable>
    800039b0:	ffffd097          	auipc	ra,0xffffd
    800039b4:	1a4080e7          	jalr	420(ra) # 80000b54 <initlock>
  for(i = 0; i < NINODE; i++) {
    800039b8:	0001c497          	auipc	s1,0x1c
    800039bc:	65848493          	addi	s1,s1,1624 # 80020010 <itable+0x28>
    800039c0:	0001e997          	auipc	s3,0x1e
    800039c4:	0e098993          	addi	s3,s3,224 # 80021aa0 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    800039c8:	00005917          	auipc	s2,0x5
    800039cc:	cb090913          	addi	s2,s2,-848 # 80008678 <syscalls+0x178>
    800039d0:	85ca                	mv	a1,s2
    800039d2:	8526                	mv	a0,s1
    800039d4:	00001097          	auipc	ra,0x1
    800039d8:	e46080e7          	jalr	-442(ra) # 8000481a <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    800039dc:	08848493          	addi	s1,s1,136
    800039e0:	ff3498e3          	bne	s1,s3,800039d0 <iinit+0x3e>
}
    800039e4:	70a2                	ld	ra,40(sp)
    800039e6:	7402                	ld	s0,32(sp)
    800039e8:	64e2                	ld	s1,24(sp)
    800039ea:	6942                	ld	s2,16(sp)
    800039ec:	69a2                	ld	s3,8(sp)
    800039ee:	6145                	addi	sp,sp,48
    800039f0:	8082                	ret

00000000800039f2 <ialloc>:
{
    800039f2:	715d                	addi	sp,sp,-80
    800039f4:	e486                	sd	ra,72(sp)
    800039f6:	e0a2                	sd	s0,64(sp)
    800039f8:	fc26                	sd	s1,56(sp)
    800039fa:	f84a                	sd	s2,48(sp)
    800039fc:	f44e                	sd	s3,40(sp)
    800039fe:	f052                	sd	s4,32(sp)
    80003a00:	ec56                	sd	s5,24(sp)
    80003a02:	e85a                	sd	s6,16(sp)
    80003a04:	e45e                	sd	s7,8(sp)
    80003a06:	0880                	addi	s0,sp,80
  for(inum = 1; inum < sb.ninodes; inum++){
    80003a08:	0001c717          	auipc	a4,0x1c
    80003a0c:	5cc72703          	lw	a4,1484(a4) # 8001ffd4 <sb+0xc>
    80003a10:	4785                	li	a5,1
    80003a12:	04e7fa63          	bgeu	a5,a4,80003a66 <ialloc+0x74>
    80003a16:	8aaa                	mv	s5,a0
    80003a18:	8bae                	mv	s7,a1
    80003a1a:	4485                	li	s1,1
    bp = bread(dev, IBLOCK(inum, sb));
    80003a1c:	0001ca17          	auipc	s4,0x1c
    80003a20:	5aca0a13          	addi	s4,s4,1452 # 8001ffc8 <sb>
    80003a24:	00048b1b          	sext.w	s6,s1
    80003a28:	0044d593          	srli	a1,s1,0x4
    80003a2c:	018a2783          	lw	a5,24(s4)
    80003a30:	9dbd                	addw	a1,a1,a5
    80003a32:	8556                	mv	a0,s5
    80003a34:	00000097          	auipc	ra,0x0
    80003a38:	954080e7          	jalr	-1708(ra) # 80003388 <bread>
    80003a3c:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    80003a3e:	05850993          	addi	s3,a0,88
    80003a42:	00f4f793          	andi	a5,s1,15
    80003a46:	079a                	slli	a5,a5,0x6
    80003a48:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    80003a4a:	00099783          	lh	a5,0(s3)
    80003a4e:	c785                	beqz	a5,80003a76 <ialloc+0x84>
    brelse(bp);
    80003a50:	00000097          	auipc	ra,0x0
    80003a54:	a68080e7          	jalr	-1432(ra) # 800034b8 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    80003a58:	0485                	addi	s1,s1,1
    80003a5a:	00ca2703          	lw	a4,12(s4)
    80003a5e:	0004879b          	sext.w	a5,s1
    80003a62:	fce7e1e3          	bltu	a5,a4,80003a24 <ialloc+0x32>
  panic("ialloc: no inodes");
    80003a66:	00005517          	auipc	a0,0x5
    80003a6a:	c1a50513          	addi	a0,a0,-998 # 80008680 <syscalls+0x180>
    80003a6e:	ffffd097          	auipc	ra,0xffffd
    80003a72:	ad0080e7          	jalr	-1328(ra) # 8000053e <panic>
      memset(dip, 0, sizeof(*dip));
    80003a76:	04000613          	li	a2,64
    80003a7a:	4581                	li	a1,0
    80003a7c:	854e                	mv	a0,s3
    80003a7e:	ffffd097          	auipc	ra,0xffffd
    80003a82:	262080e7          	jalr	610(ra) # 80000ce0 <memset>
      dip->type = type;
    80003a86:	01799023          	sh	s7,0(s3)
      log_write(bp);   // mark it allocated on the disk
    80003a8a:	854a                	mv	a0,s2
    80003a8c:	00001097          	auipc	ra,0x1
    80003a90:	ca8080e7          	jalr	-856(ra) # 80004734 <log_write>
      brelse(bp);
    80003a94:	854a                	mv	a0,s2
    80003a96:	00000097          	auipc	ra,0x0
    80003a9a:	a22080e7          	jalr	-1502(ra) # 800034b8 <brelse>
      return iget(dev, inum);
    80003a9e:	85da                	mv	a1,s6
    80003aa0:	8556                	mv	a0,s5
    80003aa2:	00000097          	auipc	ra,0x0
    80003aa6:	db4080e7          	jalr	-588(ra) # 80003856 <iget>
}
    80003aaa:	60a6                	ld	ra,72(sp)
    80003aac:	6406                	ld	s0,64(sp)
    80003aae:	74e2                	ld	s1,56(sp)
    80003ab0:	7942                	ld	s2,48(sp)
    80003ab2:	79a2                	ld	s3,40(sp)
    80003ab4:	7a02                	ld	s4,32(sp)
    80003ab6:	6ae2                	ld	s5,24(sp)
    80003ab8:	6b42                	ld	s6,16(sp)
    80003aba:	6ba2                	ld	s7,8(sp)
    80003abc:	6161                	addi	sp,sp,80
    80003abe:	8082                	ret

0000000080003ac0 <iupdate>:
{
    80003ac0:	1101                	addi	sp,sp,-32
    80003ac2:	ec06                	sd	ra,24(sp)
    80003ac4:	e822                	sd	s0,16(sp)
    80003ac6:	e426                	sd	s1,8(sp)
    80003ac8:	e04a                	sd	s2,0(sp)
    80003aca:	1000                	addi	s0,sp,32
    80003acc:	84aa                	mv	s1,a0
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003ace:	415c                	lw	a5,4(a0)
    80003ad0:	0047d79b          	srliw	a5,a5,0x4
    80003ad4:	0001c597          	auipc	a1,0x1c
    80003ad8:	50c5a583          	lw	a1,1292(a1) # 8001ffe0 <sb+0x18>
    80003adc:	9dbd                	addw	a1,a1,a5
    80003ade:	4108                	lw	a0,0(a0)
    80003ae0:	00000097          	auipc	ra,0x0
    80003ae4:	8a8080e7          	jalr	-1880(ra) # 80003388 <bread>
    80003ae8:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003aea:	05850793          	addi	a5,a0,88
    80003aee:	40c8                	lw	a0,4(s1)
    80003af0:	893d                	andi	a0,a0,15
    80003af2:	051a                	slli	a0,a0,0x6
    80003af4:	953e                	add	a0,a0,a5
  dip->type = ip->type;
    80003af6:	04449703          	lh	a4,68(s1)
    80003afa:	00e51023          	sh	a4,0(a0)
  dip->major = ip->major;
    80003afe:	04649703          	lh	a4,70(s1)
    80003b02:	00e51123          	sh	a4,2(a0)
  dip->minor = ip->minor;
    80003b06:	04849703          	lh	a4,72(s1)
    80003b0a:	00e51223          	sh	a4,4(a0)
  dip->nlink = ip->nlink;
    80003b0e:	04a49703          	lh	a4,74(s1)
    80003b12:	00e51323          	sh	a4,6(a0)
  dip->size = ip->size;
    80003b16:	44f8                	lw	a4,76(s1)
    80003b18:	c518                	sw	a4,8(a0)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    80003b1a:	03400613          	li	a2,52
    80003b1e:	05048593          	addi	a1,s1,80
    80003b22:	0531                	addi	a0,a0,12
    80003b24:	ffffd097          	auipc	ra,0xffffd
    80003b28:	21c080e7          	jalr	540(ra) # 80000d40 <memmove>
  log_write(bp);
    80003b2c:	854a                	mv	a0,s2
    80003b2e:	00001097          	auipc	ra,0x1
    80003b32:	c06080e7          	jalr	-1018(ra) # 80004734 <log_write>
  brelse(bp);
    80003b36:	854a                	mv	a0,s2
    80003b38:	00000097          	auipc	ra,0x0
    80003b3c:	980080e7          	jalr	-1664(ra) # 800034b8 <brelse>
}
    80003b40:	60e2                	ld	ra,24(sp)
    80003b42:	6442                	ld	s0,16(sp)
    80003b44:	64a2                	ld	s1,8(sp)
    80003b46:	6902                	ld	s2,0(sp)
    80003b48:	6105                	addi	sp,sp,32
    80003b4a:	8082                	ret

0000000080003b4c <idup>:
{
    80003b4c:	1101                	addi	sp,sp,-32
    80003b4e:	ec06                	sd	ra,24(sp)
    80003b50:	e822                	sd	s0,16(sp)
    80003b52:	e426                	sd	s1,8(sp)
    80003b54:	1000                	addi	s0,sp,32
    80003b56:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    80003b58:	0001c517          	auipc	a0,0x1c
    80003b5c:	49050513          	addi	a0,a0,1168 # 8001ffe8 <itable>
    80003b60:	ffffd097          	auipc	ra,0xffffd
    80003b64:	084080e7          	jalr	132(ra) # 80000be4 <acquire>
  ip->ref++;
    80003b68:	449c                	lw	a5,8(s1)
    80003b6a:	2785                	addiw	a5,a5,1
    80003b6c:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003b6e:	0001c517          	auipc	a0,0x1c
    80003b72:	47a50513          	addi	a0,a0,1146 # 8001ffe8 <itable>
    80003b76:	ffffd097          	auipc	ra,0xffffd
    80003b7a:	122080e7          	jalr	290(ra) # 80000c98 <release>
}
    80003b7e:	8526                	mv	a0,s1
    80003b80:	60e2                	ld	ra,24(sp)
    80003b82:	6442                	ld	s0,16(sp)
    80003b84:	64a2                	ld	s1,8(sp)
    80003b86:	6105                	addi	sp,sp,32
    80003b88:	8082                	ret

0000000080003b8a <ilock>:
{
    80003b8a:	1101                	addi	sp,sp,-32
    80003b8c:	ec06                	sd	ra,24(sp)
    80003b8e:	e822                	sd	s0,16(sp)
    80003b90:	e426                	sd	s1,8(sp)
    80003b92:	e04a                	sd	s2,0(sp)
    80003b94:	1000                	addi	s0,sp,32
  if(ip == 0 || ip->ref < 1)
    80003b96:	c115                	beqz	a0,80003bba <ilock+0x30>
    80003b98:	84aa                	mv	s1,a0
    80003b9a:	451c                	lw	a5,8(a0)
    80003b9c:	00f05f63          	blez	a5,80003bba <ilock+0x30>
  acquiresleep(&ip->lock);
    80003ba0:	0541                	addi	a0,a0,16
    80003ba2:	00001097          	auipc	ra,0x1
    80003ba6:	cb2080e7          	jalr	-846(ra) # 80004854 <acquiresleep>
  if(ip->valid == 0){
    80003baa:	40bc                	lw	a5,64(s1)
    80003bac:	cf99                	beqz	a5,80003bca <ilock+0x40>
}
    80003bae:	60e2                	ld	ra,24(sp)
    80003bb0:	6442                	ld	s0,16(sp)
    80003bb2:	64a2                	ld	s1,8(sp)
    80003bb4:	6902                	ld	s2,0(sp)
    80003bb6:	6105                	addi	sp,sp,32
    80003bb8:	8082                	ret
    panic("ilock");
    80003bba:	00005517          	auipc	a0,0x5
    80003bbe:	ade50513          	addi	a0,a0,-1314 # 80008698 <syscalls+0x198>
    80003bc2:	ffffd097          	auipc	ra,0xffffd
    80003bc6:	97c080e7          	jalr	-1668(ra) # 8000053e <panic>
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003bca:	40dc                	lw	a5,4(s1)
    80003bcc:	0047d79b          	srliw	a5,a5,0x4
    80003bd0:	0001c597          	auipc	a1,0x1c
    80003bd4:	4105a583          	lw	a1,1040(a1) # 8001ffe0 <sb+0x18>
    80003bd8:	9dbd                	addw	a1,a1,a5
    80003bda:	4088                	lw	a0,0(s1)
    80003bdc:	fffff097          	auipc	ra,0xfffff
    80003be0:	7ac080e7          	jalr	1964(ra) # 80003388 <bread>
    80003be4:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003be6:	05850593          	addi	a1,a0,88
    80003bea:	40dc                	lw	a5,4(s1)
    80003bec:	8bbd                	andi	a5,a5,15
    80003bee:	079a                	slli	a5,a5,0x6
    80003bf0:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    80003bf2:	00059783          	lh	a5,0(a1)
    80003bf6:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    80003bfa:	00259783          	lh	a5,2(a1)
    80003bfe:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    80003c02:	00459783          	lh	a5,4(a1)
    80003c06:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    80003c0a:	00659783          	lh	a5,6(a1)
    80003c0e:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    80003c12:	459c                	lw	a5,8(a1)
    80003c14:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    80003c16:	03400613          	li	a2,52
    80003c1a:	05b1                	addi	a1,a1,12
    80003c1c:	05048513          	addi	a0,s1,80
    80003c20:	ffffd097          	auipc	ra,0xffffd
    80003c24:	120080e7          	jalr	288(ra) # 80000d40 <memmove>
    brelse(bp);
    80003c28:	854a                	mv	a0,s2
    80003c2a:	00000097          	auipc	ra,0x0
    80003c2e:	88e080e7          	jalr	-1906(ra) # 800034b8 <brelse>
    ip->valid = 1;
    80003c32:	4785                	li	a5,1
    80003c34:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    80003c36:	04449783          	lh	a5,68(s1)
    80003c3a:	fbb5                	bnez	a5,80003bae <ilock+0x24>
      panic("ilock: no type");
    80003c3c:	00005517          	auipc	a0,0x5
    80003c40:	a6450513          	addi	a0,a0,-1436 # 800086a0 <syscalls+0x1a0>
    80003c44:	ffffd097          	auipc	ra,0xffffd
    80003c48:	8fa080e7          	jalr	-1798(ra) # 8000053e <panic>

0000000080003c4c <iunlock>:
{
    80003c4c:	1101                	addi	sp,sp,-32
    80003c4e:	ec06                	sd	ra,24(sp)
    80003c50:	e822                	sd	s0,16(sp)
    80003c52:	e426                	sd	s1,8(sp)
    80003c54:	e04a                	sd	s2,0(sp)
    80003c56:	1000                	addi	s0,sp,32
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    80003c58:	c905                	beqz	a0,80003c88 <iunlock+0x3c>
    80003c5a:	84aa                	mv	s1,a0
    80003c5c:	01050913          	addi	s2,a0,16
    80003c60:	854a                	mv	a0,s2
    80003c62:	00001097          	auipc	ra,0x1
    80003c66:	c8c080e7          	jalr	-884(ra) # 800048ee <holdingsleep>
    80003c6a:	cd19                	beqz	a0,80003c88 <iunlock+0x3c>
    80003c6c:	449c                	lw	a5,8(s1)
    80003c6e:	00f05d63          	blez	a5,80003c88 <iunlock+0x3c>
  releasesleep(&ip->lock);
    80003c72:	854a                	mv	a0,s2
    80003c74:	00001097          	auipc	ra,0x1
    80003c78:	c36080e7          	jalr	-970(ra) # 800048aa <releasesleep>
}
    80003c7c:	60e2                	ld	ra,24(sp)
    80003c7e:	6442                	ld	s0,16(sp)
    80003c80:	64a2                	ld	s1,8(sp)
    80003c82:	6902                	ld	s2,0(sp)
    80003c84:	6105                	addi	sp,sp,32
    80003c86:	8082                	ret
    panic("iunlock");
    80003c88:	00005517          	auipc	a0,0x5
    80003c8c:	a2850513          	addi	a0,a0,-1496 # 800086b0 <syscalls+0x1b0>
    80003c90:	ffffd097          	auipc	ra,0xffffd
    80003c94:	8ae080e7          	jalr	-1874(ra) # 8000053e <panic>

0000000080003c98 <itrunc>:

// Truncate inode (discard contents).
// Caller must hold ip->lock.
void
itrunc(struct inode *ip)
{
    80003c98:	7179                	addi	sp,sp,-48
    80003c9a:	f406                	sd	ra,40(sp)
    80003c9c:	f022                	sd	s0,32(sp)
    80003c9e:	ec26                	sd	s1,24(sp)
    80003ca0:	e84a                	sd	s2,16(sp)
    80003ca2:	e44e                	sd	s3,8(sp)
    80003ca4:	e052                	sd	s4,0(sp)
    80003ca6:	1800                	addi	s0,sp,48
    80003ca8:	89aa                	mv	s3,a0
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
    80003caa:	05050493          	addi	s1,a0,80
    80003cae:	08050913          	addi	s2,a0,128
    80003cb2:	a021                	j	80003cba <itrunc+0x22>
    80003cb4:	0491                	addi	s1,s1,4
    80003cb6:	01248d63          	beq	s1,s2,80003cd0 <itrunc+0x38>
    if(ip->addrs[i]){
    80003cba:	408c                	lw	a1,0(s1)
    80003cbc:	dde5                	beqz	a1,80003cb4 <itrunc+0x1c>
      bfree(ip->dev, ip->addrs[i]);
    80003cbe:	0009a503          	lw	a0,0(s3)
    80003cc2:	00000097          	auipc	ra,0x0
    80003cc6:	90c080e7          	jalr	-1780(ra) # 800035ce <bfree>
      ip->addrs[i] = 0;
    80003cca:	0004a023          	sw	zero,0(s1)
    80003cce:	b7dd                	j	80003cb4 <itrunc+0x1c>
    }
  }

  if(ip->addrs[NDIRECT]){
    80003cd0:	0809a583          	lw	a1,128(s3)
    80003cd4:	e185                	bnez	a1,80003cf4 <itrunc+0x5c>
    brelse(bp);
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
    80003cd6:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    80003cda:	854e                	mv	a0,s3
    80003cdc:	00000097          	auipc	ra,0x0
    80003ce0:	de4080e7          	jalr	-540(ra) # 80003ac0 <iupdate>
}
    80003ce4:	70a2                	ld	ra,40(sp)
    80003ce6:	7402                	ld	s0,32(sp)
    80003ce8:	64e2                	ld	s1,24(sp)
    80003cea:	6942                	ld	s2,16(sp)
    80003cec:	69a2                	ld	s3,8(sp)
    80003cee:	6a02                	ld	s4,0(sp)
    80003cf0:	6145                	addi	sp,sp,48
    80003cf2:	8082                	ret
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    80003cf4:	0009a503          	lw	a0,0(s3)
    80003cf8:	fffff097          	auipc	ra,0xfffff
    80003cfc:	690080e7          	jalr	1680(ra) # 80003388 <bread>
    80003d00:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    80003d02:	05850493          	addi	s1,a0,88
    80003d06:	45850913          	addi	s2,a0,1112
    80003d0a:	a811                	j	80003d1e <itrunc+0x86>
        bfree(ip->dev, a[j]);
    80003d0c:	0009a503          	lw	a0,0(s3)
    80003d10:	00000097          	auipc	ra,0x0
    80003d14:	8be080e7          	jalr	-1858(ra) # 800035ce <bfree>
    for(j = 0; j < NINDIRECT; j++){
    80003d18:	0491                	addi	s1,s1,4
    80003d1a:	01248563          	beq	s1,s2,80003d24 <itrunc+0x8c>
      if(a[j])
    80003d1e:	408c                	lw	a1,0(s1)
    80003d20:	dde5                	beqz	a1,80003d18 <itrunc+0x80>
    80003d22:	b7ed                	j	80003d0c <itrunc+0x74>
    brelse(bp);
    80003d24:	8552                	mv	a0,s4
    80003d26:	fffff097          	auipc	ra,0xfffff
    80003d2a:	792080e7          	jalr	1938(ra) # 800034b8 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    80003d2e:	0809a583          	lw	a1,128(s3)
    80003d32:	0009a503          	lw	a0,0(s3)
    80003d36:	00000097          	auipc	ra,0x0
    80003d3a:	898080e7          	jalr	-1896(ra) # 800035ce <bfree>
    ip->addrs[NDIRECT] = 0;
    80003d3e:	0809a023          	sw	zero,128(s3)
    80003d42:	bf51                	j	80003cd6 <itrunc+0x3e>

0000000080003d44 <iput>:
{
    80003d44:	1101                	addi	sp,sp,-32
    80003d46:	ec06                	sd	ra,24(sp)
    80003d48:	e822                	sd	s0,16(sp)
    80003d4a:	e426                	sd	s1,8(sp)
    80003d4c:	e04a                	sd	s2,0(sp)
    80003d4e:	1000                	addi	s0,sp,32
    80003d50:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    80003d52:	0001c517          	auipc	a0,0x1c
    80003d56:	29650513          	addi	a0,a0,662 # 8001ffe8 <itable>
    80003d5a:	ffffd097          	auipc	ra,0xffffd
    80003d5e:	e8a080e7          	jalr	-374(ra) # 80000be4 <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003d62:	4498                	lw	a4,8(s1)
    80003d64:	4785                	li	a5,1
    80003d66:	02f70363          	beq	a4,a5,80003d8c <iput+0x48>
  ip->ref--;
    80003d6a:	449c                	lw	a5,8(s1)
    80003d6c:	37fd                	addiw	a5,a5,-1
    80003d6e:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003d70:	0001c517          	auipc	a0,0x1c
    80003d74:	27850513          	addi	a0,a0,632 # 8001ffe8 <itable>
    80003d78:	ffffd097          	auipc	ra,0xffffd
    80003d7c:	f20080e7          	jalr	-224(ra) # 80000c98 <release>
}
    80003d80:	60e2                	ld	ra,24(sp)
    80003d82:	6442                	ld	s0,16(sp)
    80003d84:	64a2                	ld	s1,8(sp)
    80003d86:	6902                	ld	s2,0(sp)
    80003d88:	6105                	addi	sp,sp,32
    80003d8a:	8082                	ret
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003d8c:	40bc                	lw	a5,64(s1)
    80003d8e:	dff1                	beqz	a5,80003d6a <iput+0x26>
    80003d90:	04a49783          	lh	a5,74(s1)
    80003d94:	fbf9                	bnez	a5,80003d6a <iput+0x26>
    acquiresleep(&ip->lock);
    80003d96:	01048913          	addi	s2,s1,16
    80003d9a:	854a                	mv	a0,s2
    80003d9c:	00001097          	auipc	ra,0x1
    80003da0:	ab8080e7          	jalr	-1352(ra) # 80004854 <acquiresleep>
    release(&itable.lock);
    80003da4:	0001c517          	auipc	a0,0x1c
    80003da8:	24450513          	addi	a0,a0,580 # 8001ffe8 <itable>
    80003dac:	ffffd097          	auipc	ra,0xffffd
    80003db0:	eec080e7          	jalr	-276(ra) # 80000c98 <release>
    itrunc(ip);
    80003db4:	8526                	mv	a0,s1
    80003db6:	00000097          	auipc	ra,0x0
    80003dba:	ee2080e7          	jalr	-286(ra) # 80003c98 <itrunc>
    ip->type = 0;
    80003dbe:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    80003dc2:	8526                	mv	a0,s1
    80003dc4:	00000097          	auipc	ra,0x0
    80003dc8:	cfc080e7          	jalr	-772(ra) # 80003ac0 <iupdate>
    ip->valid = 0;
    80003dcc:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    80003dd0:	854a                	mv	a0,s2
    80003dd2:	00001097          	auipc	ra,0x1
    80003dd6:	ad8080e7          	jalr	-1320(ra) # 800048aa <releasesleep>
    acquire(&itable.lock);
    80003dda:	0001c517          	auipc	a0,0x1c
    80003dde:	20e50513          	addi	a0,a0,526 # 8001ffe8 <itable>
    80003de2:	ffffd097          	auipc	ra,0xffffd
    80003de6:	e02080e7          	jalr	-510(ra) # 80000be4 <acquire>
    80003dea:	b741                	j	80003d6a <iput+0x26>

0000000080003dec <iunlockput>:
{
    80003dec:	1101                	addi	sp,sp,-32
    80003dee:	ec06                	sd	ra,24(sp)
    80003df0:	e822                	sd	s0,16(sp)
    80003df2:	e426                	sd	s1,8(sp)
    80003df4:	1000                	addi	s0,sp,32
    80003df6:	84aa                	mv	s1,a0
  iunlock(ip);
    80003df8:	00000097          	auipc	ra,0x0
    80003dfc:	e54080e7          	jalr	-428(ra) # 80003c4c <iunlock>
  iput(ip);
    80003e00:	8526                	mv	a0,s1
    80003e02:	00000097          	auipc	ra,0x0
    80003e06:	f42080e7          	jalr	-190(ra) # 80003d44 <iput>
}
    80003e0a:	60e2                	ld	ra,24(sp)
    80003e0c:	6442                	ld	s0,16(sp)
    80003e0e:	64a2                	ld	s1,8(sp)
    80003e10:	6105                	addi	sp,sp,32
    80003e12:	8082                	ret

0000000080003e14 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
    80003e14:	1141                	addi	sp,sp,-16
    80003e16:	e422                	sd	s0,8(sp)
    80003e18:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    80003e1a:	411c                	lw	a5,0(a0)
    80003e1c:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    80003e1e:	415c                	lw	a5,4(a0)
    80003e20:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    80003e22:	04451783          	lh	a5,68(a0)
    80003e26:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    80003e2a:	04a51783          	lh	a5,74(a0)
    80003e2e:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    80003e32:	04c56783          	lwu	a5,76(a0)
    80003e36:	e99c                	sd	a5,16(a1)
}
    80003e38:	6422                	ld	s0,8(sp)
    80003e3a:	0141                	addi	sp,sp,16
    80003e3c:	8082                	ret

0000000080003e3e <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003e3e:	457c                	lw	a5,76(a0)
    80003e40:	0ed7e963          	bltu	a5,a3,80003f32 <readi+0xf4>
{
    80003e44:	7159                	addi	sp,sp,-112
    80003e46:	f486                	sd	ra,104(sp)
    80003e48:	f0a2                	sd	s0,96(sp)
    80003e4a:	eca6                	sd	s1,88(sp)
    80003e4c:	e8ca                	sd	s2,80(sp)
    80003e4e:	e4ce                	sd	s3,72(sp)
    80003e50:	e0d2                	sd	s4,64(sp)
    80003e52:	fc56                	sd	s5,56(sp)
    80003e54:	f85a                	sd	s6,48(sp)
    80003e56:	f45e                	sd	s7,40(sp)
    80003e58:	f062                	sd	s8,32(sp)
    80003e5a:	ec66                	sd	s9,24(sp)
    80003e5c:	e86a                	sd	s10,16(sp)
    80003e5e:	e46e                	sd	s11,8(sp)
    80003e60:	1880                	addi	s0,sp,112
    80003e62:	8baa                	mv	s7,a0
    80003e64:	8c2e                	mv	s8,a1
    80003e66:	8ab2                	mv	s5,a2
    80003e68:	84b6                	mv	s1,a3
    80003e6a:	8b3a                	mv	s6,a4
  if(off > ip->size || off + n < off)
    80003e6c:	9f35                	addw	a4,a4,a3
    return 0;
    80003e6e:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    80003e70:	0ad76063          	bltu	a4,a3,80003f10 <readi+0xd2>
  if(off + n > ip->size)
    80003e74:	00e7f463          	bgeu	a5,a4,80003e7c <readi+0x3e>
    n = ip->size - off;
    80003e78:	40d78b3b          	subw	s6,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003e7c:	0a0b0963          	beqz	s6,80003f2e <readi+0xf0>
    80003e80:	4981                	li	s3,0
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    80003e82:	40000d13          	li	s10,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    80003e86:	5cfd                	li	s9,-1
    80003e88:	a82d                	j	80003ec2 <readi+0x84>
    80003e8a:	020a1d93          	slli	s11,s4,0x20
    80003e8e:	020ddd93          	srli	s11,s11,0x20
    80003e92:	05890613          	addi	a2,s2,88
    80003e96:	86ee                	mv	a3,s11
    80003e98:	963a                	add	a2,a2,a4
    80003e9a:	85d6                	mv	a1,s5
    80003e9c:	8562                	mv	a0,s8
    80003e9e:	fffff097          	auipc	ra,0xfffff
    80003ea2:	988080e7          	jalr	-1656(ra) # 80002826 <either_copyout>
    80003ea6:	05950d63          	beq	a0,s9,80003f00 <readi+0xc2>
      brelse(bp);
      tot = -1;
      break;
    }
    brelse(bp);
    80003eaa:	854a                	mv	a0,s2
    80003eac:	fffff097          	auipc	ra,0xfffff
    80003eb0:	60c080e7          	jalr	1548(ra) # 800034b8 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003eb4:	013a09bb          	addw	s3,s4,s3
    80003eb8:	009a04bb          	addw	s1,s4,s1
    80003ebc:	9aee                	add	s5,s5,s11
    80003ebe:	0569f763          	bgeu	s3,s6,80003f0c <readi+0xce>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    80003ec2:	000ba903          	lw	s2,0(s7)
    80003ec6:	00a4d59b          	srliw	a1,s1,0xa
    80003eca:	855e                	mv	a0,s7
    80003ecc:	00000097          	auipc	ra,0x0
    80003ed0:	8b0080e7          	jalr	-1872(ra) # 8000377c <bmap>
    80003ed4:	0005059b          	sext.w	a1,a0
    80003ed8:	854a                	mv	a0,s2
    80003eda:	fffff097          	auipc	ra,0xfffff
    80003ede:	4ae080e7          	jalr	1198(ra) # 80003388 <bread>
    80003ee2:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003ee4:	3ff4f713          	andi	a4,s1,1023
    80003ee8:	40ed07bb          	subw	a5,s10,a4
    80003eec:	413b06bb          	subw	a3,s6,s3
    80003ef0:	8a3e                	mv	s4,a5
    80003ef2:	2781                	sext.w	a5,a5
    80003ef4:	0006861b          	sext.w	a2,a3
    80003ef8:	f8f679e3          	bgeu	a2,a5,80003e8a <readi+0x4c>
    80003efc:	8a36                	mv	s4,a3
    80003efe:	b771                	j	80003e8a <readi+0x4c>
      brelse(bp);
    80003f00:	854a                	mv	a0,s2
    80003f02:	fffff097          	auipc	ra,0xfffff
    80003f06:	5b6080e7          	jalr	1462(ra) # 800034b8 <brelse>
      tot = -1;
    80003f0a:	59fd                	li	s3,-1
  }
  return tot;
    80003f0c:	0009851b          	sext.w	a0,s3
}
    80003f10:	70a6                	ld	ra,104(sp)
    80003f12:	7406                	ld	s0,96(sp)
    80003f14:	64e6                	ld	s1,88(sp)
    80003f16:	6946                	ld	s2,80(sp)
    80003f18:	69a6                	ld	s3,72(sp)
    80003f1a:	6a06                	ld	s4,64(sp)
    80003f1c:	7ae2                	ld	s5,56(sp)
    80003f1e:	7b42                	ld	s6,48(sp)
    80003f20:	7ba2                	ld	s7,40(sp)
    80003f22:	7c02                	ld	s8,32(sp)
    80003f24:	6ce2                	ld	s9,24(sp)
    80003f26:	6d42                	ld	s10,16(sp)
    80003f28:	6da2                	ld	s11,8(sp)
    80003f2a:	6165                	addi	sp,sp,112
    80003f2c:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003f2e:	89da                	mv	s3,s6
    80003f30:	bff1                	j	80003f0c <readi+0xce>
    return 0;
    80003f32:	4501                	li	a0,0
}
    80003f34:	8082                	ret

0000000080003f36 <writei>:
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003f36:	457c                	lw	a5,76(a0)
    80003f38:	10d7e863          	bltu	a5,a3,80004048 <writei+0x112>
{
    80003f3c:	7159                	addi	sp,sp,-112
    80003f3e:	f486                	sd	ra,104(sp)
    80003f40:	f0a2                	sd	s0,96(sp)
    80003f42:	eca6                	sd	s1,88(sp)
    80003f44:	e8ca                	sd	s2,80(sp)
    80003f46:	e4ce                	sd	s3,72(sp)
    80003f48:	e0d2                	sd	s4,64(sp)
    80003f4a:	fc56                	sd	s5,56(sp)
    80003f4c:	f85a                	sd	s6,48(sp)
    80003f4e:	f45e                	sd	s7,40(sp)
    80003f50:	f062                	sd	s8,32(sp)
    80003f52:	ec66                	sd	s9,24(sp)
    80003f54:	e86a                	sd	s10,16(sp)
    80003f56:	e46e                	sd	s11,8(sp)
    80003f58:	1880                	addi	s0,sp,112
    80003f5a:	8b2a                	mv	s6,a0
    80003f5c:	8c2e                	mv	s8,a1
    80003f5e:	8ab2                	mv	s5,a2
    80003f60:	8936                	mv	s2,a3
    80003f62:	8bba                	mv	s7,a4
  if(off > ip->size || off + n < off)
    80003f64:	00e687bb          	addw	a5,a3,a4
    80003f68:	0ed7e263          	bltu	a5,a3,8000404c <writei+0x116>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    80003f6c:	00043737          	lui	a4,0x43
    80003f70:	0ef76063          	bltu	a4,a5,80004050 <writei+0x11a>
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003f74:	0c0b8863          	beqz	s7,80004044 <writei+0x10e>
    80003f78:	4a01                	li	s4,0
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    80003f7a:	40000d13          	li	s10,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    80003f7e:	5cfd                	li	s9,-1
    80003f80:	a091                	j	80003fc4 <writei+0x8e>
    80003f82:	02099d93          	slli	s11,s3,0x20
    80003f86:	020ddd93          	srli	s11,s11,0x20
    80003f8a:	05848513          	addi	a0,s1,88
    80003f8e:	86ee                	mv	a3,s11
    80003f90:	8656                	mv	a2,s5
    80003f92:	85e2                	mv	a1,s8
    80003f94:	953a                	add	a0,a0,a4
    80003f96:	fffff097          	auipc	ra,0xfffff
    80003f9a:	8e6080e7          	jalr	-1818(ra) # 8000287c <either_copyin>
    80003f9e:	07950263          	beq	a0,s9,80004002 <writei+0xcc>
      brelse(bp);
      break;
    }
    log_write(bp);
    80003fa2:	8526                	mv	a0,s1
    80003fa4:	00000097          	auipc	ra,0x0
    80003fa8:	790080e7          	jalr	1936(ra) # 80004734 <log_write>
    brelse(bp);
    80003fac:	8526                	mv	a0,s1
    80003fae:	fffff097          	auipc	ra,0xfffff
    80003fb2:	50a080e7          	jalr	1290(ra) # 800034b8 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003fb6:	01498a3b          	addw	s4,s3,s4
    80003fba:	0129893b          	addw	s2,s3,s2
    80003fbe:	9aee                	add	s5,s5,s11
    80003fc0:	057a7663          	bgeu	s4,s7,8000400c <writei+0xd6>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    80003fc4:	000b2483          	lw	s1,0(s6)
    80003fc8:	00a9559b          	srliw	a1,s2,0xa
    80003fcc:	855a                	mv	a0,s6
    80003fce:	fffff097          	auipc	ra,0xfffff
    80003fd2:	7ae080e7          	jalr	1966(ra) # 8000377c <bmap>
    80003fd6:	0005059b          	sext.w	a1,a0
    80003fda:	8526                	mv	a0,s1
    80003fdc:	fffff097          	auipc	ra,0xfffff
    80003fe0:	3ac080e7          	jalr	940(ra) # 80003388 <bread>
    80003fe4:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003fe6:	3ff97713          	andi	a4,s2,1023
    80003fea:	40ed07bb          	subw	a5,s10,a4
    80003fee:	414b86bb          	subw	a3,s7,s4
    80003ff2:	89be                	mv	s3,a5
    80003ff4:	2781                	sext.w	a5,a5
    80003ff6:	0006861b          	sext.w	a2,a3
    80003ffa:	f8f674e3          	bgeu	a2,a5,80003f82 <writei+0x4c>
    80003ffe:	89b6                	mv	s3,a3
    80004000:	b749                	j	80003f82 <writei+0x4c>
      brelse(bp);
    80004002:	8526                	mv	a0,s1
    80004004:	fffff097          	auipc	ra,0xfffff
    80004008:	4b4080e7          	jalr	1204(ra) # 800034b8 <brelse>
  }

  if(off > ip->size)
    8000400c:	04cb2783          	lw	a5,76(s6)
    80004010:	0127f463          	bgeu	a5,s2,80004018 <writei+0xe2>
    ip->size = off;
    80004014:	052b2623          	sw	s2,76(s6)

  // write the i-node back to disk even if the size didn't change
  // because the loop above might have called bmap() and added a new
  // block to ip->addrs[].
  iupdate(ip);
    80004018:	855a                	mv	a0,s6
    8000401a:	00000097          	auipc	ra,0x0
    8000401e:	aa6080e7          	jalr	-1370(ra) # 80003ac0 <iupdate>

  return tot;
    80004022:	000a051b          	sext.w	a0,s4
}
    80004026:	70a6                	ld	ra,104(sp)
    80004028:	7406                	ld	s0,96(sp)
    8000402a:	64e6                	ld	s1,88(sp)
    8000402c:	6946                	ld	s2,80(sp)
    8000402e:	69a6                	ld	s3,72(sp)
    80004030:	6a06                	ld	s4,64(sp)
    80004032:	7ae2                	ld	s5,56(sp)
    80004034:	7b42                	ld	s6,48(sp)
    80004036:	7ba2                	ld	s7,40(sp)
    80004038:	7c02                	ld	s8,32(sp)
    8000403a:	6ce2                	ld	s9,24(sp)
    8000403c:	6d42                	ld	s10,16(sp)
    8000403e:	6da2                	ld	s11,8(sp)
    80004040:	6165                	addi	sp,sp,112
    80004042:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80004044:	8a5e                	mv	s4,s7
    80004046:	bfc9                	j	80004018 <writei+0xe2>
    return -1;
    80004048:	557d                	li	a0,-1
}
    8000404a:	8082                	ret
    return -1;
    8000404c:	557d                	li	a0,-1
    8000404e:	bfe1                	j	80004026 <writei+0xf0>
    return -1;
    80004050:	557d                	li	a0,-1
    80004052:	bfd1                	j	80004026 <writei+0xf0>

0000000080004054 <namecmp>:

// Directories

int
namecmp(const char *s, const char *t)
{
    80004054:	1141                	addi	sp,sp,-16
    80004056:	e406                	sd	ra,8(sp)
    80004058:	e022                	sd	s0,0(sp)
    8000405a:	0800                	addi	s0,sp,16
  return strncmp(s, t, DIRSIZ);
    8000405c:	4639                	li	a2,14
    8000405e:	ffffd097          	auipc	ra,0xffffd
    80004062:	d5a080e7          	jalr	-678(ra) # 80000db8 <strncmp>
}
    80004066:	60a2                	ld	ra,8(sp)
    80004068:	6402                	ld	s0,0(sp)
    8000406a:	0141                	addi	sp,sp,16
    8000406c:	8082                	ret

000000008000406e <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
    8000406e:	7139                	addi	sp,sp,-64
    80004070:	fc06                	sd	ra,56(sp)
    80004072:	f822                	sd	s0,48(sp)
    80004074:	f426                	sd	s1,40(sp)
    80004076:	f04a                	sd	s2,32(sp)
    80004078:	ec4e                	sd	s3,24(sp)
    8000407a:	e852                	sd	s4,16(sp)
    8000407c:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    8000407e:	04451703          	lh	a4,68(a0)
    80004082:	4785                	li	a5,1
    80004084:	00f71a63          	bne	a4,a5,80004098 <dirlookup+0x2a>
    80004088:	892a                	mv	s2,a0
    8000408a:	89ae                	mv	s3,a1
    8000408c:	8a32                	mv	s4,a2
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
    8000408e:	457c                	lw	a5,76(a0)
    80004090:	4481                	li	s1,0
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
    80004092:	4501                	li	a0,0
  for(off = 0; off < dp->size; off += sizeof(de)){
    80004094:	e79d                	bnez	a5,800040c2 <dirlookup+0x54>
    80004096:	a8a5                	j	8000410e <dirlookup+0xa0>
    panic("dirlookup not DIR");
    80004098:	00004517          	auipc	a0,0x4
    8000409c:	62050513          	addi	a0,a0,1568 # 800086b8 <syscalls+0x1b8>
    800040a0:	ffffc097          	auipc	ra,0xffffc
    800040a4:	49e080e7          	jalr	1182(ra) # 8000053e <panic>
      panic("dirlookup read");
    800040a8:	00004517          	auipc	a0,0x4
    800040ac:	62850513          	addi	a0,a0,1576 # 800086d0 <syscalls+0x1d0>
    800040b0:	ffffc097          	auipc	ra,0xffffc
    800040b4:	48e080e7          	jalr	1166(ra) # 8000053e <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800040b8:	24c1                	addiw	s1,s1,16
    800040ba:	04c92783          	lw	a5,76(s2)
    800040be:	04f4f763          	bgeu	s1,a5,8000410c <dirlookup+0x9e>
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800040c2:	4741                	li	a4,16
    800040c4:	86a6                	mv	a3,s1
    800040c6:	fc040613          	addi	a2,s0,-64
    800040ca:	4581                	li	a1,0
    800040cc:	854a                	mv	a0,s2
    800040ce:	00000097          	auipc	ra,0x0
    800040d2:	d70080e7          	jalr	-656(ra) # 80003e3e <readi>
    800040d6:	47c1                	li	a5,16
    800040d8:	fcf518e3          	bne	a0,a5,800040a8 <dirlookup+0x3a>
    if(de.inum == 0)
    800040dc:	fc045783          	lhu	a5,-64(s0)
    800040e0:	dfe1                	beqz	a5,800040b8 <dirlookup+0x4a>
    if(namecmp(name, de.name) == 0){
    800040e2:	fc240593          	addi	a1,s0,-62
    800040e6:	854e                	mv	a0,s3
    800040e8:	00000097          	auipc	ra,0x0
    800040ec:	f6c080e7          	jalr	-148(ra) # 80004054 <namecmp>
    800040f0:	f561                	bnez	a0,800040b8 <dirlookup+0x4a>
      if(poff)
    800040f2:	000a0463          	beqz	s4,800040fa <dirlookup+0x8c>
        *poff = off;
    800040f6:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    800040fa:	fc045583          	lhu	a1,-64(s0)
    800040fe:	00092503          	lw	a0,0(s2)
    80004102:	fffff097          	auipc	ra,0xfffff
    80004106:	754080e7          	jalr	1876(ra) # 80003856 <iget>
    8000410a:	a011                	j	8000410e <dirlookup+0xa0>
  return 0;
    8000410c:	4501                	li	a0,0
}
    8000410e:	70e2                	ld	ra,56(sp)
    80004110:	7442                	ld	s0,48(sp)
    80004112:	74a2                	ld	s1,40(sp)
    80004114:	7902                	ld	s2,32(sp)
    80004116:	69e2                	ld	s3,24(sp)
    80004118:	6a42                	ld	s4,16(sp)
    8000411a:	6121                	addi	sp,sp,64
    8000411c:	8082                	ret

000000008000411e <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
    8000411e:	711d                	addi	sp,sp,-96
    80004120:	ec86                	sd	ra,88(sp)
    80004122:	e8a2                	sd	s0,80(sp)
    80004124:	e4a6                	sd	s1,72(sp)
    80004126:	e0ca                	sd	s2,64(sp)
    80004128:	fc4e                	sd	s3,56(sp)
    8000412a:	f852                	sd	s4,48(sp)
    8000412c:	f456                	sd	s5,40(sp)
    8000412e:	f05a                	sd	s6,32(sp)
    80004130:	ec5e                	sd	s7,24(sp)
    80004132:	e862                	sd	s8,16(sp)
    80004134:	e466                	sd	s9,8(sp)
    80004136:	1080                	addi	s0,sp,96
    80004138:	84aa                	mv	s1,a0
    8000413a:	8b2e                	mv	s6,a1
    8000413c:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    8000413e:	00054703          	lbu	a4,0(a0)
    80004142:	02f00793          	li	a5,47
    80004146:	02f70363          	beq	a4,a5,8000416c <namex+0x4e>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    8000414a:	ffffe097          	auipc	ra,0xffffe
    8000414e:	894080e7          	jalr	-1900(ra) # 800019de <myproc>
    80004152:	17053503          	ld	a0,368(a0)
    80004156:	00000097          	auipc	ra,0x0
    8000415a:	9f6080e7          	jalr	-1546(ra) # 80003b4c <idup>
    8000415e:	89aa                	mv	s3,a0
  while(*path == '/')
    80004160:	02f00913          	li	s2,47
  len = path - s;
    80004164:	4b81                	li	s7,0
  if(len >= DIRSIZ)
    80004166:	4cb5                	li	s9,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    80004168:	4c05                	li	s8,1
    8000416a:	a865                	j	80004222 <namex+0x104>
    ip = iget(ROOTDEV, ROOTINO);
    8000416c:	4585                	li	a1,1
    8000416e:	4505                	li	a0,1
    80004170:	fffff097          	auipc	ra,0xfffff
    80004174:	6e6080e7          	jalr	1766(ra) # 80003856 <iget>
    80004178:	89aa                	mv	s3,a0
    8000417a:	b7dd                	j	80004160 <namex+0x42>
      iunlockput(ip);
    8000417c:	854e                	mv	a0,s3
    8000417e:	00000097          	auipc	ra,0x0
    80004182:	c6e080e7          	jalr	-914(ra) # 80003dec <iunlockput>
      return 0;
    80004186:	4981                	li	s3,0
  if(nameiparent){
    iput(ip);
    return 0;
  }
  return ip;
}
    80004188:	854e                	mv	a0,s3
    8000418a:	60e6                	ld	ra,88(sp)
    8000418c:	6446                	ld	s0,80(sp)
    8000418e:	64a6                	ld	s1,72(sp)
    80004190:	6906                	ld	s2,64(sp)
    80004192:	79e2                	ld	s3,56(sp)
    80004194:	7a42                	ld	s4,48(sp)
    80004196:	7aa2                	ld	s5,40(sp)
    80004198:	7b02                	ld	s6,32(sp)
    8000419a:	6be2                	ld	s7,24(sp)
    8000419c:	6c42                	ld	s8,16(sp)
    8000419e:	6ca2                	ld	s9,8(sp)
    800041a0:	6125                	addi	sp,sp,96
    800041a2:	8082                	ret
      iunlock(ip);
    800041a4:	854e                	mv	a0,s3
    800041a6:	00000097          	auipc	ra,0x0
    800041aa:	aa6080e7          	jalr	-1370(ra) # 80003c4c <iunlock>
      return ip;
    800041ae:	bfe9                	j	80004188 <namex+0x6a>
      iunlockput(ip);
    800041b0:	854e                	mv	a0,s3
    800041b2:	00000097          	auipc	ra,0x0
    800041b6:	c3a080e7          	jalr	-966(ra) # 80003dec <iunlockput>
      return 0;
    800041ba:	89d2                	mv	s3,s4
    800041bc:	b7f1                	j	80004188 <namex+0x6a>
  len = path - s;
    800041be:	40b48633          	sub	a2,s1,a1
    800041c2:	00060a1b          	sext.w	s4,a2
  if(len >= DIRSIZ)
    800041c6:	094cd463          	bge	s9,s4,8000424e <namex+0x130>
    memmove(name, s, DIRSIZ);
    800041ca:	4639                	li	a2,14
    800041cc:	8556                	mv	a0,s5
    800041ce:	ffffd097          	auipc	ra,0xffffd
    800041d2:	b72080e7          	jalr	-1166(ra) # 80000d40 <memmove>
  while(*path == '/')
    800041d6:	0004c783          	lbu	a5,0(s1)
    800041da:	01279763          	bne	a5,s2,800041e8 <namex+0xca>
    path++;
    800041de:	0485                	addi	s1,s1,1
  while(*path == '/')
    800041e0:	0004c783          	lbu	a5,0(s1)
    800041e4:	ff278de3          	beq	a5,s2,800041de <namex+0xc0>
    ilock(ip);
    800041e8:	854e                	mv	a0,s3
    800041ea:	00000097          	auipc	ra,0x0
    800041ee:	9a0080e7          	jalr	-1632(ra) # 80003b8a <ilock>
    if(ip->type != T_DIR){
    800041f2:	04499783          	lh	a5,68(s3)
    800041f6:	f98793e3          	bne	a5,s8,8000417c <namex+0x5e>
    if(nameiparent && *path == '\0'){
    800041fa:	000b0563          	beqz	s6,80004204 <namex+0xe6>
    800041fe:	0004c783          	lbu	a5,0(s1)
    80004202:	d3cd                	beqz	a5,800041a4 <namex+0x86>
    if((next = dirlookup(ip, name, 0)) == 0){
    80004204:	865e                	mv	a2,s7
    80004206:	85d6                	mv	a1,s5
    80004208:	854e                	mv	a0,s3
    8000420a:	00000097          	auipc	ra,0x0
    8000420e:	e64080e7          	jalr	-412(ra) # 8000406e <dirlookup>
    80004212:	8a2a                	mv	s4,a0
    80004214:	dd51                	beqz	a0,800041b0 <namex+0x92>
    iunlockput(ip);
    80004216:	854e                	mv	a0,s3
    80004218:	00000097          	auipc	ra,0x0
    8000421c:	bd4080e7          	jalr	-1068(ra) # 80003dec <iunlockput>
    ip = next;
    80004220:	89d2                	mv	s3,s4
  while(*path == '/')
    80004222:	0004c783          	lbu	a5,0(s1)
    80004226:	05279763          	bne	a5,s2,80004274 <namex+0x156>
    path++;
    8000422a:	0485                	addi	s1,s1,1
  while(*path == '/')
    8000422c:	0004c783          	lbu	a5,0(s1)
    80004230:	ff278de3          	beq	a5,s2,8000422a <namex+0x10c>
  if(*path == 0)
    80004234:	c79d                	beqz	a5,80004262 <namex+0x144>
    path++;
    80004236:	85a6                	mv	a1,s1
  len = path - s;
    80004238:	8a5e                	mv	s4,s7
    8000423a:	865e                	mv	a2,s7
  while(*path != '/' && *path != 0)
    8000423c:	01278963          	beq	a5,s2,8000424e <namex+0x130>
    80004240:	dfbd                	beqz	a5,800041be <namex+0xa0>
    path++;
    80004242:	0485                	addi	s1,s1,1
  while(*path != '/' && *path != 0)
    80004244:	0004c783          	lbu	a5,0(s1)
    80004248:	ff279ce3          	bne	a5,s2,80004240 <namex+0x122>
    8000424c:	bf8d                	j	800041be <namex+0xa0>
    memmove(name, s, len);
    8000424e:	2601                	sext.w	a2,a2
    80004250:	8556                	mv	a0,s5
    80004252:	ffffd097          	auipc	ra,0xffffd
    80004256:	aee080e7          	jalr	-1298(ra) # 80000d40 <memmove>
    name[len] = 0;
    8000425a:	9a56                	add	s4,s4,s5
    8000425c:	000a0023          	sb	zero,0(s4)
    80004260:	bf9d                	j	800041d6 <namex+0xb8>
  if(nameiparent){
    80004262:	f20b03e3          	beqz	s6,80004188 <namex+0x6a>
    iput(ip);
    80004266:	854e                	mv	a0,s3
    80004268:	00000097          	auipc	ra,0x0
    8000426c:	adc080e7          	jalr	-1316(ra) # 80003d44 <iput>
    return 0;
    80004270:	4981                	li	s3,0
    80004272:	bf19                	j	80004188 <namex+0x6a>
  if(*path == 0)
    80004274:	d7fd                	beqz	a5,80004262 <namex+0x144>
  while(*path != '/' && *path != 0)
    80004276:	0004c783          	lbu	a5,0(s1)
    8000427a:	85a6                	mv	a1,s1
    8000427c:	b7d1                	j	80004240 <namex+0x122>

000000008000427e <dirlink>:
{
    8000427e:	7139                	addi	sp,sp,-64
    80004280:	fc06                	sd	ra,56(sp)
    80004282:	f822                	sd	s0,48(sp)
    80004284:	f426                	sd	s1,40(sp)
    80004286:	f04a                	sd	s2,32(sp)
    80004288:	ec4e                	sd	s3,24(sp)
    8000428a:	e852                	sd	s4,16(sp)
    8000428c:	0080                	addi	s0,sp,64
    8000428e:	892a                	mv	s2,a0
    80004290:	8a2e                	mv	s4,a1
    80004292:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    80004294:	4601                	li	a2,0
    80004296:	00000097          	auipc	ra,0x0
    8000429a:	dd8080e7          	jalr	-552(ra) # 8000406e <dirlookup>
    8000429e:	e93d                	bnez	a0,80004314 <dirlink+0x96>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800042a0:	04c92483          	lw	s1,76(s2)
    800042a4:	c49d                	beqz	s1,800042d2 <dirlink+0x54>
    800042a6:	4481                	li	s1,0
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800042a8:	4741                	li	a4,16
    800042aa:	86a6                	mv	a3,s1
    800042ac:	fc040613          	addi	a2,s0,-64
    800042b0:	4581                	li	a1,0
    800042b2:	854a                	mv	a0,s2
    800042b4:	00000097          	auipc	ra,0x0
    800042b8:	b8a080e7          	jalr	-1142(ra) # 80003e3e <readi>
    800042bc:	47c1                	li	a5,16
    800042be:	06f51163          	bne	a0,a5,80004320 <dirlink+0xa2>
    if(de.inum == 0)
    800042c2:	fc045783          	lhu	a5,-64(s0)
    800042c6:	c791                	beqz	a5,800042d2 <dirlink+0x54>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800042c8:	24c1                	addiw	s1,s1,16
    800042ca:	04c92783          	lw	a5,76(s2)
    800042ce:	fcf4ede3          	bltu	s1,a5,800042a8 <dirlink+0x2a>
  strncpy(de.name, name, DIRSIZ);
    800042d2:	4639                	li	a2,14
    800042d4:	85d2                	mv	a1,s4
    800042d6:	fc240513          	addi	a0,s0,-62
    800042da:	ffffd097          	auipc	ra,0xffffd
    800042de:	b1a080e7          	jalr	-1254(ra) # 80000df4 <strncpy>
  de.inum = inum;
    800042e2:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800042e6:	4741                	li	a4,16
    800042e8:	86a6                	mv	a3,s1
    800042ea:	fc040613          	addi	a2,s0,-64
    800042ee:	4581                	li	a1,0
    800042f0:	854a                	mv	a0,s2
    800042f2:	00000097          	auipc	ra,0x0
    800042f6:	c44080e7          	jalr	-956(ra) # 80003f36 <writei>
    800042fa:	872a                	mv	a4,a0
    800042fc:	47c1                	li	a5,16
  return 0;
    800042fe:	4501                	li	a0,0
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004300:	02f71863          	bne	a4,a5,80004330 <dirlink+0xb2>
}
    80004304:	70e2                	ld	ra,56(sp)
    80004306:	7442                	ld	s0,48(sp)
    80004308:	74a2                	ld	s1,40(sp)
    8000430a:	7902                	ld	s2,32(sp)
    8000430c:	69e2                	ld	s3,24(sp)
    8000430e:	6a42                	ld	s4,16(sp)
    80004310:	6121                	addi	sp,sp,64
    80004312:	8082                	ret
    iput(ip);
    80004314:	00000097          	auipc	ra,0x0
    80004318:	a30080e7          	jalr	-1488(ra) # 80003d44 <iput>
    return -1;
    8000431c:	557d                	li	a0,-1
    8000431e:	b7dd                	j	80004304 <dirlink+0x86>
      panic("dirlink read");
    80004320:	00004517          	auipc	a0,0x4
    80004324:	3c050513          	addi	a0,a0,960 # 800086e0 <syscalls+0x1e0>
    80004328:	ffffc097          	auipc	ra,0xffffc
    8000432c:	216080e7          	jalr	534(ra) # 8000053e <panic>
    panic("dirlink");
    80004330:	00004517          	auipc	a0,0x4
    80004334:	4c050513          	addi	a0,a0,1216 # 800087f0 <syscalls+0x2f0>
    80004338:	ffffc097          	auipc	ra,0xffffc
    8000433c:	206080e7          	jalr	518(ra) # 8000053e <panic>

0000000080004340 <namei>:

struct inode*
namei(char *path)
{
    80004340:	1101                	addi	sp,sp,-32
    80004342:	ec06                	sd	ra,24(sp)
    80004344:	e822                	sd	s0,16(sp)
    80004346:	1000                	addi	s0,sp,32
  char name[DIRSIZ];
  return namex(path, 0, name);
    80004348:	fe040613          	addi	a2,s0,-32
    8000434c:	4581                	li	a1,0
    8000434e:	00000097          	auipc	ra,0x0
    80004352:	dd0080e7          	jalr	-560(ra) # 8000411e <namex>
}
    80004356:	60e2                	ld	ra,24(sp)
    80004358:	6442                	ld	s0,16(sp)
    8000435a:	6105                	addi	sp,sp,32
    8000435c:	8082                	ret

000000008000435e <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
    8000435e:	1141                	addi	sp,sp,-16
    80004360:	e406                	sd	ra,8(sp)
    80004362:	e022                	sd	s0,0(sp)
    80004364:	0800                	addi	s0,sp,16
    80004366:	862e                	mv	a2,a1
  return namex(path, 1, name);
    80004368:	4585                	li	a1,1
    8000436a:	00000097          	auipc	ra,0x0
    8000436e:	db4080e7          	jalr	-588(ra) # 8000411e <namex>
}
    80004372:	60a2                	ld	ra,8(sp)
    80004374:	6402                	ld	s0,0(sp)
    80004376:	0141                	addi	sp,sp,16
    80004378:	8082                	ret

000000008000437a <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
    8000437a:	1101                	addi	sp,sp,-32
    8000437c:	ec06                	sd	ra,24(sp)
    8000437e:	e822                	sd	s0,16(sp)
    80004380:	e426                	sd	s1,8(sp)
    80004382:	e04a                	sd	s2,0(sp)
    80004384:	1000                	addi	s0,sp,32
  struct buf *buf = bread(log.dev, log.start);
    80004386:	0001d917          	auipc	s2,0x1d
    8000438a:	70a90913          	addi	s2,s2,1802 # 80021a90 <log>
    8000438e:	01892583          	lw	a1,24(s2)
    80004392:	02892503          	lw	a0,40(s2)
    80004396:	fffff097          	auipc	ra,0xfffff
    8000439a:	ff2080e7          	jalr	-14(ra) # 80003388 <bread>
    8000439e:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    800043a0:	02c92683          	lw	a3,44(s2)
    800043a4:	cd34                	sw	a3,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    800043a6:	02d05763          	blez	a3,800043d4 <write_head+0x5a>
    800043aa:	0001d797          	auipc	a5,0x1d
    800043ae:	71678793          	addi	a5,a5,1814 # 80021ac0 <log+0x30>
    800043b2:	05c50713          	addi	a4,a0,92
    800043b6:	36fd                	addiw	a3,a3,-1
    800043b8:	1682                	slli	a3,a3,0x20
    800043ba:	9281                	srli	a3,a3,0x20
    800043bc:	068a                	slli	a3,a3,0x2
    800043be:	0001d617          	auipc	a2,0x1d
    800043c2:	70660613          	addi	a2,a2,1798 # 80021ac4 <log+0x34>
    800043c6:	96b2                	add	a3,a3,a2
    hb->block[i] = log.lh.block[i];
    800043c8:	4390                	lw	a2,0(a5)
    800043ca:	c310                	sw	a2,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    800043cc:	0791                	addi	a5,a5,4
    800043ce:	0711                	addi	a4,a4,4
    800043d0:	fed79ce3          	bne	a5,a3,800043c8 <write_head+0x4e>
  }
  bwrite(buf);
    800043d4:	8526                	mv	a0,s1
    800043d6:	fffff097          	auipc	ra,0xfffff
    800043da:	0a4080e7          	jalr	164(ra) # 8000347a <bwrite>
  brelse(buf);
    800043de:	8526                	mv	a0,s1
    800043e0:	fffff097          	auipc	ra,0xfffff
    800043e4:	0d8080e7          	jalr	216(ra) # 800034b8 <brelse>
}
    800043e8:	60e2                	ld	ra,24(sp)
    800043ea:	6442                	ld	s0,16(sp)
    800043ec:	64a2                	ld	s1,8(sp)
    800043ee:	6902                	ld	s2,0(sp)
    800043f0:	6105                	addi	sp,sp,32
    800043f2:	8082                	ret

00000000800043f4 <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    800043f4:	0001d797          	auipc	a5,0x1d
    800043f8:	6c87a783          	lw	a5,1736(a5) # 80021abc <log+0x2c>
    800043fc:	0af05d63          	blez	a5,800044b6 <install_trans+0xc2>
{
    80004400:	7139                	addi	sp,sp,-64
    80004402:	fc06                	sd	ra,56(sp)
    80004404:	f822                	sd	s0,48(sp)
    80004406:	f426                	sd	s1,40(sp)
    80004408:	f04a                	sd	s2,32(sp)
    8000440a:	ec4e                	sd	s3,24(sp)
    8000440c:	e852                	sd	s4,16(sp)
    8000440e:	e456                	sd	s5,8(sp)
    80004410:	e05a                	sd	s6,0(sp)
    80004412:	0080                	addi	s0,sp,64
    80004414:	8b2a                	mv	s6,a0
    80004416:	0001da97          	auipc	s5,0x1d
    8000441a:	6aaa8a93          	addi	s5,s5,1706 # 80021ac0 <log+0x30>
  for (tail = 0; tail < log.lh.n; tail++) {
    8000441e:	4a01                	li	s4,0
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80004420:	0001d997          	auipc	s3,0x1d
    80004424:	67098993          	addi	s3,s3,1648 # 80021a90 <log>
    80004428:	a035                	j	80004454 <install_trans+0x60>
      bunpin(dbuf);
    8000442a:	8526                	mv	a0,s1
    8000442c:	fffff097          	auipc	ra,0xfffff
    80004430:	166080e7          	jalr	358(ra) # 80003592 <bunpin>
    brelse(lbuf);
    80004434:	854a                	mv	a0,s2
    80004436:	fffff097          	auipc	ra,0xfffff
    8000443a:	082080e7          	jalr	130(ra) # 800034b8 <brelse>
    brelse(dbuf);
    8000443e:	8526                	mv	a0,s1
    80004440:	fffff097          	auipc	ra,0xfffff
    80004444:	078080e7          	jalr	120(ra) # 800034b8 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80004448:	2a05                	addiw	s4,s4,1
    8000444a:	0a91                	addi	s5,s5,4
    8000444c:	02c9a783          	lw	a5,44(s3)
    80004450:	04fa5963          	bge	s4,a5,800044a2 <install_trans+0xae>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80004454:	0189a583          	lw	a1,24(s3)
    80004458:	014585bb          	addw	a1,a1,s4
    8000445c:	2585                	addiw	a1,a1,1
    8000445e:	0289a503          	lw	a0,40(s3)
    80004462:	fffff097          	auipc	ra,0xfffff
    80004466:	f26080e7          	jalr	-218(ra) # 80003388 <bread>
    8000446a:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    8000446c:	000aa583          	lw	a1,0(s5)
    80004470:	0289a503          	lw	a0,40(s3)
    80004474:	fffff097          	auipc	ra,0xfffff
    80004478:	f14080e7          	jalr	-236(ra) # 80003388 <bread>
    8000447c:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    8000447e:	40000613          	li	a2,1024
    80004482:	05890593          	addi	a1,s2,88
    80004486:	05850513          	addi	a0,a0,88
    8000448a:	ffffd097          	auipc	ra,0xffffd
    8000448e:	8b6080e7          	jalr	-1866(ra) # 80000d40 <memmove>
    bwrite(dbuf);  // write dst to disk
    80004492:	8526                	mv	a0,s1
    80004494:	fffff097          	auipc	ra,0xfffff
    80004498:	fe6080e7          	jalr	-26(ra) # 8000347a <bwrite>
    if(recovering == 0)
    8000449c:	f80b1ce3          	bnez	s6,80004434 <install_trans+0x40>
    800044a0:	b769                	j	8000442a <install_trans+0x36>
}
    800044a2:	70e2                	ld	ra,56(sp)
    800044a4:	7442                	ld	s0,48(sp)
    800044a6:	74a2                	ld	s1,40(sp)
    800044a8:	7902                	ld	s2,32(sp)
    800044aa:	69e2                	ld	s3,24(sp)
    800044ac:	6a42                	ld	s4,16(sp)
    800044ae:	6aa2                	ld	s5,8(sp)
    800044b0:	6b02                	ld	s6,0(sp)
    800044b2:	6121                	addi	sp,sp,64
    800044b4:	8082                	ret
    800044b6:	8082                	ret

00000000800044b8 <initlog>:
{
    800044b8:	7179                	addi	sp,sp,-48
    800044ba:	f406                	sd	ra,40(sp)
    800044bc:	f022                	sd	s0,32(sp)
    800044be:	ec26                	sd	s1,24(sp)
    800044c0:	e84a                	sd	s2,16(sp)
    800044c2:	e44e                	sd	s3,8(sp)
    800044c4:	1800                	addi	s0,sp,48
    800044c6:	892a                	mv	s2,a0
    800044c8:	89ae                	mv	s3,a1
  initlock(&log.lock, "log");
    800044ca:	0001d497          	auipc	s1,0x1d
    800044ce:	5c648493          	addi	s1,s1,1478 # 80021a90 <log>
    800044d2:	00004597          	auipc	a1,0x4
    800044d6:	21e58593          	addi	a1,a1,542 # 800086f0 <syscalls+0x1f0>
    800044da:	8526                	mv	a0,s1
    800044dc:	ffffc097          	auipc	ra,0xffffc
    800044e0:	678080e7          	jalr	1656(ra) # 80000b54 <initlock>
  log.start = sb->logstart;
    800044e4:	0149a583          	lw	a1,20(s3)
    800044e8:	cc8c                	sw	a1,24(s1)
  log.size = sb->nlog;
    800044ea:	0109a783          	lw	a5,16(s3)
    800044ee:	ccdc                	sw	a5,28(s1)
  log.dev = dev;
    800044f0:	0324a423          	sw	s2,40(s1)
  struct buf *buf = bread(log.dev, log.start);
    800044f4:	854a                	mv	a0,s2
    800044f6:	fffff097          	auipc	ra,0xfffff
    800044fa:	e92080e7          	jalr	-366(ra) # 80003388 <bread>
  log.lh.n = lh->n;
    800044fe:	4d3c                	lw	a5,88(a0)
    80004500:	d4dc                	sw	a5,44(s1)
  for (i = 0; i < log.lh.n; i++) {
    80004502:	02f05563          	blez	a5,8000452c <initlog+0x74>
    80004506:	05c50713          	addi	a4,a0,92
    8000450a:	0001d697          	auipc	a3,0x1d
    8000450e:	5b668693          	addi	a3,a3,1462 # 80021ac0 <log+0x30>
    80004512:	37fd                	addiw	a5,a5,-1
    80004514:	1782                	slli	a5,a5,0x20
    80004516:	9381                	srli	a5,a5,0x20
    80004518:	078a                	slli	a5,a5,0x2
    8000451a:	06050613          	addi	a2,a0,96
    8000451e:	97b2                	add	a5,a5,a2
    log.lh.block[i] = lh->block[i];
    80004520:	4310                	lw	a2,0(a4)
    80004522:	c290                	sw	a2,0(a3)
  for (i = 0; i < log.lh.n; i++) {
    80004524:	0711                	addi	a4,a4,4
    80004526:	0691                	addi	a3,a3,4
    80004528:	fef71ce3          	bne	a4,a5,80004520 <initlog+0x68>
  brelse(buf);
    8000452c:	fffff097          	auipc	ra,0xfffff
    80004530:	f8c080e7          	jalr	-116(ra) # 800034b8 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(1); // if committed, copy from log to disk
    80004534:	4505                	li	a0,1
    80004536:	00000097          	auipc	ra,0x0
    8000453a:	ebe080e7          	jalr	-322(ra) # 800043f4 <install_trans>
  log.lh.n = 0;
    8000453e:	0001d797          	auipc	a5,0x1d
    80004542:	5607af23          	sw	zero,1406(a5) # 80021abc <log+0x2c>
  write_head(); // clear the log
    80004546:	00000097          	auipc	ra,0x0
    8000454a:	e34080e7          	jalr	-460(ra) # 8000437a <write_head>
}
    8000454e:	70a2                	ld	ra,40(sp)
    80004550:	7402                	ld	s0,32(sp)
    80004552:	64e2                	ld	s1,24(sp)
    80004554:	6942                	ld	s2,16(sp)
    80004556:	69a2                	ld	s3,8(sp)
    80004558:	6145                	addi	sp,sp,48
    8000455a:	8082                	ret

000000008000455c <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
    8000455c:	1101                	addi	sp,sp,-32
    8000455e:	ec06                	sd	ra,24(sp)
    80004560:	e822                	sd	s0,16(sp)
    80004562:	e426                	sd	s1,8(sp)
    80004564:	e04a                	sd	s2,0(sp)
    80004566:	1000                	addi	s0,sp,32
  acquire(&log.lock);
    80004568:	0001d517          	auipc	a0,0x1d
    8000456c:	52850513          	addi	a0,a0,1320 # 80021a90 <log>
    80004570:	ffffc097          	auipc	ra,0xffffc
    80004574:	674080e7          	jalr	1652(ra) # 80000be4 <acquire>
  while(1){
    if(log.committing){
    80004578:	0001d497          	auipc	s1,0x1d
    8000457c:	51848493          	addi	s1,s1,1304 # 80021a90 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
    80004580:	4979                	li	s2,30
    80004582:	a039                	j	80004590 <begin_op+0x34>
      sleep(&log, &log.lock);
    80004584:	85a6                	mv	a1,s1
    80004586:	8526                	mv	a0,s1
    80004588:	ffffe097          	auipc	ra,0xffffe
    8000458c:	e28080e7          	jalr	-472(ra) # 800023b0 <sleep>
    if(log.committing){
    80004590:	50dc                	lw	a5,36(s1)
    80004592:	fbed                	bnez	a5,80004584 <begin_op+0x28>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
    80004594:	509c                	lw	a5,32(s1)
    80004596:	0017871b          	addiw	a4,a5,1
    8000459a:	0007069b          	sext.w	a3,a4
    8000459e:	0027179b          	slliw	a5,a4,0x2
    800045a2:	9fb9                	addw	a5,a5,a4
    800045a4:	0017979b          	slliw	a5,a5,0x1
    800045a8:	54d8                	lw	a4,44(s1)
    800045aa:	9fb9                	addw	a5,a5,a4
    800045ac:	00f95963          	bge	s2,a5,800045be <begin_op+0x62>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    800045b0:	85a6                	mv	a1,s1
    800045b2:	8526                	mv	a0,s1
    800045b4:	ffffe097          	auipc	ra,0xffffe
    800045b8:	dfc080e7          	jalr	-516(ra) # 800023b0 <sleep>
    800045bc:	bfd1                	j	80004590 <begin_op+0x34>
    } else {
      log.outstanding += 1;
    800045be:	0001d517          	auipc	a0,0x1d
    800045c2:	4d250513          	addi	a0,a0,1234 # 80021a90 <log>
    800045c6:	d114                	sw	a3,32(a0)
      release(&log.lock);
    800045c8:	ffffc097          	auipc	ra,0xffffc
    800045cc:	6d0080e7          	jalr	1744(ra) # 80000c98 <release>
      break;
    }
  }
}
    800045d0:	60e2                	ld	ra,24(sp)
    800045d2:	6442                	ld	s0,16(sp)
    800045d4:	64a2                	ld	s1,8(sp)
    800045d6:	6902                	ld	s2,0(sp)
    800045d8:	6105                	addi	sp,sp,32
    800045da:	8082                	ret

00000000800045dc <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
    800045dc:	7139                	addi	sp,sp,-64
    800045de:	fc06                	sd	ra,56(sp)
    800045e0:	f822                	sd	s0,48(sp)
    800045e2:	f426                	sd	s1,40(sp)
    800045e4:	f04a                	sd	s2,32(sp)
    800045e6:	ec4e                	sd	s3,24(sp)
    800045e8:	e852                	sd	s4,16(sp)
    800045ea:	e456                	sd	s5,8(sp)
    800045ec:	0080                	addi	s0,sp,64
  int do_commit = 0;

  acquire(&log.lock);
    800045ee:	0001d497          	auipc	s1,0x1d
    800045f2:	4a248493          	addi	s1,s1,1186 # 80021a90 <log>
    800045f6:	8526                	mv	a0,s1
    800045f8:	ffffc097          	auipc	ra,0xffffc
    800045fc:	5ec080e7          	jalr	1516(ra) # 80000be4 <acquire>
  log.outstanding -= 1;
    80004600:	509c                	lw	a5,32(s1)
    80004602:	37fd                	addiw	a5,a5,-1
    80004604:	0007891b          	sext.w	s2,a5
    80004608:	d09c                	sw	a5,32(s1)
  if(log.committing)
    8000460a:	50dc                	lw	a5,36(s1)
    8000460c:	efb9                	bnez	a5,8000466a <end_op+0x8e>
    panic("log.committing");
  if(log.outstanding == 0){
    8000460e:	06091663          	bnez	s2,8000467a <end_op+0x9e>
    do_commit = 1;
    log.committing = 1;
    80004612:	0001d497          	auipc	s1,0x1d
    80004616:	47e48493          	addi	s1,s1,1150 # 80021a90 <log>
    8000461a:	4785                	li	a5,1
    8000461c:	d0dc                	sw	a5,36(s1)
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
    8000461e:	8526                	mv	a0,s1
    80004620:	ffffc097          	auipc	ra,0xffffc
    80004624:	678080e7          	jalr	1656(ra) # 80000c98 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
    80004628:	54dc                	lw	a5,44(s1)
    8000462a:	06f04763          	bgtz	a5,80004698 <end_op+0xbc>
    acquire(&log.lock);
    8000462e:	0001d497          	auipc	s1,0x1d
    80004632:	46248493          	addi	s1,s1,1122 # 80021a90 <log>
    80004636:	8526                	mv	a0,s1
    80004638:	ffffc097          	auipc	ra,0xffffc
    8000463c:	5ac080e7          	jalr	1452(ra) # 80000be4 <acquire>
    log.committing = 0;
    80004640:	0204a223          	sw	zero,36(s1)
    wakeup(&log);
    80004644:	8526                	mv	a0,s1
    80004646:	ffffe097          	auipc	ra,0xffffe
    8000464a:	f00080e7          	jalr	-256(ra) # 80002546 <wakeup>
    release(&log.lock);
    8000464e:	8526                	mv	a0,s1
    80004650:	ffffc097          	auipc	ra,0xffffc
    80004654:	648080e7          	jalr	1608(ra) # 80000c98 <release>
}
    80004658:	70e2                	ld	ra,56(sp)
    8000465a:	7442                	ld	s0,48(sp)
    8000465c:	74a2                	ld	s1,40(sp)
    8000465e:	7902                	ld	s2,32(sp)
    80004660:	69e2                	ld	s3,24(sp)
    80004662:	6a42                	ld	s4,16(sp)
    80004664:	6aa2                	ld	s5,8(sp)
    80004666:	6121                	addi	sp,sp,64
    80004668:	8082                	ret
    panic("log.committing");
    8000466a:	00004517          	auipc	a0,0x4
    8000466e:	08e50513          	addi	a0,a0,142 # 800086f8 <syscalls+0x1f8>
    80004672:	ffffc097          	auipc	ra,0xffffc
    80004676:	ecc080e7          	jalr	-308(ra) # 8000053e <panic>
    wakeup(&log);
    8000467a:	0001d497          	auipc	s1,0x1d
    8000467e:	41648493          	addi	s1,s1,1046 # 80021a90 <log>
    80004682:	8526                	mv	a0,s1
    80004684:	ffffe097          	auipc	ra,0xffffe
    80004688:	ec2080e7          	jalr	-318(ra) # 80002546 <wakeup>
  release(&log.lock);
    8000468c:	8526                	mv	a0,s1
    8000468e:	ffffc097          	auipc	ra,0xffffc
    80004692:	60a080e7          	jalr	1546(ra) # 80000c98 <release>
  if(do_commit){
    80004696:	b7c9                	j	80004658 <end_op+0x7c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80004698:	0001da97          	auipc	s5,0x1d
    8000469c:	428a8a93          	addi	s5,s5,1064 # 80021ac0 <log+0x30>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    800046a0:	0001da17          	auipc	s4,0x1d
    800046a4:	3f0a0a13          	addi	s4,s4,1008 # 80021a90 <log>
    800046a8:	018a2583          	lw	a1,24(s4)
    800046ac:	012585bb          	addw	a1,a1,s2
    800046b0:	2585                	addiw	a1,a1,1
    800046b2:	028a2503          	lw	a0,40(s4)
    800046b6:	fffff097          	auipc	ra,0xfffff
    800046ba:	cd2080e7          	jalr	-814(ra) # 80003388 <bread>
    800046be:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    800046c0:	000aa583          	lw	a1,0(s5)
    800046c4:	028a2503          	lw	a0,40(s4)
    800046c8:	fffff097          	auipc	ra,0xfffff
    800046cc:	cc0080e7          	jalr	-832(ra) # 80003388 <bread>
    800046d0:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    800046d2:	40000613          	li	a2,1024
    800046d6:	05850593          	addi	a1,a0,88
    800046da:	05848513          	addi	a0,s1,88
    800046de:	ffffc097          	auipc	ra,0xffffc
    800046e2:	662080e7          	jalr	1634(ra) # 80000d40 <memmove>
    bwrite(to);  // write the log
    800046e6:	8526                	mv	a0,s1
    800046e8:	fffff097          	auipc	ra,0xfffff
    800046ec:	d92080e7          	jalr	-622(ra) # 8000347a <bwrite>
    brelse(from);
    800046f0:	854e                	mv	a0,s3
    800046f2:	fffff097          	auipc	ra,0xfffff
    800046f6:	dc6080e7          	jalr	-570(ra) # 800034b8 <brelse>
    brelse(to);
    800046fa:	8526                	mv	a0,s1
    800046fc:	fffff097          	auipc	ra,0xfffff
    80004700:	dbc080e7          	jalr	-580(ra) # 800034b8 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80004704:	2905                	addiw	s2,s2,1
    80004706:	0a91                	addi	s5,s5,4
    80004708:	02ca2783          	lw	a5,44(s4)
    8000470c:	f8f94ee3          	blt	s2,a5,800046a8 <end_op+0xcc>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    80004710:	00000097          	auipc	ra,0x0
    80004714:	c6a080e7          	jalr	-918(ra) # 8000437a <write_head>
    install_trans(0); // Now install writes to home locations
    80004718:	4501                	li	a0,0
    8000471a:	00000097          	auipc	ra,0x0
    8000471e:	cda080e7          	jalr	-806(ra) # 800043f4 <install_trans>
    log.lh.n = 0;
    80004722:	0001d797          	auipc	a5,0x1d
    80004726:	3807ad23          	sw	zero,922(a5) # 80021abc <log+0x2c>
    write_head();    // Erase the transaction from the log
    8000472a:	00000097          	auipc	ra,0x0
    8000472e:	c50080e7          	jalr	-944(ra) # 8000437a <write_head>
    80004732:	bdf5                	j	8000462e <end_op+0x52>

0000000080004734 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
    80004734:	1101                	addi	sp,sp,-32
    80004736:	ec06                	sd	ra,24(sp)
    80004738:	e822                	sd	s0,16(sp)
    8000473a:	e426                	sd	s1,8(sp)
    8000473c:	e04a                	sd	s2,0(sp)
    8000473e:	1000                	addi	s0,sp,32
    80004740:	84aa                	mv	s1,a0
  int i;

  acquire(&log.lock);
    80004742:	0001d917          	auipc	s2,0x1d
    80004746:	34e90913          	addi	s2,s2,846 # 80021a90 <log>
    8000474a:	854a                	mv	a0,s2
    8000474c:	ffffc097          	auipc	ra,0xffffc
    80004750:	498080e7          	jalr	1176(ra) # 80000be4 <acquire>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    80004754:	02c92603          	lw	a2,44(s2)
    80004758:	47f5                	li	a5,29
    8000475a:	06c7c563          	blt	a5,a2,800047c4 <log_write+0x90>
    8000475e:	0001d797          	auipc	a5,0x1d
    80004762:	34e7a783          	lw	a5,846(a5) # 80021aac <log+0x1c>
    80004766:	37fd                	addiw	a5,a5,-1
    80004768:	04f65e63          	bge	a2,a5,800047c4 <log_write+0x90>
    panic("too big a transaction");
  if (log.outstanding < 1)
    8000476c:	0001d797          	auipc	a5,0x1d
    80004770:	3447a783          	lw	a5,836(a5) # 80021ab0 <log+0x20>
    80004774:	06f05063          	blez	a5,800047d4 <log_write+0xa0>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80004778:	4781                	li	a5,0
    8000477a:	06c05563          	blez	a2,800047e4 <log_write+0xb0>
    if (log.lh.block[i] == b->blockno)   // log absorption
    8000477e:	44cc                	lw	a1,12(s1)
    80004780:	0001d717          	auipc	a4,0x1d
    80004784:	34070713          	addi	a4,a4,832 # 80021ac0 <log+0x30>
  for (i = 0; i < log.lh.n; i++) {
    80004788:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    8000478a:	4314                	lw	a3,0(a4)
    8000478c:	04b68c63          	beq	a3,a1,800047e4 <log_write+0xb0>
  for (i = 0; i < log.lh.n; i++) {
    80004790:	2785                	addiw	a5,a5,1
    80004792:	0711                	addi	a4,a4,4
    80004794:	fef61be3          	bne	a2,a5,8000478a <log_write+0x56>
      break;
  }
  log.lh.block[i] = b->blockno;
    80004798:	0621                	addi	a2,a2,8
    8000479a:	060a                	slli	a2,a2,0x2
    8000479c:	0001d797          	auipc	a5,0x1d
    800047a0:	2f478793          	addi	a5,a5,756 # 80021a90 <log>
    800047a4:	963e                	add	a2,a2,a5
    800047a6:	44dc                	lw	a5,12(s1)
    800047a8:	ca1c                	sw	a5,16(a2)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    800047aa:	8526                	mv	a0,s1
    800047ac:	fffff097          	auipc	ra,0xfffff
    800047b0:	daa080e7          	jalr	-598(ra) # 80003556 <bpin>
    log.lh.n++;
    800047b4:	0001d717          	auipc	a4,0x1d
    800047b8:	2dc70713          	addi	a4,a4,732 # 80021a90 <log>
    800047bc:	575c                	lw	a5,44(a4)
    800047be:	2785                	addiw	a5,a5,1
    800047c0:	d75c                	sw	a5,44(a4)
    800047c2:	a835                	j	800047fe <log_write+0xca>
    panic("too big a transaction");
    800047c4:	00004517          	auipc	a0,0x4
    800047c8:	f4450513          	addi	a0,a0,-188 # 80008708 <syscalls+0x208>
    800047cc:	ffffc097          	auipc	ra,0xffffc
    800047d0:	d72080e7          	jalr	-654(ra) # 8000053e <panic>
    panic("log_write outside of trans");
    800047d4:	00004517          	auipc	a0,0x4
    800047d8:	f4c50513          	addi	a0,a0,-180 # 80008720 <syscalls+0x220>
    800047dc:	ffffc097          	auipc	ra,0xffffc
    800047e0:	d62080e7          	jalr	-670(ra) # 8000053e <panic>
  log.lh.block[i] = b->blockno;
    800047e4:	00878713          	addi	a4,a5,8
    800047e8:	00271693          	slli	a3,a4,0x2
    800047ec:	0001d717          	auipc	a4,0x1d
    800047f0:	2a470713          	addi	a4,a4,676 # 80021a90 <log>
    800047f4:	9736                	add	a4,a4,a3
    800047f6:	44d4                	lw	a3,12(s1)
    800047f8:	cb14                	sw	a3,16(a4)
  if (i == log.lh.n) {  // Add new block to log?
    800047fa:	faf608e3          	beq	a2,a5,800047aa <log_write+0x76>
  }
  release(&log.lock);
    800047fe:	0001d517          	auipc	a0,0x1d
    80004802:	29250513          	addi	a0,a0,658 # 80021a90 <log>
    80004806:	ffffc097          	auipc	ra,0xffffc
    8000480a:	492080e7          	jalr	1170(ra) # 80000c98 <release>
}
    8000480e:	60e2                	ld	ra,24(sp)
    80004810:	6442                	ld	s0,16(sp)
    80004812:	64a2                	ld	s1,8(sp)
    80004814:	6902                	ld	s2,0(sp)
    80004816:	6105                	addi	sp,sp,32
    80004818:	8082                	ret

000000008000481a <initsleeplock>:
#include "proc.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
    8000481a:	1101                	addi	sp,sp,-32
    8000481c:	ec06                	sd	ra,24(sp)
    8000481e:	e822                	sd	s0,16(sp)
    80004820:	e426                	sd	s1,8(sp)
    80004822:	e04a                	sd	s2,0(sp)
    80004824:	1000                	addi	s0,sp,32
    80004826:	84aa                	mv	s1,a0
    80004828:	892e                	mv	s2,a1
  initlock(&lk->lk, "sleep lock");
    8000482a:	00004597          	auipc	a1,0x4
    8000482e:	f1658593          	addi	a1,a1,-234 # 80008740 <syscalls+0x240>
    80004832:	0521                	addi	a0,a0,8
    80004834:	ffffc097          	auipc	ra,0xffffc
    80004838:	320080e7          	jalr	800(ra) # 80000b54 <initlock>
  lk->name = name;
    8000483c:	0324b023          	sd	s2,32(s1)
  lk->locked = 0;
    80004840:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80004844:	0204a423          	sw	zero,40(s1)
}
    80004848:	60e2                	ld	ra,24(sp)
    8000484a:	6442                	ld	s0,16(sp)
    8000484c:	64a2                	ld	s1,8(sp)
    8000484e:	6902                	ld	s2,0(sp)
    80004850:	6105                	addi	sp,sp,32
    80004852:	8082                	ret

0000000080004854 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
    80004854:	1101                	addi	sp,sp,-32
    80004856:	ec06                	sd	ra,24(sp)
    80004858:	e822                	sd	s0,16(sp)
    8000485a:	e426                	sd	s1,8(sp)
    8000485c:	e04a                	sd	s2,0(sp)
    8000485e:	1000                	addi	s0,sp,32
    80004860:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80004862:	00850913          	addi	s2,a0,8
    80004866:	854a                	mv	a0,s2
    80004868:	ffffc097          	auipc	ra,0xffffc
    8000486c:	37c080e7          	jalr	892(ra) # 80000be4 <acquire>
  while (lk->locked) {
    80004870:	409c                	lw	a5,0(s1)
    80004872:	cb89                	beqz	a5,80004884 <acquiresleep+0x30>
    sleep(lk, &lk->lk);
    80004874:	85ca                	mv	a1,s2
    80004876:	8526                	mv	a0,s1
    80004878:	ffffe097          	auipc	ra,0xffffe
    8000487c:	b38080e7          	jalr	-1224(ra) # 800023b0 <sleep>
  while (lk->locked) {
    80004880:	409c                	lw	a5,0(s1)
    80004882:	fbed                	bnez	a5,80004874 <acquiresleep+0x20>
  }
  lk->locked = 1;
    80004884:	4785                	li	a5,1
    80004886:	c09c                	sw	a5,0(s1)
  lk->pid = myproc()->pid;
    80004888:	ffffd097          	auipc	ra,0xffffd
    8000488c:	156080e7          	jalr	342(ra) # 800019de <myproc>
    80004890:	591c                	lw	a5,48(a0)
    80004892:	d49c                	sw	a5,40(s1)
  release(&lk->lk);
    80004894:	854a                	mv	a0,s2
    80004896:	ffffc097          	auipc	ra,0xffffc
    8000489a:	402080e7          	jalr	1026(ra) # 80000c98 <release>
}
    8000489e:	60e2                	ld	ra,24(sp)
    800048a0:	6442                	ld	s0,16(sp)
    800048a2:	64a2                	ld	s1,8(sp)
    800048a4:	6902                	ld	s2,0(sp)
    800048a6:	6105                	addi	sp,sp,32
    800048a8:	8082                	ret

00000000800048aa <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
    800048aa:	1101                	addi	sp,sp,-32
    800048ac:	ec06                	sd	ra,24(sp)
    800048ae:	e822                	sd	s0,16(sp)
    800048b0:	e426                	sd	s1,8(sp)
    800048b2:	e04a                	sd	s2,0(sp)
    800048b4:	1000                	addi	s0,sp,32
    800048b6:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    800048b8:	00850913          	addi	s2,a0,8
    800048bc:	854a                	mv	a0,s2
    800048be:	ffffc097          	auipc	ra,0xffffc
    800048c2:	326080e7          	jalr	806(ra) # 80000be4 <acquire>
  lk->locked = 0;
    800048c6:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    800048ca:	0204a423          	sw	zero,40(s1)
  wakeup(lk);
    800048ce:	8526                	mv	a0,s1
    800048d0:	ffffe097          	auipc	ra,0xffffe
    800048d4:	c76080e7          	jalr	-906(ra) # 80002546 <wakeup>
  release(&lk->lk);
    800048d8:	854a                	mv	a0,s2
    800048da:	ffffc097          	auipc	ra,0xffffc
    800048de:	3be080e7          	jalr	958(ra) # 80000c98 <release>
}
    800048e2:	60e2                	ld	ra,24(sp)
    800048e4:	6442                	ld	s0,16(sp)
    800048e6:	64a2                	ld	s1,8(sp)
    800048e8:	6902                	ld	s2,0(sp)
    800048ea:	6105                	addi	sp,sp,32
    800048ec:	8082                	ret

00000000800048ee <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
    800048ee:	7179                	addi	sp,sp,-48
    800048f0:	f406                	sd	ra,40(sp)
    800048f2:	f022                	sd	s0,32(sp)
    800048f4:	ec26                	sd	s1,24(sp)
    800048f6:	e84a                	sd	s2,16(sp)
    800048f8:	e44e                	sd	s3,8(sp)
    800048fa:	1800                	addi	s0,sp,48
    800048fc:	84aa                	mv	s1,a0
  int r;
  
  acquire(&lk->lk);
    800048fe:	00850913          	addi	s2,a0,8
    80004902:	854a                	mv	a0,s2
    80004904:	ffffc097          	auipc	ra,0xffffc
    80004908:	2e0080e7          	jalr	736(ra) # 80000be4 <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
    8000490c:	409c                	lw	a5,0(s1)
    8000490e:	ef99                	bnez	a5,8000492c <holdingsleep+0x3e>
    80004910:	4481                	li	s1,0
  release(&lk->lk);
    80004912:	854a                	mv	a0,s2
    80004914:	ffffc097          	auipc	ra,0xffffc
    80004918:	384080e7          	jalr	900(ra) # 80000c98 <release>
  return r;
}
    8000491c:	8526                	mv	a0,s1
    8000491e:	70a2                	ld	ra,40(sp)
    80004920:	7402                	ld	s0,32(sp)
    80004922:	64e2                	ld	s1,24(sp)
    80004924:	6942                	ld	s2,16(sp)
    80004926:	69a2                	ld	s3,8(sp)
    80004928:	6145                	addi	sp,sp,48
    8000492a:	8082                	ret
  r = lk->locked && (lk->pid == myproc()->pid);
    8000492c:	0284a983          	lw	s3,40(s1)
    80004930:	ffffd097          	auipc	ra,0xffffd
    80004934:	0ae080e7          	jalr	174(ra) # 800019de <myproc>
    80004938:	5904                	lw	s1,48(a0)
    8000493a:	413484b3          	sub	s1,s1,s3
    8000493e:	0014b493          	seqz	s1,s1
    80004942:	bfc1                	j	80004912 <holdingsleep+0x24>

0000000080004944 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
    80004944:	1141                	addi	sp,sp,-16
    80004946:	e406                	sd	ra,8(sp)
    80004948:	e022                	sd	s0,0(sp)
    8000494a:	0800                	addi	s0,sp,16
  initlock(&ftable.lock, "ftable");
    8000494c:	00004597          	auipc	a1,0x4
    80004950:	e0458593          	addi	a1,a1,-508 # 80008750 <syscalls+0x250>
    80004954:	0001d517          	auipc	a0,0x1d
    80004958:	28450513          	addi	a0,a0,644 # 80021bd8 <ftable>
    8000495c:	ffffc097          	auipc	ra,0xffffc
    80004960:	1f8080e7          	jalr	504(ra) # 80000b54 <initlock>
}
    80004964:	60a2                	ld	ra,8(sp)
    80004966:	6402                	ld	s0,0(sp)
    80004968:	0141                	addi	sp,sp,16
    8000496a:	8082                	ret

000000008000496c <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
    8000496c:	1101                	addi	sp,sp,-32
    8000496e:	ec06                	sd	ra,24(sp)
    80004970:	e822                	sd	s0,16(sp)
    80004972:	e426                	sd	s1,8(sp)
    80004974:	1000                	addi	s0,sp,32
  struct file *f;

  acquire(&ftable.lock);
    80004976:	0001d517          	auipc	a0,0x1d
    8000497a:	26250513          	addi	a0,a0,610 # 80021bd8 <ftable>
    8000497e:	ffffc097          	auipc	ra,0xffffc
    80004982:	266080e7          	jalr	614(ra) # 80000be4 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80004986:	0001d497          	auipc	s1,0x1d
    8000498a:	26a48493          	addi	s1,s1,618 # 80021bf0 <ftable+0x18>
    8000498e:	0001e717          	auipc	a4,0x1e
    80004992:	20270713          	addi	a4,a4,514 # 80022b90 <ftable+0xfb8>
    if(f->ref == 0){
    80004996:	40dc                	lw	a5,4(s1)
    80004998:	cf99                	beqz	a5,800049b6 <filealloc+0x4a>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    8000499a:	02848493          	addi	s1,s1,40
    8000499e:	fee49ce3          	bne	s1,a4,80004996 <filealloc+0x2a>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
    800049a2:	0001d517          	auipc	a0,0x1d
    800049a6:	23650513          	addi	a0,a0,566 # 80021bd8 <ftable>
    800049aa:	ffffc097          	auipc	ra,0xffffc
    800049ae:	2ee080e7          	jalr	750(ra) # 80000c98 <release>
  return 0;
    800049b2:	4481                	li	s1,0
    800049b4:	a819                	j	800049ca <filealloc+0x5e>
      f->ref = 1;
    800049b6:	4785                	li	a5,1
    800049b8:	c0dc                	sw	a5,4(s1)
      release(&ftable.lock);
    800049ba:	0001d517          	auipc	a0,0x1d
    800049be:	21e50513          	addi	a0,a0,542 # 80021bd8 <ftable>
    800049c2:	ffffc097          	auipc	ra,0xffffc
    800049c6:	2d6080e7          	jalr	726(ra) # 80000c98 <release>
}
    800049ca:	8526                	mv	a0,s1
    800049cc:	60e2                	ld	ra,24(sp)
    800049ce:	6442                	ld	s0,16(sp)
    800049d0:	64a2                	ld	s1,8(sp)
    800049d2:	6105                	addi	sp,sp,32
    800049d4:	8082                	ret

00000000800049d6 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
    800049d6:	1101                	addi	sp,sp,-32
    800049d8:	ec06                	sd	ra,24(sp)
    800049da:	e822                	sd	s0,16(sp)
    800049dc:	e426                	sd	s1,8(sp)
    800049de:	1000                	addi	s0,sp,32
    800049e0:	84aa                	mv	s1,a0
  acquire(&ftable.lock);
    800049e2:	0001d517          	auipc	a0,0x1d
    800049e6:	1f650513          	addi	a0,a0,502 # 80021bd8 <ftable>
    800049ea:	ffffc097          	auipc	ra,0xffffc
    800049ee:	1fa080e7          	jalr	506(ra) # 80000be4 <acquire>
  if(f->ref < 1)
    800049f2:	40dc                	lw	a5,4(s1)
    800049f4:	02f05263          	blez	a5,80004a18 <filedup+0x42>
    panic("filedup");
  f->ref++;
    800049f8:	2785                	addiw	a5,a5,1
    800049fa:	c0dc                	sw	a5,4(s1)
  release(&ftable.lock);
    800049fc:	0001d517          	auipc	a0,0x1d
    80004a00:	1dc50513          	addi	a0,a0,476 # 80021bd8 <ftable>
    80004a04:	ffffc097          	auipc	ra,0xffffc
    80004a08:	294080e7          	jalr	660(ra) # 80000c98 <release>
  return f;
}
    80004a0c:	8526                	mv	a0,s1
    80004a0e:	60e2                	ld	ra,24(sp)
    80004a10:	6442                	ld	s0,16(sp)
    80004a12:	64a2                	ld	s1,8(sp)
    80004a14:	6105                	addi	sp,sp,32
    80004a16:	8082                	ret
    panic("filedup");
    80004a18:	00004517          	auipc	a0,0x4
    80004a1c:	d4050513          	addi	a0,a0,-704 # 80008758 <syscalls+0x258>
    80004a20:	ffffc097          	auipc	ra,0xffffc
    80004a24:	b1e080e7          	jalr	-1250(ra) # 8000053e <panic>

0000000080004a28 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
    80004a28:	7139                	addi	sp,sp,-64
    80004a2a:	fc06                	sd	ra,56(sp)
    80004a2c:	f822                	sd	s0,48(sp)
    80004a2e:	f426                	sd	s1,40(sp)
    80004a30:	f04a                	sd	s2,32(sp)
    80004a32:	ec4e                	sd	s3,24(sp)
    80004a34:	e852                	sd	s4,16(sp)
    80004a36:	e456                	sd	s5,8(sp)
    80004a38:	0080                	addi	s0,sp,64
    80004a3a:	84aa                	mv	s1,a0
  struct file ff;

  acquire(&ftable.lock);
    80004a3c:	0001d517          	auipc	a0,0x1d
    80004a40:	19c50513          	addi	a0,a0,412 # 80021bd8 <ftable>
    80004a44:	ffffc097          	auipc	ra,0xffffc
    80004a48:	1a0080e7          	jalr	416(ra) # 80000be4 <acquire>
  if(f->ref < 1)
    80004a4c:	40dc                	lw	a5,4(s1)
    80004a4e:	06f05163          	blez	a5,80004ab0 <fileclose+0x88>
    panic("fileclose");
  if(--f->ref > 0){
    80004a52:	37fd                	addiw	a5,a5,-1
    80004a54:	0007871b          	sext.w	a4,a5
    80004a58:	c0dc                	sw	a5,4(s1)
    80004a5a:	06e04363          	bgtz	a4,80004ac0 <fileclose+0x98>
    release(&ftable.lock);
    return;
  }
  ff = *f;
    80004a5e:	0004a903          	lw	s2,0(s1)
    80004a62:	0094ca83          	lbu	s5,9(s1)
    80004a66:	0104ba03          	ld	s4,16(s1)
    80004a6a:	0184b983          	ld	s3,24(s1)
  f->ref = 0;
    80004a6e:	0004a223          	sw	zero,4(s1)
  f->type = FD_NONE;
    80004a72:	0004a023          	sw	zero,0(s1)
  release(&ftable.lock);
    80004a76:	0001d517          	auipc	a0,0x1d
    80004a7a:	16250513          	addi	a0,a0,354 # 80021bd8 <ftable>
    80004a7e:	ffffc097          	auipc	ra,0xffffc
    80004a82:	21a080e7          	jalr	538(ra) # 80000c98 <release>

  if(ff.type == FD_PIPE){
    80004a86:	4785                	li	a5,1
    80004a88:	04f90d63          	beq	s2,a5,80004ae2 <fileclose+0xba>
    pipeclose(ff.pipe, ff.writable);
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){
    80004a8c:	3979                	addiw	s2,s2,-2
    80004a8e:	4785                	li	a5,1
    80004a90:	0527e063          	bltu	a5,s2,80004ad0 <fileclose+0xa8>
    begin_op();
    80004a94:	00000097          	auipc	ra,0x0
    80004a98:	ac8080e7          	jalr	-1336(ra) # 8000455c <begin_op>
    iput(ff.ip);
    80004a9c:	854e                	mv	a0,s3
    80004a9e:	fffff097          	auipc	ra,0xfffff
    80004aa2:	2a6080e7          	jalr	678(ra) # 80003d44 <iput>
    end_op();
    80004aa6:	00000097          	auipc	ra,0x0
    80004aaa:	b36080e7          	jalr	-1226(ra) # 800045dc <end_op>
    80004aae:	a00d                	j	80004ad0 <fileclose+0xa8>
    panic("fileclose");
    80004ab0:	00004517          	auipc	a0,0x4
    80004ab4:	cb050513          	addi	a0,a0,-848 # 80008760 <syscalls+0x260>
    80004ab8:	ffffc097          	auipc	ra,0xffffc
    80004abc:	a86080e7          	jalr	-1402(ra) # 8000053e <panic>
    release(&ftable.lock);
    80004ac0:	0001d517          	auipc	a0,0x1d
    80004ac4:	11850513          	addi	a0,a0,280 # 80021bd8 <ftable>
    80004ac8:	ffffc097          	auipc	ra,0xffffc
    80004acc:	1d0080e7          	jalr	464(ra) # 80000c98 <release>
  }
}
    80004ad0:	70e2                	ld	ra,56(sp)
    80004ad2:	7442                	ld	s0,48(sp)
    80004ad4:	74a2                	ld	s1,40(sp)
    80004ad6:	7902                	ld	s2,32(sp)
    80004ad8:	69e2                	ld	s3,24(sp)
    80004ada:	6a42                	ld	s4,16(sp)
    80004adc:	6aa2                	ld	s5,8(sp)
    80004ade:	6121                	addi	sp,sp,64
    80004ae0:	8082                	ret
    pipeclose(ff.pipe, ff.writable);
    80004ae2:	85d6                	mv	a1,s5
    80004ae4:	8552                	mv	a0,s4
    80004ae6:	00000097          	auipc	ra,0x0
    80004aea:	34c080e7          	jalr	844(ra) # 80004e32 <pipeclose>
    80004aee:	b7cd                	j	80004ad0 <fileclose+0xa8>

0000000080004af0 <filestat>:

// Get metadata about file f.
// addr is a user virtual address, pointing to a struct stat.
int
filestat(struct file *f, uint64 addr)
{
    80004af0:	715d                	addi	sp,sp,-80
    80004af2:	e486                	sd	ra,72(sp)
    80004af4:	e0a2                	sd	s0,64(sp)
    80004af6:	fc26                	sd	s1,56(sp)
    80004af8:	f84a                	sd	s2,48(sp)
    80004afa:	f44e                	sd	s3,40(sp)
    80004afc:	0880                	addi	s0,sp,80
    80004afe:	84aa                	mv	s1,a0
    80004b00:	89ae                	mv	s3,a1
  struct proc *p = myproc();
    80004b02:	ffffd097          	auipc	ra,0xffffd
    80004b06:	edc080e7          	jalr	-292(ra) # 800019de <myproc>
  struct stat st;
  
  if(f->type == FD_INODE || f->type == FD_DEVICE){
    80004b0a:	409c                	lw	a5,0(s1)
    80004b0c:	37f9                	addiw	a5,a5,-2
    80004b0e:	4705                	li	a4,1
    80004b10:	04f76763          	bltu	a4,a5,80004b5e <filestat+0x6e>
    80004b14:	892a                	mv	s2,a0
    ilock(f->ip);
    80004b16:	6c88                	ld	a0,24(s1)
    80004b18:	fffff097          	auipc	ra,0xfffff
    80004b1c:	072080e7          	jalr	114(ra) # 80003b8a <ilock>
    stati(f->ip, &st);
    80004b20:	fb840593          	addi	a1,s0,-72
    80004b24:	6c88                	ld	a0,24(s1)
    80004b26:	fffff097          	auipc	ra,0xfffff
    80004b2a:	2ee080e7          	jalr	750(ra) # 80003e14 <stati>
    iunlock(f->ip);
    80004b2e:	6c88                	ld	a0,24(s1)
    80004b30:	fffff097          	auipc	ra,0xfffff
    80004b34:	11c080e7          	jalr	284(ra) # 80003c4c <iunlock>
    if(copyout(p->pagetable, addr, (char *)&st, sizeof(st)) < 0)
    80004b38:	46e1                	li	a3,24
    80004b3a:	fb840613          	addi	a2,s0,-72
    80004b3e:	85ce                	mv	a1,s3
    80004b40:	07093503          	ld	a0,112(s2)
    80004b44:	ffffd097          	auipc	ra,0xffffd
    80004b48:	b36080e7          	jalr	-1226(ra) # 8000167a <copyout>
    80004b4c:	41f5551b          	sraiw	a0,a0,0x1f
      return -1;
    return 0;
  }
  return -1;
}
    80004b50:	60a6                	ld	ra,72(sp)
    80004b52:	6406                	ld	s0,64(sp)
    80004b54:	74e2                	ld	s1,56(sp)
    80004b56:	7942                	ld	s2,48(sp)
    80004b58:	79a2                	ld	s3,40(sp)
    80004b5a:	6161                	addi	sp,sp,80
    80004b5c:	8082                	ret
  return -1;
    80004b5e:	557d                	li	a0,-1
    80004b60:	bfc5                	j	80004b50 <filestat+0x60>

0000000080004b62 <fileread>:

// Read from file f.
// addr is a user virtual address.
int
fileread(struct file *f, uint64 addr, int n)
{
    80004b62:	7179                	addi	sp,sp,-48
    80004b64:	f406                	sd	ra,40(sp)
    80004b66:	f022                	sd	s0,32(sp)
    80004b68:	ec26                	sd	s1,24(sp)
    80004b6a:	e84a                	sd	s2,16(sp)
    80004b6c:	e44e                	sd	s3,8(sp)
    80004b6e:	1800                	addi	s0,sp,48
  int r = 0;

  if(f->readable == 0)
    80004b70:	00854783          	lbu	a5,8(a0)
    80004b74:	c3d5                	beqz	a5,80004c18 <fileread+0xb6>
    80004b76:	84aa                	mv	s1,a0
    80004b78:	89ae                	mv	s3,a1
    80004b7a:	8932                	mv	s2,a2
    return -1;

  if(f->type == FD_PIPE){
    80004b7c:	411c                	lw	a5,0(a0)
    80004b7e:	4705                	li	a4,1
    80004b80:	04e78963          	beq	a5,a4,80004bd2 <fileread+0x70>
    r = piperead(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004b84:	470d                	li	a4,3
    80004b86:	04e78d63          	beq	a5,a4,80004be0 <fileread+0x7e>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
    r = devsw[f->major].read(1, addr, n);
  } else if(f->type == FD_INODE){
    80004b8a:	4709                	li	a4,2
    80004b8c:	06e79e63          	bne	a5,a4,80004c08 <fileread+0xa6>
    ilock(f->ip);
    80004b90:	6d08                	ld	a0,24(a0)
    80004b92:	fffff097          	auipc	ra,0xfffff
    80004b96:	ff8080e7          	jalr	-8(ra) # 80003b8a <ilock>
    if((r = readi(f->ip, 1, addr, f->off, n)) > 0)
    80004b9a:	874a                	mv	a4,s2
    80004b9c:	5094                	lw	a3,32(s1)
    80004b9e:	864e                	mv	a2,s3
    80004ba0:	4585                	li	a1,1
    80004ba2:	6c88                	ld	a0,24(s1)
    80004ba4:	fffff097          	auipc	ra,0xfffff
    80004ba8:	29a080e7          	jalr	666(ra) # 80003e3e <readi>
    80004bac:	892a                	mv	s2,a0
    80004bae:	00a05563          	blez	a0,80004bb8 <fileread+0x56>
      f->off += r;
    80004bb2:	509c                	lw	a5,32(s1)
    80004bb4:	9fa9                	addw	a5,a5,a0
    80004bb6:	d09c                	sw	a5,32(s1)
    iunlock(f->ip);
    80004bb8:	6c88                	ld	a0,24(s1)
    80004bba:	fffff097          	auipc	ra,0xfffff
    80004bbe:	092080e7          	jalr	146(ra) # 80003c4c <iunlock>
  } else {
    panic("fileread");
  }

  return r;
}
    80004bc2:	854a                	mv	a0,s2
    80004bc4:	70a2                	ld	ra,40(sp)
    80004bc6:	7402                	ld	s0,32(sp)
    80004bc8:	64e2                	ld	s1,24(sp)
    80004bca:	6942                	ld	s2,16(sp)
    80004bcc:	69a2                	ld	s3,8(sp)
    80004bce:	6145                	addi	sp,sp,48
    80004bd0:	8082                	ret
    r = piperead(f->pipe, addr, n);
    80004bd2:	6908                	ld	a0,16(a0)
    80004bd4:	00000097          	auipc	ra,0x0
    80004bd8:	3c8080e7          	jalr	968(ra) # 80004f9c <piperead>
    80004bdc:	892a                	mv	s2,a0
    80004bde:	b7d5                	j	80004bc2 <fileread+0x60>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
    80004be0:	02451783          	lh	a5,36(a0)
    80004be4:	03079693          	slli	a3,a5,0x30
    80004be8:	92c1                	srli	a3,a3,0x30
    80004bea:	4725                	li	a4,9
    80004bec:	02d76863          	bltu	a4,a3,80004c1c <fileread+0xba>
    80004bf0:	0792                	slli	a5,a5,0x4
    80004bf2:	0001d717          	auipc	a4,0x1d
    80004bf6:	f4670713          	addi	a4,a4,-186 # 80021b38 <devsw>
    80004bfa:	97ba                	add	a5,a5,a4
    80004bfc:	639c                	ld	a5,0(a5)
    80004bfe:	c38d                	beqz	a5,80004c20 <fileread+0xbe>
    r = devsw[f->major].read(1, addr, n);
    80004c00:	4505                	li	a0,1
    80004c02:	9782                	jalr	a5
    80004c04:	892a                	mv	s2,a0
    80004c06:	bf75                	j	80004bc2 <fileread+0x60>
    panic("fileread");
    80004c08:	00004517          	auipc	a0,0x4
    80004c0c:	b6850513          	addi	a0,a0,-1176 # 80008770 <syscalls+0x270>
    80004c10:	ffffc097          	auipc	ra,0xffffc
    80004c14:	92e080e7          	jalr	-1746(ra) # 8000053e <panic>
    return -1;
    80004c18:	597d                	li	s2,-1
    80004c1a:	b765                	j	80004bc2 <fileread+0x60>
      return -1;
    80004c1c:	597d                	li	s2,-1
    80004c1e:	b755                	j	80004bc2 <fileread+0x60>
    80004c20:	597d                	li	s2,-1
    80004c22:	b745                	j	80004bc2 <fileread+0x60>

0000000080004c24 <filewrite>:

// Write to file f.
// addr is a user virtual address.
int
filewrite(struct file *f, uint64 addr, int n)
{
    80004c24:	715d                	addi	sp,sp,-80
    80004c26:	e486                	sd	ra,72(sp)
    80004c28:	e0a2                	sd	s0,64(sp)
    80004c2a:	fc26                	sd	s1,56(sp)
    80004c2c:	f84a                	sd	s2,48(sp)
    80004c2e:	f44e                	sd	s3,40(sp)
    80004c30:	f052                	sd	s4,32(sp)
    80004c32:	ec56                	sd	s5,24(sp)
    80004c34:	e85a                	sd	s6,16(sp)
    80004c36:	e45e                	sd	s7,8(sp)
    80004c38:	e062                	sd	s8,0(sp)
    80004c3a:	0880                	addi	s0,sp,80
  int r, ret = 0;

  if(f->writable == 0)
    80004c3c:	00954783          	lbu	a5,9(a0)
    80004c40:	10078663          	beqz	a5,80004d4c <filewrite+0x128>
    80004c44:	892a                	mv	s2,a0
    80004c46:	8aae                	mv	s5,a1
    80004c48:	8a32                	mv	s4,a2
    return -1;

  if(f->type == FD_PIPE){
    80004c4a:	411c                	lw	a5,0(a0)
    80004c4c:	4705                	li	a4,1
    80004c4e:	02e78263          	beq	a5,a4,80004c72 <filewrite+0x4e>
    ret = pipewrite(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004c52:	470d                	li	a4,3
    80004c54:	02e78663          	beq	a5,a4,80004c80 <filewrite+0x5c>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
    ret = devsw[f->major].write(1, addr, n);
  } else if(f->type == FD_INODE){
    80004c58:	4709                	li	a4,2
    80004c5a:	0ee79163          	bne	a5,a4,80004d3c <filewrite+0x118>
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0;
    while(i < n){
    80004c5e:	0ac05d63          	blez	a2,80004d18 <filewrite+0xf4>
    int i = 0;
    80004c62:	4981                	li	s3,0
    80004c64:	6b05                	lui	s6,0x1
    80004c66:	c00b0b13          	addi	s6,s6,-1024 # c00 <_entry-0x7ffff400>
    80004c6a:	6b85                	lui	s7,0x1
    80004c6c:	c00b8b9b          	addiw	s7,s7,-1024
    80004c70:	a861                	j	80004d08 <filewrite+0xe4>
    ret = pipewrite(f->pipe, addr, n);
    80004c72:	6908                	ld	a0,16(a0)
    80004c74:	00000097          	auipc	ra,0x0
    80004c78:	22e080e7          	jalr	558(ra) # 80004ea2 <pipewrite>
    80004c7c:	8a2a                	mv	s4,a0
    80004c7e:	a045                	j	80004d1e <filewrite+0xfa>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
    80004c80:	02451783          	lh	a5,36(a0)
    80004c84:	03079693          	slli	a3,a5,0x30
    80004c88:	92c1                	srli	a3,a3,0x30
    80004c8a:	4725                	li	a4,9
    80004c8c:	0cd76263          	bltu	a4,a3,80004d50 <filewrite+0x12c>
    80004c90:	0792                	slli	a5,a5,0x4
    80004c92:	0001d717          	auipc	a4,0x1d
    80004c96:	ea670713          	addi	a4,a4,-346 # 80021b38 <devsw>
    80004c9a:	97ba                	add	a5,a5,a4
    80004c9c:	679c                	ld	a5,8(a5)
    80004c9e:	cbdd                	beqz	a5,80004d54 <filewrite+0x130>
    ret = devsw[f->major].write(1, addr, n);
    80004ca0:	4505                	li	a0,1
    80004ca2:	9782                	jalr	a5
    80004ca4:	8a2a                	mv	s4,a0
    80004ca6:	a8a5                	j	80004d1e <filewrite+0xfa>
    80004ca8:	00048c1b          	sext.w	s8,s1
      int n1 = n - i;
      if(n1 > max)
        n1 = max;

      begin_op();
    80004cac:	00000097          	auipc	ra,0x0
    80004cb0:	8b0080e7          	jalr	-1872(ra) # 8000455c <begin_op>
      ilock(f->ip);
    80004cb4:	01893503          	ld	a0,24(s2)
    80004cb8:	fffff097          	auipc	ra,0xfffff
    80004cbc:	ed2080e7          	jalr	-302(ra) # 80003b8a <ilock>
      if ((r = writei(f->ip, 1, addr + i, f->off, n1)) > 0)
    80004cc0:	8762                	mv	a4,s8
    80004cc2:	02092683          	lw	a3,32(s2)
    80004cc6:	01598633          	add	a2,s3,s5
    80004cca:	4585                	li	a1,1
    80004ccc:	01893503          	ld	a0,24(s2)
    80004cd0:	fffff097          	auipc	ra,0xfffff
    80004cd4:	266080e7          	jalr	614(ra) # 80003f36 <writei>
    80004cd8:	84aa                	mv	s1,a0
    80004cda:	00a05763          	blez	a0,80004ce8 <filewrite+0xc4>
        f->off += r;
    80004cde:	02092783          	lw	a5,32(s2)
    80004ce2:	9fa9                	addw	a5,a5,a0
    80004ce4:	02f92023          	sw	a5,32(s2)
      iunlock(f->ip);
    80004ce8:	01893503          	ld	a0,24(s2)
    80004cec:	fffff097          	auipc	ra,0xfffff
    80004cf0:	f60080e7          	jalr	-160(ra) # 80003c4c <iunlock>
      end_op();
    80004cf4:	00000097          	auipc	ra,0x0
    80004cf8:	8e8080e7          	jalr	-1816(ra) # 800045dc <end_op>

      if(r != n1){
    80004cfc:	009c1f63          	bne	s8,s1,80004d1a <filewrite+0xf6>
        // error from writei
        break;
      }
      i += r;
    80004d00:	013489bb          	addw	s3,s1,s3
    while(i < n){
    80004d04:	0149db63          	bge	s3,s4,80004d1a <filewrite+0xf6>
      int n1 = n - i;
    80004d08:	413a07bb          	subw	a5,s4,s3
      if(n1 > max)
    80004d0c:	84be                	mv	s1,a5
    80004d0e:	2781                	sext.w	a5,a5
    80004d10:	f8fb5ce3          	bge	s6,a5,80004ca8 <filewrite+0x84>
    80004d14:	84de                	mv	s1,s7
    80004d16:	bf49                	j	80004ca8 <filewrite+0x84>
    int i = 0;
    80004d18:	4981                	li	s3,0
    }
    ret = (i == n ? n : -1);
    80004d1a:	013a1f63          	bne	s4,s3,80004d38 <filewrite+0x114>
  } else {
    panic("filewrite");
  }

  return ret;
}
    80004d1e:	8552                	mv	a0,s4
    80004d20:	60a6                	ld	ra,72(sp)
    80004d22:	6406                	ld	s0,64(sp)
    80004d24:	74e2                	ld	s1,56(sp)
    80004d26:	7942                	ld	s2,48(sp)
    80004d28:	79a2                	ld	s3,40(sp)
    80004d2a:	7a02                	ld	s4,32(sp)
    80004d2c:	6ae2                	ld	s5,24(sp)
    80004d2e:	6b42                	ld	s6,16(sp)
    80004d30:	6ba2                	ld	s7,8(sp)
    80004d32:	6c02                	ld	s8,0(sp)
    80004d34:	6161                	addi	sp,sp,80
    80004d36:	8082                	ret
    ret = (i == n ? n : -1);
    80004d38:	5a7d                	li	s4,-1
    80004d3a:	b7d5                	j	80004d1e <filewrite+0xfa>
    panic("filewrite");
    80004d3c:	00004517          	auipc	a0,0x4
    80004d40:	a4450513          	addi	a0,a0,-1468 # 80008780 <syscalls+0x280>
    80004d44:	ffffb097          	auipc	ra,0xffffb
    80004d48:	7fa080e7          	jalr	2042(ra) # 8000053e <panic>
    return -1;
    80004d4c:	5a7d                	li	s4,-1
    80004d4e:	bfc1                	j	80004d1e <filewrite+0xfa>
      return -1;
    80004d50:	5a7d                	li	s4,-1
    80004d52:	b7f1                	j	80004d1e <filewrite+0xfa>
    80004d54:	5a7d                	li	s4,-1
    80004d56:	b7e1                	j	80004d1e <filewrite+0xfa>

0000000080004d58 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
    80004d58:	7179                	addi	sp,sp,-48
    80004d5a:	f406                	sd	ra,40(sp)
    80004d5c:	f022                	sd	s0,32(sp)
    80004d5e:	ec26                	sd	s1,24(sp)
    80004d60:	e84a                	sd	s2,16(sp)
    80004d62:	e44e                	sd	s3,8(sp)
    80004d64:	e052                	sd	s4,0(sp)
    80004d66:	1800                	addi	s0,sp,48
    80004d68:	84aa                	mv	s1,a0
    80004d6a:	8a2e                	mv	s4,a1
  struct pipe *pi;

  pi = 0;
  *f0 = *f1 = 0;
    80004d6c:	0005b023          	sd	zero,0(a1)
    80004d70:	00053023          	sd	zero,0(a0)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    80004d74:	00000097          	auipc	ra,0x0
    80004d78:	bf8080e7          	jalr	-1032(ra) # 8000496c <filealloc>
    80004d7c:	e088                	sd	a0,0(s1)
    80004d7e:	c551                	beqz	a0,80004e0a <pipealloc+0xb2>
    80004d80:	00000097          	auipc	ra,0x0
    80004d84:	bec080e7          	jalr	-1044(ra) # 8000496c <filealloc>
    80004d88:	00aa3023          	sd	a0,0(s4)
    80004d8c:	c92d                	beqz	a0,80004dfe <pipealloc+0xa6>
    goto bad;
  if((pi = (struct pipe*)kalloc()) == 0)
    80004d8e:	ffffc097          	auipc	ra,0xffffc
    80004d92:	d66080e7          	jalr	-666(ra) # 80000af4 <kalloc>
    80004d96:	892a                	mv	s2,a0
    80004d98:	c125                	beqz	a0,80004df8 <pipealloc+0xa0>
    goto bad;
  pi->readopen = 1;
    80004d9a:	4985                	li	s3,1
    80004d9c:	23352023          	sw	s3,544(a0)
  pi->writeopen = 1;
    80004da0:	23352223          	sw	s3,548(a0)
  pi->nwrite = 0;
    80004da4:	20052e23          	sw	zero,540(a0)
  pi->nread = 0;
    80004da8:	20052c23          	sw	zero,536(a0)
  initlock(&pi->lock, "pipe");
    80004dac:	00004597          	auipc	a1,0x4
    80004db0:	9e458593          	addi	a1,a1,-1564 # 80008790 <syscalls+0x290>
    80004db4:	ffffc097          	auipc	ra,0xffffc
    80004db8:	da0080e7          	jalr	-608(ra) # 80000b54 <initlock>
  (*f0)->type = FD_PIPE;
    80004dbc:	609c                	ld	a5,0(s1)
    80004dbe:	0137a023          	sw	s3,0(a5)
  (*f0)->readable = 1;
    80004dc2:	609c                	ld	a5,0(s1)
    80004dc4:	01378423          	sb	s3,8(a5)
  (*f0)->writable = 0;
    80004dc8:	609c                	ld	a5,0(s1)
    80004dca:	000784a3          	sb	zero,9(a5)
  (*f0)->pipe = pi;
    80004dce:	609c                	ld	a5,0(s1)
    80004dd0:	0127b823          	sd	s2,16(a5)
  (*f1)->type = FD_PIPE;
    80004dd4:	000a3783          	ld	a5,0(s4)
    80004dd8:	0137a023          	sw	s3,0(a5)
  (*f1)->readable = 0;
    80004ddc:	000a3783          	ld	a5,0(s4)
    80004de0:	00078423          	sb	zero,8(a5)
  (*f1)->writable = 1;
    80004de4:	000a3783          	ld	a5,0(s4)
    80004de8:	013784a3          	sb	s3,9(a5)
  (*f1)->pipe = pi;
    80004dec:	000a3783          	ld	a5,0(s4)
    80004df0:	0127b823          	sd	s2,16(a5)
  return 0;
    80004df4:	4501                	li	a0,0
    80004df6:	a025                	j	80004e1e <pipealloc+0xc6>

 bad:
  if(pi)
    kfree((char*)pi);
  if(*f0)
    80004df8:	6088                	ld	a0,0(s1)
    80004dfa:	e501                	bnez	a0,80004e02 <pipealloc+0xaa>
    80004dfc:	a039                	j	80004e0a <pipealloc+0xb2>
    80004dfe:	6088                	ld	a0,0(s1)
    80004e00:	c51d                	beqz	a0,80004e2e <pipealloc+0xd6>
    fileclose(*f0);
    80004e02:	00000097          	auipc	ra,0x0
    80004e06:	c26080e7          	jalr	-986(ra) # 80004a28 <fileclose>
  if(*f1)
    80004e0a:	000a3783          	ld	a5,0(s4)
    fileclose(*f1);
  return -1;
    80004e0e:	557d                	li	a0,-1
  if(*f1)
    80004e10:	c799                	beqz	a5,80004e1e <pipealloc+0xc6>
    fileclose(*f1);
    80004e12:	853e                	mv	a0,a5
    80004e14:	00000097          	auipc	ra,0x0
    80004e18:	c14080e7          	jalr	-1004(ra) # 80004a28 <fileclose>
  return -1;
    80004e1c:	557d                	li	a0,-1
}
    80004e1e:	70a2                	ld	ra,40(sp)
    80004e20:	7402                	ld	s0,32(sp)
    80004e22:	64e2                	ld	s1,24(sp)
    80004e24:	6942                	ld	s2,16(sp)
    80004e26:	69a2                	ld	s3,8(sp)
    80004e28:	6a02                	ld	s4,0(sp)
    80004e2a:	6145                	addi	sp,sp,48
    80004e2c:	8082                	ret
  return -1;
    80004e2e:	557d                	li	a0,-1
    80004e30:	b7fd                	j	80004e1e <pipealloc+0xc6>

0000000080004e32 <pipeclose>:

void
pipeclose(struct pipe *pi, int writable)
{
    80004e32:	1101                	addi	sp,sp,-32
    80004e34:	ec06                	sd	ra,24(sp)
    80004e36:	e822                	sd	s0,16(sp)
    80004e38:	e426                	sd	s1,8(sp)
    80004e3a:	e04a                	sd	s2,0(sp)
    80004e3c:	1000                	addi	s0,sp,32
    80004e3e:	84aa                	mv	s1,a0
    80004e40:	892e                	mv	s2,a1
  acquire(&pi->lock);
    80004e42:	ffffc097          	auipc	ra,0xffffc
    80004e46:	da2080e7          	jalr	-606(ra) # 80000be4 <acquire>
  if(writable){
    80004e4a:	02090d63          	beqz	s2,80004e84 <pipeclose+0x52>
    pi->writeopen = 0;
    80004e4e:	2204a223          	sw	zero,548(s1)
    wakeup(&pi->nread);
    80004e52:	21848513          	addi	a0,s1,536
    80004e56:	ffffd097          	auipc	ra,0xffffd
    80004e5a:	6f0080e7          	jalr	1776(ra) # 80002546 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    80004e5e:	2204b783          	ld	a5,544(s1)
    80004e62:	eb95                	bnez	a5,80004e96 <pipeclose+0x64>
    release(&pi->lock);
    80004e64:	8526                	mv	a0,s1
    80004e66:	ffffc097          	auipc	ra,0xffffc
    80004e6a:	e32080e7          	jalr	-462(ra) # 80000c98 <release>
    kfree((char*)pi);
    80004e6e:	8526                	mv	a0,s1
    80004e70:	ffffc097          	auipc	ra,0xffffc
    80004e74:	b88080e7          	jalr	-1144(ra) # 800009f8 <kfree>
  } else
    release(&pi->lock);
}
    80004e78:	60e2                	ld	ra,24(sp)
    80004e7a:	6442                	ld	s0,16(sp)
    80004e7c:	64a2                	ld	s1,8(sp)
    80004e7e:	6902                	ld	s2,0(sp)
    80004e80:	6105                	addi	sp,sp,32
    80004e82:	8082                	ret
    pi->readopen = 0;
    80004e84:	2204a023          	sw	zero,544(s1)
    wakeup(&pi->nwrite);
    80004e88:	21c48513          	addi	a0,s1,540
    80004e8c:	ffffd097          	auipc	ra,0xffffd
    80004e90:	6ba080e7          	jalr	1722(ra) # 80002546 <wakeup>
    80004e94:	b7e9                	j	80004e5e <pipeclose+0x2c>
    release(&pi->lock);
    80004e96:	8526                	mv	a0,s1
    80004e98:	ffffc097          	auipc	ra,0xffffc
    80004e9c:	e00080e7          	jalr	-512(ra) # 80000c98 <release>
}
    80004ea0:	bfe1                	j	80004e78 <pipeclose+0x46>

0000000080004ea2 <pipewrite>:

int
pipewrite(struct pipe *pi, uint64 addr, int n)
{
    80004ea2:	7159                	addi	sp,sp,-112
    80004ea4:	f486                	sd	ra,104(sp)
    80004ea6:	f0a2                	sd	s0,96(sp)
    80004ea8:	eca6                	sd	s1,88(sp)
    80004eaa:	e8ca                	sd	s2,80(sp)
    80004eac:	e4ce                	sd	s3,72(sp)
    80004eae:	e0d2                	sd	s4,64(sp)
    80004eb0:	fc56                	sd	s5,56(sp)
    80004eb2:	f85a                	sd	s6,48(sp)
    80004eb4:	f45e                	sd	s7,40(sp)
    80004eb6:	f062                	sd	s8,32(sp)
    80004eb8:	ec66                	sd	s9,24(sp)
    80004eba:	1880                	addi	s0,sp,112
    80004ebc:	84aa                	mv	s1,a0
    80004ebe:	8aae                	mv	s5,a1
    80004ec0:	8a32                	mv	s4,a2
  int i = 0;
  struct proc *pr = myproc();
    80004ec2:	ffffd097          	auipc	ra,0xffffd
    80004ec6:	b1c080e7          	jalr	-1252(ra) # 800019de <myproc>
    80004eca:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    80004ecc:	8526                	mv	a0,s1
    80004ece:	ffffc097          	auipc	ra,0xffffc
    80004ed2:	d16080e7          	jalr	-746(ra) # 80000be4 <acquire>
  while(i < n){
    80004ed6:	0d405163          	blez	s4,80004f98 <pipewrite+0xf6>
    80004eda:	8ba6                	mv	s7,s1
  int i = 0;
    80004edc:	4901                	li	s2,0
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
      wakeup(&pi->nread);
      sleep(&pi->nwrite, &pi->lock);
    } else {
      char ch;
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004ede:	5b7d                	li	s6,-1
      wakeup(&pi->nread);
    80004ee0:	21848c93          	addi	s9,s1,536
      sleep(&pi->nwrite, &pi->lock);
    80004ee4:	21c48c13          	addi	s8,s1,540
    80004ee8:	a08d                	j	80004f4a <pipewrite+0xa8>
      release(&pi->lock);
    80004eea:	8526                	mv	a0,s1
    80004eec:	ffffc097          	auipc	ra,0xffffc
    80004ef0:	dac080e7          	jalr	-596(ra) # 80000c98 <release>
      return -1;
    80004ef4:	597d                	li	s2,-1
  }
  wakeup(&pi->nread);
  release(&pi->lock);

  return i;
}
    80004ef6:	854a                	mv	a0,s2
    80004ef8:	70a6                	ld	ra,104(sp)
    80004efa:	7406                	ld	s0,96(sp)
    80004efc:	64e6                	ld	s1,88(sp)
    80004efe:	6946                	ld	s2,80(sp)
    80004f00:	69a6                	ld	s3,72(sp)
    80004f02:	6a06                	ld	s4,64(sp)
    80004f04:	7ae2                	ld	s5,56(sp)
    80004f06:	7b42                	ld	s6,48(sp)
    80004f08:	7ba2                	ld	s7,40(sp)
    80004f0a:	7c02                	ld	s8,32(sp)
    80004f0c:	6ce2                	ld	s9,24(sp)
    80004f0e:	6165                	addi	sp,sp,112
    80004f10:	8082                	ret
      wakeup(&pi->nread);
    80004f12:	8566                	mv	a0,s9
    80004f14:	ffffd097          	auipc	ra,0xffffd
    80004f18:	632080e7          	jalr	1586(ra) # 80002546 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    80004f1c:	85de                	mv	a1,s7
    80004f1e:	8562                	mv	a0,s8
    80004f20:	ffffd097          	auipc	ra,0xffffd
    80004f24:	490080e7          	jalr	1168(ra) # 800023b0 <sleep>
    80004f28:	a839                	j	80004f46 <pipewrite+0xa4>
      pi->data[pi->nwrite++ % PIPESIZE] = ch;
    80004f2a:	21c4a783          	lw	a5,540(s1)
    80004f2e:	0017871b          	addiw	a4,a5,1
    80004f32:	20e4ae23          	sw	a4,540(s1)
    80004f36:	1ff7f793          	andi	a5,a5,511
    80004f3a:	97a6                	add	a5,a5,s1
    80004f3c:	f9f44703          	lbu	a4,-97(s0)
    80004f40:	00e78c23          	sb	a4,24(a5)
      i++;
    80004f44:	2905                	addiw	s2,s2,1
  while(i < n){
    80004f46:	03495d63          	bge	s2,s4,80004f80 <pipewrite+0xde>
    if(pi->readopen == 0 || pr->killed){
    80004f4a:	2204a783          	lw	a5,544(s1)
    80004f4e:	dfd1                	beqz	a5,80004eea <pipewrite+0x48>
    80004f50:	0289a783          	lw	a5,40(s3)
    80004f54:	fbd9                	bnez	a5,80004eea <pipewrite+0x48>
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
    80004f56:	2184a783          	lw	a5,536(s1)
    80004f5a:	21c4a703          	lw	a4,540(s1)
    80004f5e:	2007879b          	addiw	a5,a5,512
    80004f62:	faf708e3          	beq	a4,a5,80004f12 <pipewrite+0x70>
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004f66:	4685                	li	a3,1
    80004f68:	01590633          	add	a2,s2,s5
    80004f6c:	f9f40593          	addi	a1,s0,-97
    80004f70:	0709b503          	ld	a0,112(s3)
    80004f74:	ffffc097          	auipc	ra,0xffffc
    80004f78:	792080e7          	jalr	1938(ra) # 80001706 <copyin>
    80004f7c:	fb6517e3          	bne	a0,s6,80004f2a <pipewrite+0x88>
  wakeup(&pi->nread);
    80004f80:	21848513          	addi	a0,s1,536
    80004f84:	ffffd097          	auipc	ra,0xffffd
    80004f88:	5c2080e7          	jalr	1474(ra) # 80002546 <wakeup>
  release(&pi->lock);
    80004f8c:	8526                	mv	a0,s1
    80004f8e:	ffffc097          	auipc	ra,0xffffc
    80004f92:	d0a080e7          	jalr	-758(ra) # 80000c98 <release>
  return i;
    80004f96:	b785                	j	80004ef6 <pipewrite+0x54>
  int i = 0;
    80004f98:	4901                	li	s2,0
    80004f9a:	b7dd                	j	80004f80 <pipewrite+0xde>

0000000080004f9c <piperead>:

int
piperead(struct pipe *pi, uint64 addr, int n)
{
    80004f9c:	715d                	addi	sp,sp,-80
    80004f9e:	e486                	sd	ra,72(sp)
    80004fa0:	e0a2                	sd	s0,64(sp)
    80004fa2:	fc26                	sd	s1,56(sp)
    80004fa4:	f84a                	sd	s2,48(sp)
    80004fa6:	f44e                	sd	s3,40(sp)
    80004fa8:	f052                	sd	s4,32(sp)
    80004faa:	ec56                	sd	s5,24(sp)
    80004fac:	e85a                	sd	s6,16(sp)
    80004fae:	0880                	addi	s0,sp,80
    80004fb0:	84aa                	mv	s1,a0
    80004fb2:	892e                	mv	s2,a1
    80004fb4:	8ab2                	mv	s5,a2
  int i;
  struct proc *pr = myproc();
    80004fb6:	ffffd097          	auipc	ra,0xffffd
    80004fba:	a28080e7          	jalr	-1496(ra) # 800019de <myproc>
    80004fbe:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004fc0:	8b26                	mv	s6,s1
    80004fc2:	8526                	mv	a0,s1
    80004fc4:	ffffc097          	auipc	ra,0xffffc
    80004fc8:	c20080e7          	jalr	-992(ra) # 80000be4 <acquire>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004fcc:	2184a703          	lw	a4,536(s1)
    80004fd0:	21c4a783          	lw	a5,540(s1)
    if(pr->killed){
      release(&pi->lock);
      return -1;
    }
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004fd4:	21848993          	addi	s3,s1,536
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004fd8:	02f71463          	bne	a4,a5,80005000 <piperead+0x64>
    80004fdc:	2244a783          	lw	a5,548(s1)
    80004fe0:	c385                	beqz	a5,80005000 <piperead+0x64>
    if(pr->killed){
    80004fe2:	028a2783          	lw	a5,40(s4)
    80004fe6:	ebc1                	bnez	a5,80005076 <piperead+0xda>
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004fe8:	85da                	mv	a1,s6
    80004fea:	854e                	mv	a0,s3
    80004fec:	ffffd097          	auipc	ra,0xffffd
    80004ff0:	3c4080e7          	jalr	964(ra) # 800023b0 <sleep>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004ff4:	2184a703          	lw	a4,536(s1)
    80004ff8:	21c4a783          	lw	a5,540(s1)
    80004ffc:	fef700e3          	beq	a4,a5,80004fdc <piperead+0x40>
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80005000:	09505263          	blez	s5,80005084 <piperead+0xe8>
    80005004:	4981                	li	s3,0
    if(pi->nread == pi->nwrite)
      break;
    ch = pi->data[pi->nread++ % PIPESIZE];
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    80005006:	5b7d                	li	s6,-1
    if(pi->nread == pi->nwrite)
    80005008:	2184a783          	lw	a5,536(s1)
    8000500c:	21c4a703          	lw	a4,540(s1)
    80005010:	02f70d63          	beq	a4,a5,8000504a <piperead+0xae>
    ch = pi->data[pi->nread++ % PIPESIZE];
    80005014:	0017871b          	addiw	a4,a5,1
    80005018:	20e4ac23          	sw	a4,536(s1)
    8000501c:	1ff7f793          	andi	a5,a5,511
    80005020:	97a6                	add	a5,a5,s1
    80005022:	0187c783          	lbu	a5,24(a5)
    80005026:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    8000502a:	4685                	li	a3,1
    8000502c:	fbf40613          	addi	a2,s0,-65
    80005030:	85ca                	mv	a1,s2
    80005032:	070a3503          	ld	a0,112(s4)
    80005036:	ffffc097          	auipc	ra,0xffffc
    8000503a:	644080e7          	jalr	1604(ra) # 8000167a <copyout>
    8000503e:	01650663          	beq	a0,s6,8000504a <piperead+0xae>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80005042:	2985                	addiw	s3,s3,1
    80005044:	0905                	addi	s2,s2,1
    80005046:	fd3a91e3          	bne	s5,s3,80005008 <piperead+0x6c>
      break;
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    8000504a:	21c48513          	addi	a0,s1,540
    8000504e:	ffffd097          	auipc	ra,0xffffd
    80005052:	4f8080e7          	jalr	1272(ra) # 80002546 <wakeup>
  release(&pi->lock);
    80005056:	8526                	mv	a0,s1
    80005058:	ffffc097          	auipc	ra,0xffffc
    8000505c:	c40080e7          	jalr	-960(ra) # 80000c98 <release>
  return i;
}
    80005060:	854e                	mv	a0,s3
    80005062:	60a6                	ld	ra,72(sp)
    80005064:	6406                	ld	s0,64(sp)
    80005066:	74e2                	ld	s1,56(sp)
    80005068:	7942                	ld	s2,48(sp)
    8000506a:	79a2                	ld	s3,40(sp)
    8000506c:	7a02                	ld	s4,32(sp)
    8000506e:	6ae2                	ld	s5,24(sp)
    80005070:	6b42                	ld	s6,16(sp)
    80005072:	6161                	addi	sp,sp,80
    80005074:	8082                	ret
      release(&pi->lock);
    80005076:	8526                	mv	a0,s1
    80005078:	ffffc097          	auipc	ra,0xffffc
    8000507c:	c20080e7          	jalr	-992(ra) # 80000c98 <release>
      return -1;
    80005080:	59fd                	li	s3,-1
    80005082:	bff9                	j	80005060 <piperead+0xc4>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80005084:	4981                	li	s3,0
    80005086:	b7d1                	j	8000504a <piperead+0xae>

0000000080005088 <exec>:

static int loadseg(pde_t *pgdir, uint64 addr, struct inode *ip, uint offset, uint sz);

int
exec(char *path, char **argv)
{
    80005088:	df010113          	addi	sp,sp,-528
    8000508c:	20113423          	sd	ra,520(sp)
    80005090:	20813023          	sd	s0,512(sp)
    80005094:	ffa6                	sd	s1,504(sp)
    80005096:	fbca                	sd	s2,496(sp)
    80005098:	f7ce                	sd	s3,488(sp)
    8000509a:	f3d2                	sd	s4,480(sp)
    8000509c:	efd6                	sd	s5,472(sp)
    8000509e:	ebda                	sd	s6,464(sp)
    800050a0:	e7de                	sd	s7,456(sp)
    800050a2:	e3e2                	sd	s8,448(sp)
    800050a4:	ff66                	sd	s9,440(sp)
    800050a6:	fb6a                	sd	s10,432(sp)
    800050a8:	f76e                	sd	s11,424(sp)
    800050aa:	0c00                	addi	s0,sp,528
    800050ac:	84aa                	mv	s1,a0
    800050ae:	dea43c23          	sd	a0,-520(s0)
    800050b2:	e0b43023          	sd	a1,-512(s0)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  struct proc *p = myproc();
    800050b6:	ffffd097          	auipc	ra,0xffffd
    800050ba:	928080e7          	jalr	-1752(ra) # 800019de <myproc>
    800050be:	892a                	mv	s2,a0

  begin_op();
    800050c0:	fffff097          	auipc	ra,0xfffff
    800050c4:	49c080e7          	jalr	1180(ra) # 8000455c <begin_op>

  if((ip = namei(path)) == 0){
    800050c8:	8526                	mv	a0,s1
    800050ca:	fffff097          	auipc	ra,0xfffff
    800050ce:	276080e7          	jalr	630(ra) # 80004340 <namei>
    800050d2:	c92d                	beqz	a0,80005144 <exec+0xbc>
    800050d4:	84aa                	mv	s1,a0
    end_op();
    return -1;
  }
  ilock(ip);
    800050d6:	fffff097          	auipc	ra,0xfffff
    800050da:	ab4080e7          	jalr	-1356(ra) # 80003b8a <ilock>

  // Check ELF header
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    800050de:	04000713          	li	a4,64
    800050e2:	4681                	li	a3,0
    800050e4:	e5040613          	addi	a2,s0,-432
    800050e8:	4581                	li	a1,0
    800050ea:	8526                	mv	a0,s1
    800050ec:	fffff097          	auipc	ra,0xfffff
    800050f0:	d52080e7          	jalr	-686(ra) # 80003e3e <readi>
    800050f4:	04000793          	li	a5,64
    800050f8:	00f51a63          	bne	a0,a5,8000510c <exec+0x84>
    goto bad;
  if(elf.magic != ELF_MAGIC)
    800050fc:	e5042703          	lw	a4,-432(s0)
    80005100:	464c47b7          	lui	a5,0x464c4
    80005104:	57f78793          	addi	a5,a5,1407 # 464c457f <_entry-0x39b3ba81>
    80005108:	04f70463          	beq	a4,a5,80005150 <exec+0xc8>

 bad:
  if(pagetable)
    proc_freepagetable(pagetable, sz);
  if(ip){
    iunlockput(ip);
    8000510c:	8526                	mv	a0,s1
    8000510e:	fffff097          	auipc	ra,0xfffff
    80005112:	cde080e7          	jalr	-802(ra) # 80003dec <iunlockput>
    end_op();
    80005116:	fffff097          	auipc	ra,0xfffff
    8000511a:	4c6080e7          	jalr	1222(ra) # 800045dc <end_op>
  }
  return -1;
    8000511e:	557d                	li	a0,-1
}
    80005120:	20813083          	ld	ra,520(sp)
    80005124:	20013403          	ld	s0,512(sp)
    80005128:	74fe                	ld	s1,504(sp)
    8000512a:	795e                	ld	s2,496(sp)
    8000512c:	79be                	ld	s3,488(sp)
    8000512e:	7a1e                	ld	s4,480(sp)
    80005130:	6afe                	ld	s5,472(sp)
    80005132:	6b5e                	ld	s6,464(sp)
    80005134:	6bbe                	ld	s7,456(sp)
    80005136:	6c1e                	ld	s8,448(sp)
    80005138:	7cfa                	ld	s9,440(sp)
    8000513a:	7d5a                	ld	s10,432(sp)
    8000513c:	7dba                	ld	s11,424(sp)
    8000513e:	21010113          	addi	sp,sp,528
    80005142:	8082                	ret
    end_op();
    80005144:	fffff097          	auipc	ra,0xfffff
    80005148:	498080e7          	jalr	1176(ra) # 800045dc <end_op>
    return -1;
    8000514c:	557d                	li	a0,-1
    8000514e:	bfc9                	j	80005120 <exec+0x98>
  if((pagetable = proc_pagetable(p)) == 0)
    80005150:	854a                	mv	a0,s2
    80005152:	ffffd097          	auipc	ra,0xffffd
    80005156:	950080e7          	jalr	-1712(ra) # 80001aa2 <proc_pagetable>
    8000515a:	8baa                	mv	s7,a0
    8000515c:	d945                	beqz	a0,8000510c <exec+0x84>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    8000515e:	e7042983          	lw	s3,-400(s0)
    80005162:	e8845783          	lhu	a5,-376(s0)
    80005166:	c7ad                	beqz	a5,800051d0 <exec+0x148>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    80005168:	4901                	li	s2,0
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    8000516a:	4b01                	li	s6,0
    if((ph.vaddr % PGSIZE) != 0)
    8000516c:	6c85                	lui	s9,0x1
    8000516e:	fffc8793          	addi	a5,s9,-1 # fff <_entry-0x7ffff001>
    80005172:	def43823          	sd	a5,-528(s0)
    80005176:	a42d                	j	800053a0 <exec+0x318>
  uint64 pa;

  for(i = 0; i < sz; i += PGSIZE){
    pa = walkaddr(pagetable, va + i);
    if(pa == 0)
      panic("loadseg: address should exist");
    80005178:	00003517          	auipc	a0,0x3
    8000517c:	62050513          	addi	a0,a0,1568 # 80008798 <syscalls+0x298>
    80005180:	ffffb097          	auipc	ra,0xffffb
    80005184:	3be080e7          	jalr	958(ra) # 8000053e <panic>
    if(sz - i < PGSIZE)
      n = sz - i;
    else
      n = PGSIZE;
    if(readi(ip, 0, (uint64)pa, offset+i, n) != n)
    80005188:	8756                	mv	a4,s5
    8000518a:	012d86bb          	addw	a3,s11,s2
    8000518e:	4581                	li	a1,0
    80005190:	8526                	mv	a0,s1
    80005192:	fffff097          	auipc	ra,0xfffff
    80005196:	cac080e7          	jalr	-852(ra) # 80003e3e <readi>
    8000519a:	2501                	sext.w	a0,a0
    8000519c:	1aaa9963          	bne	s5,a0,8000534e <exec+0x2c6>
  for(i = 0; i < sz; i += PGSIZE){
    800051a0:	6785                	lui	a5,0x1
    800051a2:	0127893b          	addw	s2,a5,s2
    800051a6:	77fd                	lui	a5,0xfffff
    800051a8:	01478a3b          	addw	s4,a5,s4
    800051ac:	1f897163          	bgeu	s2,s8,8000538e <exec+0x306>
    pa = walkaddr(pagetable, va + i);
    800051b0:	02091593          	slli	a1,s2,0x20
    800051b4:	9181                	srli	a1,a1,0x20
    800051b6:	95ea                	add	a1,a1,s10
    800051b8:	855e                	mv	a0,s7
    800051ba:	ffffc097          	auipc	ra,0xffffc
    800051be:	ebc080e7          	jalr	-324(ra) # 80001076 <walkaddr>
    800051c2:	862a                	mv	a2,a0
    if(pa == 0)
    800051c4:	d955                	beqz	a0,80005178 <exec+0xf0>
      n = PGSIZE;
    800051c6:	8ae6                	mv	s5,s9
    if(sz - i < PGSIZE)
    800051c8:	fd9a70e3          	bgeu	s4,s9,80005188 <exec+0x100>
      n = sz - i;
    800051cc:	8ad2                	mv	s5,s4
    800051ce:	bf6d                	j	80005188 <exec+0x100>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    800051d0:	4901                	li	s2,0
  iunlockput(ip);
    800051d2:	8526                	mv	a0,s1
    800051d4:	fffff097          	auipc	ra,0xfffff
    800051d8:	c18080e7          	jalr	-1000(ra) # 80003dec <iunlockput>
  end_op();
    800051dc:	fffff097          	auipc	ra,0xfffff
    800051e0:	400080e7          	jalr	1024(ra) # 800045dc <end_op>
  p = myproc();
    800051e4:	ffffc097          	auipc	ra,0xffffc
    800051e8:	7fa080e7          	jalr	2042(ra) # 800019de <myproc>
    800051ec:	8aaa                	mv	s5,a0
  uint64 oldsz = p->sz;
    800051ee:	06853d03          	ld	s10,104(a0)
  sz = PGROUNDUP(sz);
    800051f2:	6785                	lui	a5,0x1
    800051f4:	17fd                	addi	a5,a5,-1
    800051f6:	993e                	add	s2,s2,a5
    800051f8:	757d                	lui	a0,0xfffff
    800051fa:	00a977b3          	and	a5,s2,a0
    800051fe:	e0f43423          	sd	a5,-504(s0)
  if((sz1 = uvmalloc(pagetable, sz, sz + 2*PGSIZE)) == 0)
    80005202:	6609                	lui	a2,0x2
    80005204:	963e                	add	a2,a2,a5
    80005206:	85be                	mv	a1,a5
    80005208:	855e                	mv	a0,s7
    8000520a:	ffffc097          	auipc	ra,0xffffc
    8000520e:	220080e7          	jalr	544(ra) # 8000142a <uvmalloc>
    80005212:	8b2a                	mv	s6,a0
  ip = 0;
    80005214:	4481                	li	s1,0
  if((sz1 = uvmalloc(pagetable, sz, sz + 2*PGSIZE)) == 0)
    80005216:	12050c63          	beqz	a0,8000534e <exec+0x2c6>
  uvmclear(pagetable, sz-2*PGSIZE);
    8000521a:	75f9                	lui	a1,0xffffe
    8000521c:	95aa                	add	a1,a1,a0
    8000521e:	855e                	mv	a0,s7
    80005220:	ffffc097          	auipc	ra,0xffffc
    80005224:	428080e7          	jalr	1064(ra) # 80001648 <uvmclear>
  stackbase = sp - PGSIZE;
    80005228:	7c7d                	lui	s8,0xfffff
    8000522a:	9c5a                	add	s8,s8,s6
  for(argc = 0; argv[argc]; argc++) {
    8000522c:	e0043783          	ld	a5,-512(s0)
    80005230:	6388                	ld	a0,0(a5)
    80005232:	c535                	beqz	a0,8000529e <exec+0x216>
    80005234:	e9040993          	addi	s3,s0,-368
    80005238:	f9040c93          	addi	s9,s0,-112
  sp = sz;
    8000523c:	895a                	mv	s2,s6
    sp -= strlen(argv[argc]) + 1;
    8000523e:	ffffc097          	auipc	ra,0xffffc
    80005242:	c26080e7          	jalr	-986(ra) # 80000e64 <strlen>
    80005246:	2505                	addiw	a0,a0,1
    80005248:	40a90933          	sub	s2,s2,a0
    sp -= sp % 16; // riscv sp must be 16-byte aligned
    8000524c:	ff097913          	andi	s2,s2,-16
    if(sp < stackbase)
    80005250:	13896363          	bltu	s2,s8,80005376 <exec+0x2ee>
    if(copyout(pagetable, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
    80005254:	e0043d83          	ld	s11,-512(s0)
    80005258:	000dba03          	ld	s4,0(s11)
    8000525c:	8552                	mv	a0,s4
    8000525e:	ffffc097          	auipc	ra,0xffffc
    80005262:	c06080e7          	jalr	-1018(ra) # 80000e64 <strlen>
    80005266:	0015069b          	addiw	a3,a0,1
    8000526a:	8652                	mv	a2,s4
    8000526c:	85ca                	mv	a1,s2
    8000526e:	855e                	mv	a0,s7
    80005270:	ffffc097          	auipc	ra,0xffffc
    80005274:	40a080e7          	jalr	1034(ra) # 8000167a <copyout>
    80005278:	10054363          	bltz	a0,8000537e <exec+0x2f6>
    ustack[argc] = sp;
    8000527c:	0129b023          	sd	s2,0(s3)
  for(argc = 0; argv[argc]; argc++) {
    80005280:	0485                	addi	s1,s1,1
    80005282:	008d8793          	addi	a5,s11,8
    80005286:	e0f43023          	sd	a5,-512(s0)
    8000528a:	008db503          	ld	a0,8(s11)
    8000528e:	c911                	beqz	a0,800052a2 <exec+0x21a>
    if(argc >= MAXARG)
    80005290:	09a1                	addi	s3,s3,8
    80005292:	fb3c96e3          	bne	s9,s3,8000523e <exec+0x1b6>
  sz = sz1;
    80005296:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000529a:	4481                	li	s1,0
    8000529c:	a84d                	j	8000534e <exec+0x2c6>
  sp = sz;
    8000529e:	895a                	mv	s2,s6
  for(argc = 0; argv[argc]; argc++) {
    800052a0:	4481                	li	s1,0
  ustack[argc] = 0;
    800052a2:	00349793          	slli	a5,s1,0x3
    800052a6:	f9040713          	addi	a4,s0,-112
    800052aa:	97ba                	add	a5,a5,a4
    800052ac:	f007b023          	sd	zero,-256(a5) # f00 <_entry-0x7ffff100>
  sp -= (argc+1) * sizeof(uint64);
    800052b0:	00148693          	addi	a3,s1,1
    800052b4:	068e                	slli	a3,a3,0x3
    800052b6:	40d90933          	sub	s2,s2,a3
  sp -= sp % 16;
    800052ba:	ff097913          	andi	s2,s2,-16
  if(sp < stackbase)
    800052be:	01897663          	bgeu	s2,s8,800052ca <exec+0x242>
  sz = sz1;
    800052c2:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    800052c6:	4481                	li	s1,0
    800052c8:	a059                	j	8000534e <exec+0x2c6>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    800052ca:	e9040613          	addi	a2,s0,-368
    800052ce:	85ca                	mv	a1,s2
    800052d0:	855e                	mv	a0,s7
    800052d2:	ffffc097          	auipc	ra,0xffffc
    800052d6:	3a8080e7          	jalr	936(ra) # 8000167a <copyout>
    800052da:	0a054663          	bltz	a0,80005386 <exec+0x2fe>
  p->trapframe->a1 = sp;
    800052de:	078ab783          	ld	a5,120(s5)
    800052e2:	0727bc23          	sd	s2,120(a5)
  for(last=s=path; *s; s++)
    800052e6:	df843783          	ld	a5,-520(s0)
    800052ea:	0007c703          	lbu	a4,0(a5)
    800052ee:	cf11                	beqz	a4,8000530a <exec+0x282>
    800052f0:	0785                	addi	a5,a5,1
    if(*s == '/')
    800052f2:	02f00693          	li	a3,47
    800052f6:	a039                	j	80005304 <exec+0x27c>
      last = s+1;
    800052f8:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    800052fc:	0785                	addi	a5,a5,1
    800052fe:	fff7c703          	lbu	a4,-1(a5)
    80005302:	c701                	beqz	a4,8000530a <exec+0x282>
    if(*s == '/')
    80005304:	fed71ce3          	bne	a4,a3,800052fc <exec+0x274>
    80005308:	bfc5                	j	800052f8 <exec+0x270>
  safestrcpy(p->name, last, sizeof(p->name));
    8000530a:	4641                	li	a2,16
    8000530c:	df843583          	ld	a1,-520(s0)
    80005310:	178a8513          	addi	a0,s5,376
    80005314:	ffffc097          	auipc	ra,0xffffc
    80005318:	b1e080e7          	jalr	-1250(ra) # 80000e32 <safestrcpy>
  oldpagetable = p->pagetable;
    8000531c:	070ab503          	ld	a0,112(s5)
  p->pagetable = pagetable;
    80005320:	077ab823          	sd	s7,112(s5)
  p->sz = sz;
    80005324:	076ab423          	sd	s6,104(s5)
  p->trapframe->epc = elf.entry;  // initial program counter = main
    80005328:	078ab783          	ld	a5,120(s5)
    8000532c:	e6843703          	ld	a4,-408(s0)
    80005330:	ef98                	sd	a4,24(a5)
  p->trapframe->sp = sp; // initial stack pointer
    80005332:	078ab783          	ld	a5,120(s5)
    80005336:	0327b823          	sd	s2,48(a5)
  proc_freepagetable(oldpagetable, oldsz);
    8000533a:	85ea                	mv	a1,s10
    8000533c:	ffffd097          	auipc	ra,0xffffd
    80005340:	802080e7          	jalr	-2046(ra) # 80001b3e <proc_freepagetable>
  return argc; // this ends up in a0, the first argument to main(argc, argv)
    80005344:	0004851b          	sext.w	a0,s1
    80005348:	bbe1                	j	80005120 <exec+0x98>
    8000534a:	e1243423          	sd	s2,-504(s0)
    proc_freepagetable(pagetable, sz);
    8000534e:	e0843583          	ld	a1,-504(s0)
    80005352:	855e                	mv	a0,s7
    80005354:	ffffc097          	auipc	ra,0xffffc
    80005358:	7ea080e7          	jalr	2026(ra) # 80001b3e <proc_freepagetable>
  if(ip){
    8000535c:	da0498e3          	bnez	s1,8000510c <exec+0x84>
  return -1;
    80005360:	557d                	li	a0,-1
    80005362:	bb7d                	j	80005120 <exec+0x98>
    80005364:	e1243423          	sd	s2,-504(s0)
    80005368:	b7dd                	j	8000534e <exec+0x2c6>
    8000536a:	e1243423          	sd	s2,-504(s0)
    8000536e:	b7c5                	j	8000534e <exec+0x2c6>
    80005370:	e1243423          	sd	s2,-504(s0)
    80005374:	bfe9                	j	8000534e <exec+0x2c6>
  sz = sz1;
    80005376:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000537a:	4481                	li	s1,0
    8000537c:	bfc9                	j	8000534e <exec+0x2c6>
  sz = sz1;
    8000537e:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    80005382:	4481                	li	s1,0
    80005384:	b7e9                	j	8000534e <exec+0x2c6>
  sz = sz1;
    80005386:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000538a:	4481                	li	s1,0
    8000538c:	b7c9                	j	8000534e <exec+0x2c6>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz)) == 0)
    8000538e:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80005392:	2b05                	addiw	s6,s6,1
    80005394:	0389899b          	addiw	s3,s3,56
    80005398:	e8845783          	lhu	a5,-376(s0)
    8000539c:	e2fb5be3          	bge	s6,a5,800051d2 <exec+0x14a>
    if(readi(ip, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
    800053a0:	2981                	sext.w	s3,s3
    800053a2:	03800713          	li	a4,56
    800053a6:	86ce                	mv	a3,s3
    800053a8:	e1840613          	addi	a2,s0,-488
    800053ac:	4581                	li	a1,0
    800053ae:	8526                	mv	a0,s1
    800053b0:	fffff097          	auipc	ra,0xfffff
    800053b4:	a8e080e7          	jalr	-1394(ra) # 80003e3e <readi>
    800053b8:	03800793          	li	a5,56
    800053bc:	f8f517e3          	bne	a0,a5,8000534a <exec+0x2c2>
    if(ph.type != ELF_PROG_LOAD)
    800053c0:	e1842783          	lw	a5,-488(s0)
    800053c4:	4705                	li	a4,1
    800053c6:	fce796e3          	bne	a5,a4,80005392 <exec+0x30a>
    if(ph.memsz < ph.filesz)
    800053ca:	e4043603          	ld	a2,-448(s0)
    800053ce:	e3843783          	ld	a5,-456(s0)
    800053d2:	f8f669e3          	bltu	a2,a5,80005364 <exec+0x2dc>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    800053d6:	e2843783          	ld	a5,-472(s0)
    800053da:	963e                	add	a2,a2,a5
    800053dc:	f8f667e3          	bltu	a2,a5,8000536a <exec+0x2e2>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz)) == 0)
    800053e0:	85ca                	mv	a1,s2
    800053e2:	855e                	mv	a0,s7
    800053e4:	ffffc097          	auipc	ra,0xffffc
    800053e8:	046080e7          	jalr	70(ra) # 8000142a <uvmalloc>
    800053ec:	e0a43423          	sd	a0,-504(s0)
    800053f0:	d141                	beqz	a0,80005370 <exec+0x2e8>
    if((ph.vaddr % PGSIZE) != 0)
    800053f2:	e2843d03          	ld	s10,-472(s0)
    800053f6:	df043783          	ld	a5,-528(s0)
    800053fa:	00fd77b3          	and	a5,s10,a5
    800053fe:	fba1                	bnez	a5,8000534e <exec+0x2c6>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    80005400:	e2042d83          	lw	s11,-480(s0)
    80005404:	e3842c03          	lw	s8,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    80005408:	f80c03e3          	beqz	s8,8000538e <exec+0x306>
    8000540c:	8a62                	mv	s4,s8
    8000540e:	4901                	li	s2,0
    80005410:	b345                	j	800051b0 <exec+0x128>

0000000080005412 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
    80005412:	7179                	addi	sp,sp,-48
    80005414:	f406                	sd	ra,40(sp)
    80005416:	f022                	sd	s0,32(sp)
    80005418:	ec26                	sd	s1,24(sp)
    8000541a:	e84a                	sd	s2,16(sp)
    8000541c:	1800                	addi	s0,sp,48
    8000541e:	892e                	mv	s2,a1
    80005420:	84b2                	mv	s1,a2
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
    80005422:	fdc40593          	addi	a1,s0,-36
    80005426:	ffffe097          	auipc	ra,0xffffe
    8000542a:	b8c080e7          	jalr	-1140(ra) # 80002fb2 <argint>
    8000542e:	04054063          	bltz	a0,8000546e <argfd+0x5c>
    return -1;
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
    80005432:	fdc42703          	lw	a4,-36(s0)
    80005436:	47bd                	li	a5,15
    80005438:	02e7ed63          	bltu	a5,a4,80005472 <argfd+0x60>
    8000543c:	ffffc097          	auipc	ra,0xffffc
    80005440:	5a2080e7          	jalr	1442(ra) # 800019de <myproc>
    80005444:	fdc42703          	lw	a4,-36(s0)
    80005448:	01e70793          	addi	a5,a4,30
    8000544c:	078e                	slli	a5,a5,0x3
    8000544e:	953e                	add	a0,a0,a5
    80005450:	611c                	ld	a5,0(a0)
    80005452:	c395                	beqz	a5,80005476 <argfd+0x64>
    return -1;
  if(pfd)
    80005454:	00090463          	beqz	s2,8000545c <argfd+0x4a>
    *pfd = fd;
    80005458:	00e92023          	sw	a4,0(s2)
  if(pf)
    *pf = f;
  return 0;
    8000545c:	4501                	li	a0,0
  if(pf)
    8000545e:	c091                	beqz	s1,80005462 <argfd+0x50>
    *pf = f;
    80005460:	e09c                	sd	a5,0(s1)
}
    80005462:	70a2                	ld	ra,40(sp)
    80005464:	7402                	ld	s0,32(sp)
    80005466:	64e2                	ld	s1,24(sp)
    80005468:	6942                	ld	s2,16(sp)
    8000546a:	6145                	addi	sp,sp,48
    8000546c:	8082                	ret
    return -1;
    8000546e:	557d                	li	a0,-1
    80005470:	bfcd                	j	80005462 <argfd+0x50>
    return -1;
    80005472:	557d                	li	a0,-1
    80005474:	b7fd                	j	80005462 <argfd+0x50>
    80005476:	557d                	li	a0,-1
    80005478:	b7ed                	j	80005462 <argfd+0x50>

000000008000547a <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
    8000547a:	1101                	addi	sp,sp,-32
    8000547c:	ec06                	sd	ra,24(sp)
    8000547e:	e822                	sd	s0,16(sp)
    80005480:	e426                	sd	s1,8(sp)
    80005482:	1000                	addi	s0,sp,32
    80005484:	84aa                	mv	s1,a0
  int fd;
  struct proc *p = myproc();
    80005486:	ffffc097          	auipc	ra,0xffffc
    8000548a:	558080e7          	jalr	1368(ra) # 800019de <myproc>
    8000548e:	862a                	mv	a2,a0

  for(fd = 0; fd < NOFILE; fd++){
    80005490:	0f050793          	addi	a5,a0,240 # fffffffffffff0f0 <end+0xffffffff7ffd90f0>
    80005494:	4501                	li	a0,0
    80005496:	46c1                	li	a3,16
    if(p->ofile[fd] == 0){
    80005498:	6398                	ld	a4,0(a5)
    8000549a:	cb19                	beqz	a4,800054b0 <fdalloc+0x36>
  for(fd = 0; fd < NOFILE; fd++){
    8000549c:	2505                	addiw	a0,a0,1
    8000549e:	07a1                	addi	a5,a5,8
    800054a0:	fed51ce3          	bne	a0,a3,80005498 <fdalloc+0x1e>
      p->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
    800054a4:	557d                	li	a0,-1
}
    800054a6:	60e2                	ld	ra,24(sp)
    800054a8:	6442                	ld	s0,16(sp)
    800054aa:	64a2                	ld	s1,8(sp)
    800054ac:	6105                	addi	sp,sp,32
    800054ae:	8082                	ret
      p->ofile[fd] = f;
    800054b0:	01e50793          	addi	a5,a0,30
    800054b4:	078e                	slli	a5,a5,0x3
    800054b6:	963e                	add	a2,a2,a5
    800054b8:	e204                	sd	s1,0(a2)
      return fd;
    800054ba:	b7f5                	j	800054a6 <fdalloc+0x2c>

00000000800054bc <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
    800054bc:	715d                	addi	sp,sp,-80
    800054be:	e486                	sd	ra,72(sp)
    800054c0:	e0a2                	sd	s0,64(sp)
    800054c2:	fc26                	sd	s1,56(sp)
    800054c4:	f84a                	sd	s2,48(sp)
    800054c6:	f44e                	sd	s3,40(sp)
    800054c8:	f052                	sd	s4,32(sp)
    800054ca:	ec56                	sd	s5,24(sp)
    800054cc:	0880                	addi	s0,sp,80
    800054ce:	89ae                	mv	s3,a1
    800054d0:	8ab2                	mv	s5,a2
    800054d2:	8a36                	mv	s4,a3
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
    800054d4:	fb040593          	addi	a1,s0,-80
    800054d8:	fffff097          	auipc	ra,0xfffff
    800054dc:	e86080e7          	jalr	-378(ra) # 8000435e <nameiparent>
    800054e0:	892a                	mv	s2,a0
    800054e2:	12050f63          	beqz	a0,80005620 <create+0x164>
    return 0;

  ilock(dp);
    800054e6:	ffffe097          	auipc	ra,0xffffe
    800054ea:	6a4080e7          	jalr	1700(ra) # 80003b8a <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    800054ee:	4601                	li	a2,0
    800054f0:	fb040593          	addi	a1,s0,-80
    800054f4:	854a                	mv	a0,s2
    800054f6:	fffff097          	auipc	ra,0xfffff
    800054fa:	b78080e7          	jalr	-1160(ra) # 8000406e <dirlookup>
    800054fe:	84aa                	mv	s1,a0
    80005500:	c921                	beqz	a0,80005550 <create+0x94>
    iunlockput(dp);
    80005502:	854a                	mv	a0,s2
    80005504:	fffff097          	auipc	ra,0xfffff
    80005508:	8e8080e7          	jalr	-1816(ra) # 80003dec <iunlockput>
    ilock(ip);
    8000550c:	8526                	mv	a0,s1
    8000550e:	ffffe097          	auipc	ra,0xffffe
    80005512:	67c080e7          	jalr	1660(ra) # 80003b8a <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    80005516:	2981                	sext.w	s3,s3
    80005518:	4789                	li	a5,2
    8000551a:	02f99463          	bne	s3,a5,80005542 <create+0x86>
    8000551e:	0444d783          	lhu	a5,68(s1)
    80005522:	37f9                	addiw	a5,a5,-2
    80005524:	17c2                	slli	a5,a5,0x30
    80005526:	93c1                	srli	a5,a5,0x30
    80005528:	4705                	li	a4,1
    8000552a:	00f76c63          	bltu	a4,a5,80005542 <create+0x86>
    panic("create: dirlink");

  iunlockput(dp);

  return ip;
}
    8000552e:	8526                	mv	a0,s1
    80005530:	60a6                	ld	ra,72(sp)
    80005532:	6406                	ld	s0,64(sp)
    80005534:	74e2                	ld	s1,56(sp)
    80005536:	7942                	ld	s2,48(sp)
    80005538:	79a2                	ld	s3,40(sp)
    8000553a:	7a02                	ld	s4,32(sp)
    8000553c:	6ae2                	ld	s5,24(sp)
    8000553e:	6161                	addi	sp,sp,80
    80005540:	8082                	ret
    iunlockput(ip);
    80005542:	8526                	mv	a0,s1
    80005544:	fffff097          	auipc	ra,0xfffff
    80005548:	8a8080e7          	jalr	-1880(ra) # 80003dec <iunlockput>
    return 0;
    8000554c:	4481                	li	s1,0
    8000554e:	b7c5                	j	8000552e <create+0x72>
  if((ip = ialloc(dp->dev, type)) == 0)
    80005550:	85ce                	mv	a1,s3
    80005552:	00092503          	lw	a0,0(s2)
    80005556:	ffffe097          	auipc	ra,0xffffe
    8000555a:	49c080e7          	jalr	1180(ra) # 800039f2 <ialloc>
    8000555e:	84aa                	mv	s1,a0
    80005560:	c529                	beqz	a0,800055aa <create+0xee>
  ilock(ip);
    80005562:	ffffe097          	auipc	ra,0xffffe
    80005566:	628080e7          	jalr	1576(ra) # 80003b8a <ilock>
  ip->major = major;
    8000556a:	05549323          	sh	s5,70(s1)
  ip->minor = minor;
    8000556e:	05449423          	sh	s4,72(s1)
  ip->nlink = 1;
    80005572:	4785                	li	a5,1
    80005574:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80005578:	8526                	mv	a0,s1
    8000557a:	ffffe097          	auipc	ra,0xffffe
    8000557e:	546080e7          	jalr	1350(ra) # 80003ac0 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    80005582:	2981                	sext.w	s3,s3
    80005584:	4785                	li	a5,1
    80005586:	02f98a63          	beq	s3,a5,800055ba <create+0xfe>
  if(dirlink(dp, name, ip->inum) < 0)
    8000558a:	40d0                	lw	a2,4(s1)
    8000558c:	fb040593          	addi	a1,s0,-80
    80005590:	854a                	mv	a0,s2
    80005592:	fffff097          	auipc	ra,0xfffff
    80005596:	cec080e7          	jalr	-788(ra) # 8000427e <dirlink>
    8000559a:	06054b63          	bltz	a0,80005610 <create+0x154>
  iunlockput(dp);
    8000559e:	854a                	mv	a0,s2
    800055a0:	fffff097          	auipc	ra,0xfffff
    800055a4:	84c080e7          	jalr	-1972(ra) # 80003dec <iunlockput>
  return ip;
    800055a8:	b759                	j	8000552e <create+0x72>
    panic("create: ialloc");
    800055aa:	00003517          	auipc	a0,0x3
    800055ae:	20e50513          	addi	a0,a0,526 # 800087b8 <syscalls+0x2b8>
    800055b2:	ffffb097          	auipc	ra,0xffffb
    800055b6:	f8c080e7          	jalr	-116(ra) # 8000053e <panic>
    dp->nlink++;  // for ".."
    800055ba:	04a95783          	lhu	a5,74(s2)
    800055be:	2785                	addiw	a5,a5,1
    800055c0:	04f91523          	sh	a5,74(s2)
    iupdate(dp);
    800055c4:	854a                	mv	a0,s2
    800055c6:	ffffe097          	auipc	ra,0xffffe
    800055ca:	4fa080e7          	jalr	1274(ra) # 80003ac0 <iupdate>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    800055ce:	40d0                	lw	a2,4(s1)
    800055d0:	00003597          	auipc	a1,0x3
    800055d4:	1f858593          	addi	a1,a1,504 # 800087c8 <syscalls+0x2c8>
    800055d8:	8526                	mv	a0,s1
    800055da:	fffff097          	auipc	ra,0xfffff
    800055de:	ca4080e7          	jalr	-860(ra) # 8000427e <dirlink>
    800055e2:	00054f63          	bltz	a0,80005600 <create+0x144>
    800055e6:	00492603          	lw	a2,4(s2)
    800055ea:	00003597          	auipc	a1,0x3
    800055ee:	1e658593          	addi	a1,a1,486 # 800087d0 <syscalls+0x2d0>
    800055f2:	8526                	mv	a0,s1
    800055f4:	fffff097          	auipc	ra,0xfffff
    800055f8:	c8a080e7          	jalr	-886(ra) # 8000427e <dirlink>
    800055fc:	f80557e3          	bgez	a0,8000558a <create+0xce>
      panic("create dots");
    80005600:	00003517          	auipc	a0,0x3
    80005604:	1d850513          	addi	a0,a0,472 # 800087d8 <syscalls+0x2d8>
    80005608:	ffffb097          	auipc	ra,0xffffb
    8000560c:	f36080e7          	jalr	-202(ra) # 8000053e <panic>
    panic("create: dirlink");
    80005610:	00003517          	auipc	a0,0x3
    80005614:	1d850513          	addi	a0,a0,472 # 800087e8 <syscalls+0x2e8>
    80005618:	ffffb097          	auipc	ra,0xffffb
    8000561c:	f26080e7          	jalr	-218(ra) # 8000053e <panic>
    return 0;
    80005620:	84aa                	mv	s1,a0
    80005622:	b731                	j	8000552e <create+0x72>

0000000080005624 <sys_dup>:
{
    80005624:	7179                	addi	sp,sp,-48
    80005626:	f406                	sd	ra,40(sp)
    80005628:	f022                	sd	s0,32(sp)
    8000562a:	ec26                	sd	s1,24(sp)
    8000562c:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0)
    8000562e:	fd840613          	addi	a2,s0,-40
    80005632:	4581                	li	a1,0
    80005634:	4501                	li	a0,0
    80005636:	00000097          	auipc	ra,0x0
    8000563a:	ddc080e7          	jalr	-548(ra) # 80005412 <argfd>
    return -1;
    8000563e:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    80005640:	02054363          	bltz	a0,80005666 <sys_dup+0x42>
  if((fd=fdalloc(f)) < 0)
    80005644:	fd843503          	ld	a0,-40(s0)
    80005648:	00000097          	auipc	ra,0x0
    8000564c:	e32080e7          	jalr	-462(ra) # 8000547a <fdalloc>
    80005650:	84aa                	mv	s1,a0
    return -1;
    80005652:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    80005654:	00054963          	bltz	a0,80005666 <sys_dup+0x42>
  filedup(f);
    80005658:	fd843503          	ld	a0,-40(s0)
    8000565c:	fffff097          	auipc	ra,0xfffff
    80005660:	37a080e7          	jalr	890(ra) # 800049d6 <filedup>
  return fd;
    80005664:	87a6                	mv	a5,s1
}
    80005666:	853e                	mv	a0,a5
    80005668:	70a2                	ld	ra,40(sp)
    8000566a:	7402                	ld	s0,32(sp)
    8000566c:	64e2                	ld	s1,24(sp)
    8000566e:	6145                	addi	sp,sp,48
    80005670:	8082                	ret

0000000080005672 <sys_read>:
{
    80005672:	7179                	addi	sp,sp,-48
    80005674:	f406                	sd	ra,40(sp)
    80005676:	f022                	sd	s0,32(sp)
    80005678:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000567a:	fe840613          	addi	a2,s0,-24
    8000567e:	4581                	li	a1,0
    80005680:	4501                	li	a0,0
    80005682:	00000097          	auipc	ra,0x0
    80005686:	d90080e7          	jalr	-624(ra) # 80005412 <argfd>
    return -1;
    8000568a:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000568c:	04054163          	bltz	a0,800056ce <sys_read+0x5c>
    80005690:	fe440593          	addi	a1,s0,-28
    80005694:	4509                	li	a0,2
    80005696:	ffffe097          	auipc	ra,0xffffe
    8000569a:	91c080e7          	jalr	-1764(ra) # 80002fb2 <argint>
    return -1;
    8000569e:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800056a0:	02054763          	bltz	a0,800056ce <sys_read+0x5c>
    800056a4:	fd840593          	addi	a1,s0,-40
    800056a8:	4505                	li	a0,1
    800056aa:	ffffe097          	auipc	ra,0xffffe
    800056ae:	92a080e7          	jalr	-1750(ra) # 80002fd4 <argaddr>
    return -1;
    800056b2:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800056b4:	00054d63          	bltz	a0,800056ce <sys_read+0x5c>
  return fileread(f, p, n);
    800056b8:	fe442603          	lw	a2,-28(s0)
    800056bc:	fd843583          	ld	a1,-40(s0)
    800056c0:	fe843503          	ld	a0,-24(s0)
    800056c4:	fffff097          	auipc	ra,0xfffff
    800056c8:	49e080e7          	jalr	1182(ra) # 80004b62 <fileread>
    800056cc:	87aa                	mv	a5,a0
}
    800056ce:	853e                	mv	a0,a5
    800056d0:	70a2                	ld	ra,40(sp)
    800056d2:	7402                	ld	s0,32(sp)
    800056d4:	6145                	addi	sp,sp,48
    800056d6:	8082                	ret

00000000800056d8 <sys_write>:
{
    800056d8:	7179                	addi	sp,sp,-48
    800056da:	f406                	sd	ra,40(sp)
    800056dc:	f022                	sd	s0,32(sp)
    800056de:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800056e0:	fe840613          	addi	a2,s0,-24
    800056e4:	4581                	li	a1,0
    800056e6:	4501                	li	a0,0
    800056e8:	00000097          	auipc	ra,0x0
    800056ec:	d2a080e7          	jalr	-726(ra) # 80005412 <argfd>
    return -1;
    800056f0:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800056f2:	04054163          	bltz	a0,80005734 <sys_write+0x5c>
    800056f6:	fe440593          	addi	a1,s0,-28
    800056fa:	4509                	li	a0,2
    800056fc:	ffffe097          	auipc	ra,0xffffe
    80005700:	8b6080e7          	jalr	-1866(ra) # 80002fb2 <argint>
    return -1;
    80005704:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    80005706:	02054763          	bltz	a0,80005734 <sys_write+0x5c>
    8000570a:	fd840593          	addi	a1,s0,-40
    8000570e:	4505                	li	a0,1
    80005710:	ffffe097          	auipc	ra,0xffffe
    80005714:	8c4080e7          	jalr	-1852(ra) # 80002fd4 <argaddr>
    return -1;
    80005718:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000571a:	00054d63          	bltz	a0,80005734 <sys_write+0x5c>
  return filewrite(f, p, n);
    8000571e:	fe442603          	lw	a2,-28(s0)
    80005722:	fd843583          	ld	a1,-40(s0)
    80005726:	fe843503          	ld	a0,-24(s0)
    8000572a:	fffff097          	auipc	ra,0xfffff
    8000572e:	4fa080e7          	jalr	1274(ra) # 80004c24 <filewrite>
    80005732:	87aa                	mv	a5,a0
}
    80005734:	853e                	mv	a0,a5
    80005736:	70a2                	ld	ra,40(sp)
    80005738:	7402                	ld	s0,32(sp)
    8000573a:	6145                	addi	sp,sp,48
    8000573c:	8082                	ret

000000008000573e <sys_close>:
{
    8000573e:	1101                	addi	sp,sp,-32
    80005740:	ec06                	sd	ra,24(sp)
    80005742:	e822                	sd	s0,16(sp)
    80005744:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    80005746:	fe040613          	addi	a2,s0,-32
    8000574a:	fec40593          	addi	a1,s0,-20
    8000574e:	4501                	li	a0,0
    80005750:	00000097          	auipc	ra,0x0
    80005754:	cc2080e7          	jalr	-830(ra) # 80005412 <argfd>
    return -1;
    80005758:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    8000575a:	02054463          	bltz	a0,80005782 <sys_close+0x44>
  myproc()->ofile[fd] = 0;
    8000575e:	ffffc097          	auipc	ra,0xffffc
    80005762:	280080e7          	jalr	640(ra) # 800019de <myproc>
    80005766:	fec42783          	lw	a5,-20(s0)
    8000576a:	07f9                	addi	a5,a5,30
    8000576c:	078e                	slli	a5,a5,0x3
    8000576e:	97aa                	add	a5,a5,a0
    80005770:	0007b023          	sd	zero,0(a5)
  fileclose(f);
    80005774:	fe043503          	ld	a0,-32(s0)
    80005778:	fffff097          	auipc	ra,0xfffff
    8000577c:	2b0080e7          	jalr	688(ra) # 80004a28 <fileclose>
  return 0;
    80005780:	4781                	li	a5,0
}
    80005782:	853e                	mv	a0,a5
    80005784:	60e2                	ld	ra,24(sp)
    80005786:	6442                	ld	s0,16(sp)
    80005788:	6105                	addi	sp,sp,32
    8000578a:	8082                	ret

000000008000578c <sys_fstat>:
{
    8000578c:	1101                	addi	sp,sp,-32
    8000578e:	ec06                	sd	ra,24(sp)
    80005790:	e822                	sd	s0,16(sp)
    80005792:	1000                	addi	s0,sp,32
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    80005794:	fe840613          	addi	a2,s0,-24
    80005798:	4581                	li	a1,0
    8000579a:	4501                	li	a0,0
    8000579c:	00000097          	auipc	ra,0x0
    800057a0:	c76080e7          	jalr	-906(ra) # 80005412 <argfd>
    return -1;
    800057a4:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    800057a6:	02054563          	bltz	a0,800057d0 <sys_fstat+0x44>
    800057aa:	fe040593          	addi	a1,s0,-32
    800057ae:	4505                	li	a0,1
    800057b0:	ffffe097          	auipc	ra,0xffffe
    800057b4:	824080e7          	jalr	-2012(ra) # 80002fd4 <argaddr>
    return -1;
    800057b8:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    800057ba:	00054b63          	bltz	a0,800057d0 <sys_fstat+0x44>
  return filestat(f, st);
    800057be:	fe043583          	ld	a1,-32(s0)
    800057c2:	fe843503          	ld	a0,-24(s0)
    800057c6:	fffff097          	auipc	ra,0xfffff
    800057ca:	32a080e7          	jalr	810(ra) # 80004af0 <filestat>
    800057ce:	87aa                	mv	a5,a0
}
    800057d0:	853e                	mv	a0,a5
    800057d2:	60e2                	ld	ra,24(sp)
    800057d4:	6442                	ld	s0,16(sp)
    800057d6:	6105                	addi	sp,sp,32
    800057d8:	8082                	ret

00000000800057da <sys_link>:
{
    800057da:	7169                	addi	sp,sp,-304
    800057dc:	f606                	sd	ra,296(sp)
    800057de:	f222                	sd	s0,288(sp)
    800057e0:	ee26                	sd	s1,280(sp)
    800057e2:	ea4a                	sd	s2,272(sp)
    800057e4:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    800057e6:	08000613          	li	a2,128
    800057ea:	ed040593          	addi	a1,s0,-304
    800057ee:	4501                	li	a0,0
    800057f0:	ffffe097          	auipc	ra,0xffffe
    800057f4:	806080e7          	jalr	-2042(ra) # 80002ff6 <argstr>
    return -1;
    800057f8:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    800057fa:	10054e63          	bltz	a0,80005916 <sys_link+0x13c>
    800057fe:	08000613          	li	a2,128
    80005802:	f5040593          	addi	a1,s0,-176
    80005806:	4505                	li	a0,1
    80005808:	ffffd097          	auipc	ra,0xffffd
    8000580c:	7ee080e7          	jalr	2030(ra) # 80002ff6 <argstr>
    return -1;
    80005810:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80005812:	10054263          	bltz	a0,80005916 <sys_link+0x13c>
  begin_op();
    80005816:	fffff097          	auipc	ra,0xfffff
    8000581a:	d46080e7          	jalr	-698(ra) # 8000455c <begin_op>
  if((ip = namei(old)) == 0){
    8000581e:	ed040513          	addi	a0,s0,-304
    80005822:	fffff097          	auipc	ra,0xfffff
    80005826:	b1e080e7          	jalr	-1250(ra) # 80004340 <namei>
    8000582a:	84aa                	mv	s1,a0
    8000582c:	c551                	beqz	a0,800058b8 <sys_link+0xde>
  ilock(ip);
    8000582e:	ffffe097          	auipc	ra,0xffffe
    80005832:	35c080e7          	jalr	860(ra) # 80003b8a <ilock>
  if(ip->type == T_DIR){
    80005836:	04449703          	lh	a4,68(s1)
    8000583a:	4785                	li	a5,1
    8000583c:	08f70463          	beq	a4,a5,800058c4 <sys_link+0xea>
  ip->nlink++;
    80005840:	04a4d783          	lhu	a5,74(s1)
    80005844:	2785                	addiw	a5,a5,1
    80005846:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    8000584a:	8526                	mv	a0,s1
    8000584c:	ffffe097          	auipc	ra,0xffffe
    80005850:	274080e7          	jalr	628(ra) # 80003ac0 <iupdate>
  iunlock(ip);
    80005854:	8526                	mv	a0,s1
    80005856:	ffffe097          	auipc	ra,0xffffe
    8000585a:	3f6080e7          	jalr	1014(ra) # 80003c4c <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    8000585e:	fd040593          	addi	a1,s0,-48
    80005862:	f5040513          	addi	a0,s0,-176
    80005866:	fffff097          	auipc	ra,0xfffff
    8000586a:	af8080e7          	jalr	-1288(ra) # 8000435e <nameiparent>
    8000586e:	892a                	mv	s2,a0
    80005870:	c935                	beqz	a0,800058e4 <sys_link+0x10a>
  ilock(dp);
    80005872:	ffffe097          	auipc	ra,0xffffe
    80005876:	318080e7          	jalr	792(ra) # 80003b8a <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    8000587a:	00092703          	lw	a4,0(s2)
    8000587e:	409c                	lw	a5,0(s1)
    80005880:	04f71d63          	bne	a4,a5,800058da <sys_link+0x100>
    80005884:	40d0                	lw	a2,4(s1)
    80005886:	fd040593          	addi	a1,s0,-48
    8000588a:	854a                	mv	a0,s2
    8000588c:	fffff097          	auipc	ra,0xfffff
    80005890:	9f2080e7          	jalr	-1550(ra) # 8000427e <dirlink>
    80005894:	04054363          	bltz	a0,800058da <sys_link+0x100>
  iunlockput(dp);
    80005898:	854a                	mv	a0,s2
    8000589a:	ffffe097          	auipc	ra,0xffffe
    8000589e:	552080e7          	jalr	1362(ra) # 80003dec <iunlockput>
  iput(ip);
    800058a2:	8526                	mv	a0,s1
    800058a4:	ffffe097          	auipc	ra,0xffffe
    800058a8:	4a0080e7          	jalr	1184(ra) # 80003d44 <iput>
  end_op();
    800058ac:	fffff097          	auipc	ra,0xfffff
    800058b0:	d30080e7          	jalr	-720(ra) # 800045dc <end_op>
  return 0;
    800058b4:	4781                	li	a5,0
    800058b6:	a085                	j	80005916 <sys_link+0x13c>
    end_op();
    800058b8:	fffff097          	auipc	ra,0xfffff
    800058bc:	d24080e7          	jalr	-732(ra) # 800045dc <end_op>
    return -1;
    800058c0:	57fd                	li	a5,-1
    800058c2:	a891                	j	80005916 <sys_link+0x13c>
    iunlockput(ip);
    800058c4:	8526                	mv	a0,s1
    800058c6:	ffffe097          	auipc	ra,0xffffe
    800058ca:	526080e7          	jalr	1318(ra) # 80003dec <iunlockput>
    end_op();
    800058ce:	fffff097          	auipc	ra,0xfffff
    800058d2:	d0e080e7          	jalr	-754(ra) # 800045dc <end_op>
    return -1;
    800058d6:	57fd                	li	a5,-1
    800058d8:	a83d                	j	80005916 <sys_link+0x13c>
    iunlockput(dp);
    800058da:	854a                	mv	a0,s2
    800058dc:	ffffe097          	auipc	ra,0xffffe
    800058e0:	510080e7          	jalr	1296(ra) # 80003dec <iunlockput>
  ilock(ip);
    800058e4:	8526                	mv	a0,s1
    800058e6:	ffffe097          	auipc	ra,0xffffe
    800058ea:	2a4080e7          	jalr	676(ra) # 80003b8a <ilock>
  ip->nlink--;
    800058ee:	04a4d783          	lhu	a5,74(s1)
    800058f2:	37fd                	addiw	a5,a5,-1
    800058f4:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    800058f8:	8526                	mv	a0,s1
    800058fa:	ffffe097          	auipc	ra,0xffffe
    800058fe:	1c6080e7          	jalr	454(ra) # 80003ac0 <iupdate>
  iunlockput(ip);
    80005902:	8526                	mv	a0,s1
    80005904:	ffffe097          	auipc	ra,0xffffe
    80005908:	4e8080e7          	jalr	1256(ra) # 80003dec <iunlockput>
  end_op();
    8000590c:	fffff097          	auipc	ra,0xfffff
    80005910:	cd0080e7          	jalr	-816(ra) # 800045dc <end_op>
  return -1;
    80005914:	57fd                	li	a5,-1
}
    80005916:	853e                	mv	a0,a5
    80005918:	70b2                	ld	ra,296(sp)
    8000591a:	7412                	ld	s0,288(sp)
    8000591c:	64f2                	ld	s1,280(sp)
    8000591e:	6952                	ld	s2,272(sp)
    80005920:	6155                	addi	sp,sp,304
    80005922:	8082                	ret

0000000080005924 <sys_unlink>:
{
    80005924:	7151                	addi	sp,sp,-240
    80005926:	f586                	sd	ra,232(sp)
    80005928:	f1a2                	sd	s0,224(sp)
    8000592a:	eda6                	sd	s1,216(sp)
    8000592c:	e9ca                	sd	s2,208(sp)
    8000592e:	e5ce                	sd	s3,200(sp)
    80005930:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    80005932:	08000613          	li	a2,128
    80005936:	f3040593          	addi	a1,s0,-208
    8000593a:	4501                	li	a0,0
    8000593c:	ffffd097          	auipc	ra,0xffffd
    80005940:	6ba080e7          	jalr	1722(ra) # 80002ff6 <argstr>
    80005944:	18054163          	bltz	a0,80005ac6 <sys_unlink+0x1a2>
  begin_op();
    80005948:	fffff097          	auipc	ra,0xfffff
    8000594c:	c14080e7          	jalr	-1004(ra) # 8000455c <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    80005950:	fb040593          	addi	a1,s0,-80
    80005954:	f3040513          	addi	a0,s0,-208
    80005958:	fffff097          	auipc	ra,0xfffff
    8000595c:	a06080e7          	jalr	-1530(ra) # 8000435e <nameiparent>
    80005960:	84aa                	mv	s1,a0
    80005962:	c979                	beqz	a0,80005a38 <sys_unlink+0x114>
  ilock(dp);
    80005964:	ffffe097          	auipc	ra,0xffffe
    80005968:	226080e7          	jalr	550(ra) # 80003b8a <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    8000596c:	00003597          	auipc	a1,0x3
    80005970:	e5c58593          	addi	a1,a1,-420 # 800087c8 <syscalls+0x2c8>
    80005974:	fb040513          	addi	a0,s0,-80
    80005978:	ffffe097          	auipc	ra,0xffffe
    8000597c:	6dc080e7          	jalr	1756(ra) # 80004054 <namecmp>
    80005980:	14050a63          	beqz	a0,80005ad4 <sys_unlink+0x1b0>
    80005984:	00003597          	auipc	a1,0x3
    80005988:	e4c58593          	addi	a1,a1,-436 # 800087d0 <syscalls+0x2d0>
    8000598c:	fb040513          	addi	a0,s0,-80
    80005990:	ffffe097          	auipc	ra,0xffffe
    80005994:	6c4080e7          	jalr	1732(ra) # 80004054 <namecmp>
    80005998:	12050e63          	beqz	a0,80005ad4 <sys_unlink+0x1b0>
  if((ip = dirlookup(dp, name, &off)) == 0)
    8000599c:	f2c40613          	addi	a2,s0,-212
    800059a0:	fb040593          	addi	a1,s0,-80
    800059a4:	8526                	mv	a0,s1
    800059a6:	ffffe097          	auipc	ra,0xffffe
    800059aa:	6c8080e7          	jalr	1736(ra) # 8000406e <dirlookup>
    800059ae:	892a                	mv	s2,a0
    800059b0:	12050263          	beqz	a0,80005ad4 <sys_unlink+0x1b0>
  ilock(ip);
    800059b4:	ffffe097          	auipc	ra,0xffffe
    800059b8:	1d6080e7          	jalr	470(ra) # 80003b8a <ilock>
  if(ip->nlink < 1)
    800059bc:	04a91783          	lh	a5,74(s2)
    800059c0:	08f05263          	blez	a5,80005a44 <sys_unlink+0x120>
  if(ip->type == T_DIR && !isdirempty(ip)){
    800059c4:	04491703          	lh	a4,68(s2)
    800059c8:	4785                	li	a5,1
    800059ca:	08f70563          	beq	a4,a5,80005a54 <sys_unlink+0x130>
  memset(&de, 0, sizeof(de));
    800059ce:	4641                	li	a2,16
    800059d0:	4581                	li	a1,0
    800059d2:	fc040513          	addi	a0,s0,-64
    800059d6:	ffffb097          	auipc	ra,0xffffb
    800059da:	30a080e7          	jalr	778(ra) # 80000ce0 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800059de:	4741                	li	a4,16
    800059e0:	f2c42683          	lw	a3,-212(s0)
    800059e4:	fc040613          	addi	a2,s0,-64
    800059e8:	4581                	li	a1,0
    800059ea:	8526                	mv	a0,s1
    800059ec:	ffffe097          	auipc	ra,0xffffe
    800059f0:	54a080e7          	jalr	1354(ra) # 80003f36 <writei>
    800059f4:	47c1                	li	a5,16
    800059f6:	0af51563          	bne	a0,a5,80005aa0 <sys_unlink+0x17c>
  if(ip->type == T_DIR){
    800059fa:	04491703          	lh	a4,68(s2)
    800059fe:	4785                	li	a5,1
    80005a00:	0af70863          	beq	a4,a5,80005ab0 <sys_unlink+0x18c>
  iunlockput(dp);
    80005a04:	8526                	mv	a0,s1
    80005a06:	ffffe097          	auipc	ra,0xffffe
    80005a0a:	3e6080e7          	jalr	998(ra) # 80003dec <iunlockput>
  ip->nlink--;
    80005a0e:	04a95783          	lhu	a5,74(s2)
    80005a12:	37fd                	addiw	a5,a5,-1
    80005a14:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    80005a18:	854a                	mv	a0,s2
    80005a1a:	ffffe097          	auipc	ra,0xffffe
    80005a1e:	0a6080e7          	jalr	166(ra) # 80003ac0 <iupdate>
  iunlockput(ip);
    80005a22:	854a                	mv	a0,s2
    80005a24:	ffffe097          	auipc	ra,0xffffe
    80005a28:	3c8080e7          	jalr	968(ra) # 80003dec <iunlockput>
  end_op();
    80005a2c:	fffff097          	auipc	ra,0xfffff
    80005a30:	bb0080e7          	jalr	-1104(ra) # 800045dc <end_op>
  return 0;
    80005a34:	4501                	li	a0,0
    80005a36:	a84d                	j	80005ae8 <sys_unlink+0x1c4>
    end_op();
    80005a38:	fffff097          	auipc	ra,0xfffff
    80005a3c:	ba4080e7          	jalr	-1116(ra) # 800045dc <end_op>
    return -1;
    80005a40:	557d                	li	a0,-1
    80005a42:	a05d                	j	80005ae8 <sys_unlink+0x1c4>
    panic("unlink: nlink < 1");
    80005a44:	00003517          	auipc	a0,0x3
    80005a48:	db450513          	addi	a0,a0,-588 # 800087f8 <syscalls+0x2f8>
    80005a4c:	ffffb097          	auipc	ra,0xffffb
    80005a50:	af2080e7          	jalr	-1294(ra) # 8000053e <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80005a54:	04c92703          	lw	a4,76(s2)
    80005a58:	02000793          	li	a5,32
    80005a5c:	f6e7f9e3          	bgeu	a5,a4,800059ce <sys_unlink+0xaa>
    80005a60:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80005a64:	4741                	li	a4,16
    80005a66:	86ce                	mv	a3,s3
    80005a68:	f1840613          	addi	a2,s0,-232
    80005a6c:	4581                	li	a1,0
    80005a6e:	854a                	mv	a0,s2
    80005a70:	ffffe097          	auipc	ra,0xffffe
    80005a74:	3ce080e7          	jalr	974(ra) # 80003e3e <readi>
    80005a78:	47c1                	li	a5,16
    80005a7a:	00f51b63          	bne	a0,a5,80005a90 <sys_unlink+0x16c>
    if(de.inum != 0)
    80005a7e:	f1845783          	lhu	a5,-232(s0)
    80005a82:	e7a1                	bnez	a5,80005aca <sys_unlink+0x1a6>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80005a84:	29c1                	addiw	s3,s3,16
    80005a86:	04c92783          	lw	a5,76(s2)
    80005a8a:	fcf9ede3          	bltu	s3,a5,80005a64 <sys_unlink+0x140>
    80005a8e:	b781                	j	800059ce <sys_unlink+0xaa>
      panic("isdirempty: readi");
    80005a90:	00003517          	auipc	a0,0x3
    80005a94:	d8050513          	addi	a0,a0,-640 # 80008810 <syscalls+0x310>
    80005a98:	ffffb097          	auipc	ra,0xffffb
    80005a9c:	aa6080e7          	jalr	-1370(ra) # 8000053e <panic>
    panic("unlink: writei");
    80005aa0:	00003517          	auipc	a0,0x3
    80005aa4:	d8850513          	addi	a0,a0,-632 # 80008828 <syscalls+0x328>
    80005aa8:	ffffb097          	auipc	ra,0xffffb
    80005aac:	a96080e7          	jalr	-1386(ra) # 8000053e <panic>
    dp->nlink--;
    80005ab0:	04a4d783          	lhu	a5,74(s1)
    80005ab4:	37fd                	addiw	a5,a5,-1
    80005ab6:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80005aba:	8526                	mv	a0,s1
    80005abc:	ffffe097          	auipc	ra,0xffffe
    80005ac0:	004080e7          	jalr	4(ra) # 80003ac0 <iupdate>
    80005ac4:	b781                	j	80005a04 <sys_unlink+0xe0>
    return -1;
    80005ac6:	557d                	li	a0,-1
    80005ac8:	a005                	j	80005ae8 <sys_unlink+0x1c4>
    iunlockput(ip);
    80005aca:	854a                	mv	a0,s2
    80005acc:	ffffe097          	auipc	ra,0xffffe
    80005ad0:	320080e7          	jalr	800(ra) # 80003dec <iunlockput>
  iunlockput(dp);
    80005ad4:	8526                	mv	a0,s1
    80005ad6:	ffffe097          	auipc	ra,0xffffe
    80005ada:	316080e7          	jalr	790(ra) # 80003dec <iunlockput>
  end_op();
    80005ade:	fffff097          	auipc	ra,0xfffff
    80005ae2:	afe080e7          	jalr	-1282(ra) # 800045dc <end_op>
  return -1;
    80005ae6:	557d                	li	a0,-1
}
    80005ae8:	70ae                	ld	ra,232(sp)
    80005aea:	740e                	ld	s0,224(sp)
    80005aec:	64ee                	ld	s1,216(sp)
    80005aee:	694e                	ld	s2,208(sp)
    80005af0:	69ae                	ld	s3,200(sp)
    80005af2:	616d                	addi	sp,sp,240
    80005af4:	8082                	ret

0000000080005af6 <sys_open>:

uint64
sys_open(void)
{
    80005af6:	7131                	addi	sp,sp,-192
    80005af8:	fd06                	sd	ra,184(sp)
    80005afa:	f922                	sd	s0,176(sp)
    80005afc:	f526                	sd	s1,168(sp)
    80005afe:	f14a                	sd	s2,160(sp)
    80005b00:	ed4e                	sd	s3,152(sp)
    80005b02:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;

  if((n = argstr(0, path, MAXPATH)) < 0 || argint(1, &omode) < 0)
    80005b04:	08000613          	li	a2,128
    80005b08:	f5040593          	addi	a1,s0,-176
    80005b0c:	4501                	li	a0,0
    80005b0e:	ffffd097          	auipc	ra,0xffffd
    80005b12:	4e8080e7          	jalr	1256(ra) # 80002ff6 <argstr>
    return -1;
    80005b16:	54fd                	li	s1,-1
  if((n = argstr(0, path, MAXPATH)) < 0 || argint(1, &omode) < 0)
    80005b18:	0c054163          	bltz	a0,80005bda <sys_open+0xe4>
    80005b1c:	f4c40593          	addi	a1,s0,-180
    80005b20:	4505                	li	a0,1
    80005b22:	ffffd097          	auipc	ra,0xffffd
    80005b26:	490080e7          	jalr	1168(ra) # 80002fb2 <argint>
    80005b2a:	0a054863          	bltz	a0,80005bda <sys_open+0xe4>

  begin_op();
    80005b2e:	fffff097          	auipc	ra,0xfffff
    80005b32:	a2e080e7          	jalr	-1490(ra) # 8000455c <begin_op>

  if(omode & O_CREATE){
    80005b36:	f4c42783          	lw	a5,-180(s0)
    80005b3a:	2007f793          	andi	a5,a5,512
    80005b3e:	cbdd                	beqz	a5,80005bf4 <sys_open+0xfe>
    ip = create(path, T_FILE, 0, 0);
    80005b40:	4681                	li	a3,0
    80005b42:	4601                	li	a2,0
    80005b44:	4589                	li	a1,2
    80005b46:	f5040513          	addi	a0,s0,-176
    80005b4a:	00000097          	auipc	ra,0x0
    80005b4e:	972080e7          	jalr	-1678(ra) # 800054bc <create>
    80005b52:	892a                	mv	s2,a0
    if(ip == 0){
    80005b54:	c959                	beqz	a0,80005bea <sys_open+0xf4>
      end_op();
      return -1;
    }
  }

  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    80005b56:	04491703          	lh	a4,68(s2)
    80005b5a:	478d                	li	a5,3
    80005b5c:	00f71763          	bne	a4,a5,80005b6a <sys_open+0x74>
    80005b60:	04695703          	lhu	a4,70(s2)
    80005b64:	47a5                	li	a5,9
    80005b66:	0ce7ec63          	bltu	a5,a4,80005c3e <sys_open+0x148>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    80005b6a:	fffff097          	auipc	ra,0xfffff
    80005b6e:	e02080e7          	jalr	-510(ra) # 8000496c <filealloc>
    80005b72:	89aa                	mv	s3,a0
    80005b74:	10050263          	beqz	a0,80005c78 <sys_open+0x182>
    80005b78:	00000097          	auipc	ra,0x0
    80005b7c:	902080e7          	jalr	-1790(ra) # 8000547a <fdalloc>
    80005b80:	84aa                	mv	s1,a0
    80005b82:	0e054663          	bltz	a0,80005c6e <sys_open+0x178>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if(ip->type == T_DEVICE){
    80005b86:	04491703          	lh	a4,68(s2)
    80005b8a:	478d                	li	a5,3
    80005b8c:	0cf70463          	beq	a4,a5,80005c54 <sys_open+0x15e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80005b90:	4789                	li	a5,2
    80005b92:	00f9a023          	sw	a5,0(s3)
    f->off = 0;
    80005b96:	0209a023          	sw	zero,32(s3)
  }
  f->ip = ip;
    80005b9a:	0129bc23          	sd	s2,24(s3)
  f->readable = !(omode & O_WRONLY);
    80005b9e:	f4c42783          	lw	a5,-180(s0)
    80005ba2:	0017c713          	xori	a4,a5,1
    80005ba6:	8b05                	andi	a4,a4,1
    80005ba8:	00e98423          	sb	a4,8(s3)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    80005bac:	0037f713          	andi	a4,a5,3
    80005bb0:	00e03733          	snez	a4,a4
    80005bb4:	00e984a3          	sb	a4,9(s3)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80005bb8:	4007f793          	andi	a5,a5,1024
    80005bbc:	c791                	beqz	a5,80005bc8 <sys_open+0xd2>
    80005bbe:	04491703          	lh	a4,68(s2)
    80005bc2:	4789                	li	a5,2
    80005bc4:	08f70f63          	beq	a4,a5,80005c62 <sys_open+0x16c>
    itrunc(ip);
  }

  iunlock(ip);
    80005bc8:	854a                	mv	a0,s2
    80005bca:	ffffe097          	auipc	ra,0xffffe
    80005bce:	082080e7          	jalr	130(ra) # 80003c4c <iunlock>
  end_op();
    80005bd2:	fffff097          	auipc	ra,0xfffff
    80005bd6:	a0a080e7          	jalr	-1526(ra) # 800045dc <end_op>

  return fd;
}
    80005bda:	8526                	mv	a0,s1
    80005bdc:	70ea                	ld	ra,184(sp)
    80005bde:	744a                	ld	s0,176(sp)
    80005be0:	74aa                	ld	s1,168(sp)
    80005be2:	790a                	ld	s2,160(sp)
    80005be4:	69ea                	ld	s3,152(sp)
    80005be6:	6129                	addi	sp,sp,192
    80005be8:	8082                	ret
      end_op();
    80005bea:	fffff097          	auipc	ra,0xfffff
    80005bee:	9f2080e7          	jalr	-1550(ra) # 800045dc <end_op>
      return -1;
    80005bf2:	b7e5                	j	80005bda <sys_open+0xe4>
    if((ip = namei(path)) == 0){
    80005bf4:	f5040513          	addi	a0,s0,-176
    80005bf8:	ffffe097          	auipc	ra,0xffffe
    80005bfc:	748080e7          	jalr	1864(ra) # 80004340 <namei>
    80005c00:	892a                	mv	s2,a0
    80005c02:	c905                	beqz	a0,80005c32 <sys_open+0x13c>
    ilock(ip);
    80005c04:	ffffe097          	auipc	ra,0xffffe
    80005c08:	f86080e7          	jalr	-122(ra) # 80003b8a <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    80005c0c:	04491703          	lh	a4,68(s2)
    80005c10:	4785                	li	a5,1
    80005c12:	f4f712e3          	bne	a4,a5,80005b56 <sys_open+0x60>
    80005c16:	f4c42783          	lw	a5,-180(s0)
    80005c1a:	dba1                	beqz	a5,80005b6a <sys_open+0x74>
      iunlockput(ip);
    80005c1c:	854a                	mv	a0,s2
    80005c1e:	ffffe097          	auipc	ra,0xffffe
    80005c22:	1ce080e7          	jalr	462(ra) # 80003dec <iunlockput>
      end_op();
    80005c26:	fffff097          	auipc	ra,0xfffff
    80005c2a:	9b6080e7          	jalr	-1610(ra) # 800045dc <end_op>
      return -1;
    80005c2e:	54fd                	li	s1,-1
    80005c30:	b76d                	j	80005bda <sys_open+0xe4>
      end_op();
    80005c32:	fffff097          	auipc	ra,0xfffff
    80005c36:	9aa080e7          	jalr	-1622(ra) # 800045dc <end_op>
      return -1;
    80005c3a:	54fd                	li	s1,-1
    80005c3c:	bf79                	j	80005bda <sys_open+0xe4>
    iunlockput(ip);
    80005c3e:	854a                	mv	a0,s2
    80005c40:	ffffe097          	auipc	ra,0xffffe
    80005c44:	1ac080e7          	jalr	428(ra) # 80003dec <iunlockput>
    end_op();
    80005c48:	fffff097          	auipc	ra,0xfffff
    80005c4c:	994080e7          	jalr	-1644(ra) # 800045dc <end_op>
    return -1;
    80005c50:	54fd                	li	s1,-1
    80005c52:	b761                	j	80005bda <sys_open+0xe4>
    f->type = FD_DEVICE;
    80005c54:	00f9a023          	sw	a5,0(s3)
    f->major = ip->major;
    80005c58:	04691783          	lh	a5,70(s2)
    80005c5c:	02f99223          	sh	a5,36(s3)
    80005c60:	bf2d                	j	80005b9a <sys_open+0xa4>
    itrunc(ip);
    80005c62:	854a                	mv	a0,s2
    80005c64:	ffffe097          	auipc	ra,0xffffe
    80005c68:	034080e7          	jalr	52(ra) # 80003c98 <itrunc>
    80005c6c:	bfb1                	j	80005bc8 <sys_open+0xd2>
      fileclose(f);
    80005c6e:	854e                	mv	a0,s3
    80005c70:	fffff097          	auipc	ra,0xfffff
    80005c74:	db8080e7          	jalr	-584(ra) # 80004a28 <fileclose>
    iunlockput(ip);
    80005c78:	854a                	mv	a0,s2
    80005c7a:	ffffe097          	auipc	ra,0xffffe
    80005c7e:	172080e7          	jalr	370(ra) # 80003dec <iunlockput>
    end_op();
    80005c82:	fffff097          	auipc	ra,0xfffff
    80005c86:	95a080e7          	jalr	-1702(ra) # 800045dc <end_op>
    return -1;
    80005c8a:	54fd                	li	s1,-1
    80005c8c:	b7b9                	j	80005bda <sys_open+0xe4>

0000000080005c8e <sys_mkdir>:

uint64
sys_mkdir(void)
{
    80005c8e:	7175                	addi	sp,sp,-144
    80005c90:	e506                	sd	ra,136(sp)
    80005c92:	e122                	sd	s0,128(sp)
    80005c94:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    80005c96:	fffff097          	auipc	ra,0xfffff
    80005c9a:	8c6080e7          	jalr	-1850(ra) # 8000455c <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    80005c9e:	08000613          	li	a2,128
    80005ca2:	f7040593          	addi	a1,s0,-144
    80005ca6:	4501                	li	a0,0
    80005ca8:	ffffd097          	auipc	ra,0xffffd
    80005cac:	34e080e7          	jalr	846(ra) # 80002ff6 <argstr>
    80005cb0:	02054963          	bltz	a0,80005ce2 <sys_mkdir+0x54>
    80005cb4:	4681                	li	a3,0
    80005cb6:	4601                	li	a2,0
    80005cb8:	4585                	li	a1,1
    80005cba:	f7040513          	addi	a0,s0,-144
    80005cbe:	fffff097          	auipc	ra,0xfffff
    80005cc2:	7fe080e7          	jalr	2046(ra) # 800054bc <create>
    80005cc6:	cd11                	beqz	a0,80005ce2 <sys_mkdir+0x54>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005cc8:	ffffe097          	auipc	ra,0xffffe
    80005ccc:	124080e7          	jalr	292(ra) # 80003dec <iunlockput>
  end_op();
    80005cd0:	fffff097          	auipc	ra,0xfffff
    80005cd4:	90c080e7          	jalr	-1780(ra) # 800045dc <end_op>
  return 0;
    80005cd8:	4501                	li	a0,0
}
    80005cda:	60aa                	ld	ra,136(sp)
    80005cdc:	640a                	ld	s0,128(sp)
    80005cde:	6149                	addi	sp,sp,144
    80005ce0:	8082                	ret
    end_op();
    80005ce2:	fffff097          	auipc	ra,0xfffff
    80005ce6:	8fa080e7          	jalr	-1798(ra) # 800045dc <end_op>
    return -1;
    80005cea:	557d                	li	a0,-1
    80005cec:	b7fd                	j	80005cda <sys_mkdir+0x4c>

0000000080005cee <sys_mknod>:

uint64
sys_mknod(void)
{
    80005cee:	7135                	addi	sp,sp,-160
    80005cf0:	ed06                	sd	ra,152(sp)
    80005cf2:	e922                	sd	s0,144(sp)
    80005cf4:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    80005cf6:	fffff097          	auipc	ra,0xfffff
    80005cfa:	866080e7          	jalr	-1946(ra) # 8000455c <begin_op>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005cfe:	08000613          	li	a2,128
    80005d02:	f7040593          	addi	a1,s0,-144
    80005d06:	4501                	li	a0,0
    80005d08:	ffffd097          	auipc	ra,0xffffd
    80005d0c:	2ee080e7          	jalr	750(ra) # 80002ff6 <argstr>
    80005d10:	04054a63          	bltz	a0,80005d64 <sys_mknod+0x76>
     argint(1, &major) < 0 ||
    80005d14:	f6c40593          	addi	a1,s0,-148
    80005d18:	4505                	li	a0,1
    80005d1a:	ffffd097          	auipc	ra,0xffffd
    80005d1e:	298080e7          	jalr	664(ra) # 80002fb2 <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005d22:	04054163          	bltz	a0,80005d64 <sys_mknod+0x76>
     argint(2, &minor) < 0 ||
    80005d26:	f6840593          	addi	a1,s0,-152
    80005d2a:	4509                	li	a0,2
    80005d2c:	ffffd097          	auipc	ra,0xffffd
    80005d30:	286080e7          	jalr	646(ra) # 80002fb2 <argint>
     argint(1, &major) < 0 ||
    80005d34:	02054863          	bltz	a0,80005d64 <sys_mknod+0x76>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    80005d38:	f6841683          	lh	a3,-152(s0)
    80005d3c:	f6c41603          	lh	a2,-148(s0)
    80005d40:	458d                	li	a1,3
    80005d42:	f7040513          	addi	a0,s0,-144
    80005d46:	fffff097          	auipc	ra,0xfffff
    80005d4a:	776080e7          	jalr	1910(ra) # 800054bc <create>
     argint(2, &minor) < 0 ||
    80005d4e:	c919                	beqz	a0,80005d64 <sys_mknod+0x76>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005d50:	ffffe097          	auipc	ra,0xffffe
    80005d54:	09c080e7          	jalr	156(ra) # 80003dec <iunlockput>
  end_op();
    80005d58:	fffff097          	auipc	ra,0xfffff
    80005d5c:	884080e7          	jalr	-1916(ra) # 800045dc <end_op>
  return 0;
    80005d60:	4501                	li	a0,0
    80005d62:	a031                	j	80005d6e <sys_mknod+0x80>
    end_op();
    80005d64:	fffff097          	auipc	ra,0xfffff
    80005d68:	878080e7          	jalr	-1928(ra) # 800045dc <end_op>
    return -1;
    80005d6c:	557d                	li	a0,-1
}
    80005d6e:	60ea                	ld	ra,152(sp)
    80005d70:	644a                	ld	s0,144(sp)
    80005d72:	610d                	addi	sp,sp,160
    80005d74:	8082                	ret

0000000080005d76 <sys_chdir>:

uint64
sys_chdir(void)
{
    80005d76:	7135                	addi	sp,sp,-160
    80005d78:	ed06                	sd	ra,152(sp)
    80005d7a:	e922                	sd	s0,144(sp)
    80005d7c:	e526                	sd	s1,136(sp)
    80005d7e:	e14a                	sd	s2,128(sp)
    80005d80:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005d82:	ffffc097          	auipc	ra,0xffffc
    80005d86:	c5c080e7          	jalr	-932(ra) # 800019de <myproc>
    80005d8a:	892a                	mv	s2,a0
  
  begin_op();
    80005d8c:	ffffe097          	auipc	ra,0xffffe
    80005d90:	7d0080e7          	jalr	2000(ra) # 8000455c <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    80005d94:	08000613          	li	a2,128
    80005d98:	f6040593          	addi	a1,s0,-160
    80005d9c:	4501                	li	a0,0
    80005d9e:	ffffd097          	auipc	ra,0xffffd
    80005da2:	258080e7          	jalr	600(ra) # 80002ff6 <argstr>
    80005da6:	04054b63          	bltz	a0,80005dfc <sys_chdir+0x86>
    80005daa:	f6040513          	addi	a0,s0,-160
    80005dae:	ffffe097          	auipc	ra,0xffffe
    80005db2:	592080e7          	jalr	1426(ra) # 80004340 <namei>
    80005db6:	84aa                	mv	s1,a0
    80005db8:	c131                	beqz	a0,80005dfc <sys_chdir+0x86>
    end_op();
    return -1;
  }
  ilock(ip);
    80005dba:	ffffe097          	auipc	ra,0xffffe
    80005dbe:	dd0080e7          	jalr	-560(ra) # 80003b8a <ilock>
  if(ip->type != T_DIR){
    80005dc2:	04449703          	lh	a4,68(s1)
    80005dc6:	4785                	li	a5,1
    80005dc8:	04f71063          	bne	a4,a5,80005e08 <sys_chdir+0x92>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    80005dcc:	8526                	mv	a0,s1
    80005dce:	ffffe097          	auipc	ra,0xffffe
    80005dd2:	e7e080e7          	jalr	-386(ra) # 80003c4c <iunlock>
  iput(p->cwd);
    80005dd6:	17093503          	ld	a0,368(s2)
    80005dda:	ffffe097          	auipc	ra,0xffffe
    80005dde:	f6a080e7          	jalr	-150(ra) # 80003d44 <iput>
  end_op();
    80005de2:	ffffe097          	auipc	ra,0xffffe
    80005de6:	7fa080e7          	jalr	2042(ra) # 800045dc <end_op>
  p->cwd = ip;
    80005dea:	16993823          	sd	s1,368(s2)
  return 0;
    80005dee:	4501                	li	a0,0
}
    80005df0:	60ea                	ld	ra,152(sp)
    80005df2:	644a                	ld	s0,144(sp)
    80005df4:	64aa                	ld	s1,136(sp)
    80005df6:	690a                	ld	s2,128(sp)
    80005df8:	610d                	addi	sp,sp,160
    80005dfa:	8082                	ret
    end_op();
    80005dfc:	ffffe097          	auipc	ra,0xffffe
    80005e00:	7e0080e7          	jalr	2016(ra) # 800045dc <end_op>
    return -1;
    80005e04:	557d                	li	a0,-1
    80005e06:	b7ed                	j	80005df0 <sys_chdir+0x7a>
    iunlockput(ip);
    80005e08:	8526                	mv	a0,s1
    80005e0a:	ffffe097          	auipc	ra,0xffffe
    80005e0e:	fe2080e7          	jalr	-30(ra) # 80003dec <iunlockput>
    end_op();
    80005e12:	ffffe097          	auipc	ra,0xffffe
    80005e16:	7ca080e7          	jalr	1994(ra) # 800045dc <end_op>
    return -1;
    80005e1a:	557d                	li	a0,-1
    80005e1c:	bfd1                	j	80005df0 <sys_chdir+0x7a>

0000000080005e1e <sys_exec>:

uint64
sys_exec(void)
{
    80005e1e:	7145                	addi	sp,sp,-464
    80005e20:	e786                	sd	ra,456(sp)
    80005e22:	e3a2                	sd	s0,448(sp)
    80005e24:	ff26                	sd	s1,440(sp)
    80005e26:	fb4a                	sd	s2,432(sp)
    80005e28:	f74e                	sd	s3,424(sp)
    80005e2a:	f352                	sd	s4,416(sp)
    80005e2c:	ef56                	sd	s5,408(sp)
    80005e2e:	0b80                	addi	s0,sp,464
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  if(argstr(0, path, MAXPATH) < 0 || argaddr(1, &uargv) < 0){
    80005e30:	08000613          	li	a2,128
    80005e34:	f4040593          	addi	a1,s0,-192
    80005e38:	4501                	li	a0,0
    80005e3a:	ffffd097          	auipc	ra,0xffffd
    80005e3e:	1bc080e7          	jalr	444(ra) # 80002ff6 <argstr>
    return -1;
    80005e42:	597d                	li	s2,-1
  if(argstr(0, path, MAXPATH) < 0 || argaddr(1, &uargv) < 0){
    80005e44:	0c054a63          	bltz	a0,80005f18 <sys_exec+0xfa>
    80005e48:	e3840593          	addi	a1,s0,-456
    80005e4c:	4505                	li	a0,1
    80005e4e:	ffffd097          	auipc	ra,0xffffd
    80005e52:	186080e7          	jalr	390(ra) # 80002fd4 <argaddr>
    80005e56:	0c054163          	bltz	a0,80005f18 <sys_exec+0xfa>
  }
  memset(argv, 0, sizeof(argv));
    80005e5a:	10000613          	li	a2,256
    80005e5e:	4581                	li	a1,0
    80005e60:	e4040513          	addi	a0,s0,-448
    80005e64:	ffffb097          	auipc	ra,0xffffb
    80005e68:	e7c080e7          	jalr	-388(ra) # 80000ce0 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    80005e6c:	e4040493          	addi	s1,s0,-448
  memset(argv, 0, sizeof(argv));
    80005e70:	89a6                	mv	s3,s1
    80005e72:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    80005e74:	02000a13          	li	s4,32
    80005e78:	00090a9b          	sext.w	s5,s2
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    80005e7c:	00391513          	slli	a0,s2,0x3
    80005e80:	e3040593          	addi	a1,s0,-464
    80005e84:	e3843783          	ld	a5,-456(s0)
    80005e88:	953e                	add	a0,a0,a5
    80005e8a:	ffffd097          	auipc	ra,0xffffd
    80005e8e:	08e080e7          	jalr	142(ra) # 80002f18 <fetchaddr>
    80005e92:	02054a63          	bltz	a0,80005ec6 <sys_exec+0xa8>
      goto bad;
    }
    if(uarg == 0){
    80005e96:	e3043783          	ld	a5,-464(s0)
    80005e9a:	c3b9                	beqz	a5,80005ee0 <sys_exec+0xc2>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    80005e9c:	ffffb097          	auipc	ra,0xffffb
    80005ea0:	c58080e7          	jalr	-936(ra) # 80000af4 <kalloc>
    80005ea4:	85aa                	mv	a1,a0
    80005ea6:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    80005eaa:	cd11                	beqz	a0,80005ec6 <sys_exec+0xa8>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005eac:	6605                	lui	a2,0x1
    80005eae:	e3043503          	ld	a0,-464(s0)
    80005eb2:	ffffd097          	auipc	ra,0xffffd
    80005eb6:	0b8080e7          	jalr	184(ra) # 80002f6a <fetchstr>
    80005eba:	00054663          	bltz	a0,80005ec6 <sys_exec+0xa8>
    if(i >= NELEM(argv)){
    80005ebe:	0905                	addi	s2,s2,1
    80005ec0:	09a1                	addi	s3,s3,8
    80005ec2:	fb491be3          	bne	s2,s4,80005e78 <sys_exec+0x5a>
    kfree(argv[i]);

  return ret;

 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005ec6:	10048913          	addi	s2,s1,256
    80005eca:	6088                	ld	a0,0(s1)
    80005ecc:	c529                	beqz	a0,80005f16 <sys_exec+0xf8>
    kfree(argv[i]);
    80005ece:	ffffb097          	auipc	ra,0xffffb
    80005ed2:	b2a080e7          	jalr	-1238(ra) # 800009f8 <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005ed6:	04a1                	addi	s1,s1,8
    80005ed8:	ff2499e3          	bne	s1,s2,80005eca <sys_exec+0xac>
  return -1;
    80005edc:	597d                	li	s2,-1
    80005ede:	a82d                	j	80005f18 <sys_exec+0xfa>
      argv[i] = 0;
    80005ee0:	0a8e                	slli	s5,s5,0x3
    80005ee2:	fc040793          	addi	a5,s0,-64
    80005ee6:	9abe                	add	s5,s5,a5
    80005ee8:	e80ab023          	sd	zero,-384(s5)
  int ret = exec(path, argv);
    80005eec:	e4040593          	addi	a1,s0,-448
    80005ef0:	f4040513          	addi	a0,s0,-192
    80005ef4:	fffff097          	auipc	ra,0xfffff
    80005ef8:	194080e7          	jalr	404(ra) # 80005088 <exec>
    80005efc:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005efe:	10048993          	addi	s3,s1,256
    80005f02:	6088                	ld	a0,0(s1)
    80005f04:	c911                	beqz	a0,80005f18 <sys_exec+0xfa>
    kfree(argv[i]);
    80005f06:	ffffb097          	auipc	ra,0xffffb
    80005f0a:	af2080e7          	jalr	-1294(ra) # 800009f8 <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005f0e:	04a1                	addi	s1,s1,8
    80005f10:	ff3499e3          	bne	s1,s3,80005f02 <sys_exec+0xe4>
    80005f14:	a011                	j	80005f18 <sys_exec+0xfa>
  return -1;
    80005f16:	597d                	li	s2,-1
}
    80005f18:	854a                	mv	a0,s2
    80005f1a:	60be                	ld	ra,456(sp)
    80005f1c:	641e                	ld	s0,448(sp)
    80005f1e:	74fa                	ld	s1,440(sp)
    80005f20:	795a                	ld	s2,432(sp)
    80005f22:	79ba                	ld	s3,424(sp)
    80005f24:	7a1a                	ld	s4,416(sp)
    80005f26:	6afa                	ld	s5,408(sp)
    80005f28:	6179                	addi	sp,sp,464
    80005f2a:	8082                	ret

0000000080005f2c <sys_pipe>:

uint64
sys_pipe(void)
{
    80005f2c:	7139                	addi	sp,sp,-64
    80005f2e:	fc06                	sd	ra,56(sp)
    80005f30:	f822                	sd	s0,48(sp)
    80005f32:	f426                	sd	s1,40(sp)
    80005f34:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    80005f36:	ffffc097          	auipc	ra,0xffffc
    80005f3a:	aa8080e7          	jalr	-1368(ra) # 800019de <myproc>
    80005f3e:	84aa                	mv	s1,a0

  if(argaddr(0, &fdarray) < 0)
    80005f40:	fd840593          	addi	a1,s0,-40
    80005f44:	4501                	li	a0,0
    80005f46:	ffffd097          	auipc	ra,0xffffd
    80005f4a:	08e080e7          	jalr	142(ra) # 80002fd4 <argaddr>
    return -1;
    80005f4e:	57fd                	li	a5,-1
  if(argaddr(0, &fdarray) < 0)
    80005f50:	0e054063          	bltz	a0,80006030 <sys_pipe+0x104>
  if(pipealloc(&rf, &wf) < 0)
    80005f54:	fc840593          	addi	a1,s0,-56
    80005f58:	fd040513          	addi	a0,s0,-48
    80005f5c:	fffff097          	auipc	ra,0xfffff
    80005f60:	dfc080e7          	jalr	-516(ra) # 80004d58 <pipealloc>
    return -1;
    80005f64:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    80005f66:	0c054563          	bltz	a0,80006030 <sys_pipe+0x104>
  fd0 = -1;
    80005f6a:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    80005f6e:	fd043503          	ld	a0,-48(s0)
    80005f72:	fffff097          	auipc	ra,0xfffff
    80005f76:	508080e7          	jalr	1288(ra) # 8000547a <fdalloc>
    80005f7a:	fca42223          	sw	a0,-60(s0)
    80005f7e:	08054c63          	bltz	a0,80006016 <sys_pipe+0xea>
    80005f82:	fc843503          	ld	a0,-56(s0)
    80005f86:	fffff097          	auipc	ra,0xfffff
    80005f8a:	4f4080e7          	jalr	1268(ra) # 8000547a <fdalloc>
    80005f8e:	fca42023          	sw	a0,-64(s0)
    80005f92:	06054863          	bltz	a0,80006002 <sys_pipe+0xd6>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005f96:	4691                	li	a3,4
    80005f98:	fc440613          	addi	a2,s0,-60
    80005f9c:	fd843583          	ld	a1,-40(s0)
    80005fa0:	78a8                	ld	a0,112(s1)
    80005fa2:	ffffb097          	auipc	ra,0xffffb
    80005fa6:	6d8080e7          	jalr	1752(ra) # 8000167a <copyout>
    80005faa:	02054063          	bltz	a0,80005fca <sys_pipe+0x9e>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    80005fae:	4691                	li	a3,4
    80005fb0:	fc040613          	addi	a2,s0,-64
    80005fb4:	fd843583          	ld	a1,-40(s0)
    80005fb8:	0591                	addi	a1,a1,4
    80005fba:	78a8                	ld	a0,112(s1)
    80005fbc:	ffffb097          	auipc	ra,0xffffb
    80005fc0:	6be080e7          	jalr	1726(ra) # 8000167a <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  return 0;
    80005fc4:	4781                	li	a5,0
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005fc6:	06055563          	bgez	a0,80006030 <sys_pipe+0x104>
    p->ofile[fd0] = 0;
    80005fca:	fc442783          	lw	a5,-60(s0)
    80005fce:	07f9                	addi	a5,a5,30
    80005fd0:	078e                	slli	a5,a5,0x3
    80005fd2:	97a6                	add	a5,a5,s1
    80005fd4:	0007b023          	sd	zero,0(a5)
    p->ofile[fd1] = 0;
    80005fd8:	fc042503          	lw	a0,-64(s0)
    80005fdc:	0579                	addi	a0,a0,30
    80005fde:	050e                	slli	a0,a0,0x3
    80005fe0:	9526                	add	a0,a0,s1
    80005fe2:	00053023          	sd	zero,0(a0)
    fileclose(rf);
    80005fe6:	fd043503          	ld	a0,-48(s0)
    80005fea:	fffff097          	auipc	ra,0xfffff
    80005fee:	a3e080e7          	jalr	-1474(ra) # 80004a28 <fileclose>
    fileclose(wf);
    80005ff2:	fc843503          	ld	a0,-56(s0)
    80005ff6:	fffff097          	auipc	ra,0xfffff
    80005ffa:	a32080e7          	jalr	-1486(ra) # 80004a28 <fileclose>
    return -1;
    80005ffe:	57fd                	li	a5,-1
    80006000:	a805                	j	80006030 <sys_pipe+0x104>
    if(fd0 >= 0)
    80006002:	fc442783          	lw	a5,-60(s0)
    80006006:	0007c863          	bltz	a5,80006016 <sys_pipe+0xea>
      p->ofile[fd0] = 0;
    8000600a:	01e78513          	addi	a0,a5,30
    8000600e:	050e                	slli	a0,a0,0x3
    80006010:	9526                	add	a0,a0,s1
    80006012:	00053023          	sd	zero,0(a0)
    fileclose(rf);
    80006016:	fd043503          	ld	a0,-48(s0)
    8000601a:	fffff097          	auipc	ra,0xfffff
    8000601e:	a0e080e7          	jalr	-1522(ra) # 80004a28 <fileclose>
    fileclose(wf);
    80006022:	fc843503          	ld	a0,-56(s0)
    80006026:	fffff097          	auipc	ra,0xfffff
    8000602a:	a02080e7          	jalr	-1534(ra) # 80004a28 <fileclose>
    return -1;
    8000602e:	57fd                	li	a5,-1
}
    80006030:	853e                	mv	a0,a5
    80006032:	70e2                	ld	ra,56(sp)
    80006034:	7442                	ld	s0,48(sp)
    80006036:	74a2                	ld	s1,40(sp)
    80006038:	6121                	addi	sp,sp,64
    8000603a:	8082                	ret
    8000603c:	0000                	unimp
	...

0000000080006040 <kernelvec>:
    80006040:	7111                	addi	sp,sp,-256
    80006042:	e006                	sd	ra,0(sp)
    80006044:	e40a                	sd	sp,8(sp)
    80006046:	e80e                	sd	gp,16(sp)
    80006048:	ec12                	sd	tp,24(sp)
    8000604a:	f016                	sd	t0,32(sp)
    8000604c:	f41a                	sd	t1,40(sp)
    8000604e:	f81e                	sd	t2,48(sp)
    80006050:	fc22                	sd	s0,56(sp)
    80006052:	e0a6                	sd	s1,64(sp)
    80006054:	e4aa                	sd	a0,72(sp)
    80006056:	e8ae                	sd	a1,80(sp)
    80006058:	ecb2                	sd	a2,88(sp)
    8000605a:	f0b6                	sd	a3,96(sp)
    8000605c:	f4ba                	sd	a4,104(sp)
    8000605e:	f8be                	sd	a5,112(sp)
    80006060:	fcc2                	sd	a6,120(sp)
    80006062:	e146                	sd	a7,128(sp)
    80006064:	e54a                	sd	s2,136(sp)
    80006066:	e94e                	sd	s3,144(sp)
    80006068:	ed52                	sd	s4,152(sp)
    8000606a:	f156                	sd	s5,160(sp)
    8000606c:	f55a                	sd	s6,168(sp)
    8000606e:	f95e                	sd	s7,176(sp)
    80006070:	fd62                	sd	s8,184(sp)
    80006072:	e1e6                	sd	s9,192(sp)
    80006074:	e5ea                	sd	s10,200(sp)
    80006076:	e9ee                	sd	s11,208(sp)
    80006078:	edf2                	sd	t3,216(sp)
    8000607a:	f1f6                	sd	t4,224(sp)
    8000607c:	f5fa                	sd	t5,232(sp)
    8000607e:	f9fe                	sd	t6,240(sp)
    80006080:	d65fc0ef          	jal	ra,80002de4 <kerneltrap>
    80006084:	6082                	ld	ra,0(sp)
    80006086:	6122                	ld	sp,8(sp)
    80006088:	61c2                	ld	gp,16(sp)
    8000608a:	7282                	ld	t0,32(sp)
    8000608c:	7322                	ld	t1,40(sp)
    8000608e:	73c2                	ld	t2,48(sp)
    80006090:	7462                	ld	s0,56(sp)
    80006092:	6486                	ld	s1,64(sp)
    80006094:	6526                	ld	a0,72(sp)
    80006096:	65c6                	ld	a1,80(sp)
    80006098:	6666                	ld	a2,88(sp)
    8000609a:	7686                	ld	a3,96(sp)
    8000609c:	7726                	ld	a4,104(sp)
    8000609e:	77c6                	ld	a5,112(sp)
    800060a0:	7866                	ld	a6,120(sp)
    800060a2:	688a                	ld	a7,128(sp)
    800060a4:	692a                	ld	s2,136(sp)
    800060a6:	69ca                	ld	s3,144(sp)
    800060a8:	6a6a                	ld	s4,152(sp)
    800060aa:	7a8a                	ld	s5,160(sp)
    800060ac:	7b2a                	ld	s6,168(sp)
    800060ae:	7bca                	ld	s7,176(sp)
    800060b0:	7c6a                	ld	s8,184(sp)
    800060b2:	6c8e                	ld	s9,192(sp)
    800060b4:	6d2e                	ld	s10,200(sp)
    800060b6:	6dce                	ld	s11,208(sp)
    800060b8:	6e6e                	ld	t3,216(sp)
    800060ba:	7e8e                	ld	t4,224(sp)
    800060bc:	7f2e                	ld	t5,232(sp)
    800060be:	7fce                	ld	t6,240(sp)
    800060c0:	6111                	addi	sp,sp,256
    800060c2:	10200073          	sret
    800060c6:	00000013          	nop
    800060ca:	00000013          	nop
    800060ce:	0001                	nop

00000000800060d0 <timervec>:
    800060d0:	34051573          	csrrw	a0,mscratch,a0
    800060d4:	e10c                	sd	a1,0(a0)
    800060d6:	e510                	sd	a2,8(a0)
    800060d8:	e914                	sd	a3,16(a0)
    800060da:	6d0c                	ld	a1,24(a0)
    800060dc:	7110                	ld	a2,32(a0)
    800060de:	6194                	ld	a3,0(a1)
    800060e0:	96b2                	add	a3,a3,a2
    800060e2:	e194                	sd	a3,0(a1)
    800060e4:	4589                	li	a1,2
    800060e6:	14459073          	csrw	sip,a1
    800060ea:	6914                	ld	a3,16(a0)
    800060ec:	6510                	ld	a2,8(a0)
    800060ee:	610c                	ld	a1,0(a0)
    800060f0:	34051573          	csrrw	a0,mscratch,a0
    800060f4:	30200073          	mret
	...

00000000800060fa <plicinit>:
// the riscv Platform Level Interrupt Controller (PLIC).
//

void
plicinit(void)
{
    800060fa:	1141                	addi	sp,sp,-16
    800060fc:	e422                	sd	s0,8(sp)
    800060fe:	0800                	addi	s0,sp,16
  // set desired IRQ priorities non-zero (otherwise disabled).
  *(uint32*)(PLIC + UART0_IRQ*4) = 1;
    80006100:	0c0007b7          	lui	a5,0xc000
    80006104:	4705                	li	a4,1
    80006106:	d798                	sw	a4,40(a5)
  *(uint32*)(PLIC + VIRTIO0_IRQ*4) = 1;
    80006108:	c3d8                	sw	a4,4(a5)
}
    8000610a:	6422                	ld	s0,8(sp)
    8000610c:	0141                	addi	sp,sp,16
    8000610e:	8082                	ret

0000000080006110 <plicinithart>:

void
plicinithart(void)
{
    80006110:	1141                	addi	sp,sp,-16
    80006112:	e406                	sd	ra,8(sp)
    80006114:	e022                	sd	s0,0(sp)
    80006116:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80006118:	ffffc097          	auipc	ra,0xffffc
    8000611c:	89a080e7          	jalr	-1894(ra) # 800019b2 <cpuid>
  
  // set uart's enable bit for this hart's S-mode. 
  *(uint32*)PLIC_SENABLE(hart)= (1 << UART0_IRQ) | (1 << VIRTIO0_IRQ);
    80006120:	0085171b          	slliw	a4,a0,0x8
    80006124:	0c0027b7          	lui	a5,0xc002
    80006128:	97ba                	add	a5,a5,a4
    8000612a:	40200713          	li	a4,1026
    8000612e:	08e7a023          	sw	a4,128(a5) # c002080 <_entry-0x73ffdf80>

  // set this hart's S-mode priority threshold to 0.
  *(uint32*)PLIC_SPRIORITY(hart) = 0;
    80006132:	00d5151b          	slliw	a0,a0,0xd
    80006136:	0c2017b7          	lui	a5,0xc201
    8000613a:	953e                	add	a0,a0,a5
    8000613c:	00052023          	sw	zero,0(a0)
}
    80006140:	60a2                	ld	ra,8(sp)
    80006142:	6402                	ld	s0,0(sp)
    80006144:	0141                	addi	sp,sp,16
    80006146:	8082                	ret

0000000080006148 <plic_claim>:

// ask the PLIC what interrupt we should serve.
int
plic_claim(void)
{
    80006148:	1141                	addi	sp,sp,-16
    8000614a:	e406                	sd	ra,8(sp)
    8000614c:	e022                	sd	s0,0(sp)
    8000614e:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80006150:	ffffc097          	auipc	ra,0xffffc
    80006154:	862080e7          	jalr	-1950(ra) # 800019b2 <cpuid>
  int irq = *(uint32*)PLIC_SCLAIM(hart);
    80006158:	00d5179b          	slliw	a5,a0,0xd
    8000615c:	0c201537          	lui	a0,0xc201
    80006160:	953e                	add	a0,a0,a5
  return irq;
}
    80006162:	4148                	lw	a0,4(a0)
    80006164:	60a2                	ld	ra,8(sp)
    80006166:	6402                	ld	s0,0(sp)
    80006168:	0141                	addi	sp,sp,16
    8000616a:	8082                	ret

000000008000616c <plic_complete>:

// tell the PLIC we've served this IRQ.
void
plic_complete(int irq)
{
    8000616c:	1101                	addi	sp,sp,-32
    8000616e:	ec06                	sd	ra,24(sp)
    80006170:	e822                	sd	s0,16(sp)
    80006172:	e426                	sd	s1,8(sp)
    80006174:	1000                	addi	s0,sp,32
    80006176:	84aa                	mv	s1,a0
  int hart = cpuid();
    80006178:	ffffc097          	auipc	ra,0xffffc
    8000617c:	83a080e7          	jalr	-1990(ra) # 800019b2 <cpuid>
  *(uint32*)PLIC_SCLAIM(hart) = irq;
    80006180:	00d5151b          	slliw	a0,a0,0xd
    80006184:	0c2017b7          	lui	a5,0xc201
    80006188:	97aa                	add	a5,a5,a0
    8000618a:	c3c4                	sw	s1,4(a5)
}
    8000618c:	60e2                	ld	ra,24(sp)
    8000618e:	6442                	ld	s0,16(sp)
    80006190:	64a2                	ld	s1,8(sp)
    80006192:	6105                	addi	sp,sp,32
    80006194:	8082                	ret

0000000080006196 <free_desc>:
}

// mark a descriptor as free.
static void
free_desc(int i)
{
    80006196:	1141                	addi	sp,sp,-16
    80006198:	e406                	sd	ra,8(sp)
    8000619a:	e022                	sd	s0,0(sp)
    8000619c:	0800                	addi	s0,sp,16
  if(i >= NUM)
    8000619e:	479d                	li	a5,7
    800061a0:	06a7c963          	blt	a5,a0,80006212 <free_desc+0x7c>
    panic("free_desc 1");
  if(disk.free[i])
    800061a4:	0001d797          	auipc	a5,0x1d
    800061a8:	e5c78793          	addi	a5,a5,-420 # 80023000 <disk>
    800061ac:	00a78733          	add	a4,a5,a0
    800061b0:	6789                	lui	a5,0x2
    800061b2:	97ba                	add	a5,a5,a4
    800061b4:	0187c783          	lbu	a5,24(a5) # 2018 <_entry-0x7fffdfe8>
    800061b8:	e7ad                	bnez	a5,80006222 <free_desc+0x8c>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    800061ba:	00451793          	slli	a5,a0,0x4
    800061be:	0001f717          	auipc	a4,0x1f
    800061c2:	e4270713          	addi	a4,a4,-446 # 80025000 <disk+0x2000>
    800061c6:	6314                	ld	a3,0(a4)
    800061c8:	96be                	add	a3,a3,a5
    800061ca:	0006b023          	sd	zero,0(a3)
  disk.desc[i].len = 0;
    800061ce:	6314                	ld	a3,0(a4)
    800061d0:	96be                	add	a3,a3,a5
    800061d2:	0006a423          	sw	zero,8(a3)
  disk.desc[i].flags = 0;
    800061d6:	6314                	ld	a3,0(a4)
    800061d8:	96be                	add	a3,a3,a5
    800061da:	00069623          	sh	zero,12(a3)
  disk.desc[i].next = 0;
    800061de:	6318                	ld	a4,0(a4)
    800061e0:	97ba                	add	a5,a5,a4
    800061e2:	00079723          	sh	zero,14(a5)
  disk.free[i] = 1;
    800061e6:	0001d797          	auipc	a5,0x1d
    800061ea:	e1a78793          	addi	a5,a5,-486 # 80023000 <disk>
    800061ee:	97aa                	add	a5,a5,a0
    800061f0:	6509                	lui	a0,0x2
    800061f2:	953e                	add	a0,a0,a5
    800061f4:	4785                	li	a5,1
    800061f6:	00f50c23          	sb	a5,24(a0) # 2018 <_entry-0x7fffdfe8>
  wakeup(&disk.free[0]);
    800061fa:	0001f517          	auipc	a0,0x1f
    800061fe:	e1e50513          	addi	a0,a0,-482 # 80025018 <disk+0x2018>
    80006202:	ffffc097          	auipc	ra,0xffffc
    80006206:	344080e7          	jalr	836(ra) # 80002546 <wakeup>
}
    8000620a:	60a2                	ld	ra,8(sp)
    8000620c:	6402                	ld	s0,0(sp)
    8000620e:	0141                	addi	sp,sp,16
    80006210:	8082                	ret
    panic("free_desc 1");
    80006212:	00002517          	auipc	a0,0x2
    80006216:	62650513          	addi	a0,a0,1574 # 80008838 <syscalls+0x338>
    8000621a:	ffffa097          	auipc	ra,0xffffa
    8000621e:	324080e7          	jalr	804(ra) # 8000053e <panic>
    panic("free_desc 2");
    80006222:	00002517          	auipc	a0,0x2
    80006226:	62650513          	addi	a0,a0,1574 # 80008848 <syscalls+0x348>
    8000622a:	ffffa097          	auipc	ra,0xffffa
    8000622e:	314080e7          	jalr	788(ra) # 8000053e <panic>

0000000080006232 <virtio_disk_init>:
{
    80006232:	1101                	addi	sp,sp,-32
    80006234:	ec06                	sd	ra,24(sp)
    80006236:	e822                	sd	s0,16(sp)
    80006238:	e426                	sd	s1,8(sp)
    8000623a:	1000                	addi	s0,sp,32
  initlock(&disk.vdisk_lock, "virtio_disk");
    8000623c:	00002597          	auipc	a1,0x2
    80006240:	61c58593          	addi	a1,a1,1564 # 80008858 <syscalls+0x358>
    80006244:	0001f517          	auipc	a0,0x1f
    80006248:	ee450513          	addi	a0,a0,-284 # 80025128 <disk+0x2128>
    8000624c:	ffffb097          	auipc	ra,0xffffb
    80006250:	908080e7          	jalr	-1784(ra) # 80000b54 <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80006254:	100017b7          	lui	a5,0x10001
    80006258:	4398                	lw	a4,0(a5)
    8000625a:	2701                	sext.w	a4,a4
    8000625c:	747277b7          	lui	a5,0x74727
    80006260:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80006264:	0ef71163          	bne	a4,a5,80006346 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_VERSION) != 1 ||
    80006268:	100017b7          	lui	a5,0x10001
    8000626c:	43dc                	lw	a5,4(a5)
    8000626e:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80006270:	4705                	li	a4,1
    80006272:	0ce79a63          	bne	a5,a4,80006346 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    80006276:	100017b7          	lui	a5,0x10001
    8000627a:	479c                	lw	a5,8(a5)
    8000627c:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 1 ||
    8000627e:	4709                	li	a4,2
    80006280:	0ce79363          	bne	a5,a4,80006346 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_VENDOR_ID) != 0x554d4551){
    80006284:	100017b7          	lui	a5,0x10001
    80006288:	47d8                	lw	a4,12(a5)
    8000628a:	2701                	sext.w	a4,a4
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    8000628c:	554d47b7          	lui	a5,0x554d4
    80006290:	55178793          	addi	a5,a5,1361 # 554d4551 <_entry-0x2ab2baaf>
    80006294:	0af71963          	bne	a4,a5,80006346 <virtio_disk_init+0x114>
  *R(VIRTIO_MMIO_STATUS) = status;
    80006298:	100017b7          	lui	a5,0x10001
    8000629c:	4705                	li	a4,1
    8000629e:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    800062a0:	470d                	li	a4,3
    800062a2:	dbb8                	sw	a4,112(a5)
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    800062a4:	4b94                	lw	a3,16(a5)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    800062a6:	c7ffe737          	lui	a4,0xc7ffe
    800062aa:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fd875f>
    800062ae:	8f75                	and	a4,a4,a3
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    800062b0:	2701                	sext.w	a4,a4
    800062b2:	d398                	sw	a4,32(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    800062b4:	472d                	li	a4,11
    800062b6:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    800062b8:	473d                	li	a4,15
    800062ba:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_GUEST_PAGE_SIZE) = PGSIZE;
    800062bc:	6705                	lui	a4,0x1
    800062be:	d798                	sw	a4,40(a5)
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    800062c0:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    800062c4:	5bdc                	lw	a5,52(a5)
    800062c6:	2781                	sext.w	a5,a5
  if(max == 0)
    800062c8:	c7d9                	beqz	a5,80006356 <virtio_disk_init+0x124>
  if(max < NUM)
    800062ca:	471d                	li	a4,7
    800062cc:	08f77d63          	bgeu	a4,a5,80006366 <virtio_disk_init+0x134>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    800062d0:	100014b7          	lui	s1,0x10001
    800062d4:	47a1                	li	a5,8
    800062d6:	dc9c                	sw	a5,56(s1)
  memset(disk.pages, 0, sizeof(disk.pages));
    800062d8:	6609                	lui	a2,0x2
    800062da:	4581                	li	a1,0
    800062dc:	0001d517          	auipc	a0,0x1d
    800062e0:	d2450513          	addi	a0,a0,-732 # 80023000 <disk>
    800062e4:	ffffb097          	auipc	ra,0xffffb
    800062e8:	9fc080e7          	jalr	-1540(ra) # 80000ce0 <memset>
  *R(VIRTIO_MMIO_QUEUE_PFN) = ((uint64)disk.pages) >> PGSHIFT;
    800062ec:	0001d717          	auipc	a4,0x1d
    800062f0:	d1470713          	addi	a4,a4,-748 # 80023000 <disk>
    800062f4:	00c75793          	srli	a5,a4,0xc
    800062f8:	2781                	sext.w	a5,a5
    800062fa:	c0bc                	sw	a5,64(s1)
  disk.desc = (struct virtq_desc *) disk.pages;
    800062fc:	0001f797          	auipc	a5,0x1f
    80006300:	d0478793          	addi	a5,a5,-764 # 80025000 <disk+0x2000>
    80006304:	e398                	sd	a4,0(a5)
  disk.avail = (struct virtq_avail *)(disk.pages + NUM*sizeof(struct virtq_desc));
    80006306:	0001d717          	auipc	a4,0x1d
    8000630a:	d7a70713          	addi	a4,a4,-646 # 80023080 <disk+0x80>
    8000630e:	e798                	sd	a4,8(a5)
  disk.used = (struct virtq_used *) (disk.pages + PGSIZE);
    80006310:	0001e717          	auipc	a4,0x1e
    80006314:	cf070713          	addi	a4,a4,-784 # 80024000 <disk+0x1000>
    80006318:	eb98                	sd	a4,16(a5)
    disk.free[i] = 1;
    8000631a:	4705                	li	a4,1
    8000631c:	00e78c23          	sb	a4,24(a5)
    80006320:	00e78ca3          	sb	a4,25(a5)
    80006324:	00e78d23          	sb	a4,26(a5)
    80006328:	00e78da3          	sb	a4,27(a5)
    8000632c:	00e78e23          	sb	a4,28(a5)
    80006330:	00e78ea3          	sb	a4,29(a5)
    80006334:	00e78f23          	sb	a4,30(a5)
    80006338:	00e78fa3          	sb	a4,31(a5)
}
    8000633c:	60e2                	ld	ra,24(sp)
    8000633e:	6442                	ld	s0,16(sp)
    80006340:	64a2                	ld	s1,8(sp)
    80006342:	6105                	addi	sp,sp,32
    80006344:	8082                	ret
    panic("could not find virtio disk");
    80006346:	00002517          	auipc	a0,0x2
    8000634a:	52250513          	addi	a0,a0,1314 # 80008868 <syscalls+0x368>
    8000634e:	ffffa097          	auipc	ra,0xffffa
    80006352:	1f0080e7          	jalr	496(ra) # 8000053e <panic>
    panic("virtio disk has no queue 0");
    80006356:	00002517          	auipc	a0,0x2
    8000635a:	53250513          	addi	a0,a0,1330 # 80008888 <syscalls+0x388>
    8000635e:	ffffa097          	auipc	ra,0xffffa
    80006362:	1e0080e7          	jalr	480(ra) # 8000053e <panic>
    panic("virtio disk max queue too short");
    80006366:	00002517          	auipc	a0,0x2
    8000636a:	54250513          	addi	a0,a0,1346 # 800088a8 <syscalls+0x3a8>
    8000636e:	ffffa097          	auipc	ra,0xffffa
    80006372:	1d0080e7          	jalr	464(ra) # 8000053e <panic>

0000000080006376 <virtio_disk_rw>:
  return 0;
}

void
virtio_disk_rw(struct buf *b, int write)
{
    80006376:	7159                	addi	sp,sp,-112
    80006378:	f486                	sd	ra,104(sp)
    8000637a:	f0a2                	sd	s0,96(sp)
    8000637c:	eca6                	sd	s1,88(sp)
    8000637e:	e8ca                	sd	s2,80(sp)
    80006380:	e4ce                	sd	s3,72(sp)
    80006382:	e0d2                	sd	s4,64(sp)
    80006384:	fc56                	sd	s5,56(sp)
    80006386:	f85a                	sd	s6,48(sp)
    80006388:	f45e                	sd	s7,40(sp)
    8000638a:	f062                	sd	s8,32(sp)
    8000638c:	ec66                	sd	s9,24(sp)
    8000638e:	e86a                	sd	s10,16(sp)
    80006390:	1880                	addi	s0,sp,112
    80006392:	892a                	mv	s2,a0
    80006394:	8d2e                	mv	s10,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    80006396:	00c52c83          	lw	s9,12(a0)
    8000639a:	001c9c9b          	slliw	s9,s9,0x1
    8000639e:	1c82                	slli	s9,s9,0x20
    800063a0:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    800063a4:	0001f517          	auipc	a0,0x1f
    800063a8:	d8450513          	addi	a0,a0,-636 # 80025128 <disk+0x2128>
    800063ac:	ffffb097          	auipc	ra,0xffffb
    800063b0:	838080e7          	jalr	-1992(ra) # 80000be4 <acquire>
  for(int i = 0; i < 3; i++){
    800063b4:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    800063b6:	4c21                	li	s8,8
      disk.free[i] = 0;
    800063b8:	0001db97          	auipc	s7,0x1d
    800063bc:	c48b8b93          	addi	s7,s7,-952 # 80023000 <disk>
    800063c0:	6b09                	lui	s6,0x2
  for(int i = 0; i < 3; i++){
    800063c2:	4a8d                	li	s5,3
  for(int i = 0; i < NUM; i++){
    800063c4:	8a4e                	mv	s4,s3
    800063c6:	a051                	j	8000644a <virtio_disk_rw+0xd4>
      disk.free[i] = 0;
    800063c8:	00fb86b3          	add	a3,s7,a5
    800063cc:	96da                	add	a3,a3,s6
    800063ce:	00068c23          	sb	zero,24(a3)
    idx[i] = alloc_desc();
    800063d2:	c21c                	sw	a5,0(a2)
    if(idx[i] < 0){
    800063d4:	0207c563          	bltz	a5,800063fe <virtio_disk_rw+0x88>
  for(int i = 0; i < 3; i++){
    800063d8:	2485                	addiw	s1,s1,1
    800063da:	0711                	addi	a4,a4,4
    800063dc:	25548063          	beq	s1,s5,8000661c <virtio_disk_rw+0x2a6>
    idx[i] = alloc_desc();
    800063e0:	863a                	mv	a2,a4
  for(int i = 0; i < NUM; i++){
    800063e2:	0001f697          	auipc	a3,0x1f
    800063e6:	c3668693          	addi	a3,a3,-970 # 80025018 <disk+0x2018>
    800063ea:	87d2                	mv	a5,s4
    if(disk.free[i]){
    800063ec:	0006c583          	lbu	a1,0(a3)
    800063f0:	fde1                	bnez	a1,800063c8 <virtio_disk_rw+0x52>
  for(int i = 0; i < NUM; i++){
    800063f2:	2785                	addiw	a5,a5,1
    800063f4:	0685                	addi	a3,a3,1
    800063f6:	ff879be3          	bne	a5,s8,800063ec <virtio_disk_rw+0x76>
    idx[i] = alloc_desc();
    800063fa:	57fd                	li	a5,-1
    800063fc:	c21c                	sw	a5,0(a2)
      for(int j = 0; j < i; j++)
    800063fe:	02905a63          	blez	s1,80006432 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    80006402:	f9042503          	lw	a0,-112(s0)
    80006406:	00000097          	auipc	ra,0x0
    8000640a:	d90080e7          	jalr	-624(ra) # 80006196 <free_desc>
      for(int j = 0; j < i; j++)
    8000640e:	4785                	li	a5,1
    80006410:	0297d163          	bge	a5,s1,80006432 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    80006414:	f9442503          	lw	a0,-108(s0)
    80006418:	00000097          	auipc	ra,0x0
    8000641c:	d7e080e7          	jalr	-642(ra) # 80006196 <free_desc>
      for(int j = 0; j < i; j++)
    80006420:	4789                	li	a5,2
    80006422:	0097d863          	bge	a5,s1,80006432 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    80006426:	f9842503          	lw	a0,-104(s0)
    8000642a:	00000097          	auipc	ra,0x0
    8000642e:	d6c080e7          	jalr	-660(ra) # 80006196 <free_desc>
  int idx[3];
  while(1){
    if(alloc3_desc(idx) == 0) {
      break;
    }
    sleep(&disk.free[0], &disk.vdisk_lock);
    80006432:	0001f597          	auipc	a1,0x1f
    80006436:	cf658593          	addi	a1,a1,-778 # 80025128 <disk+0x2128>
    8000643a:	0001f517          	auipc	a0,0x1f
    8000643e:	bde50513          	addi	a0,a0,-1058 # 80025018 <disk+0x2018>
    80006442:	ffffc097          	auipc	ra,0xffffc
    80006446:	f6e080e7          	jalr	-146(ra) # 800023b0 <sleep>
  for(int i = 0; i < 3; i++){
    8000644a:	f9040713          	addi	a4,s0,-112
    8000644e:	84ce                	mv	s1,s3
    80006450:	bf41                	j	800063e0 <virtio_disk_rw+0x6a>
  // qemu's virtio-blk.c reads them.

  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];

  if(write)
    buf0->type = VIRTIO_BLK_T_OUT; // write the disk
    80006452:	20058713          	addi	a4,a1,512
    80006456:	00471693          	slli	a3,a4,0x4
    8000645a:	0001d717          	auipc	a4,0x1d
    8000645e:	ba670713          	addi	a4,a4,-1114 # 80023000 <disk>
    80006462:	9736                	add	a4,a4,a3
    80006464:	4685                	li	a3,1
    80006466:	0ad72423          	sw	a3,168(a4)
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    8000646a:	20058713          	addi	a4,a1,512
    8000646e:	00471693          	slli	a3,a4,0x4
    80006472:	0001d717          	auipc	a4,0x1d
    80006476:	b8e70713          	addi	a4,a4,-1138 # 80023000 <disk>
    8000647a:	9736                	add	a4,a4,a3
    8000647c:	0a072623          	sw	zero,172(a4)
  buf0->sector = sector;
    80006480:	0b973823          	sd	s9,176(a4)

  disk.desc[idx[0]].addr = (uint64) buf0;
    80006484:	7679                	lui	a2,0xffffe
    80006486:	963e                	add	a2,a2,a5
    80006488:	0001f697          	auipc	a3,0x1f
    8000648c:	b7868693          	addi	a3,a3,-1160 # 80025000 <disk+0x2000>
    80006490:	6298                	ld	a4,0(a3)
    80006492:	9732                	add	a4,a4,a2
    80006494:	e308                	sd	a0,0(a4)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    80006496:	6298                	ld	a4,0(a3)
    80006498:	9732                	add	a4,a4,a2
    8000649a:	4541                	li	a0,16
    8000649c:	c708                	sw	a0,8(a4)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    8000649e:	6298                	ld	a4,0(a3)
    800064a0:	9732                	add	a4,a4,a2
    800064a2:	4505                	li	a0,1
    800064a4:	00a71623          	sh	a0,12(a4)
  disk.desc[idx[0]].next = idx[1];
    800064a8:	f9442703          	lw	a4,-108(s0)
    800064ac:	6288                	ld	a0,0(a3)
    800064ae:	962a                	add	a2,a2,a0
    800064b0:	00e61723          	sh	a4,14(a2) # ffffffffffffe00e <end+0xffffffff7ffd800e>

  disk.desc[idx[1]].addr = (uint64) b->data;
    800064b4:	0712                	slli	a4,a4,0x4
    800064b6:	6290                	ld	a2,0(a3)
    800064b8:	963a                	add	a2,a2,a4
    800064ba:	05890513          	addi	a0,s2,88
    800064be:	e208                	sd	a0,0(a2)
  disk.desc[idx[1]].len = BSIZE;
    800064c0:	6294                	ld	a3,0(a3)
    800064c2:	96ba                	add	a3,a3,a4
    800064c4:	40000613          	li	a2,1024
    800064c8:	c690                	sw	a2,8(a3)
  if(write)
    800064ca:	140d0063          	beqz	s10,8000660a <virtio_disk_rw+0x294>
    disk.desc[idx[1]].flags = 0; // device reads b->data
    800064ce:	0001f697          	auipc	a3,0x1f
    800064d2:	b326b683          	ld	a3,-1230(a3) # 80025000 <disk+0x2000>
    800064d6:	96ba                	add	a3,a3,a4
    800064d8:	00069623          	sh	zero,12(a3)
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    800064dc:	0001d817          	auipc	a6,0x1d
    800064e0:	b2480813          	addi	a6,a6,-1244 # 80023000 <disk>
    800064e4:	0001f517          	auipc	a0,0x1f
    800064e8:	b1c50513          	addi	a0,a0,-1252 # 80025000 <disk+0x2000>
    800064ec:	6114                	ld	a3,0(a0)
    800064ee:	96ba                	add	a3,a3,a4
    800064f0:	00c6d603          	lhu	a2,12(a3)
    800064f4:	00166613          	ori	a2,a2,1
    800064f8:	00c69623          	sh	a2,12(a3)
  disk.desc[idx[1]].next = idx[2];
    800064fc:	f9842683          	lw	a3,-104(s0)
    80006500:	6110                	ld	a2,0(a0)
    80006502:	9732                	add	a4,a4,a2
    80006504:	00d71723          	sh	a3,14(a4)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    80006508:	20058613          	addi	a2,a1,512
    8000650c:	0612                	slli	a2,a2,0x4
    8000650e:	9642                	add	a2,a2,a6
    80006510:	577d                	li	a4,-1
    80006512:	02e60823          	sb	a4,48(a2)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    80006516:	00469713          	slli	a4,a3,0x4
    8000651a:	6114                	ld	a3,0(a0)
    8000651c:	96ba                	add	a3,a3,a4
    8000651e:	03078793          	addi	a5,a5,48
    80006522:	97c2                	add	a5,a5,a6
    80006524:	e29c                	sd	a5,0(a3)
  disk.desc[idx[2]].len = 1;
    80006526:	611c                	ld	a5,0(a0)
    80006528:	97ba                	add	a5,a5,a4
    8000652a:	4685                	li	a3,1
    8000652c:	c794                	sw	a3,8(a5)
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    8000652e:	611c                	ld	a5,0(a0)
    80006530:	97ba                	add	a5,a5,a4
    80006532:	4809                	li	a6,2
    80006534:	01079623          	sh	a6,12(a5)
  disk.desc[idx[2]].next = 0;
    80006538:	611c                	ld	a5,0(a0)
    8000653a:	973e                	add	a4,a4,a5
    8000653c:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    80006540:	00d92223          	sw	a3,4(s2)
  disk.info[idx[0]].b = b;
    80006544:	03263423          	sd	s2,40(a2)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    80006548:	6518                	ld	a4,8(a0)
    8000654a:	00275783          	lhu	a5,2(a4)
    8000654e:	8b9d                	andi	a5,a5,7
    80006550:	0786                	slli	a5,a5,0x1
    80006552:	97ba                	add	a5,a5,a4
    80006554:	00b79223          	sh	a1,4(a5)

  __sync_synchronize();
    80006558:	0ff0000f          	fence

  // tell the device another avail ring entry is available.
  disk.avail->idx += 1; // not % NUM ...
    8000655c:	6518                	ld	a4,8(a0)
    8000655e:	00275783          	lhu	a5,2(a4)
    80006562:	2785                	addiw	a5,a5,1
    80006564:	00f71123          	sh	a5,2(a4)

  __sync_synchronize();
    80006568:	0ff0000f          	fence

  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
    8000656c:	100017b7          	lui	a5,0x10001
    80006570:	0407a823          	sw	zero,80(a5) # 10001050 <_entry-0x6fffefb0>

  // Wait for virtio_disk_intr() to say request has finished.
  while(b->disk == 1) {
    80006574:	00492703          	lw	a4,4(s2)
    80006578:	4785                	li	a5,1
    8000657a:	02f71163          	bne	a4,a5,8000659c <virtio_disk_rw+0x226>
    sleep(b, &disk.vdisk_lock);
    8000657e:	0001f997          	auipc	s3,0x1f
    80006582:	baa98993          	addi	s3,s3,-1110 # 80025128 <disk+0x2128>
  while(b->disk == 1) {
    80006586:	4485                	li	s1,1
    sleep(b, &disk.vdisk_lock);
    80006588:	85ce                	mv	a1,s3
    8000658a:	854a                	mv	a0,s2
    8000658c:	ffffc097          	auipc	ra,0xffffc
    80006590:	e24080e7          	jalr	-476(ra) # 800023b0 <sleep>
  while(b->disk == 1) {
    80006594:	00492783          	lw	a5,4(s2)
    80006598:	fe9788e3          	beq	a5,s1,80006588 <virtio_disk_rw+0x212>
  }

  disk.info[idx[0]].b = 0;
    8000659c:	f9042903          	lw	s2,-112(s0)
    800065a0:	20090793          	addi	a5,s2,512
    800065a4:	00479713          	slli	a4,a5,0x4
    800065a8:	0001d797          	auipc	a5,0x1d
    800065ac:	a5878793          	addi	a5,a5,-1448 # 80023000 <disk>
    800065b0:	97ba                	add	a5,a5,a4
    800065b2:	0207b423          	sd	zero,40(a5)
    int flag = disk.desc[i].flags;
    800065b6:	0001f997          	auipc	s3,0x1f
    800065ba:	a4a98993          	addi	s3,s3,-1462 # 80025000 <disk+0x2000>
    800065be:	00491713          	slli	a4,s2,0x4
    800065c2:	0009b783          	ld	a5,0(s3)
    800065c6:	97ba                	add	a5,a5,a4
    800065c8:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    800065cc:	854a                	mv	a0,s2
    800065ce:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    800065d2:	00000097          	auipc	ra,0x0
    800065d6:	bc4080e7          	jalr	-1084(ra) # 80006196 <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    800065da:	8885                	andi	s1,s1,1
    800065dc:	f0ed                	bnez	s1,800065be <virtio_disk_rw+0x248>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    800065de:	0001f517          	auipc	a0,0x1f
    800065e2:	b4a50513          	addi	a0,a0,-1206 # 80025128 <disk+0x2128>
    800065e6:	ffffa097          	auipc	ra,0xffffa
    800065ea:	6b2080e7          	jalr	1714(ra) # 80000c98 <release>
}
    800065ee:	70a6                	ld	ra,104(sp)
    800065f0:	7406                	ld	s0,96(sp)
    800065f2:	64e6                	ld	s1,88(sp)
    800065f4:	6946                	ld	s2,80(sp)
    800065f6:	69a6                	ld	s3,72(sp)
    800065f8:	6a06                	ld	s4,64(sp)
    800065fa:	7ae2                	ld	s5,56(sp)
    800065fc:	7b42                	ld	s6,48(sp)
    800065fe:	7ba2                	ld	s7,40(sp)
    80006600:	7c02                	ld	s8,32(sp)
    80006602:	6ce2                	ld	s9,24(sp)
    80006604:	6d42                	ld	s10,16(sp)
    80006606:	6165                	addi	sp,sp,112
    80006608:	8082                	ret
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
    8000660a:	0001f697          	auipc	a3,0x1f
    8000660e:	9f66b683          	ld	a3,-1546(a3) # 80025000 <disk+0x2000>
    80006612:	96ba                	add	a3,a3,a4
    80006614:	4609                	li	a2,2
    80006616:	00c69623          	sh	a2,12(a3)
    8000661a:	b5c9                	j	800064dc <virtio_disk_rw+0x166>
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    8000661c:	f9042583          	lw	a1,-112(s0)
    80006620:	20058793          	addi	a5,a1,512
    80006624:	0792                	slli	a5,a5,0x4
    80006626:	0001d517          	auipc	a0,0x1d
    8000662a:	a8250513          	addi	a0,a0,-1406 # 800230a8 <disk+0xa8>
    8000662e:	953e                	add	a0,a0,a5
  if(write)
    80006630:	e20d11e3          	bnez	s10,80006452 <virtio_disk_rw+0xdc>
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
    80006634:	20058713          	addi	a4,a1,512
    80006638:	00471693          	slli	a3,a4,0x4
    8000663c:	0001d717          	auipc	a4,0x1d
    80006640:	9c470713          	addi	a4,a4,-1596 # 80023000 <disk>
    80006644:	9736                	add	a4,a4,a3
    80006646:	0a072423          	sw	zero,168(a4)
    8000664a:	b505                	j	8000646a <virtio_disk_rw+0xf4>

000000008000664c <virtio_disk_intr>:

void
virtio_disk_intr()
{
    8000664c:	1101                	addi	sp,sp,-32
    8000664e:	ec06                	sd	ra,24(sp)
    80006650:	e822                	sd	s0,16(sp)
    80006652:	e426                	sd	s1,8(sp)
    80006654:	e04a                	sd	s2,0(sp)
    80006656:	1000                	addi	s0,sp,32
  acquire(&disk.vdisk_lock);
    80006658:	0001f517          	auipc	a0,0x1f
    8000665c:	ad050513          	addi	a0,a0,-1328 # 80025128 <disk+0x2128>
    80006660:	ffffa097          	auipc	ra,0xffffa
    80006664:	584080e7          	jalr	1412(ra) # 80000be4 <acquire>
  // we've seen this interrupt, which the following line does.
  // this may race with the device writing new entries to
  // the "used" ring, in which case we may process the new
  // completion entries in this interrupt, and have nothing to do
  // in the next interrupt, which is harmless.
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80006668:	10001737          	lui	a4,0x10001
    8000666c:	533c                	lw	a5,96(a4)
    8000666e:	8b8d                	andi	a5,a5,3
    80006670:	d37c                	sw	a5,100(a4)

  __sync_synchronize();
    80006672:	0ff0000f          	fence

  // the device increments disk.used->idx when it
  // adds an entry to the used ring.

  while(disk.used_idx != disk.used->idx){
    80006676:	0001f797          	auipc	a5,0x1f
    8000667a:	98a78793          	addi	a5,a5,-1654 # 80025000 <disk+0x2000>
    8000667e:	6b94                	ld	a3,16(a5)
    80006680:	0207d703          	lhu	a4,32(a5)
    80006684:	0026d783          	lhu	a5,2(a3)
    80006688:	06f70163          	beq	a4,a5,800066ea <virtio_disk_intr+0x9e>
    __sync_synchronize();
    int id = disk.used->ring[disk.used_idx % NUM].id;
    8000668c:	0001d917          	auipc	s2,0x1d
    80006690:	97490913          	addi	s2,s2,-1676 # 80023000 <disk>
    80006694:	0001f497          	auipc	s1,0x1f
    80006698:	96c48493          	addi	s1,s1,-1684 # 80025000 <disk+0x2000>
    __sync_synchronize();
    8000669c:	0ff0000f          	fence
    int id = disk.used->ring[disk.used_idx % NUM].id;
    800066a0:	6898                	ld	a4,16(s1)
    800066a2:	0204d783          	lhu	a5,32(s1)
    800066a6:	8b9d                	andi	a5,a5,7
    800066a8:	078e                	slli	a5,a5,0x3
    800066aa:	97ba                	add	a5,a5,a4
    800066ac:	43dc                	lw	a5,4(a5)

    if(disk.info[id].status != 0)
    800066ae:	20078713          	addi	a4,a5,512
    800066b2:	0712                	slli	a4,a4,0x4
    800066b4:	974a                	add	a4,a4,s2
    800066b6:	03074703          	lbu	a4,48(a4) # 10001030 <_entry-0x6fffefd0>
    800066ba:	e731                	bnez	a4,80006706 <virtio_disk_intr+0xba>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    800066bc:	20078793          	addi	a5,a5,512
    800066c0:	0792                	slli	a5,a5,0x4
    800066c2:	97ca                	add	a5,a5,s2
    800066c4:	7788                	ld	a0,40(a5)
    b->disk = 0;   // disk is done with buf
    800066c6:	00052223          	sw	zero,4(a0)
    wakeup(b);
    800066ca:	ffffc097          	auipc	ra,0xffffc
    800066ce:	e7c080e7          	jalr	-388(ra) # 80002546 <wakeup>

    disk.used_idx += 1;
    800066d2:	0204d783          	lhu	a5,32(s1)
    800066d6:	2785                	addiw	a5,a5,1
    800066d8:	17c2                	slli	a5,a5,0x30
    800066da:	93c1                	srli	a5,a5,0x30
    800066dc:	02f49023          	sh	a5,32(s1)
  while(disk.used_idx != disk.used->idx){
    800066e0:	6898                	ld	a4,16(s1)
    800066e2:	00275703          	lhu	a4,2(a4)
    800066e6:	faf71be3          	bne	a4,a5,8000669c <virtio_disk_intr+0x50>
  }

  release(&disk.vdisk_lock);
    800066ea:	0001f517          	auipc	a0,0x1f
    800066ee:	a3e50513          	addi	a0,a0,-1474 # 80025128 <disk+0x2128>
    800066f2:	ffffa097          	auipc	ra,0xffffa
    800066f6:	5a6080e7          	jalr	1446(ra) # 80000c98 <release>
}
    800066fa:	60e2                	ld	ra,24(sp)
    800066fc:	6442                	ld	s0,16(sp)
    800066fe:	64a2                	ld	s1,8(sp)
    80006700:	6902                	ld	s2,0(sp)
    80006702:	6105                	addi	sp,sp,32
    80006704:	8082                	ret
      panic("virtio_disk_intr status");
    80006706:	00002517          	auipc	a0,0x2
    8000670a:	1c250513          	addi	a0,a0,450 # 800088c8 <syscalls+0x3c8>
    8000670e:	ffffa097          	auipc	ra,0xffffa
    80006712:	e30080e7          	jalr	-464(ra) # 8000053e <panic>
	...

0000000080007000 <_trampoline>:
    80007000:	14051573          	csrrw	a0,sscratch,a0
    80007004:	02153423          	sd	ra,40(a0)
    80007008:	02253823          	sd	sp,48(a0)
    8000700c:	02353c23          	sd	gp,56(a0)
    80007010:	04453023          	sd	tp,64(a0)
    80007014:	04553423          	sd	t0,72(a0)
    80007018:	04653823          	sd	t1,80(a0)
    8000701c:	04753c23          	sd	t2,88(a0)
    80007020:	f120                	sd	s0,96(a0)
    80007022:	f524                	sd	s1,104(a0)
    80007024:	fd2c                	sd	a1,120(a0)
    80007026:	e150                	sd	a2,128(a0)
    80007028:	e554                	sd	a3,136(a0)
    8000702a:	e958                	sd	a4,144(a0)
    8000702c:	ed5c                	sd	a5,152(a0)
    8000702e:	0b053023          	sd	a6,160(a0)
    80007032:	0b153423          	sd	a7,168(a0)
    80007036:	0b253823          	sd	s2,176(a0)
    8000703a:	0b353c23          	sd	s3,184(a0)
    8000703e:	0d453023          	sd	s4,192(a0)
    80007042:	0d553423          	sd	s5,200(a0)
    80007046:	0d653823          	sd	s6,208(a0)
    8000704a:	0d753c23          	sd	s7,216(a0)
    8000704e:	0f853023          	sd	s8,224(a0)
    80007052:	0f953423          	sd	s9,232(a0)
    80007056:	0fa53823          	sd	s10,240(a0)
    8000705a:	0fb53c23          	sd	s11,248(a0)
    8000705e:	11c53023          	sd	t3,256(a0)
    80007062:	11d53423          	sd	t4,264(a0)
    80007066:	11e53823          	sd	t5,272(a0)
    8000706a:	11f53c23          	sd	t6,280(a0)
    8000706e:	140022f3          	csrr	t0,sscratch
    80007072:	06553823          	sd	t0,112(a0)
    80007076:	00853103          	ld	sp,8(a0)
    8000707a:	02053203          	ld	tp,32(a0)
    8000707e:	01053283          	ld	t0,16(a0)
    80007082:	00053303          	ld	t1,0(a0)
    80007086:	18031073          	csrw	satp,t1
    8000708a:	12000073          	sfence.vma
    8000708e:	8282                	jr	t0

0000000080007090 <userret>:
    80007090:	18059073          	csrw	satp,a1
    80007094:	12000073          	sfence.vma
    80007098:	07053283          	ld	t0,112(a0)
    8000709c:	14029073          	csrw	sscratch,t0
    800070a0:	02853083          	ld	ra,40(a0)
    800070a4:	03053103          	ld	sp,48(a0)
    800070a8:	03853183          	ld	gp,56(a0)
    800070ac:	04053203          	ld	tp,64(a0)
    800070b0:	04853283          	ld	t0,72(a0)
    800070b4:	05053303          	ld	t1,80(a0)
    800070b8:	05853383          	ld	t2,88(a0)
    800070bc:	7120                	ld	s0,96(a0)
    800070be:	7524                	ld	s1,104(a0)
    800070c0:	7d2c                	ld	a1,120(a0)
    800070c2:	6150                	ld	a2,128(a0)
    800070c4:	6554                	ld	a3,136(a0)
    800070c6:	6958                	ld	a4,144(a0)
    800070c8:	6d5c                	ld	a5,152(a0)
    800070ca:	0a053803          	ld	a6,160(a0)
    800070ce:	0a853883          	ld	a7,168(a0)
    800070d2:	0b053903          	ld	s2,176(a0)
    800070d6:	0b853983          	ld	s3,184(a0)
    800070da:	0c053a03          	ld	s4,192(a0)
    800070de:	0c853a83          	ld	s5,200(a0)
    800070e2:	0d053b03          	ld	s6,208(a0)
    800070e6:	0d853b83          	ld	s7,216(a0)
    800070ea:	0e053c03          	ld	s8,224(a0)
    800070ee:	0e853c83          	ld	s9,232(a0)
    800070f2:	0f053d03          	ld	s10,240(a0)
    800070f6:	0f853d83          	ld	s11,248(a0)
    800070fa:	10053e03          	ld	t3,256(a0)
    800070fe:	10853e83          	ld	t4,264(a0)
    80007102:	11053f03          	ld	t5,272(a0)
    80007106:	11853f83          	ld	t6,280(a0)
    8000710a:	14051573          	csrrw	a0,sscratch,a0
    8000710e:	10200073          	sret
	...
