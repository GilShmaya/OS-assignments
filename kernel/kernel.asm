
kernel/kernel:     file format elf64-littleriscv


Disassembly of section .text:

0000000080000000 <_entry>:
    80000000:	00009117          	auipc	sp,0x9
    80000004:	88013103          	ld	sp,-1920(sp) # 80008880 <_GLOBAL_OFFSET_TABLE_+0x8>
    80000008:	6505                	lui	a0,0x1
    8000000a:	f14025f3          	csrr	a1,mhartid
    8000000e:	0585                	addi	a1,a1,1
    80000010:	02b50533          	mul	a0,a0,a1
    80000014:	912a                	add	sp,sp,a0
    80000016:	078000ef          	jal	ra,8000008e <start>

000000008000001a <spin>:
    8000001a:	a001                	j	8000001a <spin>

000000008000001c <timerinit>:
// which arrive at timervec in kernelvec.S,
// which turns them into software interrupts for
// devintr() in trap.c.
void
timerinit()
{
    8000001c:	1141                	addi	sp,sp,-16
    8000001e:	e422                	sd	s0,8(sp)
    80000020:	0800                	addi	s0,sp,16
// which hart (core) is this?
static inline uint64
r_mhartid()
{
  uint64 x;
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    80000022:	f14027f3          	csrr	a5,mhartid
  // each CPU has a separate source of timer interrupts.
  int id = r_mhartid();
    80000026:	0007869b          	sext.w	a3,a5

  // ask the CLINT for a timer interrupt.
  int interval = 1000000; // cycles; about 1/10th second in qemu.
  *(uint64*)CLINT_MTIMECMP(id) = *(uint64*)CLINT_MTIME + interval;
    8000002a:	0037979b          	slliw	a5,a5,0x3
    8000002e:	02004737          	lui	a4,0x2004
    80000032:	97ba                	add	a5,a5,a4
    80000034:	0200c737          	lui	a4,0x200c
    80000038:	ff873583          	ld	a1,-8(a4) # 200bff8 <_entry-0x7dff4008>
    8000003c:	000f4637          	lui	a2,0xf4
    80000040:	24060613          	addi	a2,a2,576 # f4240 <_entry-0x7ff0bdc0>
    80000044:	95b2                	add	a1,a1,a2
    80000046:	e38c                	sd	a1,0(a5)

  // prepare information in scratch[] for timervec.
  // scratch[0..2] : space for timervec to save registers.
  // scratch[3] : address of CLINT MTIMECMP register.
  // scratch[4] : desired interval (in cycles) between timer interrupts.
  uint64 *scratch = &timer_scratch[id][0];
    80000048:	00269713          	slli	a4,a3,0x2
    8000004c:	9736                	add	a4,a4,a3
    8000004e:	00371693          	slli	a3,a4,0x3
    80000052:	00009717          	auipc	a4,0x9
    80000056:	ffe70713          	addi	a4,a4,-2 # 80009050 <timer_scratch>
    8000005a:	9736                	add	a4,a4,a3
  scratch[3] = CLINT_MTIMECMP(id);
    8000005c:	ef1c                	sd	a5,24(a4)
  scratch[4] = interval;
    8000005e:	f310                	sd	a2,32(a4)
}

static inline void 
w_mscratch(uint64 x)
{
  asm volatile("csrw mscratch, %0" : : "r" (x));
    80000060:	34071073          	csrw	mscratch,a4
  asm volatile("csrw mtvec, %0" : : "r" (x));
    80000064:	00006797          	auipc	a5,0x6
    80000068:	f0c78793          	addi	a5,a5,-244 # 80005f70 <timervec>
    8000006c:	30579073          	csrw	mtvec,a5
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000070:	300027f3          	csrr	a5,mstatus

  // set the machine-mode trap handler.
  w_mtvec((uint64)timervec);

  // enable machine-mode interrupts.
  w_mstatus(r_mstatus() | MSTATUS_MIE);
    80000074:	0087e793          	ori	a5,a5,8
  asm volatile("csrw mstatus, %0" : : "r" (x));
    80000078:	30079073          	csrw	mstatus,a5
  asm volatile("csrr %0, mie" : "=r" (x) );
    8000007c:	304027f3          	csrr	a5,mie

  // enable machine-mode timer interrupts.
  w_mie(r_mie() | MIE_MTIE);
    80000080:	0807e793          	ori	a5,a5,128
  asm volatile("csrw mie, %0" : : "r" (x));
    80000084:	30479073          	csrw	mie,a5
}
    80000088:	6422                	ld	s0,8(sp)
    8000008a:	0141                	addi	sp,sp,16
    8000008c:	8082                	ret

000000008000008e <start>:
{
    8000008e:	1141                	addi	sp,sp,-16
    80000090:	e406                	sd	ra,8(sp)
    80000092:	e022                	sd	s0,0(sp)
    80000094:	0800                	addi	s0,sp,16
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000096:	300027f3          	csrr	a5,mstatus
  x &= ~MSTATUS_MPP_MASK;
    8000009a:	7779                	lui	a4,0xffffe
    8000009c:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffd87ff>
    800000a0:	8ff9                	and	a5,a5,a4
  x |= MSTATUS_MPP_S;
    800000a2:	6705                	lui	a4,0x1
    800000a4:	80070713          	addi	a4,a4,-2048 # 800 <_entry-0x7ffff800>
    800000a8:	8fd9                	or	a5,a5,a4
  asm volatile("csrw mstatus, %0" : : "r" (x));
    800000aa:	30079073          	csrw	mstatus,a5
  asm volatile("csrw mepc, %0" : : "r" (x));
    800000ae:	00001797          	auipc	a5,0x1
    800000b2:	de078793          	addi	a5,a5,-544 # 80000e8e <main>
    800000b6:	34179073          	csrw	mepc,a5
  asm volatile("csrw satp, %0" : : "r" (x));
    800000ba:	4781                	li	a5,0
    800000bc:	18079073          	csrw	satp,a5
  asm volatile("csrw medeleg, %0" : : "r" (x));
    800000c0:	67c1                	lui	a5,0x10
    800000c2:	17fd                	addi	a5,a5,-1
    800000c4:	30279073          	csrw	medeleg,a5
  asm volatile("csrw mideleg, %0" : : "r" (x));
    800000c8:	30379073          	csrw	mideleg,a5
  asm volatile("csrr %0, sie" : "=r" (x) );
    800000cc:	104027f3          	csrr	a5,sie
  w_sie(r_sie() | SIE_SEIE | SIE_STIE | SIE_SSIE);
    800000d0:	2227e793          	ori	a5,a5,546
  asm volatile("csrw sie, %0" : : "r" (x));
    800000d4:	10479073          	csrw	sie,a5
  asm volatile("csrw pmpaddr0, %0" : : "r" (x));
    800000d8:	57fd                	li	a5,-1
    800000da:	83a9                	srli	a5,a5,0xa
    800000dc:	3b079073          	csrw	pmpaddr0,a5
  asm volatile("csrw pmpcfg0, %0" : : "r" (x));
    800000e0:	47bd                	li	a5,15
    800000e2:	3a079073          	csrw	pmpcfg0,a5
  timerinit();
    800000e6:	00000097          	auipc	ra,0x0
    800000ea:	f36080e7          	jalr	-202(ra) # 8000001c <timerinit>
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    800000ee:	f14027f3          	csrr	a5,mhartid
  w_tp(id);
    800000f2:	2781                	sext.w	a5,a5
}

static inline void 
w_tp(uint64 x)
{
  asm volatile("mv tp, %0" : : "r" (x));
    800000f4:	823e                	mv	tp,a5
  asm volatile("mret");
    800000f6:	30200073          	mret
}
    800000fa:	60a2                	ld	ra,8(sp)
    800000fc:	6402                	ld	s0,0(sp)
    800000fe:	0141                	addi	sp,sp,16
    80000100:	8082                	ret

0000000080000102 <consolewrite>:
//
// user write()s to the console go here.
//
int
consolewrite(int user_src, uint64 src, int n)
{
    80000102:	715d                	addi	sp,sp,-80
    80000104:	e486                	sd	ra,72(sp)
    80000106:	e0a2                	sd	s0,64(sp)
    80000108:	fc26                	sd	s1,56(sp)
    8000010a:	f84a                	sd	s2,48(sp)
    8000010c:	f44e                	sd	s3,40(sp)
    8000010e:	f052                	sd	s4,32(sp)
    80000110:	ec56                	sd	s5,24(sp)
    80000112:	0880                	addi	s0,sp,80
  int i;

  for(i = 0; i < n; i++){
    80000114:	04c05663          	blez	a2,80000160 <consolewrite+0x5e>
    80000118:	8a2a                	mv	s4,a0
    8000011a:	84ae                	mv	s1,a1
    8000011c:	89b2                	mv	s3,a2
    8000011e:	4901                	li	s2,0
    char c;
    if(either_copyin(&c, user_src, src+i, 1) == -1)
    80000120:	5afd                	li	s5,-1
    80000122:	4685                	li	a3,1
    80000124:	8626                	mv	a2,s1
    80000126:	85d2                	mv	a1,s4
    80000128:	fbf40513          	addi	a0,s0,-65
    8000012c:	00002097          	auipc	ra,0x2
    80000130:	6d8080e7          	jalr	1752(ra) # 80002804 <either_copyin>
    80000134:	01550c63          	beq	a0,s5,8000014c <consolewrite+0x4a>
      break;
    uartputc(c);
    80000138:	fbf44503          	lbu	a0,-65(s0)
    8000013c:	00000097          	auipc	ra,0x0
    80000140:	78e080e7          	jalr	1934(ra) # 800008ca <uartputc>
  for(i = 0; i < n; i++){
    80000144:	2905                	addiw	s2,s2,1
    80000146:	0485                	addi	s1,s1,1
    80000148:	fd299de3          	bne	s3,s2,80000122 <consolewrite+0x20>
  }

  return i;
}
    8000014c:	854a                	mv	a0,s2
    8000014e:	60a6                	ld	ra,72(sp)
    80000150:	6406                	ld	s0,64(sp)
    80000152:	74e2                	ld	s1,56(sp)
    80000154:	7942                	ld	s2,48(sp)
    80000156:	79a2                	ld	s3,40(sp)
    80000158:	7a02                	ld	s4,32(sp)
    8000015a:	6ae2                	ld	s5,24(sp)
    8000015c:	6161                	addi	sp,sp,80
    8000015e:	8082                	ret
  for(i = 0; i < n; i++){
    80000160:	4901                	li	s2,0
    80000162:	b7ed                	j	8000014c <consolewrite+0x4a>

0000000080000164 <consoleread>:
// user_dist indicates whether dst is a user
// or kernel address.
//
int
consoleread(int user_dst, uint64 dst, int n)
{
    80000164:	7119                	addi	sp,sp,-128
    80000166:	fc86                	sd	ra,120(sp)
    80000168:	f8a2                	sd	s0,112(sp)
    8000016a:	f4a6                	sd	s1,104(sp)
    8000016c:	f0ca                	sd	s2,96(sp)
    8000016e:	ecce                	sd	s3,88(sp)
    80000170:	e8d2                	sd	s4,80(sp)
    80000172:	e4d6                	sd	s5,72(sp)
    80000174:	e0da                	sd	s6,64(sp)
    80000176:	fc5e                	sd	s7,56(sp)
    80000178:	f862                	sd	s8,48(sp)
    8000017a:	f466                	sd	s9,40(sp)
    8000017c:	f06a                	sd	s10,32(sp)
    8000017e:	ec6e                	sd	s11,24(sp)
    80000180:	0100                	addi	s0,sp,128
    80000182:	8b2a                	mv	s6,a0
    80000184:	8aae                	mv	s5,a1
    80000186:	8a32                	mv	s4,a2
  uint target;
  int c;
  char cbuf;

  target = n;
    80000188:	00060b9b          	sext.w	s7,a2
  acquire(&cons.lock);
    8000018c:	00011517          	auipc	a0,0x11
    80000190:	00450513          	addi	a0,a0,4 # 80011190 <cons>
    80000194:	00001097          	auipc	ra,0x1
    80000198:	a50080e7          	jalr	-1456(ra) # 80000be4 <acquire>
  while(n > 0){
    // wait until interrupt handler has put some
    // input into cons.buffer.
    while(cons.r == cons.w){
    8000019c:	00011497          	auipc	s1,0x11
    800001a0:	ff448493          	addi	s1,s1,-12 # 80011190 <cons>
      if(myproc()->killed){
        release(&cons.lock);
        return -1;
      }
      sleep(&cons.r, &cons.lock);
    800001a4:	89a6                	mv	s3,s1
    800001a6:	00011917          	auipc	s2,0x11
    800001aa:	08290913          	addi	s2,s2,130 # 80011228 <cons+0x98>
    }

    c = cons.buf[cons.r++ % INPUT_BUF];

    if(c == C('D')){  // end-of-file
    800001ae:	4c91                	li	s9,4
      break;
    }

    // copy the input byte to the user-space buffer.
    cbuf = c;
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    800001b0:	5d7d                	li	s10,-1
      break;

    dst++;
    --n;

    if(c == '\n'){
    800001b2:	4da9                	li	s11,10
  while(n > 0){
    800001b4:	07405863          	blez	s4,80000224 <consoleread+0xc0>
    while(cons.r == cons.w){
    800001b8:	0984a783          	lw	a5,152(s1)
    800001bc:	09c4a703          	lw	a4,156(s1)
    800001c0:	02f71463          	bne	a4,a5,800001e8 <consoleread+0x84>
      if(myproc()->killed){
    800001c4:	00002097          	auipc	ra,0x2
    800001c8:	9c0080e7          	jalr	-1600(ra) # 80001b84 <myproc>
    800001cc:	551c                	lw	a5,40(a0)
    800001ce:	e7b5                	bnez	a5,8000023a <consoleread+0xd6>
      sleep(&cons.r, &cons.lock);
    800001d0:	85ce                	mv	a1,s3
    800001d2:	854a                	mv	a0,s2
    800001d4:	00002097          	auipc	ra,0x2
    800001d8:	1b0080e7          	jalr	432(ra) # 80002384 <sleep>
    while(cons.r == cons.w){
    800001dc:	0984a783          	lw	a5,152(s1)
    800001e0:	09c4a703          	lw	a4,156(s1)
    800001e4:	fef700e3          	beq	a4,a5,800001c4 <consoleread+0x60>
    c = cons.buf[cons.r++ % INPUT_BUF];
    800001e8:	0017871b          	addiw	a4,a5,1
    800001ec:	08e4ac23          	sw	a4,152(s1)
    800001f0:	07f7f713          	andi	a4,a5,127
    800001f4:	9726                	add	a4,a4,s1
    800001f6:	01874703          	lbu	a4,24(a4)
    800001fa:	00070c1b          	sext.w	s8,a4
    if(c == C('D')){  // end-of-file
    800001fe:	079c0663          	beq	s8,s9,8000026a <consoleread+0x106>
    cbuf = c;
    80000202:	f8e407a3          	sb	a4,-113(s0)
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    80000206:	4685                	li	a3,1
    80000208:	f8f40613          	addi	a2,s0,-113
    8000020c:	85d6                	mv	a1,s5
    8000020e:	855a                	mv	a0,s6
    80000210:	00002097          	auipc	ra,0x2
    80000214:	59e080e7          	jalr	1438(ra) # 800027ae <either_copyout>
    80000218:	01a50663          	beq	a0,s10,80000224 <consoleread+0xc0>
    dst++;
    8000021c:	0a85                	addi	s5,s5,1
    --n;
    8000021e:	3a7d                	addiw	s4,s4,-1
    if(c == '\n'){
    80000220:	f9bc1ae3          	bne	s8,s11,800001b4 <consoleread+0x50>
      // a whole line has arrived, return to
      // the user-level read().
      break;
    }
  }
  release(&cons.lock);
    80000224:	00011517          	auipc	a0,0x11
    80000228:	f6c50513          	addi	a0,a0,-148 # 80011190 <cons>
    8000022c:	00001097          	auipc	ra,0x1
    80000230:	a6c080e7          	jalr	-1428(ra) # 80000c98 <release>

  return target - n;
    80000234:	414b853b          	subw	a0,s7,s4
    80000238:	a811                	j	8000024c <consoleread+0xe8>
        release(&cons.lock);
    8000023a:	00011517          	auipc	a0,0x11
    8000023e:	f5650513          	addi	a0,a0,-170 # 80011190 <cons>
    80000242:	00001097          	auipc	ra,0x1
    80000246:	a56080e7          	jalr	-1450(ra) # 80000c98 <release>
        return -1;
    8000024a:	557d                	li	a0,-1
}
    8000024c:	70e6                	ld	ra,120(sp)
    8000024e:	7446                	ld	s0,112(sp)
    80000250:	74a6                	ld	s1,104(sp)
    80000252:	7906                	ld	s2,96(sp)
    80000254:	69e6                	ld	s3,88(sp)
    80000256:	6a46                	ld	s4,80(sp)
    80000258:	6aa6                	ld	s5,72(sp)
    8000025a:	6b06                	ld	s6,64(sp)
    8000025c:	7be2                	ld	s7,56(sp)
    8000025e:	7c42                	ld	s8,48(sp)
    80000260:	7ca2                	ld	s9,40(sp)
    80000262:	7d02                	ld	s10,32(sp)
    80000264:	6de2                	ld	s11,24(sp)
    80000266:	6109                	addi	sp,sp,128
    80000268:	8082                	ret
      if(n < target){
    8000026a:	000a071b          	sext.w	a4,s4
    8000026e:	fb777be3          	bgeu	a4,s7,80000224 <consoleread+0xc0>
        cons.r--;
    80000272:	00011717          	auipc	a4,0x11
    80000276:	faf72b23          	sw	a5,-74(a4) # 80011228 <cons+0x98>
    8000027a:	b76d                	j	80000224 <consoleread+0xc0>

000000008000027c <consputc>:
{
    8000027c:	1141                	addi	sp,sp,-16
    8000027e:	e406                	sd	ra,8(sp)
    80000280:	e022                	sd	s0,0(sp)
    80000282:	0800                	addi	s0,sp,16
  if(c == BACKSPACE){
    80000284:	10000793          	li	a5,256
    80000288:	00f50a63          	beq	a0,a5,8000029c <consputc+0x20>
    uartputc_sync(c);
    8000028c:	00000097          	auipc	ra,0x0
    80000290:	564080e7          	jalr	1380(ra) # 800007f0 <uartputc_sync>
}
    80000294:	60a2                	ld	ra,8(sp)
    80000296:	6402                	ld	s0,0(sp)
    80000298:	0141                	addi	sp,sp,16
    8000029a:	8082                	ret
    uartputc_sync('\b'); uartputc_sync(' '); uartputc_sync('\b');
    8000029c:	4521                	li	a0,8
    8000029e:	00000097          	auipc	ra,0x0
    800002a2:	552080e7          	jalr	1362(ra) # 800007f0 <uartputc_sync>
    800002a6:	02000513          	li	a0,32
    800002aa:	00000097          	auipc	ra,0x0
    800002ae:	546080e7          	jalr	1350(ra) # 800007f0 <uartputc_sync>
    800002b2:	4521                	li	a0,8
    800002b4:	00000097          	auipc	ra,0x0
    800002b8:	53c080e7          	jalr	1340(ra) # 800007f0 <uartputc_sync>
    800002bc:	bfe1                	j	80000294 <consputc+0x18>

00000000800002be <consoleintr>:
// do erase/kill processing, append to cons.buf,
// wake up consoleread() if a whole line has arrived.
//
void
consoleintr(int c)
{
    800002be:	1101                	addi	sp,sp,-32
    800002c0:	ec06                	sd	ra,24(sp)
    800002c2:	e822                	sd	s0,16(sp)
    800002c4:	e426                	sd	s1,8(sp)
    800002c6:	e04a                	sd	s2,0(sp)
    800002c8:	1000                	addi	s0,sp,32
    800002ca:	84aa                	mv	s1,a0
  acquire(&cons.lock);
    800002cc:	00011517          	auipc	a0,0x11
    800002d0:	ec450513          	addi	a0,a0,-316 # 80011190 <cons>
    800002d4:	00001097          	auipc	ra,0x1
    800002d8:	910080e7          	jalr	-1776(ra) # 80000be4 <acquire>

  switch(c){
    800002dc:	47d5                	li	a5,21
    800002de:	0af48663          	beq	s1,a5,8000038a <consoleintr+0xcc>
    800002e2:	0297ca63          	blt	a5,s1,80000316 <consoleintr+0x58>
    800002e6:	47a1                	li	a5,8
    800002e8:	0ef48763          	beq	s1,a5,800003d6 <consoleintr+0x118>
    800002ec:	47c1                	li	a5,16
    800002ee:	10f49a63          	bne	s1,a5,80000402 <consoleintr+0x144>
  case C('P'):  // Print process list.
    procdump();
    800002f2:	00002097          	auipc	ra,0x2
    800002f6:	568080e7          	jalr	1384(ra) # 8000285a <procdump>
      }
    }
    break;
  }
  
  release(&cons.lock);
    800002fa:	00011517          	auipc	a0,0x11
    800002fe:	e9650513          	addi	a0,a0,-362 # 80011190 <cons>
    80000302:	00001097          	auipc	ra,0x1
    80000306:	996080e7          	jalr	-1642(ra) # 80000c98 <release>
}
    8000030a:	60e2                	ld	ra,24(sp)
    8000030c:	6442                	ld	s0,16(sp)
    8000030e:	64a2                	ld	s1,8(sp)
    80000310:	6902                	ld	s2,0(sp)
    80000312:	6105                	addi	sp,sp,32
    80000314:	8082                	ret
  switch(c){
    80000316:	07f00793          	li	a5,127
    8000031a:	0af48e63          	beq	s1,a5,800003d6 <consoleintr+0x118>
    if(c != 0 && cons.e-cons.r < INPUT_BUF){
    8000031e:	00011717          	auipc	a4,0x11
    80000322:	e7270713          	addi	a4,a4,-398 # 80011190 <cons>
    80000326:	0a072783          	lw	a5,160(a4)
    8000032a:	09872703          	lw	a4,152(a4)
    8000032e:	9f99                	subw	a5,a5,a4
    80000330:	07f00713          	li	a4,127
    80000334:	fcf763e3          	bltu	a4,a5,800002fa <consoleintr+0x3c>
      c = (c == '\r') ? '\n' : c;
    80000338:	47b5                	li	a5,13
    8000033a:	0cf48763          	beq	s1,a5,80000408 <consoleintr+0x14a>
      consputc(c);
    8000033e:	8526                	mv	a0,s1
    80000340:	00000097          	auipc	ra,0x0
    80000344:	f3c080e7          	jalr	-196(ra) # 8000027c <consputc>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000348:	00011797          	auipc	a5,0x11
    8000034c:	e4878793          	addi	a5,a5,-440 # 80011190 <cons>
    80000350:	0a07a703          	lw	a4,160(a5)
    80000354:	0017069b          	addiw	a3,a4,1
    80000358:	0006861b          	sext.w	a2,a3
    8000035c:	0ad7a023          	sw	a3,160(a5)
    80000360:	07f77713          	andi	a4,a4,127
    80000364:	97ba                	add	a5,a5,a4
    80000366:	00978c23          	sb	s1,24(a5)
      if(c == '\n' || c == C('D') || cons.e == cons.r+INPUT_BUF){
    8000036a:	47a9                	li	a5,10
    8000036c:	0cf48563          	beq	s1,a5,80000436 <consoleintr+0x178>
    80000370:	4791                	li	a5,4
    80000372:	0cf48263          	beq	s1,a5,80000436 <consoleintr+0x178>
    80000376:	00011797          	auipc	a5,0x11
    8000037a:	eb27a783          	lw	a5,-334(a5) # 80011228 <cons+0x98>
    8000037e:	0807879b          	addiw	a5,a5,128
    80000382:	f6f61ce3          	bne	a2,a5,800002fa <consoleintr+0x3c>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000386:	863e                	mv	a2,a5
    80000388:	a07d                	j	80000436 <consoleintr+0x178>
    while(cons.e != cons.w &&
    8000038a:	00011717          	auipc	a4,0x11
    8000038e:	e0670713          	addi	a4,a4,-506 # 80011190 <cons>
    80000392:	0a072783          	lw	a5,160(a4)
    80000396:	09c72703          	lw	a4,156(a4)
          cons.buf[(cons.e-1) % INPUT_BUF] != '\n'){
    8000039a:	00011497          	auipc	s1,0x11
    8000039e:	df648493          	addi	s1,s1,-522 # 80011190 <cons>
    while(cons.e != cons.w &&
    800003a2:	4929                	li	s2,10
    800003a4:	f4f70be3          	beq	a4,a5,800002fa <consoleintr+0x3c>
          cons.buf[(cons.e-1) % INPUT_BUF] != '\n'){
    800003a8:	37fd                	addiw	a5,a5,-1
    800003aa:	07f7f713          	andi	a4,a5,127
    800003ae:	9726                	add	a4,a4,s1
    while(cons.e != cons.w &&
    800003b0:	01874703          	lbu	a4,24(a4)
    800003b4:	f52703e3          	beq	a4,s2,800002fa <consoleintr+0x3c>
      cons.e--;
    800003b8:	0af4a023          	sw	a5,160(s1)
      consputc(BACKSPACE);
    800003bc:	10000513          	li	a0,256
    800003c0:	00000097          	auipc	ra,0x0
    800003c4:	ebc080e7          	jalr	-324(ra) # 8000027c <consputc>
    while(cons.e != cons.w &&
    800003c8:	0a04a783          	lw	a5,160(s1)
    800003cc:	09c4a703          	lw	a4,156(s1)
    800003d0:	fcf71ce3          	bne	a4,a5,800003a8 <consoleintr+0xea>
    800003d4:	b71d                	j	800002fa <consoleintr+0x3c>
    if(cons.e != cons.w){
    800003d6:	00011717          	auipc	a4,0x11
    800003da:	dba70713          	addi	a4,a4,-582 # 80011190 <cons>
    800003de:	0a072783          	lw	a5,160(a4)
    800003e2:	09c72703          	lw	a4,156(a4)
    800003e6:	f0f70ae3          	beq	a4,a5,800002fa <consoleintr+0x3c>
      cons.e--;
    800003ea:	37fd                	addiw	a5,a5,-1
    800003ec:	00011717          	auipc	a4,0x11
    800003f0:	e4f72223          	sw	a5,-444(a4) # 80011230 <cons+0xa0>
      consputc(BACKSPACE);
    800003f4:	10000513          	li	a0,256
    800003f8:	00000097          	auipc	ra,0x0
    800003fc:	e84080e7          	jalr	-380(ra) # 8000027c <consputc>
    80000400:	bded                	j	800002fa <consoleintr+0x3c>
    if(c != 0 && cons.e-cons.r < INPUT_BUF){
    80000402:	ee048ce3          	beqz	s1,800002fa <consoleintr+0x3c>
    80000406:	bf21                	j	8000031e <consoleintr+0x60>
      consputc(c);
    80000408:	4529                	li	a0,10
    8000040a:	00000097          	auipc	ra,0x0
    8000040e:	e72080e7          	jalr	-398(ra) # 8000027c <consputc>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000412:	00011797          	auipc	a5,0x11
    80000416:	d7e78793          	addi	a5,a5,-642 # 80011190 <cons>
    8000041a:	0a07a703          	lw	a4,160(a5)
    8000041e:	0017069b          	addiw	a3,a4,1
    80000422:	0006861b          	sext.w	a2,a3
    80000426:	0ad7a023          	sw	a3,160(a5)
    8000042a:	07f77713          	andi	a4,a4,127
    8000042e:	97ba                	add	a5,a5,a4
    80000430:	4729                	li	a4,10
    80000432:	00e78c23          	sb	a4,24(a5)
        cons.w = cons.e;
    80000436:	00011797          	auipc	a5,0x11
    8000043a:	dec7ab23          	sw	a2,-522(a5) # 8001122c <cons+0x9c>
        wakeup(&cons.r);
    8000043e:	00011517          	auipc	a0,0x11
    80000442:	dea50513          	addi	a0,a0,-534 # 80011228 <cons+0x98>
    80000446:	00002097          	auipc	ra,0x2
    8000044a:	0dc080e7          	jalr	220(ra) # 80002522 <wakeup>
    8000044e:	b575                	j	800002fa <consoleintr+0x3c>

0000000080000450 <consoleinit>:

void
consoleinit(void)
{
    80000450:	1141                	addi	sp,sp,-16
    80000452:	e406                	sd	ra,8(sp)
    80000454:	e022                	sd	s0,0(sp)
    80000456:	0800                	addi	s0,sp,16
  initlock(&cons.lock, "cons");
    80000458:	00008597          	auipc	a1,0x8
    8000045c:	bb858593          	addi	a1,a1,-1096 # 80008010 <etext+0x10>
    80000460:	00011517          	auipc	a0,0x11
    80000464:	d3050513          	addi	a0,a0,-720 # 80011190 <cons>
    80000468:	00000097          	auipc	ra,0x0
    8000046c:	6ec080e7          	jalr	1772(ra) # 80000b54 <initlock>

  uartinit();
    80000470:	00000097          	auipc	ra,0x0
    80000474:	330080e7          	jalr	816(ra) # 800007a0 <uartinit>

  // connect read and write system calls
  // to consoleread and consolewrite.
  devsw[CONSOLE].read = consoleread;
    80000478:	00021797          	auipc	a5,0x21
    8000047c:	33078793          	addi	a5,a5,816 # 800217a8 <devsw>
    80000480:	00000717          	auipc	a4,0x0
    80000484:	ce470713          	addi	a4,a4,-796 # 80000164 <consoleread>
    80000488:	eb98                	sd	a4,16(a5)
  devsw[CONSOLE].write = consolewrite;
    8000048a:	00000717          	auipc	a4,0x0
    8000048e:	c7870713          	addi	a4,a4,-904 # 80000102 <consolewrite>
    80000492:	ef98                	sd	a4,24(a5)
}
    80000494:	60a2                	ld	ra,8(sp)
    80000496:	6402                	ld	s0,0(sp)
    80000498:	0141                	addi	sp,sp,16
    8000049a:	8082                	ret

000000008000049c <printint>:

static char digits[] = "0123456789abcdef";

static void
printint(int xx, int base, int sign)
{
    8000049c:	7179                	addi	sp,sp,-48
    8000049e:	f406                	sd	ra,40(sp)
    800004a0:	f022                	sd	s0,32(sp)
    800004a2:	ec26                	sd	s1,24(sp)
    800004a4:	e84a                	sd	s2,16(sp)
    800004a6:	1800                	addi	s0,sp,48
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
    800004a8:	c219                	beqz	a2,800004ae <printint+0x12>
    800004aa:	08054663          	bltz	a0,80000536 <printint+0x9a>
    x = -xx;
  else
    x = xx;
    800004ae:	2501                	sext.w	a0,a0
    800004b0:	4881                	li	a7,0
    800004b2:	fd040693          	addi	a3,s0,-48

  i = 0;
    800004b6:	4701                	li	a4,0
  do {
    buf[i++] = digits[x % base];
    800004b8:	2581                	sext.w	a1,a1
    800004ba:	00008617          	auipc	a2,0x8
    800004be:	b8660613          	addi	a2,a2,-1146 # 80008040 <digits>
    800004c2:	883a                	mv	a6,a4
    800004c4:	2705                	addiw	a4,a4,1
    800004c6:	02b577bb          	remuw	a5,a0,a1
    800004ca:	1782                	slli	a5,a5,0x20
    800004cc:	9381                	srli	a5,a5,0x20
    800004ce:	97b2                	add	a5,a5,a2
    800004d0:	0007c783          	lbu	a5,0(a5)
    800004d4:	00f68023          	sb	a5,0(a3)
  } while((x /= base) != 0);
    800004d8:	0005079b          	sext.w	a5,a0
    800004dc:	02b5553b          	divuw	a0,a0,a1
    800004e0:	0685                	addi	a3,a3,1
    800004e2:	feb7f0e3          	bgeu	a5,a1,800004c2 <printint+0x26>

  if(sign)
    800004e6:	00088b63          	beqz	a7,800004fc <printint+0x60>
    buf[i++] = '-';
    800004ea:	fe040793          	addi	a5,s0,-32
    800004ee:	973e                	add	a4,a4,a5
    800004f0:	02d00793          	li	a5,45
    800004f4:	fef70823          	sb	a5,-16(a4)
    800004f8:	0028071b          	addiw	a4,a6,2

  while(--i >= 0)
    800004fc:	02e05763          	blez	a4,8000052a <printint+0x8e>
    80000500:	fd040793          	addi	a5,s0,-48
    80000504:	00e784b3          	add	s1,a5,a4
    80000508:	fff78913          	addi	s2,a5,-1
    8000050c:	993a                	add	s2,s2,a4
    8000050e:	377d                	addiw	a4,a4,-1
    80000510:	1702                	slli	a4,a4,0x20
    80000512:	9301                	srli	a4,a4,0x20
    80000514:	40e90933          	sub	s2,s2,a4
    consputc(buf[i]);
    80000518:	fff4c503          	lbu	a0,-1(s1)
    8000051c:	00000097          	auipc	ra,0x0
    80000520:	d60080e7          	jalr	-672(ra) # 8000027c <consputc>
  while(--i >= 0)
    80000524:	14fd                	addi	s1,s1,-1
    80000526:	ff2499e3          	bne	s1,s2,80000518 <printint+0x7c>
}
    8000052a:	70a2                	ld	ra,40(sp)
    8000052c:	7402                	ld	s0,32(sp)
    8000052e:	64e2                	ld	s1,24(sp)
    80000530:	6942                	ld	s2,16(sp)
    80000532:	6145                	addi	sp,sp,48
    80000534:	8082                	ret
    x = -xx;
    80000536:	40a0053b          	negw	a0,a0
  if(sign && (sign = xx < 0))
    8000053a:	4885                	li	a7,1
    x = -xx;
    8000053c:	bf9d                	j	800004b2 <printint+0x16>

000000008000053e <panic>:
    release(&pr.lock);
}

void
panic(char *s)
{
    8000053e:	1101                	addi	sp,sp,-32
    80000540:	ec06                	sd	ra,24(sp)
    80000542:	e822                	sd	s0,16(sp)
    80000544:	e426                	sd	s1,8(sp)
    80000546:	1000                	addi	s0,sp,32
    80000548:	84aa                	mv	s1,a0
  pr.locking = 0;
    8000054a:	00011797          	auipc	a5,0x11
    8000054e:	d007a323          	sw	zero,-762(a5) # 80011250 <pr+0x18>
  printf("panic: ");
    80000552:	00008517          	auipc	a0,0x8
    80000556:	ac650513          	addi	a0,a0,-1338 # 80008018 <etext+0x18>
    8000055a:	00000097          	auipc	ra,0x0
    8000055e:	02e080e7          	jalr	46(ra) # 80000588 <printf>
  printf(s);
    80000562:	8526                	mv	a0,s1
    80000564:	00000097          	auipc	ra,0x0
    80000568:	024080e7          	jalr	36(ra) # 80000588 <printf>
  printf("\n");
    8000056c:	00008517          	auipc	a0,0x8
    80000570:	b5c50513          	addi	a0,a0,-1188 # 800080c8 <digits+0x88>
    80000574:	00000097          	auipc	ra,0x0
    80000578:	014080e7          	jalr	20(ra) # 80000588 <printf>
  panicked = 1; // freeze uart output from other CPUs
    8000057c:	4785                	li	a5,1
    8000057e:	00009717          	auipc	a4,0x9
    80000582:	a8f72123          	sw	a5,-1406(a4) # 80009000 <panicked>
  for(;;)
    80000586:	a001                	j	80000586 <panic+0x48>

0000000080000588 <printf>:
{
    80000588:	7131                	addi	sp,sp,-192
    8000058a:	fc86                	sd	ra,120(sp)
    8000058c:	f8a2                	sd	s0,112(sp)
    8000058e:	f4a6                	sd	s1,104(sp)
    80000590:	f0ca                	sd	s2,96(sp)
    80000592:	ecce                	sd	s3,88(sp)
    80000594:	e8d2                	sd	s4,80(sp)
    80000596:	e4d6                	sd	s5,72(sp)
    80000598:	e0da                	sd	s6,64(sp)
    8000059a:	fc5e                	sd	s7,56(sp)
    8000059c:	f862                	sd	s8,48(sp)
    8000059e:	f466                	sd	s9,40(sp)
    800005a0:	f06a                	sd	s10,32(sp)
    800005a2:	ec6e                	sd	s11,24(sp)
    800005a4:	0100                	addi	s0,sp,128
    800005a6:	8a2a                	mv	s4,a0
    800005a8:	e40c                	sd	a1,8(s0)
    800005aa:	e810                	sd	a2,16(s0)
    800005ac:	ec14                	sd	a3,24(s0)
    800005ae:	f018                	sd	a4,32(s0)
    800005b0:	f41c                	sd	a5,40(s0)
    800005b2:	03043823          	sd	a6,48(s0)
    800005b6:	03143c23          	sd	a7,56(s0)
  locking = pr.locking;
    800005ba:	00011d97          	auipc	s11,0x11
    800005be:	c96dad83          	lw	s11,-874(s11) # 80011250 <pr+0x18>
  if(locking)
    800005c2:	020d9b63          	bnez	s11,800005f8 <printf+0x70>
  if (fmt == 0)
    800005c6:	040a0263          	beqz	s4,8000060a <printf+0x82>
  va_start(ap, fmt);
    800005ca:	00840793          	addi	a5,s0,8
    800005ce:	f8f43423          	sd	a5,-120(s0)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
    800005d2:	000a4503          	lbu	a0,0(s4)
    800005d6:	16050263          	beqz	a0,8000073a <printf+0x1b2>
    800005da:	4481                	li	s1,0
    if(c != '%'){
    800005dc:	02500a93          	li	s5,37
    switch(c){
    800005e0:	07000b13          	li	s6,112
  consputc('x');
    800005e4:	4d41                	li	s10,16
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    800005e6:	00008b97          	auipc	s7,0x8
    800005ea:	a5ab8b93          	addi	s7,s7,-1446 # 80008040 <digits>
    switch(c){
    800005ee:	07300c93          	li	s9,115
    800005f2:	06400c13          	li	s8,100
    800005f6:	a82d                	j	80000630 <printf+0xa8>
    acquire(&pr.lock);
    800005f8:	00011517          	auipc	a0,0x11
    800005fc:	c4050513          	addi	a0,a0,-960 # 80011238 <pr>
    80000600:	00000097          	auipc	ra,0x0
    80000604:	5e4080e7          	jalr	1508(ra) # 80000be4 <acquire>
    80000608:	bf7d                	j	800005c6 <printf+0x3e>
    panic("null fmt");
    8000060a:	00008517          	auipc	a0,0x8
    8000060e:	a1e50513          	addi	a0,a0,-1506 # 80008028 <etext+0x28>
    80000612:	00000097          	auipc	ra,0x0
    80000616:	f2c080e7          	jalr	-212(ra) # 8000053e <panic>
      consputc(c);
    8000061a:	00000097          	auipc	ra,0x0
    8000061e:	c62080e7          	jalr	-926(ra) # 8000027c <consputc>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
    80000622:	2485                	addiw	s1,s1,1
    80000624:	009a07b3          	add	a5,s4,s1
    80000628:	0007c503          	lbu	a0,0(a5)
    8000062c:	10050763          	beqz	a0,8000073a <printf+0x1b2>
    if(c != '%'){
    80000630:	ff5515e3          	bne	a0,s5,8000061a <printf+0x92>
    c = fmt[++i] & 0xff;
    80000634:	2485                	addiw	s1,s1,1
    80000636:	009a07b3          	add	a5,s4,s1
    8000063a:	0007c783          	lbu	a5,0(a5)
    8000063e:	0007891b          	sext.w	s2,a5
    if(c == 0)
    80000642:	cfe5                	beqz	a5,8000073a <printf+0x1b2>
    switch(c){
    80000644:	05678a63          	beq	a5,s6,80000698 <printf+0x110>
    80000648:	02fb7663          	bgeu	s6,a5,80000674 <printf+0xec>
    8000064c:	09978963          	beq	a5,s9,800006de <printf+0x156>
    80000650:	07800713          	li	a4,120
    80000654:	0ce79863          	bne	a5,a4,80000724 <printf+0x19c>
      printint(va_arg(ap, int), 16, 1);
    80000658:	f8843783          	ld	a5,-120(s0)
    8000065c:	00878713          	addi	a4,a5,8
    80000660:	f8e43423          	sd	a4,-120(s0)
    80000664:	4605                	li	a2,1
    80000666:	85ea                	mv	a1,s10
    80000668:	4388                	lw	a0,0(a5)
    8000066a:	00000097          	auipc	ra,0x0
    8000066e:	e32080e7          	jalr	-462(ra) # 8000049c <printint>
      break;
    80000672:	bf45                	j	80000622 <printf+0x9a>
    switch(c){
    80000674:	0b578263          	beq	a5,s5,80000718 <printf+0x190>
    80000678:	0b879663          	bne	a5,s8,80000724 <printf+0x19c>
      printint(va_arg(ap, int), 10, 1);
    8000067c:	f8843783          	ld	a5,-120(s0)
    80000680:	00878713          	addi	a4,a5,8
    80000684:	f8e43423          	sd	a4,-120(s0)
    80000688:	4605                	li	a2,1
    8000068a:	45a9                	li	a1,10
    8000068c:	4388                	lw	a0,0(a5)
    8000068e:	00000097          	auipc	ra,0x0
    80000692:	e0e080e7          	jalr	-498(ra) # 8000049c <printint>
      break;
    80000696:	b771                	j	80000622 <printf+0x9a>
      printptr(va_arg(ap, uint64));
    80000698:	f8843783          	ld	a5,-120(s0)
    8000069c:	00878713          	addi	a4,a5,8
    800006a0:	f8e43423          	sd	a4,-120(s0)
    800006a4:	0007b983          	ld	s3,0(a5)
  consputc('0');
    800006a8:	03000513          	li	a0,48
    800006ac:	00000097          	auipc	ra,0x0
    800006b0:	bd0080e7          	jalr	-1072(ra) # 8000027c <consputc>
  consputc('x');
    800006b4:	07800513          	li	a0,120
    800006b8:	00000097          	auipc	ra,0x0
    800006bc:	bc4080e7          	jalr	-1084(ra) # 8000027c <consputc>
    800006c0:	896a                	mv	s2,s10
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    800006c2:	03c9d793          	srli	a5,s3,0x3c
    800006c6:	97de                	add	a5,a5,s7
    800006c8:	0007c503          	lbu	a0,0(a5)
    800006cc:	00000097          	auipc	ra,0x0
    800006d0:	bb0080e7          	jalr	-1104(ra) # 8000027c <consputc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
    800006d4:	0992                	slli	s3,s3,0x4
    800006d6:	397d                	addiw	s2,s2,-1
    800006d8:	fe0915e3          	bnez	s2,800006c2 <printf+0x13a>
    800006dc:	b799                	j	80000622 <printf+0x9a>
      if((s = va_arg(ap, char*)) == 0)
    800006de:	f8843783          	ld	a5,-120(s0)
    800006e2:	00878713          	addi	a4,a5,8
    800006e6:	f8e43423          	sd	a4,-120(s0)
    800006ea:	0007b903          	ld	s2,0(a5)
    800006ee:	00090e63          	beqz	s2,8000070a <printf+0x182>
      for(; *s; s++)
    800006f2:	00094503          	lbu	a0,0(s2)
    800006f6:	d515                	beqz	a0,80000622 <printf+0x9a>
        consputc(*s);
    800006f8:	00000097          	auipc	ra,0x0
    800006fc:	b84080e7          	jalr	-1148(ra) # 8000027c <consputc>
      for(; *s; s++)
    80000700:	0905                	addi	s2,s2,1
    80000702:	00094503          	lbu	a0,0(s2)
    80000706:	f96d                	bnez	a0,800006f8 <printf+0x170>
    80000708:	bf29                	j	80000622 <printf+0x9a>
        s = "(null)";
    8000070a:	00008917          	auipc	s2,0x8
    8000070e:	91690913          	addi	s2,s2,-1770 # 80008020 <etext+0x20>
      for(; *s; s++)
    80000712:	02800513          	li	a0,40
    80000716:	b7cd                	j	800006f8 <printf+0x170>
      consputc('%');
    80000718:	8556                	mv	a0,s5
    8000071a:	00000097          	auipc	ra,0x0
    8000071e:	b62080e7          	jalr	-1182(ra) # 8000027c <consputc>
      break;
    80000722:	b701                	j	80000622 <printf+0x9a>
      consputc('%');
    80000724:	8556                	mv	a0,s5
    80000726:	00000097          	auipc	ra,0x0
    8000072a:	b56080e7          	jalr	-1194(ra) # 8000027c <consputc>
      consputc(c);
    8000072e:	854a                	mv	a0,s2
    80000730:	00000097          	auipc	ra,0x0
    80000734:	b4c080e7          	jalr	-1204(ra) # 8000027c <consputc>
      break;
    80000738:	b5ed                	j	80000622 <printf+0x9a>
  if(locking)
    8000073a:	020d9163          	bnez	s11,8000075c <printf+0x1d4>
}
    8000073e:	70e6                	ld	ra,120(sp)
    80000740:	7446                	ld	s0,112(sp)
    80000742:	74a6                	ld	s1,104(sp)
    80000744:	7906                	ld	s2,96(sp)
    80000746:	69e6                	ld	s3,88(sp)
    80000748:	6a46                	ld	s4,80(sp)
    8000074a:	6aa6                	ld	s5,72(sp)
    8000074c:	6b06                	ld	s6,64(sp)
    8000074e:	7be2                	ld	s7,56(sp)
    80000750:	7c42                	ld	s8,48(sp)
    80000752:	7ca2                	ld	s9,40(sp)
    80000754:	7d02                	ld	s10,32(sp)
    80000756:	6de2                	ld	s11,24(sp)
    80000758:	6129                	addi	sp,sp,192
    8000075a:	8082                	ret
    release(&pr.lock);
    8000075c:	00011517          	auipc	a0,0x11
    80000760:	adc50513          	addi	a0,a0,-1316 # 80011238 <pr>
    80000764:	00000097          	auipc	ra,0x0
    80000768:	534080e7          	jalr	1332(ra) # 80000c98 <release>
}
    8000076c:	bfc9                	j	8000073e <printf+0x1b6>

000000008000076e <printfinit>:
    ;
}

void
printfinit(void)
{
    8000076e:	1101                	addi	sp,sp,-32
    80000770:	ec06                	sd	ra,24(sp)
    80000772:	e822                	sd	s0,16(sp)
    80000774:	e426                	sd	s1,8(sp)
    80000776:	1000                	addi	s0,sp,32
  initlock(&pr.lock, "pr");
    80000778:	00011497          	auipc	s1,0x11
    8000077c:	ac048493          	addi	s1,s1,-1344 # 80011238 <pr>
    80000780:	00008597          	auipc	a1,0x8
    80000784:	8b858593          	addi	a1,a1,-1864 # 80008038 <etext+0x38>
    80000788:	8526                	mv	a0,s1
    8000078a:	00000097          	auipc	ra,0x0
    8000078e:	3ca080e7          	jalr	970(ra) # 80000b54 <initlock>
  pr.locking = 1;
    80000792:	4785                	li	a5,1
    80000794:	cc9c                	sw	a5,24(s1)
}
    80000796:	60e2                	ld	ra,24(sp)
    80000798:	6442                	ld	s0,16(sp)
    8000079a:	64a2                	ld	s1,8(sp)
    8000079c:	6105                	addi	sp,sp,32
    8000079e:	8082                	ret

00000000800007a0 <uartinit>:

void uartstart();

void
uartinit(void)
{
    800007a0:	1141                	addi	sp,sp,-16
    800007a2:	e406                	sd	ra,8(sp)
    800007a4:	e022                	sd	s0,0(sp)
    800007a6:	0800                	addi	s0,sp,16
  // disable interrupts.
  WriteReg(IER, 0x00);
    800007a8:	100007b7          	lui	a5,0x10000
    800007ac:	000780a3          	sb	zero,1(a5) # 10000001 <_entry-0x6fffffff>

  // special mode to set baud rate.
  WriteReg(LCR, LCR_BAUD_LATCH);
    800007b0:	f8000713          	li	a4,-128
    800007b4:	00e781a3          	sb	a4,3(a5)

  // LSB for baud rate of 38.4K.
  WriteReg(0, 0x03);
    800007b8:	470d                	li	a4,3
    800007ba:	00e78023          	sb	a4,0(a5)

  // MSB for baud rate of 38.4K.
  WriteReg(1, 0x00);
    800007be:	000780a3          	sb	zero,1(a5)

  // leave set-baud mode,
  // and set word length to 8 bits, no parity.
  WriteReg(LCR, LCR_EIGHT_BITS);
    800007c2:	00e781a3          	sb	a4,3(a5)

  // reset and enable FIFOs.
  WriteReg(FCR, FCR_FIFO_ENABLE | FCR_FIFO_CLEAR);
    800007c6:	469d                	li	a3,7
    800007c8:	00d78123          	sb	a3,2(a5)

  // enable transmit and receive interrupts.
  WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);
    800007cc:	00e780a3          	sb	a4,1(a5)

  initlock(&uart_tx_lock, "uart");
    800007d0:	00008597          	auipc	a1,0x8
    800007d4:	88858593          	addi	a1,a1,-1912 # 80008058 <digits+0x18>
    800007d8:	00011517          	auipc	a0,0x11
    800007dc:	a8050513          	addi	a0,a0,-1408 # 80011258 <uart_tx_lock>
    800007e0:	00000097          	auipc	ra,0x0
    800007e4:	374080e7          	jalr	884(ra) # 80000b54 <initlock>
}
    800007e8:	60a2                	ld	ra,8(sp)
    800007ea:	6402                	ld	s0,0(sp)
    800007ec:	0141                	addi	sp,sp,16
    800007ee:	8082                	ret

00000000800007f0 <uartputc_sync>:
// use interrupts, for use by kernel printf() and
// to echo characters. it spins waiting for the uart's
// output register to be empty.
void
uartputc_sync(int c)
{
    800007f0:	1101                	addi	sp,sp,-32
    800007f2:	ec06                	sd	ra,24(sp)
    800007f4:	e822                	sd	s0,16(sp)
    800007f6:	e426                	sd	s1,8(sp)
    800007f8:	1000                	addi	s0,sp,32
    800007fa:	84aa                	mv	s1,a0
  push_off();
    800007fc:	00000097          	auipc	ra,0x0
    80000800:	39c080e7          	jalr	924(ra) # 80000b98 <push_off>

  if(panicked){
    80000804:	00008797          	auipc	a5,0x8
    80000808:	7fc7a783          	lw	a5,2044(a5) # 80009000 <panicked>
    for(;;)
      ;
  }

  // wait for Transmit Holding Empty to be set in LSR.
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    8000080c:	10000737          	lui	a4,0x10000
  if(panicked){
    80000810:	c391                	beqz	a5,80000814 <uartputc_sync+0x24>
    for(;;)
    80000812:	a001                	j	80000812 <uartputc_sync+0x22>
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    80000814:	00574783          	lbu	a5,5(a4) # 10000005 <_entry-0x6ffffffb>
    80000818:	0ff7f793          	andi	a5,a5,255
    8000081c:	0207f793          	andi	a5,a5,32
    80000820:	dbf5                	beqz	a5,80000814 <uartputc_sync+0x24>
    ;
  WriteReg(THR, c);
    80000822:	0ff4f793          	andi	a5,s1,255
    80000826:	10000737          	lui	a4,0x10000
    8000082a:	00f70023          	sb	a5,0(a4) # 10000000 <_entry-0x70000000>

  pop_off();
    8000082e:	00000097          	auipc	ra,0x0
    80000832:	40a080e7          	jalr	1034(ra) # 80000c38 <pop_off>
}
    80000836:	60e2                	ld	ra,24(sp)
    80000838:	6442                	ld	s0,16(sp)
    8000083a:	64a2                	ld	s1,8(sp)
    8000083c:	6105                	addi	sp,sp,32
    8000083e:	8082                	ret

0000000080000840 <uartstart>:
// called from both the top- and bottom-half.
void
uartstart()
{
  while(1){
    if(uart_tx_w == uart_tx_r){
    80000840:	00008717          	auipc	a4,0x8
    80000844:	7c873703          	ld	a4,1992(a4) # 80009008 <uart_tx_r>
    80000848:	00008797          	auipc	a5,0x8
    8000084c:	7c87b783          	ld	a5,1992(a5) # 80009010 <uart_tx_w>
    80000850:	06e78c63          	beq	a5,a4,800008c8 <uartstart+0x88>
{
    80000854:	7139                	addi	sp,sp,-64
    80000856:	fc06                	sd	ra,56(sp)
    80000858:	f822                	sd	s0,48(sp)
    8000085a:	f426                	sd	s1,40(sp)
    8000085c:	f04a                	sd	s2,32(sp)
    8000085e:	ec4e                	sd	s3,24(sp)
    80000860:	e852                	sd	s4,16(sp)
    80000862:	e456                	sd	s5,8(sp)
    80000864:	0080                	addi	s0,sp,64
      // transmit buffer is empty.
      return;
    }
    
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){
    80000866:	10000937          	lui	s2,0x10000
      // so we cannot give it another byte.
      // it will interrupt when it's ready for a new byte.
      return;
    }
    
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE];
    8000086a:	00011a17          	auipc	s4,0x11
    8000086e:	9eea0a13          	addi	s4,s4,-1554 # 80011258 <uart_tx_lock>
    uart_tx_r += 1;
    80000872:	00008497          	auipc	s1,0x8
    80000876:	79648493          	addi	s1,s1,1942 # 80009008 <uart_tx_r>
    if(uart_tx_w == uart_tx_r){
    8000087a:	00008997          	auipc	s3,0x8
    8000087e:	79698993          	addi	s3,s3,1942 # 80009010 <uart_tx_w>
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){
    80000882:	00594783          	lbu	a5,5(s2) # 10000005 <_entry-0x6ffffffb>
    80000886:	0ff7f793          	andi	a5,a5,255
    8000088a:	0207f793          	andi	a5,a5,32
    8000088e:	c785                	beqz	a5,800008b6 <uartstart+0x76>
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE];
    80000890:	01f77793          	andi	a5,a4,31
    80000894:	97d2                	add	a5,a5,s4
    80000896:	0187ca83          	lbu	s5,24(a5)
    uart_tx_r += 1;
    8000089a:	0705                	addi	a4,a4,1
    8000089c:	e098                	sd	a4,0(s1)
    
    // maybe uartputc() is waiting for space in the buffer.
    wakeup(&uart_tx_r);
    8000089e:	8526                	mv	a0,s1
    800008a0:	00002097          	auipc	ra,0x2
    800008a4:	c82080e7          	jalr	-894(ra) # 80002522 <wakeup>
    
    WriteReg(THR, c);
    800008a8:	01590023          	sb	s5,0(s2)
    if(uart_tx_w == uart_tx_r){
    800008ac:	6098                	ld	a4,0(s1)
    800008ae:	0009b783          	ld	a5,0(s3)
    800008b2:	fce798e3          	bne	a5,a4,80000882 <uartstart+0x42>
  }
}
    800008b6:	70e2                	ld	ra,56(sp)
    800008b8:	7442                	ld	s0,48(sp)
    800008ba:	74a2                	ld	s1,40(sp)
    800008bc:	7902                	ld	s2,32(sp)
    800008be:	69e2                	ld	s3,24(sp)
    800008c0:	6a42                	ld	s4,16(sp)
    800008c2:	6aa2                	ld	s5,8(sp)
    800008c4:	6121                	addi	sp,sp,64
    800008c6:	8082                	ret
    800008c8:	8082                	ret

00000000800008ca <uartputc>:
{
    800008ca:	7179                	addi	sp,sp,-48
    800008cc:	f406                	sd	ra,40(sp)
    800008ce:	f022                	sd	s0,32(sp)
    800008d0:	ec26                	sd	s1,24(sp)
    800008d2:	e84a                	sd	s2,16(sp)
    800008d4:	e44e                	sd	s3,8(sp)
    800008d6:	e052                	sd	s4,0(sp)
    800008d8:	1800                	addi	s0,sp,48
    800008da:	89aa                	mv	s3,a0
  acquire(&uart_tx_lock);
    800008dc:	00011517          	auipc	a0,0x11
    800008e0:	97c50513          	addi	a0,a0,-1668 # 80011258 <uart_tx_lock>
    800008e4:	00000097          	auipc	ra,0x0
    800008e8:	300080e7          	jalr	768(ra) # 80000be4 <acquire>
  if(panicked){
    800008ec:	00008797          	auipc	a5,0x8
    800008f0:	7147a783          	lw	a5,1812(a5) # 80009000 <panicked>
    800008f4:	c391                	beqz	a5,800008f8 <uartputc+0x2e>
    for(;;)
    800008f6:	a001                	j	800008f6 <uartputc+0x2c>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    800008f8:	00008797          	auipc	a5,0x8
    800008fc:	7187b783          	ld	a5,1816(a5) # 80009010 <uart_tx_w>
    80000900:	00008717          	auipc	a4,0x8
    80000904:	70873703          	ld	a4,1800(a4) # 80009008 <uart_tx_r>
    80000908:	02070713          	addi	a4,a4,32
    8000090c:	02f71b63          	bne	a4,a5,80000942 <uartputc+0x78>
      sleep(&uart_tx_r, &uart_tx_lock);
    80000910:	00011a17          	auipc	s4,0x11
    80000914:	948a0a13          	addi	s4,s4,-1720 # 80011258 <uart_tx_lock>
    80000918:	00008497          	auipc	s1,0x8
    8000091c:	6f048493          	addi	s1,s1,1776 # 80009008 <uart_tx_r>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    80000920:	00008917          	auipc	s2,0x8
    80000924:	6f090913          	addi	s2,s2,1776 # 80009010 <uart_tx_w>
      sleep(&uart_tx_r, &uart_tx_lock);
    80000928:	85d2                	mv	a1,s4
    8000092a:	8526                	mv	a0,s1
    8000092c:	00002097          	auipc	ra,0x2
    80000930:	a58080e7          	jalr	-1448(ra) # 80002384 <sleep>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    80000934:	00093783          	ld	a5,0(s2)
    80000938:	6098                	ld	a4,0(s1)
    8000093a:	02070713          	addi	a4,a4,32
    8000093e:	fef705e3          	beq	a4,a5,80000928 <uartputc+0x5e>
      uart_tx_buf[uart_tx_w % UART_TX_BUF_SIZE] = c;
    80000942:	00011497          	auipc	s1,0x11
    80000946:	91648493          	addi	s1,s1,-1770 # 80011258 <uart_tx_lock>
    8000094a:	01f7f713          	andi	a4,a5,31
    8000094e:	9726                	add	a4,a4,s1
    80000950:	01370c23          	sb	s3,24(a4)
      uart_tx_w += 1;
    80000954:	0785                	addi	a5,a5,1
    80000956:	00008717          	auipc	a4,0x8
    8000095a:	6af73d23          	sd	a5,1722(a4) # 80009010 <uart_tx_w>
      uartstart();
    8000095e:	00000097          	auipc	ra,0x0
    80000962:	ee2080e7          	jalr	-286(ra) # 80000840 <uartstart>
      release(&uart_tx_lock);
    80000966:	8526                	mv	a0,s1
    80000968:	00000097          	auipc	ra,0x0
    8000096c:	330080e7          	jalr	816(ra) # 80000c98 <release>
}
    80000970:	70a2                	ld	ra,40(sp)
    80000972:	7402                	ld	s0,32(sp)
    80000974:	64e2                	ld	s1,24(sp)
    80000976:	6942                	ld	s2,16(sp)
    80000978:	69a2                	ld	s3,8(sp)
    8000097a:	6a02                	ld	s4,0(sp)
    8000097c:	6145                	addi	sp,sp,48
    8000097e:	8082                	ret

0000000080000980 <uartgetc>:

// read one input character from the UART.
// return -1 if none is waiting.
int
uartgetc(void)
{
    80000980:	1141                	addi	sp,sp,-16
    80000982:	e422                	sd	s0,8(sp)
    80000984:	0800                	addi	s0,sp,16
  if(ReadReg(LSR) & 0x01){
    80000986:	100007b7          	lui	a5,0x10000
    8000098a:	0057c783          	lbu	a5,5(a5) # 10000005 <_entry-0x6ffffffb>
    8000098e:	8b85                	andi	a5,a5,1
    80000990:	cb91                	beqz	a5,800009a4 <uartgetc+0x24>
    // input data is ready.
    return ReadReg(RHR);
    80000992:	100007b7          	lui	a5,0x10000
    80000996:	0007c503          	lbu	a0,0(a5) # 10000000 <_entry-0x70000000>
    8000099a:	0ff57513          	andi	a0,a0,255
  } else {
    return -1;
  }
}
    8000099e:	6422                	ld	s0,8(sp)
    800009a0:	0141                	addi	sp,sp,16
    800009a2:	8082                	ret
    return -1;
    800009a4:	557d                	li	a0,-1
    800009a6:	bfe5                	j	8000099e <uartgetc+0x1e>

00000000800009a8 <uartintr>:
// handle a uart interrupt, raised because input has
// arrived, or the uart is ready for more output, or
// both. called from trap.c.
void
uartintr(void)
{
    800009a8:	1101                	addi	sp,sp,-32
    800009aa:	ec06                	sd	ra,24(sp)
    800009ac:	e822                	sd	s0,16(sp)
    800009ae:	e426                	sd	s1,8(sp)
    800009b0:	1000                	addi	s0,sp,32
  // read and process incoming characters.
  while(1){
    int c = uartgetc();
    if(c == -1)
    800009b2:	54fd                	li	s1,-1
    int c = uartgetc();
    800009b4:	00000097          	auipc	ra,0x0
    800009b8:	fcc080e7          	jalr	-52(ra) # 80000980 <uartgetc>
    if(c == -1)
    800009bc:	00950763          	beq	a0,s1,800009ca <uartintr+0x22>
      break;
    consoleintr(c);
    800009c0:	00000097          	auipc	ra,0x0
    800009c4:	8fe080e7          	jalr	-1794(ra) # 800002be <consoleintr>
  while(1){
    800009c8:	b7f5                	j	800009b4 <uartintr+0xc>
  }

  // send buffered characters.
  acquire(&uart_tx_lock);
    800009ca:	00011497          	auipc	s1,0x11
    800009ce:	88e48493          	addi	s1,s1,-1906 # 80011258 <uart_tx_lock>
    800009d2:	8526                	mv	a0,s1
    800009d4:	00000097          	auipc	ra,0x0
    800009d8:	210080e7          	jalr	528(ra) # 80000be4 <acquire>
  uartstart();
    800009dc:	00000097          	auipc	ra,0x0
    800009e0:	e64080e7          	jalr	-412(ra) # 80000840 <uartstart>
  release(&uart_tx_lock);
    800009e4:	8526                	mv	a0,s1
    800009e6:	00000097          	auipc	ra,0x0
    800009ea:	2b2080e7          	jalr	690(ra) # 80000c98 <release>
}
    800009ee:	60e2                	ld	ra,24(sp)
    800009f0:	6442                	ld	s0,16(sp)
    800009f2:	64a2                	ld	s1,8(sp)
    800009f4:	6105                	addi	sp,sp,32
    800009f6:	8082                	ret

00000000800009f8 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
    800009f8:	1101                	addi	sp,sp,-32
    800009fa:	ec06                	sd	ra,24(sp)
    800009fc:	e822                	sd	s0,16(sp)
    800009fe:	e426                	sd	s1,8(sp)
    80000a00:	e04a                	sd	s2,0(sp)
    80000a02:	1000                	addi	s0,sp,32
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    80000a04:	03451793          	slli	a5,a0,0x34
    80000a08:	ebb9                	bnez	a5,80000a5e <kfree+0x66>
    80000a0a:	84aa                	mv	s1,a0
    80000a0c:	00025797          	auipc	a5,0x25
    80000a10:	5f478793          	addi	a5,a5,1524 # 80026000 <end>
    80000a14:	04f56563          	bltu	a0,a5,80000a5e <kfree+0x66>
    80000a18:	47c5                	li	a5,17
    80000a1a:	07ee                	slli	a5,a5,0x1b
    80000a1c:	04f57163          	bgeu	a0,a5,80000a5e <kfree+0x66>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);
    80000a20:	6605                	lui	a2,0x1
    80000a22:	4585                	li	a1,1
    80000a24:	00000097          	auipc	ra,0x0
    80000a28:	2bc080e7          	jalr	700(ra) # 80000ce0 <memset>

  r = (struct run*)pa;

  acquire(&kmem.lock);
    80000a2c:	00011917          	auipc	s2,0x11
    80000a30:	86490913          	addi	s2,s2,-1948 # 80011290 <kmem>
    80000a34:	854a                	mv	a0,s2
    80000a36:	00000097          	auipc	ra,0x0
    80000a3a:	1ae080e7          	jalr	430(ra) # 80000be4 <acquire>
  r->next = kmem.freelist;
    80000a3e:	01893783          	ld	a5,24(s2)
    80000a42:	e09c                	sd	a5,0(s1)
  kmem.freelist = r;
    80000a44:	00993c23          	sd	s1,24(s2)
  release(&kmem.lock);
    80000a48:	854a                	mv	a0,s2
    80000a4a:	00000097          	auipc	ra,0x0
    80000a4e:	24e080e7          	jalr	590(ra) # 80000c98 <release>
}
    80000a52:	60e2                	ld	ra,24(sp)
    80000a54:	6442                	ld	s0,16(sp)
    80000a56:	64a2                	ld	s1,8(sp)
    80000a58:	6902                	ld	s2,0(sp)
    80000a5a:	6105                	addi	sp,sp,32
    80000a5c:	8082                	ret
    panic("kfree");
    80000a5e:	00007517          	auipc	a0,0x7
    80000a62:	60250513          	addi	a0,a0,1538 # 80008060 <digits+0x20>
    80000a66:	00000097          	auipc	ra,0x0
    80000a6a:	ad8080e7          	jalr	-1320(ra) # 8000053e <panic>

0000000080000a6e <freerange>:
{
    80000a6e:	7179                	addi	sp,sp,-48
    80000a70:	f406                	sd	ra,40(sp)
    80000a72:	f022                	sd	s0,32(sp)
    80000a74:	ec26                	sd	s1,24(sp)
    80000a76:	e84a                	sd	s2,16(sp)
    80000a78:	e44e                	sd	s3,8(sp)
    80000a7a:	e052                	sd	s4,0(sp)
    80000a7c:	1800                	addi	s0,sp,48
  p = (char*)PGROUNDUP((uint64)pa_start);
    80000a7e:	6785                	lui	a5,0x1
    80000a80:	fff78493          	addi	s1,a5,-1 # fff <_entry-0x7ffff001>
    80000a84:	94aa                	add	s1,s1,a0
    80000a86:	757d                	lui	a0,0xfffff
    80000a88:	8ce9                	and	s1,s1,a0
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a8a:	94be                	add	s1,s1,a5
    80000a8c:	0095ee63          	bltu	a1,s1,80000aa8 <freerange+0x3a>
    80000a90:	892e                	mv	s2,a1
    kfree(p);
    80000a92:	7a7d                	lui	s4,0xfffff
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a94:	6985                	lui	s3,0x1
    kfree(p);
    80000a96:	01448533          	add	a0,s1,s4
    80000a9a:	00000097          	auipc	ra,0x0
    80000a9e:	f5e080e7          	jalr	-162(ra) # 800009f8 <kfree>
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000aa2:	94ce                	add	s1,s1,s3
    80000aa4:	fe9979e3          	bgeu	s2,s1,80000a96 <freerange+0x28>
}
    80000aa8:	70a2                	ld	ra,40(sp)
    80000aaa:	7402                	ld	s0,32(sp)
    80000aac:	64e2                	ld	s1,24(sp)
    80000aae:	6942                	ld	s2,16(sp)
    80000ab0:	69a2                	ld	s3,8(sp)
    80000ab2:	6a02                	ld	s4,0(sp)
    80000ab4:	6145                	addi	sp,sp,48
    80000ab6:	8082                	ret

0000000080000ab8 <kinit>:
{
    80000ab8:	1141                	addi	sp,sp,-16
    80000aba:	e406                	sd	ra,8(sp)
    80000abc:	e022                	sd	s0,0(sp)
    80000abe:	0800                	addi	s0,sp,16
  initlock(&kmem.lock, "kmem");
    80000ac0:	00007597          	auipc	a1,0x7
    80000ac4:	5a858593          	addi	a1,a1,1448 # 80008068 <digits+0x28>
    80000ac8:	00010517          	auipc	a0,0x10
    80000acc:	7c850513          	addi	a0,a0,1992 # 80011290 <kmem>
    80000ad0:	00000097          	auipc	ra,0x0
    80000ad4:	084080e7          	jalr	132(ra) # 80000b54 <initlock>
  freerange(end, (void*)PHYSTOP);
    80000ad8:	45c5                	li	a1,17
    80000ada:	05ee                	slli	a1,a1,0x1b
    80000adc:	00025517          	auipc	a0,0x25
    80000ae0:	52450513          	addi	a0,a0,1316 # 80026000 <end>
    80000ae4:	00000097          	auipc	ra,0x0
    80000ae8:	f8a080e7          	jalr	-118(ra) # 80000a6e <freerange>
}
    80000aec:	60a2                	ld	ra,8(sp)
    80000aee:	6402                	ld	s0,0(sp)
    80000af0:	0141                	addi	sp,sp,16
    80000af2:	8082                	ret

0000000080000af4 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
    80000af4:	1101                	addi	sp,sp,-32
    80000af6:	ec06                	sd	ra,24(sp)
    80000af8:	e822                	sd	s0,16(sp)
    80000afa:	e426                	sd	s1,8(sp)
    80000afc:	1000                	addi	s0,sp,32
  struct run *r;

  acquire(&kmem.lock);
    80000afe:	00010497          	auipc	s1,0x10
    80000b02:	79248493          	addi	s1,s1,1938 # 80011290 <kmem>
    80000b06:	8526                	mv	a0,s1
    80000b08:	00000097          	auipc	ra,0x0
    80000b0c:	0dc080e7          	jalr	220(ra) # 80000be4 <acquire>
  r = kmem.freelist;
    80000b10:	6c84                	ld	s1,24(s1)
  if(r)
    80000b12:	c885                	beqz	s1,80000b42 <kalloc+0x4e>
    kmem.freelist = r->next;
    80000b14:	609c                	ld	a5,0(s1)
    80000b16:	00010517          	auipc	a0,0x10
    80000b1a:	77a50513          	addi	a0,a0,1914 # 80011290 <kmem>
    80000b1e:	ed1c                	sd	a5,24(a0)
  release(&kmem.lock);
    80000b20:	00000097          	auipc	ra,0x0
    80000b24:	178080e7          	jalr	376(ra) # 80000c98 <release>

  if(r)
    memset((char*)r, 5, PGSIZE); // fill with junk
    80000b28:	6605                	lui	a2,0x1
    80000b2a:	4595                	li	a1,5
    80000b2c:	8526                	mv	a0,s1
    80000b2e:	00000097          	auipc	ra,0x0
    80000b32:	1b2080e7          	jalr	434(ra) # 80000ce0 <memset>
  return (void*)r;
}
    80000b36:	8526                	mv	a0,s1
    80000b38:	60e2                	ld	ra,24(sp)
    80000b3a:	6442                	ld	s0,16(sp)
    80000b3c:	64a2                	ld	s1,8(sp)
    80000b3e:	6105                	addi	sp,sp,32
    80000b40:	8082                	ret
  release(&kmem.lock);
    80000b42:	00010517          	auipc	a0,0x10
    80000b46:	74e50513          	addi	a0,a0,1870 # 80011290 <kmem>
    80000b4a:	00000097          	auipc	ra,0x0
    80000b4e:	14e080e7          	jalr	334(ra) # 80000c98 <release>
  if(r)
    80000b52:	b7d5                	j	80000b36 <kalloc+0x42>

0000000080000b54 <initlock>:
#include "proc.h"
#include "defs.h"

void
initlock(struct spinlock *lk, char *name)
{
    80000b54:	1141                	addi	sp,sp,-16
    80000b56:	e422                	sd	s0,8(sp)
    80000b58:	0800                	addi	s0,sp,16
  lk->name = name;
    80000b5a:	e50c                	sd	a1,8(a0)
  lk->locked = 0;
    80000b5c:	00052023          	sw	zero,0(a0)
  lk->cpu = 0;
    80000b60:	00053823          	sd	zero,16(a0)
}
    80000b64:	6422                	ld	s0,8(sp)
    80000b66:	0141                	addi	sp,sp,16
    80000b68:	8082                	ret

0000000080000b6a <holding>:
// Interrupts must be off.
int
holding(struct spinlock *lk)
{
  int r;
  r = (lk->locked && lk->cpu == mycpu());
    80000b6a:	411c                	lw	a5,0(a0)
    80000b6c:	e399                	bnez	a5,80000b72 <holding+0x8>
    80000b6e:	4501                	li	a0,0
  return r;
}
    80000b70:	8082                	ret
{
    80000b72:	1101                	addi	sp,sp,-32
    80000b74:	ec06                	sd	ra,24(sp)
    80000b76:	e822                	sd	s0,16(sp)
    80000b78:	e426                	sd	s1,8(sp)
    80000b7a:	1000                	addi	s0,sp,32
  r = (lk->locked && lk->cpu == mycpu());
    80000b7c:	6904                	ld	s1,16(a0)
    80000b7e:	00001097          	auipc	ra,0x1
    80000b82:	fe2080e7          	jalr	-30(ra) # 80001b60 <mycpu>
    80000b86:	40a48533          	sub	a0,s1,a0
    80000b8a:	00153513          	seqz	a0,a0
}
    80000b8e:	60e2                	ld	ra,24(sp)
    80000b90:	6442                	ld	s0,16(sp)
    80000b92:	64a2                	ld	s1,8(sp)
    80000b94:	6105                	addi	sp,sp,32
    80000b96:	8082                	ret

0000000080000b98 <push_off>:
// it takes two pop_off()s to undo two push_off()s.  Also, if interrupts
// are initially off, then push_off, pop_off leaves them off.

void
push_off(void)
{
    80000b98:	1101                	addi	sp,sp,-32
    80000b9a:	ec06                	sd	ra,24(sp)
    80000b9c:	e822                	sd	s0,16(sp)
    80000b9e:	e426                	sd	s1,8(sp)
    80000ba0:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000ba2:	100024f3          	csrr	s1,sstatus
    80000ba6:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80000baa:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000bac:	10079073          	csrw	sstatus,a5
  int old = intr_get();

  intr_off();
  if(mycpu()->noff == 0)
    80000bb0:	00001097          	auipc	ra,0x1
    80000bb4:	fb0080e7          	jalr	-80(ra) # 80001b60 <mycpu>
    80000bb8:	5d3c                	lw	a5,120(a0)
    80000bba:	cf89                	beqz	a5,80000bd4 <push_off+0x3c>
    mycpu()->intena = old;
  mycpu()->noff += 1;
    80000bbc:	00001097          	auipc	ra,0x1
    80000bc0:	fa4080e7          	jalr	-92(ra) # 80001b60 <mycpu>
    80000bc4:	5d3c                	lw	a5,120(a0)
    80000bc6:	2785                	addiw	a5,a5,1
    80000bc8:	dd3c                	sw	a5,120(a0)
}
    80000bca:	60e2                	ld	ra,24(sp)
    80000bcc:	6442                	ld	s0,16(sp)
    80000bce:	64a2                	ld	s1,8(sp)
    80000bd0:	6105                	addi	sp,sp,32
    80000bd2:	8082                	ret
    mycpu()->intena = old;
    80000bd4:	00001097          	auipc	ra,0x1
    80000bd8:	f8c080e7          	jalr	-116(ra) # 80001b60 <mycpu>
  return (x & SSTATUS_SIE) != 0;
    80000bdc:	8085                	srli	s1,s1,0x1
    80000bde:	8885                	andi	s1,s1,1
    80000be0:	dd64                	sw	s1,124(a0)
    80000be2:	bfe9                	j	80000bbc <push_off+0x24>

0000000080000be4 <acquire>:
{
    80000be4:	1101                	addi	sp,sp,-32
    80000be6:	ec06                	sd	ra,24(sp)
    80000be8:	e822                	sd	s0,16(sp)
    80000bea:	e426                	sd	s1,8(sp)
    80000bec:	1000                	addi	s0,sp,32
    80000bee:	84aa                	mv	s1,a0
  push_off(); // disable interrupts to avoid deadlock.
    80000bf0:	00000097          	auipc	ra,0x0
    80000bf4:	fa8080e7          	jalr	-88(ra) # 80000b98 <push_off>
  if(holding(lk))
    80000bf8:	8526                	mv	a0,s1
    80000bfa:	00000097          	auipc	ra,0x0
    80000bfe:	f70080e7          	jalr	-144(ra) # 80000b6a <holding>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000c02:	4705                	li	a4,1
  if(holding(lk))
    80000c04:	e115                	bnez	a0,80000c28 <acquire+0x44>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000c06:	87ba                	mv	a5,a4
    80000c08:	0cf4a7af          	amoswap.w.aq	a5,a5,(s1)
    80000c0c:	2781                	sext.w	a5,a5
    80000c0e:	ffe5                	bnez	a5,80000c06 <acquire+0x22>
  __sync_synchronize();
    80000c10:	0ff0000f          	fence
  lk->cpu = mycpu();
    80000c14:	00001097          	auipc	ra,0x1
    80000c18:	f4c080e7          	jalr	-180(ra) # 80001b60 <mycpu>
    80000c1c:	e888                	sd	a0,16(s1)
}
    80000c1e:	60e2                	ld	ra,24(sp)
    80000c20:	6442                	ld	s0,16(sp)
    80000c22:	64a2                	ld	s1,8(sp)
    80000c24:	6105                	addi	sp,sp,32
    80000c26:	8082                	ret
    panic("acquire");
    80000c28:	00007517          	auipc	a0,0x7
    80000c2c:	44850513          	addi	a0,a0,1096 # 80008070 <digits+0x30>
    80000c30:	00000097          	auipc	ra,0x0
    80000c34:	90e080e7          	jalr	-1778(ra) # 8000053e <panic>

0000000080000c38 <pop_off>:

void
pop_off(void)
{
    80000c38:	1141                	addi	sp,sp,-16
    80000c3a:	e406                	sd	ra,8(sp)
    80000c3c:	e022                	sd	s0,0(sp)
    80000c3e:	0800                	addi	s0,sp,16
  struct cpu *c = mycpu();
    80000c40:	00001097          	auipc	ra,0x1
    80000c44:	f20080e7          	jalr	-224(ra) # 80001b60 <mycpu>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c48:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80000c4c:	8b89                	andi	a5,a5,2
  if(intr_get())
    80000c4e:	e78d                	bnez	a5,80000c78 <pop_off+0x40>
    panic("pop_off - interruptible");
  if(c->noff < 1)
    80000c50:	5d3c                	lw	a5,120(a0)
    80000c52:	02f05b63          	blez	a5,80000c88 <pop_off+0x50>
    panic("pop_off");
  c->noff -= 1;
    80000c56:	37fd                	addiw	a5,a5,-1
    80000c58:	0007871b          	sext.w	a4,a5
    80000c5c:	dd3c                	sw	a5,120(a0)
  if(c->noff == 0 && c->intena)
    80000c5e:	eb09                	bnez	a4,80000c70 <pop_off+0x38>
    80000c60:	5d7c                	lw	a5,124(a0)
    80000c62:	c799                	beqz	a5,80000c70 <pop_off+0x38>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c64:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80000c68:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000c6c:	10079073          	csrw	sstatus,a5
    intr_on();
}
    80000c70:	60a2                	ld	ra,8(sp)
    80000c72:	6402                	ld	s0,0(sp)
    80000c74:	0141                	addi	sp,sp,16
    80000c76:	8082                	ret
    panic("pop_off - interruptible");
    80000c78:	00007517          	auipc	a0,0x7
    80000c7c:	40050513          	addi	a0,a0,1024 # 80008078 <digits+0x38>
    80000c80:	00000097          	auipc	ra,0x0
    80000c84:	8be080e7          	jalr	-1858(ra) # 8000053e <panic>
    panic("pop_off");
    80000c88:	00007517          	auipc	a0,0x7
    80000c8c:	40850513          	addi	a0,a0,1032 # 80008090 <digits+0x50>
    80000c90:	00000097          	auipc	ra,0x0
    80000c94:	8ae080e7          	jalr	-1874(ra) # 8000053e <panic>

0000000080000c98 <release>:
{
    80000c98:	1101                	addi	sp,sp,-32
    80000c9a:	ec06                	sd	ra,24(sp)
    80000c9c:	e822                	sd	s0,16(sp)
    80000c9e:	e426                	sd	s1,8(sp)
    80000ca0:	1000                	addi	s0,sp,32
    80000ca2:	84aa                	mv	s1,a0
  if(!holding(lk))
    80000ca4:	00000097          	auipc	ra,0x0
    80000ca8:	ec6080e7          	jalr	-314(ra) # 80000b6a <holding>
    80000cac:	c115                	beqz	a0,80000cd0 <release+0x38>
  lk->cpu = 0;
    80000cae:	0004b823          	sd	zero,16(s1)
  __sync_synchronize();
    80000cb2:	0ff0000f          	fence
  __sync_lock_release(&lk->locked);
    80000cb6:	0f50000f          	fence	iorw,ow
    80000cba:	0804a02f          	amoswap.w	zero,zero,(s1)
  pop_off();
    80000cbe:	00000097          	auipc	ra,0x0
    80000cc2:	f7a080e7          	jalr	-134(ra) # 80000c38 <pop_off>
}
    80000cc6:	60e2                	ld	ra,24(sp)
    80000cc8:	6442                	ld	s0,16(sp)
    80000cca:	64a2                	ld	s1,8(sp)
    80000ccc:	6105                	addi	sp,sp,32
    80000cce:	8082                	ret
    panic("release");
    80000cd0:	00007517          	auipc	a0,0x7
    80000cd4:	3c850513          	addi	a0,a0,968 # 80008098 <digits+0x58>
    80000cd8:	00000097          	auipc	ra,0x0
    80000cdc:	866080e7          	jalr	-1946(ra) # 8000053e <panic>

0000000080000ce0 <memset>:
#include "types.h"

void*
memset(void *dst, int c, uint n)
{
    80000ce0:	1141                	addi	sp,sp,-16
    80000ce2:	e422                	sd	s0,8(sp)
    80000ce4:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
    80000ce6:	ce09                	beqz	a2,80000d00 <memset+0x20>
    80000ce8:	87aa                	mv	a5,a0
    80000cea:	fff6071b          	addiw	a4,a2,-1
    80000cee:	1702                	slli	a4,a4,0x20
    80000cf0:	9301                	srli	a4,a4,0x20
    80000cf2:	0705                	addi	a4,a4,1
    80000cf4:	972a                	add	a4,a4,a0
    cdst[i] = c;
    80000cf6:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
    80000cfa:	0785                	addi	a5,a5,1
    80000cfc:	fee79de3          	bne	a5,a4,80000cf6 <memset+0x16>
  }
  return dst;
}
    80000d00:	6422                	ld	s0,8(sp)
    80000d02:	0141                	addi	sp,sp,16
    80000d04:	8082                	ret

0000000080000d06 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
    80000d06:	1141                	addi	sp,sp,-16
    80000d08:	e422                	sd	s0,8(sp)
    80000d0a:	0800                	addi	s0,sp,16
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
    80000d0c:	ca05                	beqz	a2,80000d3c <memcmp+0x36>
    80000d0e:	fff6069b          	addiw	a3,a2,-1
    80000d12:	1682                	slli	a3,a3,0x20
    80000d14:	9281                	srli	a3,a3,0x20
    80000d16:	0685                	addi	a3,a3,1
    80000d18:	96aa                	add	a3,a3,a0
    if(*s1 != *s2)
    80000d1a:	00054783          	lbu	a5,0(a0)
    80000d1e:	0005c703          	lbu	a4,0(a1)
    80000d22:	00e79863          	bne	a5,a4,80000d32 <memcmp+0x2c>
      return *s1 - *s2;
    s1++, s2++;
    80000d26:	0505                	addi	a0,a0,1
    80000d28:	0585                	addi	a1,a1,1
  while(n-- > 0){
    80000d2a:	fed518e3          	bne	a0,a3,80000d1a <memcmp+0x14>
  }

  return 0;
    80000d2e:	4501                	li	a0,0
    80000d30:	a019                	j	80000d36 <memcmp+0x30>
      return *s1 - *s2;
    80000d32:	40e7853b          	subw	a0,a5,a4
}
    80000d36:	6422                	ld	s0,8(sp)
    80000d38:	0141                	addi	sp,sp,16
    80000d3a:	8082                	ret
  return 0;
    80000d3c:	4501                	li	a0,0
    80000d3e:	bfe5                	j	80000d36 <memcmp+0x30>

0000000080000d40 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
    80000d40:	1141                	addi	sp,sp,-16
    80000d42:	e422                	sd	s0,8(sp)
    80000d44:	0800                	addi	s0,sp,16
  const char *s;
  char *d;

  if(n == 0)
    80000d46:	ca0d                	beqz	a2,80000d78 <memmove+0x38>
    return dst;
  
  s = src;
  d = dst;
  if(s < d && s + n > d){
    80000d48:	00a5f963          	bgeu	a1,a0,80000d5a <memmove+0x1a>
    80000d4c:	02061693          	slli	a3,a2,0x20
    80000d50:	9281                	srli	a3,a3,0x20
    80000d52:	00d58733          	add	a4,a1,a3
    80000d56:	02e56463          	bltu	a0,a4,80000d7e <memmove+0x3e>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
    80000d5a:	fff6079b          	addiw	a5,a2,-1
    80000d5e:	1782                	slli	a5,a5,0x20
    80000d60:	9381                	srli	a5,a5,0x20
    80000d62:	0785                	addi	a5,a5,1
    80000d64:	97ae                	add	a5,a5,a1
    80000d66:	872a                	mv	a4,a0
      *d++ = *s++;
    80000d68:	0585                	addi	a1,a1,1
    80000d6a:	0705                	addi	a4,a4,1
    80000d6c:	fff5c683          	lbu	a3,-1(a1)
    80000d70:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
    80000d74:	fef59ae3          	bne	a1,a5,80000d68 <memmove+0x28>

  return dst;
}
    80000d78:	6422                	ld	s0,8(sp)
    80000d7a:	0141                	addi	sp,sp,16
    80000d7c:	8082                	ret
    d += n;
    80000d7e:	96aa                	add	a3,a3,a0
    while(n-- > 0)
    80000d80:	fff6079b          	addiw	a5,a2,-1
    80000d84:	1782                	slli	a5,a5,0x20
    80000d86:	9381                	srli	a5,a5,0x20
    80000d88:	fff7c793          	not	a5,a5
    80000d8c:	97ba                	add	a5,a5,a4
      *--d = *--s;
    80000d8e:	177d                	addi	a4,a4,-1
    80000d90:	16fd                	addi	a3,a3,-1
    80000d92:	00074603          	lbu	a2,0(a4)
    80000d96:	00c68023          	sb	a2,0(a3)
    while(n-- > 0)
    80000d9a:	fef71ae3          	bne	a4,a5,80000d8e <memmove+0x4e>
    80000d9e:	bfe9                	j	80000d78 <memmove+0x38>

0000000080000da0 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
    80000da0:	1141                	addi	sp,sp,-16
    80000da2:	e406                	sd	ra,8(sp)
    80000da4:	e022                	sd	s0,0(sp)
    80000da6:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
    80000da8:	00000097          	auipc	ra,0x0
    80000dac:	f98080e7          	jalr	-104(ra) # 80000d40 <memmove>
}
    80000db0:	60a2                	ld	ra,8(sp)
    80000db2:	6402                	ld	s0,0(sp)
    80000db4:	0141                	addi	sp,sp,16
    80000db6:	8082                	ret

0000000080000db8 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
    80000db8:	1141                	addi	sp,sp,-16
    80000dba:	e422                	sd	s0,8(sp)
    80000dbc:	0800                	addi	s0,sp,16
  while(n > 0 && *p && *p == *q)
    80000dbe:	ce11                	beqz	a2,80000dda <strncmp+0x22>
    80000dc0:	00054783          	lbu	a5,0(a0)
    80000dc4:	cf89                	beqz	a5,80000dde <strncmp+0x26>
    80000dc6:	0005c703          	lbu	a4,0(a1)
    80000dca:	00f71a63          	bne	a4,a5,80000dde <strncmp+0x26>
    n--, p++, q++;
    80000dce:	367d                	addiw	a2,a2,-1
    80000dd0:	0505                	addi	a0,a0,1
    80000dd2:	0585                	addi	a1,a1,1
  while(n > 0 && *p && *p == *q)
    80000dd4:	f675                	bnez	a2,80000dc0 <strncmp+0x8>
  if(n == 0)
    return 0;
    80000dd6:	4501                	li	a0,0
    80000dd8:	a809                	j	80000dea <strncmp+0x32>
    80000dda:	4501                	li	a0,0
    80000ddc:	a039                	j	80000dea <strncmp+0x32>
  if(n == 0)
    80000dde:	ca09                	beqz	a2,80000df0 <strncmp+0x38>
  return (uchar)*p - (uchar)*q;
    80000de0:	00054503          	lbu	a0,0(a0)
    80000de4:	0005c783          	lbu	a5,0(a1)
    80000de8:	9d1d                	subw	a0,a0,a5
}
    80000dea:	6422                	ld	s0,8(sp)
    80000dec:	0141                	addi	sp,sp,16
    80000dee:	8082                	ret
    return 0;
    80000df0:	4501                	li	a0,0
    80000df2:	bfe5                	j	80000dea <strncmp+0x32>

0000000080000df4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
    80000df4:	1141                	addi	sp,sp,-16
    80000df6:	e422                	sd	s0,8(sp)
    80000df8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    80000dfa:	872a                	mv	a4,a0
    80000dfc:	8832                	mv	a6,a2
    80000dfe:	367d                	addiw	a2,a2,-1
    80000e00:	01005963          	blez	a6,80000e12 <strncpy+0x1e>
    80000e04:	0705                	addi	a4,a4,1
    80000e06:	0005c783          	lbu	a5,0(a1)
    80000e0a:	fef70fa3          	sb	a5,-1(a4)
    80000e0e:	0585                	addi	a1,a1,1
    80000e10:	f7f5                	bnez	a5,80000dfc <strncpy+0x8>
    ;
  while(n-- > 0)
    80000e12:	00c05d63          	blez	a2,80000e2c <strncpy+0x38>
    80000e16:	86ba                	mv	a3,a4
    *s++ = 0;
    80000e18:	0685                	addi	a3,a3,1
    80000e1a:	fe068fa3          	sb	zero,-1(a3)
  while(n-- > 0)
    80000e1e:	fff6c793          	not	a5,a3
    80000e22:	9fb9                	addw	a5,a5,a4
    80000e24:	010787bb          	addw	a5,a5,a6
    80000e28:	fef048e3          	bgtz	a5,80000e18 <strncpy+0x24>
  return os;
}
    80000e2c:	6422                	ld	s0,8(sp)
    80000e2e:	0141                	addi	sp,sp,16
    80000e30:	8082                	ret

0000000080000e32 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
    80000e32:	1141                	addi	sp,sp,-16
    80000e34:	e422                	sd	s0,8(sp)
    80000e36:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  if(n <= 0)
    80000e38:	02c05363          	blez	a2,80000e5e <safestrcpy+0x2c>
    80000e3c:	fff6069b          	addiw	a3,a2,-1
    80000e40:	1682                	slli	a3,a3,0x20
    80000e42:	9281                	srli	a3,a3,0x20
    80000e44:	96ae                	add	a3,a3,a1
    80000e46:	87aa                	mv	a5,a0
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
    80000e48:	00d58963          	beq	a1,a3,80000e5a <safestrcpy+0x28>
    80000e4c:	0585                	addi	a1,a1,1
    80000e4e:	0785                	addi	a5,a5,1
    80000e50:	fff5c703          	lbu	a4,-1(a1)
    80000e54:	fee78fa3          	sb	a4,-1(a5)
    80000e58:	fb65                	bnez	a4,80000e48 <safestrcpy+0x16>
    ;
  *s = 0;
    80000e5a:	00078023          	sb	zero,0(a5)
  return os;
}
    80000e5e:	6422                	ld	s0,8(sp)
    80000e60:	0141                	addi	sp,sp,16
    80000e62:	8082                	ret

0000000080000e64 <strlen>:

int
strlen(const char *s)
{
    80000e64:	1141                	addi	sp,sp,-16
    80000e66:	e422                	sd	s0,8(sp)
    80000e68:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
    80000e6a:	00054783          	lbu	a5,0(a0)
    80000e6e:	cf91                	beqz	a5,80000e8a <strlen+0x26>
    80000e70:	0505                	addi	a0,a0,1
    80000e72:	87aa                	mv	a5,a0
    80000e74:	4685                	li	a3,1
    80000e76:	9e89                	subw	a3,a3,a0
    80000e78:	00f6853b          	addw	a0,a3,a5
    80000e7c:	0785                	addi	a5,a5,1
    80000e7e:	fff7c703          	lbu	a4,-1(a5)
    80000e82:	fb7d                	bnez	a4,80000e78 <strlen+0x14>
    ;
  return n;
}
    80000e84:	6422                	ld	s0,8(sp)
    80000e86:	0141                	addi	sp,sp,16
    80000e88:	8082                	ret
  for(n = 0; s[n]; n++)
    80000e8a:	4501                	li	a0,0
    80000e8c:	bfe5                	j	80000e84 <strlen+0x20>

0000000080000e8e <main>:
volatile static int started = 0;

// start() jumps here in supervisor mode on all CPUs.
void
main()
{
    80000e8e:	1141                	addi	sp,sp,-16
    80000e90:	e406                	sd	ra,8(sp)
    80000e92:	e022                	sd	s0,0(sp)
    80000e94:	0800                	addi	s0,sp,16
  if(cpuid() == 0){
    80000e96:	00001097          	auipc	ra,0x1
    80000e9a:	cba080e7          	jalr	-838(ra) # 80001b50 <cpuid>
    virtio_disk_init(); // emulated hard disk
    userinit();      // first user process
    __sync_synchronize();
    started = 1;
  } else {
    while(started == 0)
    80000e9e:	00008717          	auipc	a4,0x8
    80000ea2:	17a70713          	addi	a4,a4,378 # 80009018 <started>
  if(cpuid() == 0){
    80000ea6:	c139                	beqz	a0,80000eec <main+0x5e>
    while(started == 0)
    80000ea8:	431c                	lw	a5,0(a4)
    80000eaa:	2781                	sext.w	a5,a5
    80000eac:	dff5                	beqz	a5,80000ea8 <main+0x1a>
      ;
    __sync_synchronize();
    80000eae:	0ff0000f          	fence
    printf("hart %d starting\n", cpuid());
    80000eb2:	00001097          	auipc	ra,0x1
    80000eb6:	c9e080e7          	jalr	-866(ra) # 80001b50 <cpuid>
    80000eba:	85aa                	mv	a1,a0
    80000ebc:	00007517          	auipc	a0,0x7
    80000ec0:	1fc50513          	addi	a0,a0,508 # 800080b8 <digits+0x78>
    80000ec4:	fffff097          	auipc	ra,0xfffff
    80000ec8:	6c4080e7          	jalr	1732(ra) # 80000588 <printf>
    kvminithart();    // turn on paging
    80000ecc:	00000097          	auipc	ra,0x0
    80000ed0:	0d8080e7          	jalr	216(ra) # 80000fa4 <kvminithart>
    trapinithart();   // install kernel trap vector
    80000ed4:	00002097          	auipc	ra,0x2
    80000ed8:	b34080e7          	jalr	-1228(ra) # 80002a08 <trapinithart>
    plicinithart();   // ask PLIC for device interrupts
    80000edc:	00005097          	auipc	ra,0x5
    80000ee0:	0d4080e7          	jalr	212(ra) # 80005fb0 <plicinithart>
  }

  scheduler();        
    80000ee4:	00001097          	auipc	ra,0x1
    80000ee8:	294080e7          	jalr	660(ra) # 80002178 <scheduler>
    consoleinit();
    80000eec:	fffff097          	auipc	ra,0xfffff
    80000ef0:	564080e7          	jalr	1380(ra) # 80000450 <consoleinit>
    printfinit();
    80000ef4:	00000097          	auipc	ra,0x0
    80000ef8:	87a080e7          	jalr	-1926(ra) # 8000076e <printfinit>
    printf("\n");
    80000efc:	00007517          	auipc	a0,0x7
    80000f00:	1cc50513          	addi	a0,a0,460 # 800080c8 <digits+0x88>
    80000f04:	fffff097          	auipc	ra,0xfffff
    80000f08:	684080e7          	jalr	1668(ra) # 80000588 <printf>
    printf("xv6 kernel is booting\n");
    80000f0c:	00007517          	auipc	a0,0x7
    80000f10:	19450513          	addi	a0,a0,404 # 800080a0 <digits+0x60>
    80000f14:	fffff097          	auipc	ra,0xfffff
    80000f18:	674080e7          	jalr	1652(ra) # 80000588 <printf>
    printf("\n");
    80000f1c:	00007517          	auipc	a0,0x7
    80000f20:	1ac50513          	addi	a0,a0,428 # 800080c8 <digits+0x88>
    80000f24:	fffff097          	auipc	ra,0xfffff
    80000f28:	664080e7          	jalr	1636(ra) # 80000588 <printf>
    kinit();         // physical page allocator
    80000f2c:	00000097          	auipc	ra,0x0
    80000f30:	b8c080e7          	jalr	-1140(ra) # 80000ab8 <kinit>
    kvminit();       // create kernel page table
    80000f34:	00000097          	auipc	ra,0x0
    80000f38:	322080e7          	jalr	802(ra) # 80001256 <kvminit>
    kvminithart();   // turn on paging
    80000f3c:	00000097          	auipc	ra,0x0
    80000f40:	068080e7          	jalr	104(ra) # 80000fa4 <kvminithart>
    procinit();      // process table
    80000f44:	00001097          	auipc	ra,0x1
    80000f48:	b28080e7          	jalr	-1240(ra) # 80001a6c <procinit>
    trapinit();      // trap vectors
    80000f4c:	00002097          	auipc	ra,0x2
    80000f50:	a94080e7          	jalr	-1388(ra) # 800029e0 <trapinit>
    trapinithart();  // install kernel trap vector
    80000f54:	00002097          	auipc	ra,0x2
    80000f58:	ab4080e7          	jalr	-1356(ra) # 80002a08 <trapinithart>
    plicinit();      // set up interrupt controller
    80000f5c:	00005097          	auipc	ra,0x5
    80000f60:	03e080e7          	jalr	62(ra) # 80005f9a <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000f64:	00005097          	auipc	ra,0x5
    80000f68:	04c080e7          	jalr	76(ra) # 80005fb0 <plicinithart>
    binit();         // buffer cache
    80000f6c:	00002097          	auipc	ra,0x2
    80000f70:	228080e7          	jalr	552(ra) # 80003194 <binit>
    iinit();         // inode table
    80000f74:	00003097          	auipc	ra,0x3
    80000f78:	8b8080e7          	jalr	-1864(ra) # 8000382c <iinit>
    fileinit();      // file table
    80000f7c:	00004097          	auipc	ra,0x4
    80000f80:	862080e7          	jalr	-1950(ra) # 800047de <fileinit>
    virtio_disk_init(); // emulated hard disk
    80000f84:	00005097          	auipc	ra,0x5
    80000f88:	14e080e7          	jalr	334(ra) # 800060d2 <virtio_disk_init>
    userinit();      // first user process
    80000f8c:	00001097          	auipc	ra,0x1
    80000f90:	f82080e7          	jalr	-126(ra) # 80001f0e <userinit>
    __sync_synchronize();
    80000f94:	0ff0000f          	fence
    started = 1;
    80000f98:	4785                	li	a5,1
    80000f9a:	00008717          	auipc	a4,0x8
    80000f9e:	06f72f23          	sw	a5,126(a4) # 80009018 <started>
    80000fa2:	b789                	j	80000ee4 <main+0x56>

0000000080000fa4 <kvminithart>:

// Switch h/w page table register to the kernel's page table,
// and enable paging.
void
kvminithart()
{
    80000fa4:	1141                	addi	sp,sp,-16
    80000fa6:	e422                	sd	s0,8(sp)
    80000fa8:	0800                	addi	s0,sp,16
  w_satp(MAKE_SATP(kernel_pagetable));
    80000faa:	00008797          	auipc	a5,0x8
    80000fae:	0767b783          	ld	a5,118(a5) # 80009020 <kernel_pagetable>
    80000fb2:	83b1                	srli	a5,a5,0xc
    80000fb4:	577d                	li	a4,-1
    80000fb6:	177e                	slli	a4,a4,0x3f
    80000fb8:	8fd9                	or	a5,a5,a4
  asm volatile("csrw satp, %0" : : "r" (x));
    80000fba:	18079073          	csrw	satp,a5
// flush the TLB.
static inline void
sfence_vma()
{
  // the zero, zero means flush all TLB entries.
  asm volatile("sfence.vma zero, zero");
    80000fbe:	12000073          	sfence.vma
  sfence_vma();
}
    80000fc2:	6422                	ld	s0,8(sp)
    80000fc4:	0141                	addi	sp,sp,16
    80000fc6:	8082                	ret

0000000080000fc8 <walk>:
//   21..29 -- 9 bits of level-1 index.
//   12..20 -- 9 bits of level-0 index.
//    0..11 -- 12 bits of byte offset within the page.
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)
{
    80000fc8:	7139                	addi	sp,sp,-64
    80000fca:	fc06                	sd	ra,56(sp)
    80000fcc:	f822                	sd	s0,48(sp)
    80000fce:	f426                	sd	s1,40(sp)
    80000fd0:	f04a                	sd	s2,32(sp)
    80000fd2:	ec4e                	sd	s3,24(sp)
    80000fd4:	e852                	sd	s4,16(sp)
    80000fd6:	e456                	sd	s5,8(sp)
    80000fd8:	e05a                	sd	s6,0(sp)
    80000fda:	0080                	addi	s0,sp,64
    80000fdc:	84aa                	mv	s1,a0
    80000fde:	89ae                	mv	s3,a1
    80000fe0:	8ab2                	mv	s5,a2
  if(va >= MAXVA)
    80000fe2:	57fd                	li	a5,-1
    80000fe4:	83e9                	srli	a5,a5,0x1a
    80000fe6:	4a79                	li	s4,30
    panic("walk");

  for(int level = 2; level > 0; level--) {
    80000fe8:	4b31                	li	s6,12
  if(va >= MAXVA)
    80000fea:	04b7f263          	bgeu	a5,a1,8000102e <walk+0x66>
    panic("walk");
    80000fee:	00007517          	auipc	a0,0x7
    80000ff2:	0e250513          	addi	a0,a0,226 # 800080d0 <digits+0x90>
    80000ff6:	fffff097          	auipc	ra,0xfffff
    80000ffa:	548080e7          	jalr	1352(ra) # 8000053e <panic>
    pte_t *pte = &pagetable[PX(level, va)];
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0)
    80000ffe:	060a8663          	beqz	s5,8000106a <walk+0xa2>
    80001002:	00000097          	auipc	ra,0x0
    80001006:	af2080e7          	jalr	-1294(ra) # 80000af4 <kalloc>
    8000100a:	84aa                	mv	s1,a0
    8000100c:	c529                	beqz	a0,80001056 <walk+0x8e>
        return 0;
      memset(pagetable, 0, PGSIZE);
    8000100e:	6605                	lui	a2,0x1
    80001010:	4581                	li	a1,0
    80001012:	00000097          	auipc	ra,0x0
    80001016:	cce080e7          	jalr	-818(ra) # 80000ce0 <memset>
      *pte = PA2PTE(pagetable) | PTE_V;
    8000101a:	00c4d793          	srli	a5,s1,0xc
    8000101e:	07aa                	slli	a5,a5,0xa
    80001020:	0017e793          	ori	a5,a5,1
    80001024:	00f93023          	sd	a5,0(s2)
  for(int level = 2; level > 0; level--) {
    80001028:	3a5d                	addiw	s4,s4,-9
    8000102a:	036a0063          	beq	s4,s6,8000104a <walk+0x82>
    pte_t *pte = &pagetable[PX(level, va)];
    8000102e:	0149d933          	srl	s2,s3,s4
    80001032:	1ff97913          	andi	s2,s2,511
    80001036:	090e                	slli	s2,s2,0x3
    80001038:	9926                	add	s2,s2,s1
    if(*pte & PTE_V) {
    8000103a:	00093483          	ld	s1,0(s2)
    8000103e:	0014f793          	andi	a5,s1,1
    80001042:	dfd5                	beqz	a5,80000ffe <walk+0x36>
      pagetable = (pagetable_t)PTE2PA(*pte);
    80001044:	80a9                	srli	s1,s1,0xa
    80001046:	04b2                	slli	s1,s1,0xc
    80001048:	b7c5                	j	80001028 <walk+0x60>
    }
  }
  return &pagetable[PX(0, va)];
    8000104a:	00c9d513          	srli	a0,s3,0xc
    8000104e:	1ff57513          	andi	a0,a0,511
    80001052:	050e                	slli	a0,a0,0x3
    80001054:	9526                	add	a0,a0,s1
}
    80001056:	70e2                	ld	ra,56(sp)
    80001058:	7442                	ld	s0,48(sp)
    8000105a:	74a2                	ld	s1,40(sp)
    8000105c:	7902                	ld	s2,32(sp)
    8000105e:	69e2                	ld	s3,24(sp)
    80001060:	6a42                	ld	s4,16(sp)
    80001062:	6aa2                	ld	s5,8(sp)
    80001064:	6b02                	ld	s6,0(sp)
    80001066:	6121                	addi	sp,sp,64
    80001068:	8082                	ret
        return 0;
    8000106a:	4501                	li	a0,0
    8000106c:	b7ed                	j	80001056 <walk+0x8e>

000000008000106e <walkaddr>:
walkaddr(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;

  if(va >= MAXVA)
    8000106e:	57fd                	li	a5,-1
    80001070:	83e9                	srli	a5,a5,0x1a
    80001072:	00b7f463          	bgeu	a5,a1,8000107a <walkaddr+0xc>
    return 0;
    80001076:	4501                	li	a0,0
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  pa = PTE2PA(*pte);
  return pa;
}
    80001078:	8082                	ret
{
    8000107a:	1141                	addi	sp,sp,-16
    8000107c:	e406                	sd	ra,8(sp)
    8000107e:	e022                	sd	s0,0(sp)
    80001080:	0800                	addi	s0,sp,16
  pte = walk(pagetable, va, 0);
    80001082:	4601                	li	a2,0
    80001084:	00000097          	auipc	ra,0x0
    80001088:	f44080e7          	jalr	-188(ra) # 80000fc8 <walk>
  if(pte == 0)
    8000108c:	c105                	beqz	a0,800010ac <walkaddr+0x3e>
  if((*pte & PTE_V) == 0)
    8000108e:	611c                	ld	a5,0(a0)
  if((*pte & PTE_U) == 0)
    80001090:	0117f693          	andi	a3,a5,17
    80001094:	4745                	li	a4,17
    return 0;
    80001096:	4501                	li	a0,0
  if((*pte & PTE_U) == 0)
    80001098:	00e68663          	beq	a3,a4,800010a4 <walkaddr+0x36>
}
    8000109c:	60a2                	ld	ra,8(sp)
    8000109e:	6402                	ld	s0,0(sp)
    800010a0:	0141                	addi	sp,sp,16
    800010a2:	8082                	ret
  pa = PTE2PA(*pte);
    800010a4:	00a7d513          	srli	a0,a5,0xa
    800010a8:	0532                	slli	a0,a0,0xc
  return pa;
    800010aa:	bfcd                	j	8000109c <walkaddr+0x2e>
    return 0;
    800010ac:	4501                	li	a0,0
    800010ae:	b7fd                	j	8000109c <walkaddr+0x2e>

00000000800010b0 <mappages>:
// physical addresses starting at pa. va and size might not
// be page-aligned. Returns 0 on success, -1 if walk() couldn't
// allocate a needed page-table page.
int
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
    800010b0:	715d                	addi	sp,sp,-80
    800010b2:	e486                	sd	ra,72(sp)
    800010b4:	e0a2                	sd	s0,64(sp)
    800010b6:	fc26                	sd	s1,56(sp)
    800010b8:	f84a                	sd	s2,48(sp)
    800010ba:	f44e                	sd	s3,40(sp)
    800010bc:	f052                	sd	s4,32(sp)
    800010be:	ec56                	sd	s5,24(sp)
    800010c0:	e85a                	sd	s6,16(sp)
    800010c2:	e45e                	sd	s7,8(sp)
    800010c4:	0880                	addi	s0,sp,80
  uint64 a, last;
  pte_t *pte;

  if(size == 0)
    800010c6:	c205                	beqz	a2,800010e6 <mappages+0x36>
    800010c8:	8aaa                	mv	s5,a0
    800010ca:	8b3a                	mv	s6,a4
    panic("mappages: size");
  
  a = PGROUNDDOWN(va);
    800010cc:	77fd                	lui	a5,0xfffff
    800010ce:	00f5fa33          	and	s4,a1,a5
  last = PGROUNDDOWN(va + size - 1);
    800010d2:	15fd                	addi	a1,a1,-1
    800010d4:	00c589b3          	add	s3,a1,a2
    800010d8:	00f9f9b3          	and	s3,s3,a5
  a = PGROUNDDOWN(va);
    800010dc:	8952                	mv	s2,s4
    800010de:	41468a33          	sub	s4,a3,s4
    if(*pte & PTE_V)
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V;
    if(a == last)
      break;
    a += PGSIZE;
    800010e2:	6b85                	lui	s7,0x1
    800010e4:	a015                	j	80001108 <mappages+0x58>
    panic("mappages: size");
    800010e6:	00007517          	auipc	a0,0x7
    800010ea:	ff250513          	addi	a0,a0,-14 # 800080d8 <digits+0x98>
    800010ee:	fffff097          	auipc	ra,0xfffff
    800010f2:	450080e7          	jalr	1104(ra) # 8000053e <panic>
      panic("mappages: remap");
    800010f6:	00007517          	auipc	a0,0x7
    800010fa:	ff250513          	addi	a0,a0,-14 # 800080e8 <digits+0xa8>
    800010fe:	fffff097          	auipc	ra,0xfffff
    80001102:	440080e7          	jalr	1088(ra) # 8000053e <panic>
    a += PGSIZE;
    80001106:	995e                	add	s2,s2,s7
  for(;;){
    80001108:	012a04b3          	add	s1,s4,s2
    if((pte = walk(pagetable, a, 1)) == 0)
    8000110c:	4605                	li	a2,1
    8000110e:	85ca                	mv	a1,s2
    80001110:	8556                	mv	a0,s5
    80001112:	00000097          	auipc	ra,0x0
    80001116:	eb6080e7          	jalr	-330(ra) # 80000fc8 <walk>
    8000111a:	cd19                	beqz	a0,80001138 <mappages+0x88>
    if(*pte & PTE_V)
    8000111c:	611c                	ld	a5,0(a0)
    8000111e:	8b85                	andi	a5,a5,1
    80001120:	fbf9                	bnez	a5,800010f6 <mappages+0x46>
    *pte = PA2PTE(pa) | perm | PTE_V;
    80001122:	80b1                	srli	s1,s1,0xc
    80001124:	04aa                	slli	s1,s1,0xa
    80001126:	0164e4b3          	or	s1,s1,s6
    8000112a:	0014e493          	ori	s1,s1,1
    8000112e:	e104                	sd	s1,0(a0)
    if(a == last)
    80001130:	fd391be3          	bne	s2,s3,80001106 <mappages+0x56>
    pa += PGSIZE;
  }
  return 0;
    80001134:	4501                	li	a0,0
    80001136:	a011                	j	8000113a <mappages+0x8a>
      return -1;
    80001138:	557d                	li	a0,-1
}
    8000113a:	60a6                	ld	ra,72(sp)
    8000113c:	6406                	ld	s0,64(sp)
    8000113e:	74e2                	ld	s1,56(sp)
    80001140:	7942                	ld	s2,48(sp)
    80001142:	79a2                	ld	s3,40(sp)
    80001144:	7a02                	ld	s4,32(sp)
    80001146:	6ae2                	ld	s5,24(sp)
    80001148:	6b42                	ld	s6,16(sp)
    8000114a:	6ba2                	ld	s7,8(sp)
    8000114c:	6161                	addi	sp,sp,80
    8000114e:	8082                	ret

0000000080001150 <kvmmap>:
{
    80001150:	1141                	addi	sp,sp,-16
    80001152:	e406                	sd	ra,8(sp)
    80001154:	e022                	sd	s0,0(sp)
    80001156:	0800                	addi	s0,sp,16
    80001158:	87b6                	mv	a5,a3
  if(mappages(kpgtbl, va, sz, pa, perm) != 0)
    8000115a:	86b2                	mv	a3,a2
    8000115c:	863e                	mv	a2,a5
    8000115e:	00000097          	auipc	ra,0x0
    80001162:	f52080e7          	jalr	-174(ra) # 800010b0 <mappages>
    80001166:	e509                	bnez	a0,80001170 <kvmmap+0x20>
}
    80001168:	60a2                	ld	ra,8(sp)
    8000116a:	6402                	ld	s0,0(sp)
    8000116c:	0141                	addi	sp,sp,16
    8000116e:	8082                	ret
    panic("kvmmap");
    80001170:	00007517          	auipc	a0,0x7
    80001174:	f8850513          	addi	a0,a0,-120 # 800080f8 <digits+0xb8>
    80001178:	fffff097          	auipc	ra,0xfffff
    8000117c:	3c6080e7          	jalr	966(ra) # 8000053e <panic>

0000000080001180 <kvmmake>:
{
    80001180:	1101                	addi	sp,sp,-32
    80001182:	ec06                	sd	ra,24(sp)
    80001184:	e822                	sd	s0,16(sp)
    80001186:	e426                	sd	s1,8(sp)
    80001188:	e04a                	sd	s2,0(sp)
    8000118a:	1000                	addi	s0,sp,32
  kpgtbl = (pagetable_t) kalloc();
    8000118c:	00000097          	auipc	ra,0x0
    80001190:	968080e7          	jalr	-1688(ra) # 80000af4 <kalloc>
    80001194:	84aa                	mv	s1,a0
  memset(kpgtbl, 0, PGSIZE);
    80001196:	6605                	lui	a2,0x1
    80001198:	4581                	li	a1,0
    8000119a:	00000097          	auipc	ra,0x0
    8000119e:	b46080e7          	jalr	-1210(ra) # 80000ce0 <memset>
  kvmmap(kpgtbl, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    800011a2:	4719                	li	a4,6
    800011a4:	6685                	lui	a3,0x1
    800011a6:	10000637          	lui	a2,0x10000
    800011aa:	100005b7          	lui	a1,0x10000
    800011ae:	8526                	mv	a0,s1
    800011b0:	00000097          	auipc	ra,0x0
    800011b4:	fa0080e7          	jalr	-96(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    800011b8:	4719                	li	a4,6
    800011ba:	6685                	lui	a3,0x1
    800011bc:	10001637          	lui	a2,0x10001
    800011c0:	100015b7          	lui	a1,0x10001
    800011c4:	8526                	mv	a0,s1
    800011c6:	00000097          	auipc	ra,0x0
    800011ca:	f8a080e7          	jalr	-118(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, PLIC, PLIC, 0x400000, PTE_R | PTE_W);
    800011ce:	4719                	li	a4,6
    800011d0:	004006b7          	lui	a3,0x400
    800011d4:	0c000637          	lui	a2,0xc000
    800011d8:	0c0005b7          	lui	a1,0xc000
    800011dc:	8526                	mv	a0,s1
    800011de:	00000097          	auipc	ra,0x0
    800011e2:	f72080e7          	jalr	-142(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
    800011e6:	00007917          	auipc	s2,0x7
    800011ea:	e1a90913          	addi	s2,s2,-486 # 80008000 <etext>
    800011ee:	4729                	li	a4,10
    800011f0:	80007697          	auipc	a3,0x80007
    800011f4:	e1068693          	addi	a3,a3,-496 # 8000 <_entry-0x7fff8000>
    800011f8:	4605                	li	a2,1
    800011fa:	067e                	slli	a2,a2,0x1f
    800011fc:	85b2                	mv	a1,a2
    800011fe:	8526                	mv	a0,s1
    80001200:	00000097          	auipc	ra,0x0
    80001204:	f50080e7          	jalr	-176(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
    80001208:	4719                	li	a4,6
    8000120a:	46c5                	li	a3,17
    8000120c:	06ee                	slli	a3,a3,0x1b
    8000120e:	412686b3          	sub	a3,a3,s2
    80001212:	864a                	mv	a2,s2
    80001214:	85ca                	mv	a1,s2
    80001216:	8526                	mv	a0,s1
    80001218:	00000097          	auipc	ra,0x0
    8000121c:	f38080e7          	jalr	-200(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    80001220:	4729                	li	a4,10
    80001222:	6685                	lui	a3,0x1
    80001224:	00006617          	auipc	a2,0x6
    80001228:	ddc60613          	addi	a2,a2,-548 # 80007000 <_trampoline>
    8000122c:	040005b7          	lui	a1,0x4000
    80001230:	15fd                	addi	a1,a1,-1
    80001232:	05b2                	slli	a1,a1,0xc
    80001234:	8526                	mv	a0,s1
    80001236:	00000097          	auipc	ra,0x0
    8000123a:	f1a080e7          	jalr	-230(ra) # 80001150 <kvmmap>
  proc_mapstacks(kpgtbl);
    8000123e:	8526                	mv	a0,s1
    80001240:	00000097          	auipc	ra,0x0
    80001244:	796080e7          	jalr	1942(ra) # 800019d6 <proc_mapstacks>
}
    80001248:	8526                	mv	a0,s1
    8000124a:	60e2                	ld	ra,24(sp)
    8000124c:	6442                	ld	s0,16(sp)
    8000124e:	64a2                	ld	s1,8(sp)
    80001250:	6902                	ld	s2,0(sp)
    80001252:	6105                	addi	sp,sp,32
    80001254:	8082                	ret

0000000080001256 <kvminit>:
{
    80001256:	1141                	addi	sp,sp,-16
    80001258:	e406                	sd	ra,8(sp)
    8000125a:	e022                	sd	s0,0(sp)
    8000125c:	0800                	addi	s0,sp,16
  kernel_pagetable = kvmmake();
    8000125e:	00000097          	auipc	ra,0x0
    80001262:	f22080e7          	jalr	-222(ra) # 80001180 <kvmmake>
    80001266:	00008797          	auipc	a5,0x8
    8000126a:	daa7bd23          	sd	a0,-582(a5) # 80009020 <kernel_pagetable>
}
    8000126e:	60a2                	ld	ra,8(sp)
    80001270:	6402                	ld	s0,0(sp)
    80001272:	0141                	addi	sp,sp,16
    80001274:	8082                	ret

0000000080001276 <uvmunmap>:
// Remove npages of mappings starting from va. va must be
// page-aligned. The mappings must exist.
// Optionally free the physical memory.
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
    80001276:	715d                	addi	sp,sp,-80
    80001278:	e486                	sd	ra,72(sp)
    8000127a:	e0a2                	sd	s0,64(sp)
    8000127c:	fc26                	sd	s1,56(sp)
    8000127e:	f84a                	sd	s2,48(sp)
    80001280:	f44e                	sd	s3,40(sp)
    80001282:	f052                	sd	s4,32(sp)
    80001284:	ec56                	sd	s5,24(sp)
    80001286:	e85a                	sd	s6,16(sp)
    80001288:	e45e                	sd	s7,8(sp)
    8000128a:	0880                	addi	s0,sp,80
  uint64 a;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    8000128c:	03459793          	slli	a5,a1,0x34
    80001290:	e795                	bnez	a5,800012bc <uvmunmap+0x46>
    80001292:	8a2a                	mv	s4,a0
    80001294:	892e                	mv	s2,a1
    80001296:	8ab6                	mv	s5,a3
    panic("uvmunmap: not aligned");

  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    80001298:	0632                	slli	a2,a2,0xc
    8000129a:	00b609b3          	add	s3,a2,a1
    if((pte = walk(pagetable, a, 0)) == 0)
      panic("uvmunmap: walk");
    if((*pte & PTE_V) == 0)
      panic("uvmunmap: not mapped");
    if(PTE_FLAGS(*pte) == PTE_V)
    8000129e:	4b85                	li	s7,1
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800012a0:	6b05                	lui	s6,0x1
    800012a2:	0735e863          	bltu	a1,s3,80001312 <uvmunmap+0x9c>
      uint64 pa = PTE2PA(*pte);
      kfree((void*)pa);
    }
    *pte = 0;
  }
}
    800012a6:	60a6                	ld	ra,72(sp)
    800012a8:	6406                	ld	s0,64(sp)
    800012aa:	74e2                	ld	s1,56(sp)
    800012ac:	7942                	ld	s2,48(sp)
    800012ae:	79a2                	ld	s3,40(sp)
    800012b0:	7a02                	ld	s4,32(sp)
    800012b2:	6ae2                	ld	s5,24(sp)
    800012b4:	6b42                	ld	s6,16(sp)
    800012b6:	6ba2                	ld	s7,8(sp)
    800012b8:	6161                	addi	sp,sp,80
    800012ba:	8082                	ret
    panic("uvmunmap: not aligned");
    800012bc:	00007517          	auipc	a0,0x7
    800012c0:	e4450513          	addi	a0,a0,-444 # 80008100 <digits+0xc0>
    800012c4:	fffff097          	auipc	ra,0xfffff
    800012c8:	27a080e7          	jalr	634(ra) # 8000053e <panic>
      panic("uvmunmap: walk");
    800012cc:	00007517          	auipc	a0,0x7
    800012d0:	e4c50513          	addi	a0,a0,-436 # 80008118 <digits+0xd8>
    800012d4:	fffff097          	auipc	ra,0xfffff
    800012d8:	26a080e7          	jalr	618(ra) # 8000053e <panic>
      panic("uvmunmap: not mapped");
    800012dc:	00007517          	auipc	a0,0x7
    800012e0:	e4c50513          	addi	a0,a0,-436 # 80008128 <digits+0xe8>
    800012e4:	fffff097          	auipc	ra,0xfffff
    800012e8:	25a080e7          	jalr	602(ra) # 8000053e <panic>
      panic("uvmunmap: not a leaf");
    800012ec:	00007517          	auipc	a0,0x7
    800012f0:	e5450513          	addi	a0,a0,-428 # 80008140 <digits+0x100>
    800012f4:	fffff097          	auipc	ra,0xfffff
    800012f8:	24a080e7          	jalr	586(ra) # 8000053e <panic>
      uint64 pa = PTE2PA(*pte);
    800012fc:	8129                	srli	a0,a0,0xa
      kfree((void*)pa);
    800012fe:	0532                	slli	a0,a0,0xc
    80001300:	fffff097          	auipc	ra,0xfffff
    80001304:	6f8080e7          	jalr	1784(ra) # 800009f8 <kfree>
    *pte = 0;
    80001308:	0004b023          	sd	zero,0(s1)
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    8000130c:	995a                	add	s2,s2,s6
    8000130e:	f9397ce3          	bgeu	s2,s3,800012a6 <uvmunmap+0x30>
    if((pte = walk(pagetable, a, 0)) == 0)
    80001312:	4601                	li	a2,0
    80001314:	85ca                	mv	a1,s2
    80001316:	8552                	mv	a0,s4
    80001318:	00000097          	auipc	ra,0x0
    8000131c:	cb0080e7          	jalr	-848(ra) # 80000fc8 <walk>
    80001320:	84aa                	mv	s1,a0
    80001322:	d54d                	beqz	a0,800012cc <uvmunmap+0x56>
    if((*pte & PTE_V) == 0)
    80001324:	6108                	ld	a0,0(a0)
    80001326:	00157793          	andi	a5,a0,1
    8000132a:	dbcd                	beqz	a5,800012dc <uvmunmap+0x66>
    if(PTE_FLAGS(*pte) == PTE_V)
    8000132c:	3ff57793          	andi	a5,a0,1023
    80001330:	fb778ee3          	beq	a5,s7,800012ec <uvmunmap+0x76>
    if(do_free){
    80001334:	fc0a8ae3          	beqz	s5,80001308 <uvmunmap+0x92>
    80001338:	b7d1                	j	800012fc <uvmunmap+0x86>

000000008000133a <uvmcreate>:

// create an empty user page table.
// returns 0 if out of memory.
pagetable_t
uvmcreate()
{
    8000133a:	1101                	addi	sp,sp,-32
    8000133c:	ec06                	sd	ra,24(sp)
    8000133e:	e822                	sd	s0,16(sp)
    80001340:	e426                	sd	s1,8(sp)
    80001342:	1000                	addi	s0,sp,32
  pagetable_t pagetable;
  pagetable = (pagetable_t) kalloc();
    80001344:	fffff097          	auipc	ra,0xfffff
    80001348:	7b0080e7          	jalr	1968(ra) # 80000af4 <kalloc>
    8000134c:	84aa                	mv	s1,a0
  if(pagetable == 0)
    8000134e:	c519                	beqz	a0,8000135c <uvmcreate+0x22>
    return 0;
  memset(pagetable, 0, PGSIZE);
    80001350:	6605                	lui	a2,0x1
    80001352:	4581                	li	a1,0
    80001354:	00000097          	auipc	ra,0x0
    80001358:	98c080e7          	jalr	-1652(ra) # 80000ce0 <memset>
  return pagetable;
}
    8000135c:	8526                	mv	a0,s1
    8000135e:	60e2                	ld	ra,24(sp)
    80001360:	6442                	ld	s0,16(sp)
    80001362:	64a2                	ld	s1,8(sp)
    80001364:	6105                	addi	sp,sp,32
    80001366:	8082                	ret

0000000080001368 <uvminit>:
// Load the user initcode into address 0 of pagetable,
// for the very first process.
// sz must be less than a page.
void
uvminit(pagetable_t pagetable, uchar *src, uint sz)
{
    80001368:	7179                	addi	sp,sp,-48
    8000136a:	f406                	sd	ra,40(sp)
    8000136c:	f022                	sd	s0,32(sp)
    8000136e:	ec26                	sd	s1,24(sp)
    80001370:	e84a                	sd	s2,16(sp)
    80001372:	e44e                	sd	s3,8(sp)
    80001374:	e052                	sd	s4,0(sp)
    80001376:	1800                	addi	s0,sp,48
  char *mem;

  if(sz >= PGSIZE)
    80001378:	6785                	lui	a5,0x1
    8000137a:	04f67863          	bgeu	a2,a5,800013ca <uvminit+0x62>
    8000137e:	8a2a                	mv	s4,a0
    80001380:	89ae                	mv	s3,a1
    80001382:	84b2                	mv	s1,a2
    panic("inituvm: more than a page");
  mem = kalloc();
    80001384:	fffff097          	auipc	ra,0xfffff
    80001388:	770080e7          	jalr	1904(ra) # 80000af4 <kalloc>
    8000138c:	892a                	mv	s2,a0
  memset(mem, 0, PGSIZE);
    8000138e:	6605                	lui	a2,0x1
    80001390:	4581                	li	a1,0
    80001392:	00000097          	auipc	ra,0x0
    80001396:	94e080e7          	jalr	-1714(ra) # 80000ce0 <memset>
  mappages(pagetable, 0, PGSIZE, (uint64)mem, PTE_W|PTE_R|PTE_X|PTE_U);
    8000139a:	4779                	li	a4,30
    8000139c:	86ca                	mv	a3,s2
    8000139e:	6605                	lui	a2,0x1
    800013a0:	4581                	li	a1,0
    800013a2:	8552                	mv	a0,s4
    800013a4:	00000097          	auipc	ra,0x0
    800013a8:	d0c080e7          	jalr	-756(ra) # 800010b0 <mappages>
  memmove(mem, src, sz);
    800013ac:	8626                	mv	a2,s1
    800013ae:	85ce                	mv	a1,s3
    800013b0:	854a                	mv	a0,s2
    800013b2:	00000097          	auipc	ra,0x0
    800013b6:	98e080e7          	jalr	-1650(ra) # 80000d40 <memmove>
}
    800013ba:	70a2                	ld	ra,40(sp)
    800013bc:	7402                	ld	s0,32(sp)
    800013be:	64e2                	ld	s1,24(sp)
    800013c0:	6942                	ld	s2,16(sp)
    800013c2:	69a2                	ld	s3,8(sp)
    800013c4:	6a02                	ld	s4,0(sp)
    800013c6:	6145                	addi	sp,sp,48
    800013c8:	8082                	ret
    panic("inituvm: more than a page");
    800013ca:	00007517          	auipc	a0,0x7
    800013ce:	d8e50513          	addi	a0,a0,-626 # 80008158 <digits+0x118>
    800013d2:	fffff097          	auipc	ra,0xfffff
    800013d6:	16c080e7          	jalr	364(ra) # 8000053e <panic>

00000000800013da <uvmdealloc>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    800013da:	1101                	addi	sp,sp,-32
    800013dc:	ec06                	sd	ra,24(sp)
    800013de:	e822                	sd	s0,16(sp)
    800013e0:	e426                	sd	s1,8(sp)
    800013e2:	1000                	addi	s0,sp,32
  if(newsz >= oldsz)
    return oldsz;
    800013e4:	84ae                	mv	s1,a1
  if(newsz >= oldsz)
    800013e6:	00b67d63          	bgeu	a2,a1,80001400 <uvmdealloc+0x26>
    800013ea:	84b2                	mv	s1,a2

  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    800013ec:	6785                	lui	a5,0x1
    800013ee:	17fd                	addi	a5,a5,-1
    800013f0:	00f60733          	add	a4,a2,a5
    800013f4:	767d                	lui	a2,0xfffff
    800013f6:	8f71                	and	a4,a4,a2
    800013f8:	97ae                	add	a5,a5,a1
    800013fa:	8ff1                	and	a5,a5,a2
    800013fc:	00f76863          	bltu	a4,a5,8000140c <uvmdealloc+0x32>
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
  }

  return newsz;
}
    80001400:	8526                	mv	a0,s1
    80001402:	60e2                	ld	ra,24(sp)
    80001404:	6442                	ld	s0,16(sp)
    80001406:	64a2                	ld	s1,8(sp)
    80001408:	6105                	addi	sp,sp,32
    8000140a:	8082                	ret
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    8000140c:	8f99                	sub	a5,a5,a4
    8000140e:	83b1                	srli	a5,a5,0xc
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
    80001410:	4685                	li	a3,1
    80001412:	0007861b          	sext.w	a2,a5
    80001416:	85ba                	mv	a1,a4
    80001418:	00000097          	auipc	ra,0x0
    8000141c:	e5e080e7          	jalr	-418(ra) # 80001276 <uvmunmap>
    80001420:	b7c5                	j	80001400 <uvmdealloc+0x26>

0000000080001422 <uvmalloc>:
  if(newsz < oldsz)
    80001422:	0ab66163          	bltu	a2,a1,800014c4 <uvmalloc+0xa2>
{
    80001426:	7139                	addi	sp,sp,-64
    80001428:	fc06                	sd	ra,56(sp)
    8000142a:	f822                	sd	s0,48(sp)
    8000142c:	f426                	sd	s1,40(sp)
    8000142e:	f04a                	sd	s2,32(sp)
    80001430:	ec4e                	sd	s3,24(sp)
    80001432:	e852                	sd	s4,16(sp)
    80001434:	e456                	sd	s5,8(sp)
    80001436:	0080                	addi	s0,sp,64
    80001438:	8aaa                	mv	s5,a0
    8000143a:	8a32                	mv	s4,a2
  oldsz = PGROUNDUP(oldsz);
    8000143c:	6985                	lui	s3,0x1
    8000143e:	19fd                	addi	s3,s3,-1
    80001440:	95ce                	add	a1,a1,s3
    80001442:	79fd                	lui	s3,0xfffff
    80001444:	0135f9b3          	and	s3,a1,s3
  for(a = oldsz; a < newsz; a += PGSIZE){
    80001448:	08c9f063          	bgeu	s3,a2,800014c8 <uvmalloc+0xa6>
    8000144c:	894e                	mv	s2,s3
    mem = kalloc();
    8000144e:	fffff097          	auipc	ra,0xfffff
    80001452:	6a6080e7          	jalr	1702(ra) # 80000af4 <kalloc>
    80001456:	84aa                	mv	s1,a0
    if(mem == 0){
    80001458:	c51d                	beqz	a0,80001486 <uvmalloc+0x64>
    memset(mem, 0, PGSIZE);
    8000145a:	6605                	lui	a2,0x1
    8000145c:	4581                	li	a1,0
    8000145e:	00000097          	auipc	ra,0x0
    80001462:	882080e7          	jalr	-1918(ra) # 80000ce0 <memset>
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_W|PTE_X|PTE_R|PTE_U) != 0){
    80001466:	4779                	li	a4,30
    80001468:	86a6                	mv	a3,s1
    8000146a:	6605                	lui	a2,0x1
    8000146c:	85ca                	mv	a1,s2
    8000146e:	8556                	mv	a0,s5
    80001470:	00000097          	auipc	ra,0x0
    80001474:	c40080e7          	jalr	-960(ra) # 800010b0 <mappages>
    80001478:	e905                	bnez	a0,800014a8 <uvmalloc+0x86>
  for(a = oldsz; a < newsz; a += PGSIZE){
    8000147a:	6785                	lui	a5,0x1
    8000147c:	993e                	add	s2,s2,a5
    8000147e:	fd4968e3          	bltu	s2,s4,8000144e <uvmalloc+0x2c>
  return newsz;
    80001482:	8552                	mv	a0,s4
    80001484:	a809                	j	80001496 <uvmalloc+0x74>
      uvmdealloc(pagetable, a, oldsz);
    80001486:	864e                	mv	a2,s3
    80001488:	85ca                	mv	a1,s2
    8000148a:	8556                	mv	a0,s5
    8000148c:	00000097          	auipc	ra,0x0
    80001490:	f4e080e7          	jalr	-178(ra) # 800013da <uvmdealloc>
      return 0;
    80001494:	4501                	li	a0,0
}
    80001496:	70e2                	ld	ra,56(sp)
    80001498:	7442                	ld	s0,48(sp)
    8000149a:	74a2                	ld	s1,40(sp)
    8000149c:	7902                	ld	s2,32(sp)
    8000149e:	69e2                	ld	s3,24(sp)
    800014a0:	6a42                	ld	s4,16(sp)
    800014a2:	6aa2                	ld	s5,8(sp)
    800014a4:	6121                	addi	sp,sp,64
    800014a6:	8082                	ret
      kfree(mem);
    800014a8:	8526                	mv	a0,s1
    800014aa:	fffff097          	auipc	ra,0xfffff
    800014ae:	54e080e7          	jalr	1358(ra) # 800009f8 <kfree>
      uvmdealloc(pagetable, a, oldsz);
    800014b2:	864e                	mv	a2,s3
    800014b4:	85ca                	mv	a1,s2
    800014b6:	8556                	mv	a0,s5
    800014b8:	00000097          	auipc	ra,0x0
    800014bc:	f22080e7          	jalr	-222(ra) # 800013da <uvmdealloc>
      return 0;
    800014c0:	4501                	li	a0,0
    800014c2:	bfd1                	j	80001496 <uvmalloc+0x74>
    return oldsz;
    800014c4:	852e                	mv	a0,a1
}
    800014c6:	8082                	ret
  return newsz;
    800014c8:	8532                	mv	a0,a2
    800014ca:	b7f1                	j	80001496 <uvmalloc+0x74>

00000000800014cc <freewalk>:

// Recursively free page-table pages.
// All leaf mappings must already have been removed.
void
freewalk(pagetable_t pagetable)
{
    800014cc:	7179                	addi	sp,sp,-48
    800014ce:	f406                	sd	ra,40(sp)
    800014d0:	f022                	sd	s0,32(sp)
    800014d2:	ec26                	sd	s1,24(sp)
    800014d4:	e84a                	sd	s2,16(sp)
    800014d6:	e44e                	sd	s3,8(sp)
    800014d8:	e052                	sd	s4,0(sp)
    800014da:	1800                	addi	s0,sp,48
    800014dc:	8a2a                	mv	s4,a0
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    800014de:	84aa                	mv	s1,a0
    800014e0:	6905                	lui	s2,0x1
    800014e2:	992a                	add	s2,s2,a0
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    800014e4:	4985                	li	s3,1
    800014e6:	a821                	j	800014fe <freewalk+0x32>
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
    800014e8:	8129                	srli	a0,a0,0xa
      freewalk((pagetable_t)child);
    800014ea:	0532                	slli	a0,a0,0xc
    800014ec:	00000097          	auipc	ra,0x0
    800014f0:	fe0080e7          	jalr	-32(ra) # 800014cc <freewalk>
      pagetable[i] = 0;
    800014f4:	0004b023          	sd	zero,0(s1)
  for(int i = 0; i < 512; i++){
    800014f8:	04a1                	addi	s1,s1,8
    800014fa:	03248163          	beq	s1,s2,8000151c <freewalk+0x50>
    pte_t pte = pagetable[i];
    800014fe:	6088                	ld	a0,0(s1)
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001500:	00f57793          	andi	a5,a0,15
    80001504:	ff3782e3          	beq	a5,s3,800014e8 <freewalk+0x1c>
    } else if(pte & PTE_V){
    80001508:	8905                	andi	a0,a0,1
    8000150a:	d57d                	beqz	a0,800014f8 <freewalk+0x2c>
      panic("freewalk: leaf");
    8000150c:	00007517          	auipc	a0,0x7
    80001510:	c6c50513          	addi	a0,a0,-916 # 80008178 <digits+0x138>
    80001514:	fffff097          	auipc	ra,0xfffff
    80001518:	02a080e7          	jalr	42(ra) # 8000053e <panic>
    }
  }
  kfree((void*)pagetable);
    8000151c:	8552                	mv	a0,s4
    8000151e:	fffff097          	auipc	ra,0xfffff
    80001522:	4da080e7          	jalr	1242(ra) # 800009f8 <kfree>
}
    80001526:	70a2                	ld	ra,40(sp)
    80001528:	7402                	ld	s0,32(sp)
    8000152a:	64e2                	ld	s1,24(sp)
    8000152c:	6942                	ld	s2,16(sp)
    8000152e:	69a2                	ld	s3,8(sp)
    80001530:	6a02                	ld	s4,0(sp)
    80001532:	6145                	addi	sp,sp,48
    80001534:	8082                	ret

0000000080001536 <uvmfree>:

// Free user memory pages,
// then free page-table pages.
void
uvmfree(pagetable_t pagetable, uint64 sz)
{
    80001536:	1101                	addi	sp,sp,-32
    80001538:	ec06                	sd	ra,24(sp)
    8000153a:	e822                	sd	s0,16(sp)
    8000153c:	e426                	sd	s1,8(sp)
    8000153e:	1000                	addi	s0,sp,32
    80001540:	84aa                	mv	s1,a0
  if(sz > 0)
    80001542:	e999                	bnez	a1,80001558 <uvmfree+0x22>
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  freewalk(pagetable);
    80001544:	8526                	mv	a0,s1
    80001546:	00000097          	auipc	ra,0x0
    8000154a:	f86080e7          	jalr	-122(ra) # 800014cc <freewalk>
}
    8000154e:	60e2                	ld	ra,24(sp)
    80001550:	6442                	ld	s0,16(sp)
    80001552:	64a2                	ld	s1,8(sp)
    80001554:	6105                	addi	sp,sp,32
    80001556:	8082                	ret
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
    80001558:	6605                	lui	a2,0x1
    8000155a:	167d                	addi	a2,a2,-1
    8000155c:	962e                	add	a2,a2,a1
    8000155e:	4685                	li	a3,1
    80001560:	8231                	srli	a2,a2,0xc
    80001562:	4581                	li	a1,0
    80001564:	00000097          	auipc	ra,0x0
    80001568:	d12080e7          	jalr	-750(ra) # 80001276 <uvmunmap>
    8000156c:	bfe1                	j	80001544 <uvmfree+0xe>

000000008000156e <uvmcopy>:
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;

  for(i = 0; i < sz; i += PGSIZE){
    8000156e:	c679                	beqz	a2,8000163c <uvmcopy+0xce>
{
    80001570:	715d                	addi	sp,sp,-80
    80001572:	e486                	sd	ra,72(sp)
    80001574:	e0a2                	sd	s0,64(sp)
    80001576:	fc26                	sd	s1,56(sp)
    80001578:	f84a                	sd	s2,48(sp)
    8000157a:	f44e                	sd	s3,40(sp)
    8000157c:	f052                	sd	s4,32(sp)
    8000157e:	ec56                	sd	s5,24(sp)
    80001580:	e85a                	sd	s6,16(sp)
    80001582:	e45e                	sd	s7,8(sp)
    80001584:	0880                	addi	s0,sp,80
    80001586:	8b2a                	mv	s6,a0
    80001588:	8aae                	mv	s5,a1
    8000158a:	8a32                	mv	s4,a2
  for(i = 0; i < sz; i += PGSIZE){
    8000158c:	4981                	li	s3,0
    if((pte = walk(old, i, 0)) == 0)
    8000158e:	4601                	li	a2,0
    80001590:	85ce                	mv	a1,s3
    80001592:	855a                	mv	a0,s6
    80001594:	00000097          	auipc	ra,0x0
    80001598:	a34080e7          	jalr	-1484(ra) # 80000fc8 <walk>
    8000159c:	c531                	beqz	a0,800015e8 <uvmcopy+0x7a>
      panic("uvmcopy: pte should exist");
    if((*pte & PTE_V) == 0)
    8000159e:	6118                	ld	a4,0(a0)
    800015a0:	00177793          	andi	a5,a4,1
    800015a4:	cbb1                	beqz	a5,800015f8 <uvmcopy+0x8a>
      panic("uvmcopy: page not present");
    pa = PTE2PA(*pte);
    800015a6:	00a75593          	srli	a1,a4,0xa
    800015aa:	00c59b93          	slli	s7,a1,0xc
    flags = PTE_FLAGS(*pte);
    800015ae:	3ff77493          	andi	s1,a4,1023
    if((mem = kalloc()) == 0)
    800015b2:	fffff097          	auipc	ra,0xfffff
    800015b6:	542080e7          	jalr	1346(ra) # 80000af4 <kalloc>
    800015ba:	892a                	mv	s2,a0
    800015bc:	c939                	beqz	a0,80001612 <uvmcopy+0xa4>
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    800015be:	6605                	lui	a2,0x1
    800015c0:	85de                	mv	a1,s7
    800015c2:	fffff097          	auipc	ra,0xfffff
    800015c6:	77e080e7          	jalr	1918(ra) # 80000d40 <memmove>
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
    800015ca:	8726                	mv	a4,s1
    800015cc:	86ca                	mv	a3,s2
    800015ce:	6605                	lui	a2,0x1
    800015d0:	85ce                	mv	a1,s3
    800015d2:	8556                	mv	a0,s5
    800015d4:	00000097          	auipc	ra,0x0
    800015d8:	adc080e7          	jalr	-1316(ra) # 800010b0 <mappages>
    800015dc:	e515                	bnez	a0,80001608 <uvmcopy+0x9a>
  for(i = 0; i < sz; i += PGSIZE){
    800015de:	6785                	lui	a5,0x1
    800015e0:	99be                	add	s3,s3,a5
    800015e2:	fb49e6e3          	bltu	s3,s4,8000158e <uvmcopy+0x20>
    800015e6:	a081                	j	80001626 <uvmcopy+0xb8>
      panic("uvmcopy: pte should exist");
    800015e8:	00007517          	auipc	a0,0x7
    800015ec:	ba050513          	addi	a0,a0,-1120 # 80008188 <digits+0x148>
    800015f0:	fffff097          	auipc	ra,0xfffff
    800015f4:	f4e080e7          	jalr	-178(ra) # 8000053e <panic>
      panic("uvmcopy: page not present");
    800015f8:	00007517          	auipc	a0,0x7
    800015fc:	bb050513          	addi	a0,a0,-1104 # 800081a8 <digits+0x168>
    80001600:	fffff097          	auipc	ra,0xfffff
    80001604:	f3e080e7          	jalr	-194(ra) # 8000053e <panic>
      kfree(mem);
    80001608:	854a                	mv	a0,s2
    8000160a:	fffff097          	auipc	ra,0xfffff
    8000160e:	3ee080e7          	jalr	1006(ra) # 800009f8 <kfree>
    }
  }
  return 0;

 err:
  uvmunmap(new, 0, i / PGSIZE, 1);
    80001612:	4685                	li	a3,1
    80001614:	00c9d613          	srli	a2,s3,0xc
    80001618:	4581                	li	a1,0
    8000161a:	8556                	mv	a0,s5
    8000161c:	00000097          	auipc	ra,0x0
    80001620:	c5a080e7          	jalr	-934(ra) # 80001276 <uvmunmap>
  return -1;
    80001624:	557d                	li	a0,-1
}
    80001626:	60a6                	ld	ra,72(sp)
    80001628:	6406                	ld	s0,64(sp)
    8000162a:	74e2                	ld	s1,56(sp)
    8000162c:	7942                	ld	s2,48(sp)
    8000162e:	79a2                	ld	s3,40(sp)
    80001630:	7a02                	ld	s4,32(sp)
    80001632:	6ae2                	ld	s5,24(sp)
    80001634:	6b42                	ld	s6,16(sp)
    80001636:	6ba2                	ld	s7,8(sp)
    80001638:	6161                	addi	sp,sp,80
    8000163a:	8082                	ret
  return 0;
    8000163c:	4501                	li	a0,0
}
    8000163e:	8082                	ret

0000000080001640 <uvmclear>:

// mark a PTE invalid for user access.
// used by exec for the user stack guard page.
void
uvmclear(pagetable_t pagetable, uint64 va)
{
    80001640:	1141                	addi	sp,sp,-16
    80001642:	e406                	sd	ra,8(sp)
    80001644:	e022                	sd	s0,0(sp)
    80001646:	0800                	addi	s0,sp,16
  pte_t *pte;
  
  pte = walk(pagetable, va, 0);
    80001648:	4601                	li	a2,0
    8000164a:	00000097          	auipc	ra,0x0
    8000164e:	97e080e7          	jalr	-1666(ra) # 80000fc8 <walk>
  if(pte == 0)
    80001652:	c901                	beqz	a0,80001662 <uvmclear+0x22>
    panic("uvmclear");
  *pte &= ~PTE_U;
    80001654:	611c                	ld	a5,0(a0)
    80001656:	9bbd                	andi	a5,a5,-17
    80001658:	e11c                	sd	a5,0(a0)
}
    8000165a:	60a2                	ld	ra,8(sp)
    8000165c:	6402                	ld	s0,0(sp)
    8000165e:	0141                	addi	sp,sp,16
    80001660:	8082                	ret
    panic("uvmclear");
    80001662:	00007517          	auipc	a0,0x7
    80001666:	b6650513          	addi	a0,a0,-1178 # 800081c8 <digits+0x188>
    8000166a:	fffff097          	auipc	ra,0xfffff
    8000166e:	ed4080e7          	jalr	-300(ra) # 8000053e <panic>

0000000080001672 <copyout>:
int
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  uint64 n, va0, pa0;

  while(len > 0){
    80001672:	c6bd                	beqz	a3,800016e0 <copyout+0x6e>
{
    80001674:	715d                	addi	sp,sp,-80
    80001676:	e486                	sd	ra,72(sp)
    80001678:	e0a2                	sd	s0,64(sp)
    8000167a:	fc26                	sd	s1,56(sp)
    8000167c:	f84a                	sd	s2,48(sp)
    8000167e:	f44e                	sd	s3,40(sp)
    80001680:	f052                	sd	s4,32(sp)
    80001682:	ec56                	sd	s5,24(sp)
    80001684:	e85a                	sd	s6,16(sp)
    80001686:	e45e                	sd	s7,8(sp)
    80001688:	e062                	sd	s8,0(sp)
    8000168a:	0880                	addi	s0,sp,80
    8000168c:	8b2a                	mv	s6,a0
    8000168e:	8c2e                	mv	s8,a1
    80001690:	8a32                	mv	s4,a2
    80001692:	89b6                	mv	s3,a3
    va0 = PGROUNDDOWN(dstva);
    80001694:	7bfd                	lui	s7,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (dstva - va0);
    80001696:	6a85                	lui	s5,0x1
    80001698:	a015                	j	800016bc <copyout+0x4a>
    if(n > len)
      n = len;
    memmove((void *)(pa0 + (dstva - va0)), src, n);
    8000169a:	9562                	add	a0,a0,s8
    8000169c:	0004861b          	sext.w	a2,s1
    800016a0:	85d2                	mv	a1,s4
    800016a2:	41250533          	sub	a0,a0,s2
    800016a6:	fffff097          	auipc	ra,0xfffff
    800016aa:	69a080e7          	jalr	1690(ra) # 80000d40 <memmove>

    len -= n;
    800016ae:	409989b3          	sub	s3,s3,s1
    src += n;
    800016b2:	9a26                	add	s4,s4,s1
    dstva = va0 + PGSIZE;
    800016b4:	01590c33          	add	s8,s2,s5
  while(len > 0){
    800016b8:	02098263          	beqz	s3,800016dc <copyout+0x6a>
    va0 = PGROUNDDOWN(dstva);
    800016bc:	017c7933          	and	s2,s8,s7
    pa0 = walkaddr(pagetable, va0);
    800016c0:	85ca                	mv	a1,s2
    800016c2:	855a                	mv	a0,s6
    800016c4:	00000097          	auipc	ra,0x0
    800016c8:	9aa080e7          	jalr	-1622(ra) # 8000106e <walkaddr>
    if(pa0 == 0)
    800016cc:	cd01                	beqz	a0,800016e4 <copyout+0x72>
    n = PGSIZE - (dstva - va0);
    800016ce:	418904b3          	sub	s1,s2,s8
    800016d2:	94d6                	add	s1,s1,s5
    if(n > len)
    800016d4:	fc99f3e3          	bgeu	s3,s1,8000169a <copyout+0x28>
    800016d8:	84ce                	mv	s1,s3
    800016da:	b7c1                	j	8000169a <copyout+0x28>
  }
  return 0;
    800016dc:	4501                	li	a0,0
    800016de:	a021                	j	800016e6 <copyout+0x74>
    800016e0:	4501                	li	a0,0
}
    800016e2:	8082                	ret
      return -1;
    800016e4:	557d                	li	a0,-1
}
    800016e6:	60a6                	ld	ra,72(sp)
    800016e8:	6406                	ld	s0,64(sp)
    800016ea:	74e2                	ld	s1,56(sp)
    800016ec:	7942                	ld	s2,48(sp)
    800016ee:	79a2                	ld	s3,40(sp)
    800016f0:	7a02                	ld	s4,32(sp)
    800016f2:	6ae2                	ld	s5,24(sp)
    800016f4:	6b42                	ld	s6,16(sp)
    800016f6:	6ba2                	ld	s7,8(sp)
    800016f8:	6c02                	ld	s8,0(sp)
    800016fa:	6161                	addi	sp,sp,80
    800016fc:	8082                	ret

00000000800016fe <copyin>:
int
copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len)
{
  uint64 n, va0, pa0;

  while(len > 0){
    800016fe:	c6bd                	beqz	a3,8000176c <copyin+0x6e>
{
    80001700:	715d                	addi	sp,sp,-80
    80001702:	e486                	sd	ra,72(sp)
    80001704:	e0a2                	sd	s0,64(sp)
    80001706:	fc26                	sd	s1,56(sp)
    80001708:	f84a                	sd	s2,48(sp)
    8000170a:	f44e                	sd	s3,40(sp)
    8000170c:	f052                	sd	s4,32(sp)
    8000170e:	ec56                	sd	s5,24(sp)
    80001710:	e85a                	sd	s6,16(sp)
    80001712:	e45e                	sd	s7,8(sp)
    80001714:	e062                	sd	s8,0(sp)
    80001716:	0880                	addi	s0,sp,80
    80001718:	8b2a                	mv	s6,a0
    8000171a:	8a2e                	mv	s4,a1
    8000171c:	8c32                	mv	s8,a2
    8000171e:	89b6                	mv	s3,a3
    va0 = PGROUNDDOWN(srcva);
    80001720:	7bfd                	lui	s7,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    80001722:	6a85                	lui	s5,0x1
    80001724:	a015                	j	80001748 <copyin+0x4a>
    if(n > len)
      n = len;
    memmove(dst, (void *)(pa0 + (srcva - va0)), n);
    80001726:	9562                	add	a0,a0,s8
    80001728:	0004861b          	sext.w	a2,s1
    8000172c:	412505b3          	sub	a1,a0,s2
    80001730:	8552                	mv	a0,s4
    80001732:	fffff097          	auipc	ra,0xfffff
    80001736:	60e080e7          	jalr	1550(ra) # 80000d40 <memmove>

    len -= n;
    8000173a:	409989b3          	sub	s3,s3,s1
    dst += n;
    8000173e:	9a26                	add	s4,s4,s1
    srcva = va0 + PGSIZE;
    80001740:	01590c33          	add	s8,s2,s5
  while(len > 0){
    80001744:	02098263          	beqz	s3,80001768 <copyin+0x6a>
    va0 = PGROUNDDOWN(srcva);
    80001748:	017c7933          	and	s2,s8,s7
    pa0 = walkaddr(pagetable, va0);
    8000174c:	85ca                	mv	a1,s2
    8000174e:	855a                	mv	a0,s6
    80001750:	00000097          	auipc	ra,0x0
    80001754:	91e080e7          	jalr	-1762(ra) # 8000106e <walkaddr>
    if(pa0 == 0)
    80001758:	cd01                	beqz	a0,80001770 <copyin+0x72>
    n = PGSIZE - (srcva - va0);
    8000175a:	418904b3          	sub	s1,s2,s8
    8000175e:	94d6                	add	s1,s1,s5
    if(n > len)
    80001760:	fc99f3e3          	bgeu	s3,s1,80001726 <copyin+0x28>
    80001764:	84ce                	mv	s1,s3
    80001766:	b7c1                	j	80001726 <copyin+0x28>
  }
  return 0;
    80001768:	4501                	li	a0,0
    8000176a:	a021                	j	80001772 <copyin+0x74>
    8000176c:	4501                	li	a0,0
}
    8000176e:	8082                	ret
      return -1;
    80001770:	557d                	li	a0,-1
}
    80001772:	60a6                	ld	ra,72(sp)
    80001774:	6406                	ld	s0,64(sp)
    80001776:	74e2                	ld	s1,56(sp)
    80001778:	7942                	ld	s2,48(sp)
    8000177a:	79a2                	ld	s3,40(sp)
    8000177c:	7a02                	ld	s4,32(sp)
    8000177e:	6ae2                	ld	s5,24(sp)
    80001780:	6b42                	ld	s6,16(sp)
    80001782:	6ba2                	ld	s7,8(sp)
    80001784:	6c02                	ld	s8,0(sp)
    80001786:	6161                	addi	sp,sp,80
    80001788:	8082                	ret

000000008000178a <copyinstr>:
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  uint64 n, va0, pa0;
  int got_null = 0;

  while(got_null == 0 && max > 0){
    8000178a:	c6c5                	beqz	a3,80001832 <copyinstr+0xa8>
{
    8000178c:	715d                	addi	sp,sp,-80
    8000178e:	e486                	sd	ra,72(sp)
    80001790:	e0a2                	sd	s0,64(sp)
    80001792:	fc26                	sd	s1,56(sp)
    80001794:	f84a                	sd	s2,48(sp)
    80001796:	f44e                	sd	s3,40(sp)
    80001798:	f052                	sd	s4,32(sp)
    8000179a:	ec56                	sd	s5,24(sp)
    8000179c:	e85a                	sd	s6,16(sp)
    8000179e:	e45e                	sd	s7,8(sp)
    800017a0:	0880                	addi	s0,sp,80
    800017a2:	8a2a                	mv	s4,a0
    800017a4:	8b2e                	mv	s6,a1
    800017a6:	8bb2                	mv	s7,a2
    800017a8:	84b6                	mv	s1,a3
    va0 = PGROUNDDOWN(srcva);
    800017aa:	7afd                	lui	s5,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    800017ac:	6985                	lui	s3,0x1
    800017ae:	a035                	j	800017da <copyinstr+0x50>
      n = max;

    char *p = (char *) (pa0 + (srcva - va0));
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
    800017b0:	00078023          	sb	zero,0(a5) # 1000 <_entry-0x7ffff000>
    800017b4:	4785                	li	a5,1
      dst++;
    }

    srcva = va0 + PGSIZE;
  }
  if(got_null){
    800017b6:	0017b793          	seqz	a5,a5
    800017ba:	40f00533          	neg	a0,a5
    return 0;
  } else {
    return -1;
  }
}
    800017be:	60a6                	ld	ra,72(sp)
    800017c0:	6406                	ld	s0,64(sp)
    800017c2:	74e2                	ld	s1,56(sp)
    800017c4:	7942                	ld	s2,48(sp)
    800017c6:	79a2                	ld	s3,40(sp)
    800017c8:	7a02                	ld	s4,32(sp)
    800017ca:	6ae2                	ld	s5,24(sp)
    800017cc:	6b42                	ld	s6,16(sp)
    800017ce:	6ba2                	ld	s7,8(sp)
    800017d0:	6161                	addi	sp,sp,80
    800017d2:	8082                	ret
    srcva = va0 + PGSIZE;
    800017d4:	01390bb3          	add	s7,s2,s3
  while(got_null == 0 && max > 0){
    800017d8:	c8a9                	beqz	s1,8000182a <copyinstr+0xa0>
    va0 = PGROUNDDOWN(srcva);
    800017da:	015bf933          	and	s2,s7,s5
    pa0 = walkaddr(pagetable, va0);
    800017de:	85ca                	mv	a1,s2
    800017e0:	8552                	mv	a0,s4
    800017e2:	00000097          	auipc	ra,0x0
    800017e6:	88c080e7          	jalr	-1908(ra) # 8000106e <walkaddr>
    if(pa0 == 0)
    800017ea:	c131                	beqz	a0,8000182e <copyinstr+0xa4>
    n = PGSIZE - (srcva - va0);
    800017ec:	41790833          	sub	a6,s2,s7
    800017f0:	984e                	add	a6,a6,s3
    if(n > max)
    800017f2:	0104f363          	bgeu	s1,a6,800017f8 <copyinstr+0x6e>
    800017f6:	8826                	mv	a6,s1
    char *p = (char *) (pa0 + (srcva - va0));
    800017f8:	955e                	add	a0,a0,s7
    800017fa:	41250533          	sub	a0,a0,s2
    while(n > 0){
    800017fe:	fc080be3          	beqz	a6,800017d4 <copyinstr+0x4a>
    80001802:	985a                	add	a6,a6,s6
    80001804:	87da                	mv	a5,s6
      if(*p == '\0'){
    80001806:	41650633          	sub	a2,a0,s6
    8000180a:	14fd                	addi	s1,s1,-1
    8000180c:	9b26                	add	s6,s6,s1
    8000180e:	00f60733          	add	a4,a2,a5
    80001812:	00074703          	lbu	a4,0(a4)
    80001816:	df49                	beqz	a4,800017b0 <copyinstr+0x26>
        *dst = *p;
    80001818:	00e78023          	sb	a4,0(a5)
      --max;
    8000181c:	40fb04b3          	sub	s1,s6,a5
      dst++;
    80001820:	0785                	addi	a5,a5,1
    while(n > 0){
    80001822:	ff0796e3          	bne	a5,a6,8000180e <copyinstr+0x84>
      dst++;
    80001826:	8b42                	mv	s6,a6
    80001828:	b775                	j	800017d4 <copyinstr+0x4a>
    8000182a:	4781                	li	a5,0
    8000182c:	b769                	j	800017b6 <copyinstr+0x2c>
      return -1;
    8000182e:	557d                	li	a0,-1
    80001830:	b779                	j	800017be <copyinstr+0x34>
  int got_null = 0;
    80001832:	4781                	li	a5,0
  if(got_null){
    80001834:	0017b793          	seqz	a5,a5
    80001838:	40f00533          	neg	a0,a5
}
    8000183c:	8082                	ret

000000008000183e <initialize_list>:

struct _list *unused_list;   // contains all UNUSED process entries.
struct _list *sleeping_list; // contains all SLEEPING processes.
struct _list *zombie_list;   // contains all ZOMBIE processes.

void initialize_list(struct _list *lst){
    8000183e:	1141                	addi	sp,sp,-16
    80001840:	e422                	sd	s0,8(sp)
    80001842:	0800                	addi	s0,sp,16
  lst->head = -1;
    80001844:	57fd                	li	a5,-1
    80001846:	c11c                	sw	a5,0(a0)
  lst->tail = -1;
    80001848:	c15c                	sw	a5,4(a0)
}
    8000184a:	6422                	ld	s0,8(sp)
    8000184c:	0141                	addi	sp,sp,16
    8000184e:	8082                	ret

0000000080001850 <initialize_lists>:

void initialize_lists(void){
    80001850:	1141                	addi	sp,sp,-16
    80001852:	e406                	sd	ra,8(sp)
    80001854:	e022                	sd	s0,0(sp)
    80001856:	0800                	addi	s0,sp,16
  initialize_list(unused_list);
    80001858:	00007717          	auipc	a4,0x7
    8000185c:	7e073703          	ld	a4,2016(a4) # 80009038 <unused_list>
  lst->head = -1;
    80001860:	57fd                	li	a5,-1
    80001862:	c31c                	sw	a5,0(a4)
  lst->tail = -1;
    80001864:	c35c                	sw	a5,4(a4)
  initialize_list(sleeping_list);
    80001866:	00007717          	auipc	a4,0x7
    8000186a:	7ca73703          	ld	a4,1994(a4) # 80009030 <sleeping_list>
  lst->head = -1;
    8000186e:	c31c                	sw	a5,0(a4)
  lst->tail = -1;
    80001870:	c35c                	sw	a5,4(a4)
  initialize_list(zombie_list);
    80001872:	00007717          	auipc	a4,0x7
    80001876:	7b673703          	ld	a4,1974(a4) # 80009028 <zombie_list>
  lst->head = -1;
    8000187a:	c31c                	sw	a5,0(a4)
  lst->tail = -1;
    8000187c:	c35c                	sw	a5,4(a4)

  struct cpu *c;
  for(c = cpus; c < &cpus[NCPU] && c != NULL ; c++)
    8000187e:	00010797          	auipc	a5,0x10
    80001882:	a3278793          	addi	a5,a5,-1486 # 800112b0 <cpus>
  lst->head = -1;
    80001886:	56fd                	li	a3,-1
  for(c = cpus; c < &cpus[NCPU] && c != NULL ; c++)
    80001888:	00010617          	auipc	a2,0x10
    8000188c:	ea860613          	addi	a2,a2,-344 # 80011730 <pid_lock>
   initialize_list(c->runnable_list);
    80001890:	63d8                	ld	a4,128(a5)
  lst->head = -1;
    80001892:	c314                	sw	a3,0(a4)
  lst->tail = -1;
    80001894:	c354                	sw	a3,4(a4)
  for(c = cpus; c < &cpus[NCPU] && c != NULL ; c++)
    80001896:	09078793          	addi	a5,a5,144
    8000189a:	fec79be3          	bne	a5,a2,80001890 <initialize_lists+0x40>
  printf("here");
    8000189e:	00007517          	auipc	a0,0x7
    800018a2:	93a50513          	addi	a0,a0,-1734 # 800081d8 <digits+0x198>
    800018a6:	fffff097          	auipc	ra,0xfffff
    800018aa:	ce2080e7          	jalr	-798(ra) # 80000588 <printf>
}
    800018ae:	60a2                	ld	ra,8(sp)
    800018b0:	6402                	ld	s0,0(sp)
    800018b2:	0141                	addi	sp,sp,16
    800018b4:	8082                	ret

00000000800018b6 <initialize_proc>:

void
initialize_proc(struct proc *p){
    800018b6:	1141                	addi	sp,sp,-16
    800018b8:	e422                	sd	s0,8(sp)
    800018ba:	0800                	addi	s0,sp,16
  proc->next_index = -1;
    800018bc:	00010797          	auipc	a5,0x10
    800018c0:	ea478793          	addi	a5,a5,-348 # 80011760 <proc>
    800018c4:	577d                	li	a4,-1
    800018c6:	16e7aa23          	sw	a4,372(a5)
  proc->prev_index = -1;
    800018ca:	16e7a823          	sw	a4,368(a5)
}
    800018ce:	6422                	ld	s0,8(sp)
    800018d0:	0141                	addi	sp,sp,16
    800018d2:	8082                	ret

00000000800018d4 <isEmpty>:

int
isEmpty(struct _list *lst){
    800018d4:	1141                	addi	sp,sp,-16
    800018d6:	e422                	sd	s0,8(sp)
    800018d8:	0800                	addi	s0,sp,16
  return lst->tail == -1;
    800018da:	4148                	lw	a0,4(a0)
    800018dc:	0505                	addi	a0,a0,1
}
    800018de:	00153513          	seqz	a0,a0
    800018e2:	6422                	ld	s0,8(sp)
    800018e4:	0141                	addi	sp,sp,16
    800018e6:	8082                	ret

00000000800018e8 <insert_proc_to_list>:

void 
insert_proc_to_list(struct _list *lst, struct proc *p){
    800018e8:	1141                	addi	sp,sp,-16
    800018ea:	e422                	sd	s0,8(sp)
    800018ec:	0800                	addi	s0,sp,16
  return lst->tail == -1;
    800018ee:	415c                	lw	a5,4(a0)
  if(isEmpty(lst)){
    800018f0:	577d                	li	a4,-1
    800018f2:	00e79b63          	bne	a5,a4,80001908 <insert_proc_to_list+0x20>
    lst->head = p->index;
    800018f6:	16c5a783          	lw	a5,364(a1) # 400016c <_entry-0x7bfffe94>
    800018fa:	c11c                	sw	a5,0(a0)
    lst->tail = p-> index;
    800018fc:	16c5a783          	lw	a5,364(a1)
    80001900:	c15c                	sw	a5,4(a0)
    struct proc *p_tail = &proc[lst->tail];
    p_tail->next_index = p->index; // update next proc of the curr tail
    p->prev_index = p_tail->index; // update the prev proc of the new proc
    lst->tail = p->index;          // update tail
  }
}
    80001902:	6422                	ld	s0,8(sp)
    80001904:	0141                	addi	sp,sp,16
    80001906:	8082                	ret
    p_tail->next_index = p->index; // update next proc of the curr tail
    80001908:	16c5a683          	lw	a3,364(a1)
    8000190c:	17800713          	li	a4,376
    80001910:	02e787b3          	mul	a5,a5,a4
    80001914:	00010717          	auipc	a4,0x10
    80001918:	e4c70713          	addi	a4,a4,-436 # 80011760 <proc>
    8000191c:	97ba                	add	a5,a5,a4
    8000191e:	16d7aa23          	sw	a3,372(a5)
    p->prev_index = p_tail->index; // update the prev proc of the new proc
    80001922:	16c7a783          	lw	a5,364(a5)
    80001926:	16f5a823          	sw	a5,368(a1)
    lst->tail = p->index;          // update tail
    8000192a:	c154                	sw	a3,4(a0)
}
    8000192c:	bfd9                	j	80001902 <insert_proc_to_list+0x1a>

000000008000192e <remove_proc_to_list>:

void 
remove_proc_to_list(struct _list *lst, struct proc *p){
    8000192e:	1141                	addi	sp,sp,-16
    80001930:	e422                	sd	s0,8(sp)
    80001932:	0800                	addi	s0,sp,16
  if(lst->head == p->index && lst->tail == p->index) // p is the only proc in the list
    80001934:	16c5a783          	lw	a5,364(a1)
    80001938:	4118                	lw	a4,0(a0)
    8000193a:	04f70763          	beq	a4,a5,80001988 <remove_proc_to_list+0x5a>
    initialize_list(lst);
  else if(lst->head == p-> index) {  // p is the head of the list
    lst->head = p->next_index;
    proc[lst->head].prev_index = -1;
  }
  else if(lst->tail == p-> index) { // p is the tail of the list
    8000193e:	4158                	lw	a4,4(a0)
    80001940:	06f70b63          	beq	a4,a5,800019b6 <remove_proc_to_list+0x88>
    lst->tail = p->prev_index;
    proc[lst->tail].next_index = -1;
  }
  else {
    proc[p->prev_index].next_index = p->next_index;
    80001944:	1705a783          	lw	a5,368(a1)
    80001948:	1745a683          	lw	a3,372(a1)
    8000194c:	00010717          	auipc	a4,0x10
    80001950:	e1470713          	addi	a4,a4,-492 # 80011760 <proc>
    80001954:	17800613          	li	a2,376
    80001958:	02c787b3          	mul	a5,a5,a2
    8000195c:	97ba                	add	a5,a5,a4
    8000195e:	16d7aa23          	sw	a3,372(a5)
    proc[p->next_index].prev_index = p->prev_index;
    80001962:	1705a783          	lw	a5,368(a1)
    80001966:	02c686b3          	mul	a3,a3,a2
    8000196a:	9736                	add	a4,a4,a3
    8000196c:	16f72823          	sw	a5,368(a4)
  proc->next_index = -1;
    80001970:	00010797          	auipc	a5,0x10
    80001974:	df078793          	addi	a5,a5,-528 # 80011760 <proc>
    80001978:	577d                	li	a4,-1
    8000197a:	16e7aa23          	sw	a4,372(a5)
  proc->prev_index = -1;
    8000197e:	16e7a823          	sw	a4,368(a5)
  }
  initialize_proc(p);
}
    80001982:	6422                	ld	s0,8(sp)
    80001984:	0141                	addi	sp,sp,16
    80001986:	8082                	ret
  if(lst->head == p->index && lst->tail == p->index) // p is the only proc in the list
    80001988:	4158                	lw	a4,4(a0)
    8000198a:	02f70263          	beq	a4,a5,800019ae <remove_proc_to_list+0x80>
    lst->head = p->next_index;
    8000198e:	1745a783          	lw	a5,372(a1)
    80001992:	c11c                	sw	a5,0(a0)
    proc[lst->head].prev_index = -1;
    80001994:	17800713          	li	a4,376
    80001998:	02e787b3          	mul	a5,a5,a4
    8000199c:	00010717          	auipc	a4,0x10
    800019a0:	dc470713          	addi	a4,a4,-572 # 80011760 <proc>
    800019a4:	97ba                	add	a5,a5,a4
    800019a6:	577d                	li	a4,-1
    800019a8:	16e7a823          	sw	a4,368(a5)
    800019ac:	b7d1                	j	80001970 <remove_proc_to_list+0x42>
  lst->head = -1;
    800019ae:	57fd                	li	a5,-1
    800019b0:	c11c                	sw	a5,0(a0)
  lst->tail = -1;
    800019b2:	c15c                	sw	a5,4(a0)
}
    800019b4:	bf75                	j	80001970 <remove_proc_to_list+0x42>
    lst->tail = p->prev_index;
    800019b6:	1705a783          	lw	a5,368(a1)
    800019ba:	c15c                	sw	a5,4(a0)
    proc[lst->tail].next_index = -1;
    800019bc:	17800713          	li	a4,376
    800019c0:	02e787b3          	mul	a5,a5,a4
    800019c4:	00010717          	auipc	a4,0x10
    800019c8:	d9c70713          	addi	a4,a4,-612 # 80011760 <proc>
    800019cc:	97ba                	add	a5,a5,a4
    800019ce:	577d                	li	a4,-1
    800019d0:	16e7aa23          	sw	a4,372(a5)
    800019d4:	bf71                	j	80001970 <remove_proc_to_list+0x42>

00000000800019d6 <proc_mapstacks>:

// Allocate a page for each process's kernel stack.
// Map it high in memory, followed by an invalid
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl) {
    800019d6:	7139                	addi	sp,sp,-64
    800019d8:	fc06                	sd	ra,56(sp)
    800019da:	f822                	sd	s0,48(sp)
    800019dc:	f426                	sd	s1,40(sp)
    800019de:	f04a                	sd	s2,32(sp)
    800019e0:	ec4e                	sd	s3,24(sp)
    800019e2:	e852                	sd	s4,16(sp)
    800019e4:	e456                	sd	s5,8(sp)
    800019e6:	e05a                	sd	s6,0(sp)
    800019e8:	0080                	addi	s0,sp,64
    800019ea:	89aa                	mv	s3,a0
  struct proc *p;
  
  for(p = proc; p < &proc[NPROC]; p++) {
    800019ec:	00010497          	auipc	s1,0x10
    800019f0:	d7448493          	addi	s1,s1,-652 # 80011760 <proc>
    char *pa = kalloc();
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));
    800019f4:	8b26                	mv	s6,s1
    800019f6:	00006a97          	auipc	s5,0x6
    800019fa:	60aa8a93          	addi	s5,s5,1546 # 80008000 <etext>
    800019fe:	04000937          	lui	s2,0x4000
    80001a02:	197d                	addi	s2,s2,-1
    80001a04:	0932                	slli	s2,s2,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    80001a06:	00016a17          	auipc	s4,0x16
    80001a0a:	b5aa0a13          	addi	s4,s4,-1190 # 80017560 <tickslock>
    char *pa = kalloc();
    80001a0e:	fffff097          	auipc	ra,0xfffff
    80001a12:	0e6080e7          	jalr	230(ra) # 80000af4 <kalloc>
    80001a16:	862a                	mv	a2,a0
    if(pa == 0)
    80001a18:	c131                	beqz	a0,80001a5c <proc_mapstacks+0x86>
    uint64 va = KSTACK((int) (p - proc));
    80001a1a:	416485b3          	sub	a1,s1,s6
    80001a1e:	858d                	srai	a1,a1,0x3
    80001a20:	000ab783          	ld	a5,0(s5)
    80001a24:	02f585b3          	mul	a1,a1,a5
    80001a28:	2585                	addiw	a1,a1,1
    80001a2a:	00d5959b          	slliw	a1,a1,0xd
    kvmmap(kpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
    80001a2e:	4719                	li	a4,6
    80001a30:	6685                	lui	a3,0x1
    80001a32:	40b905b3          	sub	a1,s2,a1
    80001a36:	854e                	mv	a0,s3
    80001a38:	fffff097          	auipc	ra,0xfffff
    80001a3c:	718080e7          	jalr	1816(ra) # 80001150 <kvmmap>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001a40:	17848493          	addi	s1,s1,376
    80001a44:	fd4495e3          	bne	s1,s4,80001a0e <proc_mapstacks+0x38>
  }
}
    80001a48:	70e2                	ld	ra,56(sp)
    80001a4a:	7442                	ld	s0,48(sp)
    80001a4c:	74a2                	ld	s1,40(sp)
    80001a4e:	7902                	ld	s2,32(sp)
    80001a50:	69e2                	ld	s3,24(sp)
    80001a52:	6a42                	ld	s4,16(sp)
    80001a54:	6aa2                	ld	s5,8(sp)
    80001a56:	6b02                	ld	s6,0(sp)
    80001a58:	6121                	addi	sp,sp,64
    80001a5a:	8082                	ret
      panic("kalloc");
    80001a5c:	00006517          	auipc	a0,0x6
    80001a60:	78450513          	addi	a0,a0,1924 # 800081e0 <digits+0x1a0>
    80001a64:	fffff097          	auipc	ra,0xfffff
    80001a68:	ada080e7          	jalr	-1318(ra) # 8000053e <panic>

0000000080001a6c <procinit>:

// initialize the proc table at boot time.
void
procinit(void)
{
    80001a6c:	711d                	addi	sp,sp,-96
    80001a6e:	ec86                	sd	ra,88(sp)
    80001a70:	e8a2                	sd	s0,80(sp)
    80001a72:	e4a6                	sd	s1,72(sp)
    80001a74:	e0ca                	sd	s2,64(sp)
    80001a76:	fc4e                	sd	s3,56(sp)
    80001a78:	f852                	sd	s4,48(sp)
    80001a7a:	f456                	sd	s5,40(sp)
    80001a7c:	f05a                	sd	s6,32(sp)
    80001a7e:	ec5e                	sd	s7,24(sp)
    80001a80:	e862                	sd	s8,16(sp)
    80001a82:	e466                	sd	s9,8(sp)
    80001a84:	1080                	addi	s0,sp,96
  struct proc *p;
  
  initlock(&pid_lock, "nextpid");
    80001a86:	00006597          	auipc	a1,0x6
    80001a8a:	76258593          	addi	a1,a1,1890 # 800081e8 <digits+0x1a8>
    80001a8e:	00010517          	auipc	a0,0x10
    80001a92:	ca250513          	addi	a0,a0,-862 # 80011730 <pid_lock>
    80001a96:	fffff097          	auipc	ra,0xfffff
    80001a9a:	0be080e7          	jalr	190(ra) # 80000b54 <initlock>
  initlock(&wait_lock, "wait_lock");
    80001a9e:	00006597          	auipc	a1,0x6
    80001aa2:	75258593          	addi	a1,a1,1874 # 800081f0 <digits+0x1b0>
    80001aa6:	00010517          	auipc	a0,0x10
    80001aaa:	ca250513          	addi	a0,a0,-862 # 80011748 <wait_lock>
    80001aae:	fffff097          	auipc	ra,0xfffff
    80001ab2:	0a6080e7          	jalr	166(ra) # 80000b54 <initlock>

  int i = 0;
    80001ab6:	4981                	li	s3,0
  for(p = proc; p < &proc[NPROC]; p++) {
    80001ab8:	00010497          	auipc	s1,0x10
    80001abc:	ca848493          	addi	s1,s1,-856 # 80011760 <proc>
      initlock(&p->lock, "proc");
    80001ac0:	00006c97          	auipc	s9,0x6
    80001ac4:	740c8c93          	addi	s9,s9,1856 # 80008200 <digits+0x1c0>
      p->kstack = KSTACK((int) (p - proc));
    80001ac8:	8926                	mv	s2,s1
    80001aca:	00006c17          	auipc	s8,0x6
    80001ace:	536c0c13          	addi	s8,s8,1334 # 80008000 <etext>
    80001ad2:	04000ab7          	lui	s5,0x4000
    80001ad6:	1afd                	addi	s5,s5,-1
    80001ad8:	0ab2                	slli	s5,s5,0xc
  proc->next_index = -1;
    80001ada:	5a7d                	li	s4,-1
      p->index = i;
      initialize_proc(p);
      insert_proc_to_list(unused_list, p); // procinit to admit all UNUSED process entries
    80001adc:	00007b97          	auipc	s7,0x7
    80001ae0:	55cb8b93          	addi	s7,s7,1372 # 80009038 <unused_list>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001ae4:	00016b17          	auipc	s6,0x16
    80001ae8:	a7cb0b13          	addi	s6,s6,-1412 # 80017560 <tickslock>
      initlock(&p->lock, "proc");
    80001aec:	85e6                	mv	a1,s9
    80001aee:	8526                	mv	a0,s1
    80001af0:	fffff097          	auipc	ra,0xfffff
    80001af4:	064080e7          	jalr	100(ra) # 80000b54 <initlock>
      p->kstack = KSTACK((int) (p - proc));
    80001af8:	412487b3          	sub	a5,s1,s2
    80001afc:	878d                	srai	a5,a5,0x3
    80001afe:	000c3703          	ld	a4,0(s8)
    80001b02:	02e787b3          	mul	a5,a5,a4
    80001b06:	2785                	addiw	a5,a5,1
    80001b08:	00d7979b          	slliw	a5,a5,0xd
    80001b0c:	40fa87b3          	sub	a5,s5,a5
    80001b10:	e0bc                	sd	a5,64(s1)
      p->index = i;
    80001b12:	1734a623          	sw	s3,364(s1)
  proc->next_index = -1;
    80001b16:	17492a23          	sw	s4,372(s2) # 4000174 <_entry-0x7bfffe8c>
  proc->prev_index = -1;
    80001b1a:	17492823          	sw	s4,368(s2)
      insert_proc_to_list(unused_list, p); // procinit to admit all UNUSED process entries
    80001b1e:	85a6                	mv	a1,s1
    80001b20:	000bb503          	ld	a0,0(s7)
    80001b24:	00000097          	auipc	ra,0x0
    80001b28:	dc4080e7          	jalr	-572(ra) # 800018e8 <insert_proc_to_list>
      i++;
    80001b2c:	2985                	addiw	s3,s3,1
  for(p = proc; p < &proc[NPROC]; p++) {
    80001b2e:	17848493          	addi	s1,s1,376
    80001b32:	fb649de3          	bne	s1,s6,80001aec <procinit+0x80>
  }
}
    80001b36:	60e6                	ld	ra,88(sp)
    80001b38:	6446                	ld	s0,80(sp)
    80001b3a:	64a6                	ld	s1,72(sp)
    80001b3c:	6906                	ld	s2,64(sp)
    80001b3e:	79e2                	ld	s3,56(sp)
    80001b40:	7a42                	ld	s4,48(sp)
    80001b42:	7aa2                	ld	s5,40(sp)
    80001b44:	7b02                	ld	s6,32(sp)
    80001b46:	6be2                	ld	s7,24(sp)
    80001b48:	6c42                	ld	s8,16(sp)
    80001b4a:	6ca2                	ld	s9,8(sp)
    80001b4c:	6125                	addi	sp,sp,96
    80001b4e:	8082                	ret

0000000080001b50 <cpuid>:
// Must be called with interrupts disabled,
// to prevent race with process being moved
// to a different CPU.
int
cpuid()
{
    80001b50:	1141                	addi	sp,sp,-16
    80001b52:	e422                	sd	s0,8(sp)
    80001b54:	0800                	addi	s0,sp,16
  asm volatile("mv %0, tp" : "=r" (x) );
    80001b56:	8512                	mv	a0,tp
  int id = r_tp();
  return id;
}
    80001b58:	2501                	sext.w	a0,a0
    80001b5a:	6422                	ld	s0,8(sp)
    80001b5c:	0141                	addi	sp,sp,16
    80001b5e:	8082                	ret

0000000080001b60 <mycpu>:

// Return this CPU's cpu struct.
// Interrupts must be disabled.
struct cpu*
mycpu(void) {
    80001b60:	1141                	addi	sp,sp,-16
    80001b62:	e422                	sd	s0,8(sp)
    80001b64:	0800                	addi	s0,sp,16
    80001b66:	8792                	mv	a5,tp
  int id = cpuid();
  struct cpu *c = &cpus[id];
    80001b68:	0007851b          	sext.w	a0,a5
    80001b6c:	00351793          	slli	a5,a0,0x3
    80001b70:	97aa                	add	a5,a5,a0
    80001b72:	0792                	slli	a5,a5,0x4
  return c;
}
    80001b74:	0000f517          	auipc	a0,0xf
    80001b78:	73c50513          	addi	a0,a0,1852 # 800112b0 <cpus>
    80001b7c:	953e                	add	a0,a0,a5
    80001b7e:	6422                	ld	s0,8(sp)
    80001b80:	0141                	addi	sp,sp,16
    80001b82:	8082                	ret

0000000080001b84 <myproc>:

// Return the current struct proc *, or zero if none.
struct proc*
myproc(void) {
    80001b84:	1101                	addi	sp,sp,-32
    80001b86:	ec06                	sd	ra,24(sp)
    80001b88:	e822                	sd	s0,16(sp)
    80001b8a:	e426                	sd	s1,8(sp)
    80001b8c:	1000                	addi	s0,sp,32
  push_off();
    80001b8e:	fffff097          	auipc	ra,0xfffff
    80001b92:	00a080e7          	jalr	10(ra) # 80000b98 <push_off>
    80001b96:	8792                	mv	a5,tp
  struct cpu *c = mycpu();
  struct proc *p = c->proc;
    80001b98:	0007871b          	sext.w	a4,a5
    80001b9c:	00371793          	slli	a5,a4,0x3
    80001ba0:	97ba                	add	a5,a5,a4
    80001ba2:	0792                	slli	a5,a5,0x4
    80001ba4:	0000f717          	auipc	a4,0xf
    80001ba8:	70c70713          	addi	a4,a4,1804 # 800112b0 <cpus>
    80001bac:	97ba                	add	a5,a5,a4
    80001bae:	6384                	ld	s1,0(a5)
  pop_off();
    80001bb0:	fffff097          	auipc	ra,0xfffff
    80001bb4:	088080e7          	jalr	136(ra) # 80000c38 <pop_off>
  return p;
}
    80001bb8:	8526                	mv	a0,s1
    80001bba:	60e2                	ld	ra,24(sp)
    80001bbc:	6442                	ld	s0,16(sp)
    80001bbe:	64a2                	ld	s1,8(sp)
    80001bc0:	6105                	addi	sp,sp,32
    80001bc2:	8082                	ret

0000000080001bc4 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch to forkret.
void
forkret(void)
{
    80001bc4:	1141                	addi	sp,sp,-16
    80001bc6:	e406                	sd	ra,8(sp)
    80001bc8:	e022                	sd	s0,0(sp)
    80001bca:	0800                	addi	s0,sp,16
  static int first = 1;

  // Still holding p->lock from scheduler.
  release(&myproc()->lock);
    80001bcc:	00000097          	auipc	ra,0x0
    80001bd0:	fb8080e7          	jalr	-72(ra) # 80001b84 <myproc>
    80001bd4:	fffff097          	auipc	ra,0xfffff
    80001bd8:	0c4080e7          	jalr	196(ra) # 80000c98 <release>

  if (first) {
    80001bdc:	00007797          	auipc	a5,0x7
    80001be0:	c547a783          	lw	a5,-940(a5) # 80008830 <first.1739>
    80001be4:	eb89                	bnez	a5,80001bf6 <forkret+0x32>
    // be run from main().
    first = 0;
    fsinit(ROOTDEV);
  }

  usertrapret();
    80001be6:	00001097          	auipc	ra,0x1
    80001bea:	e3a080e7          	jalr	-454(ra) # 80002a20 <usertrapret>
}
    80001bee:	60a2                	ld	ra,8(sp)
    80001bf0:	6402                	ld	s0,0(sp)
    80001bf2:	0141                	addi	sp,sp,16
    80001bf4:	8082                	ret
    first = 0;
    80001bf6:	00007797          	auipc	a5,0x7
    80001bfa:	c207ad23          	sw	zero,-966(a5) # 80008830 <first.1739>
    fsinit(ROOTDEV);
    80001bfe:	4505                	li	a0,1
    80001c00:	00002097          	auipc	ra,0x2
    80001c04:	bac080e7          	jalr	-1108(ra) # 800037ac <fsinit>
    80001c08:	bff9                	j	80001be6 <forkret+0x22>

0000000080001c0a <allocpid>:
allocpid() {
    80001c0a:	1101                	addi	sp,sp,-32
    80001c0c:	ec06                	sd	ra,24(sp)
    80001c0e:	e822                	sd	s0,16(sp)
    80001c10:	e426                	sd	s1,8(sp)
    80001c12:	e04a                	sd	s2,0(sp)
    80001c14:	1000                	addi	s0,sp,32
    pid = nextpid;
    80001c16:	00007917          	auipc	s2,0x7
    80001c1a:	c1e90913          	addi	s2,s2,-994 # 80008834 <nextpid>
    80001c1e:	00092483          	lw	s1,0(s2)
  } while(cas(&nextpid, pid, nextpid + 1));
    80001c22:	0014861b          	addiw	a2,s1,1
    80001c26:	85a6                	mv	a1,s1
    80001c28:	854a                	mv	a0,s2
    80001c2a:	00005097          	auipc	ra,0x5
    80001c2e:	98c080e7          	jalr	-1652(ra) # 800065b6 <cas>
    80001c32:	2501                	sext.w	a0,a0
    80001c34:	f56d                	bnez	a0,80001c1e <allocpid+0x14>
}
    80001c36:	8526                	mv	a0,s1
    80001c38:	60e2                	ld	ra,24(sp)
    80001c3a:	6442                	ld	s0,16(sp)
    80001c3c:	64a2                	ld	s1,8(sp)
    80001c3e:	6902                	ld	s2,0(sp)
    80001c40:	6105                	addi	sp,sp,32
    80001c42:	8082                	ret

0000000080001c44 <proc_pagetable>:
{
    80001c44:	1101                	addi	sp,sp,-32
    80001c46:	ec06                	sd	ra,24(sp)
    80001c48:	e822                	sd	s0,16(sp)
    80001c4a:	e426                	sd	s1,8(sp)
    80001c4c:	e04a                	sd	s2,0(sp)
    80001c4e:	1000                	addi	s0,sp,32
    80001c50:	892a                	mv	s2,a0
  pagetable = uvmcreate();
    80001c52:	fffff097          	auipc	ra,0xfffff
    80001c56:	6e8080e7          	jalr	1768(ra) # 8000133a <uvmcreate>
    80001c5a:	84aa                	mv	s1,a0
  if(pagetable == 0)
    80001c5c:	c121                	beqz	a0,80001c9c <proc_pagetable+0x58>
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
    80001c5e:	4729                	li	a4,10
    80001c60:	00005697          	auipc	a3,0x5
    80001c64:	3a068693          	addi	a3,a3,928 # 80007000 <_trampoline>
    80001c68:	6605                	lui	a2,0x1
    80001c6a:	040005b7          	lui	a1,0x4000
    80001c6e:	15fd                	addi	a1,a1,-1
    80001c70:	05b2                	slli	a1,a1,0xc
    80001c72:	fffff097          	auipc	ra,0xfffff
    80001c76:	43e080e7          	jalr	1086(ra) # 800010b0 <mappages>
    80001c7a:	02054863          	bltz	a0,80001caa <proc_pagetable+0x66>
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
    80001c7e:	4719                	li	a4,6
    80001c80:	05893683          	ld	a3,88(s2)
    80001c84:	6605                	lui	a2,0x1
    80001c86:	020005b7          	lui	a1,0x2000
    80001c8a:	15fd                	addi	a1,a1,-1
    80001c8c:	05b6                	slli	a1,a1,0xd
    80001c8e:	8526                	mv	a0,s1
    80001c90:	fffff097          	auipc	ra,0xfffff
    80001c94:	420080e7          	jalr	1056(ra) # 800010b0 <mappages>
    80001c98:	02054163          	bltz	a0,80001cba <proc_pagetable+0x76>
}
    80001c9c:	8526                	mv	a0,s1
    80001c9e:	60e2                	ld	ra,24(sp)
    80001ca0:	6442                	ld	s0,16(sp)
    80001ca2:	64a2                	ld	s1,8(sp)
    80001ca4:	6902                	ld	s2,0(sp)
    80001ca6:	6105                	addi	sp,sp,32
    80001ca8:	8082                	ret
    uvmfree(pagetable, 0);
    80001caa:	4581                	li	a1,0
    80001cac:	8526                	mv	a0,s1
    80001cae:	00000097          	auipc	ra,0x0
    80001cb2:	888080e7          	jalr	-1912(ra) # 80001536 <uvmfree>
    return 0;
    80001cb6:	4481                	li	s1,0
    80001cb8:	b7d5                	j	80001c9c <proc_pagetable+0x58>
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001cba:	4681                	li	a3,0
    80001cbc:	4605                	li	a2,1
    80001cbe:	040005b7          	lui	a1,0x4000
    80001cc2:	15fd                	addi	a1,a1,-1
    80001cc4:	05b2                	slli	a1,a1,0xc
    80001cc6:	8526                	mv	a0,s1
    80001cc8:	fffff097          	auipc	ra,0xfffff
    80001ccc:	5ae080e7          	jalr	1454(ra) # 80001276 <uvmunmap>
    uvmfree(pagetable, 0);
    80001cd0:	4581                	li	a1,0
    80001cd2:	8526                	mv	a0,s1
    80001cd4:	00000097          	auipc	ra,0x0
    80001cd8:	862080e7          	jalr	-1950(ra) # 80001536 <uvmfree>
    return 0;
    80001cdc:	4481                	li	s1,0
    80001cde:	bf7d                	j	80001c9c <proc_pagetable+0x58>

0000000080001ce0 <proc_freepagetable>:
{
    80001ce0:	1101                	addi	sp,sp,-32
    80001ce2:	ec06                	sd	ra,24(sp)
    80001ce4:	e822                	sd	s0,16(sp)
    80001ce6:	e426                	sd	s1,8(sp)
    80001ce8:	e04a                	sd	s2,0(sp)
    80001cea:	1000                	addi	s0,sp,32
    80001cec:	84aa                	mv	s1,a0
    80001cee:	892e                	mv	s2,a1
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001cf0:	4681                	li	a3,0
    80001cf2:	4605                	li	a2,1
    80001cf4:	040005b7          	lui	a1,0x4000
    80001cf8:	15fd                	addi	a1,a1,-1
    80001cfa:	05b2                	slli	a1,a1,0xc
    80001cfc:	fffff097          	auipc	ra,0xfffff
    80001d00:	57a080e7          	jalr	1402(ra) # 80001276 <uvmunmap>
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
    80001d04:	4681                	li	a3,0
    80001d06:	4605                	li	a2,1
    80001d08:	020005b7          	lui	a1,0x2000
    80001d0c:	15fd                	addi	a1,a1,-1
    80001d0e:	05b6                	slli	a1,a1,0xd
    80001d10:	8526                	mv	a0,s1
    80001d12:	fffff097          	auipc	ra,0xfffff
    80001d16:	564080e7          	jalr	1380(ra) # 80001276 <uvmunmap>
  uvmfree(pagetable, sz);
    80001d1a:	85ca                	mv	a1,s2
    80001d1c:	8526                	mv	a0,s1
    80001d1e:	00000097          	auipc	ra,0x0
    80001d22:	818080e7          	jalr	-2024(ra) # 80001536 <uvmfree>
}
    80001d26:	60e2                	ld	ra,24(sp)
    80001d28:	6442                	ld	s0,16(sp)
    80001d2a:	64a2                	ld	s1,8(sp)
    80001d2c:	6902                	ld	s2,0(sp)
    80001d2e:	6105                	addi	sp,sp,32
    80001d30:	8082                	ret

0000000080001d32 <freeproc>:
{
    80001d32:	1101                	addi	sp,sp,-32
    80001d34:	ec06                	sd	ra,24(sp)
    80001d36:	e822                	sd	s0,16(sp)
    80001d38:	e426                	sd	s1,8(sp)
    80001d3a:	1000                	addi	s0,sp,32
    80001d3c:	84aa                	mv	s1,a0
  if(p->trapframe)
    80001d3e:	6d28                	ld	a0,88(a0)
    80001d40:	c509                	beqz	a0,80001d4a <freeproc+0x18>
    kfree((void*)p->trapframe);
    80001d42:	fffff097          	auipc	ra,0xfffff
    80001d46:	cb6080e7          	jalr	-842(ra) # 800009f8 <kfree>
  p->trapframe = 0;
    80001d4a:	0404bc23          	sd	zero,88(s1)
  if(p->pagetable)
    80001d4e:	68a8                	ld	a0,80(s1)
    80001d50:	c511                	beqz	a0,80001d5c <freeproc+0x2a>
    proc_freepagetable(p->pagetable, p->sz);
    80001d52:	64ac                	ld	a1,72(s1)
    80001d54:	00000097          	auipc	ra,0x0
    80001d58:	f8c080e7          	jalr	-116(ra) # 80001ce0 <proc_freepagetable>
  p->pagetable = 0;
    80001d5c:	0404b823          	sd	zero,80(s1)
  p->sz = 0;
    80001d60:	0404b423          	sd	zero,72(s1)
  p->pid = 0;
    80001d64:	0204a823          	sw	zero,48(s1)
  p->parent = 0;
    80001d68:	0204bc23          	sd	zero,56(s1)
  p->name[0] = 0;
    80001d6c:	14048c23          	sb	zero,344(s1)
  p->chan = 0;
    80001d70:	0204b023          	sd	zero,32(s1)
  p->killed = 0;
    80001d74:	0204a423          	sw	zero,40(s1)
  p->xstate = 0;
    80001d78:	0204a623          	sw	zero,44(s1)
  p->state = UNUSED;
    80001d7c:	0004ac23          	sw	zero,24(s1)
  remove_proc_to_list(zombie_list, p); // remove the freed process from the ZOMBIE list
    80001d80:	85a6                	mv	a1,s1
    80001d82:	00007517          	auipc	a0,0x7
    80001d86:	2a653503          	ld	a0,678(a0) # 80009028 <zombie_list>
    80001d8a:	00000097          	auipc	ra,0x0
    80001d8e:	ba4080e7          	jalr	-1116(ra) # 8000192e <remove_proc_to_list>
  insert_proc_to_list(unused_list, p); // admit its entry to the UNUSED entry list.
    80001d92:	85a6                	mv	a1,s1
    80001d94:	00007517          	auipc	a0,0x7
    80001d98:	2a453503          	ld	a0,676(a0) # 80009038 <unused_list>
    80001d9c:	00000097          	auipc	ra,0x0
    80001da0:	b4c080e7          	jalr	-1204(ra) # 800018e8 <insert_proc_to_list>
}
    80001da4:	60e2                	ld	ra,24(sp)
    80001da6:	6442                	ld	s0,16(sp)
    80001da8:	64a2                	ld	s1,8(sp)
    80001daa:	6105                	addi	sp,sp,32
    80001dac:	8082                	ret

0000000080001dae <allocproc>:
{
    80001dae:	715d                	addi	sp,sp,-80
    80001db0:	e486                	sd	ra,72(sp)
    80001db2:	e0a2                	sd	s0,64(sp)
    80001db4:	fc26                	sd	s1,56(sp)
    80001db6:	f84a                	sd	s2,48(sp)
    80001db8:	f44e                	sd	s3,40(sp)
    80001dba:	f052                	sd	s4,32(sp)
    80001dbc:	ec56                	sd	s5,24(sp)
    80001dbe:	e85a                	sd	s6,16(sp)
    80001dc0:	e45e                	sd	s7,8(sp)
    80001dc2:	0880                	addi	s0,sp,80
  while(unused_list->head != -1){
    80001dc4:	00007797          	auipc	a5,0x7
    80001dc8:	2747b783          	ld	a5,628(a5) # 80009038 <unused_list>
    80001dcc:	0007a903          	lw	s2,0(a5)
    80001dd0:	57fd                	li	a5,-1
    80001dd2:	12f90c63          	beq	s2,a5,80001f0a <allocproc+0x15c>
    80001dd6:	17800a93          	li	s5,376
    p = &proc[unused_list->head];
    80001dda:	00010a17          	auipc	s4,0x10
    80001dde:	986a0a13          	addi	s4,s4,-1658 # 80011760 <proc>
  while(unused_list->head != -1){
    80001de2:	00007b97          	auipc	s7,0x7
    80001de6:	256b8b93          	addi	s7,s7,598 # 80009038 <unused_list>
    80001dea:	5b7d                	li	s6,-1
    p = &proc[unused_list->head];
    80001dec:	035909b3          	mul	s3,s2,s5
    80001df0:	014984b3          	add	s1,s3,s4
    acquire(&p->lock);
    80001df4:	8526                	mv	a0,s1
    80001df6:	fffff097          	auipc	ra,0xfffff
    80001dfa:	dee080e7          	jalr	-530(ra) # 80000be4 <acquire>
    if(p->state == UNUSED) {
    80001dfe:	4c9c                	lw	a5,24(s1)
    80001e00:	cb8d                	beqz	a5,80001e32 <allocproc+0x84>
      release(&p->lock);
    80001e02:	8526                	mv	a0,s1
    80001e04:	fffff097          	auipc	ra,0xfffff
    80001e08:	e94080e7          	jalr	-364(ra) # 80000c98 <release>
  while(unused_list->head != -1){
    80001e0c:	000bb783          	ld	a5,0(s7)
    80001e10:	0007a903          	lw	s2,0(a5)
    80001e14:	fd691ce3          	bne	s2,s6,80001dec <allocproc+0x3e>
  return 0;
    80001e18:	4481                	li	s1,0
}
    80001e1a:	8526                	mv	a0,s1
    80001e1c:	60a6                	ld	ra,72(sp)
    80001e1e:	6406                	ld	s0,64(sp)
    80001e20:	74e2                	ld	s1,56(sp)
    80001e22:	7942                	ld	s2,48(sp)
    80001e24:	79a2                	ld	s3,40(sp)
    80001e26:	7a02                	ld	s4,32(sp)
    80001e28:	6ae2                	ld	s5,24(sp)
    80001e2a:	6b42                	ld	s6,16(sp)
    80001e2c:	6ba2                	ld	s7,8(sp)
    80001e2e:	6161                	addi	sp,sp,80
    80001e30:	8082                	ret
      remove_proc_to_list(unused_list, p); // choose the new process entry to initialize from the UNUSED entry list.
    80001e32:	85a6                	mv	a1,s1
    80001e34:	00007517          	auipc	a0,0x7
    80001e38:	20453503          	ld	a0,516(a0) # 80009038 <unused_list>
    80001e3c:	00000097          	auipc	ra,0x0
    80001e40:	af2080e7          	jalr	-1294(ra) # 8000192e <remove_proc_to_list>
  p->pid = allocpid();
    80001e44:	00000097          	auipc	ra,0x0
    80001e48:	dc6080e7          	jalr	-570(ra) # 80001c0a <allocpid>
    80001e4c:	17800a13          	li	s4,376
    80001e50:	034907b3          	mul	a5,s2,s4
    80001e54:	00010a17          	auipc	s4,0x10
    80001e58:	90ca0a13          	addi	s4,s4,-1780 # 80011760 <proc>
    80001e5c:	9a3e                	add	s4,s4,a5
    80001e5e:	02aa2823          	sw	a0,48(s4)
  p->state = USED;
    80001e62:	4785                	li	a5,1
    80001e64:	00fa2c23          	sw	a5,24(s4)
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    80001e68:	fffff097          	auipc	ra,0xfffff
    80001e6c:	c8c080e7          	jalr	-884(ra) # 80000af4 <kalloc>
    80001e70:	8aaa                	mv	s5,a0
    80001e72:	04aa3c23          	sd	a0,88(s4)
    80001e76:	c135                	beqz	a0,80001eda <allocproc+0x12c>
  p->pagetable = proc_pagetable(p);
    80001e78:	8526                	mv	a0,s1
    80001e7a:	00000097          	auipc	ra,0x0
    80001e7e:	dca080e7          	jalr	-566(ra) # 80001c44 <proc_pagetable>
    80001e82:	8a2a                	mv	s4,a0
    80001e84:	17800793          	li	a5,376
    80001e88:	02f90733          	mul	a4,s2,a5
    80001e8c:	00010797          	auipc	a5,0x10
    80001e90:	8d478793          	addi	a5,a5,-1836 # 80011760 <proc>
    80001e94:	97ba                	add	a5,a5,a4
    80001e96:	eba8                	sd	a0,80(a5)
  if(p->pagetable == 0){
    80001e98:	cd29                	beqz	a0,80001ef2 <allocproc+0x144>
  memset(&p->context, 0, sizeof(p->context));
    80001e9a:	06098513          	addi	a0,s3,96 # 1060 <_entry-0x7fffefa0>
    80001e9e:	00010997          	auipc	s3,0x10
    80001ea2:	8c298993          	addi	s3,s3,-1854 # 80011760 <proc>
    80001ea6:	07000613          	li	a2,112
    80001eaa:	4581                	li	a1,0
    80001eac:	954e                	add	a0,a0,s3
    80001eae:	fffff097          	auipc	ra,0xfffff
    80001eb2:	e32080e7          	jalr	-462(ra) # 80000ce0 <memset>
  p->context.ra = (uint64)forkret;
    80001eb6:	17800793          	li	a5,376
    80001eba:	02f90933          	mul	s2,s2,a5
    80001ebe:	994e                	add	s2,s2,s3
    80001ec0:	00000797          	auipc	a5,0x0
    80001ec4:	d0478793          	addi	a5,a5,-764 # 80001bc4 <forkret>
    80001ec8:	06f93023          	sd	a5,96(s2)
  p->context.sp = p->kstack + PGSIZE;
    80001ecc:	04093783          	ld	a5,64(s2)
    80001ed0:	6705                	lui	a4,0x1
    80001ed2:	97ba                	add	a5,a5,a4
    80001ed4:	06f93423          	sd	a5,104(s2)
  return p;
    80001ed8:	b789                	j	80001e1a <allocproc+0x6c>
    freeproc(p);
    80001eda:	8526                	mv	a0,s1
    80001edc:	00000097          	auipc	ra,0x0
    80001ee0:	e56080e7          	jalr	-426(ra) # 80001d32 <freeproc>
    release(&p->lock);
    80001ee4:	8526                	mv	a0,s1
    80001ee6:	fffff097          	auipc	ra,0xfffff
    80001eea:	db2080e7          	jalr	-590(ra) # 80000c98 <release>
    return 0;
    80001eee:	84d6                	mv	s1,s5
    80001ef0:	b72d                	j	80001e1a <allocproc+0x6c>
    freeproc(p);
    80001ef2:	8526                	mv	a0,s1
    80001ef4:	00000097          	auipc	ra,0x0
    80001ef8:	e3e080e7          	jalr	-450(ra) # 80001d32 <freeproc>
    release(&p->lock);
    80001efc:	8526                	mv	a0,s1
    80001efe:	fffff097          	auipc	ra,0xfffff
    80001f02:	d9a080e7          	jalr	-614(ra) # 80000c98 <release>
    return 0;
    80001f06:	84d2                	mv	s1,s4
    80001f08:	bf09                	j	80001e1a <allocproc+0x6c>
  return 0;
    80001f0a:	4481                	li	s1,0
    80001f0c:	b739                	j	80001e1a <allocproc+0x6c>

0000000080001f0e <userinit>:
{
    80001f0e:	1101                	addi	sp,sp,-32
    80001f10:	ec06                	sd	ra,24(sp)
    80001f12:	e822                	sd	s0,16(sp)
    80001f14:	e426                	sd	s1,8(sp)
    80001f16:	1000                	addi	s0,sp,32
  p = allocproc();
    80001f18:	00000097          	auipc	ra,0x0
    80001f1c:	e96080e7          	jalr	-362(ra) # 80001dae <allocproc>
    80001f20:	84aa                	mv	s1,a0
  initproc = p;
    80001f22:	00007797          	auipc	a5,0x7
    80001f26:	10a7bf23          	sd	a0,286(a5) # 80009040 <initproc>
  uvminit(p->pagetable, initcode, sizeof(initcode));
    80001f2a:	03400613          	li	a2,52
    80001f2e:	00007597          	auipc	a1,0x7
    80001f32:	91258593          	addi	a1,a1,-1774 # 80008840 <initcode>
    80001f36:	6928                	ld	a0,80(a0)
    80001f38:	fffff097          	auipc	ra,0xfffff
    80001f3c:	430080e7          	jalr	1072(ra) # 80001368 <uvminit>
  p->sz = PGSIZE;
    80001f40:	6785                	lui	a5,0x1
    80001f42:	e4bc                	sd	a5,72(s1)
  p->trapframe->epc = 0;      // user program counter
    80001f44:	6cb8                	ld	a4,88(s1)
    80001f46:	00073c23          	sd	zero,24(a4) # 1018 <_entry-0x7fffefe8>
  p->trapframe->sp = PGSIZE;  // user stack pointer
    80001f4a:	6cb8                	ld	a4,88(s1)
    80001f4c:	fb1c                	sd	a5,48(a4)
  safestrcpy(p->name, "initcode", sizeof(p->name));
    80001f4e:	4641                	li	a2,16
    80001f50:	00006597          	auipc	a1,0x6
    80001f54:	2b858593          	addi	a1,a1,696 # 80008208 <digits+0x1c8>
    80001f58:	15848513          	addi	a0,s1,344
    80001f5c:	fffff097          	auipc	ra,0xfffff
    80001f60:	ed6080e7          	jalr	-298(ra) # 80000e32 <safestrcpy>
  p->cwd = namei("/");
    80001f64:	00006517          	auipc	a0,0x6
    80001f68:	2b450513          	addi	a0,a0,692 # 80008218 <digits+0x1d8>
    80001f6c:	00002097          	auipc	ra,0x2
    80001f70:	26e080e7          	jalr	622(ra) # 800041da <namei>
    80001f74:	14a4b823          	sd	a0,336(s1)
  p->state = RUNNABLE;
    80001f78:	478d                	li	a5,3
    80001f7a:	cc9c                	sw	a5,24(s1)
  insert_proc_to_list(cpus[0].runnable_list, p); // admit the init process (the first process in the OS) to the first CPU’s list.
    80001f7c:	85a6                	mv	a1,s1
    80001f7e:	0000f517          	auipc	a0,0xf
    80001f82:	3b253503          	ld	a0,946(a0) # 80011330 <cpus+0x80>
    80001f86:	00000097          	auipc	ra,0x0
    80001f8a:	962080e7          	jalr	-1694(ra) # 800018e8 <insert_proc_to_list>
  release(&p->lock);
    80001f8e:	8526                	mv	a0,s1
    80001f90:	fffff097          	auipc	ra,0xfffff
    80001f94:	d08080e7          	jalr	-760(ra) # 80000c98 <release>
}
    80001f98:	60e2                	ld	ra,24(sp)
    80001f9a:	6442                	ld	s0,16(sp)
    80001f9c:	64a2                	ld	s1,8(sp)
    80001f9e:	6105                	addi	sp,sp,32
    80001fa0:	8082                	ret

0000000080001fa2 <growproc>:
{
    80001fa2:	1101                	addi	sp,sp,-32
    80001fa4:	ec06                	sd	ra,24(sp)
    80001fa6:	e822                	sd	s0,16(sp)
    80001fa8:	e426                	sd	s1,8(sp)
    80001faa:	e04a                	sd	s2,0(sp)
    80001fac:	1000                	addi	s0,sp,32
    80001fae:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80001fb0:	00000097          	auipc	ra,0x0
    80001fb4:	bd4080e7          	jalr	-1068(ra) # 80001b84 <myproc>
    80001fb8:	892a                	mv	s2,a0
  sz = p->sz;
    80001fba:	652c                	ld	a1,72(a0)
    80001fbc:	0005861b          	sext.w	a2,a1
  if(n > 0){
    80001fc0:	00904f63          	bgtz	s1,80001fde <growproc+0x3c>
  } else if(n < 0){
    80001fc4:	0204cc63          	bltz	s1,80001ffc <growproc+0x5a>
  p->sz = sz;
    80001fc8:	1602                	slli	a2,a2,0x20
    80001fca:	9201                	srli	a2,a2,0x20
    80001fcc:	04c93423          	sd	a2,72(s2)
  return 0;
    80001fd0:	4501                	li	a0,0
}
    80001fd2:	60e2                	ld	ra,24(sp)
    80001fd4:	6442                	ld	s0,16(sp)
    80001fd6:	64a2                	ld	s1,8(sp)
    80001fd8:	6902                	ld	s2,0(sp)
    80001fda:	6105                	addi	sp,sp,32
    80001fdc:	8082                	ret
    if((sz = uvmalloc(p->pagetable, sz, sz + n)) == 0) {
    80001fde:	9e25                	addw	a2,a2,s1
    80001fe0:	1602                	slli	a2,a2,0x20
    80001fe2:	9201                	srli	a2,a2,0x20
    80001fe4:	1582                	slli	a1,a1,0x20
    80001fe6:	9181                	srli	a1,a1,0x20
    80001fe8:	6928                	ld	a0,80(a0)
    80001fea:	fffff097          	auipc	ra,0xfffff
    80001fee:	438080e7          	jalr	1080(ra) # 80001422 <uvmalloc>
    80001ff2:	0005061b          	sext.w	a2,a0
    80001ff6:	fa69                	bnez	a2,80001fc8 <growproc+0x26>
      return -1;
    80001ff8:	557d                	li	a0,-1
    80001ffa:	bfe1                	j	80001fd2 <growproc+0x30>
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    80001ffc:	9e25                	addw	a2,a2,s1
    80001ffe:	1602                	slli	a2,a2,0x20
    80002000:	9201                	srli	a2,a2,0x20
    80002002:	1582                	slli	a1,a1,0x20
    80002004:	9181                	srli	a1,a1,0x20
    80002006:	6928                	ld	a0,80(a0)
    80002008:	fffff097          	auipc	ra,0xfffff
    8000200c:	3d2080e7          	jalr	978(ra) # 800013da <uvmdealloc>
    80002010:	0005061b          	sext.w	a2,a0
    80002014:	bf55                	j	80001fc8 <growproc+0x26>

0000000080002016 <fork>:
{
    80002016:	7179                	addi	sp,sp,-48
    80002018:	f406                	sd	ra,40(sp)
    8000201a:	f022                	sd	s0,32(sp)
    8000201c:	ec26                	sd	s1,24(sp)
    8000201e:	e84a                	sd	s2,16(sp)
    80002020:	e44e                	sd	s3,8(sp)
    80002022:	e052                	sd	s4,0(sp)
    80002024:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80002026:	00000097          	auipc	ra,0x0
    8000202a:	b5e080e7          	jalr	-1186(ra) # 80001b84 <myproc>
    8000202e:	892a                	mv	s2,a0
  if((np = allocproc()) == 0){
    80002030:	00000097          	auipc	ra,0x0
    80002034:	d7e080e7          	jalr	-642(ra) # 80001dae <allocproc>
    80002038:	12050e63          	beqz	a0,80002174 <fork+0x15e>
    8000203c:	89aa                	mv	s3,a0
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0){
    8000203e:	04893603          	ld	a2,72(s2)
    80002042:	692c                	ld	a1,80(a0)
    80002044:	05093503          	ld	a0,80(s2)
    80002048:	fffff097          	auipc	ra,0xfffff
    8000204c:	526080e7          	jalr	1318(ra) # 8000156e <uvmcopy>
    80002050:	04054663          	bltz	a0,8000209c <fork+0x86>
  np->sz = p->sz;
    80002054:	04893783          	ld	a5,72(s2)
    80002058:	04f9b423          	sd	a5,72(s3)
  *(np->trapframe) = *(p->trapframe);
    8000205c:	05893683          	ld	a3,88(s2)
    80002060:	87b6                	mv	a5,a3
    80002062:	0589b703          	ld	a4,88(s3)
    80002066:	12068693          	addi	a3,a3,288
    8000206a:	0007b803          	ld	a6,0(a5) # 1000 <_entry-0x7ffff000>
    8000206e:	6788                	ld	a0,8(a5)
    80002070:	6b8c                	ld	a1,16(a5)
    80002072:	6f90                	ld	a2,24(a5)
    80002074:	01073023          	sd	a6,0(a4)
    80002078:	e708                	sd	a0,8(a4)
    8000207a:	eb0c                	sd	a1,16(a4)
    8000207c:	ef10                	sd	a2,24(a4)
    8000207e:	02078793          	addi	a5,a5,32
    80002082:	02070713          	addi	a4,a4,32
    80002086:	fed792e3          	bne	a5,a3,8000206a <fork+0x54>
  np->trapframe->a0 = 0;
    8000208a:	0589b783          	ld	a5,88(s3)
    8000208e:	0607b823          	sd	zero,112(a5)
    80002092:	0d000493          	li	s1,208
  for(i = 0; i < NOFILE; i++)
    80002096:	15000a13          	li	s4,336
    8000209a:	a03d                	j	800020c8 <fork+0xb2>
    freeproc(np);
    8000209c:	854e                	mv	a0,s3
    8000209e:	00000097          	auipc	ra,0x0
    800020a2:	c94080e7          	jalr	-876(ra) # 80001d32 <freeproc>
    release(&np->lock);
    800020a6:	854e                	mv	a0,s3
    800020a8:	fffff097          	auipc	ra,0xfffff
    800020ac:	bf0080e7          	jalr	-1040(ra) # 80000c98 <release>
    return -1;
    800020b0:	5a7d                	li	s4,-1
    800020b2:	a845                	j	80002162 <fork+0x14c>
      np->ofile[i] = filedup(p->ofile[i]);
    800020b4:	00002097          	auipc	ra,0x2
    800020b8:	7bc080e7          	jalr	1980(ra) # 80004870 <filedup>
    800020bc:	009987b3          	add	a5,s3,s1
    800020c0:	e388                	sd	a0,0(a5)
  for(i = 0; i < NOFILE; i++)
    800020c2:	04a1                	addi	s1,s1,8
    800020c4:	01448763          	beq	s1,s4,800020d2 <fork+0xbc>
    if(p->ofile[i])
    800020c8:	009907b3          	add	a5,s2,s1
    800020cc:	6388                	ld	a0,0(a5)
    800020ce:	f17d                	bnez	a0,800020b4 <fork+0x9e>
    800020d0:	bfcd                	j	800020c2 <fork+0xac>
  np->cwd = idup(p->cwd);
    800020d2:	15093503          	ld	a0,336(s2)
    800020d6:	00002097          	auipc	ra,0x2
    800020da:	910080e7          	jalr	-1776(ra) # 800039e6 <idup>
    800020de:	14a9b823          	sd	a0,336(s3)
  safestrcpy(np->name, p->name, sizeof(p->name));
    800020e2:	4641                	li	a2,16
    800020e4:	15890593          	addi	a1,s2,344
    800020e8:	15898513          	addi	a0,s3,344
    800020ec:	fffff097          	auipc	ra,0xfffff
    800020f0:	d46080e7          	jalr	-698(ra) # 80000e32 <safestrcpy>
  pid = np->pid;
    800020f4:	0309aa03          	lw	s4,48(s3)
  release(&np->lock);
    800020f8:	854e                	mv	a0,s3
    800020fa:	fffff097          	auipc	ra,0xfffff
    800020fe:	b9e080e7          	jalr	-1122(ra) # 80000c98 <release>
  acquire(&wait_lock);
    80002102:	0000f497          	auipc	s1,0xf
    80002106:	64648493          	addi	s1,s1,1606 # 80011748 <wait_lock>
    8000210a:	8526                	mv	a0,s1
    8000210c:	fffff097          	auipc	ra,0xfffff
    80002110:	ad8080e7          	jalr	-1320(ra) # 80000be4 <acquire>
  np->parent = p;
    80002114:	0329bc23          	sd	s2,56(s3)
  release(&wait_lock);
    80002118:	8526                	mv	a0,s1
    8000211a:	fffff097          	auipc	ra,0xfffff
    8000211e:	b7e080e7          	jalr	-1154(ra) # 80000c98 <release>
  acquire(&np->lock);
    80002122:	854e                	mv	a0,s3
    80002124:	fffff097          	auipc	ra,0xfffff
    80002128:	ac0080e7          	jalr	-1344(ra) # 80000be4 <acquire>
  np->state = RUNNABLE;
    8000212c:	478d                	li	a5,3
    8000212e:	00f9ac23          	sw	a5,24(s3)
  np->last_cpu = p->last_cpu; 
    80002132:	16892703          	lw	a4,360(s2)
    80002136:	16e9a423          	sw	a4,360(s3)
  insert_proc_to_list(cpus[np->last_cpu].runnable_list, p); // admit the new process to the father’s current CPU’s ready list
    8000213a:	00371793          	slli	a5,a4,0x3
    8000213e:	97ba                	add	a5,a5,a4
    80002140:	0792                	slli	a5,a5,0x4
    80002142:	0000f717          	auipc	a4,0xf
    80002146:	16e70713          	addi	a4,a4,366 # 800112b0 <cpus>
    8000214a:	97ba                	add	a5,a5,a4
    8000214c:	85ca                	mv	a1,s2
    8000214e:	63c8                	ld	a0,128(a5)
    80002150:	fffff097          	auipc	ra,0xfffff
    80002154:	798080e7          	jalr	1944(ra) # 800018e8 <insert_proc_to_list>
  release(&np->lock);
    80002158:	854e                	mv	a0,s3
    8000215a:	fffff097          	auipc	ra,0xfffff
    8000215e:	b3e080e7          	jalr	-1218(ra) # 80000c98 <release>
}
    80002162:	8552                	mv	a0,s4
    80002164:	70a2                	ld	ra,40(sp)
    80002166:	7402                	ld	s0,32(sp)
    80002168:	64e2                	ld	s1,24(sp)
    8000216a:	6942                	ld	s2,16(sp)
    8000216c:	69a2                	ld	s3,8(sp)
    8000216e:	6a02                	ld	s4,0(sp)
    80002170:	6145                	addi	sp,sp,48
    80002172:	8082                	ret
    return -1;
    80002174:	5a7d                	li	s4,-1
    80002176:	b7f5                	j	80002162 <fork+0x14c>

0000000080002178 <scheduler>:
{
    80002178:	715d                	addi	sp,sp,-80
    8000217a:	e486                	sd	ra,72(sp)
    8000217c:	e0a2                	sd	s0,64(sp)
    8000217e:	fc26                	sd	s1,56(sp)
    80002180:	f84a                	sd	s2,48(sp)
    80002182:	f44e                	sd	s3,40(sp)
    80002184:	f052                	sd	s4,32(sp)
    80002186:	ec56                	sd	s5,24(sp)
    80002188:	e85a                	sd	s6,16(sp)
    8000218a:	e45e                	sd	s7,8(sp)
    8000218c:	e062                	sd	s8,0(sp)
    8000218e:	0880                	addi	s0,sp,80
    80002190:	8712                	mv	a4,tp
  int id = r_tp();
    80002192:	2701                	sext.w	a4,a4
  c->proc = 0;
    80002194:	0000fb17          	auipc	s6,0xf
    80002198:	11cb0b13          	addi	s6,s6,284 # 800112b0 <cpus>
    8000219c:	00371793          	slli	a5,a4,0x3
    800021a0:	00e786b3          	add	a3,a5,a4
    800021a4:	0692                	slli	a3,a3,0x4
    800021a6:	96da                	add	a3,a3,s6
    800021a8:	0006b023          	sd	zero,0(a3)
      swtch(&c->context, &p->context);
    800021ac:	97ba                	add	a5,a5,a4
    800021ae:	0792                	slli	a5,a5,0x4
    800021b0:	07a1                	addi	a5,a5,8
    800021b2:	9b3e                	add	s6,s6,a5
    if(!isEmpty(c->runnable_list)) { // check whether there is a ready process in the cpu
    800021b4:	89b6                	mv	s3,a3
    800021b6:	5a7d                	li	s4,-1
    800021b8:	17800c13          	li	s8,376
      p =  &proc[c->runnable_list->head]; //  pick the first process from the correct CPU’s list.
    800021bc:	0000fa97          	auipc	s5,0xf
    800021c0:	5a4a8a93          	addi	s5,s5,1444 # 80011760 <proc>
      p->state = RUNNING;
    800021c4:	4b91                	li	s7,4
    800021c6:	a891                	j	8000221a <scheduler+0xa2>
      p =  &proc[c->runnable_list->head]; //  pick the first process from the correct CPU’s list.
    800021c8:	0006a903          	lw	s2,0(a3)
    800021cc:	03890933          	mul	s2,s2,s8
    800021d0:	015904b3          	add	s1,s2,s5
      acquire(&p->lock);
    800021d4:	8526                	mv	a0,s1
    800021d6:	fffff097          	auipc	ra,0xfffff
    800021da:	a0e080e7          	jalr	-1522(ra) # 80000be4 <acquire>
      p->state = RUNNING;
    800021de:	0174ac23          	sw	s7,24(s1)
      remove_proc_to_list(c->runnable_list, p);
    800021e2:	85a6                	mv	a1,s1
    800021e4:	0809b503          	ld	a0,128(s3)
    800021e8:	fffff097          	auipc	ra,0xfffff
    800021ec:	746080e7          	jalr	1862(ra) # 8000192e <remove_proc_to_list>
      p->last_cpu = c->cpu_id;
    800021f0:	0889a783          	lw	a5,136(s3)
    800021f4:	16f4a423          	sw	a5,360(s1)
      c->proc = p;
    800021f8:	0099b023          	sd	s1,0(s3)
      swtch(&c->context, &p->context);
    800021fc:	06090593          	addi	a1,s2,96
    80002200:	95d6                	add	a1,a1,s5
    80002202:	855a                	mv	a0,s6
    80002204:	00000097          	auipc	ra,0x0
    80002208:	772080e7          	jalr	1906(ra) # 80002976 <swtch>
      c->proc = 0;
    8000220c:	0009b023          	sd	zero,0(s3)
      release(&p->lock);
    80002210:	8526                	mv	a0,s1
    80002212:	fffff097          	auipc	ra,0xfffff
    80002216:	a86080e7          	jalr	-1402(ra) # 80000c98 <release>
    if(!isEmpty(c->runnable_list)) { // check whether there is a ready process in the cpu
    8000221a:	0809b683          	ld	a3,128(s3)
  return lst->tail == -1;
    8000221e:	42d8                	lw	a4,4(a3)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002220:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002224:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002228:	10079073          	csrw	sstatus,a5
    if(!isEmpty(c->runnable_list)) { // check whether there is a ready process in the cpu
    8000222c:	ff470ae3          	beq	a4,s4,80002220 <scheduler+0xa8>
    80002230:	bf61                	j	800021c8 <scheduler+0x50>

0000000080002232 <sched>:
{
    80002232:	7179                	addi	sp,sp,-48
    80002234:	f406                	sd	ra,40(sp)
    80002236:	f022                	sd	s0,32(sp)
    80002238:	ec26                	sd	s1,24(sp)
    8000223a:	e84a                	sd	s2,16(sp)
    8000223c:	e44e                	sd	s3,8(sp)
    8000223e:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80002240:	00000097          	auipc	ra,0x0
    80002244:	944080e7          	jalr	-1724(ra) # 80001b84 <myproc>
    80002248:	84aa                	mv	s1,a0
  if(!holding(&p->lock))
    8000224a:	fffff097          	auipc	ra,0xfffff
    8000224e:	920080e7          	jalr	-1760(ra) # 80000b6a <holding>
    80002252:	c559                	beqz	a0,800022e0 <sched+0xae>
  asm volatile("mv %0, tp" : "=r" (x) );
    80002254:	8792                	mv	a5,tp
  if(mycpu()->noff != 1)
    80002256:	0007871b          	sext.w	a4,a5
    8000225a:	00371793          	slli	a5,a4,0x3
    8000225e:	97ba                	add	a5,a5,a4
    80002260:	0792                	slli	a5,a5,0x4
    80002262:	0000f717          	auipc	a4,0xf
    80002266:	04e70713          	addi	a4,a4,78 # 800112b0 <cpus>
    8000226a:	97ba                	add	a5,a5,a4
    8000226c:	5fb8                	lw	a4,120(a5)
    8000226e:	4785                	li	a5,1
    80002270:	08f71063          	bne	a4,a5,800022f0 <sched+0xbe>
  if(p->state == RUNNING)
    80002274:	4c98                	lw	a4,24(s1)
    80002276:	4791                	li	a5,4
    80002278:	08f70463          	beq	a4,a5,80002300 <sched+0xce>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000227c:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80002280:	8b89                	andi	a5,a5,2
  if(intr_get())
    80002282:	e7d9                	bnez	a5,80002310 <sched+0xde>
  asm volatile("mv %0, tp" : "=r" (x) );
    80002284:	8792                	mv	a5,tp
  intena = mycpu()->intena;
    80002286:	0000f917          	auipc	s2,0xf
    8000228a:	02a90913          	addi	s2,s2,42 # 800112b0 <cpus>
    8000228e:	0007871b          	sext.w	a4,a5
    80002292:	00371793          	slli	a5,a4,0x3
    80002296:	97ba                	add	a5,a5,a4
    80002298:	0792                	slli	a5,a5,0x4
    8000229a:	97ca                	add	a5,a5,s2
    8000229c:	07c7a983          	lw	s3,124(a5)
    800022a0:	8592                	mv	a1,tp
  swtch(&p->context, &mycpu()->context);
    800022a2:	0005879b          	sext.w	a5,a1
    800022a6:	00379593          	slli	a1,a5,0x3
    800022aa:	95be                	add	a1,a1,a5
    800022ac:	0592                	slli	a1,a1,0x4
    800022ae:	05a1                	addi	a1,a1,8
    800022b0:	95ca                	add	a1,a1,s2
    800022b2:	06048513          	addi	a0,s1,96
    800022b6:	00000097          	auipc	ra,0x0
    800022ba:	6c0080e7          	jalr	1728(ra) # 80002976 <swtch>
    800022be:	8792                	mv	a5,tp
  mycpu()->intena = intena;
    800022c0:	0007871b          	sext.w	a4,a5
    800022c4:	00371793          	slli	a5,a4,0x3
    800022c8:	97ba                	add	a5,a5,a4
    800022ca:	0792                	slli	a5,a5,0x4
    800022cc:	993e                	add	s2,s2,a5
    800022ce:	07392e23          	sw	s3,124(s2)
}
    800022d2:	70a2                	ld	ra,40(sp)
    800022d4:	7402                	ld	s0,32(sp)
    800022d6:	64e2                	ld	s1,24(sp)
    800022d8:	6942                	ld	s2,16(sp)
    800022da:	69a2                	ld	s3,8(sp)
    800022dc:	6145                	addi	sp,sp,48
    800022de:	8082                	ret
    panic("sched p->lock");
    800022e0:	00006517          	auipc	a0,0x6
    800022e4:	f4050513          	addi	a0,a0,-192 # 80008220 <digits+0x1e0>
    800022e8:	ffffe097          	auipc	ra,0xffffe
    800022ec:	256080e7          	jalr	598(ra) # 8000053e <panic>
    panic("sched locks");
    800022f0:	00006517          	auipc	a0,0x6
    800022f4:	f4050513          	addi	a0,a0,-192 # 80008230 <digits+0x1f0>
    800022f8:	ffffe097          	auipc	ra,0xffffe
    800022fc:	246080e7          	jalr	582(ra) # 8000053e <panic>
    panic("sched running");
    80002300:	00006517          	auipc	a0,0x6
    80002304:	f4050513          	addi	a0,a0,-192 # 80008240 <digits+0x200>
    80002308:	ffffe097          	auipc	ra,0xffffe
    8000230c:	236080e7          	jalr	566(ra) # 8000053e <panic>
    panic("sched interruptible");
    80002310:	00006517          	auipc	a0,0x6
    80002314:	f4050513          	addi	a0,a0,-192 # 80008250 <digits+0x210>
    80002318:	ffffe097          	auipc	ra,0xffffe
    8000231c:	226080e7          	jalr	550(ra) # 8000053e <panic>

0000000080002320 <yield>:
{
    80002320:	1101                	addi	sp,sp,-32
    80002322:	ec06                	sd	ra,24(sp)
    80002324:	e822                	sd	s0,16(sp)
    80002326:	e426                	sd	s1,8(sp)
    80002328:	e04a                	sd	s2,0(sp)
    8000232a:	1000                	addi	s0,sp,32
  struct proc *p = myproc();
    8000232c:	00000097          	auipc	ra,0x0
    80002330:	858080e7          	jalr	-1960(ra) # 80001b84 <myproc>
    80002334:	84aa                	mv	s1,a0
    80002336:	8912                	mv	s2,tp
  acquire(&p->lock);
    80002338:	fffff097          	auipc	ra,0xfffff
    8000233c:	8ac080e7          	jalr	-1876(ra) # 80000be4 <acquire>
  p->state = RUNNABLE;
    80002340:	478d                	li	a5,3
    80002342:	cc9c                	sw	a5,24(s1)
  insert_proc_to_list(c->runnable_list, p); // insert_proc_to_list(c->runnable_list, p); // TODO: check
    80002344:	0009071b          	sext.w	a4,s2
    80002348:	00371793          	slli	a5,a4,0x3
    8000234c:	97ba                	add	a5,a5,a4
    8000234e:	0792                	slli	a5,a5,0x4
    80002350:	0000f717          	auipc	a4,0xf
    80002354:	f6070713          	addi	a4,a4,-160 # 800112b0 <cpus>
    80002358:	97ba                	add	a5,a5,a4
    8000235a:	85a6                	mv	a1,s1
    8000235c:	63c8                	ld	a0,128(a5)
    8000235e:	fffff097          	auipc	ra,0xfffff
    80002362:	58a080e7          	jalr	1418(ra) # 800018e8 <insert_proc_to_list>
  sched();
    80002366:	00000097          	auipc	ra,0x0
    8000236a:	ecc080e7          	jalr	-308(ra) # 80002232 <sched>
  release(&p->lock);
    8000236e:	8526                	mv	a0,s1
    80002370:	fffff097          	auipc	ra,0xfffff
    80002374:	928080e7          	jalr	-1752(ra) # 80000c98 <release>
}
    80002378:	60e2                	ld	ra,24(sp)
    8000237a:	6442                	ld	s0,16(sp)
    8000237c:	64a2                	ld	s1,8(sp)
    8000237e:	6902                	ld	s2,0(sp)
    80002380:	6105                	addi	sp,sp,32
    80002382:	8082                	ret

0000000080002384 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
    80002384:	7179                	addi	sp,sp,-48
    80002386:	f406                	sd	ra,40(sp)
    80002388:	f022                	sd	s0,32(sp)
    8000238a:	ec26                	sd	s1,24(sp)
    8000238c:	e84a                	sd	s2,16(sp)
    8000238e:	e44e                	sd	s3,8(sp)
    80002390:	1800                	addi	s0,sp,48
    80002392:	89aa                	mv	s3,a0
    80002394:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80002396:	fffff097          	auipc	ra,0xfffff
    8000239a:	7ee080e7          	jalr	2030(ra) # 80001b84 <myproc>
    8000239e:	84aa                	mv	s1,a0
  // Once we hold p->lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup locks p->lock),
  // so it's okay to release lk.

  acquire(&p->lock);  //DOC: sleeplock1
    800023a0:	fffff097          	auipc	ra,0xfffff
    800023a4:	844080e7          	jalr	-1980(ra) # 80000be4 <acquire>
  release(lk);
    800023a8:	854a                	mv	a0,s2
    800023aa:	fffff097          	auipc	ra,0xfffff
    800023ae:	8ee080e7          	jalr	-1810(ra) # 80000c98 <release>

  // Go to sleep.
  p->chan = chan;
    800023b2:	0334b023          	sd	s3,32(s1)
  p->state = SLEEPING;
    800023b6:	4789                	li	a5,2
    800023b8:	cc9c                	sw	a5,24(s1)
  insert_proc_to_list(sleeping_list, p);
    800023ba:	85a6                	mv	a1,s1
    800023bc:	00007517          	auipc	a0,0x7
    800023c0:	c7453503          	ld	a0,-908(a0) # 80009030 <sleeping_list>
    800023c4:	fffff097          	auipc	ra,0xfffff
    800023c8:	524080e7          	jalr	1316(ra) # 800018e8 <insert_proc_to_list>

  sched();
    800023cc:	00000097          	auipc	ra,0x0
    800023d0:	e66080e7          	jalr	-410(ra) # 80002232 <sched>

  // Tidy up.
  p->chan = 0;
    800023d4:	0204b023          	sd	zero,32(s1)

  // Reacquire original lock.
  release(&p->lock);
    800023d8:	8526                	mv	a0,s1
    800023da:	fffff097          	auipc	ra,0xfffff
    800023de:	8be080e7          	jalr	-1858(ra) # 80000c98 <release>
  acquire(lk);
    800023e2:	854a                	mv	a0,s2
    800023e4:	fffff097          	auipc	ra,0xfffff
    800023e8:	800080e7          	jalr	-2048(ra) # 80000be4 <acquire>
}
    800023ec:	70a2                	ld	ra,40(sp)
    800023ee:	7402                	ld	s0,32(sp)
    800023f0:	64e2                	ld	s1,24(sp)
    800023f2:	6942                	ld	s2,16(sp)
    800023f4:	69a2                	ld	s3,8(sp)
    800023f6:	6145                	addi	sp,sp,48
    800023f8:	8082                	ret

00000000800023fa <wait>:
{
    800023fa:	715d                	addi	sp,sp,-80
    800023fc:	e486                	sd	ra,72(sp)
    800023fe:	e0a2                	sd	s0,64(sp)
    80002400:	fc26                	sd	s1,56(sp)
    80002402:	f84a                	sd	s2,48(sp)
    80002404:	f44e                	sd	s3,40(sp)
    80002406:	f052                	sd	s4,32(sp)
    80002408:	ec56                	sd	s5,24(sp)
    8000240a:	e85a                	sd	s6,16(sp)
    8000240c:	e45e                	sd	s7,8(sp)
    8000240e:	e062                	sd	s8,0(sp)
    80002410:	0880                	addi	s0,sp,80
    80002412:	8b2a                	mv	s6,a0
  struct proc *p = myproc();
    80002414:	fffff097          	auipc	ra,0xfffff
    80002418:	770080e7          	jalr	1904(ra) # 80001b84 <myproc>
    8000241c:	892a                	mv	s2,a0
  acquire(&wait_lock);
    8000241e:	0000f517          	auipc	a0,0xf
    80002422:	32a50513          	addi	a0,a0,810 # 80011748 <wait_lock>
    80002426:	ffffe097          	auipc	ra,0xffffe
    8000242a:	7be080e7          	jalr	1982(ra) # 80000be4 <acquire>
    havekids = 0;
    8000242e:	4b81                	li	s7,0
        if(np->state == ZOMBIE){
    80002430:	4a15                	li	s4,5
    for(np = proc; np < &proc[NPROC]; np++){
    80002432:	00015997          	auipc	s3,0x15
    80002436:	12e98993          	addi	s3,s3,302 # 80017560 <tickslock>
        havekids = 1;
    8000243a:	4a85                	li	s5,1
    sleep(p, &wait_lock);  //DOC: wait-sleep
    8000243c:	0000fc17          	auipc	s8,0xf
    80002440:	30cc0c13          	addi	s8,s8,780 # 80011748 <wait_lock>
    havekids = 0;
    80002444:	875e                	mv	a4,s7
    for(np = proc; np < &proc[NPROC]; np++){
    80002446:	0000f497          	auipc	s1,0xf
    8000244a:	31a48493          	addi	s1,s1,794 # 80011760 <proc>
    8000244e:	a0bd                	j	800024bc <wait+0xc2>
          pid = np->pid;
    80002450:	0304a983          	lw	s3,48(s1)
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&np->xstate,
    80002454:	000b0e63          	beqz	s6,80002470 <wait+0x76>
    80002458:	4691                	li	a3,4
    8000245a:	02c48613          	addi	a2,s1,44
    8000245e:	85da                	mv	a1,s6
    80002460:	05093503          	ld	a0,80(s2)
    80002464:	fffff097          	auipc	ra,0xfffff
    80002468:	20e080e7          	jalr	526(ra) # 80001672 <copyout>
    8000246c:	02054563          	bltz	a0,80002496 <wait+0x9c>
          freeproc(np);
    80002470:	8526                	mv	a0,s1
    80002472:	00000097          	auipc	ra,0x0
    80002476:	8c0080e7          	jalr	-1856(ra) # 80001d32 <freeproc>
          release(&np->lock);
    8000247a:	8526                	mv	a0,s1
    8000247c:	fffff097          	auipc	ra,0xfffff
    80002480:	81c080e7          	jalr	-2020(ra) # 80000c98 <release>
          release(&wait_lock);
    80002484:	0000f517          	auipc	a0,0xf
    80002488:	2c450513          	addi	a0,a0,708 # 80011748 <wait_lock>
    8000248c:	fffff097          	auipc	ra,0xfffff
    80002490:	80c080e7          	jalr	-2036(ra) # 80000c98 <release>
          return pid;
    80002494:	a09d                	j	800024fa <wait+0x100>
            release(&np->lock);
    80002496:	8526                	mv	a0,s1
    80002498:	fffff097          	auipc	ra,0xfffff
    8000249c:	800080e7          	jalr	-2048(ra) # 80000c98 <release>
            release(&wait_lock);
    800024a0:	0000f517          	auipc	a0,0xf
    800024a4:	2a850513          	addi	a0,a0,680 # 80011748 <wait_lock>
    800024a8:	ffffe097          	auipc	ra,0xffffe
    800024ac:	7f0080e7          	jalr	2032(ra) # 80000c98 <release>
            return -1;
    800024b0:	59fd                	li	s3,-1
    800024b2:	a0a1                	j	800024fa <wait+0x100>
    for(np = proc; np < &proc[NPROC]; np++){
    800024b4:	17848493          	addi	s1,s1,376
    800024b8:	03348463          	beq	s1,s3,800024e0 <wait+0xe6>
      if(np->parent == p){
    800024bc:	7c9c                	ld	a5,56(s1)
    800024be:	ff279be3          	bne	a5,s2,800024b4 <wait+0xba>
        acquire(&np->lock);
    800024c2:	8526                	mv	a0,s1
    800024c4:	ffffe097          	auipc	ra,0xffffe
    800024c8:	720080e7          	jalr	1824(ra) # 80000be4 <acquire>
        if(np->state == ZOMBIE){
    800024cc:	4c9c                	lw	a5,24(s1)
    800024ce:	f94781e3          	beq	a5,s4,80002450 <wait+0x56>
        release(&np->lock);
    800024d2:	8526                	mv	a0,s1
    800024d4:	ffffe097          	auipc	ra,0xffffe
    800024d8:	7c4080e7          	jalr	1988(ra) # 80000c98 <release>
        havekids = 1;
    800024dc:	8756                	mv	a4,s5
    800024de:	bfd9                	j	800024b4 <wait+0xba>
    if(!havekids || p->killed){
    800024e0:	c701                	beqz	a4,800024e8 <wait+0xee>
    800024e2:	02892783          	lw	a5,40(s2)
    800024e6:	c79d                	beqz	a5,80002514 <wait+0x11a>
      release(&wait_lock);
    800024e8:	0000f517          	auipc	a0,0xf
    800024ec:	26050513          	addi	a0,a0,608 # 80011748 <wait_lock>
    800024f0:	ffffe097          	auipc	ra,0xffffe
    800024f4:	7a8080e7          	jalr	1960(ra) # 80000c98 <release>
      return -1;
    800024f8:	59fd                	li	s3,-1
}
    800024fa:	854e                	mv	a0,s3
    800024fc:	60a6                	ld	ra,72(sp)
    800024fe:	6406                	ld	s0,64(sp)
    80002500:	74e2                	ld	s1,56(sp)
    80002502:	7942                	ld	s2,48(sp)
    80002504:	79a2                	ld	s3,40(sp)
    80002506:	7a02                	ld	s4,32(sp)
    80002508:	6ae2                	ld	s5,24(sp)
    8000250a:	6b42                	ld	s6,16(sp)
    8000250c:	6ba2                	ld	s7,8(sp)
    8000250e:	6c02                	ld	s8,0(sp)
    80002510:	6161                	addi	sp,sp,80
    80002512:	8082                	ret
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002514:	85e2                	mv	a1,s8
    80002516:	854a                	mv	a0,s2
    80002518:	00000097          	auipc	ra,0x0
    8000251c:	e6c080e7          	jalr	-404(ra) # 80002384 <sleep>
    havekids = 0;
    80002520:	b715                	j	80002444 <wait+0x4a>

0000000080002522 <wakeup>:

// Wake up all processes sleeping on chan.
// Must be called without any p->lock.
void
wakeup(void *chan)
{
    80002522:	7159                	addi	sp,sp,-112
    80002524:	f486                	sd	ra,104(sp)
    80002526:	f0a2                	sd	s0,96(sp)
    80002528:	eca6                	sd	s1,88(sp)
    8000252a:	e8ca                	sd	s2,80(sp)
    8000252c:	e4ce                	sd	s3,72(sp)
    8000252e:	e0d2                	sd	s4,64(sp)
    80002530:	fc56                	sd	s5,56(sp)
    80002532:	f85a                	sd	s6,48(sp)
    80002534:	f45e                	sd	s7,40(sp)
    80002536:	f062                	sd	s8,32(sp)
    80002538:	ec66                	sd	s9,24(sp)
    8000253a:	e86a                	sd	s10,16(sp)
    8000253c:	e46e                	sd	s11,8(sp)
    8000253e:	1880                	addi	s0,sp,112
  struct proc *p;
  struct cpu *c;
  int curr = sleeping_list->head;
    80002540:	00007797          	auipc	a5,0x7
    80002544:	af07b783          	ld	a5,-1296(a5) # 80009030 <sleeping_list>
    80002548:	0007a903          	lw	s2,0(a5)
  int next;

  while(curr > -1) {
    8000254c:	08094863          	bltz	s2,800025dc <wakeup+0xba>
    80002550:	8baa                	mv	s7,a0
    p = &proc[curr];
    80002552:	17800a93          	li	s5,376
    80002556:	0000fa17          	auipc	s4,0xf
    8000255a:	20aa0a13          	addi	s4,s4,522 # 80011760 <proc>
    next = p->next_index;
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
    8000255e:	4b09                	li	s6,2
        c = &cpus[p->last_cpu];
        p->state = RUNNABLE;
    80002560:	4d0d                	li	s10,3
        remove_proc_to_list(sleeping_list, p);
    80002562:	00007c97          	auipc	s9,0x7
    80002566:	acec8c93          	addi	s9,s9,-1330 # 80009030 <sleeping_list>
        insert_proc_to_list(c->runnable_list, p);
    8000256a:	0000fc17          	auipc	s8,0xf
    8000256e:	d46c0c13          	addi	s8,s8,-698 # 800112b0 <cpus>
    80002572:	a801                	j	80002582 <wakeup+0x60>
      }
      release(&p->lock);
    80002574:	8526                	mv	a0,s1
    80002576:	ffffe097          	auipc	ra,0xffffe
    8000257a:	722080e7          	jalr	1826(ra) # 80000c98 <release>
  while(curr > -1) {
    8000257e:	04094f63          	bltz	s2,800025dc <wakeup+0xba>
    p = &proc[curr];
    80002582:	035904b3          	mul	s1,s2,s5
    80002586:	94d2                	add	s1,s1,s4
    next = p->next_index;
    80002588:	1744a903          	lw	s2,372(s1)
    if(p != myproc()){
    8000258c:	fffff097          	auipc	ra,0xfffff
    80002590:	5f8080e7          	jalr	1528(ra) # 80001b84 <myproc>
    80002594:	fea485e3          	beq	s1,a0,8000257e <wakeup+0x5c>
      acquire(&p->lock);
    80002598:	8526                	mv	a0,s1
    8000259a:	ffffe097          	auipc	ra,0xffffe
    8000259e:	64a080e7          	jalr	1610(ra) # 80000be4 <acquire>
      if(p->state == SLEEPING && p->chan == chan) {
    800025a2:	4c9c                	lw	a5,24(s1)
    800025a4:	fd6798e3          	bne	a5,s6,80002574 <wakeup+0x52>
    800025a8:	709c                	ld	a5,32(s1)
    800025aa:	fd7795e3          	bne	a5,s7,80002574 <wakeup+0x52>
        c = &cpus[p->last_cpu];
    800025ae:	1684ad83          	lw	s11,360(s1)
        p->state = RUNNABLE;
    800025b2:	01a4ac23          	sw	s10,24(s1)
        remove_proc_to_list(sleeping_list, p);
    800025b6:	85a6                	mv	a1,s1
    800025b8:	000cb503          	ld	a0,0(s9)
    800025bc:	fffff097          	auipc	ra,0xfffff
    800025c0:	372080e7          	jalr	882(ra) # 8000192e <remove_proc_to_list>
        insert_proc_to_list(c->runnable_list, p);
    800025c4:	003d9793          	slli	a5,s11,0x3
    800025c8:	97ee                	add	a5,a5,s11
    800025ca:	0792                	slli	a5,a5,0x4
    800025cc:	97e2                	add	a5,a5,s8
    800025ce:	85a6                	mv	a1,s1
    800025d0:	63c8                	ld	a0,128(a5)
    800025d2:	fffff097          	auipc	ra,0xfffff
    800025d6:	316080e7          	jalr	790(ra) # 800018e8 <insert_proc_to_list>
    800025da:	bf69                	j	80002574 <wakeup+0x52>
    }
    curr = next;
  }
}
    800025dc:	70a6                	ld	ra,104(sp)
    800025de:	7406                	ld	s0,96(sp)
    800025e0:	64e6                	ld	s1,88(sp)
    800025e2:	6946                	ld	s2,80(sp)
    800025e4:	69a6                	ld	s3,72(sp)
    800025e6:	6a06                	ld	s4,64(sp)
    800025e8:	7ae2                	ld	s5,56(sp)
    800025ea:	7b42                	ld	s6,48(sp)
    800025ec:	7ba2                	ld	s7,40(sp)
    800025ee:	7c02                	ld	s8,32(sp)
    800025f0:	6ce2                	ld	s9,24(sp)
    800025f2:	6d42                	ld	s10,16(sp)
    800025f4:	6da2                	ld	s11,8(sp)
    800025f6:	6165                	addi	sp,sp,112
    800025f8:	8082                	ret

00000000800025fa <reparent>:
{
    800025fa:	7179                	addi	sp,sp,-48
    800025fc:	f406                	sd	ra,40(sp)
    800025fe:	f022                	sd	s0,32(sp)
    80002600:	ec26                	sd	s1,24(sp)
    80002602:	e84a                	sd	s2,16(sp)
    80002604:	e44e                	sd	s3,8(sp)
    80002606:	e052                	sd	s4,0(sp)
    80002608:	1800                	addi	s0,sp,48
    8000260a:	892a                	mv	s2,a0
  for(pp = proc; pp < &proc[NPROC]; pp++){
    8000260c:	0000f497          	auipc	s1,0xf
    80002610:	15448493          	addi	s1,s1,340 # 80011760 <proc>
      pp->parent = initproc;
    80002614:	00007a17          	auipc	s4,0x7
    80002618:	a2ca0a13          	addi	s4,s4,-1492 # 80009040 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    8000261c:	00015997          	auipc	s3,0x15
    80002620:	f4498993          	addi	s3,s3,-188 # 80017560 <tickslock>
    80002624:	a029                	j	8000262e <reparent+0x34>
    80002626:	17848493          	addi	s1,s1,376
    8000262a:	01348d63          	beq	s1,s3,80002644 <reparent+0x4a>
    if(pp->parent == p){
    8000262e:	7c9c                	ld	a5,56(s1)
    80002630:	ff279be3          	bne	a5,s2,80002626 <reparent+0x2c>
      pp->parent = initproc;
    80002634:	000a3503          	ld	a0,0(s4)
    80002638:	fc88                	sd	a0,56(s1)
      wakeup(initproc);
    8000263a:	00000097          	auipc	ra,0x0
    8000263e:	ee8080e7          	jalr	-280(ra) # 80002522 <wakeup>
    80002642:	b7d5                	j	80002626 <reparent+0x2c>
}
    80002644:	70a2                	ld	ra,40(sp)
    80002646:	7402                	ld	s0,32(sp)
    80002648:	64e2                	ld	s1,24(sp)
    8000264a:	6942                	ld	s2,16(sp)
    8000264c:	69a2                	ld	s3,8(sp)
    8000264e:	6a02                	ld	s4,0(sp)
    80002650:	6145                	addi	sp,sp,48
    80002652:	8082                	ret

0000000080002654 <exit>:
{
    80002654:	7179                	addi	sp,sp,-48
    80002656:	f406                	sd	ra,40(sp)
    80002658:	f022                	sd	s0,32(sp)
    8000265a:	ec26                	sd	s1,24(sp)
    8000265c:	e84a                	sd	s2,16(sp)
    8000265e:	e44e                	sd	s3,8(sp)
    80002660:	e052                	sd	s4,0(sp)
    80002662:	1800                	addi	s0,sp,48
    80002664:	8a2a                	mv	s4,a0
  struct proc *p = myproc();
    80002666:	fffff097          	auipc	ra,0xfffff
    8000266a:	51e080e7          	jalr	1310(ra) # 80001b84 <myproc>
    8000266e:	89aa                	mv	s3,a0
  if(p == initproc)
    80002670:	00007797          	auipc	a5,0x7
    80002674:	9d07b783          	ld	a5,-1584(a5) # 80009040 <initproc>
    80002678:	0d050493          	addi	s1,a0,208
    8000267c:	15050913          	addi	s2,a0,336
    80002680:	02a79363          	bne	a5,a0,800026a6 <exit+0x52>
    panic("init exiting");
    80002684:	00006517          	auipc	a0,0x6
    80002688:	be450513          	addi	a0,a0,-1052 # 80008268 <digits+0x228>
    8000268c:	ffffe097          	auipc	ra,0xffffe
    80002690:	eb2080e7          	jalr	-334(ra) # 8000053e <panic>
      fileclose(f);
    80002694:	00002097          	auipc	ra,0x2
    80002698:	22e080e7          	jalr	558(ra) # 800048c2 <fileclose>
      p->ofile[fd] = 0;
    8000269c:	0004b023          	sd	zero,0(s1)
  for(int fd = 0; fd < NOFILE; fd++){
    800026a0:	04a1                	addi	s1,s1,8
    800026a2:	01248563          	beq	s1,s2,800026ac <exit+0x58>
    if(p->ofile[fd]){
    800026a6:	6088                	ld	a0,0(s1)
    800026a8:	f575                	bnez	a0,80002694 <exit+0x40>
    800026aa:	bfdd                	j	800026a0 <exit+0x4c>
  begin_op();
    800026ac:	00002097          	auipc	ra,0x2
    800026b0:	d4a080e7          	jalr	-694(ra) # 800043f6 <begin_op>
  iput(p->cwd);
    800026b4:	1509b503          	ld	a0,336(s3)
    800026b8:	00001097          	auipc	ra,0x1
    800026bc:	526080e7          	jalr	1318(ra) # 80003bde <iput>
  end_op();
    800026c0:	00002097          	auipc	ra,0x2
    800026c4:	db6080e7          	jalr	-586(ra) # 80004476 <end_op>
  p->cwd = 0;
    800026c8:	1409b823          	sd	zero,336(s3)
  acquire(&wait_lock);
    800026cc:	0000f497          	auipc	s1,0xf
    800026d0:	07c48493          	addi	s1,s1,124 # 80011748 <wait_lock>
    800026d4:	8526                	mv	a0,s1
    800026d6:	ffffe097          	auipc	ra,0xffffe
    800026da:	50e080e7          	jalr	1294(ra) # 80000be4 <acquire>
  reparent(p);
    800026de:	854e                	mv	a0,s3
    800026e0:	00000097          	auipc	ra,0x0
    800026e4:	f1a080e7          	jalr	-230(ra) # 800025fa <reparent>
  wakeup(p->parent);
    800026e8:	0389b503          	ld	a0,56(s3)
    800026ec:	00000097          	auipc	ra,0x0
    800026f0:	e36080e7          	jalr	-458(ra) # 80002522 <wakeup>
  acquire(&p->lock);
    800026f4:	854e                	mv	a0,s3
    800026f6:	ffffe097          	auipc	ra,0xffffe
    800026fa:	4ee080e7          	jalr	1262(ra) # 80000be4 <acquire>
  p->xstate = status;
    800026fe:	0349a623          	sw	s4,44(s3)
  p->state = ZOMBIE;
    80002702:	4795                	li	a5,5
    80002704:	00f9ac23          	sw	a5,24(s3)
  insert_proc_to_list(zombie_list, p); // exit to admit the exiting process to the ZOMBIE list
    80002708:	85ce                	mv	a1,s3
    8000270a:	00007517          	auipc	a0,0x7
    8000270e:	91e53503          	ld	a0,-1762(a0) # 80009028 <zombie_list>
    80002712:	fffff097          	auipc	ra,0xfffff
    80002716:	1d6080e7          	jalr	470(ra) # 800018e8 <insert_proc_to_list>
  release(&wait_lock);
    8000271a:	8526                	mv	a0,s1
    8000271c:	ffffe097          	auipc	ra,0xffffe
    80002720:	57c080e7          	jalr	1404(ra) # 80000c98 <release>
  sched();
    80002724:	00000097          	auipc	ra,0x0
    80002728:	b0e080e7          	jalr	-1266(ra) # 80002232 <sched>
  panic("zombie exit");
    8000272c:	00006517          	auipc	a0,0x6
    80002730:	b4c50513          	addi	a0,a0,-1204 # 80008278 <digits+0x238>
    80002734:	ffffe097          	auipc	ra,0xffffe
    80002738:	e0a080e7          	jalr	-502(ra) # 8000053e <panic>

000000008000273c <kill>:
// Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).
int
kill(int pid)
{
    8000273c:	7179                	addi	sp,sp,-48
    8000273e:	f406                	sd	ra,40(sp)
    80002740:	f022                	sd	s0,32(sp)
    80002742:	ec26                	sd	s1,24(sp)
    80002744:	e84a                	sd	s2,16(sp)
    80002746:	e44e                	sd	s3,8(sp)
    80002748:	1800                	addi	s0,sp,48
    8000274a:	892a                	mv	s2,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++){
    8000274c:	0000f497          	auipc	s1,0xf
    80002750:	01448493          	addi	s1,s1,20 # 80011760 <proc>
    80002754:	00015997          	auipc	s3,0x15
    80002758:	e0c98993          	addi	s3,s3,-500 # 80017560 <tickslock>
    acquire(&p->lock);
    8000275c:	8526                	mv	a0,s1
    8000275e:	ffffe097          	auipc	ra,0xffffe
    80002762:	486080e7          	jalr	1158(ra) # 80000be4 <acquire>
    if(p->pid == pid){
    80002766:	589c                	lw	a5,48(s1)
    80002768:	01278d63          	beq	a5,s2,80002782 <kill+0x46>
        p->state = RUNNABLE;
      }
      release(&p->lock);
      return 0;
    }
    release(&p->lock);
    8000276c:	8526                	mv	a0,s1
    8000276e:	ffffe097          	auipc	ra,0xffffe
    80002772:	52a080e7          	jalr	1322(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    80002776:	17848493          	addi	s1,s1,376
    8000277a:	ff3491e3          	bne	s1,s3,8000275c <kill+0x20>
  }
  return -1;
    8000277e:	557d                	li	a0,-1
    80002780:	a829                	j	8000279a <kill+0x5e>
      p->killed = 1;
    80002782:	4785                	li	a5,1
    80002784:	d49c                	sw	a5,40(s1)
      if(p->state == SLEEPING){
    80002786:	4c98                	lw	a4,24(s1)
    80002788:	4789                	li	a5,2
    8000278a:	00f70f63          	beq	a4,a5,800027a8 <kill+0x6c>
      release(&p->lock);
    8000278e:	8526                	mv	a0,s1
    80002790:	ffffe097          	auipc	ra,0xffffe
    80002794:	508080e7          	jalr	1288(ra) # 80000c98 <release>
      return 0;
    80002798:	4501                	li	a0,0
}
    8000279a:	70a2                	ld	ra,40(sp)
    8000279c:	7402                	ld	s0,32(sp)
    8000279e:	64e2                	ld	s1,24(sp)
    800027a0:	6942                	ld	s2,16(sp)
    800027a2:	69a2                	ld	s3,8(sp)
    800027a4:	6145                	addi	sp,sp,48
    800027a6:	8082                	ret
        p->state = RUNNABLE;
    800027a8:	478d                	li	a5,3
    800027aa:	cc9c                	sw	a5,24(s1)
    800027ac:	b7cd                	j	8000278e <kill+0x52>

00000000800027ae <either_copyout>:
// Copy to either a user address, or kernel address,
// depending on usr_dst.
// Returns 0 on success, -1 on error.
int
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
    800027ae:	7179                	addi	sp,sp,-48
    800027b0:	f406                	sd	ra,40(sp)
    800027b2:	f022                	sd	s0,32(sp)
    800027b4:	ec26                	sd	s1,24(sp)
    800027b6:	e84a                	sd	s2,16(sp)
    800027b8:	e44e                	sd	s3,8(sp)
    800027ba:	e052                	sd	s4,0(sp)
    800027bc:	1800                	addi	s0,sp,48
    800027be:	84aa                	mv	s1,a0
    800027c0:	892e                	mv	s2,a1
    800027c2:	89b2                	mv	s3,a2
    800027c4:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    800027c6:	fffff097          	auipc	ra,0xfffff
    800027ca:	3be080e7          	jalr	958(ra) # 80001b84 <myproc>
  if(user_dst){
    800027ce:	c08d                	beqz	s1,800027f0 <either_copyout+0x42>
    return copyout(p->pagetable, dst, src, len);
    800027d0:	86d2                	mv	a3,s4
    800027d2:	864e                	mv	a2,s3
    800027d4:	85ca                	mv	a1,s2
    800027d6:	6928                	ld	a0,80(a0)
    800027d8:	fffff097          	auipc	ra,0xfffff
    800027dc:	e9a080e7          	jalr	-358(ra) # 80001672 <copyout>
  } else {
    memmove((char *)dst, src, len);
    return 0;
  }
}
    800027e0:	70a2                	ld	ra,40(sp)
    800027e2:	7402                	ld	s0,32(sp)
    800027e4:	64e2                	ld	s1,24(sp)
    800027e6:	6942                	ld	s2,16(sp)
    800027e8:	69a2                	ld	s3,8(sp)
    800027ea:	6a02                	ld	s4,0(sp)
    800027ec:	6145                	addi	sp,sp,48
    800027ee:	8082                	ret
    memmove((char *)dst, src, len);
    800027f0:	000a061b          	sext.w	a2,s4
    800027f4:	85ce                	mv	a1,s3
    800027f6:	854a                	mv	a0,s2
    800027f8:	ffffe097          	auipc	ra,0xffffe
    800027fc:	548080e7          	jalr	1352(ra) # 80000d40 <memmove>
    return 0;
    80002800:	8526                	mv	a0,s1
    80002802:	bff9                	j	800027e0 <either_copyout+0x32>

0000000080002804 <either_copyin>:
// Copy from either a user address, or kernel address,
// depending on usr_src.
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
    80002804:	7179                	addi	sp,sp,-48
    80002806:	f406                	sd	ra,40(sp)
    80002808:	f022                	sd	s0,32(sp)
    8000280a:	ec26                	sd	s1,24(sp)
    8000280c:	e84a                	sd	s2,16(sp)
    8000280e:	e44e                	sd	s3,8(sp)
    80002810:	e052                	sd	s4,0(sp)
    80002812:	1800                	addi	s0,sp,48
    80002814:	892a                	mv	s2,a0
    80002816:	84ae                	mv	s1,a1
    80002818:	89b2                	mv	s3,a2
    8000281a:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    8000281c:	fffff097          	auipc	ra,0xfffff
    80002820:	368080e7          	jalr	872(ra) # 80001b84 <myproc>
  if(user_src){
    80002824:	c08d                	beqz	s1,80002846 <either_copyin+0x42>
    return copyin(p->pagetable, dst, src, len);
    80002826:	86d2                	mv	a3,s4
    80002828:	864e                	mv	a2,s3
    8000282a:	85ca                	mv	a1,s2
    8000282c:	6928                	ld	a0,80(a0)
    8000282e:	fffff097          	auipc	ra,0xfffff
    80002832:	ed0080e7          	jalr	-304(ra) # 800016fe <copyin>
  } else {
    memmove(dst, (char*)src, len);
    return 0;
  }
}
    80002836:	70a2                	ld	ra,40(sp)
    80002838:	7402                	ld	s0,32(sp)
    8000283a:	64e2                	ld	s1,24(sp)
    8000283c:	6942                	ld	s2,16(sp)
    8000283e:	69a2                	ld	s3,8(sp)
    80002840:	6a02                	ld	s4,0(sp)
    80002842:	6145                	addi	sp,sp,48
    80002844:	8082                	ret
    memmove(dst, (char*)src, len);
    80002846:	000a061b          	sext.w	a2,s4
    8000284a:	85ce                	mv	a1,s3
    8000284c:	854a                	mv	a0,s2
    8000284e:	ffffe097          	auipc	ra,0xffffe
    80002852:	4f2080e7          	jalr	1266(ra) # 80000d40 <memmove>
    return 0;
    80002856:	8526                	mv	a0,s1
    80002858:	bff9                	j	80002836 <either_copyin+0x32>

000000008000285a <procdump>:

// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further..
void
procdump(void){
    8000285a:	715d                	addi	sp,sp,-80
    8000285c:	e486                	sd	ra,72(sp)
    8000285e:	e0a2                	sd	s0,64(sp)
    80002860:	fc26                	sd	s1,56(sp)
    80002862:	f84a                	sd	s2,48(sp)
    80002864:	f44e                	sd	s3,40(sp)
    80002866:	f052                	sd	s4,32(sp)
    80002868:	ec56                	sd	s5,24(sp)
    8000286a:	e85a                	sd	s6,16(sp)
    8000286c:	e45e                	sd	s7,8(sp)
    8000286e:	0880                	addi	s0,sp,80
  [ZOMBIE]    "zombie"
  };
  struct proc *p;
  char *state;

  printf("\n");
    80002870:	00006517          	auipc	a0,0x6
    80002874:	85850513          	addi	a0,a0,-1960 # 800080c8 <digits+0x88>
    80002878:	ffffe097          	auipc	ra,0xffffe
    8000287c:	d10080e7          	jalr	-752(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    80002880:	0000f497          	auipc	s1,0xf
    80002884:	03848493          	addi	s1,s1,56 # 800118b8 <proc+0x158>
    80002888:	00015917          	auipc	s2,0x15
    8000288c:	e3090913          	addi	s2,s2,-464 # 800176b8 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002890:	4b15                	li	s6,5
      state = states[p->state];
    else
      state = "???"; 
    80002892:	00006997          	auipc	s3,0x6
    80002896:	9f698993          	addi	s3,s3,-1546 # 80008288 <digits+0x248>
    printf("%d %s %s", p->pid, state, p->name);
    8000289a:	00006a97          	auipc	s5,0x6
    8000289e:	9f6a8a93          	addi	s5,s5,-1546 # 80008290 <digits+0x250>
    printf("\n");
    800028a2:	00006a17          	auipc	s4,0x6
    800028a6:	826a0a13          	addi	s4,s4,-2010 # 800080c8 <digits+0x88>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    800028aa:	00006b97          	auipc	s7,0x6
    800028ae:	a1eb8b93          	addi	s7,s7,-1506 # 800082c8 <states.1779>
    800028b2:	a00d                	j	800028d4 <procdump+0x7a>
    printf("%d %s %s", p->pid, state, p->name);
    800028b4:	ed86a583          	lw	a1,-296(a3)
    800028b8:	8556                	mv	a0,s5
    800028ba:	ffffe097          	auipc	ra,0xffffe
    800028be:	cce080e7          	jalr	-818(ra) # 80000588 <printf>
    printf("\n");
    800028c2:	8552                	mv	a0,s4
    800028c4:	ffffe097          	auipc	ra,0xffffe
    800028c8:	cc4080e7          	jalr	-828(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    800028cc:	17848493          	addi	s1,s1,376
    800028d0:	03248163          	beq	s1,s2,800028f2 <procdump+0x98>
    if(p->state == UNUSED)
    800028d4:	86a6                	mv	a3,s1
    800028d6:	ec04a783          	lw	a5,-320(s1)
    800028da:	dbed                	beqz	a5,800028cc <procdump+0x72>
      state = "???"; 
    800028dc:	864e                	mv	a2,s3
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    800028de:	fcfb6be3          	bltu	s6,a5,800028b4 <procdump+0x5a>
    800028e2:	1782                	slli	a5,a5,0x20
    800028e4:	9381                	srli	a5,a5,0x20
    800028e6:	078e                	slli	a5,a5,0x3
    800028e8:	97de                	add	a5,a5,s7
    800028ea:	6390                	ld	a2,0(a5)
    800028ec:	f661                	bnez	a2,800028b4 <procdump+0x5a>
      state = "???"; 
    800028ee:	864e                	mv	a2,s3
    800028f0:	b7d1                	j	800028b4 <procdump+0x5a>
  }
}
    800028f2:	60a6                	ld	ra,72(sp)
    800028f4:	6406                	ld	s0,64(sp)
    800028f6:	74e2                	ld	s1,56(sp)
    800028f8:	7942                	ld	s2,48(sp)
    800028fa:	79a2                	ld	s3,40(sp)
    800028fc:	7a02                	ld	s4,32(sp)
    800028fe:	6ae2                	ld	s5,24(sp)
    80002900:	6b42                	ld	s6,16(sp)
    80002902:	6ba2                	ld	s7,8(sp)
    80002904:	6161                	addi	sp,sp,80
    80002906:	8082                	ret

0000000080002908 <set_cpu>:

// assign current process to a different CPU. 
int
set_cpu(int cpu_num){
    80002908:	1101                	addi	sp,sp,-32
    8000290a:	ec06                	sd	ra,24(sp)
    8000290c:	e822                	sd	s0,16(sp)
    8000290e:	e426                	sd	s1,8(sp)
    80002910:	e04a                	sd	s2,0(sp)
    80002912:	1000                	addi	s0,sp,32
    80002914:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80002916:	fffff097          	auipc	ra,0xfffff
    8000291a:	26e080e7          	jalr	622(ra) # 80001b84 <myproc>
  if(cpu_num >= 0 && cpu_num < NCPU && &cpus[cpu_num] != NULL){
    8000291e:	0004871b          	sext.w	a4,s1
    80002922:	479d                	li	a5,7
    80002924:	02e7e963          	bltu	a5,a4,80002956 <set_cpu+0x4e>
    80002928:	892a                	mv	s2,a0
    acquire(&p->lock);
    8000292a:	ffffe097          	auipc	ra,0xffffe
    8000292e:	2ba080e7          	jalr	698(ra) # 80000be4 <acquire>
    p->last_cpu = cpu_num;
    80002932:	16992423          	sw	s1,360(s2)
    release(&p->lock);
    80002936:	854a                	mv	a0,s2
    80002938:	ffffe097          	auipc	ra,0xffffe
    8000293c:	360080e7          	jalr	864(ra) # 80000c98 <release>

    yield();
    80002940:	00000097          	auipc	ra,0x0
    80002944:	9e0080e7          	jalr	-1568(ra) # 80002320 <yield>

    return cpu_num;
    80002948:	8526                	mv	a0,s1
  }
  return -1;
}
    8000294a:	60e2                	ld	ra,24(sp)
    8000294c:	6442                	ld	s0,16(sp)
    8000294e:	64a2                	ld	s1,8(sp)
    80002950:	6902                	ld	s2,0(sp)
    80002952:	6105                	addi	sp,sp,32
    80002954:	8082                	ret
  return -1;
    80002956:	557d                	li	a0,-1
    80002958:	bfcd                	j	8000294a <set_cpu+0x42>

000000008000295a <get_cpu>:

// return the current CPU id.
int
get_cpu(void){
    8000295a:	1141                	addi	sp,sp,-16
    8000295c:	e406                	sd	ra,8(sp)
    8000295e:	e022                	sd	s0,0(sp)
    80002960:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    80002962:	fffff097          	auipc	ra,0xfffff
    80002966:	222080e7          	jalr	546(ra) # 80001b84 <myproc>
  return p->last_cpu;
    8000296a:	16852503          	lw	a0,360(a0)
    8000296e:	60a2                	ld	ra,8(sp)
    80002970:	6402                	ld	s0,0(sp)
    80002972:	0141                	addi	sp,sp,16
    80002974:	8082                	ret

0000000080002976 <swtch>:
    80002976:	00153023          	sd	ra,0(a0)
    8000297a:	00253423          	sd	sp,8(a0)
    8000297e:	e900                	sd	s0,16(a0)
    80002980:	ed04                	sd	s1,24(a0)
    80002982:	03253023          	sd	s2,32(a0)
    80002986:	03353423          	sd	s3,40(a0)
    8000298a:	03453823          	sd	s4,48(a0)
    8000298e:	03553c23          	sd	s5,56(a0)
    80002992:	05653023          	sd	s6,64(a0)
    80002996:	05753423          	sd	s7,72(a0)
    8000299a:	05853823          	sd	s8,80(a0)
    8000299e:	05953c23          	sd	s9,88(a0)
    800029a2:	07a53023          	sd	s10,96(a0)
    800029a6:	07b53423          	sd	s11,104(a0)
    800029aa:	0005b083          	ld	ra,0(a1)
    800029ae:	0085b103          	ld	sp,8(a1)
    800029b2:	6980                	ld	s0,16(a1)
    800029b4:	6d84                	ld	s1,24(a1)
    800029b6:	0205b903          	ld	s2,32(a1)
    800029ba:	0285b983          	ld	s3,40(a1)
    800029be:	0305ba03          	ld	s4,48(a1)
    800029c2:	0385ba83          	ld	s5,56(a1)
    800029c6:	0405bb03          	ld	s6,64(a1)
    800029ca:	0485bb83          	ld	s7,72(a1)
    800029ce:	0505bc03          	ld	s8,80(a1)
    800029d2:	0585bc83          	ld	s9,88(a1)
    800029d6:	0605bd03          	ld	s10,96(a1)
    800029da:	0685bd83          	ld	s11,104(a1)
    800029de:	8082                	ret

00000000800029e0 <trapinit>:

extern int devintr();

void
trapinit(void)
{
    800029e0:	1141                	addi	sp,sp,-16
    800029e2:	e406                	sd	ra,8(sp)
    800029e4:	e022                	sd	s0,0(sp)
    800029e6:	0800                	addi	s0,sp,16
  initlock(&tickslock, "time");
    800029e8:	00006597          	auipc	a1,0x6
    800029ec:	91058593          	addi	a1,a1,-1776 # 800082f8 <states.1779+0x30>
    800029f0:	00015517          	auipc	a0,0x15
    800029f4:	b7050513          	addi	a0,a0,-1168 # 80017560 <tickslock>
    800029f8:	ffffe097          	auipc	ra,0xffffe
    800029fc:	15c080e7          	jalr	348(ra) # 80000b54 <initlock>
}
    80002a00:	60a2                	ld	ra,8(sp)
    80002a02:	6402                	ld	s0,0(sp)
    80002a04:	0141                	addi	sp,sp,16
    80002a06:	8082                	ret

0000000080002a08 <trapinithart>:

// set up to take exceptions and traps while in the kernel.
void
trapinithart(void)
{
    80002a08:	1141                	addi	sp,sp,-16
    80002a0a:	e422                	sd	s0,8(sp)
    80002a0c:	0800                	addi	s0,sp,16
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002a0e:	00003797          	auipc	a5,0x3
    80002a12:	4d278793          	addi	a5,a5,1234 # 80005ee0 <kernelvec>
    80002a16:	10579073          	csrw	stvec,a5
  w_stvec((uint64)kernelvec);
}
    80002a1a:	6422                	ld	s0,8(sp)
    80002a1c:	0141                	addi	sp,sp,16
    80002a1e:	8082                	ret

0000000080002a20 <usertrapret>:
//
// return to user space
//
void
usertrapret(void)
{
    80002a20:	1141                	addi	sp,sp,-16
    80002a22:	e406                	sd	ra,8(sp)
    80002a24:	e022                	sd	s0,0(sp)
    80002a26:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    80002a28:	fffff097          	auipc	ra,0xfffff
    80002a2c:	15c080e7          	jalr	348(ra) # 80001b84 <myproc>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002a30:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80002a34:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002a36:	10079073          	csrw	sstatus,a5
  // kerneltrap() to usertrap(), so turn off interrupts until
  // we're back in user space, where usertrap() is correct.
  intr_off();

  // send syscalls, interrupts, and exceptions to trampoline.S
  w_stvec(TRAMPOLINE + (uservec - trampoline));
    80002a3a:	00004617          	auipc	a2,0x4
    80002a3e:	5c660613          	addi	a2,a2,1478 # 80007000 <_trampoline>
    80002a42:	00004697          	auipc	a3,0x4
    80002a46:	5be68693          	addi	a3,a3,1470 # 80007000 <_trampoline>
    80002a4a:	8e91                	sub	a3,a3,a2
    80002a4c:	040007b7          	lui	a5,0x4000
    80002a50:	17fd                	addi	a5,a5,-1
    80002a52:	07b2                	slli	a5,a5,0xc
    80002a54:	96be                	add	a3,a3,a5
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002a56:	10569073          	csrw	stvec,a3

  // set up trapframe values that uservec will need when
  // the process next re-enters the kernel.
  p->trapframe->kernel_satp = r_satp();         // kernel page table
    80002a5a:	6d38                	ld	a4,88(a0)
  asm volatile("csrr %0, satp" : "=r" (x) );
    80002a5c:	180026f3          	csrr	a3,satp
    80002a60:	e314                	sd	a3,0(a4)
  p->trapframe->kernel_sp = p->kstack + PGSIZE; // process's kernel stack
    80002a62:	6d38                	ld	a4,88(a0)
    80002a64:	6134                	ld	a3,64(a0)
    80002a66:	6585                	lui	a1,0x1
    80002a68:	96ae                	add	a3,a3,a1
    80002a6a:	e714                	sd	a3,8(a4)
  p->trapframe->kernel_trap = (uint64)usertrap;
    80002a6c:	6d38                	ld	a4,88(a0)
    80002a6e:	00000697          	auipc	a3,0x0
    80002a72:	13868693          	addi	a3,a3,312 # 80002ba6 <usertrap>
    80002a76:	eb14                	sd	a3,16(a4)
  p->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
    80002a78:	6d38                	ld	a4,88(a0)
  asm volatile("mv %0, tp" : "=r" (x) );
    80002a7a:	8692                	mv	a3,tp
    80002a7c:	f314                	sd	a3,32(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002a7e:	100026f3          	csrr	a3,sstatus
  // set up the registers that trampoline.S's sret will use
  // to get to user space.
  
  // set S Previous Privilege mode to User.
  unsigned long x = r_sstatus();
  x &= ~SSTATUS_SPP; // clear SPP to 0 for user mode
    80002a82:	eff6f693          	andi	a3,a3,-257
  x |= SSTATUS_SPIE; // enable interrupts in user mode
    80002a86:	0206e693          	ori	a3,a3,32
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002a8a:	10069073          	csrw	sstatus,a3
  w_sstatus(x);

  // set S Exception Program Counter to the saved user pc.
  w_sepc(p->trapframe->epc);
    80002a8e:	6d38                	ld	a4,88(a0)
  asm volatile("csrw sepc, %0" : : "r" (x));
    80002a90:	6f18                	ld	a4,24(a4)
    80002a92:	14171073          	csrw	sepc,a4

  // tell trampoline.S the user page table to switch to.
  uint64 satp = MAKE_SATP(p->pagetable);
    80002a96:	692c                	ld	a1,80(a0)
    80002a98:	81b1                	srli	a1,a1,0xc

  // jump to trampoline.S at the top of memory, which 
  // switches to the user page table, restores user registers,
  // and switches to user mode with sret.
  uint64 fn = TRAMPOLINE + (userret - trampoline);
    80002a9a:	00004717          	auipc	a4,0x4
    80002a9e:	5f670713          	addi	a4,a4,1526 # 80007090 <userret>
    80002aa2:	8f11                	sub	a4,a4,a2
    80002aa4:	97ba                	add	a5,a5,a4
  ((void (*)(uint64,uint64))fn)(TRAPFRAME, satp);
    80002aa6:	577d                	li	a4,-1
    80002aa8:	177e                	slli	a4,a4,0x3f
    80002aaa:	8dd9                	or	a1,a1,a4
    80002aac:	02000537          	lui	a0,0x2000
    80002ab0:	157d                	addi	a0,a0,-1
    80002ab2:	0536                	slli	a0,a0,0xd
    80002ab4:	9782                	jalr	a5
}
    80002ab6:	60a2                	ld	ra,8(sp)
    80002ab8:	6402                	ld	s0,0(sp)
    80002aba:	0141                	addi	sp,sp,16
    80002abc:	8082                	ret

0000000080002abe <clockintr>:
  w_sstatus(sstatus);
}

void
clockintr()
{
    80002abe:	1101                	addi	sp,sp,-32
    80002ac0:	ec06                	sd	ra,24(sp)
    80002ac2:	e822                	sd	s0,16(sp)
    80002ac4:	e426                	sd	s1,8(sp)
    80002ac6:	1000                	addi	s0,sp,32
  acquire(&tickslock);
    80002ac8:	00015497          	auipc	s1,0x15
    80002acc:	a9848493          	addi	s1,s1,-1384 # 80017560 <tickslock>
    80002ad0:	8526                	mv	a0,s1
    80002ad2:	ffffe097          	auipc	ra,0xffffe
    80002ad6:	112080e7          	jalr	274(ra) # 80000be4 <acquire>
  ticks++;
    80002ada:	00006517          	auipc	a0,0x6
    80002ade:	56e50513          	addi	a0,a0,1390 # 80009048 <ticks>
    80002ae2:	411c                	lw	a5,0(a0)
    80002ae4:	2785                	addiw	a5,a5,1
    80002ae6:	c11c                	sw	a5,0(a0)
  wakeup(&ticks);
    80002ae8:	00000097          	auipc	ra,0x0
    80002aec:	a3a080e7          	jalr	-1478(ra) # 80002522 <wakeup>
  release(&tickslock);
    80002af0:	8526                	mv	a0,s1
    80002af2:	ffffe097          	auipc	ra,0xffffe
    80002af6:	1a6080e7          	jalr	422(ra) # 80000c98 <release>
}
    80002afa:	60e2                	ld	ra,24(sp)
    80002afc:	6442                	ld	s0,16(sp)
    80002afe:	64a2                	ld	s1,8(sp)
    80002b00:	6105                	addi	sp,sp,32
    80002b02:	8082                	ret

0000000080002b04 <devintr>:
// returns 2 if timer interrupt,
// 1 if other device,
// 0 if not recognized.
int
devintr()
{
    80002b04:	1101                	addi	sp,sp,-32
    80002b06:	ec06                	sd	ra,24(sp)
    80002b08:	e822                	sd	s0,16(sp)
    80002b0a:	e426                	sd	s1,8(sp)
    80002b0c:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002b0e:	14202773          	csrr	a4,scause
  uint64 scause = r_scause();

  if((scause & 0x8000000000000000L) &&
    80002b12:	00074d63          	bltz	a4,80002b2c <devintr+0x28>
    // now allowed to interrupt again.
    if(irq)
      plic_complete(irq);

    return 1;
  } else if(scause == 0x8000000000000001L){
    80002b16:	57fd                	li	a5,-1
    80002b18:	17fe                	slli	a5,a5,0x3f
    80002b1a:	0785                	addi	a5,a5,1
    // the SSIP bit in sip.
    w_sip(r_sip() & ~2);

    return 2;
  } else {
    return 0;
    80002b1c:	4501                	li	a0,0
  } else if(scause == 0x8000000000000001L){
    80002b1e:	06f70363          	beq	a4,a5,80002b84 <devintr+0x80>
  }
}
    80002b22:	60e2                	ld	ra,24(sp)
    80002b24:	6442                	ld	s0,16(sp)
    80002b26:	64a2                	ld	s1,8(sp)
    80002b28:	6105                	addi	sp,sp,32
    80002b2a:	8082                	ret
     (scause & 0xff) == 9){
    80002b2c:	0ff77793          	andi	a5,a4,255
  if((scause & 0x8000000000000000L) &&
    80002b30:	46a5                	li	a3,9
    80002b32:	fed792e3          	bne	a5,a3,80002b16 <devintr+0x12>
    int irq = plic_claim();
    80002b36:	00003097          	auipc	ra,0x3
    80002b3a:	4b2080e7          	jalr	1202(ra) # 80005fe8 <plic_claim>
    80002b3e:	84aa                	mv	s1,a0
    if(irq == UART0_IRQ){
    80002b40:	47a9                	li	a5,10
    80002b42:	02f50763          	beq	a0,a5,80002b70 <devintr+0x6c>
    } else if(irq == VIRTIO0_IRQ){
    80002b46:	4785                	li	a5,1
    80002b48:	02f50963          	beq	a0,a5,80002b7a <devintr+0x76>
    return 1;
    80002b4c:	4505                	li	a0,1
    } else if(irq){
    80002b4e:	d8f1                	beqz	s1,80002b22 <devintr+0x1e>
      printf("unexpected interrupt irq=%d\n", irq);
    80002b50:	85a6                	mv	a1,s1
    80002b52:	00005517          	auipc	a0,0x5
    80002b56:	7ae50513          	addi	a0,a0,1966 # 80008300 <states.1779+0x38>
    80002b5a:	ffffe097          	auipc	ra,0xffffe
    80002b5e:	a2e080e7          	jalr	-1490(ra) # 80000588 <printf>
      plic_complete(irq);
    80002b62:	8526                	mv	a0,s1
    80002b64:	00003097          	auipc	ra,0x3
    80002b68:	4a8080e7          	jalr	1192(ra) # 8000600c <plic_complete>
    return 1;
    80002b6c:	4505                	li	a0,1
    80002b6e:	bf55                	j	80002b22 <devintr+0x1e>
      uartintr();
    80002b70:	ffffe097          	auipc	ra,0xffffe
    80002b74:	e38080e7          	jalr	-456(ra) # 800009a8 <uartintr>
    80002b78:	b7ed                	j	80002b62 <devintr+0x5e>
      virtio_disk_intr();
    80002b7a:	00004097          	auipc	ra,0x4
    80002b7e:	972080e7          	jalr	-1678(ra) # 800064ec <virtio_disk_intr>
    80002b82:	b7c5                	j	80002b62 <devintr+0x5e>
    if(cpuid() == 0){
    80002b84:	fffff097          	auipc	ra,0xfffff
    80002b88:	fcc080e7          	jalr	-52(ra) # 80001b50 <cpuid>
    80002b8c:	c901                	beqz	a0,80002b9c <devintr+0x98>
  asm volatile("csrr %0, sip" : "=r" (x) );
    80002b8e:	144027f3          	csrr	a5,sip
    w_sip(r_sip() & ~2);
    80002b92:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sip, %0" : : "r" (x));
    80002b94:	14479073          	csrw	sip,a5
    return 2;
    80002b98:	4509                	li	a0,2
    80002b9a:	b761                	j	80002b22 <devintr+0x1e>
      clockintr();
    80002b9c:	00000097          	auipc	ra,0x0
    80002ba0:	f22080e7          	jalr	-222(ra) # 80002abe <clockintr>
    80002ba4:	b7ed                	j	80002b8e <devintr+0x8a>

0000000080002ba6 <usertrap>:
{
    80002ba6:	1101                	addi	sp,sp,-32
    80002ba8:	ec06                	sd	ra,24(sp)
    80002baa:	e822                	sd	s0,16(sp)
    80002bac:	e426                	sd	s1,8(sp)
    80002bae:	e04a                	sd	s2,0(sp)
    80002bb0:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002bb2:	100027f3          	csrr	a5,sstatus
  if((r_sstatus() & SSTATUS_SPP) != 0)
    80002bb6:	1007f793          	andi	a5,a5,256
    80002bba:	e3ad                	bnez	a5,80002c1c <usertrap+0x76>
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002bbc:	00003797          	auipc	a5,0x3
    80002bc0:	32478793          	addi	a5,a5,804 # 80005ee0 <kernelvec>
    80002bc4:	10579073          	csrw	stvec,a5
  struct proc *p = myproc();
    80002bc8:	fffff097          	auipc	ra,0xfffff
    80002bcc:	fbc080e7          	jalr	-68(ra) # 80001b84 <myproc>
    80002bd0:	84aa                	mv	s1,a0
  p->trapframe->epc = r_sepc();
    80002bd2:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002bd4:	14102773          	csrr	a4,sepc
    80002bd8:	ef98                	sd	a4,24(a5)
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002bda:	14202773          	csrr	a4,scause
  if(r_scause() == 8){
    80002bde:	47a1                	li	a5,8
    80002be0:	04f71c63          	bne	a4,a5,80002c38 <usertrap+0x92>
    if(p->killed)
    80002be4:	551c                	lw	a5,40(a0)
    80002be6:	e3b9                	bnez	a5,80002c2c <usertrap+0x86>
    p->trapframe->epc += 4;
    80002be8:	6cb8                	ld	a4,88(s1)
    80002bea:	6f1c                	ld	a5,24(a4)
    80002bec:	0791                	addi	a5,a5,4
    80002bee:	ef1c                	sd	a5,24(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002bf0:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002bf4:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002bf8:	10079073          	csrw	sstatus,a5
    syscall();
    80002bfc:	00000097          	auipc	ra,0x0
    80002c00:	2e0080e7          	jalr	736(ra) # 80002edc <syscall>
  if(p->killed)
    80002c04:	549c                	lw	a5,40(s1)
    80002c06:	ebc1                	bnez	a5,80002c96 <usertrap+0xf0>
  usertrapret();
    80002c08:	00000097          	auipc	ra,0x0
    80002c0c:	e18080e7          	jalr	-488(ra) # 80002a20 <usertrapret>
}
    80002c10:	60e2                	ld	ra,24(sp)
    80002c12:	6442                	ld	s0,16(sp)
    80002c14:	64a2                	ld	s1,8(sp)
    80002c16:	6902                	ld	s2,0(sp)
    80002c18:	6105                	addi	sp,sp,32
    80002c1a:	8082                	ret
    panic("usertrap: not from user mode");
    80002c1c:	00005517          	auipc	a0,0x5
    80002c20:	70450513          	addi	a0,a0,1796 # 80008320 <states.1779+0x58>
    80002c24:	ffffe097          	auipc	ra,0xffffe
    80002c28:	91a080e7          	jalr	-1766(ra) # 8000053e <panic>
      exit(-1);
    80002c2c:	557d                	li	a0,-1
    80002c2e:	00000097          	auipc	ra,0x0
    80002c32:	a26080e7          	jalr	-1498(ra) # 80002654 <exit>
    80002c36:	bf4d                	j	80002be8 <usertrap+0x42>
  } else if((which_dev = devintr()) != 0){
    80002c38:	00000097          	auipc	ra,0x0
    80002c3c:	ecc080e7          	jalr	-308(ra) # 80002b04 <devintr>
    80002c40:	892a                	mv	s2,a0
    80002c42:	c501                	beqz	a0,80002c4a <usertrap+0xa4>
  if(p->killed)
    80002c44:	549c                	lw	a5,40(s1)
    80002c46:	c3a1                	beqz	a5,80002c86 <usertrap+0xe0>
    80002c48:	a815                	j	80002c7c <usertrap+0xd6>
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002c4a:	142025f3          	csrr	a1,scause
    printf("usertrap(): unexpected scause %p pid=%d\n", r_scause(), p->pid);
    80002c4e:	5890                	lw	a2,48(s1)
    80002c50:	00005517          	auipc	a0,0x5
    80002c54:	6f050513          	addi	a0,a0,1776 # 80008340 <states.1779+0x78>
    80002c58:	ffffe097          	auipc	ra,0xffffe
    80002c5c:	930080e7          	jalr	-1744(ra) # 80000588 <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002c60:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002c64:	14302673          	csrr	a2,stval
    printf("            sepc=%p stval=%p\n", r_sepc(), r_stval());
    80002c68:	00005517          	auipc	a0,0x5
    80002c6c:	70850513          	addi	a0,a0,1800 # 80008370 <states.1779+0xa8>
    80002c70:	ffffe097          	auipc	ra,0xffffe
    80002c74:	918080e7          	jalr	-1768(ra) # 80000588 <printf>
    p->killed = 1;
    80002c78:	4785                	li	a5,1
    80002c7a:	d49c                	sw	a5,40(s1)
    exit(-1);
    80002c7c:	557d                	li	a0,-1
    80002c7e:	00000097          	auipc	ra,0x0
    80002c82:	9d6080e7          	jalr	-1578(ra) # 80002654 <exit>
  if(which_dev == 2)
    80002c86:	4789                	li	a5,2
    80002c88:	f8f910e3          	bne	s2,a5,80002c08 <usertrap+0x62>
    yield();
    80002c8c:	fffff097          	auipc	ra,0xfffff
    80002c90:	694080e7          	jalr	1684(ra) # 80002320 <yield>
    80002c94:	bf95                	j	80002c08 <usertrap+0x62>
  int which_dev = 0;
    80002c96:	4901                	li	s2,0
    80002c98:	b7d5                	j	80002c7c <usertrap+0xd6>

0000000080002c9a <kerneltrap>:
{
    80002c9a:	7179                	addi	sp,sp,-48
    80002c9c:	f406                	sd	ra,40(sp)
    80002c9e:	f022                	sd	s0,32(sp)
    80002ca0:	ec26                	sd	s1,24(sp)
    80002ca2:	e84a                	sd	s2,16(sp)
    80002ca4:	e44e                	sd	s3,8(sp)
    80002ca6:	1800                	addi	s0,sp,48
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002ca8:	14102973          	csrr	s2,sepc
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002cac:	100024f3          	csrr	s1,sstatus
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002cb0:	142029f3          	csrr	s3,scause
  if((sstatus & SSTATUS_SPP) == 0)
    80002cb4:	1004f793          	andi	a5,s1,256
    80002cb8:	cb85                	beqz	a5,80002ce8 <kerneltrap+0x4e>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002cba:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80002cbe:	8b89                	andi	a5,a5,2
  if(intr_get() != 0)
    80002cc0:	ef85                	bnez	a5,80002cf8 <kerneltrap+0x5e>
  if((which_dev = devintr()) == 0){
    80002cc2:	00000097          	auipc	ra,0x0
    80002cc6:	e42080e7          	jalr	-446(ra) # 80002b04 <devintr>
    80002cca:	cd1d                	beqz	a0,80002d08 <kerneltrap+0x6e>
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
    80002ccc:	4789                	li	a5,2
    80002cce:	06f50a63          	beq	a0,a5,80002d42 <kerneltrap+0xa8>
  asm volatile("csrw sepc, %0" : : "r" (x));
    80002cd2:	14191073          	csrw	sepc,s2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002cd6:	10049073          	csrw	sstatus,s1
}
    80002cda:	70a2                	ld	ra,40(sp)
    80002cdc:	7402                	ld	s0,32(sp)
    80002cde:	64e2                	ld	s1,24(sp)
    80002ce0:	6942                	ld	s2,16(sp)
    80002ce2:	69a2                	ld	s3,8(sp)
    80002ce4:	6145                	addi	sp,sp,48
    80002ce6:	8082                	ret
    panic("kerneltrap: not from supervisor mode");
    80002ce8:	00005517          	auipc	a0,0x5
    80002cec:	6a850513          	addi	a0,a0,1704 # 80008390 <states.1779+0xc8>
    80002cf0:	ffffe097          	auipc	ra,0xffffe
    80002cf4:	84e080e7          	jalr	-1970(ra) # 8000053e <panic>
    panic("kerneltrap: interrupts enabled");
    80002cf8:	00005517          	auipc	a0,0x5
    80002cfc:	6c050513          	addi	a0,a0,1728 # 800083b8 <states.1779+0xf0>
    80002d00:	ffffe097          	auipc	ra,0xffffe
    80002d04:	83e080e7          	jalr	-1986(ra) # 8000053e <panic>
    printf("scause %p\n", scause);
    80002d08:	85ce                	mv	a1,s3
    80002d0a:	00005517          	auipc	a0,0x5
    80002d0e:	6ce50513          	addi	a0,a0,1742 # 800083d8 <states.1779+0x110>
    80002d12:	ffffe097          	auipc	ra,0xffffe
    80002d16:	876080e7          	jalr	-1930(ra) # 80000588 <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002d1a:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002d1e:	14302673          	csrr	a2,stval
    printf("sepc=%p stval=%p\n", r_sepc(), r_stval());
    80002d22:	00005517          	auipc	a0,0x5
    80002d26:	6c650513          	addi	a0,a0,1734 # 800083e8 <states.1779+0x120>
    80002d2a:	ffffe097          	auipc	ra,0xffffe
    80002d2e:	85e080e7          	jalr	-1954(ra) # 80000588 <printf>
    panic("kerneltrap");
    80002d32:	00005517          	auipc	a0,0x5
    80002d36:	6ce50513          	addi	a0,a0,1742 # 80008400 <states.1779+0x138>
    80002d3a:	ffffe097          	auipc	ra,0xffffe
    80002d3e:	804080e7          	jalr	-2044(ra) # 8000053e <panic>
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
    80002d42:	fffff097          	auipc	ra,0xfffff
    80002d46:	e42080e7          	jalr	-446(ra) # 80001b84 <myproc>
    80002d4a:	d541                	beqz	a0,80002cd2 <kerneltrap+0x38>
    80002d4c:	fffff097          	auipc	ra,0xfffff
    80002d50:	e38080e7          	jalr	-456(ra) # 80001b84 <myproc>
    80002d54:	4d18                	lw	a4,24(a0)
    80002d56:	4791                	li	a5,4
    80002d58:	f6f71de3          	bne	a4,a5,80002cd2 <kerneltrap+0x38>
    yield();
    80002d5c:	fffff097          	auipc	ra,0xfffff
    80002d60:	5c4080e7          	jalr	1476(ra) # 80002320 <yield>
    80002d64:	b7bd                	j	80002cd2 <kerneltrap+0x38>

0000000080002d66 <argraw>:
  return strlen(buf);
}

static uint64
argraw(int n)
{
    80002d66:	1101                	addi	sp,sp,-32
    80002d68:	ec06                	sd	ra,24(sp)
    80002d6a:	e822                	sd	s0,16(sp)
    80002d6c:	e426                	sd	s1,8(sp)
    80002d6e:	1000                	addi	s0,sp,32
    80002d70:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80002d72:	fffff097          	auipc	ra,0xfffff
    80002d76:	e12080e7          	jalr	-494(ra) # 80001b84 <myproc>
  switch (n) {
    80002d7a:	4795                	li	a5,5
    80002d7c:	0497e163          	bltu	a5,s1,80002dbe <argraw+0x58>
    80002d80:	048a                	slli	s1,s1,0x2
    80002d82:	00005717          	auipc	a4,0x5
    80002d86:	6b670713          	addi	a4,a4,1718 # 80008438 <states.1779+0x170>
    80002d8a:	94ba                	add	s1,s1,a4
    80002d8c:	409c                	lw	a5,0(s1)
    80002d8e:	97ba                	add	a5,a5,a4
    80002d90:	8782                	jr	a5
  case 0:
    return p->trapframe->a0;
    80002d92:	6d3c                	ld	a5,88(a0)
    80002d94:	7ba8                	ld	a0,112(a5)
  case 5:
    return p->trapframe->a5;
  }
  panic("argraw");
  return -1;
}
    80002d96:	60e2                	ld	ra,24(sp)
    80002d98:	6442                	ld	s0,16(sp)
    80002d9a:	64a2                	ld	s1,8(sp)
    80002d9c:	6105                	addi	sp,sp,32
    80002d9e:	8082                	ret
    return p->trapframe->a1;
    80002da0:	6d3c                	ld	a5,88(a0)
    80002da2:	7fa8                	ld	a0,120(a5)
    80002da4:	bfcd                	j	80002d96 <argraw+0x30>
    return p->trapframe->a2;
    80002da6:	6d3c                	ld	a5,88(a0)
    80002da8:	63c8                	ld	a0,128(a5)
    80002daa:	b7f5                	j	80002d96 <argraw+0x30>
    return p->trapframe->a3;
    80002dac:	6d3c                	ld	a5,88(a0)
    80002dae:	67c8                	ld	a0,136(a5)
    80002db0:	b7dd                	j	80002d96 <argraw+0x30>
    return p->trapframe->a4;
    80002db2:	6d3c                	ld	a5,88(a0)
    80002db4:	6bc8                	ld	a0,144(a5)
    80002db6:	b7c5                	j	80002d96 <argraw+0x30>
    return p->trapframe->a5;
    80002db8:	6d3c                	ld	a5,88(a0)
    80002dba:	6fc8                	ld	a0,152(a5)
    80002dbc:	bfe9                	j	80002d96 <argraw+0x30>
  panic("argraw");
    80002dbe:	00005517          	auipc	a0,0x5
    80002dc2:	65250513          	addi	a0,a0,1618 # 80008410 <states.1779+0x148>
    80002dc6:	ffffd097          	auipc	ra,0xffffd
    80002dca:	778080e7          	jalr	1912(ra) # 8000053e <panic>

0000000080002dce <fetchaddr>:
{
    80002dce:	1101                	addi	sp,sp,-32
    80002dd0:	ec06                	sd	ra,24(sp)
    80002dd2:	e822                	sd	s0,16(sp)
    80002dd4:	e426                	sd	s1,8(sp)
    80002dd6:	e04a                	sd	s2,0(sp)
    80002dd8:	1000                	addi	s0,sp,32
    80002dda:	84aa                	mv	s1,a0
    80002ddc:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80002dde:	fffff097          	auipc	ra,0xfffff
    80002de2:	da6080e7          	jalr	-602(ra) # 80001b84 <myproc>
  if(addr >= p->sz || addr+sizeof(uint64) > p->sz)
    80002de6:	653c                	ld	a5,72(a0)
    80002de8:	02f4f863          	bgeu	s1,a5,80002e18 <fetchaddr+0x4a>
    80002dec:	00848713          	addi	a4,s1,8
    80002df0:	02e7e663          	bltu	a5,a4,80002e1c <fetchaddr+0x4e>
  if(copyin(p->pagetable, (char *)ip, addr, sizeof(*ip)) != 0)
    80002df4:	46a1                	li	a3,8
    80002df6:	8626                	mv	a2,s1
    80002df8:	85ca                	mv	a1,s2
    80002dfa:	6928                	ld	a0,80(a0)
    80002dfc:	fffff097          	auipc	ra,0xfffff
    80002e00:	902080e7          	jalr	-1790(ra) # 800016fe <copyin>
    80002e04:	00a03533          	snez	a0,a0
    80002e08:	40a00533          	neg	a0,a0
}
    80002e0c:	60e2                	ld	ra,24(sp)
    80002e0e:	6442                	ld	s0,16(sp)
    80002e10:	64a2                	ld	s1,8(sp)
    80002e12:	6902                	ld	s2,0(sp)
    80002e14:	6105                	addi	sp,sp,32
    80002e16:	8082                	ret
    return -1;
    80002e18:	557d                	li	a0,-1
    80002e1a:	bfcd                	j	80002e0c <fetchaddr+0x3e>
    80002e1c:	557d                	li	a0,-1
    80002e1e:	b7fd                	j	80002e0c <fetchaddr+0x3e>

0000000080002e20 <fetchstr>:
{
    80002e20:	7179                	addi	sp,sp,-48
    80002e22:	f406                	sd	ra,40(sp)
    80002e24:	f022                	sd	s0,32(sp)
    80002e26:	ec26                	sd	s1,24(sp)
    80002e28:	e84a                	sd	s2,16(sp)
    80002e2a:	e44e                	sd	s3,8(sp)
    80002e2c:	1800                	addi	s0,sp,48
    80002e2e:	892a                	mv	s2,a0
    80002e30:	84ae                	mv	s1,a1
    80002e32:	89b2                	mv	s3,a2
  struct proc *p = myproc();
    80002e34:	fffff097          	auipc	ra,0xfffff
    80002e38:	d50080e7          	jalr	-688(ra) # 80001b84 <myproc>
  int err = copyinstr(p->pagetable, buf, addr, max);
    80002e3c:	86ce                	mv	a3,s3
    80002e3e:	864a                	mv	a2,s2
    80002e40:	85a6                	mv	a1,s1
    80002e42:	6928                	ld	a0,80(a0)
    80002e44:	fffff097          	auipc	ra,0xfffff
    80002e48:	946080e7          	jalr	-1722(ra) # 8000178a <copyinstr>
  if(err < 0)
    80002e4c:	00054763          	bltz	a0,80002e5a <fetchstr+0x3a>
  return strlen(buf);
    80002e50:	8526                	mv	a0,s1
    80002e52:	ffffe097          	auipc	ra,0xffffe
    80002e56:	012080e7          	jalr	18(ra) # 80000e64 <strlen>
}
    80002e5a:	70a2                	ld	ra,40(sp)
    80002e5c:	7402                	ld	s0,32(sp)
    80002e5e:	64e2                	ld	s1,24(sp)
    80002e60:	6942                	ld	s2,16(sp)
    80002e62:	69a2                	ld	s3,8(sp)
    80002e64:	6145                	addi	sp,sp,48
    80002e66:	8082                	ret

0000000080002e68 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
    80002e68:	1101                	addi	sp,sp,-32
    80002e6a:	ec06                	sd	ra,24(sp)
    80002e6c:	e822                	sd	s0,16(sp)
    80002e6e:	e426                	sd	s1,8(sp)
    80002e70:	1000                	addi	s0,sp,32
    80002e72:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002e74:	00000097          	auipc	ra,0x0
    80002e78:	ef2080e7          	jalr	-270(ra) # 80002d66 <argraw>
    80002e7c:	c088                	sw	a0,0(s1)
  return 0;
}
    80002e7e:	4501                	li	a0,0
    80002e80:	60e2                	ld	ra,24(sp)
    80002e82:	6442                	ld	s0,16(sp)
    80002e84:	64a2                	ld	s1,8(sp)
    80002e86:	6105                	addi	sp,sp,32
    80002e88:	8082                	ret

0000000080002e8a <argaddr>:
// Retrieve an argument as a pointer.
// Doesn't check for legality, since
// copyin/copyout will do that.
int
argaddr(int n, uint64 *ip)
{
    80002e8a:	1101                	addi	sp,sp,-32
    80002e8c:	ec06                	sd	ra,24(sp)
    80002e8e:	e822                	sd	s0,16(sp)
    80002e90:	e426                	sd	s1,8(sp)
    80002e92:	1000                	addi	s0,sp,32
    80002e94:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002e96:	00000097          	auipc	ra,0x0
    80002e9a:	ed0080e7          	jalr	-304(ra) # 80002d66 <argraw>
    80002e9e:	e088                	sd	a0,0(s1)
  return 0;
}
    80002ea0:	4501                	li	a0,0
    80002ea2:	60e2                	ld	ra,24(sp)
    80002ea4:	6442                	ld	s0,16(sp)
    80002ea6:	64a2                	ld	s1,8(sp)
    80002ea8:	6105                	addi	sp,sp,32
    80002eaa:	8082                	ret

0000000080002eac <argstr>:
// Fetch the nth word-sized system call argument as a null-terminated string.
// Copies into buf, at most max.
// Returns string length if OK (including nul), -1 if error.
int
argstr(int n, char *buf, int max)
{
    80002eac:	1101                	addi	sp,sp,-32
    80002eae:	ec06                	sd	ra,24(sp)
    80002eb0:	e822                	sd	s0,16(sp)
    80002eb2:	e426                	sd	s1,8(sp)
    80002eb4:	e04a                	sd	s2,0(sp)
    80002eb6:	1000                	addi	s0,sp,32
    80002eb8:	84ae                	mv	s1,a1
    80002eba:	8932                	mv	s2,a2
  *ip = argraw(n);
    80002ebc:	00000097          	auipc	ra,0x0
    80002ec0:	eaa080e7          	jalr	-342(ra) # 80002d66 <argraw>
  uint64 addr;
  if(argaddr(n, &addr) < 0)
    return -1;
  return fetchstr(addr, buf, max);
    80002ec4:	864a                	mv	a2,s2
    80002ec6:	85a6                	mv	a1,s1
    80002ec8:	00000097          	auipc	ra,0x0
    80002ecc:	f58080e7          	jalr	-168(ra) # 80002e20 <fetchstr>
}
    80002ed0:	60e2                	ld	ra,24(sp)
    80002ed2:	6442                	ld	s0,16(sp)
    80002ed4:	64a2                	ld	s1,8(sp)
    80002ed6:	6902                	ld	s2,0(sp)
    80002ed8:	6105                	addi	sp,sp,32
    80002eda:	8082                	ret

0000000080002edc <syscall>:
[SYS_get_cpu] sys_get_cpu,
};

void
syscall(void)
{
    80002edc:	1101                	addi	sp,sp,-32
    80002ede:	ec06                	sd	ra,24(sp)
    80002ee0:	e822                	sd	s0,16(sp)
    80002ee2:	e426                	sd	s1,8(sp)
    80002ee4:	e04a                	sd	s2,0(sp)
    80002ee6:	1000                	addi	s0,sp,32
  int num;
  struct proc *p = myproc();
    80002ee8:	fffff097          	auipc	ra,0xfffff
    80002eec:	c9c080e7          	jalr	-868(ra) # 80001b84 <myproc>
    80002ef0:	84aa                	mv	s1,a0

  num = p->trapframe->a7;
    80002ef2:	05853903          	ld	s2,88(a0)
    80002ef6:	0a893783          	ld	a5,168(s2)
    80002efa:	0007869b          	sext.w	a3,a5
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    80002efe:	37fd                	addiw	a5,a5,-1
    80002f00:	4759                	li	a4,22
    80002f02:	00f76f63          	bltu	a4,a5,80002f20 <syscall+0x44>
    80002f06:	00369713          	slli	a4,a3,0x3
    80002f0a:	00005797          	auipc	a5,0x5
    80002f0e:	54678793          	addi	a5,a5,1350 # 80008450 <syscalls>
    80002f12:	97ba                	add	a5,a5,a4
    80002f14:	639c                	ld	a5,0(a5)
    80002f16:	c789                	beqz	a5,80002f20 <syscall+0x44>
    p->trapframe->a0 = syscalls[num]();
    80002f18:	9782                	jalr	a5
    80002f1a:	06a93823          	sd	a0,112(s2)
    80002f1e:	a839                	j	80002f3c <syscall+0x60>
  } else {
    printf("%d %s: unknown sys call %d\n",
    80002f20:	15848613          	addi	a2,s1,344
    80002f24:	588c                	lw	a1,48(s1)
    80002f26:	00005517          	auipc	a0,0x5
    80002f2a:	4f250513          	addi	a0,a0,1266 # 80008418 <states.1779+0x150>
    80002f2e:	ffffd097          	auipc	ra,0xffffd
    80002f32:	65a080e7          	jalr	1626(ra) # 80000588 <printf>
            p->pid, p->name, num);
    p->trapframe->a0 = -1;
    80002f36:	6cbc                	ld	a5,88(s1)
    80002f38:	577d                	li	a4,-1
    80002f3a:	fbb8                	sd	a4,112(a5)
  }
}
    80002f3c:	60e2                	ld	ra,24(sp)
    80002f3e:	6442                	ld	s0,16(sp)
    80002f40:	64a2                	ld	s1,8(sp)
    80002f42:	6902                	ld	s2,0(sp)
    80002f44:	6105                	addi	sp,sp,32
    80002f46:	8082                	ret

0000000080002f48 <sys_exit>:
#include "spinlock.h"
#include "proc.h"

uint64
sys_exit(void)
{
    80002f48:	1101                	addi	sp,sp,-32
    80002f4a:	ec06                	sd	ra,24(sp)
    80002f4c:	e822                	sd	s0,16(sp)
    80002f4e:	1000                	addi	s0,sp,32
  int n;
  if(argint(0, &n) < 0)
    80002f50:	fec40593          	addi	a1,s0,-20
    80002f54:	4501                	li	a0,0
    80002f56:	00000097          	auipc	ra,0x0
    80002f5a:	f12080e7          	jalr	-238(ra) # 80002e68 <argint>
    return -1;
    80002f5e:	57fd                	li	a5,-1
  if(argint(0, &n) < 0)
    80002f60:	00054963          	bltz	a0,80002f72 <sys_exit+0x2a>
  exit(n);
    80002f64:	fec42503          	lw	a0,-20(s0)
    80002f68:	fffff097          	auipc	ra,0xfffff
    80002f6c:	6ec080e7          	jalr	1772(ra) # 80002654 <exit>
  return 0;  // not reached
    80002f70:	4781                	li	a5,0
}
    80002f72:	853e                	mv	a0,a5
    80002f74:	60e2                	ld	ra,24(sp)
    80002f76:	6442                	ld	s0,16(sp)
    80002f78:	6105                	addi	sp,sp,32
    80002f7a:	8082                	ret

0000000080002f7c <sys_getpid>:

uint64
sys_getpid(void)
{
    80002f7c:	1141                	addi	sp,sp,-16
    80002f7e:	e406                	sd	ra,8(sp)
    80002f80:	e022                	sd	s0,0(sp)
    80002f82:	0800                	addi	s0,sp,16
  return myproc()->pid;
    80002f84:	fffff097          	auipc	ra,0xfffff
    80002f88:	c00080e7          	jalr	-1024(ra) # 80001b84 <myproc>
}
    80002f8c:	5908                	lw	a0,48(a0)
    80002f8e:	60a2                	ld	ra,8(sp)
    80002f90:	6402                	ld	s0,0(sp)
    80002f92:	0141                	addi	sp,sp,16
    80002f94:	8082                	ret

0000000080002f96 <sys_fork>:

uint64
sys_fork(void)
{
    80002f96:	1141                	addi	sp,sp,-16
    80002f98:	e406                	sd	ra,8(sp)
    80002f9a:	e022                	sd	s0,0(sp)
    80002f9c:	0800                	addi	s0,sp,16
  return fork();
    80002f9e:	fffff097          	auipc	ra,0xfffff
    80002fa2:	078080e7          	jalr	120(ra) # 80002016 <fork>
}
    80002fa6:	60a2                	ld	ra,8(sp)
    80002fa8:	6402                	ld	s0,0(sp)
    80002faa:	0141                	addi	sp,sp,16
    80002fac:	8082                	ret

0000000080002fae <sys_wait>:

uint64
sys_wait(void)
{
    80002fae:	1101                	addi	sp,sp,-32
    80002fb0:	ec06                	sd	ra,24(sp)
    80002fb2:	e822                	sd	s0,16(sp)
    80002fb4:	1000                	addi	s0,sp,32
  uint64 p;
  if(argaddr(0, &p) < 0)
    80002fb6:	fe840593          	addi	a1,s0,-24
    80002fba:	4501                	li	a0,0
    80002fbc:	00000097          	auipc	ra,0x0
    80002fc0:	ece080e7          	jalr	-306(ra) # 80002e8a <argaddr>
    80002fc4:	87aa                	mv	a5,a0
    return -1;
    80002fc6:	557d                	li	a0,-1
  if(argaddr(0, &p) < 0)
    80002fc8:	0007c863          	bltz	a5,80002fd8 <sys_wait+0x2a>
  return wait(p);
    80002fcc:	fe843503          	ld	a0,-24(s0)
    80002fd0:	fffff097          	auipc	ra,0xfffff
    80002fd4:	42a080e7          	jalr	1066(ra) # 800023fa <wait>
}
    80002fd8:	60e2                	ld	ra,24(sp)
    80002fda:	6442                	ld	s0,16(sp)
    80002fdc:	6105                	addi	sp,sp,32
    80002fde:	8082                	ret

0000000080002fe0 <sys_sbrk>:

uint64
sys_sbrk(void)
{
    80002fe0:	7179                	addi	sp,sp,-48
    80002fe2:	f406                	sd	ra,40(sp)
    80002fe4:	f022                	sd	s0,32(sp)
    80002fe6:	ec26                	sd	s1,24(sp)
    80002fe8:	1800                	addi	s0,sp,48
  int addr;
  int n;

  if(argint(0, &n) < 0)
    80002fea:	fdc40593          	addi	a1,s0,-36
    80002fee:	4501                	li	a0,0
    80002ff0:	00000097          	auipc	ra,0x0
    80002ff4:	e78080e7          	jalr	-392(ra) # 80002e68 <argint>
    80002ff8:	87aa                	mv	a5,a0
    return -1;
    80002ffa:	557d                	li	a0,-1
  if(argint(0, &n) < 0)
    80002ffc:	0207c063          	bltz	a5,8000301c <sys_sbrk+0x3c>
  addr = myproc()->sz;
    80003000:	fffff097          	auipc	ra,0xfffff
    80003004:	b84080e7          	jalr	-1148(ra) # 80001b84 <myproc>
    80003008:	4524                	lw	s1,72(a0)
  if(growproc(n) < 0)
    8000300a:	fdc42503          	lw	a0,-36(s0)
    8000300e:	fffff097          	auipc	ra,0xfffff
    80003012:	f94080e7          	jalr	-108(ra) # 80001fa2 <growproc>
    80003016:	00054863          	bltz	a0,80003026 <sys_sbrk+0x46>
    return -1;
  return addr;
    8000301a:	8526                	mv	a0,s1
}
    8000301c:	70a2                	ld	ra,40(sp)
    8000301e:	7402                	ld	s0,32(sp)
    80003020:	64e2                	ld	s1,24(sp)
    80003022:	6145                	addi	sp,sp,48
    80003024:	8082                	ret
    return -1;
    80003026:	557d                	li	a0,-1
    80003028:	bfd5                	j	8000301c <sys_sbrk+0x3c>

000000008000302a <sys_sleep>:

uint64
sys_sleep(void)
{
    8000302a:	7139                	addi	sp,sp,-64
    8000302c:	fc06                	sd	ra,56(sp)
    8000302e:	f822                	sd	s0,48(sp)
    80003030:	f426                	sd	s1,40(sp)
    80003032:	f04a                	sd	s2,32(sp)
    80003034:	ec4e                	sd	s3,24(sp)
    80003036:	0080                	addi	s0,sp,64
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
    80003038:	fcc40593          	addi	a1,s0,-52
    8000303c:	4501                	li	a0,0
    8000303e:	00000097          	auipc	ra,0x0
    80003042:	e2a080e7          	jalr	-470(ra) # 80002e68 <argint>
    return -1;
    80003046:	57fd                	li	a5,-1
  if(argint(0, &n) < 0)
    80003048:	06054563          	bltz	a0,800030b2 <sys_sleep+0x88>
  acquire(&tickslock);
    8000304c:	00014517          	auipc	a0,0x14
    80003050:	51450513          	addi	a0,a0,1300 # 80017560 <tickslock>
    80003054:	ffffe097          	auipc	ra,0xffffe
    80003058:	b90080e7          	jalr	-1136(ra) # 80000be4 <acquire>
  ticks0 = ticks;
    8000305c:	00006917          	auipc	s2,0x6
    80003060:	fec92903          	lw	s2,-20(s2) # 80009048 <ticks>
  while(ticks - ticks0 < n){
    80003064:	fcc42783          	lw	a5,-52(s0)
    80003068:	cf85                	beqz	a5,800030a0 <sys_sleep+0x76>
    if(myproc()->killed){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
    8000306a:	00014997          	auipc	s3,0x14
    8000306e:	4f698993          	addi	s3,s3,1270 # 80017560 <tickslock>
    80003072:	00006497          	auipc	s1,0x6
    80003076:	fd648493          	addi	s1,s1,-42 # 80009048 <ticks>
    if(myproc()->killed){
    8000307a:	fffff097          	auipc	ra,0xfffff
    8000307e:	b0a080e7          	jalr	-1270(ra) # 80001b84 <myproc>
    80003082:	551c                	lw	a5,40(a0)
    80003084:	ef9d                	bnez	a5,800030c2 <sys_sleep+0x98>
    sleep(&ticks, &tickslock);
    80003086:	85ce                	mv	a1,s3
    80003088:	8526                	mv	a0,s1
    8000308a:	fffff097          	auipc	ra,0xfffff
    8000308e:	2fa080e7          	jalr	762(ra) # 80002384 <sleep>
  while(ticks - ticks0 < n){
    80003092:	409c                	lw	a5,0(s1)
    80003094:	412787bb          	subw	a5,a5,s2
    80003098:	fcc42703          	lw	a4,-52(s0)
    8000309c:	fce7efe3          	bltu	a5,a4,8000307a <sys_sleep+0x50>
  }
  release(&tickslock);
    800030a0:	00014517          	auipc	a0,0x14
    800030a4:	4c050513          	addi	a0,a0,1216 # 80017560 <tickslock>
    800030a8:	ffffe097          	auipc	ra,0xffffe
    800030ac:	bf0080e7          	jalr	-1040(ra) # 80000c98 <release>
  return 0;
    800030b0:	4781                	li	a5,0
}
    800030b2:	853e                	mv	a0,a5
    800030b4:	70e2                	ld	ra,56(sp)
    800030b6:	7442                	ld	s0,48(sp)
    800030b8:	74a2                	ld	s1,40(sp)
    800030ba:	7902                	ld	s2,32(sp)
    800030bc:	69e2                	ld	s3,24(sp)
    800030be:	6121                	addi	sp,sp,64
    800030c0:	8082                	ret
      release(&tickslock);
    800030c2:	00014517          	auipc	a0,0x14
    800030c6:	49e50513          	addi	a0,a0,1182 # 80017560 <tickslock>
    800030ca:	ffffe097          	auipc	ra,0xffffe
    800030ce:	bce080e7          	jalr	-1074(ra) # 80000c98 <release>
      return -1;
    800030d2:	57fd                	li	a5,-1
    800030d4:	bff9                	j	800030b2 <sys_sleep+0x88>

00000000800030d6 <sys_kill>:

uint64
sys_kill(void)
{
    800030d6:	1101                	addi	sp,sp,-32
    800030d8:	ec06                	sd	ra,24(sp)
    800030da:	e822                	sd	s0,16(sp)
    800030dc:	1000                	addi	s0,sp,32
  int pid;

  if(argint(0, &pid) < 0)
    800030de:	fec40593          	addi	a1,s0,-20
    800030e2:	4501                	li	a0,0
    800030e4:	00000097          	auipc	ra,0x0
    800030e8:	d84080e7          	jalr	-636(ra) # 80002e68 <argint>
    800030ec:	87aa                	mv	a5,a0
    return -1;
    800030ee:	557d                	li	a0,-1
  if(argint(0, &pid) < 0)
    800030f0:	0007c863          	bltz	a5,80003100 <sys_kill+0x2a>
  return kill(pid);
    800030f4:	fec42503          	lw	a0,-20(s0)
    800030f8:	fffff097          	auipc	ra,0xfffff
    800030fc:	644080e7          	jalr	1604(ra) # 8000273c <kill>
}
    80003100:	60e2                	ld	ra,24(sp)
    80003102:	6442                	ld	s0,16(sp)
    80003104:	6105                	addi	sp,sp,32
    80003106:	8082                	ret

0000000080003108 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
uint64
sys_uptime(void)
{
    80003108:	1101                	addi	sp,sp,-32
    8000310a:	ec06                	sd	ra,24(sp)
    8000310c:	e822                	sd	s0,16(sp)
    8000310e:	e426                	sd	s1,8(sp)
    80003110:	1000                	addi	s0,sp,32
  uint xticks;

  acquire(&tickslock);
    80003112:	00014517          	auipc	a0,0x14
    80003116:	44e50513          	addi	a0,a0,1102 # 80017560 <tickslock>
    8000311a:	ffffe097          	auipc	ra,0xffffe
    8000311e:	aca080e7          	jalr	-1334(ra) # 80000be4 <acquire>
  xticks = ticks;
    80003122:	00006497          	auipc	s1,0x6
    80003126:	f264a483          	lw	s1,-218(s1) # 80009048 <ticks>
  release(&tickslock);
    8000312a:	00014517          	auipc	a0,0x14
    8000312e:	43650513          	addi	a0,a0,1078 # 80017560 <tickslock>
    80003132:	ffffe097          	auipc	ra,0xffffe
    80003136:	b66080e7          	jalr	-1178(ra) # 80000c98 <release>
  return xticks;
}
    8000313a:	02049513          	slli	a0,s1,0x20
    8000313e:	9101                	srli	a0,a0,0x20
    80003140:	60e2                	ld	ra,24(sp)
    80003142:	6442                	ld	s0,16(sp)
    80003144:	64a2                	ld	s1,8(sp)
    80003146:	6105                	addi	sp,sp,32
    80003148:	8082                	ret

000000008000314a <sys_set_cpu>:

uint64
sys_set_cpu(void)
{
    8000314a:	1101                	addi	sp,sp,-32
    8000314c:	ec06                	sd	ra,24(sp)
    8000314e:	e822                	sd	s0,16(sp)
    80003150:	1000                	addi	s0,sp,32
  int cpu_id;

  if(argint(0, &cpu_id) < 0)
    80003152:	fec40593          	addi	a1,s0,-20
    80003156:	4501                	li	a0,0
    80003158:	00000097          	auipc	ra,0x0
    8000315c:	d10080e7          	jalr	-752(ra) # 80002e68 <argint>
    80003160:	87aa                	mv	a5,a0
    return -1;
    80003162:	557d                	li	a0,-1
  if(argint(0, &cpu_id) < 0)
    80003164:	0007c863          	bltz	a5,80003174 <sys_set_cpu+0x2a>
  return set_cpu(cpu_id);
    80003168:	fec42503          	lw	a0,-20(s0)
    8000316c:	fffff097          	auipc	ra,0xfffff
    80003170:	79c080e7          	jalr	1948(ra) # 80002908 <set_cpu>
}
    80003174:	60e2                	ld	ra,24(sp)
    80003176:	6442                	ld	s0,16(sp)
    80003178:	6105                	addi	sp,sp,32
    8000317a:	8082                	ret

000000008000317c <sys_get_cpu>:

uint64
sys_get_cpu(void)
{
    8000317c:	1141                	addi	sp,sp,-16
    8000317e:	e406                	sd	ra,8(sp)
    80003180:	e022                	sd	s0,0(sp)
    80003182:	0800                	addi	s0,sp,16
  return get_cpu();
    80003184:	fffff097          	auipc	ra,0xfffff
    80003188:	7d6080e7          	jalr	2006(ra) # 8000295a <get_cpu>
}
    8000318c:	60a2                	ld	ra,8(sp)
    8000318e:	6402                	ld	s0,0(sp)
    80003190:	0141                	addi	sp,sp,16
    80003192:	8082                	ret

0000000080003194 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
    80003194:	7179                	addi	sp,sp,-48
    80003196:	f406                	sd	ra,40(sp)
    80003198:	f022                	sd	s0,32(sp)
    8000319a:	ec26                	sd	s1,24(sp)
    8000319c:	e84a                	sd	s2,16(sp)
    8000319e:	e44e                	sd	s3,8(sp)
    800031a0:	e052                	sd	s4,0(sp)
    800031a2:	1800                	addi	s0,sp,48
  struct buf *b;

  initlock(&bcache.lock, "bcache");
    800031a4:	00005597          	auipc	a1,0x5
    800031a8:	36c58593          	addi	a1,a1,876 # 80008510 <syscalls+0xc0>
    800031ac:	00014517          	auipc	a0,0x14
    800031b0:	3cc50513          	addi	a0,a0,972 # 80017578 <bcache>
    800031b4:	ffffe097          	auipc	ra,0xffffe
    800031b8:	9a0080e7          	jalr	-1632(ra) # 80000b54 <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    800031bc:	0001c797          	auipc	a5,0x1c
    800031c0:	3bc78793          	addi	a5,a5,956 # 8001f578 <bcache+0x8000>
    800031c4:	0001c717          	auipc	a4,0x1c
    800031c8:	61c70713          	addi	a4,a4,1564 # 8001f7e0 <bcache+0x8268>
    800031cc:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    800031d0:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    800031d4:	00014497          	auipc	s1,0x14
    800031d8:	3bc48493          	addi	s1,s1,956 # 80017590 <bcache+0x18>
    b->next = bcache.head.next;
    800031dc:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    800031de:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    800031e0:	00005a17          	auipc	s4,0x5
    800031e4:	338a0a13          	addi	s4,s4,824 # 80008518 <syscalls+0xc8>
    b->next = bcache.head.next;
    800031e8:	2b893783          	ld	a5,696(s2)
    800031ec:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    800031ee:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    800031f2:	85d2                	mv	a1,s4
    800031f4:	01048513          	addi	a0,s1,16
    800031f8:	00001097          	auipc	ra,0x1
    800031fc:	4bc080e7          	jalr	1212(ra) # 800046b4 <initsleeplock>
    bcache.head.next->prev = b;
    80003200:	2b893783          	ld	a5,696(s2)
    80003204:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    80003206:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    8000320a:	45848493          	addi	s1,s1,1112
    8000320e:	fd349de3          	bne	s1,s3,800031e8 <binit+0x54>
  }
}
    80003212:	70a2                	ld	ra,40(sp)
    80003214:	7402                	ld	s0,32(sp)
    80003216:	64e2                	ld	s1,24(sp)
    80003218:	6942                	ld	s2,16(sp)
    8000321a:	69a2                	ld	s3,8(sp)
    8000321c:	6a02                	ld	s4,0(sp)
    8000321e:	6145                	addi	sp,sp,48
    80003220:	8082                	ret

0000000080003222 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
    80003222:	7179                	addi	sp,sp,-48
    80003224:	f406                	sd	ra,40(sp)
    80003226:	f022                	sd	s0,32(sp)
    80003228:	ec26                	sd	s1,24(sp)
    8000322a:	e84a                	sd	s2,16(sp)
    8000322c:	e44e                	sd	s3,8(sp)
    8000322e:	1800                	addi	s0,sp,48
    80003230:	89aa                	mv	s3,a0
    80003232:	892e                	mv	s2,a1
  acquire(&bcache.lock);
    80003234:	00014517          	auipc	a0,0x14
    80003238:	34450513          	addi	a0,a0,836 # 80017578 <bcache>
    8000323c:	ffffe097          	auipc	ra,0xffffe
    80003240:	9a8080e7          	jalr	-1624(ra) # 80000be4 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80003244:	0001c497          	auipc	s1,0x1c
    80003248:	5ec4b483          	ld	s1,1516(s1) # 8001f830 <bcache+0x82b8>
    8000324c:	0001c797          	auipc	a5,0x1c
    80003250:	59478793          	addi	a5,a5,1428 # 8001f7e0 <bcache+0x8268>
    80003254:	02f48f63          	beq	s1,a5,80003292 <bread+0x70>
    80003258:	873e                	mv	a4,a5
    8000325a:	a021                	j	80003262 <bread+0x40>
    8000325c:	68a4                	ld	s1,80(s1)
    8000325e:	02e48a63          	beq	s1,a4,80003292 <bread+0x70>
    if(b->dev == dev && b->blockno == blockno){
    80003262:	449c                	lw	a5,8(s1)
    80003264:	ff379ce3          	bne	a5,s3,8000325c <bread+0x3a>
    80003268:	44dc                	lw	a5,12(s1)
    8000326a:	ff2799e3          	bne	a5,s2,8000325c <bread+0x3a>
      b->refcnt++;
    8000326e:	40bc                	lw	a5,64(s1)
    80003270:	2785                	addiw	a5,a5,1
    80003272:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80003274:	00014517          	auipc	a0,0x14
    80003278:	30450513          	addi	a0,a0,772 # 80017578 <bcache>
    8000327c:	ffffe097          	auipc	ra,0xffffe
    80003280:	a1c080e7          	jalr	-1508(ra) # 80000c98 <release>
      acquiresleep(&b->lock);
    80003284:	01048513          	addi	a0,s1,16
    80003288:	00001097          	auipc	ra,0x1
    8000328c:	466080e7          	jalr	1126(ra) # 800046ee <acquiresleep>
      return b;
    80003290:	a8b9                	j	800032ee <bread+0xcc>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80003292:	0001c497          	auipc	s1,0x1c
    80003296:	5964b483          	ld	s1,1430(s1) # 8001f828 <bcache+0x82b0>
    8000329a:	0001c797          	auipc	a5,0x1c
    8000329e:	54678793          	addi	a5,a5,1350 # 8001f7e0 <bcache+0x8268>
    800032a2:	00f48863          	beq	s1,a5,800032b2 <bread+0x90>
    800032a6:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    800032a8:	40bc                	lw	a5,64(s1)
    800032aa:	cf81                	beqz	a5,800032c2 <bread+0xa0>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    800032ac:	64a4                	ld	s1,72(s1)
    800032ae:	fee49de3          	bne	s1,a4,800032a8 <bread+0x86>
  panic("bget: no buffers");
    800032b2:	00005517          	auipc	a0,0x5
    800032b6:	26e50513          	addi	a0,a0,622 # 80008520 <syscalls+0xd0>
    800032ba:	ffffd097          	auipc	ra,0xffffd
    800032be:	284080e7          	jalr	644(ra) # 8000053e <panic>
      b->dev = dev;
    800032c2:	0134a423          	sw	s3,8(s1)
      b->blockno = blockno;
    800032c6:	0124a623          	sw	s2,12(s1)
      b->valid = 0;
    800032ca:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    800032ce:	4785                	li	a5,1
    800032d0:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    800032d2:	00014517          	auipc	a0,0x14
    800032d6:	2a650513          	addi	a0,a0,678 # 80017578 <bcache>
    800032da:	ffffe097          	auipc	ra,0xffffe
    800032de:	9be080e7          	jalr	-1602(ra) # 80000c98 <release>
      acquiresleep(&b->lock);
    800032e2:	01048513          	addi	a0,s1,16
    800032e6:	00001097          	auipc	ra,0x1
    800032ea:	408080e7          	jalr	1032(ra) # 800046ee <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    800032ee:	409c                	lw	a5,0(s1)
    800032f0:	cb89                	beqz	a5,80003302 <bread+0xe0>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
}
    800032f2:	8526                	mv	a0,s1
    800032f4:	70a2                	ld	ra,40(sp)
    800032f6:	7402                	ld	s0,32(sp)
    800032f8:	64e2                	ld	s1,24(sp)
    800032fa:	6942                	ld	s2,16(sp)
    800032fc:	69a2                	ld	s3,8(sp)
    800032fe:	6145                	addi	sp,sp,48
    80003300:	8082                	ret
    virtio_disk_rw(b, 0);
    80003302:	4581                	li	a1,0
    80003304:	8526                	mv	a0,s1
    80003306:	00003097          	auipc	ra,0x3
    8000330a:	f10080e7          	jalr	-240(ra) # 80006216 <virtio_disk_rw>
    b->valid = 1;
    8000330e:	4785                	li	a5,1
    80003310:	c09c                	sw	a5,0(s1)
  return b;
    80003312:	b7c5                	j	800032f2 <bread+0xd0>

0000000080003314 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
    80003314:	1101                	addi	sp,sp,-32
    80003316:	ec06                	sd	ra,24(sp)
    80003318:	e822                	sd	s0,16(sp)
    8000331a:	e426                	sd	s1,8(sp)
    8000331c:	1000                	addi	s0,sp,32
    8000331e:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80003320:	0541                	addi	a0,a0,16
    80003322:	00001097          	auipc	ra,0x1
    80003326:	466080e7          	jalr	1126(ra) # 80004788 <holdingsleep>
    8000332a:	cd01                	beqz	a0,80003342 <bwrite+0x2e>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    8000332c:	4585                	li	a1,1
    8000332e:	8526                	mv	a0,s1
    80003330:	00003097          	auipc	ra,0x3
    80003334:	ee6080e7          	jalr	-282(ra) # 80006216 <virtio_disk_rw>
}
    80003338:	60e2                	ld	ra,24(sp)
    8000333a:	6442                	ld	s0,16(sp)
    8000333c:	64a2                	ld	s1,8(sp)
    8000333e:	6105                	addi	sp,sp,32
    80003340:	8082                	ret
    panic("bwrite");
    80003342:	00005517          	auipc	a0,0x5
    80003346:	1f650513          	addi	a0,a0,502 # 80008538 <syscalls+0xe8>
    8000334a:	ffffd097          	auipc	ra,0xffffd
    8000334e:	1f4080e7          	jalr	500(ra) # 8000053e <panic>

0000000080003352 <brelse>:

// Release a locked buffer.
// Move to the head of the most-recently-used list.
void
brelse(struct buf *b)
{
    80003352:	1101                	addi	sp,sp,-32
    80003354:	ec06                	sd	ra,24(sp)
    80003356:	e822                	sd	s0,16(sp)
    80003358:	e426                	sd	s1,8(sp)
    8000335a:	e04a                	sd	s2,0(sp)
    8000335c:	1000                	addi	s0,sp,32
    8000335e:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80003360:	01050913          	addi	s2,a0,16
    80003364:	854a                	mv	a0,s2
    80003366:	00001097          	auipc	ra,0x1
    8000336a:	422080e7          	jalr	1058(ra) # 80004788 <holdingsleep>
    8000336e:	c92d                	beqz	a0,800033e0 <brelse+0x8e>
    panic("brelse");

  releasesleep(&b->lock);
    80003370:	854a                	mv	a0,s2
    80003372:	00001097          	auipc	ra,0x1
    80003376:	3d2080e7          	jalr	978(ra) # 80004744 <releasesleep>

  acquire(&bcache.lock);
    8000337a:	00014517          	auipc	a0,0x14
    8000337e:	1fe50513          	addi	a0,a0,510 # 80017578 <bcache>
    80003382:	ffffe097          	auipc	ra,0xffffe
    80003386:	862080e7          	jalr	-1950(ra) # 80000be4 <acquire>
  b->refcnt--;
    8000338a:	40bc                	lw	a5,64(s1)
    8000338c:	37fd                	addiw	a5,a5,-1
    8000338e:	0007871b          	sext.w	a4,a5
    80003392:	c0bc                	sw	a5,64(s1)
  if (b->refcnt == 0) {
    80003394:	eb05                	bnez	a4,800033c4 <brelse+0x72>
    // no one is waiting for it.
    b->next->prev = b->prev;
    80003396:	68bc                	ld	a5,80(s1)
    80003398:	64b8                	ld	a4,72(s1)
    8000339a:	e7b8                	sd	a4,72(a5)
    b->prev->next = b->next;
    8000339c:	64bc                	ld	a5,72(s1)
    8000339e:	68b8                	ld	a4,80(s1)
    800033a0:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    800033a2:	0001c797          	auipc	a5,0x1c
    800033a6:	1d678793          	addi	a5,a5,470 # 8001f578 <bcache+0x8000>
    800033aa:	2b87b703          	ld	a4,696(a5)
    800033ae:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    800033b0:	0001c717          	auipc	a4,0x1c
    800033b4:	43070713          	addi	a4,a4,1072 # 8001f7e0 <bcache+0x8268>
    800033b8:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    800033ba:	2b87b703          	ld	a4,696(a5)
    800033be:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    800033c0:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    800033c4:	00014517          	auipc	a0,0x14
    800033c8:	1b450513          	addi	a0,a0,436 # 80017578 <bcache>
    800033cc:	ffffe097          	auipc	ra,0xffffe
    800033d0:	8cc080e7          	jalr	-1844(ra) # 80000c98 <release>
}
    800033d4:	60e2                	ld	ra,24(sp)
    800033d6:	6442                	ld	s0,16(sp)
    800033d8:	64a2                	ld	s1,8(sp)
    800033da:	6902                	ld	s2,0(sp)
    800033dc:	6105                	addi	sp,sp,32
    800033de:	8082                	ret
    panic("brelse");
    800033e0:	00005517          	auipc	a0,0x5
    800033e4:	16050513          	addi	a0,a0,352 # 80008540 <syscalls+0xf0>
    800033e8:	ffffd097          	auipc	ra,0xffffd
    800033ec:	156080e7          	jalr	342(ra) # 8000053e <panic>

00000000800033f0 <bpin>:

void
bpin(struct buf *b) {
    800033f0:	1101                	addi	sp,sp,-32
    800033f2:	ec06                	sd	ra,24(sp)
    800033f4:	e822                	sd	s0,16(sp)
    800033f6:	e426                	sd	s1,8(sp)
    800033f8:	1000                	addi	s0,sp,32
    800033fa:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    800033fc:	00014517          	auipc	a0,0x14
    80003400:	17c50513          	addi	a0,a0,380 # 80017578 <bcache>
    80003404:	ffffd097          	auipc	ra,0xffffd
    80003408:	7e0080e7          	jalr	2016(ra) # 80000be4 <acquire>
  b->refcnt++;
    8000340c:	40bc                	lw	a5,64(s1)
    8000340e:	2785                	addiw	a5,a5,1
    80003410:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80003412:	00014517          	auipc	a0,0x14
    80003416:	16650513          	addi	a0,a0,358 # 80017578 <bcache>
    8000341a:	ffffe097          	auipc	ra,0xffffe
    8000341e:	87e080e7          	jalr	-1922(ra) # 80000c98 <release>
}
    80003422:	60e2                	ld	ra,24(sp)
    80003424:	6442                	ld	s0,16(sp)
    80003426:	64a2                	ld	s1,8(sp)
    80003428:	6105                	addi	sp,sp,32
    8000342a:	8082                	ret

000000008000342c <bunpin>:

void
bunpin(struct buf *b) {
    8000342c:	1101                	addi	sp,sp,-32
    8000342e:	ec06                	sd	ra,24(sp)
    80003430:	e822                	sd	s0,16(sp)
    80003432:	e426                	sd	s1,8(sp)
    80003434:	1000                	addi	s0,sp,32
    80003436:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80003438:	00014517          	auipc	a0,0x14
    8000343c:	14050513          	addi	a0,a0,320 # 80017578 <bcache>
    80003440:	ffffd097          	auipc	ra,0xffffd
    80003444:	7a4080e7          	jalr	1956(ra) # 80000be4 <acquire>
  b->refcnt--;
    80003448:	40bc                	lw	a5,64(s1)
    8000344a:	37fd                	addiw	a5,a5,-1
    8000344c:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    8000344e:	00014517          	auipc	a0,0x14
    80003452:	12a50513          	addi	a0,a0,298 # 80017578 <bcache>
    80003456:	ffffe097          	auipc	ra,0xffffe
    8000345a:	842080e7          	jalr	-1982(ra) # 80000c98 <release>
}
    8000345e:	60e2                	ld	ra,24(sp)
    80003460:	6442                	ld	s0,16(sp)
    80003462:	64a2                	ld	s1,8(sp)
    80003464:	6105                	addi	sp,sp,32
    80003466:	8082                	ret

0000000080003468 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
    80003468:	1101                	addi	sp,sp,-32
    8000346a:	ec06                	sd	ra,24(sp)
    8000346c:	e822                	sd	s0,16(sp)
    8000346e:	e426                	sd	s1,8(sp)
    80003470:	e04a                	sd	s2,0(sp)
    80003472:	1000                	addi	s0,sp,32
    80003474:	84ae                	mv	s1,a1
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
    80003476:	00d5d59b          	srliw	a1,a1,0xd
    8000347a:	0001c797          	auipc	a5,0x1c
    8000347e:	7da7a783          	lw	a5,2010(a5) # 8001fc54 <sb+0x1c>
    80003482:	9dbd                	addw	a1,a1,a5
    80003484:	00000097          	auipc	ra,0x0
    80003488:	d9e080e7          	jalr	-610(ra) # 80003222 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    8000348c:	0074f713          	andi	a4,s1,7
    80003490:	4785                	li	a5,1
    80003492:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    80003496:	14ce                	slli	s1,s1,0x33
    80003498:	90d9                	srli	s1,s1,0x36
    8000349a:	00950733          	add	a4,a0,s1
    8000349e:	05874703          	lbu	a4,88(a4)
    800034a2:	00e7f6b3          	and	a3,a5,a4
    800034a6:	c69d                	beqz	a3,800034d4 <bfree+0x6c>
    800034a8:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    800034aa:	94aa                	add	s1,s1,a0
    800034ac:	fff7c793          	not	a5,a5
    800034b0:	8ff9                	and	a5,a5,a4
    800034b2:	04f48c23          	sb	a5,88(s1)
  log_write(bp);
    800034b6:	00001097          	auipc	ra,0x1
    800034ba:	118080e7          	jalr	280(ra) # 800045ce <log_write>
  brelse(bp);
    800034be:	854a                	mv	a0,s2
    800034c0:	00000097          	auipc	ra,0x0
    800034c4:	e92080e7          	jalr	-366(ra) # 80003352 <brelse>
}
    800034c8:	60e2                	ld	ra,24(sp)
    800034ca:	6442                	ld	s0,16(sp)
    800034cc:	64a2                	ld	s1,8(sp)
    800034ce:	6902                	ld	s2,0(sp)
    800034d0:	6105                	addi	sp,sp,32
    800034d2:	8082                	ret
    panic("freeing free block");
    800034d4:	00005517          	auipc	a0,0x5
    800034d8:	07450513          	addi	a0,a0,116 # 80008548 <syscalls+0xf8>
    800034dc:	ffffd097          	auipc	ra,0xffffd
    800034e0:	062080e7          	jalr	98(ra) # 8000053e <panic>

00000000800034e4 <balloc>:
{
    800034e4:	711d                	addi	sp,sp,-96
    800034e6:	ec86                	sd	ra,88(sp)
    800034e8:	e8a2                	sd	s0,80(sp)
    800034ea:	e4a6                	sd	s1,72(sp)
    800034ec:	e0ca                	sd	s2,64(sp)
    800034ee:	fc4e                	sd	s3,56(sp)
    800034f0:	f852                	sd	s4,48(sp)
    800034f2:	f456                	sd	s5,40(sp)
    800034f4:	f05a                	sd	s6,32(sp)
    800034f6:	ec5e                	sd	s7,24(sp)
    800034f8:	e862                	sd	s8,16(sp)
    800034fa:	e466                	sd	s9,8(sp)
    800034fc:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    800034fe:	0001c797          	auipc	a5,0x1c
    80003502:	73e7a783          	lw	a5,1854(a5) # 8001fc3c <sb+0x4>
    80003506:	cbd1                	beqz	a5,8000359a <balloc+0xb6>
    80003508:	8baa                	mv	s7,a0
    8000350a:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    8000350c:	0001cb17          	auipc	s6,0x1c
    80003510:	72cb0b13          	addi	s6,s6,1836 # 8001fc38 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003514:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80003516:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003518:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    8000351a:	6c89                	lui	s9,0x2
    8000351c:	a831                	j	80003538 <balloc+0x54>
    brelse(bp);
    8000351e:	854a                	mv	a0,s2
    80003520:	00000097          	auipc	ra,0x0
    80003524:	e32080e7          	jalr	-462(ra) # 80003352 <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80003528:	015c87bb          	addw	a5,s9,s5
    8000352c:	00078a9b          	sext.w	s5,a5
    80003530:	004b2703          	lw	a4,4(s6)
    80003534:	06eaf363          	bgeu	s5,a4,8000359a <balloc+0xb6>
    bp = bread(dev, BBLOCK(b, sb));
    80003538:	41fad79b          	sraiw	a5,s5,0x1f
    8000353c:	0137d79b          	srliw	a5,a5,0x13
    80003540:	015787bb          	addw	a5,a5,s5
    80003544:	40d7d79b          	sraiw	a5,a5,0xd
    80003548:	01cb2583          	lw	a1,28(s6)
    8000354c:	9dbd                	addw	a1,a1,a5
    8000354e:	855e                	mv	a0,s7
    80003550:	00000097          	auipc	ra,0x0
    80003554:	cd2080e7          	jalr	-814(ra) # 80003222 <bread>
    80003558:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    8000355a:	004b2503          	lw	a0,4(s6)
    8000355e:	000a849b          	sext.w	s1,s5
    80003562:	8662                	mv	a2,s8
    80003564:	faa4fde3          	bgeu	s1,a0,8000351e <balloc+0x3a>
      m = 1 << (bi % 8);
    80003568:	41f6579b          	sraiw	a5,a2,0x1f
    8000356c:	01d7d69b          	srliw	a3,a5,0x1d
    80003570:	00c6873b          	addw	a4,a3,a2
    80003574:	00777793          	andi	a5,a4,7
    80003578:	9f95                	subw	a5,a5,a3
    8000357a:	00f997bb          	sllw	a5,s3,a5
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    8000357e:	4037571b          	sraiw	a4,a4,0x3
    80003582:	00e906b3          	add	a3,s2,a4
    80003586:	0586c683          	lbu	a3,88(a3)
    8000358a:	00d7f5b3          	and	a1,a5,a3
    8000358e:	cd91                	beqz	a1,800035aa <balloc+0xc6>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003590:	2605                	addiw	a2,a2,1
    80003592:	2485                	addiw	s1,s1,1
    80003594:	fd4618e3          	bne	a2,s4,80003564 <balloc+0x80>
    80003598:	b759                	j	8000351e <balloc+0x3a>
  panic("balloc: out of blocks");
    8000359a:	00005517          	auipc	a0,0x5
    8000359e:	fc650513          	addi	a0,a0,-58 # 80008560 <syscalls+0x110>
    800035a2:	ffffd097          	auipc	ra,0xffffd
    800035a6:	f9c080e7          	jalr	-100(ra) # 8000053e <panic>
        bp->data[bi/8] |= m;  // Mark block in use.
    800035aa:	974a                	add	a4,a4,s2
    800035ac:	8fd5                	or	a5,a5,a3
    800035ae:	04f70c23          	sb	a5,88(a4)
        log_write(bp);
    800035b2:	854a                	mv	a0,s2
    800035b4:	00001097          	auipc	ra,0x1
    800035b8:	01a080e7          	jalr	26(ra) # 800045ce <log_write>
        brelse(bp);
    800035bc:	854a                	mv	a0,s2
    800035be:	00000097          	auipc	ra,0x0
    800035c2:	d94080e7          	jalr	-620(ra) # 80003352 <brelse>
  bp = bread(dev, bno);
    800035c6:	85a6                	mv	a1,s1
    800035c8:	855e                	mv	a0,s7
    800035ca:	00000097          	auipc	ra,0x0
    800035ce:	c58080e7          	jalr	-936(ra) # 80003222 <bread>
    800035d2:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    800035d4:	40000613          	li	a2,1024
    800035d8:	4581                	li	a1,0
    800035da:	05850513          	addi	a0,a0,88
    800035de:	ffffd097          	auipc	ra,0xffffd
    800035e2:	702080e7          	jalr	1794(ra) # 80000ce0 <memset>
  log_write(bp);
    800035e6:	854a                	mv	a0,s2
    800035e8:	00001097          	auipc	ra,0x1
    800035ec:	fe6080e7          	jalr	-26(ra) # 800045ce <log_write>
  brelse(bp);
    800035f0:	854a                	mv	a0,s2
    800035f2:	00000097          	auipc	ra,0x0
    800035f6:	d60080e7          	jalr	-672(ra) # 80003352 <brelse>
}
    800035fa:	8526                	mv	a0,s1
    800035fc:	60e6                	ld	ra,88(sp)
    800035fe:	6446                	ld	s0,80(sp)
    80003600:	64a6                	ld	s1,72(sp)
    80003602:	6906                	ld	s2,64(sp)
    80003604:	79e2                	ld	s3,56(sp)
    80003606:	7a42                	ld	s4,48(sp)
    80003608:	7aa2                	ld	s5,40(sp)
    8000360a:	7b02                	ld	s6,32(sp)
    8000360c:	6be2                	ld	s7,24(sp)
    8000360e:	6c42                	ld	s8,16(sp)
    80003610:	6ca2                	ld	s9,8(sp)
    80003612:	6125                	addi	sp,sp,96
    80003614:	8082                	ret

0000000080003616 <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
    80003616:	7179                	addi	sp,sp,-48
    80003618:	f406                	sd	ra,40(sp)
    8000361a:	f022                	sd	s0,32(sp)
    8000361c:	ec26                	sd	s1,24(sp)
    8000361e:	e84a                	sd	s2,16(sp)
    80003620:	e44e                	sd	s3,8(sp)
    80003622:	e052                	sd	s4,0(sp)
    80003624:	1800                	addi	s0,sp,48
    80003626:	892a                	mv	s2,a0
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
    80003628:	47ad                	li	a5,11
    8000362a:	04b7fe63          	bgeu	a5,a1,80003686 <bmap+0x70>
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
    8000362e:	ff45849b          	addiw	s1,a1,-12
    80003632:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80003636:	0ff00793          	li	a5,255
    8000363a:	0ae7e363          	bltu	a5,a4,800036e0 <bmap+0xca>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
    8000363e:	08052583          	lw	a1,128(a0)
    80003642:	c5ad                	beqz	a1,800036ac <bmap+0x96>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    80003644:	00092503          	lw	a0,0(s2)
    80003648:	00000097          	auipc	ra,0x0
    8000364c:	bda080e7          	jalr	-1062(ra) # 80003222 <bread>
    80003650:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    80003652:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    80003656:	02049593          	slli	a1,s1,0x20
    8000365a:	9181                	srli	a1,a1,0x20
    8000365c:	058a                	slli	a1,a1,0x2
    8000365e:	00b784b3          	add	s1,a5,a1
    80003662:	0004a983          	lw	s3,0(s1)
    80003666:	04098d63          	beqz	s3,800036c0 <bmap+0xaa>
      a[bn] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    8000366a:	8552                	mv	a0,s4
    8000366c:	00000097          	auipc	ra,0x0
    80003670:	ce6080e7          	jalr	-794(ra) # 80003352 <brelse>
    return addr;
  }

  panic("bmap: out of range");
}
    80003674:	854e                	mv	a0,s3
    80003676:	70a2                	ld	ra,40(sp)
    80003678:	7402                	ld	s0,32(sp)
    8000367a:	64e2                	ld	s1,24(sp)
    8000367c:	6942                	ld	s2,16(sp)
    8000367e:	69a2                	ld	s3,8(sp)
    80003680:	6a02                	ld	s4,0(sp)
    80003682:	6145                	addi	sp,sp,48
    80003684:	8082                	ret
    if((addr = ip->addrs[bn]) == 0)
    80003686:	02059493          	slli	s1,a1,0x20
    8000368a:	9081                	srli	s1,s1,0x20
    8000368c:	048a                	slli	s1,s1,0x2
    8000368e:	94aa                	add	s1,s1,a0
    80003690:	0504a983          	lw	s3,80(s1)
    80003694:	fe0990e3          	bnez	s3,80003674 <bmap+0x5e>
      ip->addrs[bn] = addr = balloc(ip->dev);
    80003698:	4108                	lw	a0,0(a0)
    8000369a:	00000097          	auipc	ra,0x0
    8000369e:	e4a080e7          	jalr	-438(ra) # 800034e4 <balloc>
    800036a2:	0005099b          	sext.w	s3,a0
    800036a6:	0534a823          	sw	s3,80(s1)
    800036aa:	b7e9                	j	80003674 <bmap+0x5e>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    800036ac:	4108                	lw	a0,0(a0)
    800036ae:	00000097          	auipc	ra,0x0
    800036b2:	e36080e7          	jalr	-458(ra) # 800034e4 <balloc>
    800036b6:	0005059b          	sext.w	a1,a0
    800036ba:	08b92023          	sw	a1,128(s2)
    800036be:	b759                	j	80003644 <bmap+0x2e>
      a[bn] = addr = balloc(ip->dev);
    800036c0:	00092503          	lw	a0,0(s2)
    800036c4:	00000097          	auipc	ra,0x0
    800036c8:	e20080e7          	jalr	-480(ra) # 800034e4 <balloc>
    800036cc:	0005099b          	sext.w	s3,a0
    800036d0:	0134a023          	sw	s3,0(s1)
      log_write(bp);
    800036d4:	8552                	mv	a0,s4
    800036d6:	00001097          	auipc	ra,0x1
    800036da:	ef8080e7          	jalr	-264(ra) # 800045ce <log_write>
    800036de:	b771                	j	8000366a <bmap+0x54>
  panic("bmap: out of range");
    800036e0:	00005517          	auipc	a0,0x5
    800036e4:	e9850513          	addi	a0,a0,-360 # 80008578 <syscalls+0x128>
    800036e8:	ffffd097          	auipc	ra,0xffffd
    800036ec:	e56080e7          	jalr	-426(ra) # 8000053e <panic>

00000000800036f0 <iget>:
{
    800036f0:	7179                	addi	sp,sp,-48
    800036f2:	f406                	sd	ra,40(sp)
    800036f4:	f022                	sd	s0,32(sp)
    800036f6:	ec26                	sd	s1,24(sp)
    800036f8:	e84a                	sd	s2,16(sp)
    800036fa:	e44e                	sd	s3,8(sp)
    800036fc:	e052                	sd	s4,0(sp)
    800036fe:	1800                	addi	s0,sp,48
    80003700:	89aa                	mv	s3,a0
    80003702:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    80003704:	0001c517          	auipc	a0,0x1c
    80003708:	55450513          	addi	a0,a0,1364 # 8001fc58 <itable>
    8000370c:	ffffd097          	auipc	ra,0xffffd
    80003710:	4d8080e7          	jalr	1240(ra) # 80000be4 <acquire>
  empty = 0;
    80003714:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80003716:	0001c497          	auipc	s1,0x1c
    8000371a:	55a48493          	addi	s1,s1,1370 # 8001fc70 <itable+0x18>
    8000371e:	0001e697          	auipc	a3,0x1e
    80003722:	fe268693          	addi	a3,a3,-30 # 80021700 <log>
    80003726:	a039                	j	80003734 <iget+0x44>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80003728:	02090b63          	beqz	s2,8000375e <iget+0x6e>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    8000372c:	08848493          	addi	s1,s1,136
    80003730:	02d48a63          	beq	s1,a3,80003764 <iget+0x74>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    80003734:	449c                	lw	a5,8(s1)
    80003736:	fef059e3          	blez	a5,80003728 <iget+0x38>
    8000373a:	4098                	lw	a4,0(s1)
    8000373c:	ff3716e3          	bne	a4,s3,80003728 <iget+0x38>
    80003740:	40d8                	lw	a4,4(s1)
    80003742:	ff4713e3          	bne	a4,s4,80003728 <iget+0x38>
      ip->ref++;
    80003746:	2785                	addiw	a5,a5,1
    80003748:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    8000374a:	0001c517          	auipc	a0,0x1c
    8000374e:	50e50513          	addi	a0,a0,1294 # 8001fc58 <itable>
    80003752:	ffffd097          	auipc	ra,0xffffd
    80003756:	546080e7          	jalr	1350(ra) # 80000c98 <release>
      return ip;
    8000375a:	8926                	mv	s2,s1
    8000375c:	a03d                	j	8000378a <iget+0x9a>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    8000375e:	f7f9                	bnez	a5,8000372c <iget+0x3c>
    80003760:	8926                	mv	s2,s1
    80003762:	b7e9                	j	8000372c <iget+0x3c>
  if(empty == 0)
    80003764:	02090c63          	beqz	s2,8000379c <iget+0xac>
  ip->dev = dev;
    80003768:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    8000376c:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    80003770:	4785                	li	a5,1
    80003772:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    80003776:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    8000377a:	0001c517          	auipc	a0,0x1c
    8000377e:	4de50513          	addi	a0,a0,1246 # 8001fc58 <itable>
    80003782:	ffffd097          	auipc	ra,0xffffd
    80003786:	516080e7          	jalr	1302(ra) # 80000c98 <release>
}
    8000378a:	854a                	mv	a0,s2
    8000378c:	70a2                	ld	ra,40(sp)
    8000378e:	7402                	ld	s0,32(sp)
    80003790:	64e2                	ld	s1,24(sp)
    80003792:	6942                	ld	s2,16(sp)
    80003794:	69a2                	ld	s3,8(sp)
    80003796:	6a02                	ld	s4,0(sp)
    80003798:	6145                	addi	sp,sp,48
    8000379a:	8082                	ret
    panic("iget: no inodes");
    8000379c:	00005517          	auipc	a0,0x5
    800037a0:	df450513          	addi	a0,a0,-524 # 80008590 <syscalls+0x140>
    800037a4:	ffffd097          	auipc	ra,0xffffd
    800037a8:	d9a080e7          	jalr	-614(ra) # 8000053e <panic>

00000000800037ac <fsinit>:
fsinit(int dev) {
    800037ac:	7179                	addi	sp,sp,-48
    800037ae:	f406                	sd	ra,40(sp)
    800037b0:	f022                	sd	s0,32(sp)
    800037b2:	ec26                	sd	s1,24(sp)
    800037b4:	e84a                	sd	s2,16(sp)
    800037b6:	e44e                	sd	s3,8(sp)
    800037b8:	1800                	addi	s0,sp,48
    800037ba:	892a                	mv	s2,a0
  bp = bread(dev, 1);
    800037bc:	4585                	li	a1,1
    800037be:	00000097          	auipc	ra,0x0
    800037c2:	a64080e7          	jalr	-1436(ra) # 80003222 <bread>
    800037c6:	84aa                	mv	s1,a0
  memmove(sb, bp->data, sizeof(*sb));
    800037c8:	0001c997          	auipc	s3,0x1c
    800037cc:	47098993          	addi	s3,s3,1136 # 8001fc38 <sb>
    800037d0:	02000613          	li	a2,32
    800037d4:	05850593          	addi	a1,a0,88
    800037d8:	854e                	mv	a0,s3
    800037da:	ffffd097          	auipc	ra,0xffffd
    800037de:	566080e7          	jalr	1382(ra) # 80000d40 <memmove>
  brelse(bp);
    800037e2:	8526                	mv	a0,s1
    800037e4:	00000097          	auipc	ra,0x0
    800037e8:	b6e080e7          	jalr	-1170(ra) # 80003352 <brelse>
  if(sb.magic != FSMAGIC)
    800037ec:	0009a703          	lw	a4,0(s3)
    800037f0:	102037b7          	lui	a5,0x10203
    800037f4:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    800037f8:	02f71263          	bne	a4,a5,8000381c <fsinit+0x70>
  initlog(dev, &sb);
    800037fc:	0001c597          	auipc	a1,0x1c
    80003800:	43c58593          	addi	a1,a1,1084 # 8001fc38 <sb>
    80003804:	854a                	mv	a0,s2
    80003806:	00001097          	auipc	ra,0x1
    8000380a:	b4c080e7          	jalr	-1204(ra) # 80004352 <initlog>
}
    8000380e:	70a2                	ld	ra,40(sp)
    80003810:	7402                	ld	s0,32(sp)
    80003812:	64e2                	ld	s1,24(sp)
    80003814:	6942                	ld	s2,16(sp)
    80003816:	69a2                	ld	s3,8(sp)
    80003818:	6145                	addi	sp,sp,48
    8000381a:	8082                	ret
    panic("invalid file system");
    8000381c:	00005517          	auipc	a0,0x5
    80003820:	d8450513          	addi	a0,a0,-636 # 800085a0 <syscalls+0x150>
    80003824:	ffffd097          	auipc	ra,0xffffd
    80003828:	d1a080e7          	jalr	-742(ra) # 8000053e <panic>

000000008000382c <iinit>:
{
    8000382c:	7179                	addi	sp,sp,-48
    8000382e:	f406                	sd	ra,40(sp)
    80003830:	f022                	sd	s0,32(sp)
    80003832:	ec26                	sd	s1,24(sp)
    80003834:	e84a                	sd	s2,16(sp)
    80003836:	e44e                	sd	s3,8(sp)
    80003838:	1800                	addi	s0,sp,48
  initlock(&itable.lock, "itable");
    8000383a:	00005597          	auipc	a1,0x5
    8000383e:	d7e58593          	addi	a1,a1,-642 # 800085b8 <syscalls+0x168>
    80003842:	0001c517          	auipc	a0,0x1c
    80003846:	41650513          	addi	a0,a0,1046 # 8001fc58 <itable>
    8000384a:	ffffd097          	auipc	ra,0xffffd
    8000384e:	30a080e7          	jalr	778(ra) # 80000b54 <initlock>
  for(i = 0; i < NINODE; i++) {
    80003852:	0001c497          	auipc	s1,0x1c
    80003856:	42e48493          	addi	s1,s1,1070 # 8001fc80 <itable+0x28>
    8000385a:	0001e997          	auipc	s3,0x1e
    8000385e:	eb698993          	addi	s3,s3,-330 # 80021710 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    80003862:	00005917          	auipc	s2,0x5
    80003866:	d5e90913          	addi	s2,s2,-674 # 800085c0 <syscalls+0x170>
    8000386a:	85ca                	mv	a1,s2
    8000386c:	8526                	mv	a0,s1
    8000386e:	00001097          	auipc	ra,0x1
    80003872:	e46080e7          	jalr	-442(ra) # 800046b4 <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    80003876:	08848493          	addi	s1,s1,136
    8000387a:	ff3498e3          	bne	s1,s3,8000386a <iinit+0x3e>
}
    8000387e:	70a2                	ld	ra,40(sp)
    80003880:	7402                	ld	s0,32(sp)
    80003882:	64e2                	ld	s1,24(sp)
    80003884:	6942                	ld	s2,16(sp)
    80003886:	69a2                	ld	s3,8(sp)
    80003888:	6145                	addi	sp,sp,48
    8000388a:	8082                	ret

000000008000388c <ialloc>:
{
    8000388c:	715d                	addi	sp,sp,-80
    8000388e:	e486                	sd	ra,72(sp)
    80003890:	e0a2                	sd	s0,64(sp)
    80003892:	fc26                	sd	s1,56(sp)
    80003894:	f84a                	sd	s2,48(sp)
    80003896:	f44e                	sd	s3,40(sp)
    80003898:	f052                	sd	s4,32(sp)
    8000389a:	ec56                	sd	s5,24(sp)
    8000389c:	e85a                	sd	s6,16(sp)
    8000389e:	e45e                	sd	s7,8(sp)
    800038a0:	0880                	addi	s0,sp,80
  for(inum = 1; inum < sb.ninodes; inum++){
    800038a2:	0001c717          	auipc	a4,0x1c
    800038a6:	3a272703          	lw	a4,930(a4) # 8001fc44 <sb+0xc>
    800038aa:	4785                	li	a5,1
    800038ac:	04e7fa63          	bgeu	a5,a4,80003900 <ialloc+0x74>
    800038b0:	8aaa                	mv	s5,a0
    800038b2:	8bae                	mv	s7,a1
    800038b4:	4485                	li	s1,1
    bp = bread(dev, IBLOCK(inum, sb));
    800038b6:	0001ca17          	auipc	s4,0x1c
    800038ba:	382a0a13          	addi	s4,s4,898 # 8001fc38 <sb>
    800038be:	00048b1b          	sext.w	s6,s1
    800038c2:	0044d593          	srli	a1,s1,0x4
    800038c6:	018a2783          	lw	a5,24(s4)
    800038ca:	9dbd                	addw	a1,a1,a5
    800038cc:	8556                	mv	a0,s5
    800038ce:	00000097          	auipc	ra,0x0
    800038d2:	954080e7          	jalr	-1708(ra) # 80003222 <bread>
    800038d6:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    800038d8:	05850993          	addi	s3,a0,88
    800038dc:	00f4f793          	andi	a5,s1,15
    800038e0:	079a                	slli	a5,a5,0x6
    800038e2:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    800038e4:	00099783          	lh	a5,0(s3)
    800038e8:	c785                	beqz	a5,80003910 <ialloc+0x84>
    brelse(bp);
    800038ea:	00000097          	auipc	ra,0x0
    800038ee:	a68080e7          	jalr	-1432(ra) # 80003352 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    800038f2:	0485                	addi	s1,s1,1
    800038f4:	00ca2703          	lw	a4,12(s4)
    800038f8:	0004879b          	sext.w	a5,s1
    800038fc:	fce7e1e3          	bltu	a5,a4,800038be <ialloc+0x32>
  panic("ialloc: no inodes");
    80003900:	00005517          	auipc	a0,0x5
    80003904:	cc850513          	addi	a0,a0,-824 # 800085c8 <syscalls+0x178>
    80003908:	ffffd097          	auipc	ra,0xffffd
    8000390c:	c36080e7          	jalr	-970(ra) # 8000053e <panic>
      memset(dip, 0, sizeof(*dip));
    80003910:	04000613          	li	a2,64
    80003914:	4581                	li	a1,0
    80003916:	854e                	mv	a0,s3
    80003918:	ffffd097          	auipc	ra,0xffffd
    8000391c:	3c8080e7          	jalr	968(ra) # 80000ce0 <memset>
      dip->type = type;
    80003920:	01799023          	sh	s7,0(s3)
      log_write(bp);   // mark it allocated on the disk
    80003924:	854a                	mv	a0,s2
    80003926:	00001097          	auipc	ra,0x1
    8000392a:	ca8080e7          	jalr	-856(ra) # 800045ce <log_write>
      brelse(bp);
    8000392e:	854a                	mv	a0,s2
    80003930:	00000097          	auipc	ra,0x0
    80003934:	a22080e7          	jalr	-1502(ra) # 80003352 <brelse>
      return iget(dev, inum);
    80003938:	85da                	mv	a1,s6
    8000393a:	8556                	mv	a0,s5
    8000393c:	00000097          	auipc	ra,0x0
    80003940:	db4080e7          	jalr	-588(ra) # 800036f0 <iget>
}
    80003944:	60a6                	ld	ra,72(sp)
    80003946:	6406                	ld	s0,64(sp)
    80003948:	74e2                	ld	s1,56(sp)
    8000394a:	7942                	ld	s2,48(sp)
    8000394c:	79a2                	ld	s3,40(sp)
    8000394e:	7a02                	ld	s4,32(sp)
    80003950:	6ae2                	ld	s5,24(sp)
    80003952:	6b42                	ld	s6,16(sp)
    80003954:	6ba2                	ld	s7,8(sp)
    80003956:	6161                	addi	sp,sp,80
    80003958:	8082                	ret

000000008000395a <iupdate>:
{
    8000395a:	1101                	addi	sp,sp,-32
    8000395c:	ec06                	sd	ra,24(sp)
    8000395e:	e822                	sd	s0,16(sp)
    80003960:	e426                	sd	s1,8(sp)
    80003962:	e04a                	sd	s2,0(sp)
    80003964:	1000                	addi	s0,sp,32
    80003966:	84aa                	mv	s1,a0
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003968:	415c                	lw	a5,4(a0)
    8000396a:	0047d79b          	srliw	a5,a5,0x4
    8000396e:	0001c597          	auipc	a1,0x1c
    80003972:	2e25a583          	lw	a1,738(a1) # 8001fc50 <sb+0x18>
    80003976:	9dbd                	addw	a1,a1,a5
    80003978:	4108                	lw	a0,0(a0)
    8000397a:	00000097          	auipc	ra,0x0
    8000397e:	8a8080e7          	jalr	-1880(ra) # 80003222 <bread>
    80003982:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003984:	05850793          	addi	a5,a0,88
    80003988:	40c8                	lw	a0,4(s1)
    8000398a:	893d                	andi	a0,a0,15
    8000398c:	051a                	slli	a0,a0,0x6
    8000398e:	953e                	add	a0,a0,a5
  dip->type = ip->type;
    80003990:	04449703          	lh	a4,68(s1)
    80003994:	00e51023          	sh	a4,0(a0)
  dip->major = ip->major;
    80003998:	04649703          	lh	a4,70(s1)
    8000399c:	00e51123          	sh	a4,2(a0)
  dip->minor = ip->minor;
    800039a0:	04849703          	lh	a4,72(s1)
    800039a4:	00e51223          	sh	a4,4(a0)
  dip->nlink = ip->nlink;
    800039a8:	04a49703          	lh	a4,74(s1)
    800039ac:	00e51323          	sh	a4,6(a0)
  dip->size = ip->size;
    800039b0:	44f8                	lw	a4,76(s1)
    800039b2:	c518                	sw	a4,8(a0)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    800039b4:	03400613          	li	a2,52
    800039b8:	05048593          	addi	a1,s1,80
    800039bc:	0531                	addi	a0,a0,12
    800039be:	ffffd097          	auipc	ra,0xffffd
    800039c2:	382080e7          	jalr	898(ra) # 80000d40 <memmove>
  log_write(bp);
    800039c6:	854a                	mv	a0,s2
    800039c8:	00001097          	auipc	ra,0x1
    800039cc:	c06080e7          	jalr	-1018(ra) # 800045ce <log_write>
  brelse(bp);
    800039d0:	854a                	mv	a0,s2
    800039d2:	00000097          	auipc	ra,0x0
    800039d6:	980080e7          	jalr	-1664(ra) # 80003352 <brelse>
}
    800039da:	60e2                	ld	ra,24(sp)
    800039dc:	6442                	ld	s0,16(sp)
    800039de:	64a2                	ld	s1,8(sp)
    800039e0:	6902                	ld	s2,0(sp)
    800039e2:	6105                	addi	sp,sp,32
    800039e4:	8082                	ret

00000000800039e6 <idup>:
{
    800039e6:	1101                	addi	sp,sp,-32
    800039e8:	ec06                	sd	ra,24(sp)
    800039ea:	e822                	sd	s0,16(sp)
    800039ec:	e426                	sd	s1,8(sp)
    800039ee:	1000                	addi	s0,sp,32
    800039f0:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    800039f2:	0001c517          	auipc	a0,0x1c
    800039f6:	26650513          	addi	a0,a0,614 # 8001fc58 <itable>
    800039fa:	ffffd097          	auipc	ra,0xffffd
    800039fe:	1ea080e7          	jalr	490(ra) # 80000be4 <acquire>
  ip->ref++;
    80003a02:	449c                	lw	a5,8(s1)
    80003a04:	2785                	addiw	a5,a5,1
    80003a06:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003a08:	0001c517          	auipc	a0,0x1c
    80003a0c:	25050513          	addi	a0,a0,592 # 8001fc58 <itable>
    80003a10:	ffffd097          	auipc	ra,0xffffd
    80003a14:	288080e7          	jalr	648(ra) # 80000c98 <release>
}
    80003a18:	8526                	mv	a0,s1
    80003a1a:	60e2                	ld	ra,24(sp)
    80003a1c:	6442                	ld	s0,16(sp)
    80003a1e:	64a2                	ld	s1,8(sp)
    80003a20:	6105                	addi	sp,sp,32
    80003a22:	8082                	ret

0000000080003a24 <ilock>:
{
    80003a24:	1101                	addi	sp,sp,-32
    80003a26:	ec06                	sd	ra,24(sp)
    80003a28:	e822                	sd	s0,16(sp)
    80003a2a:	e426                	sd	s1,8(sp)
    80003a2c:	e04a                	sd	s2,0(sp)
    80003a2e:	1000                	addi	s0,sp,32
  if(ip == 0 || ip->ref < 1)
    80003a30:	c115                	beqz	a0,80003a54 <ilock+0x30>
    80003a32:	84aa                	mv	s1,a0
    80003a34:	451c                	lw	a5,8(a0)
    80003a36:	00f05f63          	blez	a5,80003a54 <ilock+0x30>
  acquiresleep(&ip->lock);
    80003a3a:	0541                	addi	a0,a0,16
    80003a3c:	00001097          	auipc	ra,0x1
    80003a40:	cb2080e7          	jalr	-846(ra) # 800046ee <acquiresleep>
  if(ip->valid == 0){
    80003a44:	40bc                	lw	a5,64(s1)
    80003a46:	cf99                	beqz	a5,80003a64 <ilock+0x40>
}
    80003a48:	60e2                	ld	ra,24(sp)
    80003a4a:	6442                	ld	s0,16(sp)
    80003a4c:	64a2                	ld	s1,8(sp)
    80003a4e:	6902                	ld	s2,0(sp)
    80003a50:	6105                	addi	sp,sp,32
    80003a52:	8082                	ret
    panic("ilock");
    80003a54:	00005517          	auipc	a0,0x5
    80003a58:	b8c50513          	addi	a0,a0,-1140 # 800085e0 <syscalls+0x190>
    80003a5c:	ffffd097          	auipc	ra,0xffffd
    80003a60:	ae2080e7          	jalr	-1310(ra) # 8000053e <panic>
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003a64:	40dc                	lw	a5,4(s1)
    80003a66:	0047d79b          	srliw	a5,a5,0x4
    80003a6a:	0001c597          	auipc	a1,0x1c
    80003a6e:	1e65a583          	lw	a1,486(a1) # 8001fc50 <sb+0x18>
    80003a72:	9dbd                	addw	a1,a1,a5
    80003a74:	4088                	lw	a0,0(s1)
    80003a76:	fffff097          	auipc	ra,0xfffff
    80003a7a:	7ac080e7          	jalr	1964(ra) # 80003222 <bread>
    80003a7e:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003a80:	05850593          	addi	a1,a0,88
    80003a84:	40dc                	lw	a5,4(s1)
    80003a86:	8bbd                	andi	a5,a5,15
    80003a88:	079a                	slli	a5,a5,0x6
    80003a8a:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    80003a8c:	00059783          	lh	a5,0(a1)
    80003a90:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    80003a94:	00259783          	lh	a5,2(a1)
    80003a98:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    80003a9c:	00459783          	lh	a5,4(a1)
    80003aa0:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    80003aa4:	00659783          	lh	a5,6(a1)
    80003aa8:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    80003aac:	459c                	lw	a5,8(a1)
    80003aae:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    80003ab0:	03400613          	li	a2,52
    80003ab4:	05b1                	addi	a1,a1,12
    80003ab6:	05048513          	addi	a0,s1,80
    80003aba:	ffffd097          	auipc	ra,0xffffd
    80003abe:	286080e7          	jalr	646(ra) # 80000d40 <memmove>
    brelse(bp);
    80003ac2:	854a                	mv	a0,s2
    80003ac4:	00000097          	auipc	ra,0x0
    80003ac8:	88e080e7          	jalr	-1906(ra) # 80003352 <brelse>
    ip->valid = 1;
    80003acc:	4785                	li	a5,1
    80003ace:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    80003ad0:	04449783          	lh	a5,68(s1)
    80003ad4:	fbb5                	bnez	a5,80003a48 <ilock+0x24>
      panic("ilock: no type");
    80003ad6:	00005517          	auipc	a0,0x5
    80003ada:	b1250513          	addi	a0,a0,-1262 # 800085e8 <syscalls+0x198>
    80003ade:	ffffd097          	auipc	ra,0xffffd
    80003ae2:	a60080e7          	jalr	-1440(ra) # 8000053e <panic>

0000000080003ae6 <iunlock>:
{
    80003ae6:	1101                	addi	sp,sp,-32
    80003ae8:	ec06                	sd	ra,24(sp)
    80003aea:	e822                	sd	s0,16(sp)
    80003aec:	e426                	sd	s1,8(sp)
    80003aee:	e04a                	sd	s2,0(sp)
    80003af0:	1000                	addi	s0,sp,32
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    80003af2:	c905                	beqz	a0,80003b22 <iunlock+0x3c>
    80003af4:	84aa                	mv	s1,a0
    80003af6:	01050913          	addi	s2,a0,16
    80003afa:	854a                	mv	a0,s2
    80003afc:	00001097          	auipc	ra,0x1
    80003b00:	c8c080e7          	jalr	-884(ra) # 80004788 <holdingsleep>
    80003b04:	cd19                	beqz	a0,80003b22 <iunlock+0x3c>
    80003b06:	449c                	lw	a5,8(s1)
    80003b08:	00f05d63          	blez	a5,80003b22 <iunlock+0x3c>
  releasesleep(&ip->lock);
    80003b0c:	854a                	mv	a0,s2
    80003b0e:	00001097          	auipc	ra,0x1
    80003b12:	c36080e7          	jalr	-970(ra) # 80004744 <releasesleep>
}
    80003b16:	60e2                	ld	ra,24(sp)
    80003b18:	6442                	ld	s0,16(sp)
    80003b1a:	64a2                	ld	s1,8(sp)
    80003b1c:	6902                	ld	s2,0(sp)
    80003b1e:	6105                	addi	sp,sp,32
    80003b20:	8082                	ret
    panic("iunlock");
    80003b22:	00005517          	auipc	a0,0x5
    80003b26:	ad650513          	addi	a0,a0,-1322 # 800085f8 <syscalls+0x1a8>
    80003b2a:	ffffd097          	auipc	ra,0xffffd
    80003b2e:	a14080e7          	jalr	-1516(ra) # 8000053e <panic>

0000000080003b32 <itrunc>:

// Truncate inode (discard contents).
// Caller must hold ip->lock.
void
itrunc(struct inode *ip)
{
    80003b32:	7179                	addi	sp,sp,-48
    80003b34:	f406                	sd	ra,40(sp)
    80003b36:	f022                	sd	s0,32(sp)
    80003b38:	ec26                	sd	s1,24(sp)
    80003b3a:	e84a                	sd	s2,16(sp)
    80003b3c:	e44e                	sd	s3,8(sp)
    80003b3e:	e052                	sd	s4,0(sp)
    80003b40:	1800                	addi	s0,sp,48
    80003b42:	89aa                	mv	s3,a0
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
    80003b44:	05050493          	addi	s1,a0,80
    80003b48:	08050913          	addi	s2,a0,128
    80003b4c:	a021                	j	80003b54 <itrunc+0x22>
    80003b4e:	0491                	addi	s1,s1,4
    80003b50:	01248d63          	beq	s1,s2,80003b6a <itrunc+0x38>
    if(ip->addrs[i]){
    80003b54:	408c                	lw	a1,0(s1)
    80003b56:	dde5                	beqz	a1,80003b4e <itrunc+0x1c>
      bfree(ip->dev, ip->addrs[i]);
    80003b58:	0009a503          	lw	a0,0(s3)
    80003b5c:	00000097          	auipc	ra,0x0
    80003b60:	90c080e7          	jalr	-1780(ra) # 80003468 <bfree>
      ip->addrs[i] = 0;
    80003b64:	0004a023          	sw	zero,0(s1)
    80003b68:	b7dd                	j	80003b4e <itrunc+0x1c>
    }
  }

  if(ip->addrs[NDIRECT]){
    80003b6a:	0809a583          	lw	a1,128(s3)
    80003b6e:	e185                	bnez	a1,80003b8e <itrunc+0x5c>
    brelse(bp);
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
    80003b70:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    80003b74:	854e                	mv	a0,s3
    80003b76:	00000097          	auipc	ra,0x0
    80003b7a:	de4080e7          	jalr	-540(ra) # 8000395a <iupdate>
}
    80003b7e:	70a2                	ld	ra,40(sp)
    80003b80:	7402                	ld	s0,32(sp)
    80003b82:	64e2                	ld	s1,24(sp)
    80003b84:	6942                	ld	s2,16(sp)
    80003b86:	69a2                	ld	s3,8(sp)
    80003b88:	6a02                	ld	s4,0(sp)
    80003b8a:	6145                	addi	sp,sp,48
    80003b8c:	8082                	ret
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    80003b8e:	0009a503          	lw	a0,0(s3)
    80003b92:	fffff097          	auipc	ra,0xfffff
    80003b96:	690080e7          	jalr	1680(ra) # 80003222 <bread>
    80003b9a:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    80003b9c:	05850493          	addi	s1,a0,88
    80003ba0:	45850913          	addi	s2,a0,1112
    80003ba4:	a811                	j	80003bb8 <itrunc+0x86>
        bfree(ip->dev, a[j]);
    80003ba6:	0009a503          	lw	a0,0(s3)
    80003baa:	00000097          	auipc	ra,0x0
    80003bae:	8be080e7          	jalr	-1858(ra) # 80003468 <bfree>
    for(j = 0; j < NINDIRECT; j++){
    80003bb2:	0491                	addi	s1,s1,4
    80003bb4:	01248563          	beq	s1,s2,80003bbe <itrunc+0x8c>
      if(a[j])
    80003bb8:	408c                	lw	a1,0(s1)
    80003bba:	dde5                	beqz	a1,80003bb2 <itrunc+0x80>
    80003bbc:	b7ed                	j	80003ba6 <itrunc+0x74>
    brelse(bp);
    80003bbe:	8552                	mv	a0,s4
    80003bc0:	fffff097          	auipc	ra,0xfffff
    80003bc4:	792080e7          	jalr	1938(ra) # 80003352 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    80003bc8:	0809a583          	lw	a1,128(s3)
    80003bcc:	0009a503          	lw	a0,0(s3)
    80003bd0:	00000097          	auipc	ra,0x0
    80003bd4:	898080e7          	jalr	-1896(ra) # 80003468 <bfree>
    ip->addrs[NDIRECT] = 0;
    80003bd8:	0809a023          	sw	zero,128(s3)
    80003bdc:	bf51                	j	80003b70 <itrunc+0x3e>

0000000080003bde <iput>:
{
    80003bde:	1101                	addi	sp,sp,-32
    80003be0:	ec06                	sd	ra,24(sp)
    80003be2:	e822                	sd	s0,16(sp)
    80003be4:	e426                	sd	s1,8(sp)
    80003be6:	e04a                	sd	s2,0(sp)
    80003be8:	1000                	addi	s0,sp,32
    80003bea:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    80003bec:	0001c517          	auipc	a0,0x1c
    80003bf0:	06c50513          	addi	a0,a0,108 # 8001fc58 <itable>
    80003bf4:	ffffd097          	auipc	ra,0xffffd
    80003bf8:	ff0080e7          	jalr	-16(ra) # 80000be4 <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003bfc:	4498                	lw	a4,8(s1)
    80003bfe:	4785                	li	a5,1
    80003c00:	02f70363          	beq	a4,a5,80003c26 <iput+0x48>
  ip->ref--;
    80003c04:	449c                	lw	a5,8(s1)
    80003c06:	37fd                	addiw	a5,a5,-1
    80003c08:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003c0a:	0001c517          	auipc	a0,0x1c
    80003c0e:	04e50513          	addi	a0,a0,78 # 8001fc58 <itable>
    80003c12:	ffffd097          	auipc	ra,0xffffd
    80003c16:	086080e7          	jalr	134(ra) # 80000c98 <release>
}
    80003c1a:	60e2                	ld	ra,24(sp)
    80003c1c:	6442                	ld	s0,16(sp)
    80003c1e:	64a2                	ld	s1,8(sp)
    80003c20:	6902                	ld	s2,0(sp)
    80003c22:	6105                	addi	sp,sp,32
    80003c24:	8082                	ret
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003c26:	40bc                	lw	a5,64(s1)
    80003c28:	dff1                	beqz	a5,80003c04 <iput+0x26>
    80003c2a:	04a49783          	lh	a5,74(s1)
    80003c2e:	fbf9                	bnez	a5,80003c04 <iput+0x26>
    acquiresleep(&ip->lock);
    80003c30:	01048913          	addi	s2,s1,16
    80003c34:	854a                	mv	a0,s2
    80003c36:	00001097          	auipc	ra,0x1
    80003c3a:	ab8080e7          	jalr	-1352(ra) # 800046ee <acquiresleep>
    release(&itable.lock);
    80003c3e:	0001c517          	auipc	a0,0x1c
    80003c42:	01a50513          	addi	a0,a0,26 # 8001fc58 <itable>
    80003c46:	ffffd097          	auipc	ra,0xffffd
    80003c4a:	052080e7          	jalr	82(ra) # 80000c98 <release>
    itrunc(ip);
    80003c4e:	8526                	mv	a0,s1
    80003c50:	00000097          	auipc	ra,0x0
    80003c54:	ee2080e7          	jalr	-286(ra) # 80003b32 <itrunc>
    ip->type = 0;
    80003c58:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    80003c5c:	8526                	mv	a0,s1
    80003c5e:	00000097          	auipc	ra,0x0
    80003c62:	cfc080e7          	jalr	-772(ra) # 8000395a <iupdate>
    ip->valid = 0;
    80003c66:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    80003c6a:	854a                	mv	a0,s2
    80003c6c:	00001097          	auipc	ra,0x1
    80003c70:	ad8080e7          	jalr	-1320(ra) # 80004744 <releasesleep>
    acquire(&itable.lock);
    80003c74:	0001c517          	auipc	a0,0x1c
    80003c78:	fe450513          	addi	a0,a0,-28 # 8001fc58 <itable>
    80003c7c:	ffffd097          	auipc	ra,0xffffd
    80003c80:	f68080e7          	jalr	-152(ra) # 80000be4 <acquire>
    80003c84:	b741                	j	80003c04 <iput+0x26>

0000000080003c86 <iunlockput>:
{
    80003c86:	1101                	addi	sp,sp,-32
    80003c88:	ec06                	sd	ra,24(sp)
    80003c8a:	e822                	sd	s0,16(sp)
    80003c8c:	e426                	sd	s1,8(sp)
    80003c8e:	1000                	addi	s0,sp,32
    80003c90:	84aa                	mv	s1,a0
  iunlock(ip);
    80003c92:	00000097          	auipc	ra,0x0
    80003c96:	e54080e7          	jalr	-428(ra) # 80003ae6 <iunlock>
  iput(ip);
    80003c9a:	8526                	mv	a0,s1
    80003c9c:	00000097          	auipc	ra,0x0
    80003ca0:	f42080e7          	jalr	-190(ra) # 80003bde <iput>
}
    80003ca4:	60e2                	ld	ra,24(sp)
    80003ca6:	6442                	ld	s0,16(sp)
    80003ca8:	64a2                	ld	s1,8(sp)
    80003caa:	6105                	addi	sp,sp,32
    80003cac:	8082                	ret

0000000080003cae <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
    80003cae:	1141                	addi	sp,sp,-16
    80003cb0:	e422                	sd	s0,8(sp)
    80003cb2:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    80003cb4:	411c                	lw	a5,0(a0)
    80003cb6:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    80003cb8:	415c                	lw	a5,4(a0)
    80003cba:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    80003cbc:	04451783          	lh	a5,68(a0)
    80003cc0:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    80003cc4:	04a51783          	lh	a5,74(a0)
    80003cc8:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    80003ccc:	04c56783          	lwu	a5,76(a0)
    80003cd0:	e99c                	sd	a5,16(a1)
}
    80003cd2:	6422                	ld	s0,8(sp)
    80003cd4:	0141                	addi	sp,sp,16
    80003cd6:	8082                	ret

0000000080003cd8 <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003cd8:	457c                	lw	a5,76(a0)
    80003cda:	0ed7e963          	bltu	a5,a3,80003dcc <readi+0xf4>
{
    80003cde:	7159                	addi	sp,sp,-112
    80003ce0:	f486                	sd	ra,104(sp)
    80003ce2:	f0a2                	sd	s0,96(sp)
    80003ce4:	eca6                	sd	s1,88(sp)
    80003ce6:	e8ca                	sd	s2,80(sp)
    80003ce8:	e4ce                	sd	s3,72(sp)
    80003cea:	e0d2                	sd	s4,64(sp)
    80003cec:	fc56                	sd	s5,56(sp)
    80003cee:	f85a                	sd	s6,48(sp)
    80003cf0:	f45e                	sd	s7,40(sp)
    80003cf2:	f062                	sd	s8,32(sp)
    80003cf4:	ec66                	sd	s9,24(sp)
    80003cf6:	e86a                	sd	s10,16(sp)
    80003cf8:	e46e                	sd	s11,8(sp)
    80003cfa:	1880                	addi	s0,sp,112
    80003cfc:	8baa                	mv	s7,a0
    80003cfe:	8c2e                	mv	s8,a1
    80003d00:	8ab2                	mv	s5,a2
    80003d02:	84b6                	mv	s1,a3
    80003d04:	8b3a                	mv	s6,a4
  if(off > ip->size || off + n < off)
    80003d06:	9f35                	addw	a4,a4,a3
    return 0;
    80003d08:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    80003d0a:	0ad76063          	bltu	a4,a3,80003daa <readi+0xd2>
  if(off + n > ip->size)
    80003d0e:	00e7f463          	bgeu	a5,a4,80003d16 <readi+0x3e>
    n = ip->size - off;
    80003d12:	40d78b3b          	subw	s6,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003d16:	0a0b0963          	beqz	s6,80003dc8 <readi+0xf0>
    80003d1a:	4981                	li	s3,0
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    80003d1c:	40000d13          	li	s10,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    80003d20:	5cfd                	li	s9,-1
    80003d22:	a82d                	j	80003d5c <readi+0x84>
    80003d24:	020a1d93          	slli	s11,s4,0x20
    80003d28:	020ddd93          	srli	s11,s11,0x20
    80003d2c:	05890613          	addi	a2,s2,88
    80003d30:	86ee                	mv	a3,s11
    80003d32:	963a                	add	a2,a2,a4
    80003d34:	85d6                	mv	a1,s5
    80003d36:	8562                	mv	a0,s8
    80003d38:	fffff097          	auipc	ra,0xfffff
    80003d3c:	a76080e7          	jalr	-1418(ra) # 800027ae <either_copyout>
    80003d40:	05950d63          	beq	a0,s9,80003d9a <readi+0xc2>
      brelse(bp);
      tot = -1;
      break;
    }
    brelse(bp);
    80003d44:	854a                	mv	a0,s2
    80003d46:	fffff097          	auipc	ra,0xfffff
    80003d4a:	60c080e7          	jalr	1548(ra) # 80003352 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003d4e:	013a09bb          	addw	s3,s4,s3
    80003d52:	009a04bb          	addw	s1,s4,s1
    80003d56:	9aee                	add	s5,s5,s11
    80003d58:	0569f763          	bgeu	s3,s6,80003da6 <readi+0xce>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    80003d5c:	000ba903          	lw	s2,0(s7)
    80003d60:	00a4d59b          	srliw	a1,s1,0xa
    80003d64:	855e                	mv	a0,s7
    80003d66:	00000097          	auipc	ra,0x0
    80003d6a:	8b0080e7          	jalr	-1872(ra) # 80003616 <bmap>
    80003d6e:	0005059b          	sext.w	a1,a0
    80003d72:	854a                	mv	a0,s2
    80003d74:	fffff097          	auipc	ra,0xfffff
    80003d78:	4ae080e7          	jalr	1198(ra) # 80003222 <bread>
    80003d7c:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003d7e:	3ff4f713          	andi	a4,s1,1023
    80003d82:	40ed07bb          	subw	a5,s10,a4
    80003d86:	413b06bb          	subw	a3,s6,s3
    80003d8a:	8a3e                	mv	s4,a5
    80003d8c:	2781                	sext.w	a5,a5
    80003d8e:	0006861b          	sext.w	a2,a3
    80003d92:	f8f679e3          	bgeu	a2,a5,80003d24 <readi+0x4c>
    80003d96:	8a36                	mv	s4,a3
    80003d98:	b771                	j	80003d24 <readi+0x4c>
      brelse(bp);
    80003d9a:	854a                	mv	a0,s2
    80003d9c:	fffff097          	auipc	ra,0xfffff
    80003da0:	5b6080e7          	jalr	1462(ra) # 80003352 <brelse>
      tot = -1;
    80003da4:	59fd                	li	s3,-1
  }
  return tot;
    80003da6:	0009851b          	sext.w	a0,s3
}
    80003daa:	70a6                	ld	ra,104(sp)
    80003dac:	7406                	ld	s0,96(sp)
    80003dae:	64e6                	ld	s1,88(sp)
    80003db0:	6946                	ld	s2,80(sp)
    80003db2:	69a6                	ld	s3,72(sp)
    80003db4:	6a06                	ld	s4,64(sp)
    80003db6:	7ae2                	ld	s5,56(sp)
    80003db8:	7b42                	ld	s6,48(sp)
    80003dba:	7ba2                	ld	s7,40(sp)
    80003dbc:	7c02                	ld	s8,32(sp)
    80003dbe:	6ce2                	ld	s9,24(sp)
    80003dc0:	6d42                	ld	s10,16(sp)
    80003dc2:	6da2                	ld	s11,8(sp)
    80003dc4:	6165                	addi	sp,sp,112
    80003dc6:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003dc8:	89da                	mv	s3,s6
    80003dca:	bff1                	j	80003da6 <readi+0xce>
    return 0;
    80003dcc:	4501                	li	a0,0
}
    80003dce:	8082                	ret

0000000080003dd0 <writei>:
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003dd0:	457c                	lw	a5,76(a0)
    80003dd2:	10d7e863          	bltu	a5,a3,80003ee2 <writei+0x112>
{
    80003dd6:	7159                	addi	sp,sp,-112
    80003dd8:	f486                	sd	ra,104(sp)
    80003dda:	f0a2                	sd	s0,96(sp)
    80003ddc:	eca6                	sd	s1,88(sp)
    80003dde:	e8ca                	sd	s2,80(sp)
    80003de0:	e4ce                	sd	s3,72(sp)
    80003de2:	e0d2                	sd	s4,64(sp)
    80003de4:	fc56                	sd	s5,56(sp)
    80003de6:	f85a                	sd	s6,48(sp)
    80003de8:	f45e                	sd	s7,40(sp)
    80003dea:	f062                	sd	s8,32(sp)
    80003dec:	ec66                	sd	s9,24(sp)
    80003dee:	e86a                	sd	s10,16(sp)
    80003df0:	e46e                	sd	s11,8(sp)
    80003df2:	1880                	addi	s0,sp,112
    80003df4:	8b2a                	mv	s6,a0
    80003df6:	8c2e                	mv	s8,a1
    80003df8:	8ab2                	mv	s5,a2
    80003dfa:	8936                	mv	s2,a3
    80003dfc:	8bba                	mv	s7,a4
  if(off > ip->size || off + n < off)
    80003dfe:	00e687bb          	addw	a5,a3,a4
    80003e02:	0ed7e263          	bltu	a5,a3,80003ee6 <writei+0x116>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    80003e06:	00043737          	lui	a4,0x43
    80003e0a:	0ef76063          	bltu	a4,a5,80003eea <writei+0x11a>
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003e0e:	0c0b8863          	beqz	s7,80003ede <writei+0x10e>
    80003e12:	4a01                	li	s4,0
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    80003e14:	40000d13          	li	s10,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    80003e18:	5cfd                	li	s9,-1
    80003e1a:	a091                	j	80003e5e <writei+0x8e>
    80003e1c:	02099d93          	slli	s11,s3,0x20
    80003e20:	020ddd93          	srli	s11,s11,0x20
    80003e24:	05848513          	addi	a0,s1,88
    80003e28:	86ee                	mv	a3,s11
    80003e2a:	8656                	mv	a2,s5
    80003e2c:	85e2                	mv	a1,s8
    80003e2e:	953a                	add	a0,a0,a4
    80003e30:	fffff097          	auipc	ra,0xfffff
    80003e34:	9d4080e7          	jalr	-1580(ra) # 80002804 <either_copyin>
    80003e38:	07950263          	beq	a0,s9,80003e9c <writei+0xcc>
      brelse(bp);
      break;
    }
    log_write(bp);
    80003e3c:	8526                	mv	a0,s1
    80003e3e:	00000097          	auipc	ra,0x0
    80003e42:	790080e7          	jalr	1936(ra) # 800045ce <log_write>
    brelse(bp);
    80003e46:	8526                	mv	a0,s1
    80003e48:	fffff097          	auipc	ra,0xfffff
    80003e4c:	50a080e7          	jalr	1290(ra) # 80003352 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003e50:	01498a3b          	addw	s4,s3,s4
    80003e54:	0129893b          	addw	s2,s3,s2
    80003e58:	9aee                	add	s5,s5,s11
    80003e5a:	057a7663          	bgeu	s4,s7,80003ea6 <writei+0xd6>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    80003e5e:	000b2483          	lw	s1,0(s6)
    80003e62:	00a9559b          	srliw	a1,s2,0xa
    80003e66:	855a                	mv	a0,s6
    80003e68:	fffff097          	auipc	ra,0xfffff
    80003e6c:	7ae080e7          	jalr	1966(ra) # 80003616 <bmap>
    80003e70:	0005059b          	sext.w	a1,a0
    80003e74:	8526                	mv	a0,s1
    80003e76:	fffff097          	auipc	ra,0xfffff
    80003e7a:	3ac080e7          	jalr	940(ra) # 80003222 <bread>
    80003e7e:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003e80:	3ff97713          	andi	a4,s2,1023
    80003e84:	40ed07bb          	subw	a5,s10,a4
    80003e88:	414b86bb          	subw	a3,s7,s4
    80003e8c:	89be                	mv	s3,a5
    80003e8e:	2781                	sext.w	a5,a5
    80003e90:	0006861b          	sext.w	a2,a3
    80003e94:	f8f674e3          	bgeu	a2,a5,80003e1c <writei+0x4c>
    80003e98:	89b6                	mv	s3,a3
    80003e9a:	b749                	j	80003e1c <writei+0x4c>
      brelse(bp);
    80003e9c:	8526                	mv	a0,s1
    80003e9e:	fffff097          	auipc	ra,0xfffff
    80003ea2:	4b4080e7          	jalr	1204(ra) # 80003352 <brelse>
  }

  if(off > ip->size)
    80003ea6:	04cb2783          	lw	a5,76(s6)
    80003eaa:	0127f463          	bgeu	a5,s2,80003eb2 <writei+0xe2>
    ip->size = off;
    80003eae:	052b2623          	sw	s2,76(s6)

  // write the i-node back to disk even if the size didn't change
  // because the loop above might have called bmap() and added a new
  // block to ip->addrs[].
  iupdate(ip);
    80003eb2:	855a                	mv	a0,s6
    80003eb4:	00000097          	auipc	ra,0x0
    80003eb8:	aa6080e7          	jalr	-1370(ra) # 8000395a <iupdate>

  return tot;
    80003ebc:	000a051b          	sext.w	a0,s4
}
    80003ec0:	70a6                	ld	ra,104(sp)
    80003ec2:	7406                	ld	s0,96(sp)
    80003ec4:	64e6                	ld	s1,88(sp)
    80003ec6:	6946                	ld	s2,80(sp)
    80003ec8:	69a6                	ld	s3,72(sp)
    80003eca:	6a06                	ld	s4,64(sp)
    80003ecc:	7ae2                	ld	s5,56(sp)
    80003ece:	7b42                	ld	s6,48(sp)
    80003ed0:	7ba2                	ld	s7,40(sp)
    80003ed2:	7c02                	ld	s8,32(sp)
    80003ed4:	6ce2                	ld	s9,24(sp)
    80003ed6:	6d42                	ld	s10,16(sp)
    80003ed8:	6da2                	ld	s11,8(sp)
    80003eda:	6165                	addi	sp,sp,112
    80003edc:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003ede:	8a5e                	mv	s4,s7
    80003ee0:	bfc9                	j	80003eb2 <writei+0xe2>
    return -1;
    80003ee2:	557d                	li	a0,-1
}
    80003ee4:	8082                	ret
    return -1;
    80003ee6:	557d                	li	a0,-1
    80003ee8:	bfe1                	j	80003ec0 <writei+0xf0>
    return -1;
    80003eea:	557d                	li	a0,-1
    80003eec:	bfd1                	j	80003ec0 <writei+0xf0>

0000000080003eee <namecmp>:

// Directories

int
namecmp(const char *s, const char *t)
{
    80003eee:	1141                	addi	sp,sp,-16
    80003ef0:	e406                	sd	ra,8(sp)
    80003ef2:	e022                	sd	s0,0(sp)
    80003ef4:	0800                	addi	s0,sp,16
  return strncmp(s, t, DIRSIZ);
    80003ef6:	4639                	li	a2,14
    80003ef8:	ffffd097          	auipc	ra,0xffffd
    80003efc:	ec0080e7          	jalr	-320(ra) # 80000db8 <strncmp>
}
    80003f00:	60a2                	ld	ra,8(sp)
    80003f02:	6402                	ld	s0,0(sp)
    80003f04:	0141                	addi	sp,sp,16
    80003f06:	8082                	ret

0000000080003f08 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
    80003f08:	7139                	addi	sp,sp,-64
    80003f0a:	fc06                	sd	ra,56(sp)
    80003f0c:	f822                	sd	s0,48(sp)
    80003f0e:	f426                	sd	s1,40(sp)
    80003f10:	f04a                	sd	s2,32(sp)
    80003f12:	ec4e                	sd	s3,24(sp)
    80003f14:	e852                	sd	s4,16(sp)
    80003f16:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    80003f18:	04451703          	lh	a4,68(a0)
    80003f1c:	4785                	li	a5,1
    80003f1e:	00f71a63          	bne	a4,a5,80003f32 <dirlookup+0x2a>
    80003f22:	892a                	mv	s2,a0
    80003f24:	89ae                	mv	s3,a1
    80003f26:	8a32                	mv	s4,a2
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
    80003f28:	457c                	lw	a5,76(a0)
    80003f2a:	4481                	li	s1,0
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
    80003f2c:	4501                	li	a0,0
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003f2e:	e79d                	bnez	a5,80003f5c <dirlookup+0x54>
    80003f30:	a8a5                	j	80003fa8 <dirlookup+0xa0>
    panic("dirlookup not DIR");
    80003f32:	00004517          	auipc	a0,0x4
    80003f36:	6ce50513          	addi	a0,a0,1742 # 80008600 <syscalls+0x1b0>
    80003f3a:	ffffc097          	auipc	ra,0xffffc
    80003f3e:	604080e7          	jalr	1540(ra) # 8000053e <panic>
      panic("dirlookup read");
    80003f42:	00004517          	auipc	a0,0x4
    80003f46:	6d650513          	addi	a0,a0,1750 # 80008618 <syscalls+0x1c8>
    80003f4a:	ffffc097          	auipc	ra,0xffffc
    80003f4e:	5f4080e7          	jalr	1524(ra) # 8000053e <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003f52:	24c1                	addiw	s1,s1,16
    80003f54:	04c92783          	lw	a5,76(s2)
    80003f58:	04f4f763          	bgeu	s1,a5,80003fa6 <dirlookup+0x9e>
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003f5c:	4741                	li	a4,16
    80003f5e:	86a6                	mv	a3,s1
    80003f60:	fc040613          	addi	a2,s0,-64
    80003f64:	4581                	li	a1,0
    80003f66:	854a                	mv	a0,s2
    80003f68:	00000097          	auipc	ra,0x0
    80003f6c:	d70080e7          	jalr	-656(ra) # 80003cd8 <readi>
    80003f70:	47c1                	li	a5,16
    80003f72:	fcf518e3          	bne	a0,a5,80003f42 <dirlookup+0x3a>
    if(de.inum == 0)
    80003f76:	fc045783          	lhu	a5,-64(s0)
    80003f7a:	dfe1                	beqz	a5,80003f52 <dirlookup+0x4a>
    if(namecmp(name, de.name) == 0){
    80003f7c:	fc240593          	addi	a1,s0,-62
    80003f80:	854e                	mv	a0,s3
    80003f82:	00000097          	auipc	ra,0x0
    80003f86:	f6c080e7          	jalr	-148(ra) # 80003eee <namecmp>
    80003f8a:	f561                	bnez	a0,80003f52 <dirlookup+0x4a>
      if(poff)
    80003f8c:	000a0463          	beqz	s4,80003f94 <dirlookup+0x8c>
        *poff = off;
    80003f90:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    80003f94:	fc045583          	lhu	a1,-64(s0)
    80003f98:	00092503          	lw	a0,0(s2)
    80003f9c:	fffff097          	auipc	ra,0xfffff
    80003fa0:	754080e7          	jalr	1876(ra) # 800036f0 <iget>
    80003fa4:	a011                	j	80003fa8 <dirlookup+0xa0>
  return 0;
    80003fa6:	4501                	li	a0,0
}
    80003fa8:	70e2                	ld	ra,56(sp)
    80003faa:	7442                	ld	s0,48(sp)
    80003fac:	74a2                	ld	s1,40(sp)
    80003fae:	7902                	ld	s2,32(sp)
    80003fb0:	69e2                	ld	s3,24(sp)
    80003fb2:	6a42                	ld	s4,16(sp)
    80003fb4:	6121                	addi	sp,sp,64
    80003fb6:	8082                	ret

0000000080003fb8 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
    80003fb8:	711d                	addi	sp,sp,-96
    80003fba:	ec86                	sd	ra,88(sp)
    80003fbc:	e8a2                	sd	s0,80(sp)
    80003fbe:	e4a6                	sd	s1,72(sp)
    80003fc0:	e0ca                	sd	s2,64(sp)
    80003fc2:	fc4e                	sd	s3,56(sp)
    80003fc4:	f852                	sd	s4,48(sp)
    80003fc6:	f456                	sd	s5,40(sp)
    80003fc8:	f05a                	sd	s6,32(sp)
    80003fca:	ec5e                	sd	s7,24(sp)
    80003fcc:	e862                	sd	s8,16(sp)
    80003fce:	e466                	sd	s9,8(sp)
    80003fd0:	1080                	addi	s0,sp,96
    80003fd2:	84aa                	mv	s1,a0
    80003fd4:	8b2e                	mv	s6,a1
    80003fd6:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    80003fd8:	00054703          	lbu	a4,0(a0)
    80003fdc:	02f00793          	li	a5,47
    80003fe0:	02f70363          	beq	a4,a5,80004006 <namex+0x4e>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    80003fe4:	ffffe097          	auipc	ra,0xffffe
    80003fe8:	ba0080e7          	jalr	-1120(ra) # 80001b84 <myproc>
    80003fec:	15053503          	ld	a0,336(a0)
    80003ff0:	00000097          	auipc	ra,0x0
    80003ff4:	9f6080e7          	jalr	-1546(ra) # 800039e6 <idup>
    80003ff8:	89aa                	mv	s3,a0
  while(*path == '/')
    80003ffa:	02f00913          	li	s2,47
  len = path - s;
    80003ffe:	4b81                	li	s7,0
  if(len >= DIRSIZ)
    80004000:	4cb5                	li	s9,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    80004002:	4c05                	li	s8,1
    80004004:	a865                	j	800040bc <namex+0x104>
    ip = iget(ROOTDEV, ROOTINO);
    80004006:	4585                	li	a1,1
    80004008:	4505                	li	a0,1
    8000400a:	fffff097          	auipc	ra,0xfffff
    8000400e:	6e6080e7          	jalr	1766(ra) # 800036f0 <iget>
    80004012:	89aa                	mv	s3,a0
    80004014:	b7dd                	j	80003ffa <namex+0x42>
      iunlockput(ip);
    80004016:	854e                	mv	a0,s3
    80004018:	00000097          	auipc	ra,0x0
    8000401c:	c6e080e7          	jalr	-914(ra) # 80003c86 <iunlockput>
      return 0;
    80004020:	4981                	li	s3,0
  if(nameiparent){
    iput(ip);
    return 0;
  }
  return ip;
}
    80004022:	854e                	mv	a0,s3
    80004024:	60e6                	ld	ra,88(sp)
    80004026:	6446                	ld	s0,80(sp)
    80004028:	64a6                	ld	s1,72(sp)
    8000402a:	6906                	ld	s2,64(sp)
    8000402c:	79e2                	ld	s3,56(sp)
    8000402e:	7a42                	ld	s4,48(sp)
    80004030:	7aa2                	ld	s5,40(sp)
    80004032:	7b02                	ld	s6,32(sp)
    80004034:	6be2                	ld	s7,24(sp)
    80004036:	6c42                	ld	s8,16(sp)
    80004038:	6ca2                	ld	s9,8(sp)
    8000403a:	6125                	addi	sp,sp,96
    8000403c:	8082                	ret
      iunlock(ip);
    8000403e:	854e                	mv	a0,s3
    80004040:	00000097          	auipc	ra,0x0
    80004044:	aa6080e7          	jalr	-1370(ra) # 80003ae6 <iunlock>
      return ip;
    80004048:	bfe9                	j	80004022 <namex+0x6a>
      iunlockput(ip);
    8000404a:	854e                	mv	a0,s3
    8000404c:	00000097          	auipc	ra,0x0
    80004050:	c3a080e7          	jalr	-966(ra) # 80003c86 <iunlockput>
      return 0;
    80004054:	89d2                	mv	s3,s4
    80004056:	b7f1                	j	80004022 <namex+0x6a>
  len = path - s;
    80004058:	40b48633          	sub	a2,s1,a1
    8000405c:	00060a1b          	sext.w	s4,a2
  if(len >= DIRSIZ)
    80004060:	094cd463          	bge	s9,s4,800040e8 <namex+0x130>
    memmove(name, s, DIRSIZ);
    80004064:	4639                	li	a2,14
    80004066:	8556                	mv	a0,s5
    80004068:	ffffd097          	auipc	ra,0xffffd
    8000406c:	cd8080e7          	jalr	-808(ra) # 80000d40 <memmove>
  while(*path == '/')
    80004070:	0004c783          	lbu	a5,0(s1)
    80004074:	01279763          	bne	a5,s2,80004082 <namex+0xca>
    path++;
    80004078:	0485                	addi	s1,s1,1
  while(*path == '/')
    8000407a:	0004c783          	lbu	a5,0(s1)
    8000407e:	ff278de3          	beq	a5,s2,80004078 <namex+0xc0>
    ilock(ip);
    80004082:	854e                	mv	a0,s3
    80004084:	00000097          	auipc	ra,0x0
    80004088:	9a0080e7          	jalr	-1632(ra) # 80003a24 <ilock>
    if(ip->type != T_DIR){
    8000408c:	04499783          	lh	a5,68(s3)
    80004090:	f98793e3          	bne	a5,s8,80004016 <namex+0x5e>
    if(nameiparent && *path == '\0'){
    80004094:	000b0563          	beqz	s6,8000409e <namex+0xe6>
    80004098:	0004c783          	lbu	a5,0(s1)
    8000409c:	d3cd                	beqz	a5,8000403e <namex+0x86>
    if((next = dirlookup(ip, name, 0)) == 0){
    8000409e:	865e                	mv	a2,s7
    800040a0:	85d6                	mv	a1,s5
    800040a2:	854e                	mv	a0,s3
    800040a4:	00000097          	auipc	ra,0x0
    800040a8:	e64080e7          	jalr	-412(ra) # 80003f08 <dirlookup>
    800040ac:	8a2a                	mv	s4,a0
    800040ae:	dd51                	beqz	a0,8000404a <namex+0x92>
    iunlockput(ip);
    800040b0:	854e                	mv	a0,s3
    800040b2:	00000097          	auipc	ra,0x0
    800040b6:	bd4080e7          	jalr	-1068(ra) # 80003c86 <iunlockput>
    ip = next;
    800040ba:	89d2                	mv	s3,s4
  while(*path == '/')
    800040bc:	0004c783          	lbu	a5,0(s1)
    800040c0:	05279763          	bne	a5,s2,8000410e <namex+0x156>
    path++;
    800040c4:	0485                	addi	s1,s1,1
  while(*path == '/')
    800040c6:	0004c783          	lbu	a5,0(s1)
    800040ca:	ff278de3          	beq	a5,s2,800040c4 <namex+0x10c>
  if(*path == 0)
    800040ce:	c79d                	beqz	a5,800040fc <namex+0x144>
    path++;
    800040d0:	85a6                	mv	a1,s1
  len = path - s;
    800040d2:	8a5e                	mv	s4,s7
    800040d4:	865e                	mv	a2,s7
  while(*path != '/' && *path != 0)
    800040d6:	01278963          	beq	a5,s2,800040e8 <namex+0x130>
    800040da:	dfbd                	beqz	a5,80004058 <namex+0xa0>
    path++;
    800040dc:	0485                	addi	s1,s1,1
  while(*path != '/' && *path != 0)
    800040de:	0004c783          	lbu	a5,0(s1)
    800040e2:	ff279ce3          	bne	a5,s2,800040da <namex+0x122>
    800040e6:	bf8d                	j	80004058 <namex+0xa0>
    memmove(name, s, len);
    800040e8:	2601                	sext.w	a2,a2
    800040ea:	8556                	mv	a0,s5
    800040ec:	ffffd097          	auipc	ra,0xffffd
    800040f0:	c54080e7          	jalr	-940(ra) # 80000d40 <memmove>
    name[len] = 0;
    800040f4:	9a56                	add	s4,s4,s5
    800040f6:	000a0023          	sb	zero,0(s4)
    800040fa:	bf9d                	j	80004070 <namex+0xb8>
  if(nameiparent){
    800040fc:	f20b03e3          	beqz	s6,80004022 <namex+0x6a>
    iput(ip);
    80004100:	854e                	mv	a0,s3
    80004102:	00000097          	auipc	ra,0x0
    80004106:	adc080e7          	jalr	-1316(ra) # 80003bde <iput>
    return 0;
    8000410a:	4981                	li	s3,0
    8000410c:	bf19                	j	80004022 <namex+0x6a>
  if(*path == 0)
    8000410e:	d7fd                	beqz	a5,800040fc <namex+0x144>
  while(*path != '/' && *path != 0)
    80004110:	0004c783          	lbu	a5,0(s1)
    80004114:	85a6                	mv	a1,s1
    80004116:	b7d1                	j	800040da <namex+0x122>

0000000080004118 <dirlink>:
{
    80004118:	7139                	addi	sp,sp,-64
    8000411a:	fc06                	sd	ra,56(sp)
    8000411c:	f822                	sd	s0,48(sp)
    8000411e:	f426                	sd	s1,40(sp)
    80004120:	f04a                	sd	s2,32(sp)
    80004122:	ec4e                	sd	s3,24(sp)
    80004124:	e852                	sd	s4,16(sp)
    80004126:	0080                	addi	s0,sp,64
    80004128:	892a                	mv	s2,a0
    8000412a:	8a2e                	mv	s4,a1
    8000412c:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    8000412e:	4601                	li	a2,0
    80004130:	00000097          	auipc	ra,0x0
    80004134:	dd8080e7          	jalr	-552(ra) # 80003f08 <dirlookup>
    80004138:	e93d                	bnez	a0,800041ae <dirlink+0x96>
  for(off = 0; off < dp->size; off += sizeof(de)){
    8000413a:	04c92483          	lw	s1,76(s2)
    8000413e:	c49d                	beqz	s1,8000416c <dirlink+0x54>
    80004140:	4481                	li	s1,0
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004142:	4741                	li	a4,16
    80004144:	86a6                	mv	a3,s1
    80004146:	fc040613          	addi	a2,s0,-64
    8000414a:	4581                	li	a1,0
    8000414c:	854a                	mv	a0,s2
    8000414e:	00000097          	auipc	ra,0x0
    80004152:	b8a080e7          	jalr	-1142(ra) # 80003cd8 <readi>
    80004156:	47c1                	li	a5,16
    80004158:	06f51163          	bne	a0,a5,800041ba <dirlink+0xa2>
    if(de.inum == 0)
    8000415c:	fc045783          	lhu	a5,-64(s0)
    80004160:	c791                	beqz	a5,8000416c <dirlink+0x54>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80004162:	24c1                	addiw	s1,s1,16
    80004164:	04c92783          	lw	a5,76(s2)
    80004168:	fcf4ede3          	bltu	s1,a5,80004142 <dirlink+0x2a>
  strncpy(de.name, name, DIRSIZ);
    8000416c:	4639                	li	a2,14
    8000416e:	85d2                	mv	a1,s4
    80004170:	fc240513          	addi	a0,s0,-62
    80004174:	ffffd097          	auipc	ra,0xffffd
    80004178:	c80080e7          	jalr	-896(ra) # 80000df4 <strncpy>
  de.inum = inum;
    8000417c:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004180:	4741                	li	a4,16
    80004182:	86a6                	mv	a3,s1
    80004184:	fc040613          	addi	a2,s0,-64
    80004188:	4581                	li	a1,0
    8000418a:	854a                	mv	a0,s2
    8000418c:	00000097          	auipc	ra,0x0
    80004190:	c44080e7          	jalr	-956(ra) # 80003dd0 <writei>
    80004194:	872a                	mv	a4,a0
    80004196:	47c1                	li	a5,16
  return 0;
    80004198:	4501                	li	a0,0
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    8000419a:	02f71863          	bne	a4,a5,800041ca <dirlink+0xb2>
}
    8000419e:	70e2                	ld	ra,56(sp)
    800041a0:	7442                	ld	s0,48(sp)
    800041a2:	74a2                	ld	s1,40(sp)
    800041a4:	7902                	ld	s2,32(sp)
    800041a6:	69e2                	ld	s3,24(sp)
    800041a8:	6a42                	ld	s4,16(sp)
    800041aa:	6121                	addi	sp,sp,64
    800041ac:	8082                	ret
    iput(ip);
    800041ae:	00000097          	auipc	ra,0x0
    800041b2:	a30080e7          	jalr	-1488(ra) # 80003bde <iput>
    return -1;
    800041b6:	557d                	li	a0,-1
    800041b8:	b7dd                	j	8000419e <dirlink+0x86>
      panic("dirlink read");
    800041ba:	00004517          	auipc	a0,0x4
    800041be:	46e50513          	addi	a0,a0,1134 # 80008628 <syscalls+0x1d8>
    800041c2:	ffffc097          	auipc	ra,0xffffc
    800041c6:	37c080e7          	jalr	892(ra) # 8000053e <panic>
    panic("dirlink");
    800041ca:	00004517          	auipc	a0,0x4
    800041ce:	56e50513          	addi	a0,a0,1390 # 80008738 <syscalls+0x2e8>
    800041d2:	ffffc097          	auipc	ra,0xffffc
    800041d6:	36c080e7          	jalr	876(ra) # 8000053e <panic>

00000000800041da <namei>:

struct inode*
namei(char *path)
{
    800041da:	1101                	addi	sp,sp,-32
    800041dc:	ec06                	sd	ra,24(sp)
    800041de:	e822                	sd	s0,16(sp)
    800041e0:	1000                	addi	s0,sp,32
  char name[DIRSIZ];
  return namex(path, 0, name);
    800041e2:	fe040613          	addi	a2,s0,-32
    800041e6:	4581                	li	a1,0
    800041e8:	00000097          	auipc	ra,0x0
    800041ec:	dd0080e7          	jalr	-560(ra) # 80003fb8 <namex>
}
    800041f0:	60e2                	ld	ra,24(sp)
    800041f2:	6442                	ld	s0,16(sp)
    800041f4:	6105                	addi	sp,sp,32
    800041f6:	8082                	ret

00000000800041f8 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
    800041f8:	1141                	addi	sp,sp,-16
    800041fa:	e406                	sd	ra,8(sp)
    800041fc:	e022                	sd	s0,0(sp)
    800041fe:	0800                	addi	s0,sp,16
    80004200:	862e                	mv	a2,a1
  return namex(path, 1, name);
    80004202:	4585                	li	a1,1
    80004204:	00000097          	auipc	ra,0x0
    80004208:	db4080e7          	jalr	-588(ra) # 80003fb8 <namex>
}
    8000420c:	60a2                	ld	ra,8(sp)
    8000420e:	6402                	ld	s0,0(sp)
    80004210:	0141                	addi	sp,sp,16
    80004212:	8082                	ret

0000000080004214 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
    80004214:	1101                	addi	sp,sp,-32
    80004216:	ec06                	sd	ra,24(sp)
    80004218:	e822                	sd	s0,16(sp)
    8000421a:	e426                	sd	s1,8(sp)
    8000421c:	e04a                	sd	s2,0(sp)
    8000421e:	1000                	addi	s0,sp,32
  struct buf *buf = bread(log.dev, log.start);
    80004220:	0001d917          	auipc	s2,0x1d
    80004224:	4e090913          	addi	s2,s2,1248 # 80021700 <log>
    80004228:	01892583          	lw	a1,24(s2)
    8000422c:	02892503          	lw	a0,40(s2)
    80004230:	fffff097          	auipc	ra,0xfffff
    80004234:	ff2080e7          	jalr	-14(ra) # 80003222 <bread>
    80004238:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    8000423a:	02c92683          	lw	a3,44(s2)
    8000423e:	cd34                	sw	a3,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    80004240:	02d05763          	blez	a3,8000426e <write_head+0x5a>
    80004244:	0001d797          	auipc	a5,0x1d
    80004248:	4ec78793          	addi	a5,a5,1260 # 80021730 <log+0x30>
    8000424c:	05c50713          	addi	a4,a0,92
    80004250:	36fd                	addiw	a3,a3,-1
    80004252:	1682                	slli	a3,a3,0x20
    80004254:	9281                	srli	a3,a3,0x20
    80004256:	068a                	slli	a3,a3,0x2
    80004258:	0001d617          	auipc	a2,0x1d
    8000425c:	4dc60613          	addi	a2,a2,1244 # 80021734 <log+0x34>
    80004260:	96b2                	add	a3,a3,a2
    hb->block[i] = log.lh.block[i];
    80004262:	4390                	lw	a2,0(a5)
    80004264:	c310                	sw	a2,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80004266:	0791                	addi	a5,a5,4
    80004268:	0711                	addi	a4,a4,4
    8000426a:	fed79ce3          	bne	a5,a3,80004262 <write_head+0x4e>
  }
  bwrite(buf);
    8000426e:	8526                	mv	a0,s1
    80004270:	fffff097          	auipc	ra,0xfffff
    80004274:	0a4080e7          	jalr	164(ra) # 80003314 <bwrite>
  brelse(buf);
    80004278:	8526                	mv	a0,s1
    8000427a:	fffff097          	auipc	ra,0xfffff
    8000427e:	0d8080e7          	jalr	216(ra) # 80003352 <brelse>
}
    80004282:	60e2                	ld	ra,24(sp)
    80004284:	6442                	ld	s0,16(sp)
    80004286:	64a2                	ld	s1,8(sp)
    80004288:	6902                	ld	s2,0(sp)
    8000428a:	6105                	addi	sp,sp,32
    8000428c:	8082                	ret

000000008000428e <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    8000428e:	0001d797          	auipc	a5,0x1d
    80004292:	49e7a783          	lw	a5,1182(a5) # 8002172c <log+0x2c>
    80004296:	0af05d63          	blez	a5,80004350 <install_trans+0xc2>
{
    8000429a:	7139                	addi	sp,sp,-64
    8000429c:	fc06                	sd	ra,56(sp)
    8000429e:	f822                	sd	s0,48(sp)
    800042a0:	f426                	sd	s1,40(sp)
    800042a2:	f04a                	sd	s2,32(sp)
    800042a4:	ec4e                	sd	s3,24(sp)
    800042a6:	e852                	sd	s4,16(sp)
    800042a8:	e456                	sd	s5,8(sp)
    800042aa:	e05a                	sd	s6,0(sp)
    800042ac:	0080                	addi	s0,sp,64
    800042ae:	8b2a                	mv	s6,a0
    800042b0:	0001da97          	auipc	s5,0x1d
    800042b4:	480a8a93          	addi	s5,s5,1152 # 80021730 <log+0x30>
  for (tail = 0; tail < log.lh.n; tail++) {
    800042b8:	4a01                	li	s4,0
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    800042ba:	0001d997          	auipc	s3,0x1d
    800042be:	44698993          	addi	s3,s3,1094 # 80021700 <log>
    800042c2:	a035                	j	800042ee <install_trans+0x60>
      bunpin(dbuf);
    800042c4:	8526                	mv	a0,s1
    800042c6:	fffff097          	auipc	ra,0xfffff
    800042ca:	166080e7          	jalr	358(ra) # 8000342c <bunpin>
    brelse(lbuf);
    800042ce:	854a                	mv	a0,s2
    800042d0:	fffff097          	auipc	ra,0xfffff
    800042d4:	082080e7          	jalr	130(ra) # 80003352 <brelse>
    brelse(dbuf);
    800042d8:	8526                	mv	a0,s1
    800042da:	fffff097          	auipc	ra,0xfffff
    800042de:	078080e7          	jalr	120(ra) # 80003352 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    800042e2:	2a05                	addiw	s4,s4,1
    800042e4:	0a91                	addi	s5,s5,4
    800042e6:	02c9a783          	lw	a5,44(s3)
    800042ea:	04fa5963          	bge	s4,a5,8000433c <install_trans+0xae>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    800042ee:	0189a583          	lw	a1,24(s3)
    800042f2:	014585bb          	addw	a1,a1,s4
    800042f6:	2585                	addiw	a1,a1,1
    800042f8:	0289a503          	lw	a0,40(s3)
    800042fc:	fffff097          	auipc	ra,0xfffff
    80004300:	f26080e7          	jalr	-218(ra) # 80003222 <bread>
    80004304:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80004306:	000aa583          	lw	a1,0(s5)
    8000430a:	0289a503          	lw	a0,40(s3)
    8000430e:	fffff097          	auipc	ra,0xfffff
    80004312:	f14080e7          	jalr	-236(ra) # 80003222 <bread>
    80004316:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80004318:	40000613          	li	a2,1024
    8000431c:	05890593          	addi	a1,s2,88
    80004320:	05850513          	addi	a0,a0,88
    80004324:	ffffd097          	auipc	ra,0xffffd
    80004328:	a1c080e7          	jalr	-1508(ra) # 80000d40 <memmove>
    bwrite(dbuf);  // write dst to disk
    8000432c:	8526                	mv	a0,s1
    8000432e:	fffff097          	auipc	ra,0xfffff
    80004332:	fe6080e7          	jalr	-26(ra) # 80003314 <bwrite>
    if(recovering == 0)
    80004336:	f80b1ce3          	bnez	s6,800042ce <install_trans+0x40>
    8000433a:	b769                	j	800042c4 <install_trans+0x36>
}
    8000433c:	70e2                	ld	ra,56(sp)
    8000433e:	7442                	ld	s0,48(sp)
    80004340:	74a2                	ld	s1,40(sp)
    80004342:	7902                	ld	s2,32(sp)
    80004344:	69e2                	ld	s3,24(sp)
    80004346:	6a42                	ld	s4,16(sp)
    80004348:	6aa2                	ld	s5,8(sp)
    8000434a:	6b02                	ld	s6,0(sp)
    8000434c:	6121                	addi	sp,sp,64
    8000434e:	8082                	ret
    80004350:	8082                	ret

0000000080004352 <initlog>:
{
    80004352:	7179                	addi	sp,sp,-48
    80004354:	f406                	sd	ra,40(sp)
    80004356:	f022                	sd	s0,32(sp)
    80004358:	ec26                	sd	s1,24(sp)
    8000435a:	e84a                	sd	s2,16(sp)
    8000435c:	e44e                	sd	s3,8(sp)
    8000435e:	1800                	addi	s0,sp,48
    80004360:	892a                	mv	s2,a0
    80004362:	89ae                	mv	s3,a1
  initlock(&log.lock, "log");
    80004364:	0001d497          	auipc	s1,0x1d
    80004368:	39c48493          	addi	s1,s1,924 # 80021700 <log>
    8000436c:	00004597          	auipc	a1,0x4
    80004370:	2cc58593          	addi	a1,a1,716 # 80008638 <syscalls+0x1e8>
    80004374:	8526                	mv	a0,s1
    80004376:	ffffc097          	auipc	ra,0xffffc
    8000437a:	7de080e7          	jalr	2014(ra) # 80000b54 <initlock>
  log.start = sb->logstart;
    8000437e:	0149a583          	lw	a1,20(s3)
    80004382:	cc8c                	sw	a1,24(s1)
  log.size = sb->nlog;
    80004384:	0109a783          	lw	a5,16(s3)
    80004388:	ccdc                	sw	a5,28(s1)
  log.dev = dev;
    8000438a:	0324a423          	sw	s2,40(s1)
  struct buf *buf = bread(log.dev, log.start);
    8000438e:	854a                	mv	a0,s2
    80004390:	fffff097          	auipc	ra,0xfffff
    80004394:	e92080e7          	jalr	-366(ra) # 80003222 <bread>
  log.lh.n = lh->n;
    80004398:	4d3c                	lw	a5,88(a0)
    8000439a:	d4dc                	sw	a5,44(s1)
  for (i = 0; i < log.lh.n; i++) {
    8000439c:	02f05563          	blez	a5,800043c6 <initlog+0x74>
    800043a0:	05c50713          	addi	a4,a0,92
    800043a4:	0001d697          	auipc	a3,0x1d
    800043a8:	38c68693          	addi	a3,a3,908 # 80021730 <log+0x30>
    800043ac:	37fd                	addiw	a5,a5,-1
    800043ae:	1782                	slli	a5,a5,0x20
    800043b0:	9381                	srli	a5,a5,0x20
    800043b2:	078a                	slli	a5,a5,0x2
    800043b4:	06050613          	addi	a2,a0,96
    800043b8:	97b2                	add	a5,a5,a2
    log.lh.block[i] = lh->block[i];
    800043ba:	4310                	lw	a2,0(a4)
    800043bc:	c290                	sw	a2,0(a3)
  for (i = 0; i < log.lh.n; i++) {
    800043be:	0711                	addi	a4,a4,4
    800043c0:	0691                	addi	a3,a3,4
    800043c2:	fef71ce3          	bne	a4,a5,800043ba <initlog+0x68>
  brelse(buf);
    800043c6:	fffff097          	auipc	ra,0xfffff
    800043ca:	f8c080e7          	jalr	-116(ra) # 80003352 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(1); // if committed, copy from log to disk
    800043ce:	4505                	li	a0,1
    800043d0:	00000097          	auipc	ra,0x0
    800043d4:	ebe080e7          	jalr	-322(ra) # 8000428e <install_trans>
  log.lh.n = 0;
    800043d8:	0001d797          	auipc	a5,0x1d
    800043dc:	3407aa23          	sw	zero,852(a5) # 8002172c <log+0x2c>
  write_head(); // clear the log
    800043e0:	00000097          	auipc	ra,0x0
    800043e4:	e34080e7          	jalr	-460(ra) # 80004214 <write_head>
}
    800043e8:	70a2                	ld	ra,40(sp)
    800043ea:	7402                	ld	s0,32(sp)
    800043ec:	64e2                	ld	s1,24(sp)
    800043ee:	6942                	ld	s2,16(sp)
    800043f0:	69a2                	ld	s3,8(sp)
    800043f2:	6145                	addi	sp,sp,48
    800043f4:	8082                	ret

00000000800043f6 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
    800043f6:	1101                	addi	sp,sp,-32
    800043f8:	ec06                	sd	ra,24(sp)
    800043fa:	e822                	sd	s0,16(sp)
    800043fc:	e426                	sd	s1,8(sp)
    800043fe:	e04a                	sd	s2,0(sp)
    80004400:	1000                	addi	s0,sp,32
  acquire(&log.lock);
    80004402:	0001d517          	auipc	a0,0x1d
    80004406:	2fe50513          	addi	a0,a0,766 # 80021700 <log>
    8000440a:	ffffc097          	auipc	ra,0xffffc
    8000440e:	7da080e7          	jalr	2010(ra) # 80000be4 <acquire>
  while(1){
    if(log.committing){
    80004412:	0001d497          	auipc	s1,0x1d
    80004416:	2ee48493          	addi	s1,s1,750 # 80021700 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
    8000441a:	4979                	li	s2,30
    8000441c:	a039                	j	8000442a <begin_op+0x34>
      sleep(&log, &log.lock);
    8000441e:	85a6                	mv	a1,s1
    80004420:	8526                	mv	a0,s1
    80004422:	ffffe097          	auipc	ra,0xffffe
    80004426:	f62080e7          	jalr	-158(ra) # 80002384 <sleep>
    if(log.committing){
    8000442a:	50dc                	lw	a5,36(s1)
    8000442c:	fbed                	bnez	a5,8000441e <begin_op+0x28>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
    8000442e:	509c                	lw	a5,32(s1)
    80004430:	0017871b          	addiw	a4,a5,1
    80004434:	0007069b          	sext.w	a3,a4
    80004438:	0027179b          	slliw	a5,a4,0x2
    8000443c:	9fb9                	addw	a5,a5,a4
    8000443e:	0017979b          	slliw	a5,a5,0x1
    80004442:	54d8                	lw	a4,44(s1)
    80004444:	9fb9                	addw	a5,a5,a4
    80004446:	00f95963          	bge	s2,a5,80004458 <begin_op+0x62>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    8000444a:	85a6                	mv	a1,s1
    8000444c:	8526                	mv	a0,s1
    8000444e:	ffffe097          	auipc	ra,0xffffe
    80004452:	f36080e7          	jalr	-202(ra) # 80002384 <sleep>
    80004456:	bfd1                	j	8000442a <begin_op+0x34>
    } else {
      log.outstanding += 1;
    80004458:	0001d517          	auipc	a0,0x1d
    8000445c:	2a850513          	addi	a0,a0,680 # 80021700 <log>
    80004460:	d114                	sw	a3,32(a0)
      release(&log.lock);
    80004462:	ffffd097          	auipc	ra,0xffffd
    80004466:	836080e7          	jalr	-1994(ra) # 80000c98 <release>
      break;
    }
  }
}
    8000446a:	60e2                	ld	ra,24(sp)
    8000446c:	6442                	ld	s0,16(sp)
    8000446e:	64a2                	ld	s1,8(sp)
    80004470:	6902                	ld	s2,0(sp)
    80004472:	6105                	addi	sp,sp,32
    80004474:	8082                	ret

0000000080004476 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
    80004476:	7139                	addi	sp,sp,-64
    80004478:	fc06                	sd	ra,56(sp)
    8000447a:	f822                	sd	s0,48(sp)
    8000447c:	f426                	sd	s1,40(sp)
    8000447e:	f04a                	sd	s2,32(sp)
    80004480:	ec4e                	sd	s3,24(sp)
    80004482:	e852                	sd	s4,16(sp)
    80004484:	e456                	sd	s5,8(sp)
    80004486:	0080                	addi	s0,sp,64
  int do_commit = 0;

  acquire(&log.lock);
    80004488:	0001d497          	auipc	s1,0x1d
    8000448c:	27848493          	addi	s1,s1,632 # 80021700 <log>
    80004490:	8526                	mv	a0,s1
    80004492:	ffffc097          	auipc	ra,0xffffc
    80004496:	752080e7          	jalr	1874(ra) # 80000be4 <acquire>
  log.outstanding -= 1;
    8000449a:	509c                	lw	a5,32(s1)
    8000449c:	37fd                	addiw	a5,a5,-1
    8000449e:	0007891b          	sext.w	s2,a5
    800044a2:	d09c                	sw	a5,32(s1)
  if(log.committing)
    800044a4:	50dc                	lw	a5,36(s1)
    800044a6:	efb9                	bnez	a5,80004504 <end_op+0x8e>
    panic("log.committing");
  if(log.outstanding == 0){
    800044a8:	06091663          	bnez	s2,80004514 <end_op+0x9e>
    do_commit = 1;
    log.committing = 1;
    800044ac:	0001d497          	auipc	s1,0x1d
    800044b0:	25448493          	addi	s1,s1,596 # 80021700 <log>
    800044b4:	4785                	li	a5,1
    800044b6:	d0dc                	sw	a5,36(s1)
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
    800044b8:	8526                	mv	a0,s1
    800044ba:	ffffc097          	auipc	ra,0xffffc
    800044be:	7de080e7          	jalr	2014(ra) # 80000c98 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
    800044c2:	54dc                	lw	a5,44(s1)
    800044c4:	06f04763          	bgtz	a5,80004532 <end_op+0xbc>
    acquire(&log.lock);
    800044c8:	0001d497          	auipc	s1,0x1d
    800044cc:	23848493          	addi	s1,s1,568 # 80021700 <log>
    800044d0:	8526                	mv	a0,s1
    800044d2:	ffffc097          	auipc	ra,0xffffc
    800044d6:	712080e7          	jalr	1810(ra) # 80000be4 <acquire>
    log.committing = 0;
    800044da:	0204a223          	sw	zero,36(s1)
    wakeup(&log);
    800044de:	8526                	mv	a0,s1
    800044e0:	ffffe097          	auipc	ra,0xffffe
    800044e4:	042080e7          	jalr	66(ra) # 80002522 <wakeup>
    release(&log.lock);
    800044e8:	8526                	mv	a0,s1
    800044ea:	ffffc097          	auipc	ra,0xffffc
    800044ee:	7ae080e7          	jalr	1966(ra) # 80000c98 <release>
}
    800044f2:	70e2                	ld	ra,56(sp)
    800044f4:	7442                	ld	s0,48(sp)
    800044f6:	74a2                	ld	s1,40(sp)
    800044f8:	7902                	ld	s2,32(sp)
    800044fa:	69e2                	ld	s3,24(sp)
    800044fc:	6a42                	ld	s4,16(sp)
    800044fe:	6aa2                	ld	s5,8(sp)
    80004500:	6121                	addi	sp,sp,64
    80004502:	8082                	ret
    panic("log.committing");
    80004504:	00004517          	auipc	a0,0x4
    80004508:	13c50513          	addi	a0,a0,316 # 80008640 <syscalls+0x1f0>
    8000450c:	ffffc097          	auipc	ra,0xffffc
    80004510:	032080e7          	jalr	50(ra) # 8000053e <panic>
    wakeup(&log);
    80004514:	0001d497          	auipc	s1,0x1d
    80004518:	1ec48493          	addi	s1,s1,492 # 80021700 <log>
    8000451c:	8526                	mv	a0,s1
    8000451e:	ffffe097          	auipc	ra,0xffffe
    80004522:	004080e7          	jalr	4(ra) # 80002522 <wakeup>
  release(&log.lock);
    80004526:	8526                	mv	a0,s1
    80004528:	ffffc097          	auipc	ra,0xffffc
    8000452c:	770080e7          	jalr	1904(ra) # 80000c98 <release>
  if(do_commit){
    80004530:	b7c9                	j	800044f2 <end_op+0x7c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80004532:	0001da97          	auipc	s5,0x1d
    80004536:	1fea8a93          	addi	s5,s5,510 # 80021730 <log+0x30>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    8000453a:	0001da17          	auipc	s4,0x1d
    8000453e:	1c6a0a13          	addi	s4,s4,454 # 80021700 <log>
    80004542:	018a2583          	lw	a1,24(s4)
    80004546:	012585bb          	addw	a1,a1,s2
    8000454a:	2585                	addiw	a1,a1,1
    8000454c:	028a2503          	lw	a0,40(s4)
    80004550:	fffff097          	auipc	ra,0xfffff
    80004554:	cd2080e7          	jalr	-814(ra) # 80003222 <bread>
    80004558:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    8000455a:	000aa583          	lw	a1,0(s5)
    8000455e:	028a2503          	lw	a0,40(s4)
    80004562:	fffff097          	auipc	ra,0xfffff
    80004566:	cc0080e7          	jalr	-832(ra) # 80003222 <bread>
    8000456a:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    8000456c:	40000613          	li	a2,1024
    80004570:	05850593          	addi	a1,a0,88
    80004574:	05848513          	addi	a0,s1,88
    80004578:	ffffc097          	auipc	ra,0xffffc
    8000457c:	7c8080e7          	jalr	1992(ra) # 80000d40 <memmove>
    bwrite(to);  // write the log
    80004580:	8526                	mv	a0,s1
    80004582:	fffff097          	auipc	ra,0xfffff
    80004586:	d92080e7          	jalr	-622(ra) # 80003314 <bwrite>
    brelse(from);
    8000458a:	854e                	mv	a0,s3
    8000458c:	fffff097          	auipc	ra,0xfffff
    80004590:	dc6080e7          	jalr	-570(ra) # 80003352 <brelse>
    brelse(to);
    80004594:	8526                	mv	a0,s1
    80004596:	fffff097          	auipc	ra,0xfffff
    8000459a:	dbc080e7          	jalr	-580(ra) # 80003352 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    8000459e:	2905                	addiw	s2,s2,1
    800045a0:	0a91                	addi	s5,s5,4
    800045a2:	02ca2783          	lw	a5,44(s4)
    800045a6:	f8f94ee3          	blt	s2,a5,80004542 <end_op+0xcc>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    800045aa:	00000097          	auipc	ra,0x0
    800045ae:	c6a080e7          	jalr	-918(ra) # 80004214 <write_head>
    install_trans(0); // Now install writes to home locations
    800045b2:	4501                	li	a0,0
    800045b4:	00000097          	auipc	ra,0x0
    800045b8:	cda080e7          	jalr	-806(ra) # 8000428e <install_trans>
    log.lh.n = 0;
    800045bc:	0001d797          	auipc	a5,0x1d
    800045c0:	1607a823          	sw	zero,368(a5) # 8002172c <log+0x2c>
    write_head();    // Erase the transaction from the log
    800045c4:	00000097          	auipc	ra,0x0
    800045c8:	c50080e7          	jalr	-944(ra) # 80004214 <write_head>
    800045cc:	bdf5                	j	800044c8 <end_op+0x52>

00000000800045ce <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
    800045ce:	1101                	addi	sp,sp,-32
    800045d0:	ec06                	sd	ra,24(sp)
    800045d2:	e822                	sd	s0,16(sp)
    800045d4:	e426                	sd	s1,8(sp)
    800045d6:	e04a                	sd	s2,0(sp)
    800045d8:	1000                	addi	s0,sp,32
    800045da:	84aa                	mv	s1,a0
  int i;

  acquire(&log.lock);
    800045dc:	0001d917          	auipc	s2,0x1d
    800045e0:	12490913          	addi	s2,s2,292 # 80021700 <log>
    800045e4:	854a                	mv	a0,s2
    800045e6:	ffffc097          	auipc	ra,0xffffc
    800045ea:	5fe080e7          	jalr	1534(ra) # 80000be4 <acquire>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    800045ee:	02c92603          	lw	a2,44(s2)
    800045f2:	47f5                	li	a5,29
    800045f4:	06c7c563          	blt	a5,a2,8000465e <log_write+0x90>
    800045f8:	0001d797          	auipc	a5,0x1d
    800045fc:	1247a783          	lw	a5,292(a5) # 8002171c <log+0x1c>
    80004600:	37fd                	addiw	a5,a5,-1
    80004602:	04f65e63          	bge	a2,a5,8000465e <log_write+0x90>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80004606:	0001d797          	auipc	a5,0x1d
    8000460a:	11a7a783          	lw	a5,282(a5) # 80021720 <log+0x20>
    8000460e:	06f05063          	blez	a5,8000466e <log_write+0xa0>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80004612:	4781                	li	a5,0
    80004614:	06c05563          	blez	a2,8000467e <log_write+0xb0>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80004618:	44cc                	lw	a1,12(s1)
    8000461a:	0001d717          	auipc	a4,0x1d
    8000461e:	11670713          	addi	a4,a4,278 # 80021730 <log+0x30>
  for (i = 0; i < log.lh.n; i++) {
    80004622:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    80004624:	4314                	lw	a3,0(a4)
    80004626:	04b68c63          	beq	a3,a1,8000467e <log_write+0xb0>
  for (i = 0; i < log.lh.n; i++) {
    8000462a:	2785                	addiw	a5,a5,1
    8000462c:	0711                	addi	a4,a4,4
    8000462e:	fef61be3          	bne	a2,a5,80004624 <log_write+0x56>
      break;
  }
  log.lh.block[i] = b->blockno;
    80004632:	0621                	addi	a2,a2,8
    80004634:	060a                	slli	a2,a2,0x2
    80004636:	0001d797          	auipc	a5,0x1d
    8000463a:	0ca78793          	addi	a5,a5,202 # 80021700 <log>
    8000463e:	963e                	add	a2,a2,a5
    80004640:	44dc                	lw	a5,12(s1)
    80004642:	ca1c                	sw	a5,16(a2)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    80004644:	8526                	mv	a0,s1
    80004646:	fffff097          	auipc	ra,0xfffff
    8000464a:	daa080e7          	jalr	-598(ra) # 800033f0 <bpin>
    log.lh.n++;
    8000464e:	0001d717          	auipc	a4,0x1d
    80004652:	0b270713          	addi	a4,a4,178 # 80021700 <log>
    80004656:	575c                	lw	a5,44(a4)
    80004658:	2785                	addiw	a5,a5,1
    8000465a:	d75c                	sw	a5,44(a4)
    8000465c:	a835                	j	80004698 <log_write+0xca>
    panic("too big a transaction");
    8000465e:	00004517          	auipc	a0,0x4
    80004662:	ff250513          	addi	a0,a0,-14 # 80008650 <syscalls+0x200>
    80004666:	ffffc097          	auipc	ra,0xffffc
    8000466a:	ed8080e7          	jalr	-296(ra) # 8000053e <panic>
    panic("log_write outside of trans");
    8000466e:	00004517          	auipc	a0,0x4
    80004672:	ffa50513          	addi	a0,a0,-6 # 80008668 <syscalls+0x218>
    80004676:	ffffc097          	auipc	ra,0xffffc
    8000467a:	ec8080e7          	jalr	-312(ra) # 8000053e <panic>
  log.lh.block[i] = b->blockno;
    8000467e:	00878713          	addi	a4,a5,8
    80004682:	00271693          	slli	a3,a4,0x2
    80004686:	0001d717          	auipc	a4,0x1d
    8000468a:	07a70713          	addi	a4,a4,122 # 80021700 <log>
    8000468e:	9736                	add	a4,a4,a3
    80004690:	44d4                	lw	a3,12(s1)
    80004692:	cb14                	sw	a3,16(a4)
  if (i == log.lh.n) {  // Add new block to log?
    80004694:	faf608e3          	beq	a2,a5,80004644 <log_write+0x76>
  }
  release(&log.lock);
    80004698:	0001d517          	auipc	a0,0x1d
    8000469c:	06850513          	addi	a0,a0,104 # 80021700 <log>
    800046a0:	ffffc097          	auipc	ra,0xffffc
    800046a4:	5f8080e7          	jalr	1528(ra) # 80000c98 <release>
}
    800046a8:	60e2                	ld	ra,24(sp)
    800046aa:	6442                	ld	s0,16(sp)
    800046ac:	64a2                	ld	s1,8(sp)
    800046ae:	6902                	ld	s2,0(sp)
    800046b0:	6105                	addi	sp,sp,32
    800046b2:	8082                	ret

00000000800046b4 <initsleeplock>:
#include "proc.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
    800046b4:	1101                	addi	sp,sp,-32
    800046b6:	ec06                	sd	ra,24(sp)
    800046b8:	e822                	sd	s0,16(sp)
    800046ba:	e426                	sd	s1,8(sp)
    800046bc:	e04a                	sd	s2,0(sp)
    800046be:	1000                	addi	s0,sp,32
    800046c0:	84aa                	mv	s1,a0
    800046c2:	892e                	mv	s2,a1
  initlock(&lk->lk, "sleep lock");
    800046c4:	00004597          	auipc	a1,0x4
    800046c8:	fc458593          	addi	a1,a1,-60 # 80008688 <syscalls+0x238>
    800046cc:	0521                	addi	a0,a0,8
    800046ce:	ffffc097          	auipc	ra,0xffffc
    800046d2:	486080e7          	jalr	1158(ra) # 80000b54 <initlock>
  lk->name = name;
    800046d6:	0324b023          	sd	s2,32(s1)
  lk->locked = 0;
    800046da:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    800046de:	0204a423          	sw	zero,40(s1)
}
    800046e2:	60e2                	ld	ra,24(sp)
    800046e4:	6442                	ld	s0,16(sp)
    800046e6:	64a2                	ld	s1,8(sp)
    800046e8:	6902                	ld	s2,0(sp)
    800046ea:	6105                	addi	sp,sp,32
    800046ec:	8082                	ret

00000000800046ee <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
    800046ee:	1101                	addi	sp,sp,-32
    800046f0:	ec06                	sd	ra,24(sp)
    800046f2:	e822                	sd	s0,16(sp)
    800046f4:	e426                	sd	s1,8(sp)
    800046f6:	e04a                	sd	s2,0(sp)
    800046f8:	1000                	addi	s0,sp,32
    800046fa:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    800046fc:	00850913          	addi	s2,a0,8
    80004700:	854a                	mv	a0,s2
    80004702:	ffffc097          	auipc	ra,0xffffc
    80004706:	4e2080e7          	jalr	1250(ra) # 80000be4 <acquire>
  while (lk->locked) {
    8000470a:	409c                	lw	a5,0(s1)
    8000470c:	cb89                	beqz	a5,8000471e <acquiresleep+0x30>
    sleep(lk, &lk->lk);
    8000470e:	85ca                	mv	a1,s2
    80004710:	8526                	mv	a0,s1
    80004712:	ffffe097          	auipc	ra,0xffffe
    80004716:	c72080e7          	jalr	-910(ra) # 80002384 <sleep>
  while (lk->locked) {
    8000471a:	409c                	lw	a5,0(s1)
    8000471c:	fbed                	bnez	a5,8000470e <acquiresleep+0x20>
  }
  lk->locked = 1;
    8000471e:	4785                	li	a5,1
    80004720:	c09c                	sw	a5,0(s1)
  lk->pid = myproc()->pid;
    80004722:	ffffd097          	auipc	ra,0xffffd
    80004726:	462080e7          	jalr	1122(ra) # 80001b84 <myproc>
    8000472a:	591c                	lw	a5,48(a0)
    8000472c:	d49c                	sw	a5,40(s1)
  release(&lk->lk);
    8000472e:	854a                	mv	a0,s2
    80004730:	ffffc097          	auipc	ra,0xffffc
    80004734:	568080e7          	jalr	1384(ra) # 80000c98 <release>
}
    80004738:	60e2                	ld	ra,24(sp)
    8000473a:	6442                	ld	s0,16(sp)
    8000473c:	64a2                	ld	s1,8(sp)
    8000473e:	6902                	ld	s2,0(sp)
    80004740:	6105                	addi	sp,sp,32
    80004742:	8082                	ret

0000000080004744 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
    80004744:	1101                	addi	sp,sp,-32
    80004746:	ec06                	sd	ra,24(sp)
    80004748:	e822                	sd	s0,16(sp)
    8000474a:	e426                	sd	s1,8(sp)
    8000474c:	e04a                	sd	s2,0(sp)
    8000474e:	1000                	addi	s0,sp,32
    80004750:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80004752:	00850913          	addi	s2,a0,8
    80004756:	854a                	mv	a0,s2
    80004758:	ffffc097          	auipc	ra,0xffffc
    8000475c:	48c080e7          	jalr	1164(ra) # 80000be4 <acquire>
  lk->locked = 0;
    80004760:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80004764:	0204a423          	sw	zero,40(s1)
  wakeup(lk);
    80004768:	8526                	mv	a0,s1
    8000476a:	ffffe097          	auipc	ra,0xffffe
    8000476e:	db8080e7          	jalr	-584(ra) # 80002522 <wakeup>
  release(&lk->lk);
    80004772:	854a                	mv	a0,s2
    80004774:	ffffc097          	auipc	ra,0xffffc
    80004778:	524080e7          	jalr	1316(ra) # 80000c98 <release>
}
    8000477c:	60e2                	ld	ra,24(sp)
    8000477e:	6442                	ld	s0,16(sp)
    80004780:	64a2                	ld	s1,8(sp)
    80004782:	6902                	ld	s2,0(sp)
    80004784:	6105                	addi	sp,sp,32
    80004786:	8082                	ret

0000000080004788 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
    80004788:	7179                	addi	sp,sp,-48
    8000478a:	f406                	sd	ra,40(sp)
    8000478c:	f022                	sd	s0,32(sp)
    8000478e:	ec26                	sd	s1,24(sp)
    80004790:	e84a                	sd	s2,16(sp)
    80004792:	e44e                	sd	s3,8(sp)
    80004794:	1800                	addi	s0,sp,48
    80004796:	84aa                	mv	s1,a0
  int r;
  
  acquire(&lk->lk);
    80004798:	00850913          	addi	s2,a0,8
    8000479c:	854a                	mv	a0,s2
    8000479e:	ffffc097          	auipc	ra,0xffffc
    800047a2:	446080e7          	jalr	1094(ra) # 80000be4 <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
    800047a6:	409c                	lw	a5,0(s1)
    800047a8:	ef99                	bnez	a5,800047c6 <holdingsleep+0x3e>
    800047aa:	4481                	li	s1,0
  release(&lk->lk);
    800047ac:	854a                	mv	a0,s2
    800047ae:	ffffc097          	auipc	ra,0xffffc
    800047b2:	4ea080e7          	jalr	1258(ra) # 80000c98 <release>
  return r;
}
    800047b6:	8526                	mv	a0,s1
    800047b8:	70a2                	ld	ra,40(sp)
    800047ba:	7402                	ld	s0,32(sp)
    800047bc:	64e2                	ld	s1,24(sp)
    800047be:	6942                	ld	s2,16(sp)
    800047c0:	69a2                	ld	s3,8(sp)
    800047c2:	6145                	addi	sp,sp,48
    800047c4:	8082                	ret
  r = lk->locked && (lk->pid == myproc()->pid);
    800047c6:	0284a983          	lw	s3,40(s1)
    800047ca:	ffffd097          	auipc	ra,0xffffd
    800047ce:	3ba080e7          	jalr	954(ra) # 80001b84 <myproc>
    800047d2:	5904                	lw	s1,48(a0)
    800047d4:	413484b3          	sub	s1,s1,s3
    800047d8:	0014b493          	seqz	s1,s1
    800047dc:	bfc1                	j	800047ac <holdingsleep+0x24>

00000000800047de <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
    800047de:	1141                	addi	sp,sp,-16
    800047e0:	e406                	sd	ra,8(sp)
    800047e2:	e022                	sd	s0,0(sp)
    800047e4:	0800                	addi	s0,sp,16
  initlock(&ftable.lock, "ftable");
    800047e6:	00004597          	auipc	a1,0x4
    800047ea:	eb258593          	addi	a1,a1,-334 # 80008698 <syscalls+0x248>
    800047ee:	0001d517          	auipc	a0,0x1d
    800047f2:	05a50513          	addi	a0,a0,90 # 80021848 <ftable>
    800047f6:	ffffc097          	auipc	ra,0xffffc
    800047fa:	35e080e7          	jalr	862(ra) # 80000b54 <initlock>
}
    800047fe:	60a2                	ld	ra,8(sp)
    80004800:	6402                	ld	s0,0(sp)
    80004802:	0141                	addi	sp,sp,16
    80004804:	8082                	ret

0000000080004806 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
    80004806:	1101                	addi	sp,sp,-32
    80004808:	ec06                	sd	ra,24(sp)
    8000480a:	e822                	sd	s0,16(sp)
    8000480c:	e426                	sd	s1,8(sp)
    8000480e:	1000                	addi	s0,sp,32
  struct file *f;

  acquire(&ftable.lock);
    80004810:	0001d517          	auipc	a0,0x1d
    80004814:	03850513          	addi	a0,a0,56 # 80021848 <ftable>
    80004818:	ffffc097          	auipc	ra,0xffffc
    8000481c:	3cc080e7          	jalr	972(ra) # 80000be4 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80004820:	0001d497          	auipc	s1,0x1d
    80004824:	04048493          	addi	s1,s1,64 # 80021860 <ftable+0x18>
    80004828:	0001e717          	auipc	a4,0x1e
    8000482c:	fd870713          	addi	a4,a4,-40 # 80022800 <ftable+0xfb8>
    if(f->ref == 0){
    80004830:	40dc                	lw	a5,4(s1)
    80004832:	cf99                	beqz	a5,80004850 <filealloc+0x4a>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80004834:	02848493          	addi	s1,s1,40
    80004838:	fee49ce3          	bne	s1,a4,80004830 <filealloc+0x2a>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
    8000483c:	0001d517          	auipc	a0,0x1d
    80004840:	00c50513          	addi	a0,a0,12 # 80021848 <ftable>
    80004844:	ffffc097          	auipc	ra,0xffffc
    80004848:	454080e7          	jalr	1108(ra) # 80000c98 <release>
  return 0;
    8000484c:	4481                	li	s1,0
    8000484e:	a819                	j	80004864 <filealloc+0x5e>
      f->ref = 1;
    80004850:	4785                	li	a5,1
    80004852:	c0dc                	sw	a5,4(s1)
      release(&ftable.lock);
    80004854:	0001d517          	auipc	a0,0x1d
    80004858:	ff450513          	addi	a0,a0,-12 # 80021848 <ftable>
    8000485c:	ffffc097          	auipc	ra,0xffffc
    80004860:	43c080e7          	jalr	1084(ra) # 80000c98 <release>
}
    80004864:	8526                	mv	a0,s1
    80004866:	60e2                	ld	ra,24(sp)
    80004868:	6442                	ld	s0,16(sp)
    8000486a:	64a2                	ld	s1,8(sp)
    8000486c:	6105                	addi	sp,sp,32
    8000486e:	8082                	ret

0000000080004870 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
    80004870:	1101                	addi	sp,sp,-32
    80004872:	ec06                	sd	ra,24(sp)
    80004874:	e822                	sd	s0,16(sp)
    80004876:	e426                	sd	s1,8(sp)
    80004878:	1000                	addi	s0,sp,32
    8000487a:	84aa                	mv	s1,a0
  acquire(&ftable.lock);
    8000487c:	0001d517          	auipc	a0,0x1d
    80004880:	fcc50513          	addi	a0,a0,-52 # 80021848 <ftable>
    80004884:	ffffc097          	auipc	ra,0xffffc
    80004888:	360080e7          	jalr	864(ra) # 80000be4 <acquire>
  if(f->ref < 1)
    8000488c:	40dc                	lw	a5,4(s1)
    8000488e:	02f05263          	blez	a5,800048b2 <filedup+0x42>
    panic("filedup");
  f->ref++;
    80004892:	2785                	addiw	a5,a5,1
    80004894:	c0dc                	sw	a5,4(s1)
  release(&ftable.lock);
    80004896:	0001d517          	auipc	a0,0x1d
    8000489a:	fb250513          	addi	a0,a0,-78 # 80021848 <ftable>
    8000489e:	ffffc097          	auipc	ra,0xffffc
    800048a2:	3fa080e7          	jalr	1018(ra) # 80000c98 <release>
  return f;
}
    800048a6:	8526                	mv	a0,s1
    800048a8:	60e2                	ld	ra,24(sp)
    800048aa:	6442                	ld	s0,16(sp)
    800048ac:	64a2                	ld	s1,8(sp)
    800048ae:	6105                	addi	sp,sp,32
    800048b0:	8082                	ret
    panic("filedup");
    800048b2:	00004517          	auipc	a0,0x4
    800048b6:	dee50513          	addi	a0,a0,-530 # 800086a0 <syscalls+0x250>
    800048ba:	ffffc097          	auipc	ra,0xffffc
    800048be:	c84080e7          	jalr	-892(ra) # 8000053e <panic>

00000000800048c2 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
    800048c2:	7139                	addi	sp,sp,-64
    800048c4:	fc06                	sd	ra,56(sp)
    800048c6:	f822                	sd	s0,48(sp)
    800048c8:	f426                	sd	s1,40(sp)
    800048ca:	f04a                	sd	s2,32(sp)
    800048cc:	ec4e                	sd	s3,24(sp)
    800048ce:	e852                	sd	s4,16(sp)
    800048d0:	e456                	sd	s5,8(sp)
    800048d2:	0080                	addi	s0,sp,64
    800048d4:	84aa                	mv	s1,a0
  struct file ff;

  acquire(&ftable.lock);
    800048d6:	0001d517          	auipc	a0,0x1d
    800048da:	f7250513          	addi	a0,a0,-142 # 80021848 <ftable>
    800048de:	ffffc097          	auipc	ra,0xffffc
    800048e2:	306080e7          	jalr	774(ra) # 80000be4 <acquire>
  if(f->ref < 1)
    800048e6:	40dc                	lw	a5,4(s1)
    800048e8:	06f05163          	blez	a5,8000494a <fileclose+0x88>
    panic("fileclose");
  if(--f->ref > 0){
    800048ec:	37fd                	addiw	a5,a5,-1
    800048ee:	0007871b          	sext.w	a4,a5
    800048f2:	c0dc                	sw	a5,4(s1)
    800048f4:	06e04363          	bgtz	a4,8000495a <fileclose+0x98>
    release(&ftable.lock);
    return;
  }
  ff = *f;
    800048f8:	0004a903          	lw	s2,0(s1)
    800048fc:	0094ca83          	lbu	s5,9(s1)
    80004900:	0104ba03          	ld	s4,16(s1)
    80004904:	0184b983          	ld	s3,24(s1)
  f->ref = 0;
    80004908:	0004a223          	sw	zero,4(s1)
  f->type = FD_NONE;
    8000490c:	0004a023          	sw	zero,0(s1)
  release(&ftable.lock);
    80004910:	0001d517          	auipc	a0,0x1d
    80004914:	f3850513          	addi	a0,a0,-200 # 80021848 <ftable>
    80004918:	ffffc097          	auipc	ra,0xffffc
    8000491c:	380080e7          	jalr	896(ra) # 80000c98 <release>

  if(ff.type == FD_PIPE){
    80004920:	4785                	li	a5,1
    80004922:	04f90d63          	beq	s2,a5,8000497c <fileclose+0xba>
    pipeclose(ff.pipe, ff.writable);
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){
    80004926:	3979                	addiw	s2,s2,-2
    80004928:	4785                	li	a5,1
    8000492a:	0527e063          	bltu	a5,s2,8000496a <fileclose+0xa8>
    begin_op();
    8000492e:	00000097          	auipc	ra,0x0
    80004932:	ac8080e7          	jalr	-1336(ra) # 800043f6 <begin_op>
    iput(ff.ip);
    80004936:	854e                	mv	a0,s3
    80004938:	fffff097          	auipc	ra,0xfffff
    8000493c:	2a6080e7          	jalr	678(ra) # 80003bde <iput>
    end_op();
    80004940:	00000097          	auipc	ra,0x0
    80004944:	b36080e7          	jalr	-1226(ra) # 80004476 <end_op>
    80004948:	a00d                	j	8000496a <fileclose+0xa8>
    panic("fileclose");
    8000494a:	00004517          	auipc	a0,0x4
    8000494e:	d5e50513          	addi	a0,a0,-674 # 800086a8 <syscalls+0x258>
    80004952:	ffffc097          	auipc	ra,0xffffc
    80004956:	bec080e7          	jalr	-1044(ra) # 8000053e <panic>
    release(&ftable.lock);
    8000495a:	0001d517          	auipc	a0,0x1d
    8000495e:	eee50513          	addi	a0,a0,-274 # 80021848 <ftable>
    80004962:	ffffc097          	auipc	ra,0xffffc
    80004966:	336080e7          	jalr	822(ra) # 80000c98 <release>
  }
}
    8000496a:	70e2                	ld	ra,56(sp)
    8000496c:	7442                	ld	s0,48(sp)
    8000496e:	74a2                	ld	s1,40(sp)
    80004970:	7902                	ld	s2,32(sp)
    80004972:	69e2                	ld	s3,24(sp)
    80004974:	6a42                	ld	s4,16(sp)
    80004976:	6aa2                	ld	s5,8(sp)
    80004978:	6121                	addi	sp,sp,64
    8000497a:	8082                	ret
    pipeclose(ff.pipe, ff.writable);
    8000497c:	85d6                	mv	a1,s5
    8000497e:	8552                	mv	a0,s4
    80004980:	00000097          	auipc	ra,0x0
    80004984:	34c080e7          	jalr	844(ra) # 80004ccc <pipeclose>
    80004988:	b7cd                	j	8000496a <fileclose+0xa8>

000000008000498a <filestat>:

// Get metadata about file f.
// addr is a user virtual address, pointing to a struct stat.
int
filestat(struct file *f, uint64 addr)
{
    8000498a:	715d                	addi	sp,sp,-80
    8000498c:	e486                	sd	ra,72(sp)
    8000498e:	e0a2                	sd	s0,64(sp)
    80004990:	fc26                	sd	s1,56(sp)
    80004992:	f84a                	sd	s2,48(sp)
    80004994:	f44e                	sd	s3,40(sp)
    80004996:	0880                	addi	s0,sp,80
    80004998:	84aa                	mv	s1,a0
    8000499a:	89ae                	mv	s3,a1
  struct proc *p = myproc();
    8000499c:	ffffd097          	auipc	ra,0xffffd
    800049a0:	1e8080e7          	jalr	488(ra) # 80001b84 <myproc>
  struct stat st;
  
  if(f->type == FD_INODE || f->type == FD_DEVICE){
    800049a4:	409c                	lw	a5,0(s1)
    800049a6:	37f9                	addiw	a5,a5,-2
    800049a8:	4705                	li	a4,1
    800049aa:	04f76763          	bltu	a4,a5,800049f8 <filestat+0x6e>
    800049ae:	892a                	mv	s2,a0
    ilock(f->ip);
    800049b0:	6c88                	ld	a0,24(s1)
    800049b2:	fffff097          	auipc	ra,0xfffff
    800049b6:	072080e7          	jalr	114(ra) # 80003a24 <ilock>
    stati(f->ip, &st);
    800049ba:	fb840593          	addi	a1,s0,-72
    800049be:	6c88                	ld	a0,24(s1)
    800049c0:	fffff097          	auipc	ra,0xfffff
    800049c4:	2ee080e7          	jalr	750(ra) # 80003cae <stati>
    iunlock(f->ip);
    800049c8:	6c88                	ld	a0,24(s1)
    800049ca:	fffff097          	auipc	ra,0xfffff
    800049ce:	11c080e7          	jalr	284(ra) # 80003ae6 <iunlock>
    if(copyout(p->pagetable, addr, (char *)&st, sizeof(st)) < 0)
    800049d2:	46e1                	li	a3,24
    800049d4:	fb840613          	addi	a2,s0,-72
    800049d8:	85ce                	mv	a1,s3
    800049da:	05093503          	ld	a0,80(s2)
    800049de:	ffffd097          	auipc	ra,0xffffd
    800049e2:	c94080e7          	jalr	-876(ra) # 80001672 <copyout>
    800049e6:	41f5551b          	sraiw	a0,a0,0x1f
      return -1;
    return 0;
  }
  return -1;
}
    800049ea:	60a6                	ld	ra,72(sp)
    800049ec:	6406                	ld	s0,64(sp)
    800049ee:	74e2                	ld	s1,56(sp)
    800049f0:	7942                	ld	s2,48(sp)
    800049f2:	79a2                	ld	s3,40(sp)
    800049f4:	6161                	addi	sp,sp,80
    800049f6:	8082                	ret
  return -1;
    800049f8:	557d                	li	a0,-1
    800049fa:	bfc5                	j	800049ea <filestat+0x60>

00000000800049fc <fileread>:

// Read from file f.
// addr is a user virtual address.
int
fileread(struct file *f, uint64 addr, int n)
{
    800049fc:	7179                	addi	sp,sp,-48
    800049fe:	f406                	sd	ra,40(sp)
    80004a00:	f022                	sd	s0,32(sp)
    80004a02:	ec26                	sd	s1,24(sp)
    80004a04:	e84a                	sd	s2,16(sp)
    80004a06:	e44e                	sd	s3,8(sp)
    80004a08:	1800                	addi	s0,sp,48
  int r = 0;

  if(f->readable == 0)
    80004a0a:	00854783          	lbu	a5,8(a0)
    80004a0e:	c3d5                	beqz	a5,80004ab2 <fileread+0xb6>
    80004a10:	84aa                	mv	s1,a0
    80004a12:	89ae                	mv	s3,a1
    80004a14:	8932                	mv	s2,a2
    return -1;

  if(f->type == FD_PIPE){
    80004a16:	411c                	lw	a5,0(a0)
    80004a18:	4705                	li	a4,1
    80004a1a:	04e78963          	beq	a5,a4,80004a6c <fileread+0x70>
    r = piperead(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004a1e:	470d                	li	a4,3
    80004a20:	04e78d63          	beq	a5,a4,80004a7a <fileread+0x7e>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
    r = devsw[f->major].read(1, addr, n);
  } else if(f->type == FD_INODE){
    80004a24:	4709                	li	a4,2
    80004a26:	06e79e63          	bne	a5,a4,80004aa2 <fileread+0xa6>
    ilock(f->ip);
    80004a2a:	6d08                	ld	a0,24(a0)
    80004a2c:	fffff097          	auipc	ra,0xfffff
    80004a30:	ff8080e7          	jalr	-8(ra) # 80003a24 <ilock>
    if((r = readi(f->ip, 1, addr, f->off, n)) > 0)
    80004a34:	874a                	mv	a4,s2
    80004a36:	5094                	lw	a3,32(s1)
    80004a38:	864e                	mv	a2,s3
    80004a3a:	4585                	li	a1,1
    80004a3c:	6c88                	ld	a0,24(s1)
    80004a3e:	fffff097          	auipc	ra,0xfffff
    80004a42:	29a080e7          	jalr	666(ra) # 80003cd8 <readi>
    80004a46:	892a                	mv	s2,a0
    80004a48:	00a05563          	blez	a0,80004a52 <fileread+0x56>
      f->off += r;
    80004a4c:	509c                	lw	a5,32(s1)
    80004a4e:	9fa9                	addw	a5,a5,a0
    80004a50:	d09c                	sw	a5,32(s1)
    iunlock(f->ip);
    80004a52:	6c88                	ld	a0,24(s1)
    80004a54:	fffff097          	auipc	ra,0xfffff
    80004a58:	092080e7          	jalr	146(ra) # 80003ae6 <iunlock>
  } else {
    panic("fileread");
  }

  return r;
}
    80004a5c:	854a                	mv	a0,s2
    80004a5e:	70a2                	ld	ra,40(sp)
    80004a60:	7402                	ld	s0,32(sp)
    80004a62:	64e2                	ld	s1,24(sp)
    80004a64:	6942                	ld	s2,16(sp)
    80004a66:	69a2                	ld	s3,8(sp)
    80004a68:	6145                	addi	sp,sp,48
    80004a6a:	8082                	ret
    r = piperead(f->pipe, addr, n);
    80004a6c:	6908                	ld	a0,16(a0)
    80004a6e:	00000097          	auipc	ra,0x0
    80004a72:	3c8080e7          	jalr	968(ra) # 80004e36 <piperead>
    80004a76:	892a                	mv	s2,a0
    80004a78:	b7d5                	j	80004a5c <fileread+0x60>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
    80004a7a:	02451783          	lh	a5,36(a0)
    80004a7e:	03079693          	slli	a3,a5,0x30
    80004a82:	92c1                	srli	a3,a3,0x30
    80004a84:	4725                	li	a4,9
    80004a86:	02d76863          	bltu	a4,a3,80004ab6 <fileread+0xba>
    80004a8a:	0792                	slli	a5,a5,0x4
    80004a8c:	0001d717          	auipc	a4,0x1d
    80004a90:	d1c70713          	addi	a4,a4,-740 # 800217a8 <devsw>
    80004a94:	97ba                	add	a5,a5,a4
    80004a96:	639c                	ld	a5,0(a5)
    80004a98:	c38d                	beqz	a5,80004aba <fileread+0xbe>
    r = devsw[f->major].read(1, addr, n);
    80004a9a:	4505                	li	a0,1
    80004a9c:	9782                	jalr	a5
    80004a9e:	892a                	mv	s2,a0
    80004aa0:	bf75                	j	80004a5c <fileread+0x60>
    panic("fileread");
    80004aa2:	00004517          	auipc	a0,0x4
    80004aa6:	c1650513          	addi	a0,a0,-1002 # 800086b8 <syscalls+0x268>
    80004aaa:	ffffc097          	auipc	ra,0xffffc
    80004aae:	a94080e7          	jalr	-1388(ra) # 8000053e <panic>
    return -1;
    80004ab2:	597d                	li	s2,-1
    80004ab4:	b765                	j	80004a5c <fileread+0x60>
      return -1;
    80004ab6:	597d                	li	s2,-1
    80004ab8:	b755                	j	80004a5c <fileread+0x60>
    80004aba:	597d                	li	s2,-1
    80004abc:	b745                	j	80004a5c <fileread+0x60>

0000000080004abe <filewrite>:

// Write to file f.
// addr is a user virtual address.
int
filewrite(struct file *f, uint64 addr, int n)
{
    80004abe:	715d                	addi	sp,sp,-80
    80004ac0:	e486                	sd	ra,72(sp)
    80004ac2:	e0a2                	sd	s0,64(sp)
    80004ac4:	fc26                	sd	s1,56(sp)
    80004ac6:	f84a                	sd	s2,48(sp)
    80004ac8:	f44e                	sd	s3,40(sp)
    80004aca:	f052                	sd	s4,32(sp)
    80004acc:	ec56                	sd	s5,24(sp)
    80004ace:	e85a                	sd	s6,16(sp)
    80004ad0:	e45e                	sd	s7,8(sp)
    80004ad2:	e062                	sd	s8,0(sp)
    80004ad4:	0880                	addi	s0,sp,80
  int r, ret = 0;

  if(f->writable == 0)
    80004ad6:	00954783          	lbu	a5,9(a0)
    80004ada:	10078663          	beqz	a5,80004be6 <filewrite+0x128>
    80004ade:	892a                	mv	s2,a0
    80004ae0:	8aae                	mv	s5,a1
    80004ae2:	8a32                	mv	s4,a2
    return -1;

  if(f->type == FD_PIPE){
    80004ae4:	411c                	lw	a5,0(a0)
    80004ae6:	4705                	li	a4,1
    80004ae8:	02e78263          	beq	a5,a4,80004b0c <filewrite+0x4e>
    ret = pipewrite(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004aec:	470d                	li	a4,3
    80004aee:	02e78663          	beq	a5,a4,80004b1a <filewrite+0x5c>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
    ret = devsw[f->major].write(1, addr, n);
  } else if(f->type == FD_INODE){
    80004af2:	4709                	li	a4,2
    80004af4:	0ee79163          	bne	a5,a4,80004bd6 <filewrite+0x118>
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0;
    while(i < n){
    80004af8:	0ac05d63          	blez	a2,80004bb2 <filewrite+0xf4>
    int i = 0;
    80004afc:	4981                	li	s3,0
    80004afe:	6b05                	lui	s6,0x1
    80004b00:	c00b0b13          	addi	s6,s6,-1024 # c00 <_entry-0x7ffff400>
    80004b04:	6b85                	lui	s7,0x1
    80004b06:	c00b8b9b          	addiw	s7,s7,-1024
    80004b0a:	a861                	j	80004ba2 <filewrite+0xe4>
    ret = pipewrite(f->pipe, addr, n);
    80004b0c:	6908                	ld	a0,16(a0)
    80004b0e:	00000097          	auipc	ra,0x0
    80004b12:	22e080e7          	jalr	558(ra) # 80004d3c <pipewrite>
    80004b16:	8a2a                	mv	s4,a0
    80004b18:	a045                	j	80004bb8 <filewrite+0xfa>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
    80004b1a:	02451783          	lh	a5,36(a0)
    80004b1e:	03079693          	slli	a3,a5,0x30
    80004b22:	92c1                	srli	a3,a3,0x30
    80004b24:	4725                	li	a4,9
    80004b26:	0cd76263          	bltu	a4,a3,80004bea <filewrite+0x12c>
    80004b2a:	0792                	slli	a5,a5,0x4
    80004b2c:	0001d717          	auipc	a4,0x1d
    80004b30:	c7c70713          	addi	a4,a4,-900 # 800217a8 <devsw>
    80004b34:	97ba                	add	a5,a5,a4
    80004b36:	679c                	ld	a5,8(a5)
    80004b38:	cbdd                	beqz	a5,80004bee <filewrite+0x130>
    ret = devsw[f->major].write(1, addr, n);
    80004b3a:	4505                	li	a0,1
    80004b3c:	9782                	jalr	a5
    80004b3e:	8a2a                	mv	s4,a0
    80004b40:	a8a5                	j	80004bb8 <filewrite+0xfa>
    80004b42:	00048c1b          	sext.w	s8,s1
      int n1 = n - i;
      if(n1 > max)
        n1 = max;

      begin_op();
    80004b46:	00000097          	auipc	ra,0x0
    80004b4a:	8b0080e7          	jalr	-1872(ra) # 800043f6 <begin_op>
      ilock(f->ip);
    80004b4e:	01893503          	ld	a0,24(s2)
    80004b52:	fffff097          	auipc	ra,0xfffff
    80004b56:	ed2080e7          	jalr	-302(ra) # 80003a24 <ilock>
      if ((r = writei(f->ip, 1, addr + i, f->off, n1)) > 0)
    80004b5a:	8762                	mv	a4,s8
    80004b5c:	02092683          	lw	a3,32(s2)
    80004b60:	01598633          	add	a2,s3,s5
    80004b64:	4585                	li	a1,1
    80004b66:	01893503          	ld	a0,24(s2)
    80004b6a:	fffff097          	auipc	ra,0xfffff
    80004b6e:	266080e7          	jalr	614(ra) # 80003dd0 <writei>
    80004b72:	84aa                	mv	s1,a0
    80004b74:	00a05763          	blez	a0,80004b82 <filewrite+0xc4>
        f->off += r;
    80004b78:	02092783          	lw	a5,32(s2)
    80004b7c:	9fa9                	addw	a5,a5,a0
    80004b7e:	02f92023          	sw	a5,32(s2)
      iunlock(f->ip);
    80004b82:	01893503          	ld	a0,24(s2)
    80004b86:	fffff097          	auipc	ra,0xfffff
    80004b8a:	f60080e7          	jalr	-160(ra) # 80003ae6 <iunlock>
      end_op();
    80004b8e:	00000097          	auipc	ra,0x0
    80004b92:	8e8080e7          	jalr	-1816(ra) # 80004476 <end_op>

      if(r != n1){
    80004b96:	009c1f63          	bne	s8,s1,80004bb4 <filewrite+0xf6>
        // error from writei
        break;
      }
      i += r;
    80004b9a:	013489bb          	addw	s3,s1,s3
    while(i < n){
    80004b9e:	0149db63          	bge	s3,s4,80004bb4 <filewrite+0xf6>
      int n1 = n - i;
    80004ba2:	413a07bb          	subw	a5,s4,s3
      if(n1 > max)
    80004ba6:	84be                	mv	s1,a5
    80004ba8:	2781                	sext.w	a5,a5
    80004baa:	f8fb5ce3          	bge	s6,a5,80004b42 <filewrite+0x84>
    80004bae:	84de                	mv	s1,s7
    80004bb0:	bf49                	j	80004b42 <filewrite+0x84>
    int i = 0;
    80004bb2:	4981                	li	s3,0
    }
    ret = (i == n ? n : -1);
    80004bb4:	013a1f63          	bne	s4,s3,80004bd2 <filewrite+0x114>
  } else {
    panic("filewrite");
  }

  return ret;
}
    80004bb8:	8552                	mv	a0,s4
    80004bba:	60a6                	ld	ra,72(sp)
    80004bbc:	6406                	ld	s0,64(sp)
    80004bbe:	74e2                	ld	s1,56(sp)
    80004bc0:	7942                	ld	s2,48(sp)
    80004bc2:	79a2                	ld	s3,40(sp)
    80004bc4:	7a02                	ld	s4,32(sp)
    80004bc6:	6ae2                	ld	s5,24(sp)
    80004bc8:	6b42                	ld	s6,16(sp)
    80004bca:	6ba2                	ld	s7,8(sp)
    80004bcc:	6c02                	ld	s8,0(sp)
    80004bce:	6161                	addi	sp,sp,80
    80004bd0:	8082                	ret
    ret = (i == n ? n : -1);
    80004bd2:	5a7d                	li	s4,-1
    80004bd4:	b7d5                	j	80004bb8 <filewrite+0xfa>
    panic("filewrite");
    80004bd6:	00004517          	auipc	a0,0x4
    80004bda:	af250513          	addi	a0,a0,-1294 # 800086c8 <syscalls+0x278>
    80004bde:	ffffc097          	auipc	ra,0xffffc
    80004be2:	960080e7          	jalr	-1696(ra) # 8000053e <panic>
    return -1;
    80004be6:	5a7d                	li	s4,-1
    80004be8:	bfc1                	j	80004bb8 <filewrite+0xfa>
      return -1;
    80004bea:	5a7d                	li	s4,-1
    80004bec:	b7f1                	j	80004bb8 <filewrite+0xfa>
    80004bee:	5a7d                	li	s4,-1
    80004bf0:	b7e1                	j	80004bb8 <filewrite+0xfa>

0000000080004bf2 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
    80004bf2:	7179                	addi	sp,sp,-48
    80004bf4:	f406                	sd	ra,40(sp)
    80004bf6:	f022                	sd	s0,32(sp)
    80004bf8:	ec26                	sd	s1,24(sp)
    80004bfa:	e84a                	sd	s2,16(sp)
    80004bfc:	e44e                	sd	s3,8(sp)
    80004bfe:	e052                	sd	s4,0(sp)
    80004c00:	1800                	addi	s0,sp,48
    80004c02:	84aa                	mv	s1,a0
    80004c04:	8a2e                	mv	s4,a1
  struct pipe *pi;

  pi = 0;
  *f0 = *f1 = 0;
    80004c06:	0005b023          	sd	zero,0(a1)
    80004c0a:	00053023          	sd	zero,0(a0)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    80004c0e:	00000097          	auipc	ra,0x0
    80004c12:	bf8080e7          	jalr	-1032(ra) # 80004806 <filealloc>
    80004c16:	e088                	sd	a0,0(s1)
    80004c18:	c551                	beqz	a0,80004ca4 <pipealloc+0xb2>
    80004c1a:	00000097          	auipc	ra,0x0
    80004c1e:	bec080e7          	jalr	-1044(ra) # 80004806 <filealloc>
    80004c22:	00aa3023          	sd	a0,0(s4)
    80004c26:	c92d                	beqz	a0,80004c98 <pipealloc+0xa6>
    goto bad;
  if((pi = (struct pipe*)kalloc()) == 0)
    80004c28:	ffffc097          	auipc	ra,0xffffc
    80004c2c:	ecc080e7          	jalr	-308(ra) # 80000af4 <kalloc>
    80004c30:	892a                	mv	s2,a0
    80004c32:	c125                	beqz	a0,80004c92 <pipealloc+0xa0>
    goto bad;
  pi->readopen = 1;
    80004c34:	4985                	li	s3,1
    80004c36:	23352023          	sw	s3,544(a0)
  pi->writeopen = 1;
    80004c3a:	23352223          	sw	s3,548(a0)
  pi->nwrite = 0;
    80004c3e:	20052e23          	sw	zero,540(a0)
  pi->nread = 0;
    80004c42:	20052c23          	sw	zero,536(a0)
  initlock(&pi->lock, "pipe");
    80004c46:	00004597          	auipc	a1,0x4
    80004c4a:	a9258593          	addi	a1,a1,-1390 # 800086d8 <syscalls+0x288>
    80004c4e:	ffffc097          	auipc	ra,0xffffc
    80004c52:	f06080e7          	jalr	-250(ra) # 80000b54 <initlock>
  (*f0)->type = FD_PIPE;
    80004c56:	609c                	ld	a5,0(s1)
    80004c58:	0137a023          	sw	s3,0(a5)
  (*f0)->readable = 1;
    80004c5c:	609c                	ld	a5,0(s1)
    80004c5e:	01378423          	sb	s3,8(a5)
  (*f0)->writable = 0;
    80004c62:	609c                	ld	a5,0(s1)
    80004c64:	000784a3          	sb	zero,9(a5)
  (*f0)->pipe = pi;
    80004c68:	609c                	ld	a5,0(s1)
    80004c6a:	0127b823          	sd	s2,16(a5)
  (*f1)->type = FD_PIPE;
    80004c6e:	000a3783          	ld	a5,0(s4)
    80004c72:	0137a023          	sw	s3,0(a5)
  (*f1)->readable = 0;
    80004c76:	000a3783          	ld	a5,0(s4)
    80004c7a:	00078423          	sb	zero,8(a5)
  (*f1)->writable = 1;
    80004c7e:	000a3783          	ld	a5,0(s4)
    80004c82:	013784a3          	sb	s3,9(a5)
  (*f1)->pipe = pi;
    80004c86:	000a3783          	ld	a5,0(s4)
    80004c8a:	0127b823          	sd	s2,16(a5)
  return 0;
    80004c8e:	4501                	li	a0,0
    80004c90:	a025                	j	80004cb8 <pipealloc+0xc6>

 bad:
  if(pi)
    kfree((char*)pi);
  if(*f0)
    80004c92:	6088                	ld	a0,0(s1)
    80004c94:	e501                	bnez	a0,80004c9c <pipealloc+0xaa>
    80004c96:	a039                	j	80004ca4 <pipealloc+0xb2>
    80004c98:	6088                	ld	a0,0(s1)
    80004c9a:	c51d                	beqz	a0,80004cc8 <pipealloc+0xd6>
    fileclose(*f0);
    80004c9c:	00000097          	auipc	ra,0x0
    80004ca0:	c26080e7          	jalr	-986(ra) # 800048c2 <fileclose>
  if(*f1)
    80004ca4:	000a3783          	ld	a5,0(s4)
    fileclose(*f1);
  return -1;
    80004ca8:	557d                	li	a0,-1
  if(*f1)
    80004caa:	c799                	beqz	a5,80004cb8 <pipealloc+0xc6>
    fileclose(*f1);
    80004cac:	853e                	mv	a0,a5
    80004cae:	00000097          	auipc	ra,0x0
    80004cb2:	c14080e7          	jalr	-1004(ra) # 800048c2 <fileclose>
  return -1;
    80004cb6:	557d                	li	a0,-1
}
    80004cb8:	70a2                	ld	ra,40(sp)
    80004cba:	7402                	ld	s0,32(sp)
    80004cbc:	64e2                	ld	s1,24(sp)
    80004cbe:	6942                	ld	s2,16(sp)
    80004cc0:	69a2                	ld	s3,8(sp)
    80004cc2:	6a02                	ld	s4,0(sp)
    80004cc4:	6145                	addi	sp,sp,48
    80004cc6:	8082                	ret
  return -1;
    80004cc8:	557d                	li	a0,-1
    80004cca:	b7fd                	j	80004cb8 <pipealloc+0xc6>

0000000080004ccc <pipeclose>:

void
pipeclose(struct pipe *pi, int writable)
{
    80004ccc:	1101                	addi	sp,sp,-32
    80004cce:	ec06                	sd	ra,24(sp)
    80004cd0:	e822                	sd	s0,16(sp)
    80004cd2:	e426                	sd	s1,8(sp)
    80004cd4:	e04a                	sd	s2,0(sp)
    80004cd6:	1000                	addi	s0,sp,32
    80004cd8:	84aa                	mv	s1,a0
    80004cda:	892e                	mv	s2,a1
  acquire(&pi->lock);
    80004cdc:	ffffc097          	auipc	ra,0xffffc
    80004ce0:	f08080e7          	jalr	-248(ra) # 80000be4 <acquire>
  if(writable){
    80004ce4:	02090d63          	beqz	s2,80004d1e <pipeclose+0x52>
    pi->writeopen = 0;
    80004ce8:	2204a223          	sw	zero,548(s1)
    wakeup(&pi->nread);
    80004cec:	21848513          	addi	a0,s1,536
    80004cf0:	ffffe097          	auipc	ra,0xffffe
    80004cf4:	832080e7          	jalr	-1998(ra) # 80002522 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    80004cf8:	2204b783          	ld	a5,544(s1)
    80004cfc:	eb95                	bnez	a5,80004d30 <pipeclose+0x64>
    release(&pi->lock);
    80004cfe:	8526                	mv	a0,s1
    80004d00:	ffffc097          	auipc	ra,0xffffc
    80004d04:	f98080e7          	jalr	-104(ra) # 80000c98 <release>
    kfree((char*)pi);
    80004d08:	8526                	mv	a0,s1
    80004d0a:	ffffc097          	auipc	ra,0xffffc
    80004d0e:	cee080e7          	jalr	-786(ra) # 800009f8 <kfree>
  } else
    release(&pi->lock);
}
    80004d12:	60e2                	ld	ra,24(sp)
    80004d14:	6442                	ld	s0,16(sp)
    80004d16:	64a2                	ld	s1,8(sp)
    80004d18:	6902                	ld	s2,0(sp)
    80004d1a:	6105                	addi	sp,sp,32
    80004d1c:	8082                	ret
    pi->readopen = 0;
    80004d1e:	2204a023          	sw	zero,544(s1)
    wakeup(&pi->nwrite);
    80004d22:	21c48513          	addi	a0,s1,540
    80004d26:	ffffd097          	auipc	ra,0xffffd
    80004d2a:	7fc080e7          	jalr	2044(ra) # 80002522 <wakeup>
    80004d2e:	b7e9                	j	80004cf8 <pipeclose+0x2c>
    release(&pi->lock);
    80004d30:	8526                	mv	a0,s1
    80004d32:	ffffc097          	auipc	ra,0xffffc
    80004d36:	f66080e7          	jalr	-154(ra) # 80000c98 <release>
}
    80004d3a:	bfe1                	j	80004d12 <pipeclose+0x46>

0000000080004d3c <pipewrite>:

int
pipewrite(struct pipe *pi, uint64 addr, int n)
{
    80004d3c:	7159                	addi	sp,sp,-112
    80004d3e:	f486                	sd	ra,104(sp)
    80004d40:	f0a2                	sd	s0,96(sp)
    80004d42:	eca6                	sd	s1,88(sp)
    80004d44:	e8ca                	sd	s2,80(sp)
    80004d46:	e4ce                	sd	s3,72(sp)
    80004d48:	e0d2                	sd	s4,64(sp)
    80004d4a:	fc56                	sd	s5,56(sp)
    80004d4c:	f85a                	sd	s6,48(sp)
    80004d4e:	f45e                	sd	s7,40(sp)
    80004d50:	f062                	sd	s8,32(sp)
    80004d52:	ec66                	sd	s9,24(sp)
    80004d54:	1880                	addi	s0,sp,112
    80004d56:	84aa                	mv	s1,a0
    80004d58:	8aae                	mv	s5,a1
    80004d5a:	8a32                	mv	s4,a2
  int i = 0;
  struct proc *pr = myproc();
    80004d5c:	ffffd097          	auipc	ra,0xffffd
    80004d60:	e28080e7          	jalr	-472(ra) # 80001b84 <myproc>
    80004d64:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    80004d66:	8526                	mv	a0,s1
    80004d68:	ffffc097          	auipc	ra,0xffffc
    80004d6c:	e7c080e7          	jalr	-388(ra) # 80000be4 <acquire>
  while(i < n){
    80004d70:	0d405163          	blez	s4,80004e32 <pipewrite+0xf6>
    80004d74:	8ba6                	mv	s7,s1
  int i = 0;
    80004d76:	4901                	li	s2,0
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
      wakeup(&pi->nread);
      sleep(&pi->nwrite, &pi->lock);
    } else {
      char ch;
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004d78:	5b7d                	li	s6,-1
      wakeup(&pi->nread);
    80004d7a:	21848c93          	addi	s9,s1,536
      sleep(&pi->nwrite, &pi->lock);
    80004d7e:	21c48c13          	addi	s8,s1,540
    80004d82:	a08d                	j	80004de4 <pipewrite+0xa8>
      release(&pi->lock);
    80004d84:	8526                	mv	a0,s1
    80004d86:	ffffc097          	auipc	ra,0xffffc
    80004d8a:	f12080e7          	jalr	-238(ra) # 80000c98 <release>
      return -1;
    80004d8e:	597d                	li	s2,-1
  }
  wakeup(&pi->nread);
  release(&pi->lock);

  return i;
}
    80004d90:	854a                	mv	a0,s2
    80004d92:	70a6                	ld	ra,104(sp)
    80004d94:	7406                	ld	s0,96(sp)
    80004d96:	64e6                	ld	s1,88(sp)
    80004d98:	6946                	ld	s2,80(sp)
    80004d9a:	69a6                	ld	s3,72(sp)
    80004d9c:	6a06                	ld	s4,64(sp)
    80004d9e:	7ae2                	ld	s5,56(sp)
    80004da0:	7b42                	ld	s6,48(sp)
    80004da2:	7ba2                	ld	s7,40(sp)
    80004da4:	7c02                	ld	s8,32(sp)
    80004da6:	6ce2                	ld	s9,24(sp)
    80004da8:	6165                	addi	sp,sp,112
    80004daa:	8082                	ret
      wakeup(&pi->nread);
    80004dac:	8566                	mv	a0,s9
    80004dae:	ffffd097          	auipc	ra,0xffffd
    80004db2:	774080e7          	jalr	1908(ra) # 80002522 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    80004db6:	85de                	mv	a1,s7
    80004db8:	8562                	mv	a0,s8
    80004dba:	ffffd097          	auipc	ra,0xffffd
    80004dbe:	5ca080e7          	jalr	1482(ra) # 80002384 <sleep>
    80004dc2:	a839                	j	80004de0 <pipewrite+0xa4>
      pi->data[pi->nwrite++ % PIPESIZE] = ch;
    80004dc4:	21c4a783          	lw	a5,540(s1)
    80004dc8:	0017871b          	addiw	a4,a5,1
    80004dcc:	20e4ae23          	sw	a4,540(s1)
    80004dd0:	1ff7f793          	andi	a5,a5,511
    80004dd4:	97a6                	add	a5,a5,s1
    80004dd6:	f9f44703          	lbu	a4,-97(s0)
    80004dda:	00e78c23          	sb	a4,24(a5)
      i++;
    80004dde:	2905                	addiw	s2,s2,1
  while(i < n){
    80004de0:	03495d63          	bge	s2,s4,80004e1a <pipewrite+0xde>
    if(pi->readopen == 0 || pr->killed){
    80004de4:	2204a783          	lw	a5,544(s1)
    80004de8:	dfd1                	beqz	a5,80004d84 <pipewrite+0x48>
    80004dea:	0289a783          	lw	a5,40(s3)
    80004dee:	fbd9                	bnez	a5,80004d84 <pipewrite+0x48>
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
    80004df0:	2184a783          	lw	a5,536(s1)
    80004df4:	21c4a703          	lw	a4,540(s1)
    80004df8:	2007879b          	addiw	a5,a5,512
    80004dfc:	faf708e3          	beq	a4,a5,80004dac <pipewrite+0x70>
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004e00:	4685                	li	a3,1
    80004e02:	01590633          	add	a2,s2,s5
    80004e06:	f9f40593          	addi	a1,s0,-97
    80004e0a:	0509b503          	ld	a0,80(s3)
    80004e0e:	ffffd097          	auipc	ra,0xffffd
    80004e12:	8f0080e7          	jalr	-1808(ra) # 800016fe <copyin>
    80004e16:	fb6517e3          	bne	a0,s6,80004dc4 <pipewrite+0x88>
  wakeup(&pi->nread);
    80004e1a:	21848513          	addi	a0,s1,536
    80004e1e:	ffffd097          	auipc	ra,0xffffd
    80004e22:	704080e7          	jalr	1796(ra) # 80002522 <wakeup>
  release(&pi->lock);
    80004e26:	8526                	mv	a0,s1
    80004e28:	ffffc097          	auipc	ra,0xffffc
    80004e2c:	e70080e7          	jalr	-400(ra) # 80000c98 <release>
  return i;
    80004e30:	b785                	j	80004d90 <pipewrite+0x54>
  int i = 0;
    80004e32:	4901                	li	s2,0
    80004e34:	b7dd                	j	80004e1a <pipewrite+0xde>

0000000080004e36 <piperead>:

int
piperead(struct pipe *pi, uint64 addr, int n)
{
    80004e36:	715d                	addi	sp,sp,-80
    80004e38:	e486                	sd	ra,72(sp)
    80004e3a:	e0a2                	sd	s0,64(sp)
    80004e3c:	fc26                	sd	s1,56(sp)
    80004e3e:	f84a                	sd	s2,48(sp)
    80004e40:	f44e                	sd	s3,40(sp)
    80004e42:	f052                	sd	s4,32(sp)
    80004e44:	ec56                	sd	s5,24(sp)
    80004e46:	e85a                	sd	s6,16(sp)
    80004e48:	0880                	addi	s0,sp,80
    80004e4a:	84aa                	mv	s1,a0
    80004e4c:	892e                	mv	s2,a1
    80004e4e:	8ab2                	mv	s5,a2
  int i;
  struct proc *pr = myproc();
    80004e50:	ffffd097          	auipc	ra,0xffffd
    80004e54:	d34080e7          	jalr	-716(ra) # 80001b84 <myproc>
    80004e58:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004e5a:	8b26                	mv	s6,s1
    80004e5c:	8526                	mv	a0,s1
    80004e5e:	ffffc097          	auipc	ra,0xffffc
    80004e62:	d86080e7          	jalr	-634(ra) # 80000be4 <acquire>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004e66:	2184a703          	lw	a4,536(s1)
    80004e6a:	21c4a783          	lw	a5,540(s1)
    if(pr->killed){
      release(&pi->lock);
      return -1;
    }
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004e6e:	21848993          	addi	s3,s1,536
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004e72:	02f71463          	bne	a4,a5,80004e9a <piperead+0x64>
    80004e76:	2244a783          	lw	a5,548(s1)
    80004e7a:	c385                	beqz	a5,80004e9a <piperead+0x64>
    if(pr->killed){
    80004e7c:	028a2783          	lw	a5,40(s4)
    80004e80:	ebc1                	bnez	a5,80004f10 <piperead+0xda>
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004e82:	85da                	mv	a1,s6
    80004e84:	854e                	mv	a0,s3
    80004e86:	ffffd097          	auipc	ra,0xffffd
    80004e8a:	4fe080e7          	jalr	1278(ra) # 80002384 <sleep>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004e8e:	2184a703          	lw	a4,536(s1)
    80004e92:	21c4a783          	lw	a5,540(s1)
    80004e96:	fef700e3          	beq	a4,a5,80004e76 <piperead+0x40>
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004e9a:	09505263          	blez	s5,80004f1e <piperead+0xe8>
    80004e9e:	4981                	li	s3,0
    if(pi->nread == pi->nwrite)
      break;
    ch = pi->data[pi->nread++ % PIPESIZE];
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    80004ea0:	5b7d                	li	s6,-1
    if(pi->nread == pi->nwrite)
    80004ea2:	2184a783          	lw	a5,536(s1)
    80004ea6:	21c4a703          	lw	a4,540(s1)
    80004eaa:	02f70d63          	beq	a4,a5,80004ee4 <piperead+0xae>
    ch = pi->data[pi->nread++ % PIPESIZE];
    80004eae:	0017871b          	addiw	a4,a5,1
    80004eb2:	20e4ac23          	sw	a4,536(s1)
    80004eb6:	1ff7f793          	andi	a5,a5,511
    80004eba:	97a6                	add	a5,a5,s1
    80004ebc:	0187c783          	lbu	a5,24(a5)
    80004ec0:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    80004ec4:	4685                	li	a3,1
    80004ec6:	fbf40613          	addi	a2,s0,-65
    80004eca:	85ca                	mv	a1,s2
    80004ecc:	050a3503          	ld	a0,80(s4)
    80004ed0:	ffffc097          	auipc	ra,0xffffc
    80004ed4:	7a2080e7          	jalr	1954(ra) # 80001672 <copyout>
    80004ed8:	01650663          	beq	a0,s6,80004ee4 <piperead+0xae>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004edc:	2985                	addiw	s3,s3,1
    80004ede:	0905                	addi	s2,s2,1
    80004ee0:	fd3a91e3          	bne	s5,s3,80004ea2 <piperead+0x6c>
      break;
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    80004ee4:	21c48513          	addi	a0,s1,540
    80004ee8:	ffffd097          	auipc	ra,0xffffd
    80004eec:	63a080e7          	jalr	1594(ra) # 80002522 <wakeup>
  release(&pi->lock);
    80004ef0:	8526                	mv	a0,s1
    80004ef2:	ffffc097          	auipc	ra,0xffffc
    80004ef6:	da6080e7          	jalr	-602(ra) # 80000c98 <release>
  return i;
}
    80004efa:	854e                	mv	a0,s3
    80004efc:	60a6                	ld	ra,72(sp)
    80004efe:	6406                	ld	s0,64(sp)
    80004f00:	74e2                	ld	s1,56(sp)
    80004f02:	7942                	ld	s2,48(sp)
    80004f04:	79a2                	ld	s3,40(sp)
    80004f06:	7a02                	ld	s4,32(sp)
    80004f08:	6ae2                	ld	s5,24(sp)
    80004f0a:	6b42                	ld	s6,16(sp)
    80004f0c:	6161                	addi	sp,sp,80
    80004f0e:	8082                	ret
      release(&pi->lock);
    80004f10:	8526                	mv	a0,s1
    80004f12:	ffffc097          	auipc	ra,0xffffc
    80004f16:	d86080e7          	jalr	-634(ra) # 80000c98 <release>
      return -1;
    80004f1a:	59fd                	li	s3,-1
    80004f1c:	bff9                	j	80004efa <piperead+0xc4>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004f1e:	4981                	li	s3,0
    80004f20:	b7d1                	j	80004ee4 <piperead+0xae>

0000000080004f22 <exec>:

static int loadseg(pde_t *pgdir, uint64 addr, struct inode *ip, uint offset, uint sz);

int
exec(char *path, char **argv)
{
    80004f22:	df010113          	addi	sp,sp,-528
    80004f26:	20113423          	sd	ra,520(sp)
    80004f2a:	20813023          	sd	s0,512(sp)
    80004f2e:	ffa6                	sd	s1,504(sp)
    80004f30:	fbca                	sd	s2,496(sp)
    80004f32:	f7ce                	sd	s3,488(sp)
    80004f34:	f3d2                	sd	s4,480(sp)
    80004f36:	efd6                	sd	s5,472(sp)
    80004f38:	ebda                	sd	s6,464(sp)
    80004f3a:	e7de                	sd	s7,456(sp)
    80004f3c:	e3e2                	sd	s8,448(sp)
    80004f3e:	ff66                	sd	s9,440(sp)
    80004f40:	fb6a                	sd	s10,432(sp)
    80004f42:	f76e                	sd	s11,424(sp)
    80004f44:	0c00                	addi	s0,sp,528
    80004f46:	84aa                	mv	s1,a0
    80004f48:	dea43c23          	sd	a0,-520(s0)
    80004f4c:	e0b43023          	sd	a1,-512(s0)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  struct proc *p = myproc();
    80004f50:	ffffd097          	auipc	ra,0xffffd
    80004f54:	c34080e7          	jalr	-972(ra) # 80001b84 <myproc>
    80004f58:	892a                	mv	s2,a0

  begin_op();
    80004f5a:	fffff097          	auipc	ra,0xfffff
    80004f5e:	49c080e7          	jalr	1180(ra) # 800043f6 <begin_op>

  if((ip = namei(path)) == 0){
    80004f62:	8526                	mv	a0,s1
    80004f64:	fffff097          	auipc	ra,0xfffff
    80004f68:	276080e7          	jalr	630(ra) # 800041da <namei>
    80004f6c:	c92d                	beqz	a0,80004fde <exec+0xbc>
    80004f6e:	84aa                	mv	s1,a0
    end_op();
    return -1;
  }
  ilock(ip);
    80004f70:	fffff097          	auipc	ra,0xfffff
    80004f74:	ab4080e7          	jalr	-1356(ra) # 80003a24 <ilock>

  // Check ELF header
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    80004f78:	04000713          	li	a4,64
    80004f7c:	4681                	li	a3,0
    80004f7e:	e5040613          	addi	a2,s0,-432
    80004f82:	4581                	li	a1,0
    80004f84:	8526                	mv	a0,s1
    80004f86:	fffff097          	auipc	ra,0xfffff
    80004f8a:	d52080e7          	jalr	-686(ra) # 80003cd8 <readi>
    80004f8e:	04000793          	li	a5,64
    80004f92:	00f51a63          	bne	a0,a5,80004fa6 <exec+0x84>
    goto bad;
  if(elf.magic != ELF_MAGIC)
    80004f96:	e5042703          	lw	a4,-432(s0)
    80004f9a:	464c47b7          	lui	a5,0x464c4
    80004f9e:	57f78793          	addi	a5,a5,1407 # 464c457f <_entry-0x39b3ba81>
    80004fa2:	04f70463          	beq	a4,a5,80004fea <exec+0xc8>

 bad:
  if(pagetable)
    proc_freepagetable(pagetable, sz);
  if(ip){
    iunlockput(ip);
    80004fa6:	8526                	mv	a0,s1
    80004fa8:	fffff097          	auipc	ra,0xfffff
    80004fac:	cde080e7          	jalr	-802(ra) # 80003c86 <iunlockput>
    end_op();
    80004fb0:	fffff097          	auipc	ra,0xfffff
    80004fb4:	4c6080e7          	jalr	1222(ra) # 80004476 <end_op>
  }
  return -1;
    80004fb8:	557d                	li	a0,-1
}
    80004fba:	20813083          	ld	ra,520(sp)
    80004fbe:	20013403          	ld	s0,512(sp)
    80004fc2:	74fe                	ld	s1,504(sp)
    80004fc4:	795e                	ld	s2,496(sp)
    80004fc6:	79be                	ld	s3,488(sp)
    80004fc8:	7a1e                	ld	s4,480(sp)
    80004fca:	6afe                	ld	s5,472(sp)
    80004fcc:	6b5e                	ld	s6,464(sp)
    80004fce:	6bbe                	ld	s7,456(sp)
    80004fd0:	6c1e                	ld	s8,448(sp)
    80004fd2:	7cfa                	ld	s9,440(sp)
    80004fd4:	7d5a                	ld	s10,432(sp)
    80004fd6:	7dba                	ld	s11,424(sp)
    80004fd8:	21010113          	addi	sp,sp,528
    80004fdc:	8082                	ret
    end_op();
    80004fde:	fffff097          	auipc	ra,0xfffff
    80004fe2:	498080e7          	jalr	1176(ra) # 80004476 <end_op>
    return -1;
    80004fe6:	557d                	li	a0,-1
    80004fe8:	bfc9                	j	80004fba <exec+0x98>
  if((pagetable = proc_pagetable(p)) == 0)
    80004fea:	854a                	mv	a0,s2
    80004fec:	ffffd097          	auipc	ra,0xffffd
    80004ff0:	c58080e7          	jalr	-936(ra) # 80001c44 <proc_pagetable>
    80004ff4:	8baa                	mv	s7,a0
    80004ff6:	d945                	beqz	a0,80004fa6 <exec+0x84>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80004ff8:	e7042983          	lw	s3,-400(s0)
    80004ffc:	e8845783          	lhu	a5,-376(s0)
    80005000:	c7ad                	beqz	a5,8000506a <exec+0x148>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    80005002:	4901                	li	s2,0
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80005004:	4b01                	li	s6,0
    if((ph.vaddr % PGSIZE) != 0)
    80005006:	6c85                	lui	s9,0x1
    80005008:	fffc8793          	addi	a5,s9,-1 # fff <_entry-0x7ffff001>
    8000500c:	def43823          	sd	a5,-528(s0)
    80005010:	a42d                	j	8000523a <exec+0x318>
  uint64 pa;

  for(i = 0; i < sz; i += PGSIZE){
    pa = walkaddr(pagetable, va + i);
    if(pa == 0)
      panic("loadseg: address should exist");
    80005012:	00003517          	auipc	a0,0x3
    80005016:	6ce50513          	addi	a0,a0,1742 # 800086e0 <syscalls+0x290>
    8000501a:	ffffb097          	auipc	ra,0xffffb
    8000501e:	524080e7          	jalr	1316(ra) # 8000053e <panic>
    if(sz - i < PGSIZE)
      n = sz - i;
    else
      n = PGSIZE;
    if(readi(ip, 0, (uint64)pa, offset+i, n) != n)
    80005022:	8756                	mv	a4,s5
    80005024:	012d86bb          	addw	a3,s11,s2
    80005028:	4581                	li	a1,0
    8000502a:	8526                	mv	a0,s1
    8000502c:	fffff097          	auipc	ra,0xfffff
    80005030:	cac080e7          	jalr	-852(ra) # 80003cd8 <readi>
    80005034:	2501                	sext.w	a0,a0
    80005036:	1aaa9963          	bne	s5,a0,800051e8 <exec+0x2c6>
  for(i = 0; i < sz; i += PGSIZE){
    8000503a:	6785                	lui	a5,0x1
    8000503c:	0127893b          	addw	s2,a5,s2
    80005040:	77fd                	lui	a5,0xfffff
    80005042:	01478a3b          	addw	s4,a5,s4
    80005046:	1f897163          	bgeu	s2,s8,80005228 <exec+0x306>
    pa = walkaddr(pagetable, va + i);
    8000504a:	02091593          	slli	a1,s2,0x20
    8000504e:	9181                	srli	a1,a1,0x20
    80005050:	95ea                	add	a1,a1,s10
    80005052:	855e                	mv	a0,s7
    80005054:	ffffc097          	auipc	ra,0xffffc
    80005058:	01a080e7          	jalr	26(ra) # 8000106e <walkaddr>
    8000505c:	862a                	mv	a2,a0
    if(pa == 0)
    8000505e:	d955                	beqz	a0,80005012 <exec+0xf0>
      n = PGSIZE;
    80005060:	8ae6                	mv	s5,s9
    if(sz - i < PGSIZE)
    80005062:	fd9a70e3          	bgeu	s4,s9,80005022 <exec+0x100>
      n = sz - i;
    80005066:	8ad2                	mv	s5,s4
    80005068:	bf6d                	j	80005022 <exec+0x100>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    8000506a:	4901                	li	s2,0
  iunlockput(ip);
    8000506c:	8526                	mv	a0,s1
    8000506e:	fffff097          	auipc	ra,0xfffff
    80005072:	c18080e7          	jalr	-1000(ra) # 80003c86 <iunlockput>
  end_op();
    80005076:	fffff097          	auipc	ra,0xfffff
    8000507a:	400080e7          	jalr	1024(ra) # 80004476 <end_op>
  p = myproc();
    8000507e:	ffffd097          	auipc	ra,0xffffd
    80005082:	b06080e7          	jalr	-1274(ra) # 80001b84 <myproc>
    80005086:	8aaa                	mv	s5,a0
  uint64 oldsz = p->sz;
    80005088:	04853d03          	ld	s10,72(a0)
  sz = PGROUNDUP(sz);
    8000508c:	6785                	lui	a5,0x1
    8000508e:	17fd                	addi	a5,a5,-1
    80005090:	993e                	add	s2,s2,a5
    80005092:	757d                	lui	a0,0xfffff
    80005094:	00a977b3          	and	a5,s2,a0
    80005098:	e0f43423          	sd	a5,-504(s0)
  if((sz1 = uvmalloc(pagetable, sz, sz + 2*PGSIZE)) == 0)
    8000509c:	6609                	lui	a2,0x2
    8000509e:	963e                	add	a2,a2,a5
    800050a0:	85be                	mv	a1,a5
    800050a2:	855e                	mv	a0,s7
    800050a4:	ffffc097          	auipc	ra,0xffffc
    800050a8:	37e080e7          	jalr	894(ra) # 80001422 <uvmalloc>
    800050ac:	8b2a                	mv	s6,a0
  ip = 0;
    800050ae:	4481                	li	s1,0
  if((sz1 = uvmalloc(pagetable, sz, sz + 2*PGSIZE)) == 0)
    800050b0:	12050c63          	beqz	a0,800051e8 <exec+0x2c6>
  uvmclear(pagetable, sz-2*PGSIZE);
    800050b4:	75f9                	lui	a1,0xffffe
    800050b6:	95aa                	add	a1,a1,a0
    800050b8:	855e                	mv	a0,s7
    800050ba:	ffffc097          	auipc	ra,0xffffc
    800050be:	586080e7          	jalr	1414(ra) # 80001640 <uvmclear>
  stackbase = sp - PGSIZE;
    800050c2:	7c7d                	lui	s8,0xfffff
    800050c4:	9c5a                	add	s8,s8,s6
  for(argc = 0; argv[argc]; argc++) {
    800050c6:	e0043783          	ld	a5,-512(s0)
    800050ca:	6388                	ld	a0,0(a5)
    800050cc:	c535                	beqz	a0,80005138 <exec+0x216>
    800050ce:	e9040993          	addi	s3,s0,-368
    800050d2:	f9040c93          	addi	s9,s0,-112
  sp = sz;
    800050d6:	895a                	mv	s2,s6
    sp -= strlen(argv[argc]) + 1;
    800050d8:	ffffc097          	auipc	ra,0xffffc
    800050dc:	d8c080e7          	jalr	-628(ra) # 80000e64 <strlen>
    800050e0:	2505                	addiw	a0,a0,1
    800050e2:	40a90933          	sub	s2,s2,a0
    sp -= sp % 16; // riscv sp must be 16-byte aligned
    800050e6:	ff097913          	andi	s2,s2,-16
    if(sp < stackbase)
    800050ea:	13896363          	bltu	s2,s8,80005210 <exec+0x2ee>
    if(copyout(pagetable, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
    800050ee:	e0043d83          	ld	s11,-512(s0)
    800050f2:	000dba03          	ld	s4,0(s11)
    800050f6:	8552                	mv	a0,s4
    800050f8:	ffffc097          	auipc	ra,0xffffc
    800050fc:	d6c080e7          	jalr	-660(ra) # 80000e64 <strlen>
    80005100:	0015069b          	addiw	a3,a0,1
    80005104:	8652                	mv	a2,s4
    80005106:	85ca                	mv	a1,s2
    80005108:	855e                	mv	a0,s7
    8000510a:	ffffc097          	auipc	ra,0xffffc
    8000510e:	568080e7          	jalr	1384(ra) # 80001672 <copyout>
    80005112:	10054363          	bltz	a0,80005218 <exec+0x2f6>
    ustack[argc] = sp;
    80005116:	0129b023          	sd	s2,0(s3)
  for(argc = 0; argv[argc]; argc++) {
    8000511a:	0485                	addi	s1,s1,1
    8000511c:	008d8793          	addi	a5,s11,8
    80005120:	e0f43023          	sd	a5,-512(s0)
    80005124:	008db503          	ld	a0,8(s11)
    80005128:	c911                	beqz	a0,8000513c <exec+0x21a>
    if(argc >= MAXARG)
    8000512a:	09a1                	addi	s3,s3,8
    8000512c:	fb3c96e3          	bne	s9,s3,800050d8 <exec+0x1b6>
  sz = sz1;
    80005130:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    80005134:	4481                	li	s1,0
    80005136:	a84d                	j	800051e8 <exec+0x2c6>
  sp = sz;
    80005138:	895a                	mv	s2,s6
  for(argc = 0; argv[argc]; argc++) {
    8000513a:	4481                	li	s1,0
  ustack[argc] = 0;
    8000513c:	00349793          	slli	a5,s1,0x3
    80005140:	f9040713          	addi	a4,s0,-112
    80005144:	97ba                	add	a5,a5,a4
    80005146:	f007b023          	sd	zero,-256(a5) # f00 <_entry-0x7ffff100>
  sp -= (argc+1) * sizeof(uint64);
    8000514a:	00148693          	addi	a3,s1,1
    8000514e:	068e                	slli	a3,a3,0x3
    80005150:	40d90933          	sub	s2,s2,a3
  sp -= sp % 16;
    80005154:	ff097913          	andi	s2,s2,-16
  if(sp < stackbase)
    80005158:	01897663          	bgeu	s2,s8,80005164 <exec+0x242>
  sz = sz1;
    8000515c:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    80005160:	4481                	li	s1,0
    80005162:	a059                	j	800051e8 <exec+0x2c6>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    80005164:	e9040613          	addi	a2,s0,-368
    80005168:	85ca                	mv	a1,s2
    8000516a:	855e                	mv	a0,s7
    8000516c:	ffffc097          	auipc	ra,0xffffc
    80005170:	506080e7          	jalr	1286(ra) # 80001672 <copyout>
    80005174:	0a054663          	bltz	a0,80005220 <exec+0x2fe>
  p->trapframe->a1 = sp;
    80005178:	058ab783          	ld	a5,88(s5)
    8000517c:	0727bc23          	sd	s2,120(a5)
  for(last=s=path; *s; s++)
    80005180:	df843783          	ld	a5,-520(s0)
    80005184:	0007c703          	lbu	a4,0(a5)
    80005188:	cf11                	beqz	a4,800051a4 <exec+0x282>
    8000518a:	0785                	addi	a5,a5,1
    if(*s == '/')
    8000518c:	02f00693          	li	a3,47
    80005190:	a039                	j	8000519e <exec+0x27c>
      last = s+1;
    80005192:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    80005196:	0785                	addi	a5,a5,1
    80005198:	fff7c703          	lbu	a4,-1(a5)
    8000519c:	c701                	beqz	a4,800051a4 <exec+0x282>
    if(*s == '/')
    8000519e:	fed71ce3          	bne	a4,a3,80005196 <exec+0x274>
    800051a2:	bfc5                	j	80005192 <exec+0x270>
  safestrcpy(p->name, last, sizeof(p->name));
    800051a4:	4641                	li	a2,16
    800051a6:	df843583          	ld	a1,-520(s0)
    800051aa:	158a8513          	addi	a0,s5,344
    800051ae:	ffffc097          	auipc	ra,0xffffc
    800051b2:	c84080e7          	jalr	-892(ra) # 80000e32 <safestrcpy>
  oldpagetable = p->pagetable;
    800051b6:	050ab503          	ld	a0,80(s5)
  p->pagetable = pagetable;
    800051ba:	057ab823          	sd	s7,80(s5)
  p->sz = sz;
    800051be:	056ab423          	sd	s6,72(s5)
  p->trapframe->epc = elf.entry;  // initial program counter = main
    800051c2:	058ab783          	ld	a5,88(s5)
    800051c6:	e6843703          	ld	a4,-408(s0)
    800051ca:	ef98                	sd	a4,24(a5)
  p->trapframe->sp = sp; // initial stack pointer
    800051cc:	058ab783          	ld	a5,88(s5)
    800051d0:	0327b823          	sd	s2,48(a5)
  proc_freepagetable(oldpagetable, oldsz);
    800051d4:	85ea                	mv	a1,s10
    800051d6:	ffffd097          	auipc	ra,0xffffd
    800051da:	b0a080e7          	jalr	-1270(ra) # 80001ce0 <proc_freepagetable>
  return argc; // this ends up in a0, the first argument to main(argc, argv)
    800051de:	0004851b          	sext.w	a0,s1
    800051e2:	bbe1                	j	80004fba <exec+0x98>
    800051e4:	e1243423          	sd	s2,-504(s0)
    proc_freepagetable(pagetable, sz);
    800051e8:	e0843583          	ld	a1,-504(s0)
    800051ec:	855e                	mv	a0,s7
    800051ee:	ffffd097          	auipc	ra,0xffffd
    800051f2:	af2080e7          	jalr	-1294(ra) # 80001ce0 <proc_freepagetable>
  if(ip){
    800051f6:	da0498e3          	bnez	s1,80004fa6 <exec+0x84>
  return -1;
    800051fa:	557d                	li	a0,-1
    800051fc:	bb7d                	j	80004fba <exec+0x98>
    800051fe:	e1243423          	sd	s2,-504(s0)
    80005202:	b7dd                	j	800051e8 <exec+0x2c6>
    80005204:	e1243423          	sd	s2,-504(s0)
    80005208:	b7c5                	j	800051e8 <exec+0x2c6>
    8000520a:	e1243423          	sd	s2,-504(s0)
    8000520e:	bfe9                	j	800051e8 <exec+0x2c6>
  sz = sz1;
    80005210:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    80005214:	4481                	li	s1,0
    80005216:	bfc9                	j	800051e8 <exec+0x2c6>
  sz = sz1;
    80005218:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000521c:	4481                	li	s1,0
    8000521e:	b7e9                	j	800051e8 <exec+0x2c6>
  sz = sz1;
    80005220:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    80005224:	4481                	li	s1,0
    80005226:	b7c9                	j	800051e8 <exec+0x2c6>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz)) == 0)
    80005228:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    8000522c:	2b05                	addiw	s6,s6,1
    8000522e:	0389899b          	addiw	s3,s3,56
    80005232:	e8845783          	lhu	a5,-376(s0)
    80005236:	e2fb5be3          	bge	s6,a5,8000506c <exec+0x14a>
    if(readi(ip, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
    8000523a:	2981                	sext.w	s3,s3
    8000523c:	03800713          	li	a4,56
    80005240:	86ce                	mv	a3,s3
    80005242:	e1840613          	addi	a2,s0,-488
    80005246:	4581                	li	a1,0
    80005248:	8526                	mv	a0,s1
    8000524a:	fffff097          	auipc	ra,0xfffff
    8000524e:	a8e080e7          	jalr	-1394(ra) # 80003cd8 <readi>
    80005252:	03800793          	li	a5,56
    80005256:	f8f517e3          	bne	a0,a5,800051e4 <exec+0x2c2>
    if(ph.type != ELF_PROG_LOAD)
    8000525a:	e1842783          	lw	a5,-488(s0)
    8000525e:	4705                	li	a4,1
    80005260:	fce796e3          	bne	a5,a4,8000522c <exec+0x30a>
    if(ph.memsz < ph.filesz)
    80005264:	e4043603          	ld	a2,-448(s0)
    80005268:	e3843783          	ld	a5,-456(s0)
    8000526c:	f8f669e3          	bltu	a2,a5,800051fe <exec+0x2dc>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    80005270:	e2843783          	ld	a5,-472(s0)
    80005274:	963e                	add	a2,a2,a5
    80005276:	f8f667e3          	bltu	a2,a5,80005204 <exec+0x2e2>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz)) == 0)
    8000527a:	85ca                	mv	a1,s2
    8000527c:	855e                	mv	a0,s7
    8000527e:	ffffc097          	auipc	ra,0xffffc
    80005282:	1a4080e7          	jalr	420(ra) # 80001422 <uvmalloc>
    80005286:	e0a43423          	sd	a0,-504(s0)
    8000528a:	d141                	beqz	a0,8000520a <exec+0x2e8>
    if((ph.vaddr % PGSIZE) != 0)
    8000528c:	e2843d03          	ld	s10,-472(s0)
    80005290:	df043783          	ld	a5,-528(s0)
    80005294:	00fd77b3          	and	a5,s10,a5
    80005298:	fba1                	bnez	a5,800051e8 <exec+0x2c6>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    8000529a:	e2042d83          	lw	s11,-480(s0)
    8000529e:	e3842c03          	lw	s8,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    800052a2:	f80c03e3          	beqz	s8,80005228 <exec+0x306>
    800052a6:	8a62                	mv	s4,s8
    800052a8:	4901                	li	s2,0
    800052aa:	b345                	j	8000504a <exec+0x128>

00000000800052ac <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
    800052ac:	7179                	addi	sp,sp,-48
    800052ae:	f406                	sd	ra,40(sp)
    800052b0:	f022                	sd	s0,32(sp)
    800052b2:	ec26                	sd	s1,24(sp)
    800052b4:	e84a                	sd	s2,16(sp)
    800052b6:	1800                	addi	s0,sp,48
    800052b8:	892e                	mv	s2,a1
    800052ba:	84b2                	mv	s1,a2
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
    800052bc:	fdc40593          	addi	a1,s0,-36
    800052c0:	ffffe097          	auipc	ra,0xffffe
    800052c4:	ba8080e7          	jalr	-1112(ra) # 80002e68 <argint>
    800052c8:	04054063          	bltz	a0,80005308 <argfd+0x5c>
    return -1;
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
    800052cc:	fdc42703          	lw	a4,-36(s0)
    800052d0:	47bd                	li	a5,15
    800052d2:	02e7ed63          	bltu	a5,a4,8000530c <argfd+0x60>
    800052d6:	ffffd097          	auipc	ra,0xffffd
    800052da:	8ae080e7          	jalr	-1874(ra) # 80001b84 <myproc>
    800052de:	fdc42703          	lw	a4,-36(s0)
    800052e2:	01a70793          	addi	a5,a4,26
    800052e6:	078e                	slli	a5,a5,0x3
    800052e8:	953e                	add	a0,a0,a5
    800052ea:	611c                	ld	a5,0(a0)
    800052ec:	c395                	beqz	a5,80005310 <argfd+0x64>
    return -1;
  if(pfd)
    800052ee:	00090463          	beqz	s2,800052f6 <argfd+0x4a>
    *pfd = fd;
    800052f2:	00e92023          	sw	a4,0(s2)
  if(pf)
    *pf = f;
  return 0;
    800052f6:	4501                	li	a0,0
  if(pf)
    800052f8:	c091                	beqz	s1,800052fc <argfd+0x50>
    *pf = f;
    800052fa:	e09c                	sd	a5,0(s1)
}
    800052fc:	70a2                	ld	ra,40(sp)
    800052fe:	7402                	ld	s0,32(sp)
    80005300:	64e2                	ld	s1,24(sp)
    80005302:	6942                	ld	s2,16(sp)
    80005304:	6145                	addi	sp,sp,48
    80005306:	8082                	ret
    return -1;
    80005308:	557d                	li	a0,-1
    8000530a:	bfcd                	j	800052fc <argfd+0x50>
    return -1;
    8000530c:	557d                	li	a0,-1
    8000530e:	b7fd                	j	800052fc <argfd+0x50>
    80005310:	557d                	li	a0,-1
    80005312:	b7ed                	j	800052fc <argfd+0x50>

0000000080005314 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
    80005314:	1101                	addi	sp,sp,-32
    80005316:	ec06                	sd	ra,24(sp)
    80005318:	e822                	sd	s0,16(sp)
    8000531a:	e426                	sd	s1,8(sp)
    8000531c:	1000                	addi	s0,sp,32
    8000531e:	84aa                	mv	s1,a0
  int fd;
  struct proc *p = myproc();
    80005320:	ffffd097          	auipc	ra,0xffffd
    80005324:	864080e7          	jalr	-1948(ra) # 80001b84 <myproc>
    80005328:	862a                	mv	a2,a0

  for(fd = 0; fd < NOFILE; fd++){
    8000532a:	0d050793          	addi	a5,a0,208 # fffffffffffff0d0 <end+0xffffffff7ffd90d0>
    8000532e:	4501                	li	a0,0
    80005330:	46c1                	li	a3,16
    if(p->ofile[fd] == 0){
    80005332:	6398                	ld	a4,0(a5)
    80005334:	cb19                	beqz	a4,8000534a <fdalloc+0x36>
  for(fd = 0; fd < NOFILE; fd++){
    80005336:	2505                	addiw	a0,a0,1
    80005338:	07a1                	addi	a5,a5,8
    8000533a:	fed51ce3          	bne	a0,a3,80005332 <fdalloc+0x1e>
      p->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
    8000533e:	557d                	li	a0,-1
}
    80005340:	60e2                	ld	ra,24(sp)
    80005342:	6442                	ld	s0,16(sp)
    80005344:	64a2                	ld	s1,8(sp)
    80005346:	6105                	addi	sp,sp,32
    80005348:	8082                	ret
      p->ofile[fd] = f;
    8000534a:	01a50793          	addi	a5,a0,26
    8000534e:	078e                	slli	a5,a5,0x3
    80005350:	963e                	add	a2,a2,a5
    80005352:	e204                	sd	s1,0(a2)
      return fd;
    80005354:	b7f5                	j	80005340 <fdalloc+0x2c>

0000000080005356 <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
    80005356:	715d                	addi	sp,sp,-80
    80005358:	e486                	sd	ra,72(sp)
    8000535a:	e0a2                	sd	s0,64(sp)
    8000535c:	fc26                	sd	s1,56(sp)
    8000535e:	f84a                	sd	s2,48(sp)
    80005360:	f44e                	sd	s3,40(sp)
    80005362:	f052                	sd	s4,32(sp)
    80005364:	ec56                	sd	s5,24(sp)
    80005366:	0880                	addi	s0,sp,80
    80005368:	89ae                	mv	s3,a1
    8000536a:	8ab2                	mv	s5,a2
    8000536c:	8a36                	mv	s4,a3
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
    8000536e:	fb040593          	addi	a1,s0,-80
    80005372:	fffff097          	auipc	ra,0xfffff
    80005376:	e86080e7          	jalr	-378(ra) # 800041f8 <nameiparent>
    8000537a:	892a                	mv	s2,a0
    8000537c:	12050f63          	beqz	a0,800054ba <create+0x164>
    return 0;

  ilock(dp);
    80005380:	ffffe097          	auipc	ra,0xffffe
    80005384:	6a4080e7          	jalr	1700(ra) # 80003a24 <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    80005388:	4601                	li	a2,0
    8000538a:	fb040593          	addi	a1,s0,-80
    8000538e:	854a                	mv	a0,s2
    80005390:	fffff097          	auipc	ra,0xfffff
    80005394:	b78080e7          	jalr	-1160(ra) # 80003f08 <dirlookup>
    80005398:	84aa                	mv	s1,a0
    8000539a:	c921                	beqz	a0,800053ea <create+0x94>
    iunlockput(dp);
    8000539c:	854a                	mv	a0,s2
    8000539e:	fffff097          	auipc	ra,0xfffff
    800053a2:	8e8080e7          	jalr	-1816(ra) # 80003c86 <iunlockput>
    ilock(ip);
    800053a6:	8526                	mv	a0,s1
    800053a8:	ffffe097          	auipc	ra,0xffffe
    800053ac:	67c080e7          	jalr	1660(ra) # 80003a24 <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    800053b0:	2981                	sext.w	s3,s3
    800053b2:	4789                	li	a5,2
    800053b4:	02f99463          	bne	s3,a5,800053dc <create+0x86>
    800053b8:	0444d783          	lhu	a5,68(s1)
    800053bc:	37f9                	addiw	a5,a5,-2
    800053be:	17c2                	slli	a5,a5,0x30
    800053c0:	93c1                	srli	a5,a5,0x30
    800053c2:	4705                	li	a4,1
    800053c4:	00f76c63          	bltu	a4,a5,800053dc <create+0x86>
    panic("create: dirlink");

  iunlockput(dp);

  return ip;
}
    800053c8:	8526                	mv	a0,s1
    800053ca:	60a6                	ld	ra,72(sp)
    800053cc:	6406                	ld	s0,64(sp)
    800053ce:	74e2                	ld	s1,56(sp)
    800053d0:	7942                	ld	s2,48(sp)
    800053d2:	79a2                	ld	s3,40(sp)
    800053d4:	7a02                	ld	s4,32(sp)
    800053d6:	6ae2                	ld	s5,24(sp)
    800053d8:	6161                	addi	sp,sp,80
    800053da:	8082                	ret
    iunlockput(ip);
    800053dc:	8526                	mv	a0,s1
    800053de:	fffff097          	auipc	ra,0xfffff
    800053e2:	8a8080e7          	jalr	-1880(ra) # 80003c86 <iunlockput>
    return 0;
    800053e6:	4481                	li	s1,0
    800053e8:	b7c5                	j	800053c8 <create+0x72>
  if((ip = ialloc(dp->dev, type)) == 0)
    800053ea:	85ce                	mv	a1,s3
    800053ec:	00092503          	lw	a0,0(s2)
    800053f0:	ffffe097          	auipc	ra,0xffffe
    800053f4:	49c080e7          	jalr	1180(ra) # 8000388c <ialloc>
    800053f8:	84aa                	mv	s1,a0
    800053fa:	c529                	beqz	a0,80005444 <create+0xee>
  ilock(ip);
    800053fc:	ffffe097          	auipc	ra,0xffffe
    80005400:	628080e7          	jalr	1576(ra) # 80003a24 <ilock>
  ip->major = major;
    80005404:	05549323          	sh	s5,70(s1)
  ip->minor = minor;
    80005408:	05449423          	sh	s4,72(s1)
  ip->nlink = 1;
    8000540c:	4785                	li	a5,1
    8000540e:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80005412:	8526                	mv	a0,s1
    80005414:	ffffe097          	auipc	ra,0xffffe
    80005418:	546080e7          	jalr	1350(ra) # 8000395a <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    8000541c:	2981                	sext.w	s3,s3
    8000541e:	4785                	li	a5,1
    80005420:	02f98a63          	beq	s3,a5,80005454 <create+0xfe>
  if(dirlink(dp, name, ip->inum) < 0)
    80005424:	40d0                	lw	a2,4(s1)
    80005426:	fb040593          	addi	a1,s0,-80
    8000542a:	854a                	mv	a0,s2
    8000542c:	fffff097          	auipc	ra,0xfffff
    80005430:	cec080e7          	jalr	-788(ra) # 80004118 <dirlink>
    80005434:	06054b63          	bltz	a0,800054aa <create+0x154>
  iunlockput(dp);
    80005438:	854a                	mv	a0,s2
    8000543a:	fffff097          	auipc	ra,0xfffff
    8000543e:	84c080e7          	jalr	-1972(ra) # 80003c86 <iunlockput>
  return ip;
    80005442:	b759                	j	800053c8 <create+0x72>
    panic("create: ialloc");
    80005444:	00003517          	auipc	a0,0x3
    80005448:	2bc50513          	addi	a0,a0,700 # 80008700 <syscalls+0x2b0>
    8000544c:	ffffb097          	auipc	ra,0xffffb
    80005450:	0f2080e7          	jalr	242(ra) # 8000053e <panic>
    dp->nlink++;  // for ".."
    80005454:	04a95783          	lhu	a5,74(s2)
    80005458:	2785                	addiw	a5,a5,1
    8000545a:	04f91523          	sh	a5,74(s2)
    iupdate(dp);
    8000545e:	854a                	mv	a0,s2
    80005460:	ffffe097          	auipc	ra,0xffffe
    80005464:	4fa080e7          	jalr	1274(ra) # 8000395a <iupdate>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    80005468:	40d0                	lw	a2,4(s1)
    8000546a:	00003597          	auipc	a1,0x3
    8000546e:	2a658593          	addi	a1,a1,678 # 80008710 <syscalls+0x2c0>
    80005472:	8526                	mv	a0,s1
    80005474:	fffff097          	auipc	ra,0xfffff
    80005478:	ca4080e7          	jalr	-860(ra) # 80004118 <dirlink>
    8000547c:	00054f63          	bltz	a0,8000549a <create+0x144>
    80005480:	00492603          	lw	a2,4(s2)
    80005484:	00003597          	auipc	a1,0x3
    80005488:	29458593          	addi	a1,a1,660 # 80008718 <syscalls+0x2c8>
    8000548c:	8526                	mv	a0,s1
    8000548e:	fffff097          	auipc	ra,0xfffff
    80005492:	c8a080e7          	jalr	-886(ra) # 80004118 <dirlink>
    80005496:	f80557e3          	bgez	a0,80005424 <create+0xce>
      panic("create dots");
    8000549a:	00003517          	auipc	a0,0x3
    8000549e:	28650513          	addi	a0,a0,646 # 80008720 <syscalls+0x2d0>
    800054a2:	ffffb097          	auipc	ra,0xffffb
    800054a6:	09c080e7          	jalr	156(ra) # 8000053e <panic>
    panic("create: dirlink");
    800054aa:	00003517          	auipc	a0,0x3
    800054ae:	28650513          	addi	a0,a0,646 # 80008730 <syscalls+0x2e0>
    800054b2:	ffffb097          	auipc	ra,0xffffb
    800054b6:	08c080e7          	jalr	140(ra) # 8000053e <panic>
    return 0;
    800054ba:	84aa                	mv	s1,a0
    800054bc:	b731                	j	800053c8 <create+0x72>

00000000800054be <sys_dup>:
{
    800054be:	7179                	addi	sp,sp,-48
    800054c0:	f406                	sd	ra,40(sp)
    800054c2:	f022                	sd	s0,32(sp)
    800054c4:	ec26                	sd	s1,24(sp)
    800054c6:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0)
    800054c8:	fd840613          	addi	a2,s0,-40
    800054cc:	4581                	li	a1,0
    800054ce:	4501                	li	a0,0
    800054d0:	00000097          	auipc	ra,0x0
    800054d4:	ddc080e7          	jalr	-548(ra) # 800052ac <argfd>
    return -1;
    800054d8:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    800054da:	02054363          	bltz	a0,80005500 <sys_dup+0x42>
  if((fd=fdalloc(f)) < 0)
    800054de:	fd843503          	ld	a0,-40(s0)
    800054e2:	00000097          	auipc	ra,0x0
    800054e6:	e32080e7          	jalr	-462(ra) # 80005314 <fdalloc>
    800054ea:	84aa                	mv	s1,a0
    return -1;
    800054ec:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    800054ee:	00054963          	bltz	a0,80005500 <sys_dup+0x42>
  filedup(f);
    800054f2:	fd843503          	ld	a0,-40(s0)
    800054f6:	fffff097          	auipc	ra,0xfffff
    800054fa:	37a080e7          	jalr	890(ra) # 80004870 <filedup>
  return fd;
    800054fe:	87a6                	mv	a5,s1
}
    80005500:	853e                	mv	a0,a5
    80005502:	70a2                	ld	ra,40(sp)
    80005504:	7402                	ld	s0,32(sp)
    80005506:	64e2                	ld	s1,24(sp)
    80005508:	6145                	addi	sp,sp,48
    8000550a:	8082                	ret

000000008000550c <sys_read>:
{
    8000550c:	7179                	addi	sp,sp,-48
    8000550e:	f406                	sd	ra,40(sp)
    80005510:	f022                	sd	s0,32(sp)
    80005512:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    80005514:	fe840613          	addi	a2,s0,-24
    80005518:	4581                	li	a1,0
    8000551a:	4501                	li	a0,0
    8000551c:	00000097          	auipc	ra,0x0
    80005520:	d90080e7          	jalr	-624(ra) # 800052ac <argfd>
    return -1;
    80005524:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    80005526:	04054163          	bltz	a0,80005568 <sys_read+0x5c>
    8000552a:	fe440593          	addi	a1,s0,-28
    8000552e:	4509                	li	a0,2
    80005530:	ffffe097          	auipc	ra,0xffffe
    80005534:	938080e7          	jalr	-1736(ra) # 80002e68 <argint>
    return -1;
    80005538:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000553a:	02054763          	bltz	a0,80005568 <sys_read+0x5c>
    8000553e:	fd840593          	addi	a1,s0,-40
    80005542:	4505                	li	a0,1
    80005544:	ffffe097          	auipc	ra,0xffffe
    80005548:	946080e7          	jalr	-1722(ra) # 80002e8a <argaddr>
    return -1;
    8000554c:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000554e:	00054d63          	bltz	a0,80005568 <sys_read+0x5c>
  return fileread(f, p, n);
    80005552:	fe442603          	lw	a2,-28(s0)
    80005556:	fd843583          	ld	a1,-40(s0)
    8000555a:	fe843503          	ld	a0,-24(s0)
    8000555e:	fffff097          	auipc	ra,0xfffff
    80005562:	49e080e7          	jalr	1182(ra) # 800049fc <fileread>
    80005566:	87aa                	mv	a5,a0
}
    80005568:	853e                	mv	a0,a5
    8000556a:	70a2                	ld	ra,40(sp)
    8000556c:	7402                	ld	s0,32(sp)
    8000556e:	6145                	addi	sp,sp,48
    80005570:	8082                	ret

0000000080005572 <sys_write>:
{
    80005572:	7179                	addi	sp,sp,-48
    80005574:	f406                	sd	ra,40(sp)
    80005576:	f022                	sd	s0,32(sp)
    80005578:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000557a:	fe840613          	addi	a2,s0,-24
    8000557e:	4581                	li	a1,0
    80005580:	4501                	li	a0,0
    80005582:	00000097          	auipc	ra,0x0
    80005586:	d2a080e7          	jalr	-726(ra) # 800052ac <argfd>
    return -1;
    8000558a:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000558c:	04054163          	bltz	a0,800055ce <sys_write+0x5c>
    80005590:	fe440593          	addi	a1,s0,-28
    80005594:	4509                	li	a0,2
    80005596:	ffffe097          	auipc	ra,0xffffe
    8000559a:	8d2080e7          	jalr	-1838(ra) # 80002e68 <argint>
    return -1;
    8000559e:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800055a0:	02054763          	bltz	a0,800055ce <sys_write+0x5c>
    800055a4:	fd840593          	addi	a1,s0,-40
    800055a8:	4505                	li	a0,1
    800055aa:	ffffe097          	auipc	ra,0xffffe
    800055ae:	8e0080e7          	jalr	-1824(ra) # 80002e8a <argaddr>
    return -1;
    800055b2:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800055b4:	00054d63          	bltz	a0,800055ce <sys_write+0x5c>
  return filewrite(f, p, n);
    800055b8:	fe442603          	lw	a2,-28(s0)
    800055bc:	fd843583          	ld	a1,-40(s0)
    800055c0:	fe843503          	ld	a0,-24(s0)
    800055c4:	fffff097          	auipc	ra,0xfffff
    800055c8:	4fa080e7          	jalr	1274(ra) # 80004abe <filewrite>
    800055cc:	87aa                	mv	a5,a0
}
    800055ce:	853e                	mv	a0,a5
    800055d0:	70a2                	ld	ra,40(sp)
    800055d2:	7402                	ld	s0,32(sp)
    800055d4:	6145                	addi	sp,sp,48
    800055d6:	8082                	ret

00000000800055d8 <sys_close>:
{
    800055d8:	1101                	addi	sp,sp,-32
    800055da:	ec06                	sd	ra,24(sp)
    800055dc:	e822                	sd	s0,16(sp)
    800055de:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    800055e0:	fe040613          	addi	a2,s0,-32
    800055e4:	fec40593          	addi	a1,s0,-20
    800055e8:	4501                	li	a0,0
    800055ea:	00000097          	auipc	ra,0x0
    800055ee:	cc2080e7          	jalr	-830(ra) # 800052ac <argfd>
    return -1;
    800055f2:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    800055f4:	02054463          	bltz	a0,8000561c <sys_close+0x44>
  myproc()->ofile[fd] = 0;
    800055f8:	ffffc097          	auipc	ra,0xffffc
    800055fc:	58c080e7          	jalr	1420(ra) # 80001b84 <myproc>
    80005600:	fec42783          	lw	a5,-20(s0)
    80005604:	07e9                	addi	a5,a5,26
    80005606:	078e                	slli	a5,a5,0x3
    80005608:	97aa                	add	a5,a5,a0
    8000560a:	0007b023          	sd	zero,0(a5)
  fileclose(f);
    8000560e:	fe043503          	ld	a0,-32(s0)
    80005612:	fffff097          	auipc	ra,0xfffff
    80005616:	2b0080e7          	jalr	688(ra) # 800048c2 <fileclose>
  return 0;
    8000561a:	4781                	li	a5,0
}
    8000561c:	853e                	mv	a0,a5
    8000561e:	60e2                	ld	ra,24(sp)
    80005620:	6442                	ld	s0,16(sp)
    80005622:	6105                	addi	sp,sp,32
    80005624:	8082                	ret

0000000080005626 <sys_fstat>:
{
    80005626:	1101                	addi	sp,sp,-32
    80005628:	ec06                	sd	ra,24(sp)
    8000562a:	e822                	sd	s0,16(sp)
    8000562c:	1000                	addi	s0,sp,32
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    8000562e:	fe840613          	addi	a2,s0,-24
    80005632:	4581                	li	a1,0
    80005634:	4501                	li	a0,0
    80005636:	00000097          	auipc	ra,0x0
    8000563a:	c76080e7          	jalr	-906(ra) # 800052ac <argfd>
    return -1;
    8000563e:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    80005640:	02054563          	bltz	a0,8000566a <sys_fstat+0x44>
    80005644:	fe040593          	addi	a1,s0,-32
    80005648:	4505                	li	a0,1
    8000564a:	ffffe097          	auipc	ra,0xffffe
    8000564e:	840080e7          	jalr	-1984(ra) # 80002e8a <argaddr>
    return -1;
    80005652:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    80005654:	00054b63          	bltz	a0,8000566a <sys_fstat+0x44>
  return filestat(f, st);
    80005658:	fe043583          	ld	a1,-32(s0)
    8000565c:	fe843503          	ld	a0,-24(s0)
    80005660:	fffff097          	auipc	ra,0xfffff
    80005664:	32a080e7          	jalr	810(ra) # 8000498a <filestat>
    80005668:	87aa                	mv	a5,a0
}
    8000566a:	853e                	mv	a0,a5
    8000566c:	60e2                	ld	ra,24(sp)
    8000566e:	6442                	ld	s0,16(sp)
    80005670:	6105                	addi	sp,sp,32
    80005672:	8082                	ret

0000000080005674 <sys_link>:
{
    80005674:	7169                	addi	sp,sp,-304
    80005676:	f606                	sd	ra,296(sp)
    80005678:	f222                	sd	s0,288(sp)
    8000567a:	ee26                	sd	s1,280(sp)
    8000567c:	ea4a                	sd	s2,272(sp)
    8000567e:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80005680:	08000613          	li	a2,128
    80005684:	ed040593          	addi	a1,s0,-304
    80005688:	4501                	li	a0,0
    8000568a:	ffffe097          	auipc	ra,0xffffe
    8000568e:	822080e7          	jalr	-2014(ra) # 80002eac <argstr>
    return -1;
    80005692:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80005694:	10054e63          	bltz	a0,800057b0 <sys_link+0x13c>
    80005698:	08000613          	li	a2,128
    8000569c:	f5040593          	addi	a1,s0,-176
    800056a0:	4505                	li	a0,1
    800056a2:	ffffe097          	auipc	ra,0xffffe
    800056a6:	80a080e7          	jalr	-2038(ra) # 80002eac <argstr>
    return -1;
    800056aa:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    800056ac:	10054263          	bltz	a0,800057b0 <sys_link+0x13c>
  begin_op();
    800056b0:	fffff097          	auipc	ra,0xfffff
    800056b4:	d46080e7          	jalr	-698(ra) # 800043f6 <begin_op>
  if((ip = namei(old)) == 0){
    800056b8:	ed040513          	addi	a0,s0,-304
    800056bc:	fffff097          	auipc	ra,0xfffff
    800056c0:	b1e080e7          	jalr	-1250(ra) # 800041da <namei>
    800056c4:	84aa                	mv	s1,a0
    800056c6:	c551                	beqz	a0,80005752 <sys_link+0xde>
  ilock(ip);
    800056c8:	ffffe097          	auipc	ra,0xffffe
    800056cc:	35c080e7          	jalr	860(ra) # 80003a24 <ilock>
  if(ip->type == T_DIR){
    800056d0:	04449703          	lh	a4,68(s1)
    800056d4:	4785                	li	a5,1
    800056d6:	08f70463          	beq	a4,a5,8000575e <sys_link+0xea>
  ip->nlink++;
    800056da:	04a4d783          	lhu	a5,74(s1)
    800056de:	2785                	addiw	a5,a5,1
    800056e0:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    800056e4:	8526                	mv	a0,s1
    800056e6:	ffffe097          	auipc	ra,0xffffe
    800056ea:	274080e7          	jalr	628(ra) # 8000395a <iupdate>
  iunlock(ip);
    800056ee:	8526                	mv	a0,s1
    800056f0:	ffffe097          	auipc	ra,0xffffe
    800056f4:	3f6080e7          	jalr	1014(ra) # 80003ae6 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    800056f8:	fd040593          	addi	a1,s0,-48
    800056fc:	f5040513          	addi	a0,s0,-176
    80005700:	fffff097          	auipc	ra,0xfffff
    80005704:	af8080e7          	jalr	-1288(ra) # 800041f8 <nameiparent>
    80005708:	892a                	mv	s2,a0
    8000570a:	c935                	beqz	a0,8000577e <sys_link+0x10a>
  ilock(dp);
    8000570c:	ffffe097          	auipc	ra,0xffffe
    80005710:	318080e7          	jalr	792(ra) # 80003a24 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    80005714:	00092703          	lw	a4,0(s2)
    80005718:	409c                	lw	a5,0(s1)
    8000571a:	04f71d63          	bne	a4,a5,80005774 <sys_link+0x100>
    8000571e:	40d0                	lw	a2,4(s1)
    80005720:	fd040593          	addi	a1,s0,-48
    80005724:	854a                	mv	a0,s2
    80005726:	fffff097          	auipc	ra,0xfffff
    8000572a:	9f2080e7          	jalr	-1550(ra) # 80004118 <dirlink>
    8000572e:	04054363          	bltz	a0,80005774 <sys_link+0x100>
  iunlockput(dp);
    80005732:	854a                	mv	a0,s2
    80005734:	ffffe097          	auipc	ra,0xffffe
    80005738:	552080e7          	jalr	1362(ra) # 80003c86 <iunlockput>
  iput(ip);
    8000573c:	8526                	mv	a0,s1
    8000573e:	ffffe097          	auipc	ra,0xffffe
    80005742:	4a0080e7          	jalr	1184(ra) # 80003bde <iput>
  end_op();
    80005746:	fffff097          	auipc	ra,0xfffff
    8000574a:	d30080e7          	jalr	-720(ra) # 80004476 <end_op>
  return 0;
    8000574e:	4781                	li	a5,0
    80005750:	a085                	j	800057b0 <sys_link+0x13c>
    end_op();
    80005752:	fffff097          	auipc	ra,0xfffff
    80005756:	d24080e7          	jalr	-732(ra) # 80004476 <end_op>
    return -1;
    8000575a:	57fd                	li	a5,-1
    8000575c:	a891                	j	800057b0 <sys_link+0x13c>
    iunlockput(ip);
    8000575e:	8526                	mv	a0,s1
    80005760:	ffffe097          	auipc	ra,0xffffe
    80005764:	526080e7          	jalr	1318(ra) # 80003c86 <iunlockput>
    end_op();
    80005768:	fffff097          	auipc	ra,0xfffff
    8000576c:	d0e080e7          	jalr	-754(ra) # 80004476 <end_op>
    return -1;
    80005770:	57fd                	li	a5,-1
    80005772:	a83d                	j	800057b0 <sys_link+0x13c>
    iunlockput(dp);
    80005774:	854a                	mv	a0,s2
    80005776:	ffffe097          	auipc	ra,0xffffe
    8000577a:	510080e7          	jalr	1296(ra) # 80003c86 <iunlockput>
  ilock(ip);
    8000577e:	8526                	mv	a0,s1
    80005780:	ffffe097          	auipc	ra,0xffffe
    80005784:	2a4080e7          	jalr	676(ra) # 80003a24 <ilock>
  ip->nlink--;
    80005788:	04a4d783          	lhu	a5,74(s1)
    8000578c:	37fd                	addiw	a5,a5,-1
    8000578e:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80005792:	8526                	mv	a0,s1
    80005794:	ffffe097          	auipc	ra,0xffffe
    80005798:	1c6080e7          	jalr	454(ra) # 8000395a <iupdate>
  iunlockput(ip);
    8000579c:	8526                	mv	a0,s1
    8000579e:	ffffe097          	auipc	ra,0xffffe
    800057a2:	4e8080e7          	jalr	1256(ra) # 80003c86 <iunlockput>
  end_op();
    800057a6:	fffff097          	auipc	ra,0xfffff
    800057aa:	cd0080e7          	jalr	-816(ra) # 80004476 <end_op>
  return -1;
    800057ae:	57fd                	li	a5,-1
}
    800057b0:	853e                	mv	a0,a5
    800057b2:	70b2                	ld	ra,296(sp)
    800057b4:	7412                	ld	s0,288(sp)
    800057b6:	64f2                	ld	s1,280(sp)
    800057b8:	6952                	ld	s2,272(sp)
    800057ba:	6155                	addi	sp,sp,304
    800057bc:	8082                	ret

00000000800057be <sys_unlink>:
{
    800057be:	7151                	addi	sp,sp,-240
    800057c0:	f586                	sd	ra,232(sp)
    800057c2:	f1a2                	sd	s0,224(sp)
    800057c4:	eda6                	sd	s1,216(sp)
    800057c6:	e9ca                	sd	s2,208(sp)
    800057c8:	e5ce                	sd	s3,200(sp)
    800057ca:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    800057cc:	08000613          	li	a2,128
    800057d0:	f3040593          	addi	a1,s0,-208
    800057d4:	4501                	li	a0,0
    800057d6:	ffffd097          	auipc	ra,0xffffd
    800057da:	6d6080e7          	jalr	1750(ra) # 80002eac <argstr>
    800057de:	18054163          	bltz	a0,80005960 <sys_unlink+0x1a2>
  begin_op();
    800057e2:	fffff097          	auipc	ra,0xfffff
    800057e6:	c14080e7          	jalr	-1004(ra) # 800043f6 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    800057ea:	fb040593          	addi	a1,s0,-80
    800057ee:	f3040513          	addi	a0,s0,-208
    800057f2:	fffff097          	auipc	ra,0xfffff
    800057f6:	a06080e7          	jalr	-1530(ra) # 800041f8 <nameiparent>
    800057fa:	84aa                	mv	s1,a0
    800057fc:	c979                	beqz	a0,800058d2 <sys_unlink+0x114>
  ilock(dp);
    800057fe:	ffffe097          	auipc	ra,0xffffe
    80005802:	226080e7          	jalr	550(ra) # 80003a24 <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    80005806:	00003597          	auipc	a1,0x3
    8000580a:	f0a58593          	addi	a1,a1,-246 # 80008710 <syscalls+0x2c0>
    8000580e:	fb040513          	addi	a0,s0,-80
    80005812:	ffffe097          	auipc	ra,0xffffe
    80005816:	6dc080e7          	jalr	1756(ra) # 80003eee <namecmp>
    8000581a:	14050a63          	beqz	a0,8000596e <sys_unlink+0x1b0>
    8000581e:	00003597          	auipc	a1,0x3
    80005822:	efa58593          	addi	a1,a1,-262 # 80008718 <syscalls+0x2c8>
    80005826:	fb040513          	addi	a0,s0,-80
    8000582a:	ffffe097          	auipc	ra,0xffffe
    8000582e:	6c4080e7          	jalr	1732(ra) # 80003eee <namecmp>
    80005832:	12050e63          	beqz	a0,8000596e <sys_unlink+0x1b0>
  if((ip = dirlookup(dp, name, &off)) == 0)
    80005836:	f2c40613          	addi	a2,s0,-212
    8000583a:	fb040593          	addi	a1,s0,-80
    8000583e:	8526                	mv	a0,s1
    80005840:	ffffe097          	auipc	ra,0xffffe
    80005844:	6c8080e7          	jalr	1736(ra) # 80003f08 <dirlookup>
    80005848:	892a                	mv	s2,a0
    8000584a:	12050263          	beqz	a0,8000596e <sys_unlink+0x1b0>
  ilock(ip);
    8000584e:	ffffe097          	auipc	ra,0xffffe
    80005852:	1d6080e7          	jalr	470(ra) # 80003a24 <ilock>
  if(ip->nlink < 1)
    80005856:	04a91783          	lh	a5,74(s2)
    8000585a:	08f05263          	blez	a5,800058de <sys_unlink+0x120>
  if(ip->type == T_DIR && !isdirempty(ip)){
    8000585e:	04491703          	lh	a4,68(s2)
    80005862:	4785                	li	a5,1
    80005864:	08f70563          	beq	a4,a5,800058ee <sys_unlink+0x130>
  memset(&de, 0, sizeof(de));
    80005868:	4641                	li	a2,16
    8000586a:	4581                	li	a1,0
    8000586c:	fc040513          	addi	a0,s0,-64
    80005870:	ffffb097          	auipc	ra,0xffffb
    80005874:	470080e7          	jalr	1136(ra) # 80000ce0 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80005878:	4741                	li	a4,16
    8000587a:	f2c42683          	lw	a3,-212(s0)
    8000587e:	fc040613          	addi	a2,s0,-64
    80005882:	4581                	li	a1,0
    80005884:	8526                	mv	a0,s1
    80005886:	ffffe097          	auipc	ra,0xffffe
    8000588a:	54a080e7          	jalr	1354(ra) # 80003dd0 <writei>
    8000588e:	47c1                	li	a5,16
    80005890:	0af51563          	bne	a0,a5,8000593a <sys_unlink+0x17c>
  if(ip->type == T_DIR){
    80005894:	04491703          	lh	a4,68(s2)
    80005898:	4785                	li	a5,1
    8000589a:	0af70863          	beq	a4,a5,8000594a <sys_unlink+0x18c>
  iunlockput(dp);
    8000589e:	8526                	mv	a0,s1
    800058a0:	ffffe097          	auipc	ra,0xffffe
    800058a4:	3e6080e7          	jalr	998(ra) # 80003c86 <iunlockput>
  ip->nlink--;
    800058a8:	04a95783          	lhu	a5,74(s2)
    800058ac:	37fd                	addiw	a5,a5,-1
    800058ae:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    800058b2:	854a                	mv	a0,s2
    800058b4:	ffffe097          	auipc	ra,0xffffe
    800058b8:	0a6080e7          	jalr	166(ra) # 8000395a <iupdate>
  iunlockput(ip);
    800058bc:	854a                	mv	a0,s2
    800058be:	ffffe097          	auipc	ra,0xffffe
    800058c2:	3c8080e7          	jalr	968(ra) # 80003c86 <iunlockput>
  end_op();
    800058c6:	fffff097          	auipc	ra,0xfffff
    800058ca:	bb0080e7          	jalr	-1104(ra) # 80004476 <end_op>
  return 0;
    800058ce:	4501                	li	a0,0
    800058d0:	a84d                	j	80005982 <sys_unlink+0x1c4>
    end_op();
    800058d2:	fffff097          	auipc	ra,0xfffff
    800058d6:	ba4080e7          	jalr	-1116(ra) # 80004476 <end_op>
    return -1;
    800058da:	557d                	li	a0,-1
    800058dc:	a05d                	j	80005982 <sys_unlink+0x1c4>
    panic("unlink: nlink < 1");
    800058de:	00003517          	auipc	a0,0x3
    800058e2:	e6250513          	addi	a0,a0,-414 # 80008740 <syscalls+0x2f0>
    800058e6:	ffffb097          	auipc	ra,0xffffb
    800058ea:	c58080e7          	jalr	-936(ra) # 8000053e <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    800058ee:	04c92703          	lw	a4,76(s2)
    800058f2:	02000793          	li	a5,32
    800058f6:	f6e7f9e3          	bgeu	a5,a4,80005868 <sys_unlink+0xaa>
    800058fa:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800058fe:	4741                	li	a4,16
    80005900:	86ce                	mv	a3,s3
    80005902:	f1840613          	addi	a2,s0,-232
    80005906:	4581                	li	a1,0
    80005908:	854a                	mv	a0,s2
    8000590a:	ffffe097          	auipc	ra,0xffffe
    8000590e:	3ce080e7          	jalr	974(ra) # 80003cd8 <readi>
    80005912:	47c1                	li	a5,16
    80005914:	00f51b63          	bne	a0,a5,8000592a <sys_unlink+0x16c>
    if(de.inum != 0)
    80005918:	f1845783          	lhu	a5,-232(s0)
    8000591c:	e7a1                	bnez	a5,80005964 <sys_unlink+0x1a6>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    8000591e:	29c1                	addiw	s3,s3,16
    80005920:	04c92783          	lw	a5,76(s2)
    80005924:	fcf9ede3          	bltu	s3,a5,800058fe <sys_unlink+0x140>
    80005928:	b781                	j	80005868 <sys_unlink+0xaa>
      panic("isdirempty: readi");
    8000592a:	00003517          	auipc	a0,0x3
    8000592e:	e2e50513          	addi	a0,a0,-466 # 80008758 <syscalls+0x308>
    80005932:	ffffb097          	auipc	ra,0xffffb
    80005936:	c0c080e7          	jalr	-1012(ra) # 8000053e <panic>
    panic("unlink: writei");
    8000593a:	00003517          	auipc	a0,0x3
    8000593e:	e3650513          	addi	a0,a0,-458 # 80008770 <syscalls+0x320>
    80005942:	ffffb097          	auipc	ra,0xffffb
    80005946:	bfc080e7          	jalr	-1028(ra) # 8000053e <panic>
    dp->nlink--;
    8000594a:	04a4d783          	lhu	a5,74(s1)
    8000594e:	37fd                	addiw	a5,a5,-1
    80005950:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80005954:	8526                	mv	a0,s1
    80005956:	ffffe097          	auipc	ra,0xffffe
    8000595a:	004080e7          	jalr	4(ra) # 8000395a <iupdate>
    8000595e:	b781                	j	8000589e <sys_unlink+0xe0>
    return -1;
    80005960:	557d                	li	a0,-1
    80005962:	a005                	j	80005982 <sys_unlink+0x1c4>
    iunlockput(ip);
    80005964:	854a                	mv	a0,s2
    80005966:	ffffe097          	auipc	ra,0xffffe
    8000596a:	320080e7          	jalr	800(ra) # 80003c86 <iunlockput>
  iunlockput(dp);
    8000596e:	8526                	mv	a0,s1
    80005970:	ffffe097          	auipc	ra,0xffffe
    80005974:	316080e7          	jalr	790(ra) # 80003c86 <iunlockput>
  end_op();
    80005978:	fffff097          	auipc	ra,0xfffff
    8000597c:	afe080e7          	jalr	-1282(ra) # 80004476 <end_op>
  return -1;
    80005980:	557d                	li	a0,-1
}
    80005982:	70ae                	ld	ra,232(sp)
    80005984:	740e                	ld	s0,224(sp)
    80005986:	64ee                	ld	s1,216(sp)
    80005988:	694e                	ld	s2,208(sp)
    8000598a:	69ae                	ld	s3,200(sp)
    8000598c:	616d                	addi	sp,sp,240
    8000598e:	8082                	ret

0000000080005990 <sys_open>:

uint64
sys_open(void)
{
    80005990:	7131                	addi	sp,sp,-192
    80005992:	fd06                	sd	ra,184(sp)
    80005994:	f922                	sd	s0,176(sp)
    80005996:	f526                	sd	s1,168(sp)
    80005998:	f14a                	sd	s2,160(sp)
    8000599a:	ed4e                	sd	s3,152(sp)
    8000599c:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;

  if((n = argstr(0, path, MAXPATH)) < 0 || argint(1, &omode) < 0)
    8000599e:	08000613          	li	a2,128
    800059a2:	f5040593          	addi	a1,s0,-176
    800059a6:	4501                	li	a0,0
    800059a8:	ffffd097          	auipc	ra,0xffffd
    800059ac:	504080e7          	jalr	1284(ra) # 80002eac <argstr>
    return -1;
    800059b0:	54fd                	li	s1,-1
  if((n = argstr(0, path, MAXPATH)) < 0 || argint(1, &omode) < 0)
    800059b2:	0c054163          	bltz	a0,80005a74 <sys_open+0xe4>
    800059b6:	f4c40593          	addi	a1,s0,-180
    800059ba:	4505                	li	a0,1
    800059bc:	ffffd097          	auipc	ra,0xffffd
    800059c0:	4ac080e7          	jalr	1196(ra) # 80002e68 <argint>
    800059c4:	0a054863          	bltz	a0,80005a74 <sys_open+0xe4>

  begin_op();
    800059c8:	fffff097          	auipc	ra,0xfffff
    800059cc:	a2e080e7          	jalr	-1490(ra) # 800043f6 <begin_op>

  if(omode & O_CREATE){
    800059d0:	f4c42783          	lw	a5,-180(s0)
    800059d4:	2007f793          	andi	a5,a5,512
    800059d8:	cbdd                	beqz	a5,80005a8e <sys_open+0xfe>
    ip = create(path, T_FILE, 0, 0);
    800059da:	4681                	li	a3,0
    800059dc:	4601                	li	a2,0
    800059de:	4589                	li	a1,2
    800059e0:	f5040513          	addi	a0,s0,-176
    800059e4:	00000097          	auipc	ra,0x0
    800059e8:	972080e7          	jalr	-1678(ra) # 80005356 <create>
    800059ec:	892a                	mv	s2,a0
    if(ip == 0){
    800059ee:	c959                	beqz	a0,80005a84 <sys_open+0xf4>
      end_op();
      return -1;
    }
  }

  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    800059f0:	04491703          	lh	a4,68(s2)
    800059f4:	478d                	li	a5,3
    800059f6:	00f71763          	bne	a4,a5,80005a04 <sys_open+0x74>
    800059fa:	04695703          	lhu	a4,70(s2)
    800059fe:	47a5                	li	a5,9
    80005a00:	0ce7ec63          	bltu	a5,a4,80005ad8 <sys_open+0x148>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    80005a04:	fffff097          	auipc	ra,0xfffff
    80005a08:	e02080e7          	jalr	-510(ra) # 80004806 <filealloc>
    80005a0c:	89aa                	mv	s3,a0
    80005a0e:	10050263          	beqz	a0,80005b12 <sys_open+0x182>
    80005a12:	00000097          	auipc	ra,0x0
    80005a16:	902080e7          	jalr	-1790(ra) # 80005314 <fdalloc>
    80005a1a:	84aa                	mv	s1,a0
    80005a1c:	0e054663          	bltz	a0,80005b08 <sys_open+0x178>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if(ip->type == T_DEVICE){
    80005a20:	04491703          	lh	a4,68(s2)
    80005a24:	478d                	li	a5,3
    80005a26:	0cf70463          	beq	a4,a5,80005aee <sys_open+0x15e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80005a2a:	4789                	li	a5,2
    80005a2c:	00f9a023          	sw	a5,0(s3)
    f->off = 0;
    80005a30:	0209a023          	sw	zero,32(s3)
  }
  f->ip = ip;
    80005a34:	0129bc23          	sd	s2,24(s3)
  f->readable = !(omode & O_WRONLY);
    80005a38:	f4c42783          	lw	a5,-180(s0)
    80005a3c:	0017c713          	xori	a4,a5,1
    80005a40:	8b05                	andi	a4,a4,1
    80005a42:	00e98423          	sb	a4,8(s3)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    80005a46:	0037f713          	andi	a4,a5,3
    80005a4a:	00e03733          	snez	a4,a4
    80005a4e:	00e984a3          	sb	a4,9(s3)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80005a52:	4007f793          	andi	a5,a5,1024
    80005a56:	c791                	beqz	a5,80005a62 <sys_open+0xd2>
    80005a58:	04491703          	lh	a4,68(s2)
    80005a5c:	4789                	li	a5,2
    80005a5e:	08f70f63          	beq	a4,a5,80005afc <sys_open+0x16c>
    itrunc(ip);
  }

  iunlock(ip);
    80005a62:	854a                	mv	a0,s2
    80005a64:	ffffe097          	auipc	ra,0xffffe
    80005a68:	082080e7          	jalr	130(ra) # 80003ae6 <iunlock>
  end_op();
    80005a6c:	fffff097          	auipc	ra,0xfffff
    80005a70:	a0a080e7          	jalr	-1526(ra) # 80004476 <end_op>

  return fd;
}
    80005a74:	8526                	mv	a0,s1
    80005a76:	70ea                	ld	ra,184(sp)
    80005a78:	744a                	ld	s0,176(sp)
    80005a7a:	74aa                	ld	s1,168(sp)
    80005a7c:	790a                	ld	s2,160(sp)
    80005a7e:	69ea                	ld	s3,152(sp)
    80005a80:	6129                	addi	sp,sp,192
    80005a82:	8082                	ret
      end_op();
    80005a84:	fffff097          	auipc	ra,0xfffff
    80005a88:	9f2080e7          	jalr	-1550(ra) # 80004476 <end_op>
      return -1;
    80005a8c:	b7e5                	j	80005a74 <sys_open+0xe4>
    if((ip = namei(path)) == 0){
    80005a8e:	f5040513          	addi	a0,s0,-176
    80005a92:	ffffe097          	auipc	ra,0xffffe
    80005a96:	748080e7          	jalr	1864(ra) # 800041da <namei>
    80005a9a:	892a                	mv	s2,a0
    80005a9c:	c905                	beqz	a0,80005acc <sys_open+0x13c>
    ilock(ip);
    80005a9e:	ffffe097          	auipc	ra,0xffffe
    80005aa2:	f86080e7          	jalr	-122(ra) # 80003a24 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    80005aa6:	04491703          	lh	a4,68(s2)
    80005aaa:	4785                	li	a5,1
    80005aac:	f4f712e3          	bne	a4,a5,800059f0 <sys_open+0x60>
    80005ab0:	f4c42783          	lw	a5,-180(s0)
    80005ab4:	dba1                	beqz	a5,80005a04 <sys_open+0x74>
      iunlockput(ip);
    80005ab6:	854a                	mv	a0,s2
    80005ab8:	ffffe097          	auipc	ra,0xffffe
    80005abc:	1ce080e7          	jalr	462(ra) # 80003c86 <iunlockput>
      end_op();
    80005ac0:	fffff097          	auipc	ra,0xfffff
    80005ac4:	9b6080e7          	jalr	-1610(ra) # 80004476 <end_op>
      return -1;
    80005ac8:	54fd                	li	s1,-1
    80005aca:	b76d                	j	80005a74 <sys_open+0xe4>
      end_op();
    80005acc:	fffff097          	auipc	ra,0xfffff
    80005ad0:	9aa080e7          	jalr	-1622(ra) # 80004476 <end_op>
      return -1;
    80005ad4:	54fd                	li	s1,-1
    80005ad6:	bf79                	j	80005a74 <sys_open+0xe4>
    iunlockput(ip);
    80005ad8:	854a                	mv	a0,s2
    80005ada:	ffffe097          	auipc	ra,0xffffe
    80005ade:	1ac080e7          	jalr	428(ra) # 80003c86 <iunlockput>
    end_op();
    80005ae2:	fffff097          	auipc	ra,0xfffff
    80005ae6:	994080e7          	jalr	-1644(ra) # 80004476 <end_op>
    return -1;
    80005aea:	54fd                	li	s1,-1
    80005aec:	b761                	j	80005a74 <sys_open+0xe4>
    f->type = FD_DEVICE;
    80005aee:	00f9a023          	sw	a5,0(s3)
    f->major = ip->major;
    80005af2:	04691783          	lh	a5,70(s2)
    80005af6:	02f99223          	sh	a5,36(s3)
    80005afa:	bf2d                	j	80005a34 <sys_open+0xa4>
    itrunc(ip);
    80005afc:	854a                	mv	a0,s2
    80005afe:	ffffe097          	auipc	ra,0xffffe
    80005b02:	034080e7          	jalr	52(ra) # 80003b32 <itrunc>
    80005b06:	bfb1                	j	80005a62 <sys_open+0xd2>
      fileclose(f);
    80005b08:	854e                	mv	a0,s3
    80005b0a:	fffff097          	auipc	ra,0xfffff
    80005b0e:	db8080e7          	jalr	-584(ra) # 800048c2 <fileclose>
    iunlockput(ip);
    80005b12:	854a                	mv	a0,s2
    80005b14:	ffffe097          	auipc	ra,0xffffe
    80005b18:	172080e7          	jalr	370(ra) # 80003c86 <iunlockput>
    end_op();
    80005b1c:	fffff097          	auipc	ra,0xfffff
    80005b20:	95a080e7          	jalr	-1702(ra) # 80004476 <end_op>
    return -1;
    80005b24:	54fd                	li	s1,-1
    80005b26:	b7b9                	j	80005a74 <sys_open+0xe4>

0000000080005b28 <sys_mkdir>:

uint64
sys_mkdir(void)
{
    80005b28:	7175                	addi	sp,sp,-144
    80005b2a:	e506                	sd	ra,136(sp)
    80005b2c:	e122                	sd	s0,128(sp)
    80005b2e:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    80005b30:	fffff097          	auipc	ra,0xfffff
    80005b34:	8c6080e7          	jalr	-1850(ra) # 800043f6 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    80005b38:	08000613          	li	a2,128
    80005b3c:	f7040593          	addi	a1,s0,-144
    80005b40:	4501                	li	a0,0
    80005b42:	ffffd097          	auipc	ra,0xffffd
    80005b46:	36a080e7          	jalr	874(ra) # 80002eac <argstr>
    80005b4a:	02054963          	bltz	a0,80005b7c <sys_mkdir+0x54>
    80005b4e:	4681                	li	a3,0
    80005b50:	4601                	li	a2,0
    80005b52:	4585                	li	a1,1
    80005b54:	f7040513          	addi	a0,s0,-144
    80005b58:	fffff097          	auipc	ra,0xfffff
    80005b5c:	7fe080e7          	jalr	2046(ra) # 80005356 <create>
    80005b60:	cd11                	beqz	a0,80005b7c <sys_mkdir+0x54>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005b62:	ffffe097          	auipc	ra,0xffffe
    80005b66:	124080e7          	jalr	292(ra) # 80003c86 <iunlockput>
  end_op();
    80005b6a:	fffff097          	auipc	ra,0xfffff
    80005b6e:	90c080e7          	jalr	-1780(ra) # 80004476 <end_op>
  return 0;
    80005b72:	4501                	li	a0,0
}
    80005b74:	60aa                	ld	ra,136(sp)
    80005b76:	640a                	ld	s0,128(sp)
    80005b78:	6149                	addi	sp,sp,144
    80005b7a:	8082                	ret
    end_op();
    80005b7c:	fffff097          	auipc	ra,0xfffff
    80005b80:	8fa080e7          	jalr	-1798(ra) # 80004476 <end_op>
    return -1;
    80005b84:	557d                	li	a0,-1
    80005b86:	b7fd                	j	80005b74 <sys_mkdir+0x4c>

0000000080005b88 <sys_mknod>:

uint64
sys_mknod(void)
{
    80005b88:	7135                	addi	sp,sp,-160
    80005b8a:	ed06                	sd	ra,152(sp)
    80005b8c:	e922                	sd	s0,144(sp)
    80005b8e:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    80005b90:	fffff097          	auipc	ra,0xfffff
    80005b94:	866080e7          	jalr	-1946(ra) # 800043f6 <begin_op>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005b98:	08000613          	li	a2,128
    80005b9c:	f7040593          	addi	a1,s0,-144
    80005ba0:	4501                	li	a0,0
    80005ba2:	ffffd097          	auipc	ra,0xffffd
    80005ba6:	30a080e7          	jalr	778(ra) # 80002eac <argstr>
    80005baa:	04054a63          	bltz	a0,80005bfe <sys_mknod+0x76>
     argint(1, &major) < 0 ||
    80005bae:	f6c40593          	addi	a1,s0,-148
    80005bb2:	4505                	li	a0,1
    80005bb4:	ffffd097          	auipc	ra,0xffffd
    80005bb8:	2b4080e7          	jalr	692(ra) # 80002e68 <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005bbc:	04054163          	bltz	a0,80005bfe <sys_mknod+0x76>
     argint(2, &minor) < 0 ||
    80005bc0:	f6840593          	addi	a1,s0,-152
    80005bc4:	4509                	li	a0,2
    80005bc6:	ffffd097          	auipc	ra,0xffffd
    80005bca:	2a2080e7          	jalr	674(ra) # 80002e68 <argint>
     argint(1, &major) < 0 ||
    80005bce:	02054863          	bltz	a0,80005bfe <sys_mknod+0x76>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    80005bd2:	f6841683          	lh	a3,-152(s0)
    80005bd6:	f6c41603          	lh	a2,-148(s0)
    80005bda:	458d                	li	a1,3
    80005bdc:	f7040513          	addi	a0,s0,-144
    80005be0:	fffff097          	auipc	ra,0xfffff
    80005be4:	776080e7          	jalr	1910(ra) # 80005356 <create>
     argint(2, &minor) < 0 ||
    80005be8:	c919                	beqz	a0,80005bfe <sys_mknod+0x76>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005bea:	ffffe097          	auipc	ra,0xffffe
    80005bee:	09c080e7          	jalr	156(ra) # 80003c86 <iunlockput>
  end_op();
    80005bf2:	fffff097          	auipc	ra,0xfffff
    80005bf6:	884080e7          	jalr	-1916(ra) # 80004476 <end_op>
  return 0;
    80005bfa:	4501                	li	a0,0
    80005bfc:	a031                	j	80005c08 <sys_mknod+0x80>
    end_op();
    80005bfe:	fffff097          	auipc	ra,0xfffff
    80005c02:	878080e7          	jalr	-1928(ra) # 80004476 <end_op>
    return -1;
    80005c06:	557d                	li	a0,-1
}
    80005c08:	60ea                	ld	ra,152(sp)
    80005c0a:	644a                	ld	s0,144(sp)
    80005c0c:	610d                	addi	sp,sp,160
    80005c0e:	8082                	ret

0000000080005c10 <sys_chdir>:

uint64
sys_chdir(void)
{
    80005c10:	7135                	addi	sp,sp,-160
    80005c12:	ed06                	sd	ra,152(sp)
    80005c14:	e922                	sd	s0,144(sp)
    80005c16:	e526                	sd	s1,136(sp)
    80005c18:	e14a                	sd	s2,128(sp)
    80005c1a:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005c1c:	ffffc097          	auipc	ra,0xffffc
    80005c20:	f68080e7          	jalr	-152(ra) # 80001b84 <myproc>
    80005c24:	892a                	mv	s2,a0
  
  begin_op();
    80005c26:	ffffe097          	auipc	ra,0xffffe
    80005c2a:	7d0080e7          	jalr	2000(ra) # 800043f6 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    80005c2e:	08000613          	li	a2,128
    80005c32:	f6040593          	addi	a1,s0,-160
    80005c36:	4501                	li	a0,0
    80005c38:	ffffd097          	auipc	ra,0xffffd
    80005c3c:	274080e7          	jalr	628(ra) # 80002eac <argstr>
    80005c40:	04054b63          	bltz	a0,80005c96 <sys_chdir+0x86>
    80005c44:	f6040513          	addi	a0,s0,-160
    80005c48:	ffffe097          	auipc	ra,0xffffe
    80005c4c:	592080e7          	jalr	1426(ra) # 800041da <namei>
    80005c50:	84aa                	mv	s1,a0
    80005c52:	c131                	beqz	a0,80005c96 <sys_chdir+0x86>
    end_op();
    return -1;
  }
  ilock(ip);
    80005c54:	ffffe097          	auipc	ra,0xffffe
    80005c58:	dd0080e7          	jalr	-560(ra) # 80003a24 <ilock>
  if(ip->type != T_DIR){
    80005c5c:	04449703          	lh	a4,68(s1)
    80005c60:	4785                	li	a5,1
    80005c62:	04f71063          	bne	a4,a5,80005ca2 <sys_chdir+0x92>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    80005c66:	8526                	mv	a0,s1
    80005c68:	ffffe097          	auipc	ra,0xffffe
    80005c6c:	e7e080e7          	jalr	-386(ra) # 80003ae6 <iunlock>
  iput(p->cwd);
    80005c70:	15093503          	ld	a0,336(s2)
    80005c74:	ffffe097          	auipc	ra,0xffffe
    80005c78:	f6a080e7          	jalr	-150(ra) # 80003bde <iput>
  end_op();
    80005c7c:	ffffe097          	auipc	ra,0xffffe
    80005c80:	7fa080e7          	jalr	2042(ra) # 80004476 <end_op>
  p->cwd = ip;
    80005c84:	14993823          	sd	s1,336(s2)
  return 0;
    80005c88:	4501                	li	a0,0
}
    80005c8a:	60ea                	ld	ra,152(sp)
    80005c8c:	644a                	ld	s0,144(sp)
    80005c8e:	64aa                	ld	s1,136(sp)
    80005c90:	690a                	ld	s2,128(sp)
    80005c92:	610d                	addi	sp,sp,160
    80005c94:	8082                	ret
    end_op();
    80005c96:	ffffe097          	auipc	ra,0xffffe
    80005c9a:	7e0080e7          	jalr	2016(ra) # 80004476 <end_op>
    return -1;
    80005c9e:	557d                	li	a0,-1
    80005ca0:	b7ed                	j	80005c8a <sys_chdir+0x7a>
    iunlockput(ip);
    80005ca2:	8526                	mv	a0,s1
    80005ca4:	ffffe097          	auipc	ra,0xffffe
    80005ca8:	fe2080e7          	jalr	-30(ra) # 80003c86 <iunlockput>
    end_op();
    80005cac:	ffffe097          	auipc	ra,0xffffe
    80005cb0:	7ca080e7          	jalr	1994(ra) # 80004476 <end_op>
    return -1;
    80005cb4:	557d                	li	a0,-1
    80005cb6:	bfd1                	j	80005c8a <sys_chdir+0x7a>

0000000080005cb8 <sys_exec>:

uint64
sys_exec(void)
{
    80005cb8:	7145                	addi	sp,sp,-464
    80005cba:	e786                	sd	ra,456(sp)
    80005cbc:	e3a2                	sd	s0,448(sp)
    80005cbe:	ff26                	sd	s1,440(sp)
    80005cc0:	fb4a                	sd	s2,432(sp)
    80005cc2:	f74e                	sd	s3,424(sp)
    80005cc4:	f352                	sd	s4,416(sp)
    80005cc6:	ef56                	sd	s5,408(sp)
    80005cc8:	0b80                	addi	s0,sp,464
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  if(argstr(0, path, MAXPATH) < 0 || argaddr(1, &uargv) < 0){
    80005cca:	08000613          	li	a2,128
    80005cce:	f4040593          	addi	a1,s0,-192
    80005cd2:	4501                	li	a0,0
    80005cd4:	ffffd097          	auipc	ra,0xffffd
    80005cd8:	1d8080e7          	jalr	472(ra) # 80002eac <argstr>
    return -1;
    80005cdc:	597d                	li	s2,-1
  if(argstr(0, path, MAXPATH) < 0 || argaddr(1, &uargv) < 0){
    80005cde:	0c054a63          	bltz	a0,80005db2 <sys_exec+0xfa>
    80005ce2:	e3840593          	addi	a1,s0,-456
    80005ce6:	4505                	li	a0,1
    80005ce8:	ffffd097          	auipc	ra,0xffffd
    80005cec:	1a2080e7          	jalr	418(ra) # 80002e8a <argaddr>
    80005cf0:	0c054163          	bltz	a0,80005db2 <sys_exec+0xfa>
  }
  memset(argv, 0, sizeof(argv));
    80005cf4:	10000613          	li	a2,256
    80005cf8:	4581                	li	a1,0
    80005cfa:	e4040513          	addi	a0,s0,-448
    80005cfe:	ffffb097          	auipc	ra,0xffffb
    80005d02:	fe2080e7          	jalr	-30(ra) # 80000ce0 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    80005d06:	e4040493          	addi	s1,s0,-448
  memset(argv, 0, sizeof(argv));
    80005d0a:	89a6                	mv	s3,s1
    80005d0c:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    80005d0e:	02000a13          	li	s4,32
    80005d12:	00090a9b          	sext.w	s5,s2
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    80005d16:	00391513          	slli	a0,s2,0x3
    80005d1a:	e3040593          	addi	a1,s0,-464
    80005d1e:	e3843783          	ld	a5,-456(s0)
    80005d22:	953e                	add	a0,a0,a5
    80005d24:	ffffd097          	auipc	ra,0xffffd
    80005d28:	0aa080e7          	jalr	170(ra) # 80002dce <fetchaddr>
    80005d2c:	02054a63          	bltz	a0,80005d60 <sys_exec+0xa8>
      goto bad;
    }
    if(uarg == 0){
    80005d30:	e3043783          	ld	a5,-464(s0)
    80005d34:	c3b9                	beqz	a5,80005d7a <sys_exec+0xc2>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    80005d36:	ffffb097          	auipc	ra,0xffffb
    80005d3a:	dbe080e7          	jalr	-578(ra) # 80000af4 <kalloc>
    80005d3e:	85aa                	mv	a1,a0
    80005d40:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    80005d44:	cd11                	beqz	a0,80005d60 <sys_exec+0xa8>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005d46:	6605                	lui	a2,0x1
    80005d48:	e3043503          	ld	a0,-464(s0)
    80005d4c:	ffffd097          	auipc	ra,0xffffd
    80005d50:	0d4080e7          	jalr	212(ra) # 80002e20 <fetchstr>
    80005d54:	00054663          	bltz	a0,80005d60 <sys_exec+0xa8>
    if(i >= NELEM(argv)){
    80005d58:	0905                	addi	s2,s2,1
    80005d5a:	09a1                	addi	s3,s3,8
    80005d5c:	fb491be3          	bne	s2,s4,80005d12 <sys_exec+0x5a>
    kfree(argv[i]);

  return ret;

 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005d60:	10048913          	addi	s2,s1,256
    80005d64:	6088                	ld	a0,0(s1)
    80005d66:	c529                	beqz	a0,80005db0 <sys_exec+0xf8>
    kfree(argv[i]);
    80005d68:	ffffb097          	auipc	ra,0xffffb
    80005d6c:	c90080e7          	jalr	-880(ra) # 800009f8 <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005d70:	04a1                	addi	s1,s1,8
    80005d72:	ff2499e3          	bne	s1,s2,80005d64 <sys_exec+0xac>
  return -1;
    80005d76:	597d                	li	s2,-1
    80005d78:	a82d                	j	80005db2 <sys_exec+0xfa>
      argv[i] = 0;
    80005d7a:	0a8e                	slli	s5,s5,0x3
    80005d7c:	fc040793          	addi	a5,s0,-64
    80005d80:	9abe                	add	s5,s5,a5
    80005d82:	e80ab023          	sd	zero,-384(s5)
  int ret = exec(path, argv);
    80005d86:	e4040593          	addi	a1,s0,-448
    80005d8a:	f4040513          	addi	a0,s0,-192
    80005d8e:	fffff097          	auipc	ra,0xfffff
    80005d92:	194080e7          	jalr	404(ra) # 80004f22 <exec>
    80005d96:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005d98:	10048993          	addi	s3,s1,256
    80005d9c:	6088                	ld	a0,0(s1)
    80005d9e:	c911                	beqz	a0,80005db2 <sys_exec+0xfa>
    kfree(argv[i]);
    80005da0:	ffffb097          	auipc	ra,0xffffb
    80005da4:	c58080e7          	jalr	-936(ra) # 800009f8 <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005da8:	04a1                	addi	s1,s1,8
    80005daa:	ff3499e3          	bne	s1,s3,80005d9c <sys_exec+0xe4>
    80005dae:	a011                	j	80005db2 <sys_exec+0xfa>
  return -1;
    80005db0:	597d                	li	s2,-1
}
    80005db2:	854a                	mv	a0,s2
    80005db4:	60be                	ld	ra,456(sp)
    80005db6:	641e                	ld	s0,448(sp)
    80005db8:	74fa                	ld	s1,440(sp)
    80005dba:	795a                	ld	s2,432(sp)
    80005dbc:	79ba                	ld	s3,424(sp)
    80005dbe:	7a1a                	ld	s4,416(sp)
    80005dc0:	6afa                	ld	s5,408(sp)
    80005dc2:	6179                	addi	sp,sp,464
    80005dc4:	8082                	ret

0000000080005dc6 <sys_pipe>:

uint64
sys_pipe(void)
{
    80005dc6:	7139                	addi	sp,sp,-64
    80005dc8:	fc06                	sd	ra,56(sp)
    80005dca:	f822                	sd	s0,48(sp)
    80005dcc:	f426                	sd	s1,40(sp)
    80005dce:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    80005dd0:	ffffc097          	auipc	ra,0xffffc
    80005dd4:	db4080e7          	jalr	-588(ra) # 80001b84 <myproc>
    80005dd8:	84aa                	mv	s1,a0

  if(argaddr(0, &fdarray) < 0)
    80005dda:	fd840593          	addi	a1,s0,-40
    80005dde:	4501                	li	a0,0
    80005de0:	ffffd097          	auipc	ra,0xffffd
    80005de4:	0aa080e7          	jalr	170(ra) # 80002e8a <argaddr>
    return -1;
    80005de8:	57fd                	li	a5,-1
  if(argaddr(0, &fdarray) < 0)
    80005dea:	0e054063          	bltz	a0,80005eca <sys_pipe+0x104>
  if(pipealloc(&rf, &wf) < 0)
    80005dee:	fc840593          	addi	a1,s0,-56
    80005df2:	fd040513          	addi	a0,s0,-48
    80005df6:	fffff097          	auipc	ra,0xfffff
    80005dfa:	dfc080e7          	jalr	-516(ra) # 80004bf2 <pipealloc>
    return -1;
    80005dfe:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    80005e00:	0c054563          	bltz	a0,80005eca <sys_pipe+0x104>
  fd0 = -1;
    80005e04:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    80005e08:	fd043503          	ld	a0,-48(s0)
    80005e0c:	fffff097          	auipc	ra,0xfffff
    80005e10:	508080e7          	jalr	1288(ra) # 80005314 <fdalloc>
    80005e14:	fca42223          	sw	a0,-60(s0)
    80005e18:	08054c63          	bltz	a0,80005eb0 <sys_pipe+0xea>
    80005e1c:	fc843503          	ld	a0,-56(s0)
    80005e20:	fffff097          	auipc	ra,0xfffff
    80005e24:	4f4080e7          	jalr	1268(ra) # 80005314 <fdalloc>
    80005e28:	fca42023          	sw	a0,-64(s0)
    80005e2c:	06054863          	bltz	a0,80005e9c <sys_pipe+0xd6>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005e30:	4691                	li	a3,4
    80005e32:	fc440613          	addi	a2,s0,-60
    80005e36:	fd843583          	ld	a1,-40(s0)
    80005e3a:	68a8                	ld	a0,80(s1)
    80005e3c:	ffffc097          	auipc	ra,0xffffc
    80005e40:	836080e7          	jalr	-1994(ra) # 80001672 <copyout>
    80005e44:	02054063          	bltz	a0,80005e64 <sys_pipe+0x9e>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    80005e48:	4691                	li	a3,4
    80005e4a:	fc040613          	addi	a2,s0,-64
    80005e4e:	fd843583          	ld	a1,-40(s0)
    80005e52:	0591                	addi	a1,a1,4
    80005e54:	68a8                	ld	a0,80(s1)
    80005e56:	ffffc097          	auipc	ra,0xffffc
    80005e5a:	81c080e7          	jalr	-2020(ra) # 80001672 <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  return 0;
    80005e5e:	4781                	li	a5,0
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005e60:	06055563          	bgez	a0,80005eca <sys_pipe+0x104>
    p->ofile[fd0] = 0;
    80005e64:	fc442783          	lw	a5,-60(s0)
    80005e68:	07e9                	addi	a5,a5,26
    80005e6a:	078e                	slli	a5,a5,0x3
    80005e6c:	97a6                	add	a5,a5,s1
    80005e6e:	0007b023          	sd	zero,0(a5)
    p->ofile[fd1] = 0;
    80005e72:	fc042503          	lw	a0,-64(s0)
    80005e76:	0569                	addi	a0,a0,26
    80005e78:	050e                	slli	a0,a0,0x3
    80005e7a:	9526                	add	a0,a0,s1
    80005e7c:	00053023          	sd	zero,0(a0)
    fileclose(rf);
    80005e80:	fd043503          	ld	a0,-48(s0)
    80005e84:	fffff097          	auipc	ra,0xfffff
    80005e88:	a3e080e7          	jalr	-1474(ra) # 800048c2 <fileclose>
    fileclose(wf);
    80005e8c:	fc843503          	ld	a0,-56(s0)
    80005e90:	fffff097          	auipc	ra,0xfffff
    80005e94:	a32080e7          	jalr	-1486(ra) # 800048c2 <fileclose>
    return -1;
    80005e98:	57fd                	li	a5,-1
    80005e9a:	a805                	j	80005eca <sys_pipe+0x104>
    if(fd0 >= 0)
    80005e9c:	fc442783          	lw	a5,-60(s0)
    80005ea0:	0007c863          	bltz	a5,80005eb0 <sys_pipe+0xea>
      p->ofile[fd0] = 0;
    80005ea4:	01a78513          	addi	a0,a5,26
    80005ea8:	050e                	slli	a0,a0,0x3
    80005eaa:	9526                	add	a0,a0,s1
    80005eac:	00053023          	sd	zero,0(a0)
    fileclose(rf);
    80005eb0:	fd043503          	ld	a0,-48(s0)
    80005eb4:	fffff097          	auipc	ra,0xfffff
    80005eb8:	a0e080e7          	jalr	-1522(ra) # 800048c2 <fileclose>
    fileclose(wf);
    80005ebc:	fc843503          	ld	a0,-56(s0)
    80005ec0:	fffff097          	auipc	ra,0xfffff
    80005ec4:	a02080e7          	jalr	-1534(ra) # 800048c2 <fileclose>
    return -1;
    80005ec8:	57fd                	li	a5,-1
}
    80005eca:	853e                	mv	a0,a5
    80005ecc:	70e2                	ld	ra,56(sp)
    80005ece:	7442                	ld	s0,48(sp)
    80005ed0:	74a2                	ld	s1,40(sp)
    80005ed2:	6121                	addi	sp,sp,64
    80005ed4:	8082                	ret
	...

0000000080005ee0 <kernelvec>:
    80005ee0:	7111                	addi	sp,sp,-256
    80005ee2:	e006                	sd	ra,0(sp)
    80005ee4:	e40a                	sd	sp,8(sp)
    80005ee6:	e80e                	sd	gp,16(sp)
    80005ee8:	ec12                	sd	tp,24(sp)
    80005eea:	f016                	sd	t0,32(sp)
    80005eec:	f41a                	sd	t1,40(sp)
    80005eee:	f81e                	sd	t2,48(sp)
    80005ef0:	fc22                	sd	s0,56(sp)
    80005ef2:	e0a6                	sd	s1,64(sp)
    80005ef4:	e4aa                	sd	a0,72(sp)
    80005ef6:	e8ae                	sd	a1,80(sp)
    80005ef8:	ecb2                	sd	a2,88(sp)
    80005efa:	f0b6                	sd	a3,96(sp)
    80005efc:	f4ba                	sd	a4,104(sp)
    80005efe:	f8be                	sd	a5,112(sp)
    80005f00:	fcc2                	sd	a6,120(sp)
    80005f02:	e146                	sd	a7,128(sp)
    80005f04:	e54a                	sd	s2,136(sp)
    80005f06:	e94e                	sd	s3,144(sp)
    80005f08:	ed52                	sd	s4,152(sp)
    80005f0a:	f156                	sd	s5,160(sp)
    80005f0c:	f55a                	sd	s6,168(sp)
    80005f0e:	f95e                	sd	s7,176(sp)
    80005f10:	fd62                	sd	s8,184(sp)
    80005f12:	e1e6                	sd	s9,192(sp)
    80005f14:	e5ea                	sd	s10,200(sp)
    80005f16:	e9ee                	sd	s11,208(sp)
    80005f18:	edf2                	sd	t3,216(sp)
    80005f1a:	f1f6                	sd	t4,224(sp)
    80005f1c:	f5fa                	sd	t5,232(sp)
    80005f1e:	f9fe                	sd	t6,240(sp)
    80005f20:	d7bfc0ef          	jal	ra,80002c9a <kerneltrap>
    80005f24:	6082                	ld	ra,0(sp)
    80005f26:	6122                	ld	sp,8(sp)
    80005f28:	61c2                	ld	gp,16(sp)
    80005f2a:	7282                	ld	t0,32(sp)
    80005f2c:	7322                	ld	t1,40(sp)
    80005f2e:	73c2                	ld	t2,48(sp)
    80005f30:	7462                	ld	s0,56(sp)
    80005f32:	6486                	ld	s1,64(sp)
    80005f34:	6526                	ld	a0,72(sp)
    80005f36:	65c6                	ld	a1,80(sp)
    80005f38:	6666                	ld	a2,88(sp)
    80005f3a:	7686                	ld	a3,96(sp)
    80005f3c:	7726                	ld	a4,104(sp)
    80005f3e:	77c6                	ld	a5,112(sp)
    80005f40:	7866                	ld	a6,120(sp)
    80005f42:	688a                	ld	a7,128(sp)
    80005f44:	692a                	ld	s2,136(sp)
    80005f46:	69ca                	ld	s3,144(sp)
    80005f48:	6a6a                	ld	s4,152(sp)
    80005f4a:	7a8a                	ld	s5,160(sp)
    80005f4c:	7b2a                	ld	s6,168(sp)
    80005f4e:	7bca                	ld	s7,176(sp)
    80005f50:	7c6a                	ld	s8,184(sp)
    80005f52:	6c8e                	ld	s9,192(sp)
    80005f54:	6d2e                	ld	s10,200(sp)
    80005f56:	6dce                	ld	s11,208(sp)
    80005f58:	6e6e                	ld	t3,216(sp)
    80005f5a:	7e8e                	ld	t4,224(sp)
    80005f5c:	7f2e                	ld	t5,232(sp)
    80005f5e:	7fce                	ld	t6,240(sp)
    80005f60:	6111                	addi	sp,sp,256
    80005f62:	10200073          	sret
    80005f66:	00000013          	nop
    80005f6a:	00000013          	nop
    80005f6e:	0001                	nop

0000000080005f70 <timervec>:
    80005f70:	34051573          	csrrw	a0,mscratch,a0
    80005f74:	e10c                	sd	a1,0(a0)
    80005f76:	e510                	sd	a2,8(a0)
    80005f78:	e914                	sd	a3,16(a0)
    80005f7a:	6d0c                	ld	a1,24(a0)
    80005f7c:	7110                	ld	a2,32(a0)
    80005f7e:	6194                	ld	a3,0(a1)
    80005f80:	96b2                	add	a3,a3,a2
    80005f82:	e194                	sd	a3,0(a1)
    80005f84:	4589                	li	a1,2
    80005f86:	14459073          	csrw	sip,a1
    80005f8a:	6914                	ld	a3,16(a0)
    80005f8c:	6510                	ld	a2,8(a0)
    80005f8e:	610c                	ld	a1,0(a0)
    80005f90:	34051573          	csrrw	a0,mscratch,a0
    80005f94:	30200073          	mret
	...

0000000080005f9a <plicinit>:
// the riscv Platform Level Interrupt Controller (PLIC).
//

void
plicinit(void)
{
    80005f9a:	1141                	addi	sp,sp,-16
    80005f9c:	e422                	sd	s0,8(sp)
    80005f9e:	0800                	addi	s0,sp,16
  // set desired IRQ priorities non-zero (otherwise disabled).
  *(uint32*)(PLIC + UART0_IRQ*4) = 1;
    80005fa0:	0c0007b7          	lui	a5,0xc000
    80005fa4:	4705                	li	a4,1
    80005fa6:	d798                	sw	a4,40(a5)
  *(uint32*)(PLIC + VIRTIO0_IRQ*4) = 1;
    80005fa8:	c3d8                	sw	a4,4(a5)
}
    80005faa:	6422                	ld	s0,8(sp)
    80005fac:	0141                	addi	sp,sp,16
    80005fae:	8082                	ret

0000000080005fb0 <plicinithart>:

void
plicinithart(void)
{
    80005fb0:	1141                	addi	sp,sp,-16
    80005fb2:	e406                	sd	ra,8(sp)
    80005fb4:	e022                	sd	s0,0(sp)
    80005fb6:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005fb8:	ffffc097          	auipc	ra,0xffffc
    80005fbc:	b98080e7          	jalr	-1128(ra) # 80001b50 <cpuid>
  
  // set uart's enable bit for this hart's S-mode. 
  *(uint32*)PLIC_SENABLE(hart)= (1 << UART0_IRQ) | (1 << VIRTIO0_IRQ);
    80005fc0:	0085171b          	slliw	a4,a0,0x8
    80005fc4:	0c0027b7          	lui	a5,0xc002
    80005fc8:	97ba                	add	a5,a5,a4
    80005fca:	40200713          	li	a4,1026
    80005fce:	08e7a023          	sw	a4,128(a5) # c002080 <_entry-0x73ffdf80>

  // set this hart's S-mode priority threshold to 0.
  *(uint32*)PLIC_SPRIORITY(hart) = 0;
    80005fd2:	00d5151b          	slliw	a0,a0,0xd
    80005fd6:	0c2017b7          	lui	a5,0xc201
    80005fda:	953e                	add	a0,a0,a5
    80005fdc:	00052023          	sw	zero,0(a0)
}
    80005fe0:	60a2                	ld	ra,8(sp)
    80005fe2:	6402                	ld	s0,0(sp)
    80005fe4:	0141                	addi	sp,sp,16
    80005fe6:	8082                	ret

0000000080005fe8 <plic_claim>:

// ask the PLIC what interrupt we should serve.
int
plic_claim(void)
{
    80005fe8:	1141                	addi	sp,sp,-16
    80005fea:	e406                	sd	ra,8(sp)
    80005fec:	e022                	sd	s0,0(sp)
    80005fee:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005ff0:	ffffc097          	auipc	ra,0xffffc
    80005ff4:	b60080e7          	jalr	-1184(ra) # 80001b50 <cpuid>
  int irq = *(uint32*)PLIC_SCLAIM(hart);
    80005ff8:	00d5179b          	slliw	a5,a0,0xd
    80005ffc:	0c201537          	lui	a0,0xc201
    80006000:	953e                	add	a0,a0,a5
  return irq;
}
    80006002:	4148                	lw	a0,4(a0)
    80006004:	60a2                	ld	ra,8(sp)
    80006006:	6402                	ld	s0,0(sp)
    80006008:	0141                	addi	sp,sp,16
    8000600a:	8082                	ret

000000008000600c <plic_complete>:

// tell the PLIC we've served this IRQ.
void
plic_complete(int irq)
{
    8000600c:	1101                	addi	sp,sp,-32
    8000600e:	ec06                	sd	ra,24(sp)
    80006010:	e822                	sd	s0,16(sp)
    80006012:	e426                	sd	s1,8(sp)
    80006014:	1000                	addi	s0,sp,32
    80006016:	84aa                	mv	s1,a0
  int hart = cpuid();
    80006018:	ffffc097          	auipc	ra,0xffffc
    8000601c:	b38080e7          	jalr	-1224(ra) # 80001b50 <cpuid>
  *(uint32*)PLIC_SCLAIM(hart) = irq;
    80006020:	00d5151b          	slliw	a0,a0,0xd
    80006024:	0c2017b7          	lui	a5,0xc201
    80006028:	97aa                	add	a5,a5,a0
    8000602a:	c3c4                	sw	s1,4(a5)
}
    8000602c:	60e2                	ld	ra,24(sp)
    8000602e:	6442                	ld	s0,16(sp)
    80006030:	64a2                	ld	s1,8(sp)
    80006032:	6105                	addi	sp,sp,32
    80006034:	8082                	ret

0000000080006036 <free_desc>:
}

// mark a descriptor as free.
static void
free_desc(int i)
{
    80006036:	1141                	addi	sp,sp,-16
    80006038:	e406                	sd	ra,8(sp)
    8000603a:	e022                	sd	s0,0(sp)
    8000603c:	0800                	addi	s0,sp,16
  if(i >= NUM)
    8000603e:	479d                	li	a5,7
    80006040:	06a7c963          	blt	a5,a0,800060b2 <free_desc+0x7c>
    panic("free_desc 1");
  if(disk.free[i])
    80006044:	0001d797          	auipc	a5,0x1d
    80006048:	fbc78793          	addi	a5,a5,-68 # 80023000 <disk>
    8000604c:	00a78733          	add	a4,a5,a0
    80006050:	6789                	lui	a5,0x2
    80006052:	97ba                	add	a5,a5,a4
    80006054:	0187c783          	lbu	a5,24(a5) # 2018 <_entry-0x7fffdfe8>
    80006058:	e7ad                	bnez	a5,800060c2 <free_desc+0x8c>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    8000605a:	00451793          	slli	a5,a0,0x4
    8000605e:	0001f717          	auipc	a4,0x1f
    80006062:	fa270713          	addi	a4,a4,-94 # 80025000 <disk+0x2000>
    80006066:	6314                	ld	a3,0(a4)
    80006068:	96be                	add	a3,a3,a5
    8000606a:	0006b023          	sd	zero,0(a3)
  disk.desc[i].len = 0;
    8000606e:	6314                	ld	a3,0(a4)
    80006070:	96be                	add	a3,a3,a5
    80006072:	0006a423          	sw	zero,8(a3)
  disk.desc[i].flags = 0;
    80006076:	6314                	ld	a3,0(a4)
    80006078:	96be                	add	a3,a3,a5
    8000607a:	00069623          	sh	zero,12(a3)
  disk.desc[i].next = 0;
    8000607e:	6318                	ld	a4,0(a4)
    80006080:	97ba                	add	a5,a5,a4
    80006082:	00079723          	sh	zero,14(a5)
  disk.free[i] = 1;
    80006086:	0001d797          	auipc	a5,0x1d
    8000608a:	f7a78793          	addi	a5,a5,-134 # 80023000 <disk>
    8000608e:	97aa                	add	a5,a5,a0
    80006090:	6509                	lui	a0,0x2
    80006092:	953e                	add	a0,a0,a5
    80006094:	4785                	li	a5,1
    80006096:	00f50c23          	sb	a5,24(a0) # 2018 <_entry-0x7fffdfe8>
  wakeup(&disk.free[0]);
    8000609a:	0001f517          	auipc	a0,0x1f
    8000609e:	f7e50513          	addi	a0,a0,-130 # 80025018 <disk+0x2018>
    800060a2:	ffffc097          	auipc	ra,0xffffc
    800060a6:	480080e7          	jalr	1152(ra) # 80002522 <wakeup>
}
    800060aa:	60a2                	ld	ra,8(sp)
    800060ac:	6402                	ld	s0,0(sp)
    800060ae:	0141                	addi	sp,sp,16
    800060b0:	8082                	ret
    panic("free_desc 1");
    800060b2:	00002517          	auipc	a0,0x2
    800060b6:	6ce50513          	addi	a0,a0,1742 # 80008780 <syscalls+0x330>
    800060ba:	ffffa097          	auipc	ra,0xffffa
    800060be:	484080e7          	jalr	1156(ra) # 8000053e <panic>
    panic("free_desc 2");
    800060c2:	00002517          	auipc	a0,0x2
    800060c6:	6ce50513          	addi	a0,a0,1742 # 80008790 <syscalls+0x340>
    800060ca:	ffffa097          	auipc	ra,0xffffa
    800060ce:	474080e7          	jalr	1140(ra) # 8000053e <panic>

00000000800060d2 <virtio_disk_init>:
{
    800060d2:	1101                	addi	sp,sp,-32
    800060d4:	ec06                	sd	ra,24(sp)
    800060d6:	e822                	sd	s0,16(sp)
    800060d8:	e426                	sd	s1,8(sp)
    800060da:	1000                	addi	s0,sp,32
  initlock(&disk.vdisk_lock, "virtio_disk");
    800060dc:	00002597          	auipc	a1,0x2
    800060e0:	6c458593          	addi	a1,a1,1732 # 800087a0 <syscalls+0x350>
    800060e4:	0001f517          	auipc	a0,0x1f
    800060e8:	04450513          	addi	a0,a0,68 # 80025128 <disk+0x2128>
    800060ec:	ffffb097          	auipc	ra,0xffffb
    800060f0:	a68080e7          	jalr	-1432(ra) # 80000b54 <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    800060f4:	100017b7          	lui	a5,0x10001
    800060f8:	4398                	lw	a4,0(a5)
    800060fa:	2701                	sext.w	a4,a4
    800060fc:	747277b7          	lui	a5,0x74727
    80006100:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80006104:	0ef71163          	bne	a4,a5,800061e6 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_VERSION) != 1 ||
    80006108:	100017b7          	lui	a5,0x10001
    8000610c:	43dc                	lw	a5,4(a5)
    8000610e:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80006110:	4705                	li	a4,1
    80006112:	0ce79a63          	bne	a5,a4,800061e6 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    80006116:	100017b7          	lui	a5,0x10001
    8000611a:	479c                	lw	a5,8(a5)
    8000611c:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 1 ||
    8000611e:	4709                	li	a4,2
    80006120:	0ce79363          	bne	a5,a4,800061e6 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_VENDOR_ID) != 0x554d4551){
    80006124:	100017b7          	lui	a5,0x10001
    80006128:	47d8                	lw	a4,12(a5)
    8000612a:	2701                	sext.w	a4,a4
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    8000612c:	554d47b7          	lui	a5,0x554d4
    80006130:	55178793          	addi	a5,a5,1361 # 554d4551 <_entry-0x2ab2baaf>
    80006134:	0af71963          	bne	a4,a5,800061e6 <virtio_disk_init+0x114>
  *R(VIRTIO_MMIO_STATUS) = status;
    80006138:	100017b7          	lui	a5,0x10001
    8000613c:	4705                	li	a4,1
    8000613e:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006140:	470d                	li	a4,3
    80006142:	dbb8                	sw	a4,112(a5)
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    80006144:	4b94                	lw	a3,16(a5)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    80006146:	c7ffe737          	lui	a4,0xc7ffe
    8000614a:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fd875f>
    8000614e:	8f75                	and	a4,a4,a3
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    80006150:	2701                	sext.w	a4,a4
    80006152:	d398                	sw	a4,32(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006154:	472d                	li	a4,11
    80006156:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006158:	473d                	li	a4,15
    8000615a:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_GUEST_PAGE_SIZE) = PGSIZE;
    8000615c:	6705                	lui	a4,0x1
    8000615e:	d798                	sw	a4,40(a5)
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    80006160:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    80006164:	5bdc                	lw	a5,52(a5)
    80006166:	2781                	sext.w	a5,a5
  if(max == 0)
    80006168:	c7d9                	beqz	a5,800061f6 <virtio_disk_init+0x124>
  if(max < NUM)
    8000616a:	471d                	li	a4,7
    8000616c:	08f77d63          	bgeu	a4,a5,80006206 <virtio_disk_init+0x134>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    80006170:	100014b7          	lui	s1,0x10001
    80006174:	47a1                	li	a5,8
    80006176:	dc9c                	sw	a5,56(s1)
  memset(disk.pages, 0, sizeof(disk.pages));
    80006178:	6609                	lui	a2,0x2
    8000617a:	4581                	li	a1,0
    8000617c:	0001d517          	auipc	a0,0x1d
    80006180:	e8450513          	addi	a0,a0,-380 # 80023000 <disk>
    80006184:	ffffb097          	auipc	ra,0xffffb
    80006188:	b5c080e7          	jalr	-1188(ra) # 80000ce0 <memset>
  *R(VIRTIO_MMIO_QUEUE_PFN) = ((uint64)disk.pages) >> PGSHIFT;
    8000618c:	0001d717          	auipc	a4,0x1d
    80006190:	e7470713          	addi	a4,a4,-396 # 80023000 <disk>
    80006194:	00c75793          	srli	a5,a4,0xc
    80006198:	2781                	sext.w	a5,a5
    8000619a:	c0bc                	sw	a5,64(s1)
  disk.desc = (struct virtq_desc *) disk.pages;
    8000619c:	0001f797          	auipc	a5,0x1f
    800061a0:	e6478793          	addi	a5,a5,-412 # 80025000 <disk+0x2000>
    800061a4:	e398                	sd	a4,0(a5)
  disk.avail = (struct virtq_avail *)(disk.pages + NUM*sizeof(struct virtq_desc));
    800061a6:	0001d717          	auipc	a4,0x1d
    800061aa:	eda70713          	addi	a4,a4,-294 # 80023080 <disk+0x80>
    800061ae:	e798                	sd	a4,8(a5)
  disk.used = (struct virtq_used *) (disk.pages + PGSIZE);
    800061b0:	0001e717          	auipc	a4,0x1e
    800061b4:	e5070713          	addi	a4,a4,-432 # 80024000 <disk+0x1000>
    800061b8:	eb98                	sd	a4,16(a5)
    disk.free[i] = 1;
    800061ba:	4705                	li	a4,1
    800061bc:	00e78c23          	sb	a4,24(a5)
    800061c0:	00e78ca3          	sb	a4,25(a5)
    800061c4:	00e78d23          	sb	a4,26(a5)
    800061c8:	00e78da3          	sb	a4,27(a5)
    800061cc:	00e78e23          	sb	a4,28(a5)
    800061d0:	00e78ea3          	sb	a4,29(a5)
    800061d4:	00e78f23          	sb	a4,30(a5)
    800061d8:	00e78fa3          	sb	a4,31(a5)
}
    800061dc:	60e2                	ld	ra,24(sp)
    800061de:	6442                	ld	s0,16(sp)
    800061e0:	64a2                	ld	s1,8(sp)
    800061e2:	6105                	addi	sp,sp,32
    800061e4:	8082                	ret
    panic("could not find virtio disk");
    800061e6:	00002517          	auipc	a0,0x2
    800061ea:	5ca50513          	addi	a0,a0,1482 # 800087b0 <syscalls+0x360>
    800061ee:	ffffa097          	auipc	ra,0xffffa
    800061f2:	350080e7          	jalr	848(ra) # 8000053e <panic>
    panic("virtio disk has no queue 0");
    800061f6:	00002517          	auipc	a0,0x2
    800061fa:	5da50513          	addi	a0,a0,1498 # 800087d0 <syscalls+0x380>
    800061fe:	ffffa097          	auipc	ra,0xffffa
    80006202:	340080e7          	jalr	832(ra) # 8000053e <panic>
    panic("virtio disk max queue too short");
    80006206:	00002517          	auipc	a0,0x2
    8000620a:	5ea50513          	addi	a0,a0,1514 # 800087f0 <syscalls+0x3a0>
    8000620e:	ffffa097          	auipc	ra,0xffffa
    80006212:	330080e7          	jalr	816(ra) # 8000053e <panic>

0000000080006216 <virtio_disk_rw>:
  return 0;
}

void
virtio_disk_rw(struct buf *b, int write)
{
    80006216:	7159                	addi	sp,sp,-112
    80006218:	f486                	sd	ra,104(sp)
    8000621a:	f0a2                	sd	s0,96(sp)
    8000621c:	eca6                	sd	s1,88(sp)
    8000621e:	e8ca                	sd	s2,80(sp)
    80006220:	e4ce                	sd	s3,72(sp)
    80006222:	e0d2                	sd	s4,64(sp)
    80006224:	fc56                	sd	s5,56(sp)
    80006226:	f85a                	sd	s6,48(sp)
    80006228:	f45e                	sd	s7,40(sp)
    8000622a:	f062                	sd	s8,32(sp)
    8000622c:	ec66                	sd	s9,24(sp)
    8000622e:	e86a                	sd	s10,16(sp)
    80006230:	1880                	addi	s0,sp,112
    80006232:	892a                	mv	s2,a0
    80006234:	8d2e                	mv	s10,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    80006236:	00c52c83          	lw	s9,12(a0)
    8000623a:	001c9c9b          	slliw	s9,s9,0x1
    8000623e:	1c82                	slli	s9,s9,0x20
    80006240:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    80006244:	0001f517          	auipc	a0,0x1f
    80006248:	ee450513          	addi	a0,a0,-284 # 80025128 <disk+0x2128>
    8000624c:	ffffb097          	auipc	ra,0xffffb
    80006250:	998080e7          	jalr	-1640(ra) # 80000be4 <acquire>
  for(int i = 0; i < 3; i++){
    80006254:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    80006256:	4c21                	li	s8,8
      disk.free[i] = 0;
    80006258:	0001db97          	auipc	s7,0x1d
    8000625c:	da8b8b93          	addi	s7,s7,-600 # 80023000 <disk>
    80006260:	6b09                	lui	s6,0x2
  for(int i = 0; i < 3; i++){
    80006262:	4a8d                	li	s5,3
  for(int i = 0; i < NUM; i++){
    80006264:	8a4e                	mv	s4,s3
    80006266:	a051                	j	800062ea <virtio_disk_rw+0xd4>
      disk.free[i] = 0;
    80006268:	00fb86b3          	add	a3,s7,a5
    8000626c:	96da                	add	a3,a3,s6
    8000626e:	00068c23          	sb	zero,24(a3)
    idx[i] = alloc_desc();
    80006272:	c21c                	sw	a5,0(a2)
    if(idx[i] < 0){
    80006274:	0207c563          	bltz	a5,8000629e <virtio_disk_rw+0x88>
  for(int i = 0; i < 3; i++){
    80006278:	2485                	addiw	s1,s1,1
    8000627a:	0711                	addi	a4,a4,4
    8000627c:	25548063          	beq	s1,s5,800064bc <virtio_disk_rw+0x2a6>
    idx[i] = alloc_desc();
    80006280:	863a                	mv	a2,a4
  for(int i = 0; i < NUM; i++){
    80006282:	0001f697          	auipc	a3,0x1f
    80006286:	d9668693          	addi	a3,a3,-618 # 80025018 <disk+0x2018>
    8000628a:	87d2                	mv	a5,s4
    if(disk.free[i]){
    8000628c:	0006c583          	lbu	a1,0(a3)
    80006290:	fde1                	bnez	a1,80006268 <virtio_disk_rw+0x52>
  for(int i = 0; i < NUM; i++){
    80006292:	2785                	addiw	a5,a5,1
    80006294:	0685                	addi	a3,a3,1
    80006296:	ff879be3          	bne	a5,s8,8000628c <virtio_disk_rw+0x76>
    idx[i] = alloc_desc();
    8000629a:	57fd                	li	a5,-1
    8000629c:	c21c                	sw	a5,0(a2)
      for(int j = 0; j < i; j++)
    8000629e:	02905a63          	blez	s1,800062d2 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    800062a2:	f9042503          	lw	a0,-112(s0)
    800062a6:	00000097          	auipc	ra,0x0
    800062aa:	d90080e7          	jalr	-624(ra) # 80006036 <free_desc>
      for(int j = 0; j < i; j++)
    800062ae:	4785                	li	a5,1
    800062b0:	0297d163          	bge	a5,s1,800062d2 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    800062b4:	f9442503          	lw	a0,-108(s0)
    800062b8:	00000097          	auipc	ra,0x0
    800062bc:	d7e080e7          	jalr	-642(ra) # 80006036 <free_desc>
      for(int j = 0; j < i; j++)
    800062c0:	4789                	li	a5,2
    800062c2:	0097d863          	bge	a5,s1,800062d2 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    800062c6:	f9842503          	lw	a0,-104(s0)
    800062ca:	00000097          	auipc	ra,0x0
    800062ce:	d6c080e7          	jalr	-660(ra) # 80006036 <free_desc>
  int idx[3];
  while(1){
    if(alloc3_desc(idx) == 0) {
      break;
    }
    sleep(&disk.free[0], &disk.vdisk_lock);
    800062d2:	0001f597          	auipc	a1,0x1f
    800062d6:	e5658593          	addi	a1,a1,-426 # 80025128 <disk+0x2128>
    800062da:	0001f517          	auipc	a0,0x1f
    800062de:	d3e50513          	addi	a0,a0,-706 # 80025018 <disk+0x2018>
    800062e2:	ffffc097          	auipc	ra,0xffffc
    800062e6:	0a2080e7          	jalr	162(ra) # 80002384 <sleep>
  for(int i = 0; i < 3; i++){
    800062ea:	f9040713          	addi	a4,s0,-112
    800062ee:	84ce                	mv	s1,s3
    800062f0:	bf41                	j	80006280 <virtio_disk_rw+0x6a>
  // qemu's virtio-blk.c reads them.

  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];

  if(write)
    buf0->type = VIRTIO_BLK_T_OUT; // write the disk
    800062f2:	20058713          	addi	a4,a1,512
    800062f6:	00471693          	slli	a3,a4,0x4
    800062fa:	0001d717          	auipc	a4,0x1d
    800062fe:	d0670713          	addi	a4,a4,-762 # 80023000 <disk>
    80006302:	9736                	add	a4,a4,a3
    80006304:	4685                	li	a3,1
    80006306:	0ad72423          	sw	a3,168(a4)
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    8000630a:	20058713          	addi	a4,a1,512
    8000630e:	00471693          	slli	a3,a4,0x4
    80006312:	0001d717          	auipc	a4,0x1d
    80006316:	cee70713          	addi	a4,a4,-786 # 80023000 <disk>
    8000631a:	9736                	add	a4,a4,a3
    8000631c:	0a072623          	sw	zero,172(a4)
  buf0->sector = sector;
    80006320:	0b973823          	sd	s9,176(a4)

  disk.desc[idx[0]].addr = (uint64) buf0;
    80006324:	7679                	lui	a2,0xffffe
    80006326:	963e                	add	a2,a2,a5
    80006328:	0001f697          	auipc	a3,0x1f
    8000632c:	cd868693          	addi	a3,a3,-808 # 80025000 <disk+0x2000>
    80006330:	6298                	ld	a4,0(a3)
    80006332:	9732                	add	a4,a4,a2
    80006334:	e308                	sd	a0,0(a4)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    80006336:	6298                	ld	a4,0(a3)
    80006338:	9732                	add	a4,a4,a2
    8000633a:	4541                	li	a0,16
    8000633c:	c708                	sw	a0,8(a4)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    8000633e:	6298                	ld	a4,0(a3)
    80006340:	9732                	add	a4,a4,a2
    80006342:	4505                	li	a0,1
    80006344:	00a71623          	sh	a0,12(a4)
  disk.desc[idx[0]].next = idx[1];
    80006348:	f9442703          	lw	a4,-108(s0)
    8000634c:	6288                	ld	a0,0(a3)
    8000634e:	962a                	add	a2,a2,a0
    80006350:	00e61723          	sh	a4,14(a2) # ffffffffffffe00e <end+0xffffffff7ffd800e>

  disk.desc[idx[1]].addr = (uint64) b->data;
    80006354:	0712                	slli	a4,a4,0x4
    80006356:	6290                	ld	a2,0(a3)
    80006358:	963a                	add	a2,a2,a4
    8000635a:	05890513          	addi	a0,s2,88
    8000635e:	e208                	sd	a0,0(a2)
  disk.desc[idx[1]].len = BSIZE;
    80006360:	6294                	ld	a3,0(a3)
    80006362:	96ba                	add	a3,a3,a4
    80006364:	40000613          	li	a2,1024
    80006368:	c690                	sw	a2,8(a3)
  if(write)
    8000636a:	140d0063          	beqz	s10,800064aa <virtio_disk_rw+0x294>
    disk.desc[idx[1]].flags = 0; // device reads b->data
    8000636e:	0001f697          	auipc	a3,0x1f
    80006372:	c926b683          	ld	a3,-878(a3) # 80025000 <disk+0x2000>
    80006376:	96ba                	add	a3,a3,a4
    80006378:	00069623          	sh	zero,12(a3)
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    8000637c:	0001d817          	auipc	a6,0x1d
    80006380:	c8480813          	addi	a6,a6,-892 # 80023000 <disk>
    80006384:	0001f517          	auipc	a0,0x1f
    80006388:	c7c50513          	addi	a0,a0,-900 # 80025000 <disk+0x2000>
    8000638c:	6114                	ld	a3,0(a0)
    8000638e:	96ba                	add	a3,a3,a4
    80006390:	00c6d603          	lhu	a2,12(a3)
    80006394:	00166613          	ori	a2,a2,1
    80006398:	00c69623          	sh	a2,12(a3)
  disk.desc[idx[1]].next = idx[2];
    8000639c:	f9842683          	lw	a3,-104(s0)
    800063a0:	6110                	ld	a2,0(a0)
    800063a2:	9732                	add	a4,a4,a2
    800063a4:	00d71723          	sh	a3,14(a4)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    800063a8:	20058613          	addi	a2,a1,512
    800063ac:	0612                	slli	a2,a2,0x4
    800063ae:	9642                	add	a2,a2,a6
    800063b0:	577d                	li	a4,-1
    800063b2:	02e60823          	sb	a4,48(a2)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    800063b6:	00469713          	slli	a4,a3,0x4
    800063ba:	6114                	ld	a3,0(a0)
    800063bc:	96ba                	add	a3,a3,a4
    800063be:	03078793          	addi	a5,a5,48
    800063c2:	97c2                	add	a5,a5,a6
    800063c4:	e29c                	sd	a5,0(a3)
  disk.desc[idx[2]].len = 1;
    800063c6:	611c                	ld	a5,0(a0)
    800063c8:	97ba                	add	a5,a5,a4
    800063ca:	4685                	li	a3,1
    800063cc:	c794                	sw	a3,8(a5)
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    800063ce:	611c                	ld	a5,0(a0)
    800063d0:	97ba                	add	a5,a5,a4
    800063d2:	4809                	li	a6,2
    800063d4:	01079623          	sh	a6,12(a5)
  disk.desc[idx[2]].next = 0;
    800063d8:	611c                	ld	a5,0(a0)
    800063da:	973e                	add	a4,a4,a5
    800063dc:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    800063e0:	00d92223          	sw	a3,4(s2)
  disk.info[idx[0]].b = b;
    800063e4:	03263423          	sd	s2,40(a2)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    800063e8:	6518                	ld	a4,8(a0)
    800063ea:	00275783          	lhu	a5,2(a4)
    800063ee:	8b9d                	andi	a5,a5,7
    800063f0:	0786                	slli	a5,a5,0x1
    800063f2:	97ba                	add	a5,a5,a4
    800063f4:	00b79223          	sh	a1,4(a5)

  __sync_synchronize();
    800063f8:	0ff0000f          	fence

  // tell the device another avail ring entry is available.
  disk.avail->idx += 1; // not % NUM ...
    800063fc:	6518                	ld	a4,8(a0)
    800063fe:	00275783          	lhu	a5,2(a4)
    80006402:	2785                	addiw	a5,a5,1
    80006404:	00f71123          	sh	a5,2(a4)

  __sync_synchronize();
    80006408:	0ff0000f          	fence

  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
    8000640c:	100017b7          	lui	a5,0x10001
    80006410:	0407a823          	sw	zero,80(a5) # 10001050 <_entry-0x6fffefb0>

  // Wait for virtio_disk_intr() to say request has finished.
  while(b->disk == 1) {
    80006414:	00492703          	lw	a4,4(s2)
    80006418:	4785                	li	a5,1
    8000641a:	02f71163          	bne	a4,a5,8000643c <virtio_disk_rw+0x226>
    sleep(b, &disk.vdisk_lock);
    8000641e:	0001f997          	auipc	s3,0x1f
    80006422:	d0a98993          	addi	s3,s3,-758 # 80025128 <disk+0x2128>
  while(b->disk == 1) {
    80006426:	4485                	li	s1,1
    sleep(b, &disk.vdisk_lock);
    80006428:	85ce                	mv	a1,s3
    8000642a:	854a                	mv	a0,s2
    8000642c:	ffffc097          	auipc	ra,0xffffc
    80006430:	f58080e7          	jalr	-168(ra) # 80002384 <sleep>
  while(b->disk == 1) {
    80006434:	00492783          	lw	a5,4(s2)
    80006438:	fe9788e3          	beq	a5,s1,80006428 <virtio_disk_rw+0x212>
  }

  disk.info[idx[0]].b = 0;
    8000643c:	f9042903          	lw	s2,-112(s0)
    80006440:	20090793          	addi	a5,s2,512
    80006444:	00479713          	slli	a4,a5,0x4
    80006448:	0001d797          	auipc	a5,0x1d
    8000644c:	bb878793          	addi	a5,a5,-1096 # 80023000 <disk>
    80006450:	97ba                	add	a5,a5,a4
    80006452:	0207b423          	sd	zero,40(a5)
    int flag = disk.desc[i].flags;
    80006456:	0001f997          	auipc	s3,0x1f
    8000645a:	baa98993          	addi	s3,s3,-1110 # 80025000 <disk+0x2000>
    8000645e:	00491713          	slli	a4,s2,0x4
    80006462:	0009b783          	ld	a5,0(s3)
    80006466:	97ba                	add	a5,a5,a4
    80006468:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    8000646c:	854a                	mv	a0,s2
    8000646e:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    80006472:	00000097          	auipc	ra,0x0
    80006476:	bc4080e7          	jalr	-1084(ra) # 80006036 <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    8000647a:	8885                	andi	s1,s1,1
    8000647c:	f0ed                	bnez	s1,8000645e <virtio_disk_rw+0x248>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    8000647e:	0001f517          	auipc	a0,0x1f
    80006482:	caa50513          	addi	a0,a0,-854 # 80025128 <disk+0x2128>
    80006486:	ffffb097          	auipc	ra,0xffffb
    8000648a:	812080e7          	jalr	-2030(ra) # 80000c98 <release>
}
    8000648e:	70a6                	ld	ra,104(sp)
    80006490:	7406                	ld	s0,96(sp)
    80006492:	64e6                	ld	s1,88(sp)
    80006494:	6946                	ld	s2,80(sp)
    80006496:	69a6                	ld	s3,72(sp)
    80006498:	6a06                	ld	s4,64(sp)
    8000649a:	7ae2                	ld	s5,56(sp)
    8000649c:	7b42                	ld	s6,48(sp)
    8000649e:	7ba2                	ld	s7,40(sp)
    800064a0:	7c02                	ld	s8,32(sp)
    800064a2:	6ce2                	ld	s9,24(sp)
    800064a4:	6d42                	ld	s10,16(sp)
    800064a6:	6165                	addi	sp,sp,112
    800064a8:	8082                	ret
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
    800064aa:	0001f697          	auipc	a3,0x1f
    800064ae:	b566b683          	ld	a3,-1194(a3) # 80025000 <disk+0x2000>
    800064b2:	96ba                	add	a3,a3,a4
    800064b4:	4609                	li	a2,2
    800064b6:	00c69623          	sh	a2,12(a3)
    800064ba:	b5c9                	j	8000637c <virtio_disk_rw+0x166>
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    800064bc:	f9042583          	lw	a1,-112(s0)
    800064c0:	20058793          	addi	a5,a1,512
    800064c4:	0792                	slli	a5,a5,0x4
    800064c6:	0001d517          	auipc	a0,0x1d
    800064ca:	be250513          	addi	a0,a0,-1054 # 800230a8 <disk+0xa8>
    800064ce:	953e                	add	a0,a0,a5
  if(write)
    800064d0:	e20d11e3          	bnez	s10,800062f2 <virtio_disk_rw+0xdc>
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
    800064d4:	20058713          	addi	a4,a1,512
    800064d8:	00471693          	slli	a3,a4,0x4
    800064dc:	0001d717          	auipc	a4,0x1d
    800064e0:	b2470713          	addi	a4,a4,-1244 # 80023000 <disk>
    800064e4:	9736                	add	a4,a4,a3
    800064e6:	0a072423          	sw	zero,168(a4)
    800064ea:	b505                	j	8000630a <virtio_disk_rw+0xf4>

00000000800064ec <virtio_disk_intr>:

void
virtio_disk_intr()
{
    800064ec:	1101                	addi	sp,sp,-32
    800064ee:	ec06                	sd	ra,24(sp)
    800064f0:	e822                	sd	s0,16(sp)
    800064f2:	e426                	sd	s1,8(sp)
    800064f4:	e04a                	sd	s2,0(sp)
    800064f6:	1000                	addi	s0,sp,32
  acquire(&disk.vdisk_lock);
    800064f8:	0001f517          	auipc	a0,0x1f
    800064fc:	c3050513          	addi	a0,a0,-976 # 80025128 <disk+0x2128>
    80006500:	ffffa097          	auipc	ra,0xffffa
    80006504:	6e4080e7          	jalr	1764(ra) # 80000be4 <acquire>
  // we've seen this interrupt, which the following line does.
  // this may race with the device writing new entries to
  // the "used" ring, in which case we may process the new
  // completion entries in this interrupt, and have nothing to do
  // in the next interrupt, which is harmless.
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80006508:	10001737          	lui	a4,0x10001
    8000650c:	533c                	lw	a5,96(a4)
    8000650e:	8b8d                	andi	a5,a5,3
    80006510:	d37c                	sw	a5,100(a4)

  __sync_synchronize();
    80006512:	0ff0000f          	fence

  // the device increments disk.used->idx when it
  // adds an entry to the used ring.

  while(disk.used_idx != disk.used->idx){
    80006516:	0001f797          	auipc	a5,0x1f
    8000651a:	aea78793          	addi	a5,a5,-1302 # 80025000 <disk+0x2000>
    8000651e:	6b94                	ld	a3,16(a5)
    80006520:	0207d703          	lhu	a4,32(a5)
    80006524:	0026d783          	lhu	a5,2(a3)
    80006528:	06f70163          	beq	a4,a5,8000658a <virtio_disk_intr+0x9e>
    __sync_synchronize();
    int id = disk.used->ring[disk.used_idx % NUM].id;
    8000652c:	0001d917          	auipc	s2,0x1d
    80006530:	ad490913          	addi	s2,s2,-1324 # 80023000 <disk>
    80006534:	0001f497          	auipc	s1,0x1f
    80006538:	acc48493          	addi	s1,s1,-1332 # 80025000 <disk+0x2000>
    __sync_synchronize();
    8000653c:	0ff0000f          	fence
    int id = disk.used->ring[disk.used_idx % NUM].id;
    80006540:	6898                	ld	a4,16(s1)
    80006542:	0204d783          	lhu	a5,32(s1)
    80006546:	8b9d                	andi	a5,a5,7
    80006548:	078e                	slli	a5,a5,0x3
    8000654a:	97ba                	add	a5,a5,a4
    8000654c:	43dc                	lw	a5,4(a5)

    if(disk.info[id].status != 0)
    8000654e:	20078713          	addi	a4,a5,512
    80006552:	0712                	slli	a4,a4,0x4
    80006554:	974a                	add	a4,a4,s2
    80006556:	03074703          	lbu	a4,48(a4) # 10001030 <_entry-0x6fffefd0>
    8000655a:	e731                	bnez	a4,800065a6 <virtio_disk_intr+0xba>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    8000655c:	20078793          	addi	a5,a5,512
    80006560:	0792                	slli	a5,a5,0x4
    80006562:	97ca                	add	a5,a5,s2
    80006564:	7788                	ld	a0,40(a5)
    b->disk = 0;   // disk is done with buf
    80006566:	00052223          	sw	zero,4(a0)
    wakeup(b);
    8000656a:	ffffc097          	auipc	ra,0xffffc
    8000656e:	fb8080e7          	jalr	-72(ra) # 80002522 <wakeup>

    disk.used_idx += 1;
    80006572:	0204d783          	lhu	a5,32(s1)
    80006576:	2785                	addiw	a5,a5,1
    80006578:	17c2                	slli	a5,a5,0x30
    8000657a:	93c1                	srli	a5,a5,0x30
    8000657c:	02f49023          	sh	a5,32(s1)
  while(disk.used_idx != disk.used->idx){
    80006580:	6898                	ld	a4,16(s1)
    80006582:	00275703          	lhu	a4,2(a4)
    80006586:	faf71be3          	bne	a4,a5,8000653c <virtio_disk_intr+0x50>
  }

  release(&disk.vdisk_lock);
    8000658a:	0001f517          	auipc	a0,0x1f
    8000658e:	b9e50513          	addi	a0,a0,-1122 # 80025128 <disk+0x2128>
    80006592:	ffffa097          	auipc	ra,0xffffa
    80006596:	706080e7          	jalr	1798(ra) # 80000c98 <release>
}
    8000659a:	60e2                	ld	ra,24(sp)
    8000659c:	6442                	ld	s0,16(sp)
    8000659e:	64a2                	ld	s1,8(sp)
    800065a0:	6902                	ld	s2,0(sp)
    800065a2:	6105                	addi	sp,sp,32
    800065a4:	8082                	ret
      panic("virtio_disk_intr status");
    800065a6:	00002517          	auipc	a0,0x2
    800065aa:	26a50513          	addi	a0,a0,618 # 80008810 <syscalls+0x3c0>
    800065ae:	ffffa097          	auipc	ra,0xffffa
    800065b2:	f90080e7          	jalr	-112(ra) # 8000053e <panic>

00000000800065b6 <cas>:
    800065b6:	100522af          	lr.w	t0,(a0)
    800065ba:	00b29563          	bne	t0,a1,800065c4 <fail>
    800065be:	18c5252f          	sc.w	a0,a2,(a0)
    800065c2:	8082                	ret

00000000800065c4 <fail>:
    800065c4:	4505                	li	a0,1
    800065c6:	8082                	ret
	...

0000000080007000 <_trampoline>:
    80007000:	14051573          	csrrw	a0,sscratch,a0
    80007004:	02153423          	sd	ra,40(a0)
    80007008:	02253823          	sd	sp,48(a0)
    8000700c:	02353c23          	sd	gp,56(a0)
    80007010:	04453023          	sd	tp,64(a0)
    80007014:	04553423          	sd	t0,72(a0)
    80007018:	04653823          	sd	t1,80(a0)
    8000701c:	04753c23          	sd	t2,88(a0)
    80007020:	f120                	sd	s0,96(a0)
    80007022:	f524                	sd	s1,104(a0)
    80007024:	fd2c                	sd	a1,120(a0)
    80007026:	e150                	sd	a2,128(a0)
    80007028:	e554                	sd	a3,136(a0)
    8000702a:	e958                	sd	a4,144(a0)
    8000702c:	ed5c                	sd	a5,152(a0)
    8000702e:	0b053023          	sd	a6,160(a0)
    80007032:	0b153423          	sd	a7,168(a0)
    80007036:	0b253823          	sd	s2,176(a0)
    8000703a:	0b353c23          	sd	s3,184(a0)
    8000703e:	0d453023          	sd	s4,192(a0)
    80007042:	0d553423          	sd	s5,200(a0)
    80007046:	0d653823          	sd	s6,208(a0)
    8000704a:	0d753c23          	sd	s7,216(a0)
    8000704e:	0f853023          	sd	s8,224(a0)
    80007052:	0f953423          	sd	s9,232(a0)
    80007056:	0fa53823          	sd	s10,240(a0)
    8000705a:	0fb53c23          	sd	s11,248(a0)
    8000705e:	11c53023          	sd	t3,256(a0)
    80007062:	11d53423          	sd	t4,264(a0)
    80007066:	11e53823          	sd	t5,272(a0)
    8000706a:	11f53c23          	sd	t6,280(a0)
    8000706e:	140022f3          	csrr	t0,sscratch
    80007072:	06553823          	sd	t0,112(a0)
    80007076:	00853103          	ld	sp,8(a0)
    8000707a:	02053203          	ld	tp,32(a0)
    8000707e:	01053283          	ld	t0,16(a0)
    80007082:	00053303          	ld	t1,0(a0)
    80007086:	18031073          	csrw	satp,t1
    8000708a:	12000073          	sfence.vma
    8000708e:	8282                	jr	t0

0000000080007090 <userret>:
    80007090:	18059073          	csrw	satp,a1
    80007094:	12000073          	sfence.vma
    80007098:	07053283          	ld	t0,112(a0)
    8000709c:	14029073          	csrw	sscratch,t0
    800070a0:	02853083          	ld	ra,40(a0)
    800070a4:	03053103          	ld	sp,48(a0)
    800070a8:	03853183          	ld	gp,56(a0)
    800070ac:	04053203          	ld	tp,64(a0)
    800070b0:	04853283          	ld	t0,72(a0)
    800070b4:	05053303          	ld	t1,80(a0)
    800070b8:	05853383          	ld	t2,88(a0)
    800070bc:	7120                	ld	s0,96(a0)
    800070be:	7524                	ld	s1,104(a0)
    800070c0:	7d2c                	ld	a1,120(a0)
    800070c2:	6150                	ld	a2,128(a0)
    800070c4:	6554                	ld	a3,136(a0)
    800070c6:	6958                	ld	a4,144(a0)
    800070c8:	6d5c                	ld	a5,152(a0)
    800070ca:	0a053803          	ld	a6,160(a0)
    800070ce:	0a853883          	ld	a7,168(a0)
    800070d2:	0b053903          	ld	s2,176(a0)
    800070d6:	0b853983          	ld	s3,184(a0)
    800070da:	0c053a03          	ld	s4,192(a0)
    800070de:	0c853a83          	ld	s5,200(a0)
    800070e2:	0d053b03          	ld	s6,208(a0)
    800070e6:	0d853b83          	ld	s7,216(a0)
    800070ea:	0e053c03          	ld	s8,224(a0)
    800070ee:	0e853c83          	ld	s9,232(a0)
    800070f2:	0f053d03          	ld	s10,240(a0)
    800070f6:	0f853d83          	ld	s11,248(a0)
    800070fa:	10053e03          	ld	t3,256(a0)
    800070fe:	10853e83          	ld	t4,264(a0)
    80007102:	11053f03          	ld	t5,272(a0)
    80007106:	11853f83          	ld	t6,280(a0)
    8000710a:	14051573          	csrrw	a0,sscratch,a0
    8000710e:	10200073          	sret
	...
